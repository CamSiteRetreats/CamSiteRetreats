import{S as $e,H as _e}from"./Header-CtRUqTZE.js";console.log("Bookings V3 - Cache Busted!");const De=()=>`
    ...
    `,Ne=()=>{let H=[],B="consult",D=[],te=[],J=[];const Te=async()=>{try{const[n,e,o]=await Promise.all([fetch("/api/tours"),fetch("/api/schedules"),fetch("/api/users")]);D=n.ok?await n.json():[],te=e.ok?await e.json():[],J=o.ok?await o.json():[],U("addTourName"),U("edit-tour"),U("wl-tour"),Ie("edit-sale")}catch(n){console.error("Lỗi tải tours/schedules:",n)}},Ie=n=>{const e=document.getElementById(n);e&&(e.innerHTML='<option value="">-- Website --</option>',J.forEach(o=>{const a=document.createElement("option");a.value=o.id,a.textContent=`${o.full_name||o.fullName} (${o.role})`,e.appendChild(a)}))},U=n=>{const e=document.getElementById(n);if(!e)return;const o=e.value,a=e.options[0].outerHTML;e.innerHTML=a,D.filter(c=>c.is_visible!==!1).forEach(c=>{const u=document.createElement("option");u.value=c.name,u.textContent=c.name,c.name===o&&(u.selected=!0),e.appendChild(u)})},K=(n,e,o,a)=>{const c=document.getElementById(n);if(!c)return;if(c.innerHTML='<option value="">-- Chọn Lịch --</option>',!e){c.innerHTML='<option value="">-- Chọn Tour trước --</option>';return}const u=te.filter(s=>s.tour_name===e||e.includes(s.tour_name)||s.tour_name.includes(e));if(u.length===0){c.innerHTML='<option value="">Chưa có lịch cho tour này</option>';return}u.sort((s,i)=>new Date(s.start_date)-new Date(i.start_date)).forEach(s=>{const i=parseInt(s.booked_count)||0,m=(s.slots||0)-i,v=new Date(s.start_date),p=s.end_date?new Date(s.end_date):null,T=L=>`${L.getDate().toString().padStart(2,"0")}/${(L.getMonth()+1).toString().padStart(2,"0")}`;let I=T(v);p&&(I+=" - "+T(p)),s.group_label&&(I+=` · ${s.group_label}`),I+=` (${m>=0?m:0} chỗ trống)`;const y=document.createElement("option"),h=`${v.getDate().toString().padStart(2,"0")}/${(v.getMonth()+1).toString().padStart(2,"0")}/${v.getFullYear()}`;y.value=`${s.id}::${h}`,y.dataset.scheduleId=s.id,y.dataset.date=h,y.textContent=I,m<=0&&(y.disabled=!0,y.textContent=I.replace("chỗ trống","HẾT CHỖ"));let w=o||"";if(w.includes("-")&&w.length>=10){const L=w.substring(0,10).split("-");L.length===3&&(w=`${L[2]}/${L[1]}/${L[0]}`)}(a&&String(s.id)===String(a)||!a&&w&&h===w)&&(y.selected=!0),c.appendChild(y)})},ne=document.getElementById("addTourName");ne&&ne.addEventListener("change",n=>{const e=n.target.value;K("addTourDate",e);const o=D.find(c=>c.name===e),a=document.getElementById("addTourPrice");if(o&&a){const c=parseInt(o.price)||0;a.value=c;const u=document.getElementById("addDiscount"),s=document.getElementById("addTotalPriceDisplay"),i=parseInt(u?u.value:0)||0,m=c-i;s&&(s.textContent=(m>0?m:0).toLocaleString("vi-VN")+"đ")}});const oe=document.getElementById("edit-tour");oe&&oe.addEventListener("change",n=>{const e=n.target.value;K("edit-date",e);const o=D.find(c=>c.name===e),a=document.getElementById("edit-total");o&&a&&(parseInt(a.value)===0||!a.value)&&(a.value=parseInt(o.price)||0,typeof window.updateEditRemaining=="function"&&window.updateEditRemaining())});const ae=document.getElementById("wl-tour");ae&&ae.addEventListener("change",n=>{const e=n.target.value;K("wl-date",e)}),Te();const R=()=>{const n=document.getElementById("bookingsTableHead"),e=document.getElementById("bookingsTableBody");if(!e||!n)return;const o=B==="upcoming"||B==="ready"||B==="completed";o?n.innerHTML=`
            ...
            `;const a=document.getElementById("filterSearch")?document.getElementById("filterSearch").value.toLowerCase():"",c=document.getElementById("filterTour")?document.getElementById("filterTour").value:"",u=document.getElementById("filterDate")?document.getElementById("filterDate").value:"",s=document.getElementById("filterSale")?document.getElementById("filterSale").value:"",i=document.getElementById("filterStatus")?document.getElementById("filterStatus").value:"";let m=H.filter(t=>{let l=!1;const d=parseInt(t.total_price)>0&&parseInt(t.total_price)===parseInt(t.deposit),r=b=>{if(!b)return!1;try{const E=b.split("/");if(E.length===3){const C=new Date(`${E[2]}-${E[1]}-${E[0]}T00:00:00`),x=new Date;return x.setHours(0,0,0,0),C.getTime()<x.getTime()}else if(b.includes("-")){const C=new Date(`${b}T00:00:00`),x=new Date;return x.setHours(0,0,0,0),C.getTime()<x.getTime()}}catch{}return!1},g=t.status||"",f=!r(t.date);return B==="consult"?l=g==="Chờ tư vấn"&&f:B==="pending"?l=(g==="Chờ cọc"||g==="Chờ xác nhận cọc"||g==="")&&f:B==="upcoming"?l=(g==="Đã cọc"||g==="Đã cọc (Chờ đi)")&&!d&&f:B==="ready"?l=(d||g==="Hoàn thành")&&f&&g!=="Đã hủy"&&g!=="Bảo lưu":B==="completed"&&(l=!f||g==="Đã đi"||g==="Đã hủy"||g==="Bảo lưu"),!(!l||c&&t.tour!==c||u&&X(t.date)!==u||s&&s!=="null"&&t.sale_name!==s||s==="null"&&t.sale_name&&t.sale_name!=="Website"||a&&!`${t.name||""} ${t.phone||""} ${t.customer_id||""} ${t.id||""} csr${t.id||""} #csr${t.id||""}`.toLowerCase().includes(a)||i&&t.status!==i)});const v=document.getElementById("statTitle1"),p=document.getElementById("statTitle2"),T=document.getElementById("statTitle3"),I=document.getElementById("statTotalCustomers"),y=document.getElementById("statTotalRevenue"),h=document.getElementById("statTotalCollected");let w=m.length;if(B==="consult"){const t=document.getElementById("addWaitingBtn"),l=document.getElementById("addBookingBtn");t&&t.classList.remove("hidden"),l&&l.classList.add("hidden"),v&&(v.textContent="Tổng Đăng Ký Chờ Lịch"),p&&(p.textContent="Chưa Được Liên Hệ"),T&&(T.textContent="Đã Có Sale Phụ Trách");let d=0,r=0;m.forEach(g=>{!g.sale_id||g.sale_id==="null"||!g.sale_name||g.sale_name==="Website"||g.sale_name==="null"?d++:r++}),I&&(I.textContent=w),y&&(y.textContent=d+" Người"),h&&(h.textContent=r+" Người")}else if(B==="pending"){const t=document.getElementById("addWaitingBtn"),l=document.getElementById("addBookingBtn");t&&t.classList.add("hidden"),l&&l.classList.remove("hidden"),v&&(v.textContent="Tổng Khách (Chờ Cọc)"),p&&(p.textContent="Khách Chưa Tư Vấn (Mới)"),T&&(T.textContent="Đã Có Sale Giữ Chỗ");let d=0,r=0;m.forEach(g=>{!g.sale_id||g.sale_id==="null"||!g.sale_name||g.sale_name==="Website"||g.sale_name==="null"?d++:r++}),I&&(I.textContent=w),y&&(y.textContent=d+" Đơn"),h&&(h.textContent=r+" Đơn")}else if(B==="upcoming"){v&&(v.textContent="Tổng Số Khách"),p&&(p.textContent="Chưa Xác Nhận Cọc"),T&&(T.textContent="Chờ Thanh Toán Còn Lại");let t=0,l=0;m.forEach(d=>{d.status==="Chờ xác nhận cọc"?t++:parseInt(d.deposit)>0&&parseInt(d.total_price)>parseInt(d.deposit)&&l++}),I&&(I.textContent=w),y&&(y.textContent=t+" Khách"),h&&(h.textContent=l+" Khách")}else if(B==="ready"){v&&(v.textContent="Khách Sẵn Sàng (Full)"),p&&(p.textContent="Tổng Doanh Thu Tab"),T&&(T.textContent="Thực Thu (Full Tận Nơi)");let t=0,l=0;m.forEach(d=>{t+=parseInt(d.total_price)||0,l+=parseInt(d.deposit)||0}),I&&(I.textContent=w),y&&(y.textContent=t>0?t.toLocaleString("vi-VN")+"đ":"0đ"),h&&(h.textContent=l>0?l.toLocaleString("vi-VN")+"đ":"0đ")}else{v&&(v.textContent="Khách Đã Xong (Lịch Sử)"),p&&(p.textContent="Tổng Doanh Thu Tab"),T&&(T.textContent="Thực Thu (Đã Đóng)");let t=0,l=0;m.forEach(d=>{t+=parseInt(d.total_price)||0,l+=parseInt(d.deposit)||0}),I&&(I.textContent=w),y&&(y.textContent=t>0?t.toLocaleString("vi-VN")+"đ":"0đ"),h&&(h.textContent=l>0?l.toLocaleString("vi-VN")+"đ":"0đ")}const L=o?8:4;if(!m||m.length===0){let t=B==="consult"?"Chưa có khách hàng tư vấn nào.":B==="pending"?"Chưa có Khách chờ cọc.":B==="upcoming"?"Chưa có Khách nào Đã Cọc.":B==="ready"?"Chưa có khách sẵn sàng.":"Danh sách rỗng.";e.innerHTML=`<tr><td colspan="${L}" class="p-8 text-center text-gray-500">${t}</td></tr>`;return}e.innerHTML=m.map(t=>{const l=B==="upcoming"||B==="ready"?"bg-green-50/20 hover:bg-green-50 transition-colors":"hover:bg-gray-100 transition-colors";if(o){let d=parseInt(t.total_price)||0;if(d===0&&t.tour&&D&&D.length>0){const k=D.find(N=>N.name&&t.tour&&N.name.toLowerCase().trim()===t.tour.toLowerCase().trim());k&&parseInt(k.price)>0&&(d=parseInt(k.price))}const r=parseInt(t.deposit)||0,g=d-r,f=d>0?d.toLocaleString("vi-VN")+"đ":"Chưa định giá",b=r>0?r.toLocaleString("vi-VN")+"đ":"0đ",E=g>0?g.toLocaleString("vi-VN")+"đ":g===0&&d>0?"Đã thu trọn":"-";let C="";t.status==="Đã hủy"?C='<span class="bg-red-50 text-red-500 border border-red-200 px-2 py-0.5 rounded text-xs block w-full text-center font-bold">❌ Đã hủy</span>':t.status==="Bảo lưu"?C='<span class="bg-gray-100 text-gray-500 border border-gray-300 px-2 py-0.5 rounded text-xs block w-full text-center font-bold">⏸️ Bảo lưu</span>':t.status==="Hoàn tất phí"?C='<span class="bg-emerald-100 text-emerald-700 border border-emerald-200 px-2 py-0.5 rounded text-xs font-bold block w-full text-center">Hoàn Tất Phí</span>':t.status==="Hoàn thành"||t.status==="Đã đi"?C='<span class="bg-violet-100 text-violet-700 border border-violet-200 px-2 py-0.5 rounded text-xs block w-full text-center font-bold">Hoàn Thành</span>':t.status==="Chờ xác nhận cọc"?C=`<button class="action-btn confirm-deposit-btn bg-csr-orange text-white px-3 py-1.5 rounded text-xs font-bold shadow-sm hover:bg-[#d65503] w-full" data-id="${t.id}">Xác nhận cọc</button>`:r>0&&g===0?C='<span class="bg-green-100 text-green-700 border border-green-200 px-2 py-0.5 rounded text-xs font-medium block w-full text-center">Hoàn tất phí</span>':C='<span class="bg-blue-100 text-blue-700 border border-blue-200 px-2 py-0.5 rounded text-xs font-medium block w-full text-center">Đã Cọc</span>';let x="";return r>0&&(x=`<a href="/invoice.html?id=${t.id}" target="_blank" class="mt-1.5 block text-center text-[11px] text-blue-600 hover:text-blue-800 hover:underline font-medium" onclick="event.stopPropagation()">🧾 Xem hóa đơn</a>`),`
                <tr class="${l} cursor-pointer row-clickable block md:table-row p-4 md:p-0 mb-4 md:mb-0 glass-panel md:glass-none border-l-4 border-green-500 md:border-none relative" data-id="${t.id}">
                    ...
                    <td class="p-4 align-top text-center hidden md:table-cell">${C}${x}</td>
    ...
    `}}).join(""),document.querySelectorAll(".claim-btn").forEach(t=>{t.addEventListener("click",async l=>{l.stopPropagation();const d=t.getAttribute("data-id"),r=localStorage.getItem("csr_user");let g=null,f="Admin";if(r){const b=JSON.parse(r);g=b.id,f=b.fullName||b.full_name||"Admin"}t.textContent="Đang xử lý...",t.disabled=!0;try{if((await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:d,sale_id:g,sale_name:f})})).ok){const E=H.find(C=>C.id==d);E&&(E.sale_id=g,E.sale_name=f),R()}else alert("Lỗi khi nhận khách. Vui lòng thử lại."),t.textContent="Nhận khách",t.disabled=!1}catch(b){console.error("Lỗi nhận khách:",b),t.textContent="Nhận khách",t.disabled=!1}})})},Ee=()=>{const n=new Set,e=new Set,o=new Set;H.forEach(s=>{if(s.tour&&n.add(s.tour),s.date){const i=X(s.date);i&&e.add(i)}s.sale_name&&s.sale_name!=="Website"&&s.sale_name!=="null"&&o.add(s.sale_name.trim())});const a=document.getElementById("filterTour"),c=document.getElementById("filterDate"),u=document.getElementById("filterSale");if(a){const s=a.value;a.innerHTML='<option value="">Tất cả Tour</option>',[...n].sort().forEach(i=>a.innerHTML+=`<option value="${i}">${i}</option>`),a.value=s}if(c){const s=c.value;c.innerHTML='<option value="">Tất cả Ngày</option>',[...e].sort().forEach(i=>c.innerHTML+=`<option value="${i}">${i}</option>`),c.value=s}if(u){const s=u.value;u.innerHTML='<option value="">Tất cả Sale</option>',u.innerHTML+='<option value="null">Nguồn Website</option>',[...o].sort().forEach(i=>{u.innerHTML+=`<option value="${i}">${i}</option>`}),u.value=s}};function X(n){if(!n)return"";if(/^\d{1,2}\/\d{1,2}\s*-\s*\d{1,2}\/\d{1,2}$/.test(n))return n.replace(/\s+/g,"");try{if(/^\d{4}-\d{2}-\d{2}$/.test(n)){const[e,o,a]=n.split("-"),c=`${parseInt(a)}/${parseInt(o)}`;return`${c}-${c}`}if(/^\d{1,2}\/\d{1,2}\/\d{4}$/.test(n)){const[e,o,a]=n.split("/"),c=`${parseInt(e)}/${parseInt(o)}`;return`${c}-${c}`}if(n.includes("-"))return n.split("-").map(o=>{const a=o.trim();if(/\/\d{4}$/.test(a)){const[c,u]=a.split("/");return`${parseInt(c)}/${parseInt(u)}`}return a}).join("-")}catch{console.warn("Date normalization failed for:",n)}return n}const M=async()=>{try{H=await(await fetch("/api/bookings")).json(),H.sort((e,o)=>parseInt(o.id)-parseInt(e.id)),Ee(),R()}catch(n){console.error("Lỗi parse list đơn hàng:",n);const e=document.getElementById("bookingsTableBody");e&&(e.innerHTML='<tr><td colspan="4" class="p-8 text-center text-red-500">Lỗi lấy dữ liệu từ Server.</td></tr>')}};M(),["filterSearch","filterTour","filterDate","filterSale","filterStatus"].forEach(n=>{const e=document.getElementById(n);e&&(n==="filterSearch"?e.addEventListener("input",R):e.addEventListener("change",R))});const se=document.getElementById("exportExcelBtn");se&&se.addEventListener("click",()=>{const n=document.getElementById("filterSearch")?document.getElementById("filterSearch").value.toLowerCase():"",e=document.getElementById("filterTour")?document.getElementById("filterTour").value:"",o=document.getElementById("filterDate")?document.getElementById("filterDate").value:"",a=document.getElementById("filterSale")?document.getElementById("filterSale").value:"";let c=H.filter(p=>{let T=!1;const I=parseInt(p.total_price)>0&&parseInt(p.total_price)===parseInt(p.deposit),y=w=>{if(!w)return!1;try{const L=w.split("/");if(L.length===3){const t=new Date(`${L[2]}-${L[1]}-${L[0]}T00:00:00`),l=new Date;return l.setHours(0,0,0,0),t.getTime()<l.getTime()}else if(w.includes("-")){const t=new Date(`${w}T00:00:00`),l=new Date;return l.setHours(0,0,0,0),t.getTime()<l.getTime()}}catch{}return!1},h=(p.status==="Hoàn thành"||p.status==="Đã đi"||I)&&y(p.date);return B==="consult"?T=p.status==="Chờ tư vấn"&&!h:B==="pending"?T=(!p.status||p.status==="Chờ cọc")&&!h:B==="upcoming"?T=p.status&&p.status!=="Chờ tư vấn"&&p.status!=="Chờ cọc"&&!I&&p.status!=="Hoàn thành"&&!h:B==="ready"?T=(I||p.status==="Hoàn thành")&&!h:B==="completed"&&(T=h),!(!T||e&&p.tour!==e||o&&X(p.date)!==o||a&&a!=="null"&&p.sale_name!==a||a==="null"&&p.sale_name&&p.sale_name!=="Website"||n&&!`${p.name||""} ${p.phone||""} ${p.customer_id||""} ${p.id||""} csr${p.id||""} #csr${p.id||""}`.toLowerCase().includes(n))});if(c.length===0){alert("Danh sách rỗng. Không có dữ liệu để xuất.");return}let u=`
        ...
        <div class="space-y-4 pb-2">
            ...
            ${(()=>{const t=e.phone;if(!t)return"";const l=H.filter(x=>x.phone===t&&!x.status?.includes("hủy")&&!x.status?.includes("Hủy")).sort((x,k)=>new Date(k.created_at||0)-new Date(x.created_at||0)),d=l.length,r=l.reduce((x,k)=>x+(parseInt(k.total_price)||0),0);let g,f,b;d>=5?(g="KHÁCH VIP",f="bg-purple-100 text-purple-700 border-purple-300",b="👑"):d>=2?(g="KHÁCH THÂN THIẾT",f="bg-green-100 text-green-700 border-green-300",b="🌟"):(g="KHÁCH MỚI",f="bg-gray-100 text-gray-500 border-gray-300",b="👋");const E=l.filter(x=>x.id!=n),C=E.length>0?E.slice(0,5).map((x,k)=>{const N=x.status?.includes("Hoàn")?"bg-green-100 text-green-600":x.status?.includes("cọc")||x.status?.includes("Cọc")?"bg-blue-100 text-blue-600":"bg-gray-100 text-gray-500";return`<div class="flex items-center gap-3 py-2 border-b border-gray-100 last:border-none">
