import{S as he,H as be}from"./Header-CamAZjvb.js";console.log("Bookings V3 - Cache Busted!");const ye=()=>`
    ...
    `,ve=()=>{let _=[],v="consult",D=[],J=[],q=[];const ue=async()=>{try{const[a,e,n]=await Promise.all([fetch("/api/tours"),fetch("/api/schedules"),fetch("/api/users")]);D=a.ok?await a.json():[],J=e.ok?await e.json():[],q=n.ok?await n.json():[],X("addTourName"),X("edit-tour"),pe("edit-sale")}catch(a){console.error("Lỗi tải tours/schedules:",a)}},pe=a=>{const e=document.getElementById(a);e&&(e.innerHTML='<option value="">-- Website --</option>',q.forEach(n=>{const l=document.createElement("option");l.value=n.id,l.textContent=`${n.full_name||n.fullName} (${n.role})`,e.appendChild(l)}))},X=a=>{const e=document.getElementById(a);if(!e)return;const n=e.value,l=e.options[0].outerHTML;e.innerHTML=l,D.filter(r=>r.is_visible!==!1).forEach(r=>{const c=document.createElement("option");c.value=r.name,c.textContent=r.name,r.name===n&&(c.selected=!0),e.appendChild(c)})},V=(a,e,n,l)=>{const r=document.getElementById(a);if(!r)return;if(r.innerHTML='<option value="">-- Chọn Lịch --</option>',!e){r.innerHTML='<option value="">-- Chọn Tour trước --</option>';return}const c=J.filter(d=>d.tour_name===e||e.includes(d.tour_name)||d.tour_name.includes(e));if(c.length===0){r.innerHTML='<option value="">Chưa có lịch cho tour này</option>';return}c.sort((d,o)=>new Date(d.start_date)-new Date(o.start_date)).forEach(d=>{const o=parseInt(d.booked_count)||0,i=(d.slots||0)-o,b=new Date(d.start_date),s=d.end_date?new Date(d.end_date):null,f=u=>`${u.getDate().toString().padStart(2,"0")}/${(u.getMonth()+1).toString().padStart(2,"0")}`;let p=f(b);s&&(p+=" - "+f(s)),d.group_label&&(p+=` · ${d.group_label}`),p+=` (${i>=0?i:0} chỗ trống)`;const h=document.createElement("option"),y=`${b.getDate().toString().padStart(2,"0")}/${(b.getMonth()+1).toString().padStart(2,"0")}/${b.getFullYear()}`;h.value=`${d.id}::${y}`,h.dataset.scheduleId=d.id,h.dataset.date=y,h.textContent=p,i<=0&&(h.disabled=!0,h.textContent=p.replace("chỗ trống","HẾT CHỖ")),(l&&String(d.id)===String(l)||!l&&n&&y===n)&&(h.selected=!0),r.appendChild(h)})},Y=document.getElementById("addTourName");Y&&Y.addEventListener("change",a=>{const e=a.target.value;V("addTourDate",e);const n=D.find(r=>r.name===e),l=document.getElementById("addTourPrice");if(n&&l){const r=parseInt(n.price)||0;l.value=r;const c=document.getElementById("addDiscount"),d=document.getElementById("addTotalPriceDisplay"),o=parseInt(c?c.value:0)||0,i=r-o;d&&(d.textContent=(i>0?i:0).toLocaleString("vi-VN")+"đ")}});const Q=document.getElementById("edit-tour");Q&&Q.addEventListener("change",a=>{const e=a.target.value;V("edit-date",e);const n=D.find(r=>r.name===e),l=document.getElementById("edit-total");n&&l&&(parseInt(l.value)===0||!l.value)&&(l.value=parseInt(n.price)||0,typeof window.updateEditRemaining=="function"&&window.updateEditRemaining())}),ue();const j=()=>{const a=document.getElementById("bookingsTableHead"),e=document.getElementById("bookingsTableBody");if(!e||!a)return;const n=v==="upcoming"||v==="ready"||v==="completed";n?a.innerHTML=`
            ...
            `;const l=document.getElementById("filterSearch")?document.getElementById("filterSearch").value.toLowerCase():"",r=document.getElementById("filterTour")?document.getElementById("filterTour").value:"",c=document.getElementById("filterDate")?document.getElementById("filterDate").value:"",d=document.getElementById("filterSale")?document.getElementById("filterSale").value:"",o=document.getElementById("filterStatus")?document.getElementById("filterStatus").value:"";let i=_.filter(t=>{let m=!1;const g=parseInt(t.total_price)>0&&parseInt(t.total_price)===parseInt(t.deposit),C=T=>{if(!T)return!1;try{const B=T.split("/");if(B.length===3){const x=new Date(`${B[2]}-${B[1]}-${B[0]}T00:00:00`),I=new Date;return I.setHours(0,0,0,0),x.getTime()<I.getTime()}else if(T.includes("-")){const x=new Date(`${T}T00:00:00`),I=new Date;return I.setHours(0,0,0,0),x.getTime()<I.getTime()}}catch{}return!1},k=(t.status==="Hoàn thành"||t.status==="Đã đi"||g)&&C(t.date);return v==="consult"?m=t.status==="Chờ tư vấn"&&!k:v==="pending"?m=(!t.status||t.status==="Chờ cọc"||t.status==="Chờ xác nhận cọc")&&!k:v==="upcoming"?m=t.status&&t.status!=="Chờ tư vấn"&&t.status!=="Chờ cọc"&&t.status!=="Chờ xác nhận cọc"&&!g&&t.status!=="Hoàn thành"&&!k:v==="ready"?m=(g||t.status==="Hoàn thành")&&!k:v==="completed"&&(m=k),!(!m||r&&t.tour!==r||c&&F(t.date)!==c||d&&d!=="null"&&t.sale_name!==d||d==="null"&&t.sale_name&&t.sale_name!=="Website"||l&&!`${t.name||""} ${t.phone||""} ${t.customer_id||""} ${t.id||""} csr${t.id||""} #csr${t.id||""}`.toLowerCase().includes(l)||o&&t.status!==o)});const b=document.getElementById("statTitle1"),s=document.getElementById("statTitle2"),f=document.getElementById("statTitle3"),p=document.getElementById("statTotalCustomers"),h=document.getElementById("statTotalRevenue"),y=document.getElementById("statTotalCollected");let u=i.length;if(v==="consult"){b&&(b.textContent="Tổng Leads Tư Vấn"),s&&(s.textContent="Chưa Có Sale Nhận"),f&&(f.textContent="Đã Có Sale Nhận");let t=0,m=0;i.forEach(g=>{!g.sale_id||g.sale_id==="null"||!g.sale_name||g.sale_name==="Website"||g.sale_name==="null"?t++:m++}),p&&(p.textContent=u),h&&(h.textContent=t+" Đơn"),y&&(y.textContent=m+" Đơn")}else if(v==="pending"){b&&(b.textContent="Tổng Khách (Chờ Cọc)"),s&&(s.textContent="Khách Chưa Tư Vấn (Mới)"),f&&(f.textContent="Đã Có Sale Giữ Chỗ");let t=0,m=0;i.forEach(g=>{!g.sale_id||g.sale_id==="null"||!g.sale_name||g.sale_name==="Website"||g.sale_name==="null"?t++:m++}),p&&(p.textContent=u),h&&(h.textContent=t+" Đơn"),y&&(y.textContent=m+" Đơn")}else if(v==="upcoming"){b&&(b.textContent="Tổng Số Khách"),s&&(s.textContent="Chưa Xác Nhận Cọc"),f&&(f.textContent="Chờ Thanh Toán Còn Lại");let t=0,m=0;i.forEach(g=>{g.status==="Chờ xác nhận cọc"?t++:parseInt(g.deposit)>0&&parseInt(g.total_price)>parseInt(g.deposit)&&m++}),p&&(p.textContent=u),h&&(h.textContent=t+" Khách"),y&&(y.textContent=m+" Khách")}else if(v==="ready"){b&&(b.textContent="Khách Sẵn Sàng (Full)"),s&&(s.textContent="Tổng Doanh Thu Tab"),f&&(f.textContent="Thực Thu (Full Tận Nơi)");let t=0,m=0;i.forEach(g=>{t+=parseInt(g.total_price)||0,m+=parseInt(g.deposit)||0}),p&&(p.textContent=u),h&&(h.textContent=t>0?t.toLocaleString("vi-VN")+"đ":"0đ"),y&&(y.textContent=m>0?m.toLocaleString("vi-VN")+"đ":"0đ")}else{b&&(b.textContent="Khách Đã Xong (Lịch Sử)"),s&&(s.textContent="Tổng Doanh Thu Tab"),f&&(f.textContent="Thực Thu (Đã Đóng)");let t=0,m=0;i.forEach(g=>{t+=parseInt(g.total_price)||0,m+=parseInt(g.deposit)||0}),p&&(p.textContent=u),h&&(h.textContent=t>0?t.toLocaleString("vi-VN")+"đ":"0đ"),y&&(y.textContent=m>0?m.toLocaleString("vi-VN")+"đ":"0đ")}const w=n?8:4;if(!i||i.length===0){let t=v==="consult"?"Chưa có khách hàng tư vấn nào.":v==="pending"?"Chưa có Khách chờ cọc.":v==="upcoming"?"Chưa có Khách nào Đã Cọc.":v==="ready"?"Chưa có khách sẵn sàng.":"Danh sách rỗng.";e.innerHTML=`<tr><td colspan="${w}" class="p-8 text-center text-gray-500">${t}</td></tr>`;return}e.innerHTML=i.map(t=>{const m=v==="upcoming"||v==="ready"?"bg-green-50/20 hover:bg-green-50 transition-colors":"hover:bg-gray-100 transition-colors";if(n){let g=parseInt(t.total_price)||0;if(g===0&&t.tour&&D&&D.length>0){const E=D.find(H=>H.name&&t.tour&&H.name.toLowerCase().trim()===t.tour.toLowerCase().trim());E&&parseInt(E.price)>0&&(g=parseInt(E.price))}const C=parseInt(t.deposit)||0,k=g-C,T=g>0?g.toLocaleString("vi-VN")+"đ":"Chưa định giá",B=C>0?C.toLocaleString("vi-VN")+"đ":"0đ",x=k>0?k.toLocaleString("vi-VN")+"đ":k===0&&g>0?"Đã thu trọn":"-";let I="";t.status==="Hoàn thành"||t.status==="Đã đi"?I='<span class="bg-gray-100 text-gray-600 border border-gray-200 px-2 py-0.5 rounded text-xs block w-full text-center">Hoàn thành</span>':t.status==="Chờ xác nhận cọc"?I=`<button class="action-btn confirm-deposit-btn bg-csr-orange text-white px-3 py-1.5 rounded text-xs font-bold shadow-sm hover:bg-[#d65503] w-full" data-id="${t.id}">Xác nhận cọc</button>`:C>0&&k===0?I='<span class="bg-green-100 text-green-700 border border-green-200 px-2 py-0.5 rounded text-xs font-medium block w-full text-center">Hoàn tất phí</span>':I='<span class="bg-blue-100 text-blue-700 border border-blue-200 px-2 py-0.5 rounded text-xs font-medium block w-full text-center">Đã Cọc</span>';let S="";return C>0&&(S=`<a href="/invoice.html?id=${t.id}" target="_blank" class="mt-1.5 block text-center text-[11px] text-blue-600 hover:text-blue-800 hover:underline font-medium" onclick="event.stopPropagation()">🧾 Xem hóa đơn</a>`),`
                <tr class="${m} cursor-pointer row-clickable block md:table-row p-4 md:p-0 mb-4 md:mb-0 glass-panel md:glass-none border-l-4 border-green-500 md:border-none relative" data-id="${t.id}">
                    ...
                    <td class="p-4 align-top text-center hidden md:table-cell">${I}${S}</td>
                ...
                `}else{const g=t.name?t.name.substring(0,2).toUpperCase():"KH",C=v==="consult",k=!t.sale_id||t.sale_id==="null"||!t.sale_name||t.sale_name==="null";let T="";C&&k?T=`<button class="action-btn claim-btn bg-csr-orange hover:bg-[#d65503] text-white px-3 py-1.5 rounded text-xs font-bold shadow-sm transition-colors" data-id="${t.id}">Nhận khách</button>`:T=`<span class="text-sm text-gray-500">${t.sale_name||"Website"}</span>`;const B=`
                ...
                `;return`
                <tr class="${m} cursor-pointer row-clickable block md:table-row p-4 md:p-0 mb-4 md:mb-0 glass-panel md:glass-none border-l-4 border-csr-orange md:border-none relative" data-id="${t.id}">
    ...
    `}}).join(""),document.querySelectorAll(".claim-btn").forEach(t=>{t.addEventListener("click",async m=>{m.stopPropagation();const g=t.getAttribute("data-id"),C=localStorage.getItem("csr_user");let k=null,T="Admin";if(C){const B=JSON.parse(C);k=B.id,T=B.fullName||B.full_name||"Admin"}t.textContent="Đang xử lý...",t.disabled=!0;try{if((await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:g,sale_id:k,sale_name:T})})).ok){const x=_.find(I=>I.id==g);x&&(x.sale_id=k,x.sale_name=T),j()}else alert("Lỗi khi nhận khách. Vui lòng thử lại."),t.textContent="Nhận khách",t.disabled=!1}catch(B){console.error("Lỗi nhận khách:",B),t.textContent="Nhận khách",t.disabled=!1}})})},ge=()=>{const a=new Set,e=new Set,n=new Set;_.forEach(d=>{if(d.tour&&a.add(d.tour),d.date){const o=F(d.date);o&&e.add(o)}d.sale_name&&d.sale_name!=="Website"&&d.sale_name!=="null"&&n.add(d.sale_name.trim())});const l=document.getElementById("filterTour"),r=document.getElementById("filterDate"),c=document.getElementById("filterSale");if(l){const d=l.value;l.innerHTML='<option value="">Tất cả Tour</option>',[...a].sort().forEach(o=>l.innerHTML+=`<option value="${o}">${o}</option>`),l.value=d}if(r){const d=r.value;r.innerHTML='<option value="">Tất cả Ngày</option>',[...e].sort().forEach(o=>r.innerHTML+=`<option value="${o}">${o}</option>`),r.value=d}if(c){const d=c.value;c.innerHTML='<option value="">Tất cả Sale</option>',c.innerHTML+='<option value="null">Nguồn Website</option>',[...n].sort().forEach(o=>{c.innerHTML+=`<option value="${o}">${o}</option>`}),c.value=d}};function F(a){if(!a)return"";if(/^\d{1,2}\/\d{1,2}\s*-\s*\d{1,2}\/\d{1,2}$/.test(a))return a.replace(/\s+/g,"");try{if(/^\d{4}-\d{2}-\d{2}$/.test(a)){const[e,n,l]=a.split("-"),r=`${parseInt(l)}/${parseInt(n)}`;return`${r}-${r}`}if(/^\d{1,2}\/\d{1,2}\/\d{4}$/.test(a)){const[e,n,l]=a.split("/"),r=`${parseInt(e)}/${parseInt(n)}`;return`${r}-${r}`}if(a.includes("-"))return a.split("-").map(n=>{const l=n.trim();if(/\/\d{4}$/.test(l)){const[r,c]=l.split("/");return`${parseInt(r)}/${parseInt(c)}`}return l}).join("-")}catch{console.warn("Date normalization failed for:",a)}return a}const M=async()=>{try{_=await(await fetch("/api/bookings")).json(),_.sort((e,n)=>parseInt(n.id)-parseInt(e.id)),ge(),j()}catch(a){console.error("Lỗi parse list đơn hàng:",a);const e=document.getElementById("bookingsTableBody");e&&(e.innerHTML='<tr><td colspan="4" class="p-8 text-center text-red-500">Lỗi lấy dữ liệu từ Server.</td></tr>')}};M(),["filterSearch","filterTour","filterDate","filterSale","filterStatus"].forEach(a=>{const e=document.getElementById(a);e&&(a==="filterSearch"?e.addEventListener("input",j):e.addEventListener("change",j))});const Z=document.getElementById("exportExcelBtn");Z&&Z.addEventListener("click",()=>{const a=document.getElementById("filterSearch")?document.getElementById("filterSearch").value.toLowerCase():"",e=document.getElementById("filterTour")?document.getElementById("filterTour").value:"",n=document.getElementById("filterDate")?document.getElementById("filterDate").value:"",l=document.getElementById("filterSale")?document.getElementById("filterSale").value:"";let r=_.filter(s=>{let f=!1;const p=parseInt(s.total_price)>0&&parseInt(s.total_price)===parseInt(s.deposit),h=u=>{if(!u)return!1;try{const w=u.split("/");if(w.length===3){const t=new Date(`${w[2]}-${w[1]}-${w[0]}T00:00:00`),m=new Date;return m.setHours(0,0,0,0),t.getTime()<m.getTime()}else if(u.includes("-")){const t=new Date(`${u}T00:00:00`),m=new Date;return m.setHours(0,0,0,0),t.getTime()<m.getTime()}}catch{}return!1},y=(s.status==="Hoàn thành"||s.status==="Đã đi"||p)&&h(s.date);return v==="consult"?f=s.status==="Chờ tư vấn"&&!y:v==="pending"?f=(!s.status||s.status==="Chờ cọc")&&!y:v==="upcoming"?f=s.status&&s.status!=="Chờ tư vấn"&&s.status!=="Chờ cọc"&&!p&&s.status!=="Hoàn thành"&&!y:v==="ready"?f=(p||s.status==="Hoàn thành")&&!y:v==="completed"&&(f=y),!(!f||e&&s.tour!==e||n&&F(s.date)!==n||l&&l!=="null"&&s.sale_name!==l||l==="null"&&s.sale_name&&s.sale_name!=="Website"||a&&!`${s.name||""} ${s.phone||""} ${s.customer_id||""} ${s.id||""} csr${s.id||""} #csr${s.id||""}`.toLowerCase().includes(a))});if(r.length===0){alert("Danh sách rỗng. Không có dữ liệu để xuất.");return}let c=`
        ...
        `;document.getElementById("ramActions").innerHTML=e,document.getElementById("rowActionModal").classList.remove("hidden")},window.addEventListener("click",a=>{const e=document.getElementById("rowActionModal");e&&a.target===e&&e.classList.add("hidden")}),window.actionView=a=>{document.getElementById("rowActionModal").classList.add("hidden");const e=_.find(u=>u.id==a);if(!e)return;const n=u=>parseInt(u||0).toLocaleString("vi-VN")+"đ",l=u=>{if(!u)return"—";if(/^\d{4}-\d{2}-\d{2}/.test(u)){const[w,t,m]=u.split("-");return`${m}/${t}/${w}`}return u},r=parseInt(e.total_price)||0,c=parseInt(e.deposit)||0,d=parseInt(e.discount)||0,o=r-c;let i=[];try{if(e.services_booked){const u=typeof e.services_booked=="string"?JSON.parse(e.services_booked):e.services_booked;Array.isArray(u)&&(i=u)}}catch{}const b=i.reduce((u,w)=>u+(parseInt(w.price)||0),0),f={"Chờ tư vấn":"bg-purple-100 text-purple-700 border-purple-200","Chờ cọc":"bg-yellow-100 text-yellow-700 border-yellow-200","Chờ xác nhận cọc":"bg-orange-100 text-orange-700 border-orange-200","Đã cọc":"bg-blue-100 text-blue-700 border-blue-200","Đã cọc (Chờ đi)":"bg-blue-100 text-blue-700 border-blue-200","Hoàn thành":"bg-green-100 text-green-700 border-green-200"}[e.status]||"bg-gray-100 text-gray-700 border-gray-200",p=(u,w,t="")=>`<div class="flex items-start gap-3 py-2.5 border-b border-gray-100 last:border-none">
        ...
        <div class="space-y-4 pb-2">
            ...
            <div class="bg-gradient-to-br from-csr-orange to-orange-600 rounded-2xl p-4 text-white shadow-lg shadow-orange-500/20">
                ...
                <div class="space-y-2 text-sm">
                    ...
                    <div class="flex justify-between items-center py-1.5 border-b border-white/10">
                        ...
                        <span class="font-bold text-green-100">+ ${n(b)}</span>
                    </div>`:""}
            ...
            ${(()=>{const u=e.phone;if(!u)return"";const w=_.filter(x=>x.phone===u&&!x.status?.includes("hủy")&&!x.status?.includes("Hủy")).sort((x,I)=>new Date(I.created_at||0)-new Date(x.created_at||0)),t=w.length,m=w.reduce((x,I)=>x+(parseInt(I.total_price)||0),0);let g,C,k;t>=5?(g="KHÁCH VIP",C="bg-purple-100 text-purple-700 border-purple-300",k="👑"):t>=2?(g="KHÁCH THÂN THIẾT",C="bg-green-100 text-green-700 border-green-300",k="🌟"):(g="KHÁCH MỚI",C="bg-gray-100 text-gray-500 border-gray-300",k="👋");const T=w.filter(x=>x.id!=a),B=T.length>0?T.slice(0,5).map((x,I)=>{const S=x.status?.includes("Hoàn")?"bg-green-100 text-green-600":x.status?.includes("cọc")||x.status?.includes("Cọc")?"bg-blue-100 text-blue-600":"bg-gray-100 text-gray-500";return`<div class="flex items-center gap-3 py-2 border-b border-gray-100 last:border-none">
                            <div class="w-7 h-7 rounded-full bg-orange-100 flex items-center justify-center text-csr-orange text-xs font-bold shrink-0">${I+1}</div>
                            ...
                            <span class="${S} text-[9px] px-2 py-0.5 rounded-full font-bold shrink-0">${x.status||"Mới"}</span>
                    ...
                    ${h("🏅","Hành Trình Cùng Cam Site","bg-slate-100 text-slate-600")}

                    <!-- Tier + LTV -->
                    <div class="flex items-center justify-between mb-4 gap-3">
                        <div class="flex items-center gap-2">
                            <span class="text-lg">${k}</span>
                            <span class="text-xs font-black uppercase tracking-widest px-3 py-1.5 rounded-full border ${C}">${g}</span>
                        </div>
                        <div class="text-right">
                            <div class="text-[10px] text-gray-400 uppercase font-semibold tracking-wide">Tổng đã chi (LTV)</div>
                            <div class="text-lg font-black text-gray-900">${m>0?m.toLocaleString("vi-VN")+"đ":"—"}</div>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center gap-2 mb-3 text-sm text-gray-500">
                        ...
                        <span class="font-black text-gray-800 text-base">${t}</span>
                    ...
                    ${T.length>0?`
                    <div class="mb-3">
                        <div class="text-[10px] font-bold text-gray-400 uppercase tracking-wide mb-2">Các tour trước đây (${T.length})</div>
                        <div class="bg-white rounded-xl border border-gray-100 px-3">${B}</div>
                    </div>`:B}
            ...
            <div class="bg-blue-50 border border-blue-100 rounded-xl px-4 py-3 flex items-center gap-3">
                ...
                <div>
                    ...
                    <div class="text-sm font-bold text-blue-800">${e.sale_name}</div>
                        ...
                        `).join(""),L.classList.remove("hidden"))}else L&&(L.innerHTML='<div class="p-3 text-xs text-gray-400 italic text-center">Không tìm thấy khách hàng cũ</div>',L.classList.remove("hidden"))}catch(l){console.error("Search error:",l)}},300)}),L&&L.addEventListener("click",e=>{const n=e.target.closest(".suggestion-item");if(n){const l=document.getElementById("addFullName"),r=document.getElementById("addPhone"),c=document.getElementById("addCsrCode");l&&(l.value=n.getAttribute("data-name")),r&&(r.value=n.getAttribute("data-phone")),c&&(c.value=n.getAttribute("data-csr"));const d=document.getElementById("addDob"),o=document.getElementById("addGender"),i=document.getElementById("addIdCard"),b=document.getElementById("addAddress");d&&(d.value=n.getAttribute("data-dob")||""),o&&(o.value=n.getAttribute("data-gender")||"Khác"),i&&(i.value=n.getAttribute("data-cccd")||""),b&&(b.value=n.getAttribute("data-address")||"");const s=document.getElementById("addAllergy"),f=document.getElementById("addDiet"),p=document.getElementById("addTrekkingPole"),h=document.getElementById("addMedalName");s&&(s.value=n.getAttribute("data-allergy")||""),f&&(f.value=n.getAttribute("data-diet")||""),p&&(p.value=n.getAttribute("data-trekking_pole")||"Không"),h&&(h.value=n.getAttribute("data-medal-name")||""),window._selectedCustomer={fullName:n.getAttribute("data-name"),phone:n.getAttribute("data-phone"),csrCode:n.getAttribute("data-csr"),dob:n.getAttribute("data-dob")||"",gender:n.getAttribute("data-gender")||"",address:n.getAttribute("data-address")||"",id_card:n.getAttribute("data-cccd")||"",allergy:n.getAttribute("data-allergy")||"",diet:n.getAttribute("data-diet")||"",trekking_pole:n.getAttribute("data-trekking_pole")||"",medal_name:n.getAttribute("data-medal-name")||""},P.value=n.getAttribute("data-name"),L.classList.add("hidden");const y=`
                    ...
                    </div>`,u=document.getElementById("loyalty-alert");u&&u.remove(),P.parentElement.insertAdjacentHTML("afterend",y)}}),document.addEventListener("click",e=>{!P.contains(e.target)&&L&&!L.contains(e.target)&&L.classList.add("hidden")})}const G=document.getElementById("addTourPrice"),O=document.getElementById("addDiscount"),ie=document.getElementById("addTotalPriceDisplay");document.getElementById("addTourName");const le=()=>{if(!G||!O||!ie)return;const a=parseInt(G.value)||0,e=parseInt(O.value)||0,n=a-e;ie.textContent=(n>0?n:0).toLocaleString("vi-VN")+"đ"};G&&G.addEventListener("input",le),O&&O.addEventListener("input",le);const re=document.getElementById("searchBtn");re&&re.addEventListener("click",a=>{a.preventDefault(),document.getElementById("smartSearch").value||alert("Vui lòng nhập Số Điện Thoại hoặc Mã #CSR")});const z=document.getElementById("bookingForm");z&&z.addEventListener("submit",async a=>{a.preventDefault();const e=a.target.querySelector('button[type="submit"]'),n=e.textContent;e.textContent="Đang lưu hệ thống...",e.disabled=!0;try{const l=document.getElementById("addFullName").value,r=document.getElementById("addPhone").value,c=document.getElementById("addTourName").value,d=document.getElementById("addTourDate").value;let o=null,i=d;if(d&&d.includes("::")){const $=d.split("::");o=parseInt($[0])||null,i=$[1]}const b=document.getElementById("addDob").value,s=document.getElementById("addGender").value,f=document.getElementById("addIdCard").value,p=document.getElementById("addAddress").value,h=document.getElementById("addSpecial").value,y=document.getElementById("addAllergy").value,u=document.getElementById("addDiet").value,w=document.getElementById("addTrekkingPole").value,t=document.getElementById("addMedalName").value,m=parseInt(document.getElementById("addTourPrice").value)||0,g=parseInt(document.getElementById("addDiscount").value)||0,C=parseInt(document.getElementById("addDepositRequired").value),k=isNaN(C)?1e6:C,T=document.getElementById("addCsrCode").value||"",B=localStorage.getItem("csr_user");let x=null,I="Admin";if(B){const $=JSON.parse(B);x=$.id,I=$.fullName||$.full_name||"Admin"}const S=document.getElementById("editingBookingId")?document.getElementById("editingBookingId").value:"",E={name:l,phone:r,tour:c,date:i,status:"Chờ cọc",total_price:m-g,discount:g,deposit:0,sale_id:x,sale_name:I,customer_id:T,deposit_required:k,commitments:!0,schedule_id:o,dob:b,gender:s,address:p,id_card:f,special:h,allergy:y,diet:u,trekking_pole:w,medal_name:t},H=r.replace(/[^0-9]/g,"");window._selectedCustomer&&(window._selectedCustomer.phone||"").replace(/[^0-9]/g,"")===H&&(E.dob=E.dob||window._selectedCustomer.dob,E.gender=E.gender&&E.gender!=="Khác"?E.gender:window._selectedCustomer.gender||"Khác",E.address=E.address||window._selectedCustomer.address,E.id_card=E.id_card||window._selectedCustomer.id_card,E.allergy=window._selectedCustomer.allergy,E.diet=window._selectedCustomer.diet,E.trekking_pole=window._selectedCustomer.trekking_pole,E.medal_name=window._selectedCustomer.medal_name),S&&(E.id=S,delete E.status,delete E.deposit,delete E.sale_id,delete E.sale_name);const R=await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(E)});if(R.ok){alert(S?"✅ Đã lưu Cập nhật Đơn hàng!":"✅ Thêm Đơn Hàng Thành Công!"),window.closeModal(),z&&z.reset(),window._selectedCustomer=null;const $=document.getElementById("loyalty-alert");$&&$.remove(),document.getElementById("addTotalPriceDisplay")&&(document.getElementById("addTotalPriceDisplay").textContent="0đ"),typeof M=="function"?M():window.location.reload()}else{const $=await R.json();throw new Error($.error||"Lỗi tạo Booking!")}}catch(l){alert("❌ "+l.message)}finally{e.textContent=n,e.disabled=!1}});const ce=document.getElementById("editForm");ce&&ce.addEventListener("submit",async a=>{a.preventDefault();const e=a.target.querySelector('button[type="submit"]'),n=e.textContent;e.textContent="Đang lưu cập nhật...",e.disabled=!0;try{const l=document.getElementById("edit-id").value,r=document.getElementById("edit-name").value,c=document.getElementById("edit-phone").value,d=document.getElementById("edit-medal-name").value,o=document.getElementById("edit-tour").value,i=document.getElementById("edit-date").value;let b=i,s;if(i&&i.includes("::")){const $=i.split("::");s=parseInt($[0])||null,b=$[1]}const f=document.getElementById("edit-dob").value,p=document.getElementById("edit-gender").value,h=document.getElementById("edit-status").value,y=document.getElementById("edit-allergy").value,u=document.getElementById("edit-address").value,w=document.getElementById("edit-diet").value,t=document.getElementById("edit-trekking-pole").value,m=document.getElementById("edit-commitments").checked,g=document.getElementById("edit-special").value,C=document.getElementById("edit-sale");let k,T;if(C&&C.parentElement&&!C.parentElement.classList.contains("hidden")&&(k=C.value||null,T="Website",k)){const $=q.find(me=>String(me.id)===String(k));$&&(T=$.full_name||$.fullName)}const B=parseInt(document.getElementById("edit-total").value)||0,x=parseInt(document.getElementById("edit-discount").value)||0,I=parseInt(document.getElementById("edit-deposit").value)||0,S=parseInt(document.getElementById("edit-deposit-required").value),E=isNaN(S)?1e6:S,H={id:l,name:r,phone:c,medal_name:d,tour:o,date:b,dob:f,gender:p,status:h,allergy:y,address:u,diet:w,trekking_pole:t,commitments:m,special:g,sale_id:k,sale_name:T,total_price:B-x,discount:x,deposit:I,deposit_required:E,...s!==void 0?{schedule_id:s}:{}},R=await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(H)});if(R.ok)alert("✅ Đã cập nhật thành công Chi tiết Đơn hàng!"),(h.includes("Đã cọc")||H.total_price>0&&H.total_price===I)&&se(H),window.closeEditModal(),typeof M=="function"&&M();else{const $=await R.json();throw new Error($.error||"Gặp sự cố khi Cập nhật API.")}}catch(l){alert("❌ Lỗi Cập nhật: "+l.message)}finally{e.textContent=n,e.disabled=!1}})};export{ve as afterRender,ye as render};
