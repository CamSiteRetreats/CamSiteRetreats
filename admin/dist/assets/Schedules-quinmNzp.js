import{S as q,H as O}from"./Header-Dmidaim7.js";const R=()=>`
    ...
    `,G=()=>{let f=[],k="",m="upcoming";const w="/api/schedules",P="/api/tours",x=document.getElementById("scheduleList"),g=document.getElementById("scheduleModal"),L=document.getElementById("scheduleModalContent"),C=document.getElementById("scheduleForm"),_=(e=null)=>{const n=document.getElementById("scheduleModalTitle");if(e){n.textContent="Chỉnh Sửa Lịch Trình",document.getElementById("sch-edit-id").value=e.id,document.getElementById("sch-tour-name").value=e.tour_name;const t=s=>s&&s.includes("T")?s.split("T")[0]:s;document.getElementById("sch-start-date").value=t(e.start_date),document.getElementById("sch-end-date").value=t(e.end_date),document.getElementById("sch-slots").value=e.slots,document.getElementById("sch-status").value=e.status||"Đang mở",document.getElementById("sch-group-label").value=e.group_label||"",document.getElementById("sch-zalo-link").value=e.zalo_link||""}else n.textContent="Thêm Lịch Khởi Hành",C.reset(),document.getElementById("sch-edit-id").value="",document.getElementById("sch-zalo-link").value="";g.classList.remove("hidden"),setTimeout(()=>{g.classList.add("opacity-100"),L.classList.remove("scale-95"),L.classList.add("scale-100")},10)},B=()=>{g.classList.remove("opacity-100"),L.classList.remove("scale-100"),L.classList.add("scale-95"),setTimeout(()=>{g.classList.add("hidden")},200)};document.getElementById("addScheduleBtn").addEventListener("click",()=>_()),document.getElementById("closeScheduleModalBtn").addEventListener("click",B),document.getElementById("cancelScheduleBtn").addEventListener("click",B),g.addEventListener("click",e=>{e.target===g&&B()});const A=async()=>{try{const e=await fetch(P);if(!e.ok)throw new Error("Failed to load tours");const n=await e.json(),t=document.getElementById("sch-tour-name");t.innerHTML='<option value="">-- Chọn Tour --</option>',n.forEach(a=>{t.innerHTML+=`<option value="${a.name}">${a.name}</option>`});const s=document.getElementById("tourFilterList");let r=`
            ...
            `}).join("")},b=document.getElementById("guidePickerModal"),v=document.getElementById("guidePickerList");let S=null;const H=async(e,n)=>{S=e,document.getElementById("guidePickerSubtitle").textContent=`Tour: ${n}`,b.classList.remove("hidden"),v.innerHTML='<div class="text-center py-8 text-gray-400">Đang tải...</div>';try{const[t,s]=await Promise.all([fetch("/api/guides"),fetch(`/api/guides?schedule_id=${e}`)]),r=await t.json(),a=await s.json(),d=new Set(a.map(o=>o.id)),l={"Hướng Dẫn Viên":"bg-blue-100 text-blue-700",Leader:"bg-purple-100 text-purple-700",Photo:"bg-pink-100 text-pink-700",Guider:"bg-cyan-100 text-cyan-700","Hậu Cần":"bg-amber-100 text-amber-700","Học Việc":"bg-gray-100 text-gray-600"};if(r.length===0){v.innerHTML='<div class="text-center py-8 text-gray-400"><div class="text-3xl mb-2">👥</div><p>Chưa có nhân sự nào.</p><a href="/admin/guides" data-link class="text-indigo-600 text-sm font-medium hover:underline">+ Thêm nhân sự mới</a></div>';return}v.innerHTML=r.map(o=>{const i=d.has(o.id),c=l[o.role]||"bg-gray-100 text-gray-600",h=(o.full_name||"X").split(" ").slice(-2).map(y=>y[0]).join("").toUpperCase();return`
                ...
                </div>`}).join(""),v.querySelectorAll(".guide-toggle-btn").forEach(o=>{o.addEventListener("click",async()=>{const i=parseInt(o.getAttribute("data-guide-id")),c=o.getAttribute("data-assigned")==="true";o.disabled=!0,o.textContent="...",await fetch(`/api/guides?action=${c?"unassign":"assign"}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({schedule_id:S,guide_id:i})}),H(S,document.getElementById("guidePickerSubtitle").textContent.replace("Tour: ",""))})})}catch(t){v.innerHTML=`<div class="text-center py-8 text-red-400">Lỗi tải dữ liệu: ${t.message}</div>`}};document.getElementById("closeGuidePickerBtn").addEventListener("click",()=>b.classList.add("hidden")),b.addEventListener("click",e=>{e.target===b&&b.classList.add("hidden")});const p=document.getElementById("photoLinksModal");let M=null,j=null;const $=e=>{const n=document.getElementById("photoLinksList");if(!e||e.length===0){n.innerHTML='<p class="text-center text-gray-400 text-sm py-2 italic">Chưa có link nào. Nhấn nút bên dưới để thêm.</p>';return}n.innerHTML=e.map((t,s)=>`
