import{S as F,H as _}from"./Header-DjwXxBfR.js";const D=()=>{let c={role:"sale"};try{const o=localStorage.getItem("csr_user");o&&(c=JSON.parse(o))}catch{}const d=c.role==="admin";return`
    ...
    `},q=()=>{let c=[],d="",o="upcoming";const b="/api/schedules",M="/api/tours",p=document.getElementById("scheduleList"),u=document.getElementById("scheduleModal"),m=document.getElementById("scheduleModalContent"),L=document.getElementById("scheduleForm"),E=(t=null)=>{const a=document.getElementById("scheduleModalTitle");if(t){a.textContent="Chỉnh Sửa Lịch Trình",document.getElementById("sch-edit-id").value=t.id,document.getElementById("sch-tour-name").value=t.tour_name;const e=n=>n&&n.includes("T")?n.split("T")[0]:n;document.getElementById("sch-start-date").value=e(t.start_date),document.getElementById("sch-end-date").value=e(t.end_date),document.getElementById("sch-slots").value=t.slots,document.getElementById("sch-status").value=t.status||"Đang mở"}else a.textContent="Thêm Lịch Khởi Hành",L.reset(),document.getElementById("sch-edit-id").value="";u.classList.remove("hidden"),setTimeout(()=>{u.classList.add("opacity-100"),m.classList.remove("scale-95"),m.classList.add("scale-100")},10)},v=()=>{u.classList.remove("opacity-100"),m.classList.remove("scale-100"),m.classList.add("scale-95"),setTimeout(()=>{u.classList.add("hidden")},200)};let S={role:"sale"};try{const t=localStorage.getItem("csr_user");t&&(S=JSON.parse(t))}catch{}S.role==="admin"&&(document.getElementById("addScheduleBtn")?.addEventListener("click",()=>E()),document.getElementById("closeScheduleModalBtn")?.addEventListener("click",v),document.getElementById("cancelScheduleBtn")?.addEventListener("click",v),u?.addEventListener("click",t=>{t.target===u&&v()}));const T=async()=>{try{const t=await fetch(M);if(!t.ok)throw new Error("Failed to load tours");const a=await t.json(),e=document.getElementById("sch-tour-name");e.innerHTML='<option value="">-- Chọn Tour --</option>',a.forEach(s=>{e.innerHTML+=`<option value="${s.name}">${s.name}</option>`});const n=document.getElementById("tourFilterList");let r=`
                <label class="flex items-center gap-3 cursor-pointer group">
                    <input type="radio" name="tourFilter" value="" class="text-csr-orange focus:ring-csr-orange border-gray-200 bg-gray-100" checked>
                    <span class="text-gray-500 group-hover:text-gray-900 transition-colors text-sm">Tất cả Tuyến</span>
                </label>
            `;a.forEach(s=>{r+=`
                ...
                `}),n.innerHTML=r,n.querySelectorAll('input[name="tourFilter"]').forEach(s=>{s.addEventListener("change",l=>{d=l.target.value,f()})}),document.querySelectorAll('input[name="timeFilter"]').forEach(s=>{s.addEventListener("change",l=>{o=l.target.value,document.querySelectorAll('input[name="timeFilter"]').forEach(g=>{const h=g.nextElementSibling;g.checked?(h.classList.add("text-gray-700","font-medium"),h.classList.remove("text-gray-500")):(h.classList.remove("text-gray-700","font-medium"),h.classList.add("text-gray-500"))}),f()})})}catch(t){console.error("Error loading tours:",t)}},y=async()=>{p.innerHTML='<div class="text-center py-12 text-gray-400">Đang tải lịch trình...</div>';try{const t=await fetch(b);if(!t.ok)throw new Error("Failed to fetch schedules");c=await t.json(),f()}catch(t){console.error("Error loading schedules:",t),p.innerHTML='<div class="text-center py-12 text-red-400 font-medium">Lỗi kết nối server.</div>'}},f=()=>{let t=c;d&&(t=t.filter(e=>e.tour_name===d));const a=new Date;if(a.setHours(0,0,0,0),t=t.filter(e=>{const n=new Date(e.start_date);n.setHours(0,0,0,0);const r=n<a;return o==="upcoming"?!r:o==="past"?r:!0}),t.sort((e,n)=>{const r=new Date(e.start_date),s=new Date(n.start_date);return o==="upcoming"?r-s:s-r}),t.length===0){p.innerHTML='<div class="text-center py-12 text-gray-400 italic">Chưa có lịch trình nào'+(d?" cho tuyến này":"")+(o==="past"?" trong quá khứ":" sắp tới")+".</div>";return}p.innerHTML=t.map(e=>{const n=new Date(e.start_date),r=new Date(e.end_date),s=k=>`${k.getDate().toString().padStart(2,"0")}/${(k.getMonth()+1).toString().padStart(2,"0")}/${k.getFullYear()}`,l=s(n),g=s(r),h=`${n.getDate().toString().padStart(2,"0")}/${(n.getMonth()+1).toString().padStart(2,"0")}`,w=parseInt(e.booked_count)||0,x=parseInt(e.slots)||0,B=Math.max(0,x-w),I=x>0?Math.min(100,Math.round(w/x*100)):0;let i=e.status||"Đang mở";B<=0?i="Hết chỗ":B<=3&&(i="Sắp hết"),e.status==="Hết chỗ"&&(i="Hết chỗ");const H=i==="Đang mở"?"text-green-600 bg-green-50 border-green-200":i==="Sắp hết"?"text-orange-600 bg-orange-50 border-orange-200":"text-red-500 bg-red-50 border-red-200",$=i==="Đang mở"?"bg-green-500":i==="Sắp hết"?"bg-orange-500":"bg-red-500";return`
            ...
            `}).join(""),p.addEventListener("click",C)},C=async t=>{const a=t.target.closest(".sch-edit-btn"),e=t.target.closest(".sch-delete-btn"),n=t.target.closest(".sch-details-btn");if(a){const r=a.getAttribute("data-id"),s=c.find(l=>l.id==r);s&&E(s)}if(e){const r=e.getAttribute("data-id");if(confirm("Bạn có chắc chắn muốn xóa lịch trình này?"))try{if((await fetch(`${b}?id=${r}`,{method:"DELETE"})).ok)y();else throw new Error("Delete failed")}catch(s){alert("Lỗi khi xóa: "+s.message)}}if(n){const r=n.getAttribute("data-tour"),s=n.getAttribute("data-date"),l=s&&s.includes("T")?s.split("T")[0]:s,g=`/admin/roster?tour=${encodeURIComponent(r)}&date=${encodeURIComponent(l)}`;history.pushState(null,null,g),window.dispatchEvent(new Event("popstate"))}};L.addEventListener("submit",async t=>{t.preventDefault();const a=t.target.querySelector('button[type="submit"]'),e=a.textContent;a.textContent="Đang lưu...",a.disabled=!0;const n=document.getElementById("sch-edit-id").value,r={id:n?parseInt(n):null,tour_name:document.getElementById("sch-tour-name").value,start_date:document.getElementById("sch-start-date").value,end_date:document.getElementById("sch-end-date").value,slots:parseInt(document.getElementById("sch-slots").value),status:document.getElementById("sch-status").value};try{if(!(await fetch(b,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(r)})).ok)throw new Error("Failed to save schedule");v(),y(),alert(n?"✅ Cập nhật lịch trình thành công!":"✅ Thêm lịch trình mới thành công! Form đặt chỗ trên website sẽ tự cập nhật trong 30 giây.")}catch(s){alert("❌ Lỗi: "+s.message)}finally{a.textContent=e,a.disabled=!1}}),T(),y()};export{q as afterRender,D as render};
