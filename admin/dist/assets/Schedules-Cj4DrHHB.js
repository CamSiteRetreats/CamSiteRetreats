import{S as q,H as O}from"./Header-Dmidaim7.js";const R=()=>`
    ...
    `,G=()=>{let f=[],k="",h="upcoming";const w="/api/schedules",P="/api/tours",m=document.getElementById("scheduleList"),g=document.getElementById("scheduleModal"),L=document.getElementById("scheduleModalContent"),C=document.getElementById("scheduleForm"),_=(e=null)=>{const n=document.getElementById("scheduleModalTitle");if(e){n.textContent="Chỉnh Sửa Lịch Trình",document.getElementById("sch-edit-id").value=e.id,document.getElementById("sch-tour-name").value=e.tour_name;const t=s=>s&&s.includes("T")?s.split("T")[0]:s;document.getElementById("sch-start-date").value=t(e.start_date),document.getElementById("sch-end-date").value=t(e.end_date),document.getElementById("sch-slots").value=e.slots,document.getElementById("sch-status").value=e.status||"Đang mở",document.getElementById("sch-group-label").value=e.group_label||"",document.getElementById("sch-zalo-link").value=e.zalo_link||""}else n.textContent="Thêm Lịch Khởi Hành",C.reset(),document.getElementById("sch-edit-id").value="",document.getElementById("sch-zalo-link").value="";g.classList.remove("hidden"),setTimeout(()=>{g.classList.add("opacity-100"),L.classList.remove("scale-95"),L.classList.add("scale-100")},10)},B=()=>{g.classList.remove("opacity-100"),L.classList.remove("scale-100"),L.classList.add("scale-95"),setTimeout(()=>{g.classList.add("hidden")},200)};document.getElementById("addScheduleBtn").addEventListener("click",()=>_()),document.getElementById("closeScheduleModalBtn").addEventListener("click",B),document.getElementById("cancelScheduleBtn").addEventListener("click",B),g.addEventListener("click",e=>{e.target===g&&B()});const A=async()=>{try{const e=await fetch(P);if(!e.ok)throw new Error("Failed to load tours");const n=await e.json(),t=document.getElementById("sch-tour-name");t.innerHTML='<option value="">-- Chọn Tour --</option>',n.forEach(a=>{t.innerHTML+=`<option value="${a.name}">${a.name}</option>`});const s=document.getElementById("tourFilterList");let r=`
                <label class="flex items-center gap-3 cursor-pointer group">
                    <input type="radio" name="tourFilter" value="" class="text-csr-orange focus:ring-csr-orange border-gray-200 bg-gray-100" checked>
                    <span class="text-gray-500 group-hover:text-gray-900 transition-colors text-sm">Tất cả Tuyến</span>
                </label>
            `;n.forEach(a=>{r+=`
                ...
                `}),s.innerHTML=r,s.querySelectorAll('input[name="tourFilter"]').forEach(a=>{a.addEventListener("change",d=>{k=d.target.value,T()})}),document.querySelectorAll('input[name="timeFilter"]').forEach(a=>{a.addEventListener("change",d=>{h=d.target.value,document.querySelectorAll('input[name="timeFilter"]').forEach(l=>{const o=l.nextElementSibling;l.checked?(o.classList.add("text-gray-700","font-medium"),o.classList.remove("text-gray-500")):(o.classList.remove("text-gray-700","font-medium"),o.classList.add("text-gray-500"))}),T()})})}catch(e){console.error("Error loading tours:",e)}},E=async()=>{m.innerHTML='<div class="text-center py-12 text-gray-400">Đang tải lịch trình...</div>';try{const e=await fetch(w);if(!e.ok)throw new Error("Failed to fetch schedules");f=await e.json(),T()}catch(e){console.error("Error loading schedules:",e),m.innerHTML='<div class="text-center py-12 text-red-400 font-medium">Lỗi kết nối server.</div>'}},T=()=>{let e=f;k&&(e=e.filter(t=>t.tour_name===k));const n=new Date;if(n.setHours(0,0,0,0),e=e.filter(t=>{const s=new Date(t.start_date);s.setHours(0,0,0,0);const r=s<n;return h==="upcoming"?!r:h==="past"?r:!0}),e.sort((t,s)=>{const r=new Date(t.start_date),a=new Date(s.start_date);return h==="upcoming"?r-a:a-r}),e.length===0){m.innerHTML='<div class="text-center py-12 text-gray-400 italic">Chưa có lịch trình nào'+(k?" cho tuyến này":"")+(h==="past"?" trong quá khứ":" sắp tới")+".</div>";return}m.innerHTML=e.map(t=>{const s=new Date(t.start_date),r=new Date(t.end_date),a=s.toLocaleDateString("vi-VN");r.toLocaleDateString("vi-VN");const d=`${s.getDate().toString().padStart(2,"0")}/${(s.getMonth()+1).toString().padStart(2,"0")}`,l=parseInt(t.booked_count)||0,o=parseInt(t.slots)||0,i=Math.max(0,o-l),c=o>0?Math.min(100,Math.round(l/o*100)):0,v=t.group_label?`<span class="ml-2 text-[10px] font-bold uppercase px-2 py-0.5 rounded-md border bg-blue-50 text-blue-600 border-blue-200">${t.group_label}</span>`:"",y=t.zalo_link?'<span class="ml-1 text-[10px] font-bold uppercase px-2 py-0.5 rounded-md border bg-blue-600 text-white border-blue-700 flex items-center gap-1"><svg class="w-2.5 h-2.5" fill="currentColor" viewBox="0 0 24 24"><path d="M12.14 1c-6.15 0-11.14 4.58-11.14 10.21 0 3.2 1.6 6.05 4.12 7.91-.12.44-.43 1.6-.43 1.6s-.1.38.16.53c.26.15.54-.04.54-.04l1.88-1.29c.6.16 1.23.25 1.88.25 6.15 0 11.14-4.58 11.14-10.21S18.29 1 12.14 1z"/></svg> ZALO</span>':"";let u=t.status||"Đang mở";i<=0?u="Hết chỗ":i<=3&&(u="Sắp hết"),t.status==="Hết chỗ"&&(u="Hết chỗ");const D=u==="Đang mở"?"text-green-600 bg-green-50 border-green-200":u==="Sắp hết"?"text-orange-600 bg-orange-50 border-orange-200":"text-red-500 bg-red-50 border-red-200",N=u==="Đang mở"?"bg-green-500":u==="Sắp hết"?"bg-orange-500":"bg-red-500";return`
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <div class="flex items-center gap-3 md:gap-4 text-xs md:text-sm text-gray-500 flex-wrap">
                                    ...
                                    <span class="flex items-center gap-1">
                                        ...
                                        <strong class="text-csr-orange">${l}</strong> / ${o}
                                    </span>
                                    <span class="text-[9px] font-bold uppercase px-1.5 py-0.5 rounded-md border ${D}">${u}</span>
                                </div>
                                ...
                                <div class="mt-2 w-full bg-gray-100 rounded-full h-1.5 overflow-hidden max-w-xs hidden md:block">
                                    <div class="${N} h-1.5 rounded-full transition-all duration-500" style="width: ${c}%"></div>
                                </div>
                            </div>
                        </div>
            ...
            `}).join("")},x=document.getElementById("guidePickerModal"),b=document.getElementById("guidePickerList");let S=null;const H=async(e,n)=>{S=e,document.getElementById("guidePickerSubtitle").textContent=`Tour: ${n}`,x.classList.remove("hidden"),b.innerHTML='<div class="text-center py-8 text-gray-400">Đang tải...</div>';try{const[t,s]=await Promise.all([fetch("/api/guides"),fetch(`/api/guides?schedule_id=${e}`)]),r=await t.json(),a=await s.json(),d=new Set(a.map(o=>o.id)),l={"Hướng Dẫn Viên":"bg-blue-100 text-blue-700",Leader:"bg-purple-100 text-purple-700",Photo:"bg-pink-100 text-pink-700",Guider:"bg-cyan-100 text-cyan-700","Hậu Cần":"bg-amber-100 text-amber-700","Học Việc":"bg-gray-100 text-gray-600"};if(r.length===0){b.innerHTML='<div class="text-center py-8 text-gray-400"><div class="text-3xl mb-2">👥</div><p>Chưa có nhân sự nào.</p><a href="/admin/guides" data-link class="text-indigo-600 text-sm font-medium hover:underline">+ Thêm nhân sự mới</a></div>';return}b.innerHTML=r.map(o=>{const i=d.has(o.id),c=l[o.role]||"bg-gray-100 text-gray-600",v=(o.full_name||"X").split(" ").slice(-2).map(y=>y[0]).join("").toUpperCase();return`
                <div class="flex items-center gap-3 p-3 rounded-xl border transition-all ${i?"border-indigo-200 bg-indigo-50":"border-gray-100 hover:border-gray-200 hover:bg-gray-50"}">
                    <div class="w-9 h-9 rounded-full bg-gradient-to-br from-csr-orange/20 to-orange-100 flex items-center justify-center text-csr-orange font-black text-xs shrink-0">${v}</div>
                    <div class="flex-1 min-w-0">
                        <div class="font-bold text-sm text-gray-900 truncate">${o.full_name}</div>
                        <span class="text-[10px] font-bold px-2 py-0.5 rounded-full ${c}">${o.role}</span>
                    </div>
                    <button class="guide-toggle-btn px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${i?"bg-red-100 text-red-600 hover:bg-red-200":"bg-indigo-600 text-white hover:bg-indigo-700"}"
                        data-guide-id="${o.id}" data-assigned="${i}">
                        ${i?"✕ Bỏ":"+ Thêm"}
                    </button>
                </div>`}).join(""),b.querySelectorAll(".guide-toggle-btn").forEach(o=>{o.addEventListener("click",async()=>{const i=parseInt(o.getAttribute("data-guide-id")),c=o.getAttribute("data-assigned")==="true";o.disabled=!0,o.textContent="...",await fetch("/api/guides",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:c?"unassign":"assign",schedule_id:S,guide_id:i})}),H(S,document.getElementById("guidePickerSubtitle").textContent.replace("Tour: ",""))})})}catch(t){b.innerHTML=`<div class="text-center py-8 text-red-400">Lỗi tải dữ liệu: ${t.message}</div>`}};document.getElementById("closeGuidePickerBtn").addEventListener("click",()=>x.classList.add("hidden")),x.addEventListener("click",e=>{e.target===x&&x.classList.add("hidden")});const p=document.getElementById("photoLinksModal");let M=null,j=null;const $=e=>{const n=document.getElementById("photoLinksList");if(!e||e.length===0){n.innerHTML='<p class="text-center text-gray-400 text-sm py-2 italic">Chưa có link nào. Nhấn nút bên dưới để thêm.</p>';return}n.innerHTML=e.map((t,s)=>`
            <div class="flex gap-2 items-start" data-link-idx="${s}">
                ...
                <button class="remove-photo-link-btn p-2 rounded-lg hover:bg-red-50 text-gray-300 hover:text-red-500 transition-colors shrink-0 mt-1" data-idx="${s}">
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/></svg>
                </button>
            </div>
        `).join(""),n.querySelectorAll(".remove-photo-link-btn").forEach(t=>{t.addEventListener("click",()=>{const s=parseInt(t.getAttribute("data-idx")),r=I();r.splice(s,1),$(r)})})},I=()=>{const e=document.querySelectorAll("#photoLinksList [data-link-idx]");return Array.from(e).map(n=>({title:n.querySelector(".photo-link-title-input").value.trim(),url:n.querySelector(".photo-link-url-input").value.trim()})).filter(n=>n.title||n.url)},z=(e,n,t)=>{M=e,j=t,document.getElementById("photoModalSubtitle").textContent=`Tour: ${n}`,$(t.photo_links||[]),p.classList.remove("hidden")};document.getElementById("closePhotoModalBtn").addEventListener("click",()=>p.classList.add("hidden")),p.addEventListener("click",e=>{e.target===p&&p.classList.add("hidden")}),document.getElementById("addPhotoLinkBtn").addEventListener("click",()=>{const e=I();e.push({title:"",url:""}),$(e);const n=document.querySelectorAll("#photoLinksList [data-link-idx]"),t=n[n.length-1];t&&t.querySelector(".photo-link-title-input").focus()}),document.getElementById("copyPhotoShareLinkBtn").addEventListener("click",()=>{const e=`${window.location.origin}/photos?id=${M}`;navigator.clipboard.writeText(e).then(()=>{const n=document.getElementById("copyPhotoShareLinkBtn"),t=n.innerHTML;n.innerHTML="✅ Đã copy link!",n.classList.add("bg-green-50","text-green-700","border-green-200"),n.classList.remove("bg-purple-50","text-purple-700","border-purple-200"),setTimeout(()=>{n.innerHTML=t,n.classList.remove("bg-green-50","text-green-700","border-green-200"),n.classList.add("bg-purple-50","text-purple-700","border-purple-200")},2500)}).catch(()=>alert(`Link chia sẻ:
${window.location.origin}/photos?id=${M}`))}),document.getElementById("savePhotoLinksBtn").addEventListener("click",async()=>{const e=document.getElementById("savePhotoLinksBtn"),n=I(),t=j;e.textContent="Đang lưu...",e.disabled=!0;const s={id:M,tour_name:t.tour_name,start_date:t.start_date&&t.start_date.includes("T")?t.start_date.split("T")[0]:t.start_date,end_date:t.end_date&&t.end_date.includes("T")?t.end_date.split("T")[0]:t.end_date,slots:t.slots,status:t.status,group_label:t.group_label||null,zalo_link:t.zalo_link||null,photo_links:n};try{if(!(await fetch(w,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(s)})).ok)throw new Error("Save failed");p.classList.add("hidden"),await E(),alert(`✅ Đã lưu ${n.length} link ảnh!`)}catch(r){alert("❌ Lỗi: "+r.message)}finally{e.innerHTML='<svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"/></svg> Lưu Thay Đổi',e.disabled=!1}});const F=async e=>{const n=e.target.closest(".sch-edit-btn"),t=e.target.closest(".sch-delete-btn"),s=e.target.closest(".sch-review-btn"),r=e.target.closest(".sch-details-btn"),a=e.target.closest(".sch-guides-btn"),d=e.target.closest(".sch-photo-btn");if(n){const l=n.getAttribute("data-id"),o=f.find(i=>i.id==l);o&&_(o)}if(a){const l=a.getAttribute("data-id"),o=a.getAttribute("data-tour");H(parseInt(l),o)}if(d){const l=parseInt(d.getAttribute("data-id")),o=d.getAttribute("data-tour"),i=f.find(c=>c.id==l);i&&z(l,o,i)}if(t){const l=t.getAttribute("data-id");if(confirm("Bạn có chắc chắn muốn xóa lịch trình này?"))try{if((await fetch(`${w}?id=${l}`,{method:"DELETE"})).ok)E();else throw new Error("Delete failed")}catch(o){alert("Lỗi khi xóa: "+o.message)}}if(s){const l=s.getAttribute("data-id"),o=`${window.location.origin}/review?schedule_id=${l}`;navigator.clipboard.writeText(o).then(()=>{const i=s.innerHTML;s.innerHTML="✅ Đã copy!",setTimeout(()=>s.innerHTML=i,2e3)}).catch(()=>{alert(`Không thể copy. Link:
`+o)})}if(r){const l=r.getAttribute("data-id"),o=r.getAttribute("data-tour"),i=r.getAttribute("data-date"),c=i&&i.includes("T")?i.split("T")[0]:i,v=r.getAttribute("data-group-label")||"",y=`/admin/roster?tour=${encodeURIComponent(o)}&date=${encodeURIComponent(c)}&scheduleId=${l}&groupLabel=${encodeURIComponent(v)}`;history.pushState(null,null,y),window.dispatchEvent(new Event("popstate"))}};C.addEventListener("submit",async e=>{e.preventDefault();const n=e.target.querySelector('button[type="submit"]'),t=n.textContent;n.textContent="Đang lưu...",n.disabled=!0;const s=document.getElementById("sch-edit-id").value,r={id:s?parseInt(s):null,tour_name:document.getElementById("sch-tour-name").value,start_date:document.getElementById("sch-start-date").value,end_date:document.getElementById("sch-end-date").value,slots:parseInt(document.getElementById("sch-slots").value),status:document.getElementById("sch-status").value,group_label:document.getElementById("sch-group-label").value.trim()||null,zalo_link:document.getElementById("sch-zalo-link").value.trim()||null};try{if(!(await fetch(w,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(r)})).ok)throw new Error("Failed to save schedule");B(),E(),alert(s?"✅ Cập nhật lịch trình thành công!":"✅ Thêm lịch trình mới thành công! Form đặt chỗ trên website sẽ tự cập nhật trong 30 giây.")}catch(a){alert("❌ Lỗi: "+a.message)}finally{n.textContent=t,n.disabled=!1}}),m.addEventListener("click",F),A(),E()};export{G as afterRender,R as render};
