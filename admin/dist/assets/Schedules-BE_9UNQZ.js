import{S as $,H as C}from"./Header-CamAZjvb.js";const F=()=>`
    ...
    `,j=()=>{let x=[],h="",u="upcoming";const v="/api/schedules",E="/api/tours",g=document.getElementById("scheduleList"),c=document.getElementById("scheduleModal"),p=document.getElementById("scheduleModalContent"),k=document.getElementById("scheduleForm"),L=(e=null)=>{const o=document.getElementById("scheduleModalTitle");if(e){o.textContent="Chỉnh Sửa Lịch Trình",document.getElementById("sch-edit-id").value=e.id,document.getElementById("sch-tour-name").value=e.tour_name;const t=n=>n&&n.includes("T")?n.split("T")[0]:n;document.getElementById("sch-start-date").value=t(e.start_date),document.getElementById("sch-end-date").value=t(e.end_date),document.getElementById("sch-slots").value=e.slots,document.getElementById("sch-status").value=e.status||"Đang mở",document.getElementById("sch-group-label").value=e.group_label||"",document.getElementById("sch-zalo-link").value=e.zalo_link||""}else o.textContent="Thêm Lịch Khởi Hành",k.reset(),document.getElementById("sch-edit-id").value="",document.getElementById("sch-zalo-link").value="";c.classList.remove("hidden"),setTimeout(()=>{c.classList.add("opacity-100"),p.classList.remove("scale-95"),p.classList.add("scale-100")},10)},m=()=>{c.classList.remove("opacity-100"),p.classList.remove("scale-100"),p.classList.add("scale-95"),setTimeout(()=>{c.classList.add("hidden")},200)};document.getElementById("addScheduleBtn").addEventListener("click",()=>L()),document.getElementById("closeScheduleModalBtn").addEventListener("click",m),document.getElementById("cancelScheduleBtn").addEventListener("click",m),c.addEventListener("click",e=>{e.target===c&&m()});const B=async()=>{try{const e=await fetch(E);if(!e.ok)throw new Error("Failed to load tours");const o=await e.json(),t=document.getElementById("sch-tour-name");t.innerHTML='<option value="">-- Chọn Tour --</option>',o.forEach(a=>{t.innerHTML+=`<option value="${a.name}">${a.name}</option>`});const n=document.getElementById("tourFilterList");let s=`
                <label class="flex items-center gap-3 cursor-pointer group">
                    <input type="radio" name="tourFilter" value="" class="text-csr-orange focus:ring-csr-orange border-gray-200 bg-gray-100" checked>
                    <span class="text-gray-500 group-hover:text-gray-900 transition-colors text-sm">Tất cả Tuyến</span>
                </label>
            `;o.forEach(a=>{s+=`
                ...
                `}),n.innerHTML=s,n.querySelectorAll('input[name="tourFilter"]').forEach(a=>{a.addEventListener("change",r=>{h=r.target.value,f()})}),document.querySelectorAll('input[name="timeFilter"]').forEach(a=>{a.addEventListener("change",r=>{u=r.target.value,document.querySelectorAll('input[name="timeFilter"]').forEach(l=>{const d=l.nextElementSibling;l.checked?(d.classList.add("text-gray-700","font-medium"),d.classList.remove("text-gray-500")):(d.classList.remove("text-gray-700","font-medium"),d.classList.add("text-gray-500"))}),f()})})}catch(e){console.error("Error loading tours:",e)}},y=async()=>{g.innerHTML='<div class="text-center py-12 text-gray-400">Đang tải lịch trình...</div>';try{const e=await fetch(v);if(!e.ok)throw new Error("Failed to fetch schedules");x=await e.json(),f()}catch(e){console.error("Error loading schedules:",e),g.innerHTML='<div class="text-center py-12 text-red-400 font-medium">Lỗi kết nối server.</div>'}},f=()=>{let e=x;h&&(e=e.filter(t=>t.tour_name===h));const o=new Date;if(o.setHours(0,0,0,0),e=e.filter(t=>{const n=new Date(t.start_date);n.setHours(0,0,0,0);const s=n<o;return u==="upcoming"?!s:u==="past"?s:!0}),e.sort((t,n)=>{const s=new Date(t.start_date),a=new Date(n.start_date);return u==="upcoming"?s-a:a-s}),e.length===0){g.innerHTML='<div class="text-center py-12 text-gray-400 italic">Chưa có lịch trình nào'+(h?" cho tuyến này":"")+(u==="past"?" trong quá khứ":" sắp tới")+".</div>";return}g.innerHTML=e.map(t=>{const n=new Date(t.start_date),s=new Date(t.end_date),a=n.toLocaleDateString("vi-VN");s.toLocaleDateString("vi-VN");const r=`${n.getDate().toString().padStart(2,"0")}/${(n.getMonth()+1).toString().padStart(2,"0")}`,l=parseInt(t.booked_count)||0,d=parseInt(t.slots)||0,b=Math.max(0,d-l),w=d>0?Math.min(100,Math.round(l/d*100)):0,M=t.group_label?`<span class="ml-2 text-[10px] font-bold uppercase px-2 py-0.5 rounded-md border bg-blue-50 text-blue-600 border-blue-200">${t.group_label}</span>`:"",S=t.zalo_link?'<span class="ml-1 text-[10px] font-bold uppercase px-2 py-0.5 rounded-md border bg-blue-600 text-white border-blue-700 flex items-center gap-1"><svg class="w-2.5 h-2.5" fill="currentColor" viewBox="0 0 24 24"><path d="M12.14 1c-6.15 0-11.14 4.58-11.14 10.21 0 3.2 1.6 6.05 4.12 7.91-.12.44-.43 1.6-.43 1.6s-.1.38.16.53c.26.15.54-.04.54-.04l1.88-1.29c.6.16 1.23.25 1.88.25 6.15 0 11.14-4.58 11.14-10.21S18.29 1 12.14 1z"/></svg> ZALO</span>':"";let i=t.status||"Đang mở";b<=0?i="Hết chỗ":b<=3&&(i="Sắp hết"),t.status==="Hết chỗ"&&(i="Hết chỗ");const I=i==="Đang mở"?"text-green-600 bg-green-50 border-green-200":i==="Sắp hết"?"text-orange-600 bg-orange-50 border-orange-200":"text-red-500 bg-red-50 border-red-200",H=i==="Đang mở"?"bg-green-500":i==="Sắp hết"?"bg-orange-500":"bg-red-500";return`
                    ...
                    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                        <div class="flex items-center gap-3 md:gap-4 flex-1 min-w-0">
                            <div class="bg-csr-orange/10 text-csr-orange font-bold rounded-xl p-2 md:p-3 text-center min-w-[64px] md:min-w-[72px] shrink-0">
                                ...
                                <div class="text-lg md:text-xl font-black">${r}</div>
                            </div>
                            <div class="flex-1 min-w-0">
                                ...
                                <div class="mt-2 w-full bg-gray-100 rounded-full h-1.5 overflow-hidden max-w-xs hidden md:block">
                                    <div class="${H} h-1.5 rounded-full transition-all duration-500" style="width: ${w}%"></div>
                                </div>
                            </div>
                        </div>
            ...
            `}).join("")},T=async e=>{const o=e.target.closest(".sch-edit-btn"),t=e.target.closest(".sch-delete-btn"),n=e.target.closest(".sch-review-btn"),s=e.target.closest(".sch-details-btn");if(o){const a=o.getAttribute("data-id"),r=x.find(l=>l.id==a);r&&L(r)}if(t){const a=t.getAttribute("data-id");if(confirm("Bạn có chắc chắn muốn xóa lịch trình này?"))try{if((await fetch(`${v}?id=${a}`,{method:"DELETE"})).ok)y();else throw new Error("Delete failed")}catch(r){alert("Lỗi khi xóa: "+r.message)}}if(n){const a=n.getAttribute("data-id"),r=`${window.location.origin}/review?schedule_id=${a}`;navigator.clipboard.writeText(r).then(()=>{const l=n.innerHTML;n.innerHTML="✅ Đã copy!",setTimeout(()=>n.innerHTML=l,2e3)}).catch(()=>{alert(`Không thể copy. Link:
`+r)})}if(s){const a=s.getAttribute("data-id"),r=s.getAttribute("data-tour"),l=s.getAttribute("data-date"),d=l&&l.includes("T")?l.split("T")[0]:l,b=s.getAttribute("data-group-label")||"",w=`/admin/roster?tour=${encodeURIComponent(r)}&date=${encodeURIComponent(d)}&scheduleId=${a}&groupLabel=${encodeURIComponent(b)}`;history.pushState(null,null,w),window.dispatchEvent(new Event("popstate"))}};k.addEventListener("submit",async e=>{e.preventDefault();const o=e.target.querySelector('button[type="submit"]'),t=o.textContent;o.textContent="Đang lưu...",o.disabled=!0;const n=document.getElementById("sch-edit-id").value,s={id:n?parseInt(n):null,tour_name:document.getElementById("sch-tour-name").value,start_date:document.getElementById("sch-start-date").value,end_date:document.getElementById("sch-end-date").value,slots:parseInt(document.getElementById("sch-slots").value),status:document.getElementById("sch-status").value,group_label:document.getElementById("sch-group-label").value.trim()||null,zalo_link:document.getElementById("sch-zalo-link").value.trim()||null};try{if(!(await fetch(v,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(s)})).ok)throw new Error("Failed to save schedule");m(),y(),alert(n?"✅ Cập nhật lịch trình thành công!":"✅ Thêm lịch trình mới thành công! Form đặt chỗ trên website sẽ tự cập nhật trong 30 giây.")}catch(a){alert("❌ Lỗi: "+a.message)}finally{o.textContent=t,o.disabled=!1}}),g.addEventListener("click",T),B(),y()};export{j as afterRender,F as render};
