import{S as $e,H as _e}from"./Header-CARqqxjt.js";console.log("Bookings V3 - Cache Busted!");const De=()=>`
    ...
    `,Ne=()=>{let D=[],S="consult",N=[],te=[],J=[];const Te=async()=>{try{const[n,e,o]=await Promise.all([fetch("/api/tours"),fetch("/api/schedules"),fetch("/api/users")]);N=n.ok?await n.json():[],te=e.ok?await e.json():[],J=o.ok?await o.json():[],X("addTourName"),X("edit-tour"),X("wl-tour"),Ie("edit-sale")}catch(n){console.error("Lỗi tải tours/schedules:",n)}},Ie=n=>{const e=document.getElementById(n);e&&(e.innerHTML='<option value="">-- Website --</option>',J.forEach(o=>{const s=document.createElement("option");s.value=o.id,s.textContent=`${o.full_name||o.fullName} (${o.role})`,e.appendChild(s)}))},X=n=>{const e=document.getElementById(n);if(!e)return;const o=e.value,s=e.options[0].outerHTML;e.innerHTML=s,N.filter(c=>c.is_visible!==!1).forEach(c=>{const g=document.createElement("option");g.value=c.name,g.textContent=c.name,c.name===o&&(g.selected=!0),e.appendChild(g)})},K=(n,e,o,s)=>{const c=document.getElementById(n);if(!c)return;if(c.innerHTML='<option value="">-- Chọn Lịch --</option>',!e){c.innerHTML='<option value="">-- Chọn Tour trước --</option>';return}const g=te.filter(i=>i.tour_name===e||e.includes(i.tour_name)||i.tour_name.includes(e));if(g.length===0){c.innerHTML='<option value="">Chưa có lịch cho tour này</option>';return}g.sort((i,u)=>new Date(i.start_date)-new Date(u.start_date)).forEach(i=>{const u=parseInt(i.booked_count)||0,h=(i.slots||0)-u,y=new Date(i.start_date),p=i.end_date?new Date(i.end_date):null,I=B=>`${B.getDate().toString().padStart(2,"0")}/${(B.getMonth()+1).toString().padStart(2,"0")}`;let E=I(y);p&&(E+=" - "+I(p)),i.group_label&&(E+=` · ${i.group_label}`),E+=` (${h>=0?h:0} chỗ trống)`;const w=document.createElement("option"),m=`${y.getDate().toString().padStart(2,"0")}/${(y.getMonth()+1).toString().padStart(2,"0")}/${y.getFullYear()}`;w.value=`${i.id}::${m}`,w.dataset.scheduleId=i.id,w.dataset.date=m,w.textContent=E,h<=0&&(w.disabled=!0,w.textContent=E.replace("chỗ trống","HẾT CHỖ"));let T=o||"";if(T.includes("-")&&T.length>=10){const B=T.substring(0,10).split("-");B.length===3&&(T=`${B[2]}/${B[1]}/${B[0]}`)}(s&&String(i.id)===String(s)||!s&&T&&m===T)&&(w.selected=!0),c.appendChild(w)})},ne=document.getElementById("addTourName");ne&&ne.addEventListener("change",n=>{const e=n.target.value;K("addTourDate",e);const o=N.find(c=>c.name===e),s=document.getElementById("addTourPrice");if(o&&s){const c=parseInt(o.price)||0;s.value=c;const g=document.getElementById("addDiscount"),i=document.getElementById("addTotalPriceDisplay"),u=parseInt(g?g.value:0)||0,h=c-u;i&&(i.textContent=(h>0?h:0).toLocaleString("vi-VN")+"đ")}});const oe=document.getElementById("edit-tour");oe&&oe.addEventListener("change",n=>{const e=n.target.value;K("edit-date",e);const o=N.find(c=>c.name===e),s=document.getElementById("edit-total");o&&s&&(parseInt(s.value)===0||!s.value)&&(s.value=parseInt(o.price)||0,typeof window.updateEditRemaining=="function"&&window.updateEditRemaining())});const se=document.getElementById("wl-tour");se&&se.addEventListener("change",n=>{const e=n.target.value;K("wl-date",e)}),Te();const R=()=>{const n=document.getElementById("bookingsTableHead"),e=document.getElementById("bookingsTableBody");if(!e||!n)return;const o=S==="upcoming"||S==="ready"||S==="completed";o?n.innerHTML=`
            ...
            `;const s=document.getElementById("filterSearch")?document.getElementById("filterSearch").value.toLowerCase():"",c=document.getElementById("filterTour")?document.getElementById("filterTour").value:"",g=document.getElementById("filterDate")?document.getElementById("filterDate").value:"",i=document.getElementById("filterSale")?document.getElementById("filterSale").value:"",u=document.getElementById("filterStatus")?document.getElementById("filterStatus").value:"";let h=D.filter(t=>{let l=!1;const a=parseInt(t.total_price)>0&&parseInt(t.total_price)===parseInt(t.deposit),d=f=>{if(!f)return!1;try{let k=f.trim();if(k.includes("-")&&!/^\d{4}-\d{2}-\d{2}/.test(k)&&(k=k.split("-")[0].trim()),k.includes("/")){const x=k.split("/");if(x.length>=2){const C=parseInt(x[0],10),v=parseInt(x[1],10)-1,_=x.length===3?parseInt(x[2],10):new Date().getFullYear(),j=new Date(_,v,C),L=new Date;return L.setHours(0,0,0,0),j.getTime()<L.getTime()}}if(/^\d{4}-\d{2}-\d{2}/.test(k)){const x=k.split("-"),C=parseInt(x[0],10),v=parseInt(x[1],10)-1,_=parseInt(x[2],10),j=new Date(C,v,_),L=new Date;return L.setHours(0,0,0,0),j.getTime()<L.getTime()}}catch(k){console.warn("isPastTour error parsing:",f,k)}return!1},r=t.status||"",b=!d(t.date);return S==="consult"?l=r==="Chờ tư vấn"&&b:S==="pending"?l=(r==="Chờ cọc"||r==="Chờ xác nhận cọc"||r==="")&&b:S==="upcoming"?l=(r==="Đã cọc"||r==="Đã cọc (Chờ đi)")&&!a&&b:S==="ready"?l=(a||r==="Hoàn thành"||r==="Hoàn tất phí"||r==="Hoàn tất")&&b&&r!=="Đã hủy"&&r!=="Bảo lưu":S==="completed"&&(l=!b||r==="Đã đi"||r==="Đã hủy"||r==="Bảo lưu"),!(!l||c&&t.tour!==c||g&&Y(t.date)!==g||i&&i!=="null"&&t.sale_name!==i||i==="null"&&t.sale_name&&t.sale_name!=="Website"||s&&!`${t.name||""} ${t.phone||""} ${t.customer_id||""} ${t.id||""} csr${t.id||""} #csr${t.id||""}`.toLowerCase().includes(s)||u&&t.status!==u)});const y=document.getElementById("statTitle1"),p=document.getElementById("statTitle2"),I=document.getElementById("statTitle3"),E=document.getElementById("statTotalCustomers"),w=document.getElementById("statTotalRevenue"),m=document.getElementById("statTotalCollected");let T=h.length;if(S==="consult"){const t=document.getElementById("addWaitingBtn"),l=document.getElementById("addBookingBtn");t&&t.classList.remove("hidden"),l&&l.classList.add("hidden"),y&&(y.textContent="Tổng Đăng Ký Chờ Lịch"),p&&(p.textContent="Chưa Được Liên Hệ"),I&&(I.textContent="Đã Có Sale Phụ Trách");let a=0,d=0;h.forEach(r=>{!r.sale_id||r.sale_id==="null"||!r.sale_name||r.sale_name==="Website"||r.sale_name==="null"?a++:d++}),E&&(E.textContent=T),w&&(w.textContent=a+" Người"),m&&(m.textContent=d+" Người")}else if(S==="pending"){const t=document.getElementById("addWaitingBtn"),l=document.getElementById("addBookingBtn");t&&t.classList.add("hidden"),l&&l.classList.remove("hidden"),y&&(y.textContent="Tổng Khách (Chờ Cọc)"),p&&(p.textContent="Khách Chưa Tư Vấn (Mới)"),I&&(I.textContent="Đã Có Sale Giữ Chỗ");let a=0,d=0;h.forEach(r=>{!r.sale_id||r.sale_id==="null"||!r.sale_name||r.sale_name==="Website"||r.sale_name==="null"?a++:d++}),E&&(E.textContent=T),w&&(w.textContent=a+" Đơn"),m&&(m.textContent=d+" Đơn")}else if(S==="upcoming"){y&&(y.textContent="Tổng Số Khách"),p&&(p.textContent="Chưa Xác Nhận Cọc"),I&&(I.textContent="Chờ Thanh Toán Còn Lại");let t=0,l=0;h.forEach(a=>{a.status==="Chờ xác nhận cọc"?t++:parseInt(a.deposit)>0&&parseInt(a.total_price)>parseInt(a.deposit)&&l++}),E&&(E.textContent=T),w&&(w.textContent=t+" Khách"),m&&(m.textContent=l+" Khách")}else if(S==="ready"){y&&(y.textContent="Khách Sẵn Sàng (Full)"),p&&(p.textContent="Tổng Doanh Thu Tab"),I&&(I.textContent="Thực Thu (Full Tận Nơi)");let t=0,l=0;h.forEach(a=>{t+=parseInt(a.total_price)||0,l+=parseInt(a.deposit)||0}),E&&(E.textContent=T),w&&(w.textContent=t>0?t.toLocaleString("vi-VN")+"đ":"0đ"),m&&(m.textContent=l>0?l.toLocaleString("vi-VN")+"đ":"0đ")}else{y&&(y.textContent="Khách Đã Xong (Lịch Sử)"),p&&(p.textContent="Tổng Doanh Thu Tab"),I&&(I.textContent="Thực Thu (Đã Đóng)");let t=0,l=0;h.forEach(a=>{t+=parseInt(a.total_price)||0,l+=parseInt(a.deposit)||0}),E&&(E.textContent=T),w&&(w.textContent=t>0?t.toLocaleString("vi-VN")+"đ":"0đ"),m&&(m.textContent=l>0?l.toLocaleString("vi-VN")+"đ":"0đ")}const B=o?8:4;if(!h||h.length===0){let t=S==="consult"?"Chưa có khách hàng tư vấn nào.":S==="pending"?"Chưa có Khách chờ cọc.":S==="upcoming"?"Chưa có Khách nào Đã Cọc.":S==="ready"?"Chưa có khách sẵn sàng.":"Danh sách rỗng.";e.innerHTML=`<tr><td colspan="${B}" class="p-8 text-center text-gray-500">${t}</td></tr>`;return}e.innerHTML=h.map(t=>{const l=S==="upcoming"||S==="ready"?"bg-green-50/20 hover:bg-green-50 transition-colors":"hover:bg-gray-100 transition-colors";if(o){let a=parseInt(t.total_price)||0;if(a===0&&t.tour&&N&&N.length>0){const v=N.find(_=>_.name&&t.tour&&_.name.toLowerCase().trim()===t.tour.toLowerCase().trim());v&&parseInt(v.price)>0&&(a=parseInt(v.price))}const d=parseInt(t.deposit)||0,r=a-d,b=a>0?a.toLocaleString("vi-VN")+"đ":"Chưa định giá",f=d>0?d.toLocaleString("vi-VN")+"đ":"0đ",k=r>0?r.toLocaleString("vi-VN")+"đ":r===0&&a>0?"Đã thu trọn":"-";let x="";return t.status==="Đã hủy"?x='<span class="bg-red-50 text-red-500 border border-red-200 px-2 py-0.5 rounded text-xs block w-full text-center font-bold">❌ Đã hủy</span>':t.status==="Bảo lưu"?x='<span class="bg-gray-100 text-gray-500 border border-gray-300 px-2 py-0.5 rounded text-xs block w-full text-center font-bold">⏸️ Bảo lưu</span>':t.status==="Hoàn tất phí"?x='<span class="bg-emerald-100 text-emerald-700 border border-emerald-200 px-2 py-0.5 rounded text-xs font-bold block w-full text-center">Hoàn Tất Phí</span>':t.status==="Hoàn thành"||t.status==="Đã đi"?x='<span class="bg-violet-100 text-violet-700 border border-violet-200 px-2 py-0.5 rounded text-xs block w-full text-center font-bold">Hoàn Thành</span>':t.status==="Chờ xác nhận cọc"?x=`<button class="action-btn confirm-deposit-btn bg-csr-orange text-white px-3 py-1.5 rounded text-xs font-bold shadow-sm hover:bg-[#d65503] w-full" data-id="${t.id}">Xác nhận cọc</button>`:d>0&&r===0?x='<span class="bg-green-100 text-green-700 border border-green-200 px-2 py-0.5 rounded text-xs font-medium block w-full text-center">Hoàn tất phí</span>':x='<span class="bg-blue-100 text-blue-700 border border-blue-200 px-2 py-0.5 rounded text-xs font-medium block w-full text-center">Đã Cọc</span>',`
                ...
                `}else{const a=t.name?t.name.substring(0,2).toUpperCase():"KH",d=S==="consult",r=!t.sale_id||t.sale_id==="null"||!t.sale_name||t.sale_name==="null";let b="";d&&r?b=`<button class="action-btn claim-btn bg-csr-orange hover:bg-[#d65503] text-white px-3 py-1.5 rounded text-xs font-bold shadow-sm transition-colors" data-id="${t.id}">Nhận khách</button>`:b=`<span class="text-sm text-gray-500">${t.sale_name||"Website"}</span>`;const f=`
    ...
    `}}).join(""),document.querySelectorAll(".claim-btn").forEach(t=>{t.addEventListener("click",async l=>{l.stopPropagation();const a=t.getAttribute("data-id"),d=localStorage.getItem("csr_user");let r=null,b="Admin";if(d){const f=JSON.parse(d);r=f.id,b=f.fullName||f.full_name||"Admin"}t.textContent="Đang xử lý...",t.disabled=!0;try{if((await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:a,sale_id:r,sale_name:b})})).ok){const k=D.find(x=>x.id==a);k&&(k.sale_id=r,k.sale_name=b),R()}else alert("Lỗi khi nhận khách. Vui lòng thử lại."),t.textContent="Nhận khách",t.disabled=!1}catch(f){console.error("Lỗi nhận khách:",f),t.textContent="Nhận khách",t.disabled=!1}})})},Ee=()=>{const n=new Set,e=new Set,o=new Set;D.forEach(i=>{if(i.tour&&n.add(i.tour),i.date){const u=Y(i.date);u&&e.add(u)}i.sale_name&&i.sale_name!=="Website"&&i.sale_name!=="null"&&o.add(i.sale_name.trim())});const s=document.getElementById("filterTour"),c=document.getElementById("filterDate"),g=document.getElementById("filterSale");if(s){const i=s.value;s.innerHTML='<option value="">Tất cả Tour</option>',[...n].sort().forEach(u=>s.innerHTML+=`<option value="${u}">${u}</option>`),s.value=i}if(c){const i=c.value;c.innerHTML='<option value="">Tất cả Ngày</option>',[...e].sort().forEach(u=>c.innerHTML+=`<option value="${u}">${u}</option>`),c.value=i}if(g){const i=g.value;g.innerHTML='<option value="">Tất cả Sale</option>',g.innerHTML+='<option value="null">Nguồn Website</option>',[...o].sort().forEach(u=>{g.innerHTML+=`<option value="${u}">${u}</option>`}),g.value=i}};function Y(n){if(!n)return"";if(/^\d{1,2}\/\d{1,2}\s*-\s*\d{1,2}\/\d{1,2}$/.test(n))return n.replace(/\s+/g,"");try{if(/^\d{4}-\d{2}-\d{2}$/.test(n)){const[e,o,s]=n.split("-"),c=`${parseInt(s)}/${parseInt(o)}`;return`${c}-${c}`}if(/^\d{1,2}\/\d{1,2}\/\d{4}$/.test(n)){const[e,o,s]=n.split("/"),c=`${parseInt(e)}/${parseInt(o)}`;return`${c}-${c}`}if(n.includes("-"))return n.split("-").map(o=>{const s=o.trim();if(/\/\d{4}$/.test(s)){const[c,g]=s.split("/");return`${parseInt(c)}/${parseInt(g)}`}return s}).join("-")}catch{console.warn("Date normalization failed for:",n)}return n}const H=async()=>{try{D=await(await fetch("/api/bookings")).json(),D.sort((e,o)=>parseInt(o.id)-parseInt(e.id)),Ee(),R()}catch(n){console.error("Lỗi parse list đơn hàng:",n);const e=document.getElementById("bookingsTableBody");e&&(e.innerHTML='<tr><td colspan="4" class="p-8 text-center text-red-500">Lỗi lấy dữ liệu từ Server.</td></tr>')}};H(),["filterSearch","filterTour","filterDate","filterSale","filterStatus"].forEach(n=>{const e=document.getElementById(n);e&&(n==="filterSearch"?e.addEventListener("input",R):e.addEventListener("change",R))});const ae=document.getElementById("exportExcelBtn");ae&&ae.addEventListener("click",()=>{const n=document.getElementById("filterSearch")?document.getElementById("filterSearch").value.toLowerCase():"",e=document.getElementById("filterTour")?document.getElementById("filterTour").value:"",o=document.getElementById("filterDate")?document.getElementById("filterDate").value:"",s=document.getElementById("filterSale")?document.getElementById("filterSale").value:"";let c=D.filter(p=>{let I=!1;const E=parseInt(p.total_price)>0&&parseInt(p.total_price)===parseInt(p.deposit),w=T=>{if(!T)return!1;try{let B=T.trim();if(B.includes("-")&&!/^\d{4}-\d{2}-\d{2}/.test(B)&&(B=B.split("-")[0].trim()),B.includes("/")){const t=B.split("/");if(t.length>=2){const l=parseInt(t[0],10),a=parseInt(t[1],10)-1,d=t.length===3?parseInt(t[2],10):new Date().getFullYear(),r=new Date(d,a,l),b=new Date;return b.setHours(0,0,0,0),r.getTime()<b.getTime()}}if(/^\d{4}-\d{2}-\d{2}/.test(B)){const t=B.split("-"),l=parseInt(t[0],10),a=parseInt(t[1],10)-1,d=parseInt(t[2],10),r=new Date(l,a,d),b=new Date;return b.setHours(0,0,0,0),r.getTime()<b.getTime()}}catch(B){console.warn("isPastTour error parsing in export:",T,B)}return!1},m=(p.status==="Hoàn thành"||p.status==="Hoàn tất"||p.status==="Hoàn tất phí"||p.status==="Đã đi"||E)&&w(p.date);return S==="consult"?I=p.status==="Chờ tư vấn"&&!m:S==="pending"?I=(!p.status||p.status==="Chờ cọc")&&!m:S==="upcoming"?I=p.status&&p.status!=="Chờ tư vấn"&&p.status!=="Chờ cọc"&&!E&&p.status!=="Hoàn thành"&&!m:S==="ready"?I=(E||p.status==="Hoàn thành")&&!m:S==="completed"&&(I=m),!(!I||e&&p.tour!==e||o&&Y(p.date)!==o||s&&s!=="null"&&p.sale_name!==s||s==="null"&&p.sale_name&&p.sale_name!=="Website"||n&&!`${p.name||""} ${p.phone||""} ${p.customer_id||""} ${p.id||""} csr${p.id||""} #csr${p.id||""}`.toLowerCase().includes(n))});if(c.length===0){alert("Danh sách rỗng. Không có dữ liệu để xuất.");return}let g=`
        ...
        <div class="space-y-4 pb-2">
            ...
            <div class="bg-gradient-to-br from-orange-50 to-amber-50 border border-orange-100 rounded-2xl p-4">
                ...
                <div class="grid grid-cols-2 gap-x-4">
                    ...
                    ${m("Ngày Khởi Hành",`<span class="font-bold text-gray-900">${s(e.date)}</span>`)}
                    ${m("Ngày Đăng Ký",e.created_at?s(e.created_at.split("T")[0]):"—")}
            ...
            <div class="bg-gradient-to-br from-csr-orange to-orange-600 rounded-2xl p-4 text-white shadow-lg shadow-orange-500/20">
                ...
                <div class="space-y-2 text-sm">
                    <div class="flex justify-between items-center py-1.5 border-b border-white/10">
                        ...
                        <span class="font-bold">${o(c+i)}</span>
                    </div>
                    ${i>0?`
                    <div class="flex justify-between items-center py-1.5 border-b border-white/10">
                    ...
                    <div class="flex justify-between items-center py-1.5 border-b border-white/10">
                        <div>
                            ...
                            ${e.deposit_date?`<div class="text-[11px] text-orange-200 mt-0.5">Ngày: ${s(e.deposit_date)}</div>`:""}
                        </div>
                        <span class="font-bold text-green-100">${g>0?o(g):"0đ (Chưa cọc)"}</span>
                    </div>
                    <div class="flex justify-between items-center py-3 bg-white text-gray-900 rounded-xl px-4 mt-3 shadow-inner">
                        ...
                        <span class="font-black text-2xl ${u>0?"text-red-600":"text-green-600"}">
                            ${u>0?o(u):"✅ ĐÃ THU ĐỦ"}
                        </span>
                    </div>
                </div>
            ...
            ${(()=>{const t=e.phone;if(!t)return"";const l=D.filter(C=>C.phone===t&&!C.status?.includes("hủy")&&!C.status?.includes("Hủy")).sort((C,v)=>new Date(v.created_at||0)-new Date(C.created_at||0)),a=l.length,d=l.reduce((C,v)=>C+(parseInt(v.total_price)||0),0);let r,b,f;a>=5?(r="KHÁCH VIP",b="bg-purple-100 text-purple-700 border-purple-300",f="👑"):a>=2?(r="KHÁCH THÂN THIẾT",b="bg-green-100 text-green-700 border-green-300",f="🌟"):(r="KHÁCH MỚI",b="bg-gray-100 text-gray-500 border-gray-300",f="👋");const k=l.filter(C=>C.id!=n),x=k.length>0?k.slice(0,5).map((C,v)=>{const _=C.status?.includes("Hoàn")?"bg-green-100 text-green-600":C.status?.includes("cọc")||C.status?.includes("Cọc")?"bg-blue-100 text-blue-600":"bg-gray-100 text-gray-500";return`<div class="flex items-center gap-3 py-2 border-b border-gray-100 last:border-none">
                    ...
                    <div class="flex items-center justify-between mb-4 gap-3">
                        <div class="flex items-center gap-2">
                            ...
                            <span class="text-xs font-black uppercase tracking-widest px-3 py-1.5 rounded-full border ${b}">${r}</span>
                        </div>
                ...
                <div>
                    ...
                    <div class="text-sm font-bold text-blue-800">${e.sale_name}</div>
        ...
        </div>`;document.getElementById("detailContentBlock").innerHTML=B,document.getElementById("detailModal").classList.remove("hidden"),setTimeout(()=>{document.getElementById("detailModal").classList.add("opacity-100");const t=document.getElementById("detailModalContent");t&&(t.classList.remove("scale-95","translate-y-4"),t.classList.add("scale-100","translate-y-0"))},10)},window.actionEdit=async n=>{document.getElementById("rowActionModal").classList.add("hidden");const e=D.find(t=>t.id==n);if(!e)return;document.getElementById("edit-id").value=n,document.getElementById("edit-name").value=e.name||"",document.getElementById("edit-phone").value=e.phone||"",document.getElementById("edit-medal-name").value=e.medal_name||"",document.getElementById("edit-tour").value=e.tour||"",typeof K=="function"&&K("edit-date",e.tour||"",e.date||"",e.schedule_id||null),(t=>{let l=t.dob||"";if(typeof l=="string"&&l.includes("/")){const r=l.split("/");r.length===3&&(l=`${r[2]}-${r[1].padStart(2,"0")}-${r[0].padStart(2,"0")}`)}document.getElementById("edit-dob").value=l,document.getElementById("edit-gender").value=t.gender||"Khác",document.getElementById("edit-allergy").value=t.allergy||t.medical_notes||"",document.getElementById("edit-address").value=t.address||"",document.getElementById("edit-diet").value=t.diet||t.dietary||"",document.getElementById("edit-trekking-pole").value=t.trekking_pole||"Không";const a=document.getElementById("edit-id-card");a&&(a.value=t.id_card||t.cccd||"");const d=document.getElementById("edit-pickup-point");d&&(d.value=t.pickup_point||"")})(e),document.getElementById("edit-status").value=e.status||"Chờ xác nhận cọc",document.getElementById("edit-commitments").checked=!!e.commitments,document.getElementById("edit-special").value=e.special||"";try{const t=localStorage.getItem("csr_user");if(t&&JSON.parse(t).role==="admin"){const a=document.getElementById("edit-sale-container");a&&a.classList.remove("hidden");const d=document.getElementById("edit-sale");d&&(d.value=e.sale_id||"")}}catch{}if((!e.dob||!e.address||!e.gender||e.gender==="Khác")&&(e.customer_id||e.phone))try{const t=e.customer_id||e.phone,l=await fetch(`/api/admin_customers?action=search&keyword=${encodeURIComponent(t)}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({keyword:t})}),a=await l.json();if(l.ok&&a.success){const d=a.data,r=document.getElementById("edit-dob"),b=document.getElementById("edit-address"),f=document.getElementById("edit-allergy"),k=document.getElementById("edit-diet"),x=document.getElementById("edit-gender"),C=document.getElementById("edit-medal-name");r&&!r.value&&(r.value=d.dob||""),b&&!b.value&&(b.value=d.address||""),f&&!f.value&&(f.value=d.medical_notes||""),k&&!k.value&&(k.value=d.dietary||""),x&&(x.value==="Khác"||!x.value)&&(x.value=d.gender||"Khác"),C&&!C.value&&(C.value=d.medal_name||"");const v=document.getElementById("edit-id-card");v&&!v.value&&(v.value=d.cccd||"")}}catch(t){console.error("Pull CRM error:",t)}let s=0;try{const t=typeof e.services_booked=="string"?JSON.parse(e.services_booked):e.services_booked||[];Array.isArray(t)&&t.forEach(l=>s+=parseInt(l.price)||0)}catch{}let g=(parseInt(e.total_price)||0)+(parseInt(e.discount)||0)-s;if(g===0&&e.tour){const t=N.find(l=>l.name===e.tour);t&&parseInt(t.price)>0&&(g=parseInt(t.price))}document.getElementById("edit-total").value=g,document.getElementById("edit-discount").value=e.discount||0,document.getElementById("edit-deposit").value=e.deposit||0;const i=parseInt(e.deposit_required);document.getElementById("edit-deposit-required").value=isNaN(i)?1e6:i,window.updateEditRemaining();const u=document.getElementById("servicesContainer"),h=document.getElementById("emptyServicesMsg"),y=document.getElementById("servicesTotalRow"),p=document.getElementById("presetServicesBlock"),I=document.getElementById("presetServicesList"),E=document.getElementById("servicesDivider");let w=[];try{if(e.services_booked){const t=typeof e.services_booked=="string"?JSON.parse(e.services_booked):e.services_booked;Array.isArray(t)&&(w=t)}}catch{}if(I&&p){I.innerHTML="";const t=N.find(a=>a.name===e.tour);let l=[];try{if(t?.services){const a=typeof t.services=="string"?JSON.parse(t.services):t.services;Array.isArray(a)&&(l=a)}}catch{}l.length>0?(p.classList.remove("hidden"),E&&E.classList.remove("hidden"),l.forEach(a=>{const d=w.some(b=>(b.label||b.name||"").toLowerCase()===(a.label||"").toLowerCase()),r=document.createElement("label");r.className="preset-service-card flex items-center gap-3 p-3 rounded-xl border-2 cursor-pointer transition-all "+(d?"border-green-400 bg-green-50":"border-gray-100 bg-gray-50 hover:border-orange-200"),r.innerHTML=`
                        <input type="checkbox" class="preset-service-check w-4 h-4 rounded accent-green-500 shrink-0" 
                               ...
                               ${d?"checked":""}>
                        ${a.image?`<img src="${a.image}" class="w-10 h-10 rounded-lg object-cover shrink-0">`:'<div class="w-10 h-10 rounded-lg bg-green-100 flex items-center justify-center text-green-500 text-lg shrink-0">✨</div>'}
                        <div class="flex-1 min-w-0">
                            <div class="font-bold text-sm text-gray-800">${a.label||""}</div>
                            ${a.description?`<div class="text-xs text-gray-400 truncate">${a.description}</div>`:""}
                        </div>
                        <div class="text-sm font-black text-green-600 shrink-0">+${parseInt(a.price||0).toLocaleString("vi-VN")}đ</div>
                    `,r.querySelector(".preset-service-check").addEventListener("change",b=>{r.className="preset-service-card flex items-center gap-3 p-3 rounded-xl border-2 cursor-pointer transition-all "+(b.target.checked?"border-green-400 bg-green-50":"border-gray-100 bg-gray-50 hover:border-orange-200"),window.updateServicesTotal()}),I.appendChild(r)})):(p.classList.add("hidden"),E&&E.classList.add("hidden"))}const m=document.getElementById("edit-pickup-point");if(m){const t=N.find(f=>f.name===e.tour);let l=[];try{if(t?.pickup_points){const f=typeof t.pickup_points=="string"?JSON.parse(t.pickup_points):t.pickup_points;Array.isArray(f)&&(l=f)}}catch{}m.innerHTML='<option value="">— Chưa chọn —</option>',l.forEach(f=>{const k=(f.label||f.name||f)+(f.time?` (${f.time})`:""),x=f.label||f.name||f,C=document.createElement("option");C.value=x,C.textContent=k,m.appendChild(C)});const a=document.createElement("option");a.value="__custom__",a.textContent="✏️ Khác (nhập tay)",m.appendChild(a);const d=document.getElementById("edit-pickup-custom"),r=f=>{d&&(f==="__custom__"?(d.classList.remove("hidden"),d.focus()):(d.classList.add("hidden"),d&&(d.value="")))},b=e.pickup_point||"";b&&(l.some(k=>(k.label||k.name||k)===b)?m.value=b:(m.value="__custom__",d&&(d.value=b,d.classList.remove("hidden")))),m.addEventListener("change",()=>r(m.value))}if(u){u.querySelectorAll(".service-row").forEach(d=>d.remove());const t=N.find(d=>d.name===e.tour);let l=new Set;try{const d=t?.services,r=d?typeof d=="string"?JSON.parse(d):d:[];Array.isArray(r)&&r.forEach(b=>l.add((b.label||"").toLowerCase()))}catch{}const a=w.filter(d=>!l.has((d.label||d.name||"").toLowerCase()));a.length>0?(h&&h.classList.add("hidden"),y&&y.classList.remove("hidden"),a.forEach(d=>window.addServiceRow(d.label||d.name||"",d.price||0))):h&&h.classList.remove("hidden"),window.updateServicesTotal()}const T=document.getElementById("editModal"),B=document.getElementById("editModalContent");T&&B&&(T.classList.remove("hidden"),setTimeout(()=>{T.classList.add("opacity-100"),B.classList.remove("scale-95","translate-y-4"),B.classList.add("scale-100","translate-y-0")},10))},window.actionDelete=async n=>{if(document.getElementById("rowActionModal").classList.add("hidden"),confirm("Bạn có chắc chắn muốn xóa Đơn hàng này? Thao tác không thể hoàn tác."))try{if((await fetch(`/api/bookings?id=${n}`,{method:"DELETE"})).ok)alert("✅ Đã xóa đơn hàng thành công!"),H();else throw new Error("Lỗi từ Server")}catch(e){alert("❌ Không thể xóa bảng ghi này: "+e.message)}},window.updateServicesTotal=()=>{let n=0;document.querySelectorAll(".preset-service-check:checked").forEach(s=>{n+=parseInt(s.dataset.price)||0}),document.querySelectorAll(".service-price-input").forEach(s=>{n+=parseInt(s.value)||0});const e=document.getElementById("servicesTotalDisplay");e&&(e.textContent=n.toLocaleString("vi-VN")+"đ");const o=document.getElementById("servicesTotalRow");return o&&o.classList.toggle("hidden",n===0),window.updateEditRemaining(),n},window.addServiceRow=(n="",e=0)=>{const o=document.getElementById("servicesContainer"),s=document.getElementById("emptyServicesMsg"),c=document.getElementById("servicesTotalRow");if(!o)return;s&&s.classList.add("hidden"),c&&c.classList.remove("hidden");const g=document.createElement("div");g.className="service-row flex items-center gap-2 bg-gray-50 border border-gray-100 rounded-xl px-3 py-2.5",g.innerHTML=`
        ...
        `,g.querySelector(".remove-service-btn").addEventListener("click",()=>{g.remove(),o.querySelectorAll(".service-row").length===0&&(s&&s.classList.remove("hidden"),c&&c.classList.add("hidden")),window.updateServicesTotal()}),g.querySelector(".service-price-input").addEventListener("input",window.updateServicesTotal),o.appendChild(g),window.updateServicesTotal()};const re=document.getElementById("addServiceBtn");re&&re.addEventListener("click",()=>window.addServiceRow()),window.updateEditRemaining=()=>{const n=parseInt(document.getElementById("edit-total")?.value)||0,e=parseInt(document.getElementById("edit-discount")?.value)||0,o=parseInt(document.getElementById("edit-deposit")?.value)||0;let s=0;document.querySelectorAll(".preset-service-check:checked").forEach(y=>{s+=parseInt(y.dataset.price)||0}),document.querySelectorAll(".service-price-input").forEach(y=>{s+=parseInt(y.value)||0});const c=n-e+s,g=c-o,i=document.getElementById("edit-grand-total");i&&(i.textContent=c.toLocaleString("vi-VN")+"đ");const u=document.getElementById("edit-remaining");u&&(u.textContent=g>0?g.toLocaleString("vi-VN")+"đ":"0đ");const h=document.getElementById("edit-status");if(h){const y=h.value;["Chờ tư vấn","Chờ cọc","Chờ xác nhận cọc"].includes(y)&&(o>0&&g>0?h.value="Đã cọc":o===0&&(h.value=y))}};const ce=document.getElementById("edit-total"),ue=document.getElementById("edit-deposit");ce&&ce.addEventListener("input",window.updateEditRemaining),ue&&ue.addEventListener("input",window.updateEditRemaining),document.querySelectorAll('#bookingModal button[onclick*="hidden"]').forEach(n=>{n.removeAttribute("onclick"),n.addEventListener("click",window.closeModal)});const pe=async n=>{try{const o=await(await fetch("/api/admin_customers?action=create",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({full_name:n.name,phone:n.phone,cccd:n.id_card||"",dob:n.dob||null,gender:n.gender||"Khác",medical_notes:n.allergy||"",dietary:n.diet||""})})).json();o.success&&o.csr_code&&(console.log("✅ Đã đồng bộ CRM thành công:",n.name,o.csr_code),(await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:n.id,customer_id:o.csr_code})})).ok&&(console.log("✅ Đã cập nhật Booking mã CRM"),typeof H=="function"&&H()))}catch(e){console.error("❌ Lỗi đồng bộ CRM:",e)}},ge=document.getElementById("bookingsTableBody");ge&&ge.addEventListener("click",async n=>{const e=n.target.closest(".action-btn");if(!e){const g=n.target.closest(".row-clickable");if(g){const i=g.getAttribute("data-id"),u=D.find(h=>h.id==i);u&&openRowActionModal(u)}return}const o=e.getAttribute("data-id"),s=D.find(g=>g.id==o);if(!s)return;const c=document.getElementById("rowActionModal");if(c&&c.classList.add("hidden"),e.classList.contains("confirm-deposit-btn")){if(confirm("Xác nhận khách hàng này đã chuyển khoản đặt cọc?"))try{if(e.disabled=!0,e.classList.add("opacity-50"),(await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:o,status:"Đã cọc"})})).ok)alert("✅ Đã xác nhận cọc thành công!"),pe(s),typeof H=="function"&&H();else throw new Error("Lỗi cập nhật API")}catch(g){alert("❌ Lỗi: "+g.message)}finally{e.disabled=!1,e.classList.remove("opacity-50")}return}if(e.classList.contains("delete-btn")){window.actionDelete(o);return}else e.classList.contains("process-btn")?window.actionEdit(o):e.classList.contains("view-btn")?(document.getElementById("rowActionModal").classList.add("hidden"),window.actionView(o)):e.classList.contains("edit-btn")&&window.actionEdit(o)});const q=document.getElementById("smartSearch"),M=document.getElementById("searchSuggestions");if(q){let n;q.addEventListener("input",e=>{const o=e.target.value.toLowerCase().trim();if(clearTimeout(n),o.length<2){M&&M.classList.add("hidden");return}n=setTimeout(async()=>{try{const s=await fetch(`/api/admin_customers?action=search&keyword=${o}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({keyword:o})}),c=await s.json();if(s.ok&&c.success){const i=[c.data];M&&(M.innerHTML=i.map(u=>`
                            <div class="suggestion-item p-3 hover:bg-orange-50 cursor-pointer border-b border-gray-100 last:border-0 transition-colors" 
                                 ...
                                 data-cccd="${u.cccd||""}"
                                 data-allergy="${u.medical_notes||""}"
                                 data-diet="${u.dietary||""}"
                                 data-trekking_pole="${u.trekking_pole||""}"
                                 data-medal-name="${u.medal_name||""}">
                                <div class="font-bold text-gray-900 text-sm">${u.full_name}</div>
                                <div class="flex justify-between items-center mt-1">
                                    <span class="text-xs text-gray-500">${u.phone}</span>
                                    ${u.csr_code?`<span class="bg-orange-100 text-csr-orange text-[10px] font-bold px-2 py-0.5 rounded">${u.csr_code}</span>`:""}
                                </div>
                            </div>
                        `).join(""),M.classList.remove("hidden"))}else M&&(M.innerHTML='<div class="p-3 text-xs text-gray-400 italic text-center">Không tìm thấy khách hàng cũ</div>',M.classList.remove("hidden"))}catch(s){console.error("Search error:",s)}},300)}),M&&M.addEventListener("click",e=>{const o=e.target.closest(".suggestion-item");if(o){const s=document.getElementById("addFullName"),c=document.getElementById("addPhone"),g=document.getElementById("addCsrCode");s&&(s.value=o.getAttribute("data-name")),c&&(c.value=o.getAttribute("data-phone")),g&&(g.value=o.getAttribute("data-csr"));const i=document.getElementById("addDob"),u=document.getElementById("addGender"),h=document.getElementById("addIdCard"),y=document.getElementById("addAddress");i&&(i.value=o.getAttribute("data-dob")||""),u&&(u.value=o.getAttribute("data-gender")||"Khác"),h&&(h.value=o.getAttribute("data-cccd")||""),y&&(y.value=o.getAttribute("data-address")||"");const p=document.getElementById("addAllergy"),I=document.getElementById("addDiet"),E=document.getElementById("addTrekkingPole"),w=document.getElementById("addMedalName");p&&(p.value=o.getAttribute("data-allergy")||""),I&&(I.value=o.getAttribute("data-diet")||""),E&&(E.value=o.getAttribute("data-trekking_pole")||"Không"),w&&(w.value=o.getAttribute("data-medal-name")||""),window._selectedCustomer={fullName:o.getAttribute("data-name"),phone:o.getAttribute("data-phone"),csrCode:o.getAttribute("data-csr"),dob:o.getAttribute("data-dob")||"",gender:o.getAttribute("data-gender")||"",address:o.getAttribute("data-address")||"",id_card:o.getAttribute("data-cccd")||"",allergy:o.getAttribute("data-allergy")||"",diet:o.getAttribute("data-diet")||"",trekking_pole:o.getAttribute("data-trekking_pole")||"",medal_name:o.getAttribute("data-medal-name")||""},q.value=o.getAttribute("data-name"),M.classList.add("hidden");const m=`
