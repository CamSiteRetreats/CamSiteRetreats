import{S as A,H as P}from"./Header-Dmidaim7.js";const z=()=>`
    ...
    `,N=()=>{let w=[],v="",h="upcoming";const k="/api/schedules",$="/api/tours",p=document.getElementById("scheduleList"),g=document.getElementById("scheduleModal"),y=document.getElementById("scheduleModalContent"),S=document.getElementById("scheduleForm"),M=(e=null)=>{const l=document.getElementById("scheduleModalTitle");if(e){l.textContent="Chỉnh Sửa Lịch Trình",document.getElementById("sch-edit-id").value=e.id,document.getElementById("sch-tour-name").value=e.tour_name;const t=s=>s&&s.includes("T")?s.split("T")[0]:s;document.getElementById("sch-start-date").value=t(e.start_date),document.getElementById("sch-end-date").value=t(e.end_date),document.getElementById("sch-slots").value=e.slots,document.getElementById("sch-status").value=e.status||"Đang mở",document.getElementById("sch-group-label").value=e.group_label||"",document.getElementById("sch-zalo-link").value=e.zalo_link||""}else l.textContent="Thêm Lịch Khởi Hành",S.reset(),document.getElementById("sch-edit-id").value="",document.getElementById("sch-zalo-link").value="";g.classList.remove("hidden"),setTimeout(()=>{g.classList.add("opacity-100"),y.classList.remove("scale-95"),y.classList.add("scale-100")},10)},f=()=>{g.classList.remove("opacity-100"),y.classList.remove("scale-100"),y.classList.add("scale-95"),setTimeout(()=>{g.classList.add("hidden")},200)};document.getElementById("addScheduleBtn").addEventListener("click",()=>M()),document.getElementById("closeScheduleModalBtn").addEventListener("click",f),document.getElementById("cancelScheduleBtn").addEventListener("click",f),g.addEventListener("click",e=>{e.target===g&&f()});const C=async()=>{try{const e=await fetch($);if(!e.ok)throw new Error("Failed to load tours");const l=await e.json(),t=document.getElementById("sch-tour-name");t.innerHTML='<option value="">-- Chọn Tour --</option>',l.forEach(a=>{t.innerHTML+=`<option value="${a.name}">${a.name}</option>`});const s=document.getElementById("tourFilterList");let r=`
                <label class="flex items-center gap-3 cursor-pointer group">
                    <input type="radio" name="tourFilter" value="" class="text-csr-orange focus:ring-csr-orange border-gray-200 bg-gray-100" checked>
                    <span class="text-gray-500 group-hover:text-gray-900 transition-colors text-sm">Tất cả Tuyến</span>
                </label>
            `;l.forEach(a=>{r+=`
                ...
                `}),s.innerHTML=r,s.querySelectorAll('input[name="tourFilter"]').forEach(a=>{a.addEventListener("change",i=>{v=i.target.value,E()})}),document.querySelectorAll('input[name="timeFilter"]').forEach(a=>{a.addEventListener("change",i=>{h=i.target.value,document.querySelectorAll('input[name="timeFilter"]').forEach(o=>{const n=o.nextElementSibling;o.checked?(n.classList.add("text-gray-700","font-medium"),n.classList.remove("text-gray-500")):(n.classList.remove("text-gray-700","font-medium"),n.classList.add("text-gray-500"))}),E()})})}catch(e){console.error("Error loading tours:",e)}},L=async()=>{p.innerHTML='<div class="text-center py-12 text-gray-400">Đang tải lịch trình...</div>';try{const e=await fetch(k);if(!e.ok)throw new Error("Failed to fetch schedules");w=await e.json(),E()}catch(e){console.error("Error loading schedules:",e),p.innerHTML='<div class="text-center py-12 text-red-400 font-medium">Lỗi kết nối server.</div>'}},E=()=>{let e=w;v&&(e=e.filter(t=>t.tour_name===v));const l=new Date;if(l.setHours(0,0,0,0),e=e.filter(t=>{const s=new Date(t.start_date);s.setHours(0,0,0,0);const r=s<l;return h==="upcoming"?!r:h==="past"?r:!0}),e.sort((t,s)=>{const r=new Date(t.start_date),a=new Date(s.start_date);return h==="upcoming"?r-a:a-r}),e.length===0){p.innerHTML='<div class="text-center py-12 text-gray-400 italic">Chưa có lịch trình nào'+(v?" cho tuyến này":"")+(h==="past"?" trong quá khứ":" sắp tới")+".</div>";return}p.innerHTML=e.map(t=>{const s=new Date(t.start_date),r=new Date(t.end_date),a=s.toLocaleDateString("vi-VN");r.toLocaleDateString("vi-VN");const i=`${s.getDate().toString().padStart(2,"0")}/${(s.getMonth()+1).toString().padStart(2,"0")}`,o=parseInt(t.booked_count)||0,n=parseInt(t.slots)||0,d=Math.max(0,n-o),u=n>0?Math.min(100,Math.round(o/n*100)):0,b=t.group_label?`<span class="ml-2 text-[10px] font-bold uppercase px-2 py-0.5 rounded-md border bg-blue-50 text-blue-600 border-blue-200">${t.group_label}</span>`:"",T=t.zalo_link?'<span class="ml-1 text-[10px] font-bold uppercase px-2 py-0.5 rounded-md border bg-blue-600 text-white border-blue-700 flex items-center gap-1"><svg class="w-2.5 h-2.5" fill="currentColor" viewBox="0 0 24 24"><path d="M12.14 1c-6.15 0-11.14 4.58-11.14 10.21 0 3.2 1.6 6.05 4.12 7.91-.12.44-.43 1.6-.43 1.6s-.1.38.16.53c.26.15.54-.04.54-.04l1.88-1.29c.6.16 1.23.25 1.88.25 6.15 0 11.14-4.58 11.14-10.21S18.29 1 12.14 1z"/></svg> ZALO</span>':"";let c=t.status||"Đang mở";d<=0?c="Hết chỗ":d<=3&&(c="Sắp hết"),t.status==="Hết chỗ"&&(c="Hết chỗ");const _=c==="Đang mở"?"text-green-600 bg-green-50 border-green-200":c==="Sắp hết"?"text-orange-600 bg-orange-50 border-orange-200":"text-red-500 bg-red-50 border-red-200",j=c==="Đang mở"?"bg-green-500":c==="Sắp hết"?"bg-orange-500":"bg-red-500";return`
                    ...
                    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                        <div class="flex items-center gap-3 md:gap-4 flex-1 min-w-0">
                            <div class="bg-csr-orange/10 text-csr-orange font-bold rounded-xl p-2 md:p-3 text-center min-w-[64px] md:min-w-[72px] shrink-0">
                                ...
                                <div class="text-lg md:text-xl font-black">${i}</div>
                            </div>
                            <div class="flex-1 min-w-0">
                                ...
                                <div class="flex items-center gap-3 md:gap-4 text-xs md:text-sm text-gray-500 flex-wrap">
                                    ...
                                    <span class="flex items-center gap-1">
                                        ...
                                        <strong class="text-csr-orange">${o}</strong> / ${n}
                                    </span>
                                    <span class="text-[9px] font-bold uppercase px-1.5 py-0.5 rounded-md border ${_}">${c}</span>
                                </div>
                                ...
                                <div class="mt-2 w-full bg-gray-100 rounded-full h-1.5 overflow-hidden max-w-xs hidden md:block">
                                    <div class="${j} h-1.5 rounded-full transition-all duration-500" style="width: ${u}%"></div>
                                </div>
                            </div>
                        </div>
            ...
            `}).join("")},m=document.getElementById("guidePickerModal"),x=document.getElementById("guidePickerList");let B=null;const I=async(e,l)=>{B=e,document.getElementById("guidePickerSubtitle").textContent=`Tour: ${l}`,m.classList.remove("hidden"),x.innerHTML='<div class="text-center py-8 text-gray-400">Đang tải...</div>';try{const[t,s]=await Promise.all([fetch("/api/guides"),fetch(`/api/guides?schedule_id=${e}`)]),r=await t.json(),a=await s.json(),i=new Set(a.map(n=>n.id)),o={"Hướng Dẫn Viên":"bg-blue-100 text-blue-700",Leader:"bg-purple-100 text-purple-700",Photo:"bg-pink-100 text-pink-700",Guider:"bg-cyan-100 text-cyan-700","Hậu Cần":"bg-amber-100 text-amber-700","Học Việc":"bg-gray-100 text-gray-600"};if(r.length===0){x.innerHTML='<div class="text-center py-8 text-gray-400"><div class="text-3xl mb-2">👥</div><p>Chưa có nhân sự nào.</p><a href="/admin/guides" data-link class="text-indigo-600 text-sm font-medium hover:underline">+ Thêm nhân sự mới</a></div>';return}x.innerHTML=r.map(n=>{const d=i.has(n.id),u=o[n.role]||"bg-gray-100 text-gray-600",b=(n.full_name||"X").split(" ").slice(-2).map(T=>T[0]).join("").toUpperCase();return`
                <div class="flex items-center gap-3 p-3 rounded-xl border transition-all ${d?"border-indigo-200 bg-indigo-50":"border-gray-100 hover:border-gray-200 hover:bg-gray-50"}">
                    <div class="w-9 h-9 rounded-full bg-gradient-to-br from-csr-orange/20 to-orange-100 flex items-center justify-center text-csr-orange font-black text-xs shrink-0">${b}</div>
                    <div class="flex-1 min-w-0">
                        <div class="font-bold text-sm text-gray-900 truncate">${n.full_name}</div>
                        <span class="text-[10px] font-bold px-2 py-0.5 rounded-full ${u}">${n.role}</span>
                    </div>
                    <button class="guide-toggle-btn px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${d?"bg-red-100 text-red-600 hover:bg-red-200":"bg-indigo-600 text-white hover:bg-indigo-700"}"
                        data-guide-id="${n.id}" data-assigned="${d}">
                        ${d?"✕ Bỏ":"+ Thêm"}
                    </button>
                </div>`}).join(""),x.querySelectorAll(".guide-toggle-btn").forEach(n=>{n.addEventListener("click",async()=>{const d=parseInt(n.getAttribute("data-guide-id")),u=n.getAttribute("data-assigned")==="true";n.disabled=!0,n.textContent="...",await fetch("/api/guides",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:u?"unassign":"assign",schedule_id:B,guide_id:d})}),I(B,document.getElementById("guidePickerSubtitle").textContent.replace("Tour: ",""))})})}catch(t){x.innerHTML=`<div class="text-center py-8 text-red-400">Lỗi tải dữ liệu: ${t.message}</div>`}};document.getElementById("closeGuidePickerBtn").addEventListener("click",()=>m.classList.add("hidden")),m.addEventListener("click",e=>{e.target===m&&m.classList.add("hidden")});const H=async e=>{const l=e.target.closest(".sch-edit-btn"),t=e.target.closest(".sch-delete-btn"),s=e.target.closest(".sch-review-btn"),r=e.target.closest(".sch-details-btn"),a=e.target.closest(".sch-guides-btn");if(l){const i=l.getAttribute("data-id"),o=w.find(n=>n.id==i);o&&M(o)}if(a){const i=a.getAttribute("data-id"),o=a.getAttribute("data-tour");I(parseInt(i),o)}if(t){const i=t.getAttribute("data-id");if(confirm("Bạn có chắc chắn muốn xóa lịch trình này?"))try{if((await fetch(`${k}?id=${i}`,{method:"DELETE"})).ok)L();else throw new Error("Delete failed")}catch(o){alert("Lỗi khi xóa: "+o.message)}}if(s){const i=s.getAttribute("data-id"),o=`${window.location.origin}/review?schedule_id=${i}`;navigator.clipboard.writeText(o).then(()=>{const n=s.innerHTML;s.innerHTML="✅ Đã copy!",setTimeout(()=>s.innerHTML=n,2e3)}).catch(()=>{alert(`Không thể copy. Link:
`+o)})}if(r){const i=r.getAttribute("data-id"),o=r.getAttribute("data-tour"),n=r.getAttribute("data-date"),d=n&&n.includes("T")?n.split("T")[0]:n,u=r.getAttribute("data-group-label")||"",b=`/admin/roster?tour=${encodeURIComponent(o)}&date=${encodeURIComponent(d)}&scheduleId=${i}&groupLabel=${encodeURIComponent(u)}`;history.pushState(null,null,b),window.dispatchEvent(new Event("popstate"))}};S.addEventListener("submit",async e=>{e.preventDefault();const l=e.target.querySelector('button[type="submit"]'),t=l.textContent;l.textContent="Đang lưu...",l.disabled=!0;const s=document.getElementById("sch-edit-id").value,r={id:s?parseInt(s):null,tour_name:document.getElementById("sch-tour-name").value,start_date:document.getElementById("sch-start-date").value,end_date:document.getElementById("sch-end-date").value,slots:parseInt(document.getElementById("sch-slots").value),status:document.getElementById("sch-status").value,group_label:document.getElementById("sch-group-label").value.trim()||null,zalo_link:document.getElementById("sch-zalo-link").value.trim()||null};try{if(!(await fetch(k,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(r)})).ok)throw new Error("Failed to save schedule");f(),L(),alert(s?"✅ Cập nhật lịch trình thành công!":"✅ Thêm lịch trình mới thành công! Form đặt chỗ trên website sẽ tự cập nhật trong 30 giây.")}catch(a){alert("❌ Lỗi: "+a.message)}finally{l.textContent=t,l.disabled=!1}}),p.addEventListener("click",H),C(),L()};export{N as afterRender,z as render};
