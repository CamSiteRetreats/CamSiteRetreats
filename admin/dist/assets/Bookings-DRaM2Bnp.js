import{S as $e,H as _e}from"./Header-CARqqxjt.js";console.log("Bookings V3 - Cache Busted!");const De=()=>`
    ...
    `,Ne=()=>{let H=[],B="consult",D=[],te=[],J=[];const Te=async()=>{try{const[n,e,o]=await Promise.all([fetch("/api/tours"),fetch("/api/schedules"),fetch("/api/users")]);D=n.ok?await n.json():[],te=e.ok?await e.json():[],J=o.ok?await o.json():[],X("addTourName"),X("edit-tour"),X("wl-tour"),Ie("edit-sale")}catch(n){console.error("Lỗi tải tours/schedules:",n)}},Ie=n=>{const e=document.getElementById(n);e&&(e.innerHTML='<option value="">-- Website --</option>',J.forEach(o=>{const a=document.createElement("option");a.value=o.id,a.textContent=`${o.full_name||o.fullName} (${o.role})`,e.appendChild(a)}))},X=n=>{const e=document.getElementById(n);if(!e)return;const o=e.value,a=e.options[0].outerHTML;e.innerHTML=a,D.filter(r=>r.is_visible!==!1).forEach(r=>{const u=document.createElement("option");u.value=r.name,u.textContent=r.name,r.name===o&&(u.selected=!0),e.appendChild(u)})},P=(n,e,o,a)=>{const r=document.getElementById(n);if(!r)return;if(r.innerHTML='<option value="">-- Chọn Lịch --</option>',!e){r.innerHTML='<option value="">-- Chọn Tour trước --</option>';return}const u=te.filter(s=>s.tour_name===e||e.includes(s.tour_name)||s.tour_name.includes(e));if(u.length===0){r.innerHTML='<option value="">Chưa có lịch cho tour này</option>';return}u.sort((s,c)=>new Date(s.start_date)-new Date(c.start_date)).forEach(s=>{const c=parseInt(s.booked_count)||0,h=(s.slots||0)-c,x=new Date(s.start_date),p=s.end_date?new Date(s.end_date):null,T=S=>`${S.getDate().toString().padStart(2,"0")}/${(S.getMonth()+1).toString().padStart(2,"0")}`;let I=T(x);p&&(I+=" - "+T(p)),s.group_label&&(I+=` · ${s.group_label}`),I+=` (${h>=0?h:0} chỗ trống)`;const y=document.createElement("option"),m=`${x.getDate().toString().padStart(2,"0")}/${(x.getMonth()+1).toString().padStart(2,"0")}/${x.getFullYear()}`;y.value=`${s.id}::${m}`,y.dataset.scheduleId=s.id,y.dataset.date=m,y.textContent=I,h<=0&&(y.disabled=!0,y.textContent=I.replace("chỗ trống","HẾT CHỖ"));let w=o||"";if(w.includes("-")&&w.length>=10){const S=w.substring(0,10).split("-");S.length===3&&(w=`${S[2]}/${S[1]}/${S[0]}`)}(a&&String(s.id)===String(a)||!a&&w&&m===w)&&(y.selected=!0),r.appendChild(y)})},ne=document.getElementById("addTourName");ne&&ne.addEventListener("change",n=>{const e=n.target.value;P("addTourDate",e);const o=D.find(r=>r.name===e),a=document.getElementById("addTourPrice");if(o&&a){const r=parseInt(o.price)||0;a.value=r;const u=document.getElementById("addDiscount"),s=document.getElementById("addTotalPriceDisplay"),c=parseInt(u?u.value:0)||0,h=r-c;s&&(s.textContent=(h>0?h:0).toLocaleString("vi-VN")+"đ")}});const oe=document.getElementById("edit-tour");oe&&oe.addEventListener("change",n=>{const e=n.target.value;P("edit-date",e);const o=D.find(r=>r.name===e),a=document.getElementById("edit-total");o&&a&&(parseInt(a.value)===0||!a.value)&&(a.value=parseInt(o.price)||0,typeof window.updateEditRemaining=="function"&&window.updateEditRemaining())});const ae=document.getElementById("wl-tour");ae&&ae.addEventListener("change",n=>{const e=n.target.value;P("wl-date",e)}),Te();const R=()=>{const n=document.getElementById("bookingsTableHead"),e=document.getElementById("bookingsTableBody");if(!e||!n)return;const o=B==="upcoming"||B==="ready"||B==="completed";o?n.innerHTML=`
            ...
            `;const a=document.getElementById("filterSearch")?document.getElementById("filterSearch").value.toLowerCase():"",r=document.getElementById("filterTour")?document.getElementById("filterTour").value:"",u=document.getElementById("filterDate")?document.getElementById("filterDate").value:"",s=document.getElementById("filterSale")?document.getElementById("filterSale").value:"",c=document.getElementById("filterStatus")?document.getElementById("filterStatus").value:"";let h=H.filter(t=>{let d=!1;const i=parseInt(t.total_price)>0&&parseInt(t.total_price)===parseInt(t.deposit),l=b=>{if(!b)return!1;try{const E=b.split("/");if(E.length===3){const C=new Date(`${E[2]}-${E[1]}-${E[0]}T00:00:00`),v=new Date;return v.setHours(0,0,0,0),C.getTime()<v.getTime()}else if(b.includes("-")){const C=new Date(`${b}T00:00:00`),v=new Date;return v.setHours(0,0,0,0),C.getTime()<v.getTime()}}catch{}return!1},g=t.status||"",f=!l(t.date);return B==="consult"?d=g==="Chờ tư vấn"&&f:B==="pending"?d=(g==="Chờ cọc"||g==="Chờ xác nhận cọc"||g==="")&&f:B==="upcoming"?d=(g==="Đã cọc"||g==="Đã cọc (Chờ đi)")&&!i&&f:B==="ready"?d=(i||g==="Hoàn thành")&&f&&g!=="Đã hủy"&&g!=="Bảo lưu":B==="completed"&&(d=!f||g==="Đã đi"||g==="Đã hủy"||g==="Bảo lưu"),!(!d||r&&t.tour!==r||u&&U(t.date)!==u||s&&s!=="null"&&t.sale_name!==s||s==="null"&&t.sale_name&&t.sale_name!=="Website"||a&&!`${t.name||""} ${t.phone||""} ${t.customer_id||""} ${t.id||""} csr${t.id||""} #csr${t.id||""}`.toLowerCase().includes(a)||c&&t.status!==c)});const x=document.getElementById("statTitle1"),p=document.getElementById("statTitle2"),T=document.getElementById("statTitle3"),I=document.getElementById("statTotalCustomers"),y=document.getElementById("statTotalRevenue"),m=document.getElementById("statTotalCollected");let w=h.length;if(B==="consult"){const t=document.getElementById("addWaitingBtn"),d=document.getElementById("addBookingBtn");t&&t.classList.remove("hidden"),d&&d.classList.add("hidden"),x&&(x.textContent="Tổng Đăng Ký Chờ Lịch"),p&&(p.textContent="Chưa Được Liên Hệ"),T&&(T.textContent="Đã Có Sale Phụ Trách");let i=0,l=0;h.forEach(g=>{!g.sale_id||g.sale_id==="null"||!g.sale_name||g.sale_name==="Website"||g.sale_name==="null"?i++:l++}),I&&(I.textContent=w),y&&(y.textContent=i+" Người"),m&&(m.textContent=l+" Người")}else if(B==="pending"){const t=document.getElementById("addWaitingBtn"),d=document.getElementById("addBookingBtn");t&&t.classList.add("hidden"),d&&d.classList.remove("hidden"),x&&(x.textContent="Tổng Khách (Chờ Cọc)"),p&&(p.textContent="Khách Chưa Tư Vấn (Mới)"),T&&(T.textContent="Đã Có Sale Giữ Chỗ");let i=0,l=0;h.forEach(g=>{!g.sale_id||g.sale_id==="null"||!g.sale_name||g.sale_name==="Website"||g.sale_name==="null"?i++:l++}),I&&(I.textContent=w),y&&(y.textContent=i+" Đơn"),m&&(m.textContent=l+" Đơn")}else if(B==="upcoming"){x&&(x.textContent="Tổng Số Khách"),p&&(p.textContent="Chưa Xác Nhận Cọc"),T&&(T.textContent="Chờ Thanh Toán Còn Lại");let t=0,d=0;h.forEach(i=>{i.status==="Chờ xác nhận cọc"?t++:parseInt(i.deposit)>0&&parseInt(i.total_price)>parseInt(i.deposit)&&d++}),I&&(I.textContent=w),y&&(y.textContent=t+" Khách"),m&&(m.textContent=d+" Khách")}else if(B==="ready"){x&&(x.textContent="Khách Sẵn Sàng (Full)"),p&&(p.textContent="Tổng Doanh Thu Tab"),T&&(T.textContent="Thực Thu (Full Tận Nơi)");let t=0,d=0;h.forEach(i=>{t+=parseInt(i.total_price)||0,d+=parseInt(i.deposit)||0}),I&&(I.textContent=w),y&&(y.textContent=t>0?t.toLocaleString("vi-VN")+"đ":"0đ"),m&&(m.textContent=d>0?d.toLocaleString("vi-VN")+"đ":"0đ")}else{x&&(x.textContent="Khách Đã Xong (Lịch Sử)"),p&&(p.textContent="Tổng Doanh Thu Tab"),T&&(T.textContent="Thực Thu (Đã Đóng)");let t=0,d=0;h.forEach(i=>{t+=parseInt(i.total_price)||0,d+=parseInt(i.deposit)||0}),I&&(I.textContent=w),y&&(y.textContent=t>0?t.toLocaleString("vi-VN")+"đ":"0đ"),m&&(m.textContent=d>0?d.toLocaleString("vi-VN")+"đ":"0đ")}const S=o?8:4;if(!h||h.length===0){let t=B==="consult"?"Chưa có khách hàng tư vấn nào.":B==="pending"?"Chưa có Khách chờ cọc.":B==="upcoming"?"Chưa có Khách nào Đã Cọc.":B==="ready"?"Chưa có khách sẵn sàng.":"Danh sách rỗng.";e.innerHTML=`<tr><td colspan="${S}" class="p-8 text-center text-gray-500">${t}</td></tr>`;return}e.innerHTML=h.map(t=>{const d=B==="upcoming"||B==="ready"?"bg-green-50/20 hover:bg-green-50 transition-colors":"hover:bg-gray-100 transition-colors";if(o){let i=parseInt(t.total_price)||0;if(i===0&&t.tour&&D&&D.length>0){const k=D.find(N=>N.name&&t.tour&&N.name.toLowerCase().trim()===t.tour.toLowerCase().trim());k&&parseInt(k.price)>0&&(i=parseInt(k.price))}const l=parseInt(t.deposit)||0,g=i-l,f=i>0?i.toLocaleString("vi-VN")+"đ":"Chưa định giá",b=l>0?l.toLocaleString("vi-VN")+"đ":"0đ",E=g>0?g.toLocaleString("vi-VN")+"đ":g===0&&i>0?"Đã thu trọn":"-";let C="";return t.status==="Đã hủy"?C='<span class="bg-red-50 text-red-500 border border-red-200 px-2 py-0.5 rounded text-xs block w-full text-center font-bold">❌ Đã hủy</span>':t.status==="Bảo lưu"?C='<span class="bg-gray-100 text-gray-500 border border-gray-300 px-2 py-0.5 rounded text-xs block w-full text-center font-bold">⏸️ Bảo lưu</span>':t.status==="Hoàn tất phí"?C='<span class="bg-emerald-100 text-emerald-700 border border-emerald-200 px-2 py-0.5 rounded text-xs font-bold block w-full text-center">Hoàn Tất Phí</span>':t.status==="Hoàn thành"||t.status==="Đã đi"?C='<span class="bg-violet-100 text-violet-700 border border-violet-200 px-2 py-0.5 rounded text-xs block w-full text-center font-bold">Hoàn Thành</span>':t.status==="Chờ xác nhận cọc"?C=`<button class="action-btn confirm-deposit-btn bg-csr-orange text-white px-3 py-1.5 rounded text-xs font-bold shadow-sm hover:bg-[#d65503] w-full" data-id="${t.id}">Xác nhận cọc</button>`:l>0&&g===0?C='<span class="bg-green-100 text-green-700 border border-green-200 px-2 py-0.5 rounded text-xs font-medium block w-full text-center">Hoàn tất phí</span>':C='<span class="bg-blue-100 text-blue-700 border border-blue-200 px-2 py-0.5 rounded text-xs font-medium block w-full text-center">Đã Cọc</span>',`
                <tr class="${d} cursor-pointer row-clickable block md:table-row p-4 md:p-0 mb-4 md:mb-0 glass-panel md:glass-none border-l-4 border-green-500 md:border-none relative" data-id="${t.id}">
                    ...
                    <td class="p-4 align-top text-center hidden md:table-cell">${C}</td>
                ...
                `}else{const i=t.name?t.name.substring(0,2).toUpperCase():"KH",l=B==="consult",g=!t.sale_id||t.sale_id==="null"||!t.sale_name||t.sale_name==="null";let f="";l&&g?f=`<button class="action-btn claim-btn bg-csr-orange hover:bg-[#d65503] text-white px-3 py-1.5 rounded text-xs font-bold shadow-sm transition-colors" data-id="${t.id}">Nhận khách</button>`:f=`<span class="text-sm text-gray-500">${t.sale_name||"Website"}</span>`;const b=`
                ...
                <tr class="${d} cursor-pointer row-clickable block md:table-row p-4 md:p-0 mb-4 md:mb-0 glass-panel md:glass-none border-l-4 border-csr-orange md:border-none relative" data-id="${t.id}">
                    <td class="p-0 md:p-4 align-top block md:table-cell">
                        <div class="flex items-center gap-3">
                            <div class="w-12 h-12 md:w-10 md:h-10 rounded-full bg-csr-orange/20 text-csr-orange flex items-center justify-center font-bold text-base md:text-sm shrink-0">${i}</div>
                            <div class="min-w-0">
    ...
    `}}).join(""),document.querySelectorAll(".claim-btn").forEach(t=>{t.addEventListener("click",async d=>{d.stopPropagation();const i=t.getAttribute("data-id"),l=localStorage.getItem("csr_user");let g=null,f="Admin";if(l){const b=JSON.parse(l);g=b.id,f=b.fullName||b.full_name||"Admin"}t.textContent="Đang xử lý...",t.disabled=!0;try{if((await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:i,sale_id:g,sale_name:f})})).ok){const E=H.find(C=>C.id==i);E&&(E.sale_id=g,E.sale_name=f),R()}else alert("Lỗi khi nhận khách. Vui lòng thử lại."),t.textContent="Nhận khách",t.disabled=!1}catch(b){console.error("Lỗi nhận khách:",b),t.textContent="Nhận khách",t.disabled=!1}})})},Ee=()=>{const n=new Set,e=new Set,o=new Set;H.forEach(s=>{if(s.tour&&n.add(s.tour),s.date){const c=U(s.date);c&&e.add(c)}s.sale_name&&s.sale_name!=="Website"&&s.sale_name!=="null"&&o.add(s.sale_name.trim())});const a=document.getElementById("filterTour"),r=document.getElementById("filterDate"),u=document.getElementById("filterSale");if(a){const s=a.value;a.innerHTML='<option value="">Tất cả Tour</option>',[...n].sort().forEach(c=>a.innerHTML+=`<option value="${c}">${c}</option>`),a.value=s}if(r){const s=r.value;r.innerHTML='<option value="">Tất cả Ngày</option>',[...e].sort().forEach(c=>r.innerHTML+=`<option value="${c}">${c}</option>`),r.value=s}if(u){const s=u.value;u.innerHTML='<option value="">Tất cả Sale</option>',u.innerHTML+='<option value="null">Nguồn Website</option>',[...o].sort().forEach(c=>{u.innerHTML+=`<option value="${c}">${c}</option>`}),u.value=s}};function U(n){if(!n)return"";if(/^\d{1,2}\/\d{1,2}\s*-\s*\d{1,2}\/\d{1,2}$/.test(n))return n.replace(/\s+/g,"");try{if(/^\d{4}-\d{2}-\d{2}$/.test(n)){const[e,o,a]=n.split("-"),r=`${parseInt(a)}/${parseInt(o)}`;return`${r}-${r}`}if(/^\d{1,2}\/\d{1,2}\/\d{4}$/.test(n)){const[e,o,a]=n.split("/"),r=`${parseInt(e)}/${parseInt(o)}`;return`${r}-${r}`}if(n.includes("-"))return n.split("-").map(o=>{const a=o.trim();if(/\/\d{4}$/.test(a)){const[r,u]=a.split("/");return`${parseInt(r)}/${parseInt(u)}`}return a}).join("-")}catch{console.warn("Date normalization failed for:",n)}return n}const M=async()=>{try{H=await(await fetch("/api/bookings")).json(),H.sort((e,o)=>parseInt(o.id)-parseInt(e.id)),Ee(),R()}catch(n){console.error("Lỗi parse list đơn hàng:",n);const e=document.getElementById("bookingsTableBody");e&&(e.innerHTML='<tr><td colspan="4" class="p-8 text-center text-red-500">Lỗi lấy dữ liệu từ Server.</td></tr>')}};M(),["filterSearch","filterTour","filterDate","filterSale","filterStatus"].forEach(n=>{const e=document.getElementById(n);e&&(n==="filterSearch"?e.addEventListener("input",R):e.addEventListener("change",R))});const se=document.getElementById("exportExcelBtn");se&&se.addEventListener("click",()=>{const n=document.getElementById("filterSearch")?document.getElementById("filterSearch").value.toLowerCase():"",e=document.getElementById("filterTour")?document.getElementById("filterTour").value:"",o=document.getElementById("filterDate")?document.getElementById("filterDate").value:"",a=document.getElementById("filterSale")?document.getElementById("filterSale").value:"";let r=H.filter(p=>{let T=!1;const I=parseInt(p.total_price)>0&&parseInt(p.total_price)===parseInt(p.deposit),y=w=>{if(!w)return!1;try{const S=w.split("/");if(S.length===3){const t=new Date(`${S[2]}-${S[1]}-${S[0]}T00:00:00`),d=new Date;return d.setHours(0,0,0,0),t.getTime()<d.getTime()}else if(w.includes("-")){const t=new Date(`${w}T00:00:00`),d=new Date;return d.setHours(0,0,0,0),t.getTime()<d.getTime()}}catch{}return!1},m=(p.status==="Hoàn thành"||p.status==="Đã đi"||I)&&y(p.date);return B==="consult"?T=p.status==="Chờ tư vấn"&&!m:B==="pending"?T=(!p.status||p.status==="Chờ cọc")&&!m:B==="upcoming"?T=p.status&&p.status!=="Chờ tư vấn"&&p.status!=="Chờ cọc"&&!I&&p.status!=="Hoàn thành"&&!m:B==="ready"?T=(I||p.status==="Hoàn thành")&&!m:B==="completed"&&(T=m),!(!T||e&&p.tour!==e||o&&U(p.date)!==o||a&&a!=="null"&&p.sale_name!==a||a==="null"&&p.sale_name&&p.sale_name!=="Website"||n&&!`${p.name||""} ${p.phone||""} ${p.customer_id||""} ${p.id||""} csr${p.id||""} #csr${p.id||""}`.toLowerCase().includes(n))});if(r.length===0){alert("Danh sách rỗng. Không có dữ liệu để xuất.");return}let u=`
        ...
        <div class="space-y-4 pb-2">
            ...
            <!-- ① Thông tin Tour -->
            <div class="bg-gradient-to-br from-orange-50 to-amber-50 border border-orange-100 rounded-2xl p-4">
                ${w("🗺️","Thông tin Tour","bg-orange-100 text-orange-600")}
                <div class="grid grid-cols-2 gap-x-4">
                    ...
                    ${m("Trạng Thái",`<span class="inline-block px-2.5 py-1 rounded-full text-xs font-bold border ${y}">${T||"Chưa rõ"}</span>`)}
            ...
            <!-- ② Thông tin cá nhân -->
            <div class="bg-white border border-gray-200 rounded-2xl p-4">
                ${w("👤","Thông tin Cá nhân","bg-blue-50 text-blue-600")}
            ...
            <!-- ③ Thông tin Hậu cần -->
            <div class="bg-white border border-gray-200 rounded-2xl p-4">
                ${w("🎒","Thông tin Hậu cần","bg-purple-50 text-purple-600")}
            ...
            <!-- ④ Dịch vụ đi kèm -->
            <div class="bg-white border border-gray-200 rounded-2xl p-4">
                ${w("✨","Dịch Vụ Đi Kèm","bg-green-50 text-green-600")}
                ${h.length>0?h.map(t=>`
            ...
            <div class="bg-gradient-to-br from-csr-orange to-orange-600 rounded-2xl p-4 text-white shadow-lg shadow-orange-500/20">
                ${w("💰","Giá Chi Tiết","bg-white/20 text-white")}
                <div class="space-y-2 text-sm">
                    ...
                    <div class="flex justify-between items-center py-3 bg-white text-gray-900 rounded-xl px-4 mt-3 shadow-inner">
                        ...
                        <span class="font-black text-2xl ${c>0?"text-red-600":"text-green-600"}">
                            ${c>0?o(c):"✅ ĐÃ THU ĐỦ"}
                        </span>
                    </div>
                </div>
            ...
            ${(()=>{const t=e.phone;if(!t)return"";const d=H.filter(v=>v.phone===t&&!v.status?.includes("hủy")&&!v.status?.includes("Hủy")).sort((v,k)=>new Date(k.created_at||0)-new Date(v.created_at||0)),i=d.length,l=d.reduce((v,k)=>v+(parseInt(k.total_price)||0),0);let g,f,b;i>=5?(g="KHÁCH VIP",f="bg-purple-100 text-purple-700 border-purple-300",b="👑"):i>=2?(g="KHÁCH THÂN THIẾT",f="bg-green-100 text-green-700 border-green-300",b="🌟"):(g="KHÁCH MỚI",f="bg-gray-100 text-gray-500 border-gray-300",b="👋");const E=d.filter(v=>v.id!=n),C=E.length>0?E.slice(0,5).map((v,k)=>{const N=v.status?.includes("Hoàn")?"bg-green-100 text-green-600":v.status?.includes("cọc")||v.status?.includes("Cọc")?"bg-blue-100 text-blue-600":"bg-gray-100 text-gray-500";return`<div class="flex items-center gap-3 py-2 border-b border-gray-100 last:border-none">
                            <div class="w-7 h-7 rounded-full bg-orange-100 flex items-center justify-center text-csr-orange text-xs font-bold shrink-0">${k+1}</div>
                    ...
                    ${w("🏅","Hành Trình Cùng Cam Site","bg-slate-100 text-slate-600")}
                    ...
                    <div class="flex items-center gap-2 mb-3 text-sm text-gray-500">
                        ...
                        <span class="font-black text-gray-800 text-base">${i}</span>
            ...
            <div class="bg-blue-50 border border-blue-100 rounded-xl px-4 py-3 flex items-center gap-3">
                ...
                <div>
        ...
        </div>`;document.getElementById("detailContentBlock").innerHTML=S,document.getElementById("detailModal").classList.remove("hidden"),setTimeout(()=>{document.getElementById("detailModal").classList.add("opacity-100");const t=document.getElementById("detailModalContent");t&&(t.classList.remove("scale-95","translate-y-4"),t.classList.add("scale-100","translate-y-0"))},10)},window.actionEdit=async n=>{document.getElementById("rowActionModal").classList.add("hidden");const e=H.find(t=>t.id==n);if(!e)return;document.getElementById("edit-id").value=n,document.getElementById("edit-name").value=e.name||"",document.getElementById("edit-phone").value=e.phone||"",document.getElementById("edit-medal-name").value=e.medal_name||"",document.getElementById("edit-tour").value=e.tour||"",typeof P=="function"&&P("edit-date",e.tour||"",e.date||"",e.schedule_id||null),(t=>{let d=t.dob||"";if(typeof d=="string"&&d.includes("/")){const g=d.split("/");g.length===3&&(d=`${g[2]}-${g[1].padStart(2,"0")}-${g[0].padStart(2,"0")}`)}document.getElementById("edit-dob").value=d,document.getElementById("edit-gender").value=t.gender||"Khác",document.getElementById("edit-allergy").value=t.allergy||t.medical_notes||"",document.getElementById("edit-address").value=t.address||"",document.getElementById("edit-diet").value=t.diet||t.dietary||"",document.getElementById("edit-trekking-pole").value=t.trekking_pole||"Không";const i=document.getElementById("edit-id-card");i&&(i.value=t.id_card||t.cccd||"");const l=document.getElementById("edit-pickup-point");l&&(l.value=t.pickup_point||"")})(e),document.getElementById("edit-status").value=e.status||"Chờ xác nhận cọc",document.getElementById("edit-commitments").checked=!!e.commitments,document.getElementById("edit-special").value=e.special||"";try{const t=localStorage.getItem("csr_user");if(t&&JSON.parse(t).role==="admin"){const i=document.getElementById("edit-sale-container");i&&i.classList.remove("hidden");const l=document.getElementById("edit-sale");l&&(l.value=e.sale_id||"")}}catch{}if((!e.dob||!e.address||!e.gender||e.gender==="Khác")&&(e.customer_id||e.phone))try{const t=e.customer_id||e.phone,d=await fetch(`/api/admin_customers?action=search&keyword=${encodeURIComponent(t)}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({keyword:t})}),i=await d.json();if(d.ok&&i.success){const l=i.data,g=document.getElementById("edit-dob"),f=document.getElementById("edit-address"),b=document.getElementById("edit-allergy"),E=document.getElementById("edit-diet"),C=document.getElementById("edit-gender"),v=document.getElementById("edit-medal-name");g&&!g.value&&(g.value=l.dob||""),f&&!f.value&&(f.value=l.address||""),b&&!b.value&&(b.value=l.medical_notes||""),E&&!E.value&&(E.value=l.dietary||""),C&&(C.value==="Khác"||!C.value)&&(C.value=l.gender||"Khác"),v&&!v.value&&(v.value=l.medal_name||"");const k=document.getElementById("edit-id-card");k&&!k.value&&(k.value=l.cccd||"")}}catch(t){console.error("Pull CRM error:",t)}let a=0;try{const t=typeof e.services_booked=="string"?JSON.parse(e.services_booked):e.services_booked||[];Array.isArray(t)&&t.forEach(d=>a+=parseInt(d.price)||0)}catch{}let u=(parseInt(e.total_price)||0)+(parseInt(e.discount)||0)-a;if(u===0&&e.tour){const t=D.find(d=>d.name===e.tour);t&&parseInt(t.price)>0&&(u=parseInt(t.price))}document.getElementById("edit-total").value=u,document.getElementById("edit-discount").value=e.discount||0,document.getElementById("edit-deposit").value=e.deposit||0;const s=parseInt(e.deposit_required);document.getElementById("edit-deposit-required").value=isNaN(s)?1e6:s,window.updateEditRemaining();const c=document.getElementById("servicesContainer"),h=document.getElementById("emptyServicesMsg"),x=document.getElementById("servicesTotalRow"),p=document.getElementById("presetServicesBlock"),T=document.getElementById("presetServicesList"),I=document.getElementById("servicesDivider");let y=[];try{if(e.services_booked){const t=typeof e.services_booked=="string"?JSON.parse(e.services_booked):e.services_booked;Array.isArray(t)&&(y=t)}}catch{}if(T&&p){T.innerHTML="";const t=D.find(i=>i.name===e.tour);let d=[];try{if(t?.services){const i=typeof t.services=="string"?JSON.parse(t.services):t.services;Array.isArray(i)&&(d=i)}}catch{}d.length>0?(p.classList.remove("hidden"),I&&I.classList.remove("hidden"),d.forEach(i=>{const l=y.some(f=>(f.label||f.name||"").toLowerCase()===(i.label||"").toLowerCase()),g=document.createElement("label");g.className="preset-service-card flex items-center gap-3 p-3 rounded-xl border-2 cursor-pointer transition-all "+(l?"border-green-400 bg-green-50":"border-gray-100 bg-gray-50 hover:border-orange-200"),g.innerHTML=`
                    ...
                    `,g.querySelector(".preset-service-check").addEventListener("change",f=>{g.className="preset-service-card flex items-center gap-3 p-3 rounded-xl border-2 cursor-pointer transition-all "+(f.target.checked?"border-green-400 bg-green-50":"border-gray-100 bg-gray-50 hover:border-orange-200"),window.updateServicesTotal()}),T.appendChild(g)})):(p.classList.add("hidden"),I&&I.classList.add("hidden"))}const m=document.getElementById("edit-pickup-point");if(m){const t=D.find(b=>b.name===e.tour);let d=[];try{if(t?.pickup_points){const b=typeof t.pickup_points=="string"?JSON.parse(t.pickup_points):t.pickup_points;Array.isArray(b)&&(d=b)}}catch{}m.innerHTML='<option value="">— Chưa chọn —</option>',d.forEach(b=>{const E=(b.label||b.name||b)+(b.time?` (${b.time})`:""),C=b.label||b.name||b,v=document.createElement("option");v.value=C,v.textContent=E,m.appendChild(v)});const i=document.createElement("option");i.value="__custom__",i.textContent="✏️ Khác (nhập tay)",m.appendChild(i);const l=document.getElementById("edit-pickup-custom"),g=b=>{l&&(b==="__custom__"?(l.classList.remove("hidden"),l.focus()):(l.classList.add("hidden"),l&&(l.value="")))},f=e.pickup_point||"";f&&(d.some(E=>(E.label||E.name||E)===f)?m.value=f:(m.value="__custom__",l&&(l.value=f,l.classList.remove("hidden")))),m.addEventListener("change",()=>g(m.value))}if(c){c.querySelectorAll(".service-row").forEach(l=>l.remove());const t=D.find(l=>l.name===e.tour);let d=new Set;try{const l=t?.services,g=l?typeof l=="string"?JSON.parse(l):l:[];Array.isArray(g)&&g.forEach(f=>d.add((f.label||"").toLowerCase()))}catch{}const i=y.filter(l=>!d.has((l.label||l.name||"").toLowerCase()));i.length>0?(h&&h.classList.add("hidden"),x&&x.classList.remove("hidden"),i.forEach(l=>window.addServiceRow(l.label||l.name||"",l.price||0))):h&&h.classList.remove("hidden"),window.updateServicesTotal()}const w=document.getElementById("editModal"),S=document.getElementById("editModalContent");w&&S&&(w.classList.remove("hidden"),setTimeout(()=>{w.classList.add("opacity-100"),S.classList.remove("scale-95","translate-y-4"),S.classList.add("scale-100","translate-y-0")},10))},window.actionDelete=async n=>{if(document.getElementById("rowActionModal").classList.add("hidden"),confirm("Bạn có chắc chắn muốn xóa Đơn hàng này? Thao tác không thể hoàn tác."))try{if((await fetch(`/api/bookings?id=${n}`,{method:"DELETE"})).ok)alert("✅ Đã xóa đơn hàng thành công!"),M();else throw new Error("Lỗi từ Server")}catch(e){alert("❌ Không thể xóa bảng ghi này: "+e.message)}},window.updateServicesTotal=()=>{let n=0;document.querySelectorAll(".preset-service-check:checked").forEach(a=>{n+=parseInt(a.dataset.price)||0}),document.querySelectorAll(".service-price-input").forEach(a=>{n+=parseInt(a.value)||0});const e=document.getElementById("servicesTotalDisplay");e&&(e.textContent=n.toLocaleString("vi-VN")+"đ");const o=document.getElementById("servicesTotalRow");return o&&o.classList.toggle("hidden",n===0),window.updateEditRemaining(),n},window.addServiceRow=(n="",e=0)=>{const o=document.getElementById("servicesContainer"),a=document.getElementById("emptyServicesMsg"),r=document.getElementById("servicesTotalRow");if(!o)return;a&&a.classList.add("hidden"),r&&r.classList.remove("hidden");const u=document.createElement("div");u.className="service-row flex items-center gap-2 bg-gray-50 border border-gray-100 rounded-xl px-3 py-2.5",u.innerHTML=`
        ...
        `,u.querySelector(".remove-service-btn").addEventListener("click",()=>{u.remove(),o.querySelectorAll(".service-row").length===0&&(a&&a.classList.remove("hidden"),r&&r.classList.add("hidden")),window.updateServicesTotal()}),u.querySelector(".service-price-input").addEventListener("input",window.updateServicesTotal),o.appendChild(u),window.updateServicesTotal()};const re=document.getElementById("addServiceBtn");re&&re.addEventListener("click",()=>window.addServiceRow()),window.updateEditRemaining=()=>{const n=parseInt(document.getElementById("edit-total")?.value)||0,e=parseInt(document.getElementById("edit-discount")?.value)||0,o=parseInt(document.getElementById("edit-deposit")?.value)||0;let a=0;document.querySelectorAll(".preset-service-check:checked").forEach(x=>{a+=parseInt(x.dataset.price)||0}),document.querySelectorAll(".service-price-input").forEach(x=>{a+=parseInt(x.value)||0});const r=n-e+a,u=r-o,s=document.getElementById("edit-grand-total");s&&(s.textContent=r.toLocaleString("vi-VN")+"đ");const c=document.getElementById("edit-remaining");c&&(c.textContent=u>0?u.toLocaleString("vi-VN")+"đ":"0đ");const h=document.getElementById("edit-status");if(h){const x=h.value;["Chờ tư vấn","Chờ cọc","Chờ xác nhận cọc"].includes(x)&&(o>0&&u>0?h.value="Đã cọc":o===0&&(h.value=x))}};const ce=document.getElementById("edit-total"),ue=document.getElementById("edit-deposit");ce&&ce.addEventListener("input",window.updateEditRemaining),ue&&ue.addEventListener("input",window.updateEditRemaining),document.querySelectorAll('#bookingModal button[onclick*="hidden"]').forEach(n=>{n.removeAttribute("onclick"),n.addEventListener("click",window.closeModal)});const pe=async n=>{try{const o=await(await fetch("/api/admin_customers?action=create",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({full_name:n.name,phone:n.phone,cccd:n.id_card||"",dob:n.dob||null,gender:n.gender||"Khác",medical_notes:n.allergy||"",dietary:n.diet||""})})).json();o.success&&o.csr_code&&(console.log("✅ Đã đồng bộ CRM thành công:",n.name,o.csr_code),(await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:n.id,customer_id:o.csr_code})})).ok&&(console.log("✅ Đã cập nhật Booking mã CRM"),typeof M=="function"&&M()))}catch(e){console.error("❌ Lỗi đồng bộ CRM:",e)}},ge=document.getElementById("bookingsTableBody");ge&&ge.addEventListener("click",async n=>{const e=n.target.closest(".action-btn");if(!e){const u=n.target.closest(".row-clickable");if(u){const s=u.getAttribute("data-id"),c=H.find(h=>h.id==s);c&&openRowActionModal(c)}return}const o=e.getAttribute("data-id"),a=H.find(u=>u.id==o);if(!a)return;const r=document.getElementById("rowActionModal");if(r&&r.classList.add("hidden"),e.classList.contains("confirm-deposit-btn")){if(confirm("Xác nhận khách hàng này đã chuyển khoản đặt cọc?"))try{if(e.disabled=!0,e.classList.add("opacity-50"),(await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:o,status:"Đã cọc"})})).ok)alert("✅ Đã xác nhận cọc thành công!"),pe(a),typeof M=="function"&&M();else throw new Error("Lỗi cập nhật API")}catch(u){alert("❌ Lỗi: "+u.message)}finally{e.disabled=!1,e.classList.remove("opacity-50")}return}if(e.classList.contains("delete-btn")){window.actionDelete(o);return}else e.classList.contains("process-btn")?window.actionEdit(o):e.classList.contains("view-btn")?(document.getElementById("rowActionModal").classList.add("hidden"),window.actionView(o)):e.classList.contains("edit-btn")&&window.actionEdit(o)});const q=document.getElementById("smartSearch"),_=document.getElementById("searchSuggestions");if(q){let n;q.addEventListener("input",e=>{const o=e.target.value.toLowerCase().trim();if(clearTimeout(n),o.length<2){_&&_.classList.add("hidden");return}n=setTimeout(async()=>{try{const a=await fetch(`/api/admin_customers?action=search&keyword=${o}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({keyword:o})}),r=await a.json();if(a.ok&&r.success){const s=[r.data];_&&(_.innerHTML=s.map(c=>`
                        ...
                        `).join(""),_.classList.remove("hidden"))}else _&&(_.innerHTML='<div class="p-3 text-xs text-gray-400 italic text-center">Không tìm thấy khách hàng cũ</div>',_.classList.remove("hidden"))}catch(a){console.error("Search error:",a)}},300)}),_&&_.addEventListener("click",e=>{const o=e.target.closest(".suggestion-item");if(o){const a=document.getElementById("addFullName"),r=document.getElementById("addPhone"),u=document.getElementById("addCsrCode");a&&(a.value=o.getAttribute("data-name")),r&&(r.value=o.getAttribute("data-phone")),u&&(u.value=o.getAttribute("data-csr"));const s=document.getElementById("addDob"),c=document.getElementById("addGender"),h=document.getElementById("addIdCard"),x=document.getElementById("addAddress");s&&(s.value=o.getAttribute("data-dob")||""),c&&(c.value=o.getAttribute("data-gender")||"Khác"),h&&(h.value=o.getAttribute("data-cccd")||""),x&&(x.value=o.getAttribute("data-address")||"");const p=document.getElementById("addAllergy"),T=document.getElementById("addDiet"),I=document.getElementById("addTrekkingPole"),y=document.getElementById("addMedalName");p&&(p.value=o.getAttribute("data-allergy")||""),T&&(T.value=o.getAttribute("data-diet")||""),I&&(I.value=o.getAttribute("data-trekking_pole")||"Không"),y&&(y.value=o.getAttribute("data-medal-name")||""),window._selectedCustomer={fullName:o.getAttribute("data-name"),phone:o.getAttribute("data-phone"),csrCode:o.getAttribute("data-csr"),dob:o.getAttribute("data-dob")||"",gender:o.getAttribute("data-gender")||"",address:o.getAttribute("data-address")||"",id_card:o.getAttribute("data-cccd")||"",allergy:o.getAttribute("data-allergy")||"",diet:o.getAttribute("data-diet")||"",trekking_pole:o.getAttribute("data-trekking_pole")||"",medal_name:o.getAttribute("data-medal-name")||""},q.value=o.getAttribute("data-name"),_.classList.add("hidden");const m=`
                    ...
                    </div>`,w=document.getElementById("loyalty-alert");w&&w.remove(),q.parentElement.insertAdjacentHTML("afterend",m)}}),document.addEventListener("click",e=>{!q.contains(e.target)&&_&&!_.contains(e.target)&&_.classList.add("hidden")})}const z=document.getElementById("addTourPrice"),G=document.getElementById("addDiscount"),me=document.getElementById("addTotalPriceDisplay");document.getElementById("addTourName");const he=()=>{if(!z||!G||!me)return;const n=parseInt(z.value)||0,e=parseInt(G.value)||0,o=n-e;me.textContent=(o>0?o:0).toLocaleString("vi-VN")+"đ"};z&&z.addEventListener("input",he),G&&G.addEventListener("input",he);const be=document.getElementById("searchBtn");be&&be.addEventListener("click",n=>{n.preventDefault(),document.getElementById("smartSearch").value||alert("Vui lòng nhập Số Điện Thoại hoặc Mã #CSR")});const W=document.getElementById("bookingForm");W&&W.addEventListener("submit",async n=>{n.preventDefault();const e=n.target.querySelector('button[type="submit"]'),o=e.textContent;e.textContent="Đang lưu hệ thống...",e.disabled=!0;try{const a=document.getElementById("addFullName").value,r=document.getElementById("addPhone").value,u=document.getElementById("addTourName").value,s=document.getElementById("addTourDate").value;let c=null,h=s;if(s&&s.includes("::")){const L=s.split("::");c=parseInt(L[0])||null,h=L[1]}const x=document.getElementById("addDob").value,p=document.getElementById("addGender").value,T=document.getElementById("addIdCard").value,I=document.getElementById("addAddress").value,y=document.getElementById("addSpecial").value,m=document.getElementById("addAllergy").value,w=document.getElementById("addDiet").value,S=document.getElementById("addTrekkingPole").value,t=document.getElementById("addMedalName").value,d=parseInt(document.getElementById("addTourPrice").value)||0,i=parseInt(document.getElementById("addDiscount").value)||0,l=parseInt(document.getElementById("addDepositRequired").value),g=isNaN(l)?1e6:l,f=document.getElementById("addCsrCode").value||"",b=localStorage.getItem("csr_user");let E=null,C="Admin";if(b){const L=JSON.parse(b);E=L.id,C=L.fullName||L.full_name||"Admin"}const v=document.getElementById("editingBookingId")?document.getElementById("editingBookingId").value:"",k={name:a,phone:r,tour:u,date:h,status:"Chờ cọc",total_price:d-i,discount:i,deposit:0,sale_id:E,sale_name:C,customer_id:f,deposit_required:g,commitments:!0,schedule_id:c,dob:x,gender:p,address:I,id_card:T,special:y,allergy:m,diet:w,trekking_pole:S,medal_name:t},N=r.replace(/[^0-9]/g,"");window._selectedCustomer&&(window._selectedCustomer.phone||"").replace(/[^0-9]/g,"")===N&&(k.dob=k.dob||window._selectedCustomer.dob,k.gender=k.gender&&k.gender!=="Khác"?k.gender:window._selectedCustomer.gender||"Khác",k.address=k.address||window._selectedCustomer.address,k.id_card=k.id_card||window._selectedCustomer.id_card,k.allergy=window._selectedCustomer.allergy,k.diet=window._selectedCustomer.diet,k.trekking_pole=window._selectedCustomer.trekking_pole,k.medal_name=window._selectedCustomer.medal_name),v&&(k.id=v,delete k.status,delete k.deposit,delete k.sale_id,delete k.sale_name);const A=await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(k)});if(A.ok){alert(v?"✅ Đã lưu Cập nhật Đơn hàng!":"✅ Thêm Đơn Hàng Thành Công!"),window.closeModal(),W&&W.reset(),window._selectedCustomer=null;const L=document.getElementById("loyalty-alert");L&&L.remove(),document.getElementById("addTotalPriceDisplay")&&(document.getElementById("addTotalPriceDisplay").textContent="0đ"),typeof M=="function"?M():window.location.reload()}else{const L=await A.json();throw new Error(L.error||"Lỗi tạo Booking!")}}catch(a){alert("❌ "+a.message)}finally{e.textContent=o,e.disabled=!1}});const fe=document.getElementById("editForm");fe&&fe.addEventListener("submit",async n=>{n.preventDefault();const e=n.target.querySelector('button[type="submit"]'),o=e.textContent;e.textContent="Đang lưu cập nhật...",e.disabled=!0;try{const a=document.getElementById("edit-id").value,r=document.getElementById("edit-name").value,u=document.getElementById("edit-phone").value,s=document.getElementById("edit-medal-name").value,c=document.getElementById("edit-tour").value,h=document.getElementById("edit-date").value;let x=h,p;if(h&&h.includes("::")){const $=h.split("::");p=parseInt($[0])||null,x=$[1]}const T=document.getElementById("edit-dob").value,I=document.getElementById("edit-gender").value,y=document.getElementById("edit-status").value,m=document.getElementById("edit-allergy").value,w=document.getElementById("edit-address").value,S=document.getElementById("edit-diet").value,t=document.getElementById("edit-trekking-pole").value,d=document.getElementById("edit-commitments").checked,i=document.getElementById("edit-special").value,l=document.getElementById("edit-id-card"),g=l?l.value:"",f=document.getElementById("edit-pickup-point"),b=document.getElementById("edit-pickup-custom"),E=f?.value==="__custom__"?b?.value?.trim()||"":f?.value||"",C=[];document.querySelectorAll(".preset-service-check:checked").forEach($=>{const K=$.dataset.label,ee=parseInt($.dataset.price)||0;K&&C.push({label:K,price:ee})}),document.querySelectorAll(".service-row").forEach($=>{const K=($.querySelector(".service-label-input")?.value||"").trim(),ee=parseInt($.querySelector(".service-price-input")?.value)||0;K&&C.push({label:K,price:ee})});const k=C.length>0?JSON.stringify(C):null,N=document.getElementById("edit-sale");let A,L;if(N&&N.parentElement&&!N.parentElement.classList.contains("hidden")&&(A=N.value||null,L="Website",A)){const $=J.find(K=>String(K.id)===String(A));$&&(L=$.full_name||$.fullName)}const Be=parseInt(document.getElementById("edit-total").value)||0,ve=parseInt(document.getElementById("edit-discount").value)||0,ye=parseInt(document.getElementById("edit-deposit").value)||0,we=parseInt(document.getElementById("edit-deposit-required").value),Se=isNaN(we)?1e6:we;let ke=0;C.forEach($=>ke+=$.price);const Le=Be-ve+ke,F={id:a,name:r,phone:u,medal_name:s,tour:c,date:x,dob:T,gender:I,status:y,allergy:m,address:w,diet:S,trekking_pole:t,commitments:d,special:i,id_card:g,pickup_point:E,services_booked:k,sale_id:A,sale_name:L,total_price:Le,discount:ve,deposit:ye,deposit_required:Se,...p!==void 0?{schedule_id:p}:{}},Ce=await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(F)});if(Ce.ok)alert("✅ Đã cập nhật thành công Chi tiết Đơn hàng!"),(y.includes("Đã cọc")||F.total_price>0&&F.total_price===ye)&&pe(F),window.closeEditModal(),typeof M=="function"&&M();else{const $=await Ce.json();throw new Error($.error||"Gặp sự cố khi Cập nhật API.")}}catch(a){alert("❌ Lỗi Cập nhật: "+a.message)}finally{e.textContent=o,e.disabled=!1}}),window.openWaitingModal=()=>{document.getElementById("waitingModal").classList.remove("hidden"),setTimeout(()=>{document.getElementById("waitingModal").classList.add("opacity-100");const n=document.getElementById("waitingModalContent");n&&(n.classList.remove("scale-95"),n.classList.add("scale-100"))},10)},window.closeWaitingModal=()=>{document.getElementById("waitingModal").classList.remove("opacity-100");const n=document.getElementById("waitingModalContent");n&&(n.classList.remove("scale-100"),n.classList.add("scale-95")),setTimeout(()=>{document.getElementById("waitingModal").classList.add("hidden")},300)};const xe=document.getElementById("addWaitingBtn");xe&&xe.addEventListener("click",()=>window.openWaitingModal());const Z=document.getElementById("waitingForm");Z&&Z.addEventListener("submit",async n=>{n.preventDefault();const e=document.getElementById("wl-submit");e.innerHTML="Đang lưu...",e.disabled=!0;const o=document.getElementById("wl-zalo").value?`Tên Zalo / Ghi Chú: ${document.getElementById("wl-zalo").value}`:"";let a={id:"null",name:"Website"};try{const s=localStorage.getItem("csr_user");if(s){const c=JSON.parse(s);a={id:c.id,name:c.full_name||c.fullName}}}catch{}const r={name:document.getElementById("wl-name").value,phone:document.getElementById("wl-phone").value,tour:document.getElementById("wl-tour").value,date:"Mở (Chưa có ngày)",status:"Chờ tư vấn",special:o,sale_id:a.id,sale_name:a.name},u=document.getElementById("wl-date");if(u&&u.value){const s=u.value.split("::");s.length===2&&s[0]&&s[0]!=="undefined"&&(r.schedule_id=s[0],r.date=s[1])}try{const c=await(await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(r)})).json();c.success?(window.closeWaitingModal(),Z.reset(),typeof M=="function"?M():(alert("✅ Đã thêm vào danh sách chờ!"),window.location.reload())):alert("Lỗi lưu danh sách: "+c.message)}catch(s){console.error(s),alert("Có lỗi xảy ra, vui lòng thử lại.")}finally{e.innerHTML="Lưu Vào Danh Sách",e.disabled=!1}})};export{Ne as afterRender,De as render};
