import{S as g,H as b}from"./Header-CtRUqTZE.js";const h=()=>{let d={role:"sale",fullName:"Guest"};try{const s=localStorage.getItem("csr_user");s&&(d=JSON.parse(s))}catch{}return d.role!=="admin"?`
    ...
    `},y=()=>{let d={role:"sale",fullName:"Guest"};try{const t=localStorage.getItem("csr_user");t&&(d=JSON.parse(t))}catch{}if(d.role!=="admin")return;let s=[];const i=async()=>{try{const t=await fetch("/api/users");t.ok?(s=await t.json(),m()):document.getElementById("usersTableBody").innerHTML='<tr><td colspan="7" class="p-8 text-center text-red-500">Lỗi lấy dữ liệu</td></tr>'}catch(t){console.error(t),document.getElementById("usersTableBody").innerHTML='<tr><td colspan="7" class="p-8 text-center text-red-500">Lỗi kết nối server</td></tr>'}},m=()=>{const t=document.getElementById("usersTableBody");if(t){if(s.length===0){t.innerHTML='<tr><td colspan="7" class="p-8 text-center text-gray-500 font-medium">Chưa có tài khoản nào.</td></tr>';return}t.innerHTML=s.map(e=>{const r=e.username==="admin";let o='<span class="bg-gray-100 text-gray-600 px-2 py-1 rounded text-[10px] font-bold uppercase border border-gray-200">Sale</span>';e.role==="admin"?o='<span class="bg-red-50 text-red-600 px-2 py-1 rounded text-[10px] font-bold uppercase border border-red-100">Admin</span>':e.role==="tour_guide"?o='<span class="bg-teal-50 text-teal-600 px-2 py-1 rounded text-[10px] font-bold uppercase border border-teal-100">🧭 Tour Guide</span>':e.role==="tour_leader"&&(o='<span class="bg-purple-50 text-purple-600 px-2 py-1 rounded text-[10px] font-bold uppercase border border-purple-100">👑 Leader Tour</span>');let a='<span class="px-2 py-1 bg-green-50 text-green-600 text-[10px] font-bold rounded uppercase flex items-center gap-1 w-max mx-auto"><div class="w-1.5 h-1.5 rounded-full bg-green-500"></div> Active</span>';return e.status!=="active"&&(a='<span class="px-2 py-1 bg-gray-100 text-gray-500 text-[10px] font-bold rounded uppercase flex items-center gap-1 w-max mx-auto"><div class="w-1.5 h-1.5 rounded-full bg-gray-400"></div> Locked</span>'),`
                <tr class="hover:bg-gray-50 transition-colors">
                    ...
                    <td class="p-4 border-b border-gray-100">${o}</td>
            ...
            `}).join("")}},n=document.getElementById("userModal"),p=document.getElementById("userModalContent"),c=document.getElementById("userForm");window.closeUserModal=()=>{n.classList.add("opacity-0"),p.classList.add("scale-95"),setTimeout(()=>{n.classList.add("hidden"),c.reset(),document.getElementById("userId").value=""},200)},window.openUserModal=(t=!1,e=null)=>{c.reset(),document.getElementById("userId").value="",document.getElementById("modalUserTitle").innerHTML=t?'<svg class="w-5 h-5 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg> Cập Nhật Tài Khoản':'<svg class="w-5 h-5 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path></svg> Thêm Tài Khoản Mới',t&&e?(document.getElementById("userId").value=e.id,document.getElementById("userName").value=e.username,document.getElementById("userFullName").value=e.fullName,document.getElementById("userRole").value=e.role,document.getElementById("userPhone").value=e.phone||"",document.getElementById("userEmail").value=e.email||"",document.getElementById("userStatus").value=e.status||"active",document.getElementById("userName").disabled=e.username==="admin",document.getElementById("userRole").disabled=e.username==="admin"):(document.getElementById("userName").disabled=!1,document.getElementById("userRole").disabled=!1),n.classList.remove("hidden"),requestAnimationFrame(()=>{n.classList.remove("opacity-0"),p.classList.remove("scale-95")})},document.getElementById("btnAddNewUser").addEventListener("click",()=>{window.openUserModal(!1)}),window.editUser=t=>{const e=s.find(r=>r.id===t);e&&window.openUserModal(!0,e)},window.deleteUser=async(t,e)=>{if(confirm(`Bạn có chắc chắn muốn XÓA tài khoản "${e}" không?`))try{const r=await fetch(`/api/users?id=${t}`,{method:"DELETE"});if(r.ok)alert("Đã xóa thành công!"),i();else{const o=await r.json();alert(o.error||"Lỗi khi xóa!")}}catch(r){console.error(r)}},c.addEventListener("submit",async t=>{t.preventDefault();const e=document.getElementById("btnSubmitUser"),r=e.textContent;e.disabled=!0,e.textContent="Đang lưu...";const o=document.getElementById("userId").value,a={id:o||void 0,username:document.getElementById("userName").value,fullName:document.getElementById("userFullName").value,role:document.getElementById("userRole").value,password:document.getElementById("userPass").value,phone:document.getElementById("userPhone").value,email:document.getElementById("userEmail").value,status:document.getElementById("userStatus").value};if(document.getElementById("userName").disabled){const l=s.find(u=>u.id==o);a.username=l.username,a.role=l.role}try{const l=await fetch("/api/users",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(a)});if(l.ok)window.closeUserModal(),i();else{const u=await l.json();alert(u.error||"Lỗi lưu thông tin")}}catch(l){console.error(l),alert("Lỗi kết nối server!")}finally{e.disabled=!1,e.textContent=r}}),n.addEventListener("click",t=>{t.target===n&&window.closeUserModal()}),i()};export{y as afterRender,h as render};
