import{S as U,H as D}from"./Header-CARqqxjt.js";const W=()=>`
                  ...
                  <form id="tourForm" class="space-y-4 md:space-y-5">
                      ...
                      <div id="tab-panel-general" class="tab-panel space-y-4 md:space-y-5">
                          ...
                          <div class="grid grid-cols-3 gap-3">
                              ${["2","3","4"].map(i=>`
                              <div>
                                  <label class="block text-[10px] font-bold text-gray-400 uppercase mb-1">Ảnh phụ ${parseInt(i)-1}</label>
                                  <div id="tour-img-zone-${i}"
                                       ...
                                       ondragover="event.preventDefault(); this.classList.add('border-csr-orange','bg-orange-50');"
                                       ondragleave="this.classList.remove('border-csr-orange','bg-orange-50');"
                                       ondrop="event.preventDefault(); this.classList.remove('border-csr-orange','bg-orange-50'); window.handleTourImgDrop(event,'${i}');"
                                       onclick="document.getElementById('tour-img-input-${i}').click()">
                                      <div id="tour-img-preview-${i}" class="w-full h-full flex flex-col items-center justify-center text-gray-300 gap-1">
                                      ...
                                      <div class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center hidden" id="tour-img-overlay-${i}">
                                          <span class="text-white font-bold text-[10px]">ĐỔI ẢNH</span>
                                      </div>
                                  </div>
                                  <input type="file" id="tour-img-input-${i}" accept="image/*" class="hidden" data-slot="${i}">
                                  <input type="hidden" id="tour-image${i}" value="">
                              </div>`).join("")}
                          </div>
    ...
    `,J=()=>{const i=document.getElementById("toursTableBody"),g=document.getElementById("tourModal"),y=document.getElementById("tourModalContent"),E=document.getElementById("tourForm");let x=[];const f="/api/admin_tours",G=()=>{const e=document.querySelectorAll("#tour-modal-tabs .tab-btn"),t=document.querySelectorAll(".tab-panel");e.forEach(n=>{n.addEventListener("click",()=>{e.forEach(o=>{o.classList.remove("active","border-csr-orange","text-csr-orange"),o.classList.add("border-transparent","text-gray-500"),o.classList.remove("font-bold"),o.classList.add("font-semibold")}),n.classList.add("active","border-csr-orange","text-csr-orange"),n.classList.remove("border-transparent","text-gray-500"),n.classList.remove("font-semibold"),n.classList.add("font-bold");const r=n.dataset.tab;t.forEach(o=>{o.id===`tab-panel-${r}`?o.classList.remove("hidden"):o.classList.add("hidden")})})})},X=()=>{const e=document.querySelector('#tour-modal-tabs .tab-btn[data-tab="general"]');e&&e.click()},T=[{value:"XeTrungChuyen.png",label:"Xe đưa đón (XeTrungChuyen.png)"},{value:"HuongDanVien-SuaLai.png",label:"Hướng dẫn viên (HuongDanVien-SuaLai.png)"},{value:"DoAn-SuaLai.png",label:"Ăn uống (DoAn-SuaLai.png)"},{value:"NuocUong.png",label:"Nước uống (NuocUong.png)"},{value:"NuocTam-SuaLai_20260113000157.png",label:"Tắm nước nóng (NuocTam...)"},{value:"GayTrekking.png",label:"Gậy trekking (GayTrekking.png)"},{value:"HuyChuong.png",label:"Huy chương (HuyChuong.png)"},{value:"AoMua.png",label:"Áo mưa (AoMua.png)"},{value:"BaoHiem.png",label:"Bảo hiểm (BaoHiem.png)"}],V=[{value:"NMG_giay.png",label:"Giày Trekking (NMG_giay.png)"},{value:"NMG_balo.png",label:"Balo Trekking (NMG_balo.png)"},{value:"NMG_quanao.png",label:"Trang phục (NMG_quanao.png)"},{value:"NMG_dungcu.png",label:"Dụng cụ cá nhân (NMG_dungcu.png)"}],I=(e={dayTitle:"",steps:[]},t)=>{const n=document.createElement("div");n.className="day-block bg-gray-50 p-4 rounded-xl border border-gray-200 space-y-3 relative",n.dataset.index=t,n.innerHTML=`
        ...
        `;const r=n.querySelector(".times-container");return(e.steps||[]).forEach((s,p)=>{r.appendChild(L(s))}),n.querySelector(".add-time-btn").addEventListener("click",()=>{r.children.length,r.appendChild(L({time:"",desc:""}))}),n.querySelector(".remove-day-btn").addEventListener("click",()=>{n.remove()}),n},L=(e={time:"",desc:""},t,n)=>{const r=document.createElement("div");return r.className="time-row flex gap-2 items-center",r.innerHTML=`
        ...
        `,r.querySelector(".remove-time-btn").addEventListener("click",()=>{r.remove()}),r},M=(e={title:"",desc:"",image:"XeTrungChuyen.png"})=>{const t=document.createElement("div");t.className="inclusion-row flex gap-2 items-center bg-gray-50 p-2.5 rounded-xl border border-gray-150";const n=T.map(s=>`<option value="${s.value}" ${e.image===s.value?"selected":""}>${s.label}</option>`).join("");t.innerHTML=`
            <select class="incl-image-select input-field bg-white text-xs w-44 shrink-0">
                ${n}
                <option value="custom" ${!T.some(s=>s.value===e.image)&&e.image?"selected":""}>Tự nhập path...</option>
            </select>
            <input type="text" class="incl-image-custom input-field bg-white text-xs w-32 shrink-0 ${!T.some(s=>s.value===e.image)&&e.image?"":"hidden"}" value="${e.image||""}" placeholder="đường_dẫn_ảnh.png">
        ...
        `;const r=t.querySelector(".incl-image-select"),o=t.querySelector(".incl-image-custom");return r.addEventListener("change",()=>{r.value==="custom"?o.classList.remove("hidden"):(o.classList.add("hidden"),o.value=r.value)}),t.querySelector(".remove-incl-btn").addEventListener("click",()=>{t.remove()}),t},q=(e={title:"",desc:""})=>{const t=document.createElement("div");return t.className="exclusion-row flex gap-2 items-start bg-gray-50 p-2.5 rounded-xl border border-gray-150",t.innerHTML=`
        ...
        `,t.querySelector(".remove-excl-btn").addEventListener("click",()=>{t.remove()}),t},$=(e={title:"",desc:"",image:"NMG_giay.png"})=>{const t=document.createElement("div");t.className="preparing-row flex gap-2 items-center bg-gray-50 p-2.5 rounded-xl border border-gray-150";const n=V.map(r=>`<option value="${r.value}" ${e.image===r.value?"selected":""}>${r.label}</option>`).join("");return t.innerHTML=`
        ...
        `,t.querySelector(".remove-prep-btn").addEventListener("click",()=>{t.remove()}),t},C=(e={q:"",a:""})=>{const t=document.createElement("div");return t.className="faq-row flex flex-col gap-2 bg-gray-50 p-3 rounded-xl border border-gray-150 relative",t.innerHTML=`
        ...
        `,t.querySelector(".remove-faq-btn").addEventListener("click",()=>{t.remove()}),t},S=(e=[])=>{const t=document.getElementById("inclusions-container");t.innerHTML="",e.forEach(n=>t.appendChild(M(n)))},N=(e=[])=>{const t=document.getElementById("exclusions-container");t.innerHTML="",e.forEach(n=>t.appendChild(q(n)))},j=(e=[])=>{const t=document.getElementById("preparing-container");t.innerHTML="",e.forEach(n=>t.appendChild($(n)))},H=(e=[])=>{const t=document.getElementById("faqs-container");t.innerHTML="",e.forEach(n=>t.appendChild(C(n)))},A=(e=[])=>{const t=document.getElementById("days-container");t.innerHTML="",e.forEach((n,r)=>t.appendChild(I(n,r)))},P=(e=null)=>{const t=document.getElementById("tourModalTitle");e?(t.textContent="Chỉnh Sửa Tour",document.getElementById("tour-edit-id").value=e.id,document.getElementById("tour-name").value=e.name||"",document.getElementById("tour-short-desc").value=e.short_desc||e.shortDesc||"",v("main",e.image||""),v("2",e.image2||""),v("3",e.image3||""),v("4",e.image4||""),document.getElementById("tour-duration").value=e.duration||"",document.getElementById("tour-price").value=e.price||"",document.getElementById("tour-level").value=e.level||"Trung Bình",document.getElementById("tour-region").value=e.region||"Miền Nam",document.getElementById("tour-type").value=e.type||"TREKKING",document.getElementById("tour-altitude").value=e.altitude||"",document.getElementById("tour-sort-order").value=e.sort_order||0,document.getElementById("tour-custom-domain").value=e.custom_domain||"",document.getElementById("tour-is-visible").checked=e.is_visible!==!1,A(e.itinerary||[]),S(e.inclusions||[]),N(e.exclusions||[]),j(e.preparing||[]),H(e.faqs||[])):(t.textContent="Thêm Tour Mới",E.reset(),document.getElementById("tour-edit-id").value="",document.getElementById("tour-is-visible").checked=!0,document.getElementById("tour-custom-domain").value="https://camsiteretreats.com/tour/",["main","2","3","4"].forEach(n=>v(n,"")),A([]),S([]),N([]),j([]),H([])),X(),g.classList.remove("hidden"),setTimeout(()=>{g.classList.add("opacity-100"),y.classList.remove("scale-95"),y.classList.add("scale-100")},10),O()},w=()=>{g.classList.remove("opacity-100"),y.classList.remove("scale-100"),y.classList.add("scale-95"),setTimeout(()=>{g.classList.add("hidden")},200)};document.getElementById("addTourBtn").addEventListener("click",()=>P()),document.getElementById("closeTourModalBtn").addEventListener("click",w),document.getElementById("cancelTourBtn").addEventListener("click",w),g.addEventListener("click",e=>{e.target===g&&w()}),document.getElementById("add-day-btn").addEventListener("click",()=>{const e=document.querySelectorAll(".day-block").length;document.getElementById("days-container").appendChild(I({dayTitle:"",steps:[]},e))}),document.getElementById("add-inclusion-btn").addEventListener("click",()=>{document.getElementById("inclusions-container").appendChild(M())}),document.getElementById("add-exclusion-btn").addEventListener("click",()=>{document.getElementById("exclusions-container").appendChild(q())}),document.getElementById("add-prep-btn").addEventListener("click",()=>{document.getElementById("preparing-container").appendChild($())}),document.getElementById("add-faq-btn").addEventListener("click",()=>{document.getElementById("faqs-container").appendChild(C())}),G();const K=["main","2","3","4"],R=(e,t)=>{const n=document.getElementById(`tour-img-preview-${e}`),r=document.getElementById(`tour-img-overlay-${e}`);document.getElementById(`tour-img-zone-${e}`),n&&(n.innerHTML=`<img src="${t}" class="w-full h-full object-cover" alt="preview">`,n.className="w-full h-full",r&&r.classList.remove("hidden"))},v=(e,t)=>{const n=document.getElementById(`tour-img-preview-${e}`),r=document.getElementById(`tour-img-overlay-${e}`),o=document.getElementById(e==="main"?"tour-image":`tour-image${e}`);if(n){if(t)n.innerHTML=`<img src="${t}" class="w-full h-full object-cover" alt="preview">`,n.className="w-full h-full",r&&r.classList.remove("hidden");else{const s=e==="main";n.innerHTML=s?'<svg class="w-8 h-8" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg><span class="text-xs font-bold uppercase">Kéo thả hoặc click để chọn ảnh</span><span class="text-[10px] text-gray-400">JPG, PNG, WEBP — tối đa 10MB, tự động nén</span>':'<svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 4v16m8-8H4"/></svg><span class="text-[9px] font-bold uppercase">Thêm ảnh</span>',n.className="w-full h-full flex flex-col items-center justify-center text-gray-300 gap-1",r&&r.classList.add("hidden")}o&&(o.value=t||"")}},z=e=>new Promise((t,n)=>{const o=new FileReader;o.onload=s=>{const p=new Image;p.onload=()=>{let{width:l,height:u}=p;(l>1200||u>1200)&&(l>u?(u=Math.round(u*1200/l),l=1200):(l=Math.round(l*1200/u),u=1200));const b=document.createElement("canvas");b.width=l,b.height=u,b.getContext("2d").drawImage(p,0,0,l,u),t(b.toDataURL("image/jpeg",.85))},p.onerror=n,p.src=s.target.result},o.onerror=n,o.readAsDataURL(e)}),_=async(e,t)=>{const n=document.getElementById(`tour-img-preview-${t}`),r=document.getElementById(t==="main"?"tour-image":`tour-image${t}`);if(n){n.innerHTML='<svg class="animate-spin w-7 h-7 text-csr-orange" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"/><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"/></svg>',n.className="w-full h-full flex items-center justify-center";try{const o=await z(e),s=(e.name||`tour-${t}`).replace(/\.[^.]+$/,""),l=await(await fetch("/api/upload",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({image:o,filename:`tour-${s}`})})).json();if(!l.success)throw new Error(l.error||"Upload thất bại");R(t,l.url),r&&(r.value=l.url),console.log(`[tour-upload] ✅ slot=${t} ${l.stats?.inputKB}KB → ${l.stats?.outputKB}KB`)}catch(o){n.innerHTML=`<span class="text-red-400 text-xs text-center p-2">❌ Lỗi: ${o.message}</span>`,n.className="w-full h-full flex items-center justify-center",alert("Lỗi upload ảnh: "+o.message)}}},O=()=>{K.forEach(e=>{const t=document.getElementById(`tour-img-input-${e}`);!t||t.dataset.bound||(t.dataset.bound="1",t.addEventListener("change",n=>{const r=n.target.files?.[0];r&&_(r,e)}))})};window.handleTourImgDrop=(e,t)=>{const n=e.dataTransfer?.files?.[0];n&&n.type.startsWith("image/")&&_(n,t)};const B=async()=>{i.innerHTML='<tr><td colspan="7" class="text-center py-8 text-gray-400">Đang tải danh sách tour...</td></tr>';try{const e=await fetch(f);if(!e.ok)throw new Error("Failed to load tours");const t=await e.json();x=Array.isArray(t)?t:t.data||[],F()}catch(e){console.error("Error loading tours:",e),i.innerHTML='<tr><td colspan="7" class="text-center py-4 text-red-500">Lỗi kết nối server.</td></tr>'}},F=()=>{if(x.length===0){i.innerHTML='<tr><td colspan="7" class="text-center py-8 text-gray-400">Chưa có tour nào.</td></tr>';return}i.innerHTML=x.map(e=>{const t=!e.price||e.price==="Update"||e.price==="0"||parseInt(e.price)===0?'<span class="text-gray-400 italic">Update...</span>':`<span class="font-bold text-csr-orange">${parseInt(e.price).toLocaleString("vi-VN")}đ</span>`,n=e.level==="Dễ"?"bg-green-100 text-green-700":e.level==="Khó"?"bg-red-100 text-red-600":"bg-orange-100 text-orange-700",r=e.is_visible!==!1?'<span class="bg-green-100 text-green-600 text-[10px] px-2 py-0.5 rounded-full font-bold">Hiển thị</span>':'<span class="bg-gray-200 text-gray-500 text-[10px] px-2 py-0.5 rounded-full font-bold">Đã ẩn</span>';return`
                <tr class="hover:bg-gray-50 transition-colors group block md:table-row border-b md:border-none p-4 md:p-0" data-tour-id="${e.id}">
                    ...
                    <td class="hidden md:table-cell p-4 text-center">${r}</td>
            ...
            `}).join("")};i.addEventListener("click",async e=>{const t=e.target.closest(".tour-edit-btn"),n=e.target.closest(".tour-delete-btn");if(t){const r=parseInt(t.getAttribute("data-id")),o=x.find(s=>s.id===r);o&&P(o)}if(n){const r=n.getAttribute("data-id");if(confirm("Dữ liệu lịch trình liên quan có thể bị ảnh hưởng. Bạn vẫn muốn xóa tour này?"))try{if((await fetch(`${f}?id=${r}`,{method:"DELETE"})).ok)B();else throw new Error("Delete failed")}catch(o){alert("Lỗi khi xóa: "+o.message)}}}),E.addEventListener("submit",async e=>{e.preventDefault();const t=E.querySelector('button[type="submit"]'),n=t.textContent;t.textContent="Đang lưu...",t.disabled=!0;const r=document.getElementById("tour-edit-id").value,o=[];document.querySelectorAll(".day-block").forEach(a=>{const d=a.querySelector(".day-title-input").value.trim(),c=[];a.querySelectorAll(".time-row").forEach(m=>{const h=m.querySelector(".time-input").value.trim(),k=m.querySelector(".desc-input").value.trim();(h||k)&&c.push({time:h,desc:k})}),d&&o.push({dayTitle:d,steps:c})});const s=[];document.querySelectorAll(".inclusion-row").forEach(a=>{const d=a.querySelector(".incl-image-select").value,c=a.querySelector(".incl-image-custom").value.trim(),m=d==="custom"?c:d,h=a.querySelector(".incl-title-input").value.trim(),k=a.querySelector(".incl-desc-input").value.trim();h&&s.push({title:h,desc:k,image:m})});const p=[];document.querySelectorAll(".exclusion-row").forEach(a=>{const d=a.querySelector(".excl-title-input").value.trim(),c=a.querySelector(".excl-desc-input").value.trim();d&&p.push({title:d,desc:c})});const l=[];document.querySelectorAll(".preparing-row").forEach(a=>{const d=a.querySelector(".prep-image-select").value,c=a.querySelector(".prep-title-input").value.trim(),m=a.querySelector(".prep-desc-input").value.trim();c&&l.push({title:c,desc:m,image:d})});const u=[];document.querySelectorAll(".faq-row").forEach(a=>{const d=a.querySelector(".faq-q-input").value.trim(),c=a.querySelector(".faq-a-input").value.trim();d&&u.push({q:d,a:c})});const b={id:r?parseInt(r):null,name:document.getElementById("tour-name").value,image:document.getElementById("tour-image").value,image2:document.getElementById("tour-image2").value||null,image3:document.getElementById("tour-image3").value||null,image4:document.getElementById("tour-image4").value||null,shortDesc:document.getElementById("tour-short-desc").value,altitude:document.getElementById("tour-altitude").value||null,level:document.getElementById("tour-level").value,region:document.getElementById("tour-region").value,type:document.getElementById("tour-type").value,duration:document.getElementById("tour-duration").value,price:document.getElementById("tour-price").value,sort_order:parseInt(document.getElementById("tour-sort-order").value)||0,custom_domain:document.getElementById("tour-custom-domain").value.trim()||null,is_visible:document.getElementById("tour-is-visible").checked,itinerary:o,inclusions:s,exclusions:p,preparing:l,faqs:u};try{const a=r?`${f}?id=${r}`:f,c=await fetch(a,{method:r?"PUT":"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(b)});if(!c.ok){const m=await c.json();throw new Error(m.message||m.error||"Save failed")}w(),B(),alert(r?"✅ Cập nhật tour thành công!":"✅ Thêm tour mới thành công! Website sẽ tự đồng bộ trong vài giây.")}catch(a){alert("❌ Lỗi: "+a.message)}finally{t.textContent=n,t.disabled=!1}}),B()};export{J as afterRender,W as render};
