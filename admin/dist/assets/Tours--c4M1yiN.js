import{S as D,H as Q}from"./Header-CARqqxjt.js";const J=()=>`
    ...
    `,Y=()=>{const i=document.getElementById("toursTableBody"),g=document.getElementById("tourModal"),y=document.getElementById("tourModalContent"),E=document.getElementById("tourForm");let x=[];const f="/api/admin_tours",G=()=>{const e=document.querySelectorAll("#tour-modal-tabs .tab-btn"),t=document.querySelectorAll(".tab-panel");e.forEach(n=>{n.addEventListener("click",()=>{e.forEach(r=>{r.classList.remove("active","border-csr-orange","text-csr-orange"),r.classList.add("border-transparent","text-gray-500"),r.classList.remove("font-bold"),r.classList.add("font-semibold")}),n.classList.add("active","border-csr-orange","text-csr-orange"),n.classList.remove("border-transparent","text-gray-500"),n.classList.remove("font-semibold"),n.classList.add("font-bold");const o=n.dataset.tab;t.forEach(r=>{r.id===`tab-panel-${o}`?r.classList.remove("hidden"):r.classList.add("hidden")})})})},X=()=>{const e=document.querySelector('#tour-modal-tabs .tab-btn[data-tab="general"]');e&&e.click()},T=[{value:"XeTrungChuyen.png",label:"Xe đưa đón (XeTrungChuyen.png)"},{value:"HuongDanVien-SuaLai.png",label:"Hướng dẫn viên (HuongDanVien-SuaLai.png)"},{value:"DoAn-SuaLai.png",label:"Ăn uống (DoAn-SuaLai.png)"},{value:"NuocUong.png",label:"Nước uống (NuocUong.png)"},{value:"NuocTam-SuaLai_20260113000157.png",label:"Tắm nước nóng (NuocTam...)"},{value:"GayTrekking.png",label:"Gậy trekking (GayTrekking.png)"},{value:"HuyChuong.png",label:"Huy chương (HuyChuong.png)"},{value:"AoMua.png",label:"Áo mưa (AoMua.png)"},{value:"BaoHiem.png",label:"Bảo hiểm (BaoHiem.png)"}],V=[{value:"NMG_giay.png",label:"Giày Trekking (NMG_giay.png)"},{value:"NMG_balo.png",label:"Balo Trekking (NMG_balo.png)"},{value:"NMG_quanao.png",label:"Trang phục (NMG_quanao.png)"},{value:"NMG_dungcu.png",label:"Dụng cụ cá nhân (NMG_dungcu.png)"}],I=(e={dayTitle:"",steps:[]},t)=>{const n=document.createElement("div");n.className="day-block bg-gray-50 p-4 rounded-xl border border-gray-200 space-y-3 relative",n.dataset.index=t,n.innerHTML=`
        ...
        `;const o=n.querySelector(".times-container");return(e.steps||[]).forEach((l,p)=>{o.appendChild(L(l))}),n.querySelector(".add-time-btn").addEventListener("click",()=>{o.children.length,o.appendChild(L({time:"",desc:""}))}),n.querySelector(".remove-day-btn").addEventListener("click",()=>{n.remove()}),n},L=(e={time:"",desc:""},t,n)=>{const o=document.createElement("div");return o.className="time-row flex gap-2 items-center",o.innerHTML=`
        ...
        `,o.querySelector(".remove-time-btn").addEventListener("click",()=>{o.remove()}),o},M=(e={title:"",desc:"",image:"XeTrungChuyen.png"})=>{const t=document.createElement("div");t.className="inclusion-row flex gap-2 items-center bg-gray-50 p-2.5 rounded-xl border border-gray-150";const n=T.map(l=>`<option value="${l.value}" ${e.image===l.value?"selected":""}>${l.label}</option>`).join("");t.innerHTML=`
            <select class="incl-image-select input-field bg-white text-xs w-44 shrink-0">
                ${n}
                <option value="custom" ${!T.some(l=>l.value===e.image)&&e.image?"selected":""}>Tự nhập path...</option>
            </select>
            <input type="text" class="incl-image-custom input-field bg-white text-xs w-32 shrink-0 ${!T.some(l=>l.value===e.image)&&e.image?"":"hidden"}" value="${e.image||""}" placeholder="đường_dẫn_ảnh.png">
        ...
        `,t.querySelector(".remove-faq-btn").addEventListener("click",()=>{t.remove()}),t},S=(e=[])=>{const t=document.getElementById("inclusions-container");t.innerHTML="",e.forEach(n=>t.appendChild(M(n)))},N=(e=[])=>{const t=document.getElementById("exclusions-container");t.innerHTML="",e.forEach(n=>t.appendChild(q(n)))},j=(e=[])=>{const t=document.getElementById("preparing-container");t.innerHTML="",e.forEach(n=>t.appendChild($(n)))},H=(e=[])=>{const t=document.getElementById("faqs-container");t.innerHTML="",e.forEach(n=>t.appendChild(C(n)))},A=(e=[])=>{const t=document.getElementById("days-container");t.innerHTML="",e.forEach((n,o)=>t.appendChild(I(n,o)))},P=(e=null)=>{const t=document.getElementById("tourModalTitle");if(e){t.textContent="Chỉnh Sửa Tour",document.getElementById("tour-edit-id").value=e.id,document.getElementById("tour-name").value=e.name||"",document.getElementById("tour-short-desc").value=e.short_desc||e.shortDesc||"",v("main",e.image||""),v("2",e.image2||""),v("3",e.image3||""),v("4",e.image4||""),document.getElementById("tour-duration").value=e.duration||"",document.getElementById("tour-price").value=e.price||"",document.getElementById("tour-level").value=e.level||"Trung Bình",document.getElementById("tour-region").value=e.region||"Miền Nam",document.getElementById("tour-type").value=e.type||"TREKKING",document.getElementById("tour-altitude").value=e.altitude||"",document.getElementById("tour-sort-order").value=e.sort_order||0,document.getElementById("tour-custom-domain").value=e.custom_domain||"",document.getElementById("tour-is-visible").checked=e.is_visible!==!1;const n=e.specs||{};document.getElementById("tour-location").value=n.location||"",document.getElementById("tour-distance").value=n.distance||"",document.getElementById("tour-transport").value=n.transport||"",document.getElementById("tour-groupsize").value=n.groupSize||"",A(e.itinerary||[]),S(e.inclusions||[]),N(e.exclusions||[]),j(e.preparing||[]),H(e.faqs||[])}else t.textContent="Thêm Tour Mới",E.reset(),document.getElementById("tour-edit-id").value="",document.getElementById("tour-is-visible").checked=!0,document.getElementById("tour-custom-domain").value="https://camsiteretreats.com/tour/",["main","2","3","4"].forEach(n=>v(n,"")),document.getElementById("tour-location").value="",document.getElementById("tour-distance").value="",document.getElementById("tour-transport").value="",document.getElementById("tour-groupsize").value="",A([]),S([]),N([]),j([]),H([]);X(),g.classList.remove("hidden"),setTimeout(()=>{g.classList.add("opacity-100"),y.classList.remove("scale-95"),y.classList.add("scale-100")},10),O()},w=()=>{g.classList.remove("opacity-100"),y.classList.remove("scale-100"),y.classList.add("scale-95"),setTimeout(()=>{g.classList.add("hidden")},200)};document.getElementById("addTourBtn").addEventListener("click",()=>P()),document.getElementById("closeTourModalBtn").addEventListener("click",w),document.getElementById("cancelTourBtn").addEventListener("click",w),g.addEventListener("click",e=>{e.target===g&&w()}),document.getElementById("add-day-btn").addEventListener("click",()=>{const e=document.querySelectorAll(".day-block").length;document.getElementById("days-container").appendChild(I({dayTitle:"",steps:[]},e))}),document.getElementById("add-inclusion-btn").addEventListener("click",()=>{document.getElementById("inclusions-container").appendChild(M())}),document.getElementById("add-exclusion-btn").addEventListener("click",()=>{document.getElementById("exclusions-container").appendChild(q())}),document.getElementById("add-prep-btn").addEventListener("click",()=>{document.getElementById("preparing-container").appendChild($())}),document.getElementById("add-faq-btn").addEventListener("click",()=>{document.getElementById("faqs-container").appendChild(C())}),G();const z=["main","2","3","4"],K=(e,t)=>{const n=document.getElementById(`tour-img-preview-${e}`),o=document.getElementById(`tour-img-overlay-${e}`);document.getElementById(`tour-img-zone-${e}`),n&&(n.innerHTML=`<img src="${t}" class="w-full h-full object-cover" alt="preview">`,n.className="w-full h-full",o&&o.classList.remove("hidden"))},v=(e,t)=>{const n=document.getElementById(`tour-img-preview-${e}`),o=document.getElementById(`tour-img-overlay-${e}`),r=document.getElementById(e==="main"?"tour-image":`tour-image${e}`);if(n){if(t)n.innerHTML=`<img src="${t}" class="w-full h-full object-cover" alt="preview">`,n.className="w-full h-full",o&&o.classList.remove("hidden");else{const l=e==="main";n.innerHTML=l?'<svg class="w-8 h-8" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg><span class="text-xs font-bold uppercase">Kéo thả hoặc click để chọn ảnh</span><span class="text-[10px] text-gray-400">JPG, PNG, WEBP — tối đa 10MB, tự động nén</span>':'<svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 4v16m8-8H4"/></svg><span class="text-[9px] font-bold uppercase">Thêm ảnh</span>',n.className="w-full h-full flex flex-col items-center justify-center text-gray-300 gap-1",o&&o.classList.add("hidden")}r&&(r.value=t||"")}},R=e=>new Promise((t,n)=>{const r=new FileReader;r.onload=l=>{const p=new Image;p.onload=()=>{let{width:s,height:u}=p;(s>1200||u>1200)&&(s>u?(u=Math.round(u*1200/s),s=1200):(s=Math.round(s*1200/u),u=1200));const b=document.createElement("canvas");b.width=s,b.height=u,b.getContext("2d").drawImage(p,0,0,s,u),t(b.toDataURL("image/jpeg",.85))},p.onerror=n,p.src=l.target.result},r.onerror=n,r.readAsDataURL(e)}),_=async(e,t)=>{const n=document.getElementById(`tour-img-preview-${t}`),o=document.getElementById(t==="main"?"tour-image":`tour-image${t}`);if(n){n.innerHTML='<svg class="animate-spin w-7 h-7 text-csr-orange" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"/><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"/></svg>',n.className="w-full h-full flex items-center justify-center";try{const r=await R(e),l=(e.name||`tour-${t}`).replace(/\.[^.]+$/,""),s=await(await fetch("/api/upload",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({image:r,filename:`tour-${l}`})})).json();if(!s.success)throw new Error(s.error||"Upload thất bại");K(t,s.url),o&&(o.value=s.url),console.log(`[tour-upload] ✅ slot=${t} ${s.stats?.inputKB}KB → ${s.stats?.outputKB}KB`)}catch(r){n.innerHTML=`<span class="text-red-400 text-xs text-center p-2">❌ Lỗi: ${r.message}</span>`,n.className="w-full h-full flex items-center justify-center",alert("Lỗi upload ảnh: "+r.message)}}},O=()=>{z.forEach(e=>{const t=document.getElementById(`tour-img-input-${e}`);!t||t.dataset.bound||(t.dataset.bound="1",t.addEventListener("change",n=>{const o=n.target.files?.[0];o&&_(o,e)}))})};window.handleTourImgDrop=(e,t)=>{const n=e.dataTransfer?.files?.[0];n&&n.type.startsWith("image/")&&_(n,t)};const B=async()=>{i.innerHTML='<tr><td colspan="7" class="text-center py-8 text-gray-400">Đang tải danh sách tour...</td></tr>';try{const e=await fetch(f);if(!e.ok)throw new Error("Failed to load tours");const t=await e.json();x=Array.isArray(t)?t:t.data||[],F()}catch(e){console.error("Error loading tours:",e),i.innerHTML='<tr><td colspan="7" class="text-center py-4 text-red-500">Lỗi kết nối server.</td></tr>'}},F=()=>{if(x.length===0){i.innerHTML='<tr><td colspan="7" class="text-center py-8 text-gray-400">Chưa có tour nào.</td></tr>';return}i.innerHTML=x.map(e=>{const t=!e.price||e.price==="Update"||e.price==="0"||parseInt(e.price)===0?'<span class="text-gray-400 italic">Update...</span>':`<span class="font-bold text-csr-orange">${parseInt(e.price).toLocaleString("vi-VN")}đ</span>`,n=e.level==="Dễ"?"bg-green-100 text-green-700":e.level==="Khó"?"bg-red-100 text-red-600":"bg-orange-100 text-orange-700",o=e.is_visible!==!1?'<span class="bg-green-100 text-green-600 text-[10px] px-2 py-0.5 rounded-full font-bold">Hiển thị</span>':'<span class="bg-gray-200 text-gray-500 text-[10px] px-2 py-0.5 rounded-full font-bold">Đã ẩn</span>';return`
            ...
            `}).join("")};i.addEventListener("click",async e=>{const t=e.target.closest(".tour-edit-btn"),n=e.target.closest(".tour-delete-btn");if(t){const o=parseInt(t.getAttribute("data-id")),r=x.find(l=>l.id===o);r&&P(r)}if(n){const o=n.getAttribute("data-id");if(confirm("Dữ liệu lịch trình liên quan có thể bị ảnh hưởng. Bạn vẫn muốn xóa tour này?"))try{if((await fetch(`${f}?id=${o}`,{method:"DELETE"})).ok)B();else throw new Error("Delete failed")}catch(r){alert("Lỗi khi xóa: "+r.message)}}}),E.addEventListener("submit",async e=>{e.preventDefault();const t=E.querySelector('button[type="submit"]'),n=t.textContent;t.textContent="Đang lưu...",t.disabled=!0;const o=document.getElementById("tour-edit-id").value,r=[];document.querySelectorAll(".day-block").forEach(a=>{const d=a.querySelector(".day-title-input").value.trim(),c=[];a.querySelectorAll(".time-row").forEach(m=>{const h=m.querySelector(".time-input").value.trim(),k=m.querySelector(".desc-input").value.trim();(h||k)&&c.push({time:h,desc:k})}),d&&r.push({dayTitle:d,steps:c})});const l=[];document.querySelectorAll(".inclusion-row").forEach(a=>{const d=a.querySelector(".incl-image-select").value,c=a.querySelector(".incl-image-custom").value.trim(),m=d==="custom"?c:d,h=a.querySelector(".incl-title-input").value.trim(),k=a.querySelector(".incl-desc-input").value.trim();h&&l.push({title:h,desc:k,image:m})});const p=[];document.querySelectorAll(".exclusion-row").forEach(a=>{const d=a.querySelector(".excl-title-input").value.trim(),c=a.querySelector(".excl-desc-input").value.trim();d&&p.push({title:d,desc:c})});const s=[];document.querySelectorAll(".preparing-row").forEach(a=>{const d=a.querySelector(".prep-image-select").value,c=a.querySelector(".prep-title-input").value.trim(),m=a.querySelector(".prep-desc-input").value.trim();c&&s.push({title:c,desc:m,image:d})});const u=[];document.querySelectorAll(".faq-row").forEach(a=>{const d=a.querySelector(".faq-q-input").value.trim(),c=a.querySelector(".faq-a-input").value.trim();d&&u.push({q:d,a:c})});const b={location:document.getElementById("tour-location").value.trim()||null,distance:document.getElementById("tour-distance").value.trim()||null,transport:document.getElementById("tour-transport").value.trim()||null,groupSize:document.getElementById("tour-groupsize").value.trim()||null},U={id:o?parseInt(o):null,name:document.getElementById("tour-name").value,image:document.getElementById("tour-image").value,image2:document.getElementById("tour-image2").value||null,image3:document.getElementById("tour-image3").value||null,image4:document.getElementById("tour-image4").value||null,shortDesc:document.getElementById("tour-short-desc").value,altitude:document.getElementById("tour-altitude").value||null,level:document.getElementById("tour-level").value,region:document.getElementById("tour-region").value,type:document.getElementById("tour-type").value,duration:document.getElementById("tour-duration").value,price:document.getElementById("tour-price").value,sort_order:parseInt(document.getElementById("tour-sort-order").value)||0,custom_domain:document.getElementById("tour-custom-domain").value.trim()||null,is_visible:document.getElementById("tour-is-visible").checked,specs:b,itinerary:r,inclusions:l,exclusions:p,preparing:s,faqs:u};try{const a=o?`${f}?id=${o}`:f,c=await fetch(a,{method:o?"PUT":"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(U)});if(!c.ok){const m=await c.json();throw new Error(m.message||m.error||"Save failed")}w(),B(),alert(o?"✅ Cập nhật tour thành công!":"✅ Thêm tour mới thành công! Website sẽ tự đồng bộ trong vài giây.")}catch(a){alert("❌ Lỗi: "+a.message)}finally{t.textContent=n,t.disabled=!1}}),B()};export{Y as afterRender,J as render};
