import{S as C,H as $}from"./Header-CamAZjvb.js";const F=()=>`
    ...
    `,j=()=>{let x=[],h="",u="upcoming";const v="/api/schedules",E="/api/tours",g=document.getElementById("scheduleList"),c=document.getElementById("scheduleModal"),p=document.getElementById("scheduleModalContent"),k=document.getElementById("scheduleForm"),L=(e=null)=>{const s=document.getElementById("scheduleModalTitle");if(e){s.textContent="Chỉnh Sửa Lịch Trình",document.getElementById("sch-edit-id").value=e.id,document.getElementById("sch-tour-name").value=e.tour_name;const t=n=>n&&n.includes("T")?n.split("T")[0]:n;document.getElementById("sch-start-date").value=t(e.start_date),document.getElementById("sch-end-date").value=t(e.end_date),document.getElementById("sch-slots").value=e.slots,document.getElementById("sch-status").value=e.status||"Đang mở",document.getElementById("sch-group-label").value=e.group_label||"",document.getElementById("sch-zalo-link").value=e.zalo_link||""}else s.textContent="Thêm Lịch Khởi Hành",k.reset(),document.getElementById("sch-edit-id").value="",document.getElementById("sch-zalo-link").value="";c.classList.remove("hidden"),setTimeout(()=>{c.classList.add("opacity-100"),p.classList.remove("scale-95"),p.classList.add("scale-100")},10)},m=()=>{c.classList.remove("opacity-100"),p.classList.remove("scale-100"),p.classList.add("scale-95"),setTimeout(()=>{c.classList.add("hidden")},200)};document.getElementById("addScheduleBtn").addEventListener("click",()=>L()),document.getElementById("closeScheduleModalBtn").addEventListener("click",m),document.getElementById("cancelScheduleBtn").addEventListener("click",m),c.addEventListener("click",e=>{e.target===c&&m()});const B=async()=>{try{const e=await fetch(E);if(!e.ok)throw new Error("Failed to load tours");const s=await e.json(),t=document.getElementById("sch-tour-name");t.innerHTML='<option value="">-- Chọn Tour --</option>',s.forEach(a=>{t.innerHTML+=`<option value="${a.name}">${a.name}</option>`});const n=document.getElementById("tourFilterList");let o=`
                <label class="flex items-center gap-3 cursor-pointer group">
                    <input type="radio" name="tourFilter" value="" class="text-csr-orange focus:ring-csr-orange border-gray-200 bg-gray-100" checked>
                    <span class="text-gray-500 group-hover:text-gray-900 transition-colors text-sm">Tất cả Tuyến</span>
                </label>
            ...
            `}).join(""),g.addEventListener("click",T)},T=async e=>{const s=e.target.closest(".sch-edit-btn"),t=e.target.closest(".sch-delete-btn"),n=e.target.closest(".sch-review-btn"),o=e.target.closest(".sch-details-btn");if(s){const a=s.getAttribute("data-id"),r=x.find(l=>l.id==a);r&&L(r)}if(t){const a=t.getAttribute("data-id");if(confirm("Bạn có chắc chắn muốn xóa lịch trình này?"))try{if((await fetch(`${v}?id=${a}`,{method:"DELETE"})).ok)y();else throw new Error("Delete failed")}catch(r){alert("Lỗi khi xóa: "+r.message)}}if(n){const a=n.getAttribute("data-id"),r=`${window.location.origin}/review?schedule_id=${a}`;navigator.clipboard.writeText(r).then(()=>{const l=n.innerHTML;n.innerHTML="✅ Đã copy!",setTimeout(()=>n.innerHTML=l,2e3)}).catch(()=>{alert(`Không thể copy. Link:
`+r)})}if(o){const a=o.getAttribute("data-id"),r=o.getAttribute("data-tour"),l=o.getAttribute("data-date"),d=l&&l.includes("T")?l.split("T")[0]:l,b=item.group_label||"",w=`/admin/roster?tour=${encodeURIComponent(r)}&date=${encodeURIComponent(d)}&scheduleId=${a}&groupLabel=${encodeURIComponent(b)}`;history.pushState(null,null,w),window.dispatchEvent(new Event("popstate"))}};k.addEventListener("submit",async e=>{e.preventDefault();const s=e.target.querySelector('button[type="submit"]'),t=s.textContent;s.textContent="Đang lưu...",s.disabled=!0;const n=document.getElementById("sch-edit-id").value,o={id:n?parseInt(n):null,tour_name:document.getElementById("sch-tour-name").value,start_date:document.getElementById("sch-start-date").value,end_date:document.getElementById("sch-end-date").value,slots:parseInt(document.getElementById("sch-slots").value),status:document.getElementById("sch-status").value,group_label:document.getElementById("sch-group-label").value.trim()||null,zalo_link:document.getElementById("sch-zalo-link").value.trim()||null};try{if(!(await fetch(v,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(o)})).ok)throw new Error("Failed to save schedule");m(),y(),alert(n?"✅ Cập nhật lịch trình thành công!":"✅ Thêm lịch trình mới thành công! Form đặt chỗ trên website sẽ tự cập nhật trong 30 giây.")}catch(a){alert("❌ Lỗi: "+a.message)}finally{s.textContent=t,s.disabled=!1}}),B(),y()};export{j as afterRender,F as render};
