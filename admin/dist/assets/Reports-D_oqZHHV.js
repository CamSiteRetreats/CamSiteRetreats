import{S as R,H as F}from"./Header-CARqqxjt.js";const V=()=>{let f={role:"sale",fullName:"Guest",id:null};try{const y=localStorage.getItem("csr_user");y&&(f=JSON.parse(y))}catch{}const T=f.role==="admin";return`
    ...
    `},G=()=>{let f={role:"sale",fullName:"Guest",id:null};try{const t=localStorage.getItem("csr_user");t&&(f=JSON.parse(t))}catch{}const T=f.role==="admin";let y=[],S=[],L=[],x=null,u=new Set,k=[];const b=t=>new Intl.NumberFormat("vi-VN").format(Math.round(t||0))+"đ",M=t=>{if(!t)return 0;try{const s=Array.isArray(t)?t:JSON.parse(t);if(Array.isArray(s))return s.reduce((e,n)=>e+(parseInt(n.price)||0),0)}catch{return(String(t).match(/price:\s*(\d+)/g)||[]).reduce((n,o)=>n+Number(o.replace(/price:\s*/,"")),0)}return 0},C=(t,s)=>{const e=s.find(c=>c.name&&t.tour&&c.name.toLowerCase().trim()===t.tour.toLowerCase().trim()),n=parseFloat(t.commission_rate??e?.commission_rate??5),o=M(t.services_booked),d=Math.max(0,(parseInt(t.total_price)||0)-o),i=d*(n/100);return{rate:n,servicesTotal:o,basePrice:d,commission:i}},P=t=>{if(!t)return null;const s=t.split("-")[0].trim();if(s.includes("/")){const e=s.split("/");if(e.length===3)return new Date(parseInt(e[2]),parseInt(e[1])-1,parseInt(e[0]));if(e.length===2)return new Date(new Date().getFullYear(),parseInt(e[1])-1,parseInt(e[0]))}return s.match(/^\d{4}-/)?new Date(s):null},H=new Date;H.setHours(0,0,0,0);const $=t=>t.status&&(t.status.includes("Đã cọc")||t.status.includes("Hoàn tất")||t.status.includes("Hoàn thành")),w=t=>{if(!(t.status&&(t.status.includes("Hoàn tất")||t.status.includes("Hoàn thành"))))return!1;const e=P(t.date);return e&&e<H},B=async()=>{try{const[t,s,e]=await Promise.all([fetch("/api/admin_tours"),fetch("/api/bookings"),fetch("/api/users")]),n=t.ok?await t.json():{};y=Array.isArray(n)?n:n.data||[],S=s.ok?await s.json():[],L=e.ok?await e.json():[],T?(A(),_()):j()}catch(t){console.error("Lỗi tải dữ liệu báo cáo:",t)}},A=()=>{const t=document.getElementById("commissionSetupContainer");if(!t)return;if(y.length===0){t.innerHTML='<div class="text-sm text-gray-500 text-center py-4">Không có Tour nào.</div>';return}const s=y.map(e=>{const n=e.commission_rate??5;return`
        ...
        `},E=()=>{const t=document.getElementById("tabContentPayHistory");if(t){if(k.length===0){t.innerHTML='<div class="p-8 text-center text-gray-400 text-sm">Chưa có lần thanh toán nào cho nhân viên này.</div>';return}t.innerHTML=`
            <table class="w-full text-sm text-left border-collapse">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <div class="overflow-x-auto">
                        ...
                        </table>`}catch{h.innerHTML='<div class="p-6 text-center text-gray-400 text-sm">Lỗi tải lịch sử.</div>'}};document.getElementById("saleDetailPanel")?.addEventListener("click",t=>{t.target.id==="saleDetailPanel"&&window.closeSalePanel()}),B()};export{G as afterRender,V as render};
