import{S as _e,H as je}from"./Header-CtRUqTZE.js";const Ne=()=>`
    ...
    `,He=async()=>{console.log("[Roster] VERSION 3.2 LOADED - READY TO EDIT");const de=t=>{const e=(t||"").toLowerCase();return e.includes("hoàn tất")||e.includes("hoàn thành")||e.includes("đã thanh toán")||e.includes("thanh toán")?`<span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold bg-emerald-100 text-emerald-700 border border-emerald-200">
        ...
        `).join("")},ye=()=>{E.forEach(t=>{const e=C.has(t.id),s=document.querySelector(`th[data-col="${t.id}"]`);s&&(e?s.classList.remove("col-hidden"):s.classList.add("col-hidden")),document.querySelectorAll(`td[data-col="${t.id}"]`).forEach(a=>{e?a.classList.remove("col-hidden"):a.classList.add("col-hidden")})})},V=t=>{if(!t)return null;t.includes(" - ")&&(t=t.split(" - ")[0].trim()),t.includes("T")&&(t=t.split("T")[0]);const e=t.split(/[-/.]/);return e.length===2?{day:parseInt(e[0]),month:parseInt(e[1]),year:null}:e.length===3?e[0].length===4?{day:parseInt(e[2]),month:parseInt(e[1]),year:parseInt(e[0])}:{day:parseInt(e[0]),month:parseInt(e[1]),year:parseInt(e[2])}:null},B=V(S),we=t=>{if(!B)return!0;const e=V(t);return!(!e||e.day!==B.day||e.month!==B.month||e.year&&B.year&&e.year!==B.year)},P=async()=>{try{if(_)try{const n=await(await fetch(`/api/guides?schedule_id=${_}`)).json();if(n&&n.length>0){const r=document.getElementById("rosterGuidesSection"),o=document.getElementById("rosterGuidesList"),c={"Hướng Dẫn Viên":"bg-blue-100 text-blue-700 border-blue-200",Leader:"bg-purple-100 text-purple-700 border-purple-200",Photo:"bg-pink-100 text-pink-700 border-pink-200",Guider:"bg-cyan-100 text-cyan-700 border-cyan-200","Hậu Cần":"bg-amber-100 text-amber-700 border-amber-200","Học Việc":"bg-gray-100 text-gray-600 border-gray-200"};r.classList.remove("hidden"),o.innerHTML=n.map(d=>`
                                <div class="flex items-center gap-2 bg-white px-3 py-2 rounded-xl border border-gray-200 shadow-sm print:shadow-none print:border-gray-400">
                                    <span class="text-[10px] font-bold px-2 py-0.5 rounded-full border ${c[d.role]||"bg-gray-100 text-gray-600 border-gray-200"} uppercase tracking-wide print:border-black print:text-black">${d.role}</span>
                            ...
                            `).join("")}}catch(a){console.error("Error loading guides:",a)}const e=await(await fetch("/api/bookings")).json();console.log("[Roster] targetTour:",k,"targetDateStr:",S,"targetParsed:",B),console.log("[Roster] Total bookings:",e.length),e.length>0&&e.filter(n=>n.tour&&n.tour.includes("Liêng")).slice(0,3).forEach(n=>console.log("[Roster] Sample Liêng:",{tour:n.tour,date:n.date,parsed:V(n.date)})),v=e.filter(a=>{if(_){if(String(a.schedule_id)===String(_))return!0;if(!(!pe&&!a.schedule_id))return!1}let n=!0,r=!0;if(k){const o=Y(a.tour),c=Y(k);n=o===c||o.includes(c)||c.includes(o)}return S&&(r=we(a.date)),n&&r}),console.log("[Roster] Matched bookings:",v.length);const s=document.getElementById("filterSort")?.value||"newest";s==="newest"?v.sort((a,n)=>parseInt(n.id)-parseInt(a.id)):v.sort((a,n)=>parseInt(a.id)-parseInt(n.id)),console.log(`[Roster] Sorted bookings (${s}):`,v.map(a=>a.id)),$()}catch(t){console.error("Lỗi khi tải dữ liệu đoàn:",t),q.innerHTML='<tr><td colspan="12" class="p-8 text-center text-red-500 font-bold">Lỗi khi tải dữ liệu. Vui lòng tải lại trang.</td></tr>'}},$=()=>{const t=G.value.toLowerCase(),e=O.value.toLowerCase(),s=document.getElementById("filterSearch")?.value?.toLowerCase()||"",a=v.filter(r=>{if(s&&!`${r.name} ${r.phone} ${r.address} ${r.medal_name} ${r.customer_id}`.toLowerCase().includes(s))return!1;const o=(r.status||"").toLowerCase();if(!(t===""||t==="chờ cọc"&&(o.includes("chờ cọc")||o.includes("chờ xác nhận"))||t==="đã cọc"&&(o.includes("đã cọc")||o.includes("hoàn tất")||o.includes("hoàn thành"))||t==="đã hủy"&&o.includes("hủy")))return!1;const d=(r.gender||"").toLowerCase();return e===""||d===e}),n=a.filter(r=>r.status!=="Đã hủy"&&r.status!=="Bảo lưu");if(document.getElementById("text-visible-count").textContent=n.length,fe.textContent=n.length,a.length===0){q.innerHTML='<tr><td colspan="12" class="p-12 text-center text-gray-500 bg-gray-50">Không tìm thấy khách hàng nào khớp với điều kiện lọc.</td></tr>';return}q.innerHTML=a.map((r,o)=>{const c=o%2===0,i=r.status==="Đã hủy"||r.status==="Bảo lưu"?"bg-gray-100 opacity-60":c?"bg-white":"bg-gray-50/40";return`<tr onclick="if(!event.target.closest('.seat-badge') && !event.target.closest('a')) { if(window.actionEdit) window.actionEdit('${r.id}'); else alert('Chưa tải xong logic chỉnh sửa!'); }" class="${i} hover:bg-amber-50/40 transition-colors border-b border-gray-100 group row-clickable cursor-pointer" data-booking-id="${r.id}">`+E.map(l=>`
                    <td class="px-4 py-3 align-middle ${l.id==="col-extra"?"min-w-[220px]":""} ${C.has(l.id)?"":"col-hidden"}" data-col="${l.id}">
                        ${l.render(r,o)}
                    </td>
                `).join("")+"</tr>"}).join("")},ke=()=>{const t=E.filter(l=>C.has(l.id)),e=G.value.toLowerCase(),s=O.value.toLowerCase(),a=document.getElementById("filterSearch")?.value?.toLowerCase()||"",n=v.filter(l=>{if(a&&!`${l.name} ${l.phone} ${l.address} ${l.medal_name} ${l.customer_id}`.toLowerCase().includes(a))return!1;const g=(l.status||"").toLowerCase();if(!(e===""||e==="chờ cọc"&&(g.includes("chờ cọc")||g.includes("chờ xác nhận"))||e==="đã cọc"&&(g.includes("đã cọc")||g.includes("hoàn tất")||g.includes("hoàn thành"))||e==="đã hủy"&&g.includes("hủy")))return!1;const m=(l.gender||"").toLowerCase();return s===""||m===s});let r=`
            ...
            <body>
                <table>
                    ...
                    <tbody>
        `;n.forEach((l,g)=>{r+="<tr>",t.forEach(p=>{let m="";if(p.id==="col-stt")m=g+1;else if(p.id==="col-name")m=l.name||"";else if(p.id==="col-order-id")m=l.id||"";else if(p.id==="col-address")m=l.address||"";else if(p.id==="col-phone")m=l.phone?String(l.phone):"";else if(p.id==="col-diet")m=l.diet==="Ăn chay"||l.diet==="Chay"||l.diet==="Có"?"Có":"Không";else if(p.id==="col-pole")m=l.trekking_pole==="Có"?"Có":"Không";else if(p.id==="col-status")m=l.status||"";else if(p.id==="col-extra"){const h=l.services_booked;if(h&&h!=="[]"&&h!=="null"&&h!==""){const b=f=>f.map(x=>{const w=x.label||x.name||x.title||"",M=x.price&&Number(x.price)>0?` (+${Number(x.price).toLocaleString("vi-VN")}đ)`:"";return(w+M).trim()}).filter(Boolean).join(", ");if(Array.isArray(h))m=b(h);else if(typeof h=="string"&&h.trim().startsWith("["))try{m=b(JSON.parse(h))}catch{const x=h.match(/label:\s*['"]([^'"]+)['"]/g)||[],w=h.match(/price:\s*(\d+)/g)||[];x.length>0&&(m=x.map((M,N)=>{const T=M.replace(/label:\s*['"]/,"").replace(/['"]$/,"").trim(),H=w[N]?Number(w[N].replace(/price:\s*/,"")):0;return(T+(H>0?` (+${H.toLocaleString("vi-VN")}đ)`:"")).trim()}).filter(Boolean).join(", "))}}m||(m=l.extra_services||"")}else{const h=String(p.render(l,g)),b=document.createElement("DIV");b.innerHTML=h,m=b.textContent||b.innerText||""}const u=p.id==="col-phone"||p.id==="col-cccd";r+=`<td ${u?'class="num"':""}>${m}</td>`}),r+="</tr>"}),r+=`
                    </tbody>
                </table>
            </body>
            </html>
        `;const o=new Blob(["\uFEFF",r],{type:"application/vnd.ms-excel;charset=utf-8"}),c=URL.createObjectURL(o),d=`DS_Doan_${k||"Tour"}_${L||"Date"}.xls`.replace(/[/\\?%*:|"<>]/g,"-"),i=document.createElement("a");i.setAttribute("href",c),i.setAttribute("download",d),document.body.appendChild(i),i.click(),document.body.removeChild(i)},Q={solati_16:{label:"🚐 Solati 16 chỗ",totalSeats:15},limo_34:{label:"🚌 Limousine 34 chỗ",totalSeats:34},sleeper_45:{label:"🛏️ Giường Nằm 45 chỗ",totalSeats:46}},Ee=t=>{if(t==="limo_34")return{type:"limo",floors:[{label:null,rows:[[{n:"C1"},{n:"C2"},null,{n:"B1"},{n:"B2"},null,{n:"A1"},{n:"A2"},{n:"LÁI",driver:!0}],[{n:"C3"},{n:"C4"},null,{n:"B3"},{n:"B4"},null,{n:"A3"},{n:"A4"}],[{n:"C5"},{n:"C6"},null,{n:"B5"},{n:"B6"},null,{n:"A5"},{n:"A6"}],[{n:"C7"},{n:"C8"},null,{n:"B7"},{n:"B8"},null,{n:"A7"},{n:"A8"}],[{n:"C9"},{n:"C10"},null,{n:"B9"},{n:"B10"},null,{n:"A9"},{n:"A10"}],[{n:"C11"},{n:"C12"},null,null,null,null,{n:"A11"},{n:"A12"}]]}],legend:['<span class="font-bold text-gray-600">Dãy C</span> (cửa lên)','<span class="font-bold text-gray-600">Dãy B</span> (giữa)','<span class="font-bold text-gray-600">Dãy A</span> (tài xế)']};if(t==="sleeper_45"){const e=(s,a,n)=>{const r=Array.from({length:n-a+1},(c,d)=>`${s}${a+d}`),o=[];for(let c=0;c<r.length;c+=4){const d=[];for(let i=0;i<4;i++)i===2&&d.push(null),d.push(r[c+i]?{n:r[c+i]}:null);o.push(d)}return o};return{type:"sleeper",floors:[{label:"🏠 Tầng Dưới",floorKey:"down",rows:e("A",1,23)},{label:"🏥 Tầng Trên",floorKey:"up",rows:e("B",1,23)}],legend:null}}return{type:"solati",floors:[{label:null,rows:[[{n:"LÁI",driver:!0},null,{n:1},{n:2}],[{n:3},{n:4},null,{n:5}],[{n:6},{n:7},null,{n:8}],[{n:9},{n:10},null,{n:11}],[{n:12},{n:13},{n:14},{n:15}]]}],legend:null}},Z=Q[X]||Q.solati_16;let K=0;const j=()=>{const t=document.getElementById("seatMapGrid");if(!t)return;const e={};v.forEach(i=>{const l=i.status==="Đã hủy"||i.status==="Bảo lưu";i.seat_number&&!l&&(e[String(i.seat_number)]=i)});const s=Object.keys(e).length,a=Z.totalSeats,n=document.getElementById("seat-map-count"),r=document.getElementById("seat-count-badge"),o=document.getElementById("vehicle-label");n&&(n.textContent=`${s}/${a}`),r&&(r.textContent=`💺 ${s}/${a} ghế đã xếp`),o&&(o.textContent=Z.label);const c=Ee(X),d=(i,l)=>{if(!i)return'<div class="w-12 h-10 flex-shrink-0"></div>';if(i.driver)return'<div class="seat-btn seat-driver w-10 h-9 rounded-lg flex flex-col items-center justify-center text-[8px] font-bold flex-shrink-0"><span>🚗</span><span>LÁI</span></div>';const g=String(i.n),p=l[g],m=p?(p.name||"").split(" ").pop().slice(0,4):"",h=/[A-Z]/.test(g)?"w-11 h-9":"w-12 h-10";return`<button
                ...
                <div class="floor-content">
                    ...
                    `).join("")}
                </div>`,t.querySelectorAll(".floor-tab").forEach(i=>{i.addEventListener("click",()=>{K=parseInt(i.dataset.floor),j()})});else{let i="";c.legend&&(i+=`<div class="flex justify-around text-[9px] text-gray-400 mb-2 px-1">${c.legend.map(l=>`<span>${l}</span>`).join("")}</div>`),c.floors[0].rows.forEach(l=>{const g=l.map(p=>d(p,e)).join("");i+=`<div class="flex items-center justify-center gap-1 mb-1">${g}</div>`}),t.innerHTML=i}t.querySelectorAll(".seat-btn[data-seat]").forEach(i=>{i.addEventListener("click",()=>ee(i.dataset.seat,i.dataset.bookingId))})},ee=(t,e)=>{const s=document.getElementById("seatModal"),a=document.getElementById("seatModalTitle"),n=document.getElementById("seatModalBody");if(!s||!a||!n)return;a.textContent=e?`Ghế ${t} — ${v.find(d=>String(d.id)===String(e))?.name||"?"}`:`Ghế ${t} — Chưa có khách`;const r=v.find(d=>String(d.id)===String(e));let o="";r&&(o+=`<div class="mb-3 p-3 bg-orange-50 border border-orange-200 rounded-xl text-sm">
            ...
            <div class="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Chọn khách khác:</div>`);const c=v.filter(d=>!d.seat_number||String(d.id)===String(e));c.length===0?o+='<p class="text-gray-400 text-sm text-center py-4">Tất cả khách đã được xếp ghế.</p>':o+=c.map(d=>`
                ...
                </button>`).join(""),n.innerHTML=o,s.style.display="flex",n.querySelector("#btnRemoveSeat")?.addEventListener("click",async()=>{await z(e,null),s.style.display="none"}),n.querySelectorAll(".btn-assign-passenger").forEach(d=>{d.addEventListener("click",async()=>{const i=d.dataset.bookingId,l=d.dataset.seat,g=v.find(p=>String(p.seat_number)===String(l)&&String(p.id)!==String(i));g&&await z(g.id,null),await z(i,l),s.style.display="none"})})},z=async(t,e)=>{try{await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:t,seat_number:e||null})});const s=v.find(a=>String(a.id)===String(t));s&&(s.seat_number=e||null),$(),j()}catch(s){console.error("Lỗi lưu ghế:",s),alert("Lỗi lưu số ghế. Vui lòng thử lại.")}};G.addEventListener("change",$),O.addEventListener("change",$),document.getElementById("filterSearch")?.addEventListener("input",$),document.getElementById("filterSort")?.addEventListener("change",P),document.getElementById("btnExportPDF").addEventListener("click",()=>window.print()),document.getElementById("btnExportExcel").addEventListener("click",()=>ke()),document.getElementById("seatModalClose")?.addEventListener("click",()=>{document.getElementById("seatModal").style.display="none"}),document.getElementById("seatModal")?.addEventListener("click",t=>{t.target===document.getElementById("seatModal")&&(document.getElementById("seatModal").style.display="none")}),document.getElementById("rosterList")?.addEventListener("mouseover",t=>{const e=t.target.closest("tr[data-booking-id]");if(!e)return;document.querySelectorAll(".seat-btn").forEach(n=>n.classList.remove("seat-highlighted"));const s=e.dataset.bookingId,a=v.find(n=>String(n.id)===String(s));a?.seat_number&&document.querySelector(`.seat-btn[data-seat="${a.seat_number}"]`)?.classList.add("seat-highlighted")}),document.getElementById("rosterList")?.addEventListener("click",t=>{const e=t.target.closest(".seat-badge");if(!e)return;const s=e.dataset.id,a=v.find(n=>String(n.id)===String(s));ee(a?.seat_number||"?",s)});let D=[],te=[],F=[];const Se=async()=>{try{const[t,e,s]=await Promise.all([fetch("/api/tours"),fetch("/api/schedules"),fetch("/api/users")]);D=t.ok?await t.json():[],te=e.ok?await e.json():[],F=s.ok?await s.json():[],Be("edit-tour"),Ce("edit-sale")}catch(t){console.error("Lỗi tải tours/schedules:",t)}},Ce=t=>{const e=document.getElementById(t);e&&(e.innerHTML='<option value="">-- Website --</option>',F.forEach(s=>{const a=document.createElement("option");a.value=s.id,a.textContent=`${s.full_name||s.fullName} (${s.role})`,e.appendChild(a)}))},Be=t=>{const e=document.getElementById(t);if(!e)return;const s=e.value,a=e.options[0]?.outerHTML||'<option value="">-- Chọn Tour --</option>';e.innerHTML=a,D.filter(n=>n.is_visible!==!1).forEach(n=>{const r=document.createElement("option");r.value=n.name,r.textContent=n.name,n.name===s&&(r.selected=!0),e.appendChild(r)})},ne=(t,e,s,a)=>{const n=document.getElementById(t);if(!n||(n.innerHTML='<option value="">-- Chọn Lịch --</option>',!e))return;const r=te.filter(o=>o.tour_name?o.tour_name===e||e.includes(o.tour_name)||o.tour_name.includes(e):!1);if(r.length===0){n.innerHTML='<option value="">Chưa có lịch cho tour này</option>';return}r.sort((o,c)=>new Date(o.start_date)-new Date(c.start_date)).forEach(o=>{const c=parseInt(o.booked_count)||0,d=(o.slots||0)-c,i=new Date(o.start_date),l=o.end_date?new Date(o.end_date):null,g=h=>`${h.getDate().toString().padStart(2,"0")}/${(h.getMonth()+1).toString().padStart(2,"0")}`;let p=g(i);l&&(p+=" - "+g(l)),o.group_label&&(p+=` · ${o.group_label}`),p+=` (${d>=0?d:0} chỗ trống)`;const m=document.createElement("option"),u=`${i.getDate().toString().padStart(2,"0")}/${(i.getMonth()+1).toString().padStart(2,"0")}/${i.getFullYear()}`;m.value=`${o.id}::${u}`,m.dataset.scheduleId=o.id,m.dataset.date=u,m.textContent=p,d<=0&&(m.disabled=!0,m.textContent=p.replace("chỗ trống","HẾT CHỖ")),(a&&String(o.id)===String(a)||!a&&s&&u===s)&&(m.selected=!0),n.appendChild(m)})},se=document.getElementById("edit-tour");se&&se.addEventListener("change",t=>{ne("edit-date",t.target.value)}),window.closeEditModal=()=>{document.getElementById("editModal").classList.remove("opacity-100");const t=document.getElementById("editModalContent");t&&(t.classList.remove("scale-100","translate-y-0"),t.classList.add("scale-95","translate-y-4")),setTimeout(()=>document.getElementById("editModal").classList.add("hidden"),300)},window.updateEditRemaining=()=>{const t=parseInt(document.getElementById("edit-total")?.value)||0,e=parseInt(document.getElementById("edit-discount")?.value)||0,s=parseInt(document.getElementById("edit-deposit")?.value)||0;let a=0;document.querySelectorAll(".preset-service-check:checked").forEach(c=>{a+=parseInt(c.dataset.price)||0}),document.querySelectorAll(".service-price-input").forEach(c=>{a+=parseInt(c.value)||0});const r=t-e+a-s,o=document.getElementById("edit-remaining");o&&(o.textContent=r>0?r.toLocaleString("vi-VN")+"đ":"0đ")},window.updateServicesTotal=()=>{let t=0;document.querySelectorAll(".preset-service-check:checked").forEach(a=>{t+=parseInt(a.dataset.price)||0}),document.querySelectorAll(".service-price-input").forEach(a=>{t+=parseInt(a.value)||0});const e=document.getElementById("servicesTotalDisplay");e&&(e.textContent=t.toLocaleString("vi-VN")+"đ");const s=document.getElementById("servicesTotalRow");return s&&s.classList.toggle("hidden",t===0),window.updateEditRemaining(),t},window.addServiceRow=(t="",e=0)=>{const s=document.getElementById("servicesContainer"),a=document.getElementById("emptyServicesMsg"),n=document.getElementById("servicesTotalRow");if(!s)return;a&&a.classList.add("hidden"),n&&n.classList.remove("hidden");const r=document.createElement("div");r.className="service-row flex items-center gap-2 bg-gray-50 border border-gray-100 rounded-xl px-3 py-2.5",r.innerHTML=`
        ...
        `,r.querySelector(".remove-service-btn").addEventListener("click",()=>{r.remove(),s.querySelectorAll(".service-row").length===0&&(a&&a.classList.remove("hidden"),n&&n.classList.add("hidden")),window.updateServicesTotal()}),r.querySelector(".service-price-input").addEventListener("input",window.updateServicesTotal),s.appendChild(r),window.updateServicesTotal()};const re=document.getElementById("addServiceBtn");re&&re.addEventListener("click",()=>window.addServiceRow()),window.actionEdit=async t=>{try{const e=v.find(u=>u.id==t);if(!e){alert("Không tìm thấy thông tin khách hàng!");return}document.getElementById("edit-id").value=t,document.getElementById("edit-name").value=e.name||"",document.getElementById("edit-phone").value=e.phone||"",document.getElementById("edit-medal-name").value=e.medal_name||"",document.getElementById("edit-tour").value=e.tour||"",ne("edit-date",e.tour||"",e.date||"",e.schedule_id||null),(u=>{let h=u.dob||"";if(h&&h.includes("/")){const b=h.split("/");b.length===3&&(h=`${b[2]}-${b[1].padStart(2,"0")}-${b[0].padStart(2,"0")}`)}document.getElementById("edit-dob").value=h,document.getElementById("edit-gender").value=u.gender||"Khác",document.getElementById("edit-allergy").value=u.allergy||u.medical_notes||"",document.getElementById("edit-address").value=u.address||"",document.getElementById("edit-diet").value=u.diet||u.dietary||"",document.getElementById("edit-trekking-pole").value=u.trekking_pole||"Không",document.getElementById("edit-id-card").value=u.id_card||u.cccd||"",document.getElementById("edit-pickup-point").value=u.pickup_point||""})(e),document.getElementById("edit-status").value=e.status||"Chờ xác nhận cọc",document.getElementById("edit-commitments").checked=!!e.commitments,document.getElementById("edit-special").value=e.special||"";try{const u=localStorage.getItem("csr_user");if(u&&JSON.parse(u).role==="admin"){const b=document.getElementById("edit-sale-container");b&&b.classList.remove("hidden");const f=document.getElementById("edit-sale");f&&(f.value=e.sale_id||"")}}catch{}const a=(parseInt(e.total_price)||0)+(parseInt(e.discount)||0);document.getElementById("edit-total").value=a,document.getElementById("edit-discount").value=e.discount||0,document.getElementById("edit-deposit").value=e.deposit||0;const n=parseInt(e.deposit_required);document.getElementById("edit-deposit-required").value=isNaN(n)?1e6:n,window.updateEditRemaining();const r=document.getElementById("servicesContainer"),o=document.getElementById("emptyServicesMsg"),c=document.getElementById("servicesTotalRow"),d=document.getElementById("presetServicesBlock"),i=document.getElementById("presetServicesList"),l=document.getElementById("servicesDivider");let g=[];try{if(e.services_booked){const u=typeof e.services_booked=="string"?JSON.parse(e.services_booked):e.services_booked;Array.isArray(u)&&(g=u)}}catch{}if(i&&d){i.innerHTML="";const u=D.find(b=>b.name===e.tour);let h=[];try{if(u?.services){const b=typeof u.services=="string"?JSON.parse(u.services):u.services;Array.isArray(b)&&(h=b)}}catch{}h.length>0?(d.classList.remove("hidden"),l&&l.classList.remove("hidden"),h.forEach(b=>{const f=g.some(w=>(w.label||w.name||"").toLowerCase()===(b.label||"").toLowerCase()),x=document.createElement("label");x.className="preset-service-card flex items-center gap-3 p-3 rounded-xl border-2 cursor-pointer transition-all "+(f?"border-green-400 bg-green-50":"border-gray-100 bg-gray-50"),x.innerHTML=`
                        ...
                        `,x.querySelector(".preset-service-check").addEventListener("change",w=>{x.className="preset-service-card flex items-center gap-3 p-3 rounded-xl border-2 cursor-pointer transition-all "+(w.target.checked?"border-green-400 bg-green-50":"border-gray-100 bg-gray-50"),window.updateServicesTotal()}),i.appendChild(x)})):(d.classList.add("hidden"),l&&l.classList.add("hidden"))}if(r){r.querySelectorAll(".service-row").forEach(f=>f.remove());const u=D.find(f=>f.name===e.tour);let h=new Set;try{const f=u?.services,x=f?typeof f=="string"?JSON.parse(f):f:[];Array.isArray(x)&&x.forEach(w=>h.add((w.label||"").toLowerCase()))}catch{}const b=g.filter(f=>!h.has((f.label||f.name||"").toLowerCase()));b.length>0?(o&&o.classList.add("hidden"),c&&c.classList.remove("hidden"),b.forEach(f=>window.addServiceRow(f.label||f.name||"",f.price||0))):o&&o.classList.remove("hidden"),window.updateServicesTotal()}const p=document.getElementById("editModal"),m=document.getElementById("editModalContent");p&&m&&(p.classList.remove("hidden"),setTimeout(()=>{p.classList.add("opacity-100"),m.classList.remove("scale-95","translate-y-4"),m.classList.add("scale-100","translate-y-0")},10))}catch(e){console.error("Lỗi khi mở form Edit:",e),alert("Lỗi khi mở form Edit: "+e.message)}};const ae=document.getElementById("editForm");ae&&ae.addEventListener("submit",async t=>{t.preventDefault();const e=t.target.querySelector('button[type="submit"]'),s=e.textContent;e.textContent="Đang lưu cập nhật...",e.disabled=!0;try{const a=document.getElementById("edit-id").value,n=document.getElementById("edit-name").value,r=document.getElementById("edit-phone").value,o=document.getElementById("edit-medal-name").value,c=document.getElementById("edit-tour").value,d=document.getElementById("edit-date").value;let i=d,l;if(d&&d.includes("::")){const y=d.split("::");l=parseInt(y[0])||null,i=y[1]}const g=document.getElementById("edit-dob").value,p=document.getElementById("edit-gender").value,m=document.getElementById("edit-status").value,u=document.getElementById("edit-allergy").value,h=document.getElementById("edit-address").value,b=document.getElementById("edit-diet").value,f=document.getElementById("edit-trekking-pole").value,x=document.getElementById("edit-commitments").checked,w=document.getElementById("edit-special").value,M=document.getElementById("edit-id-card").value,N=document.getElementById("edit-pickup-point").value,T=[];document.querySelectorAll(".preset-service-check:checked").forEach(y=>{T.push({label:y.dataset.label,price:parseInt(y.dataset.price)||0})}),document.querySelectorAll(".service-row").forEach(y=>{const R=(y.querySelector(".service-label-input")?.value||"").trim(),Me=parseInt(y.querySelector(".service-price-input")?.value)||0;R&&T.push({label:R,price:Me})});const H=T.length>0?JSON.stringify(T):null,J=document.getElementById("edit-sale");let A,W;if(J&&!J.parentElement.classList.contains("hidden")&&(A=J.value||null,W="Website",A)){const y=F.find(R=>String(R.id)===String(A));y&&(W=y.full_name||y.fullName)}const Te=parseInt(document.getElementById("edit-total").value)||0,oe=parseInt(document.getElementById("edit-discount").value)||0,Ie=parseInt(document.getElementById("edit-deposit").value)||0,le=parseInt(document.getElementById("edit-deposit-required").value),Le=isNaN(le)?1e6:le,$e={id:a,name:n,phone:r,medal_name:o,tour:c,date:i,dob:g,gender:p,status:m,allergy:u,address:h,diet:b,trekking_pole:f,commitments:x,special:w,id_card:M,pickup_point:N,services_booked:H,sale_id:A,sale_name:W,total_price:Te-oe,discount:oe,deposit:Ie,deposit_required:Le,...l!==void 0?{schedule_id:l}:{}},ie=await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify($e)});if(ie.ok)alert("✅ Đã cập nhật thành công Chi tiết Đơn hàng!"),window.closeEditModal(),P();else{const y=await ie.json();throw new Error(y.error||"Gặp sự cố khi Cập nhật API.")}}catch(a){alert("❌ Lỗi Cập nhật: "+a.message)}finally{e.textContent=s,e.disabled=!1}}),xe(),ve(),Promise.all([P(),Se()]).then(()=>j()).catch(()=>j())};export{He as afterRender,Ne as render};
