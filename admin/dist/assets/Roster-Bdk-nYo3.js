import{S as te,H as re}from"./Header-Dmidaim7.js";const ae=()=>`
    ...
    `,le=async()=>{const R=e=>{const t=(e||"").toLowerCase();return t.includes("hoàn tất")||t.includes("hoàn thành")||t.includes("đã thanh toán")||t.includes("thanh toán")?`<span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold bg-emerald-100 text-emerald-700 border border-emerald-200">
        ...
        `).join(""),document.querySelectorAll(".col-toggle-cb").forEach(t=>{t.addEventListener("change",a=>{a.target.checked?y.add(a.target.value):y.delete(a.target.value),X()})})},q=()=>{const e=document.getElementById("table-header-row");e.innerHTML=b.map(t=>`
            <th class="p-4 whitespace-nowrap ${t.id}" data-col="${t.id}">${t.label}</th>
        `).join("")},X=()=>{b.forEach(e=>{const t=y.has(e.id),a=document.querySelector(`th[data-col="${e.id}"]`);a&&(t?a.classList.remove("col-hidden"):a.classList.add("col-hidden")),document.querySelectorAll(`td[data-col="${e.id}"]`).forEach(l=>{t?l.classList.remove("col-hidden"):l.classList.add("col-hidden")})})},B=e=>{if(!e)return null;e.includes(" - ")&&(e=e.split(" - ")[0].trim()),e.includes("T")&&(e=e.split("T")[0]);const t=e.split(/[-/.]/);return t.length===2?{day:parseInt(t[0]),month:parseInt(t[1]),year:null}:t.length===3?t[0].length===4?{day:parseInt(t[2]),month:parseInt(t[1]),year:parseInt(t[0])}:{day:parseInt(t[0]),month:parseInt(t[1]),year:parseInt(t[2])}:null},w=B(v),Q=e=>{if(!w)return!0;const t=B(e);return!(!t||t.day!==w.day||t.month!==w.month||t.year&&w.year&&t.year!==w.year)},D=async()=>{try{const t=await(await fetch("/api/bookings")).json();console.log("[Roster] targetTour:",m,"targetDateStr:",v,"targetParsed:",w),console.log("[Roster] Total bookings:",t.length),t.length>0&&t.filter(r=>r.tour&&r.tour.includes("Liêng")).slice(0,3).forEach(r=>console.log("[Roster] Sample Liêng:",{tour:r.tour,date:r.date,parsed:B(r.date)})),f=t.filter(l=>{if(H){if(String(l.schedule_id)===String(H))return!0;if(!(!P&&!l.schedule_id))return!1}let r=!0,n=!0;if(m){const c=N(l.tour),o=N(m);r=c===o||c.includes(o)||o.includes(c)}return v&&(n=Q(l.date)),r&&n}),console.log("[Roster] Matched bookings:",f.length);const a=document.getElementById("filterSort")?.value||"newest";a==="newest"?f.sort((l,r)=>parseInt(r.id)-parseInt(l.id)):f.sort((l,r)=>parseInt(l.id)-parseInt(r.id)),console.log(`[Roster] Sorted bookings (${a}):`,f.map(l=>l.id)),C()}catch(e){console.error("Lỗi khi tải dữ liệu đoàn:",e),L.innerHTML='<tr><td colspan="12" class="p-8 text-center text-red-500 font-bold">Lỗi khi tải dữ liệu. Vui lòng tải lại trang.</td></tr>'}},C=()=>{const e=E.value.toLowerCase(),t=$.value.toLowerCase(),a=document.getElementById("filterSearch")?.value?.toLowerCase()||"",l=f.filter(r=>{if(a&&!`${r.name} ${r.phone} ${r.address} ${r.medal_name} ${r.customer_id}`.toLowerCase().includes(a))return!1;const n=(r.status||"").toLowerCase();if(!(e===""||e==="chờ cọc"&&(n.includes("chờ cọc")||n.includes("chờ xác nhận"))||e==="đã cọc"&&(n.includes("đã cọc")||n.includes("hoàn tất")||n.includes("hoàn thành"))||e==="đã hủy"&&n.includes("hủy")))return!1;const o=(r.gender||"").toLowerCase();return t===""||o===t});if(document.getElementById("text-visible-count").textContent=l.length,J.textContent=l.length,l.length===0){L.innerHTML='<tr><td colspan="12" class="p-12 text-center text-gray-500 bg-gray-50">Không tìm thấy khách hàng nào khớp với điều kiện lọc.</td></tr>';return}L.innerHTML=l.map((r,n)=>`<tr class="${n%2===0?"bg-white":"bg-gray-50/40"} hover:bg-amber-50/40 transition-colors border-b border-gray-100 group">`+b.map(o=>`
                    <td class="px-4 py-3 align-middle ${o.id==="col-extra"?"min-w-[220px]":""} ${y.has(o.id)?"":"col-hidden"}" data-col="${o.id}">
                        ${o.render(r,n)}
                    </td>
                `).join("")+"</tr>").join("")},Y=()=>{const e=b.filter(s=>y.has(s.id)),t=E.value.toLowerCase(),a=$.value.toLowerCase(),l=document.getElementById("filterSearch")?.value?.toLowerCase()||"",r=f.filter(s=>{if(l&&!`${s.name} ${s.phone} ${s.address} ${s.medal_name} ${s.customer_id}`.toLowerCase().includes(l))return!1;const p=(s.status||"").toLowerCase();if(!(t===""||t==="chờ cọc"&&(p.includes("chờ cọc")||p.includes("chờ xác nhận"))||t==="đã cọc"&&(p.includes("đã cọc")||p.includes("hoàn tất")||p.includes("hoàn thành"))||t==="đã hủy"&&p.includes("hủy")))return!1;const i=(s.gender||"").toLowerCase();return a===""||i===a});let n=`
            ...
            <body>
                <table>
                    ...
                    <tbody>
        `;r.forEach((s,p)=>{n+="<tr>",e.forEach(d=>{let i="";if(d.id==="col-stt")i=p+1;else if(d.id==="col-name")i=s.name||"";else if(d.id==="col-order-id")i=s.id||"";else if(d.id==="col-address")i=s.address||"";else if(d.id==="col-phone")i=s.phone?String(s.phone):"";else if(d.id==="col-diet")i=s.diet==="Ăn chay"||s.diet==="Chay"||s.diet==="Có"?"Có":"Không";else if(d.id==="col-pole")i=s.trekking_pole==="Có"?"Có":"Không";else if(d.id==="col-status")i=s.status||"";else if(d.id==="col-extra"){const u=s.services_booked;if(u&&u.trim()!==""&&u!=="[]"&&u!=="null")try{const g=JSON.parse(u);Array.isArray(g)&&g.length>0&&(i=g.map(h=>{const I=h.label||h.name||h.title||"",M=h.price&&Number(h.price)>0?` (+${Number(h.price).toLocaleString("vi-VN")}đ)`:"";return(I+M).trim()}).filter(Boolean).join(", "))}catch{const h=u.match(/label:\s*'([^']+)'/g)||u.match(/label:\s*"([^"]+)"/g)||[],I=u.match(/price:\s*(\d+)/g)||[];h.length>0&&(i=h.map((M,W)=>{const Z=M.replace(/label:\s*['"]/,"").replace(/['"]$/,"").trim(),_=I[W]||"",A=_?Number(_.replace("price:","").trim()):0,ee=A>0?` (+${A.toLocaleString("vi-VN")}đ)`:"";return(Z+ee).trim()}).filter(Boolean).join(", "))}i||(i=s.extra_services||"")}else{const u=String(d.render(s,p)),g=document.createElement("DIV");g.innerHTML=u,i=g.textContent||g.innerText||""}const j=d.id==="col-phone"||d.id==="col-cccd";n+=`<td ${j?'class="num"':""}>${i}</td>`}),n+="</tr>"}),n+=`
                    </tbody>
                </table>
            </body>
            </html>
        `;const c=new Blob(["\uFEFF",n],{type:"application/vnd.ms-excel;charset=utf-8"}),o=URL.createObjectURL(c),T=`DS_Doan_${m||"Tour"}_${k||"Date"}.xls`.replace(/[/\\?%*:|"<>]/g,"-"),x=document.createElement("a");x.setAttribute("href",o),x.setAttribute("download",T),document.body.appendChild(x),x.click(),document.body.removeChild(x)};E.addEventListener("change",C),$.addEventListener("change",C),document.getElementById("filterSearch")?.addEventListener("input",C),document.getElementById("filterSort")?.addEventListener("change",D),document.getElementById("btnExportPDF").addEventListener("click",()=>{window.print()}),document.getElementById("btnExportExcel").addEventListener("click",()=>{Y()}),U(),q(),D()};export{le as afterRender,ae as render};
