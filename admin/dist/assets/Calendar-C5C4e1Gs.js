import{S as ee,H as te}from"./Header-CARqqxjt.js";const re=()=>`
...
`,se=()=>{let w=[],B=[],E=[],b=new Date,L="",_="",h="blue",j=null;const T={blue:"bg-blue-100 text-blue-700 border-blue-200",green:"bg-green-100 text-green-700 border-green-200",red:"bg-red-100 text-red-700 border-red-200",yellow:"bg-yellow-100 text-yellow-700 border-yellow-200",purple:"bg-purple-100 text-purple-700 border-purple-200",gray:"bg-gray-100 text-gray-600 border-gray-200"},H={blue:"bg-blue-500",green:"bg-green-500",red:"bg-red-500",yellow:"bg-yellow-400",purple:"bg-purple-500",gray:"bg-gray-400"},A={Leader:"bg-purple-100 text-purple-700","Hướng Dẫn Viên":"bg-blue-100 text-blue-700",Photo:"bg-pink-100 text-pink-700",Guider:"bg-cyan-100 text-cyan-700","Hậu Cần":"bg-amber-100 text-amber-700","Học Việc":"bg-gray-100 text-gray-600"};let D="sale";try{D=JSON.parse(localStorage.getItem("csr_user")||"{}").role||"sale"}catch{}const V=()=>{const t=b.getFullYear(),e=b.getMonth();return`${t}-${String(e+1).padStart(2,"0")}`},$=async()=>{try{E=await(await fetch(`/api/calendar-notes?month=${V()}`)).json()}catch{E=[]}},z=async()=>{m();const[t,e,c]=await Promise.all([fetch("/api/schedules"),fetch("/api/guides"),fetch("/api/tours")]);w=await t.json(),B=await e.json();const g=await c.json(),y=document.getElementById("calFilterTour");g.forEach(r=>{const l=document.createElement("option");l.value=r.name,l.textContent=r.name,y.appendChild(l)});const d=document.getElementById("calFilterGuide");B.forEach(r=>{const l=document.createElement("option");l.value=r.id,l.textContent=r.full_name,d.appendChild(l)}),await $(),m()},X=()=>{const t=b.getFullYear(),e=b.getMonth();return w.filter(c=>{const g=new Date(c.start_date);return!(g.getFullYear()!==t||g.getMonth()!==e||L&&c.tour_name!==L||_&&!(c.assigned_guides||[]).some(d=>String(d.id)===String(_)))})},m=()=>{const t=b.getFullYear(),e=b.getMonth(),c=["Tháng 1","Tháng 2","Tháng 3","Tháng 4","Tháng 5","Tháng 6","Tháng 7","Tháng 8","Tháng 9","Tháng 10","Tháng 11","Tháng 12"];document.getElementById("calTitle").textContent=`${c[e]} / ${t}`;const g=X(),y=g.reduce((n,s)=>n+(parseInt(s.booked_count)||0),0),d=new Set(g.flatMap(n=>(n.assigned_guides||[]).map(s=>s.id)));document.getElementById("statTours").textContent=g.length,document.getElementById("statGuests").textContent=y,document.getElementById("statGuides").textContent=d.size;const r={};g.forEach(n=>{const s=new Date(n.start_date).toISOString().split("T")[0];r[s]||(r[s]=[]),r[s].push(n)});const l={};E.forEach(n=>{const s=n.date.split("T")[0];l[s]||(l[s]=[]),l[s].push(n)});const v=new Date(t,e,1).getDay(),a=new Date(t,e+1,0).getDate(),i=new Date;i.setHours(0,0,0,0);let o="";for(let n=0;n<v;n++)o+='<div class="min-h-[80px] border-r border-b border-gray-100 bg-gray-50/50 p-1"></div>';for(let n=1;n<=a;n++){const s=new Date(t,e,n),f=`${t}-${String(e+1).padStart(2,"0")}-${String(n).padStart(2,"0")}`,C=s.getTime()===i.getTime(),O=r[f]||[],J=O.slice(0,3).map(p=>{const S=parseInt(p.booked_count)||0,I=parseInt(p.slots)||0,q=Math.max(0,I-S),Q=q<=0?"bg-red-500":q<=3?"bg-orange-500":"bg-green-500",K=(p.assigned_guides||[]).slice(0,2).map(M=>{const W=(M.full_name||"X").split(" ").slice(-1)[0][0].toUpperCase();return`<span class="inline-flex items-center justify-center w-5 h-5 rounded-full text-[9px] font-black ${A[M.role]||"bg-gray-100 text-gray-600"}" title="${M.full_name} (${M.role})">${W}</span>`}).join(""),Z=p.tour_name.split(" ").slice(0,2).join(" ");return`<div class="cal-sch-badge rounded-md px-1.5 py-1 bg-csr-orange/8 border border-csr-orange/20 cursor-pointer hover:bg-csr-orange/15 transition-colors mb-0.5" data-id="${p.id}">
            ...
            <div class="cal-day-cell min-h-[80px] border-r border-b border-gray-100 p-1 ${C?"bg-csr-orange/5 ring-2 ring-inset ring-csr-orange/30":"hover:bg-gray-50"} transition-colors cursor-pointer" data-date="${f}">
                ...
                ${J}${U}${Y}
            </div>`}const u=v+a,x=u%7===0?0:7-u%7;for(let n=0;n<x;n++)o+='<div class="min-h-[80px] border-r border-b border-gray-100 bg-gray-50/50 p-1"></div>';document.getElementById("calGrid").innerHTML=o,document.querySelectorAll(".cal-sch-badge").forEach(n=>{n.addEventListener("click",s=>{s.stopPropagation(),N(n.getAttribute("data-id"))})}),document.querySelectorAll(".cal-note-badge").forEach(n=>{n.addEventListener("click",s=>{s.stopPropagation();const f=E.find(C=>String(C.id)===n.dataset.noteId);f&&G(f.date.split("T")[0],f)})}),document.querySelectorAll(".cal-day-cell").forEach(n=>{n.addEventListener("click",()=>G(n.dataset.date))})},N=async t=>{const e=w.find(o=>String(o.id)===String(t));if(!e)return;const c=document.getElementById("calModal");c.classList.remove("hidden"),c.classList.add("flex");const g=new Date(e.start_date).toLocaleDateString("vi-VN"),y=new Date(e.end_date).toLocaleDateString("vi-VN"),d=parseInt(e.booked_count)||0,r=parseInt(e.slots)||0,l=e.assigned_guides||[];document.getElementById("calModalTitle").textContent=e.tour_name,document.getElementById("calModalSub").textContent=`${g} → ${y} • ${d}/${r} chỗ${e.group_label?" • "+e.group_label:""}`;const v=l.length===0?'<p class="text-gray-400 text-sm italic">Chưa có nhân sự được phân công.</p>':l.map(o=>{const u=A[o.role]||"bg-gray-100 text-gray-600",x=(o.full_name||"X").split(" ").slice(-2).map(s=>s[0]).join("").toUpperCase(),n=D==="admin"?`<button class="modal-unassign-btn px-2 py-1 rounded-lg text-xs bg-red-50 text-red-500 hover:bg-red-100 transition-colors" data-guide-id="${o.id}" data-sch-id="${e.id}">✕</button>`:"";return`<div class="flex items-center gap-3 p-2.5 rounded-xl border border-gray-100 hover:bg-gray-50">
                    ...
                    ${n}
                </div>`}).join("");let a="";if(D==="admin"){const o=B.filter(u=>!l.some(x=>x.id===u.id));o.length>0&&(a=`
                <div class="border-t border-gray-100 pt-3">
            ...
            <div class="space-y-2">
                <div class="flex items-center gap-4 text-sm text-gray-600 flex-wrap">
                    ...
                    <span class="text-xs font-bold ${d>=r?"text-red-500":d>=r*.8?"text-orange-500":"text-green-600"}">${d>=r?"Hết chỗ":r-d<=3?`Còn ${r-d} chỗ`:`Còn ${r-d} chỗ trống`}</span>
                </div>
            </div>
            <div>
            ...
            <button id="calModalCloseFt" class="px-4 py-2.5 border border-gray-200 text-gray-500 text-sm font-bold rounded-xl hover:bg-gray-50 transition-all">Đóng</button>`,document.querySelectorAll(".modal-unassign-btn").forEach(o=>{o.addEventListener("click",async()=>{o.textContent="...",o.disabled=!0,await fetch("/api/guides?action=unassign",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({schedule_id:parseInt(o.dataset.schId),guide_id:parseInt(o.dataset.guideId)})}),await P(),N(t)})});const i=document.getElementById("modalAssignBtn");i&&i.addEventListener("click",async()=>{const o=document.getElementById("modalAssignSelect");o.value&&(i.textContent="...",i.disabled=!0,await fetch("/api/guides?action=assign",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({schedule_id:parseInt(i.dataset.schId),guide_id:parseInt(o.value)})}),await P(),N(t))}),document.getElementById("calModalCloseFt")?.addEventListener("click",F)},F=()=>{const t=document.getElementById("calModal");t.classList.add("hidden"),t.classList.remove("flex")},P=async()=>{w=await(await fetch("/api/schedules")).json(),m()},G=(t,e=null)=>{j=e?e.id:null;const g=new Date(t+"T00:00:00").toLocaleDateString("vi-VN",{weekday:"long",day:"2-digit",month:"2-digit",year:"numeric"});document.getElementById("noteModalTitle").textContent=`📝 Ghi chú — ${g}`,document.getElementById("noteTitle").value=e?e.title:"",document.getElementById("noteContent").value=e&&e.note||"",h=e&&e.color||"blue",document.querySelectorAll(".note-color-btn").forEach(a=>{a.className=a.className.replace(/ring-2 ring-offset-1 ring-\S+/,"").trim(),a.dataset.color===h&&a.classList.add("ring-2","ring-offset-1",`ring-${h}-500`)});const d=E.filter(a=>a.date.split("T")[0]===t&&(!e||a.id!==e.id)).map(a=>`<div class="flex items-center gap-2 p-2 rounded-lg border ${T[a.color]||T.blue}">
            ...
            </div>`).join("");document.getElementById("noteExistingList").innerHTML=d||"",document.querySelectorAll(".note-quick-edit").forEach(a=>{a.addEventListener("click",i=>{i.stopPropagation();const o=E.find(u=>String(u.id)===a.dataset.nid);o&&G(t,o)})}),document.getElementById("noteFormLabel").textContent=e?"✏️ Chỉnh sửa ghi chú":"➕ Thêm ghi chú mới";const r=document.getElementById("noteSaveBtn"),l=r.cloneNode(!0);if(r.parentNode.replaceChild(l,r),l.textContent=e?"Cập Nhật":"Lưu Ghi Chú",l.addEventListener("click",async()=>{const a=document.getElementById("noteTitle").value.trim();if(!a){alert("Vui lòng nhập tên công việc!");return}l.disabled=!0,l.textContent="Đang lưu...";let i={};try{i=JSON.parse(localStorage.getItem("csr_user")||"{}")}catch{}const o=j?{id:j,title:a,note:document.getElementById("noteContent").value.trim(),color:h}:{date:t,title:a,note:document.getElementById("noteContent").value.trim(),color:h,created_by:i.name||i.username||""};try{await fetch("/api/calendar-notes",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(o)}),await $(),m(),k()}catch(u){alert("Lỗi: "+u.message)}finally{l.disabled=!1,l.textContent=e?"Cập Nhật":"Lưu Ghi Chú"}}),e){const a=document.createElement("button");a.textContent="🗑 Xóa",a.className="w-full mt-2 py-2 text-sm font-bold text-red-500 border border-red-200 rounded-xl hover:bg-red-50 transition-all",a.addEventListener("click",async()=>{confirm("Xóa ghi chú này?")&&(await fetch(`/api/calendar-notes?id=${e.id}`,{method:"DELETE"}),await $(),m(),k())}),document.getElementById("noteContent").parentNode.appendChild(a)}const v=document.getElementById("noteModal");v.classList.remove("hidden"),v.classList.add("flex")},k=()=>{const t=document.getElementById("noteModal");t.classList.add("hidden"),t.classList.remove("flex");const e=document.querySelector("#noteModal button.text-red-500");e&&e.remove()};document.getElementById("noteColorPicker").addEventListener("click",t=>{const e=t.target.closest(".note-color-btn");e&&(h=e.dataset.color,document.querySelectorAll(".note-color-btn").forEach(c=>{c.className=c.className.replace(/\sring-2 ring-offset-1 ring-\S+/g,"").trim()}),e.classList.add("ring-2","ring-offset-1",`ring-${h}-500`))}),document.getElementById("noteModalClose").addEventListener("click",k),document.getElementById("noteModalCloseFt").addEventListener("click",k),document.getElementById("noteModal").addEventListener("click",t=>{t.target===document.getElementById("noteModal")&&k()});const R=async t=>{b=new Date(b.getFullYear(),b.getMonth()+t,1),await $(),m()};document.getElementById("calPrev").addEventListener("click",()=>R(-1)),document.getElementById("calNext").addEventListener("click",()=>R(1)),document.getElementById("calToday").addEventListener("click",async()=>{b=new Date,await $(),m()}),document.getElementById("calFilterTour").addEventListener("change",t=>{L=t.target.value,m()}),document.getElementById("calFilterGuide").addEventListener("change",t=>{_=t.target.value,m()}),document.getElementById("calModal").addEventListener("click",t=>{t.target===document.getElementById("calModal")&&F()}),document.getElementById("calModalClose").addEventListener("click",F),z()};export{se as afterRender,re as render};
