import{S as pt,H as mt}from"./Header-n5pNPUhI.js";console.log("Bookings V3 - Cache Busted!");const ft=()=>`
    ...
    `,yt=()=>{let _=[],f="consult",V=[],U=[],G=[];const rt=async()=>{try{const[n,t,o]=await Promise.all([fetch("/api/tours"),fetch("/api/schedules"),fetch("/api/users")]);V=n.ok?await n.json():[],U=t.ok?await t.json():[],G=o.ok?await o.json():[],W("addTourName"),W("edit-tour"),ct("edit-sale")}catch(n){console.error("Lỗi tải tours/schedules:",n)}},ct=n=>{const t=document.getElementById(n);t&&(t.innerHTML='<option value="">-- Website --</option>',G.forEach(o=>{const l=document.createElement("option");l.value=o.id,l.textContent=`${o.full_name||o.fullName} (${o.role})`,t.appendChild(l)}))},W=n=>{const t=document.getElementById(n);if(!t)return;const o=t.value,l=t.options[0].outerHTML;t.innerHTML=l,V.filter(d=>d.is_visible!==!1).forEach(d=>{const a=document.createElement("option");a.value=d.name,a.textContent=d.name,d.name===o&&(a.selected=!0),t.appendChild(a)})},K=(n,t,o)=>{const l=document.getElementById(n);if(!l)return;if(l.innerHTML='<option value="">-- Chọn Lịch --</option>',!t){l.innerHTML='<option value="">-- Chọn Tour trước --</option>';return}const d=U.filter(a=>a.tour_name===t||t.includes(a.tour_name)||a.tour_name.includes(t));if(d.length===0){l.innerHTML='<option value="">Chưa có lịch cho tour này</option>';return}d.sort((a,r)=>new Date(a.start_date)-new Date(r.start_date)).forEach(a=>{const r=parseInt(a.booked_count)||0,s=(a.slots||0)-r,g=new Date(a.start_date),y=a.end_date?new Date(a.end_date):null,h=p=>`${p.getDate().toString().padStart(2,"0")}/${(p.getMonth()+1).toString().padStart(2,"0")}`;let b=h(g);y&&(b+=" - "+h(y)),b+=` (${s>=0?s:0} chỗ trống)`;const i=document.createElement("option");i.value=`${g.getDate().toString().padStart(2,"0")}/${(g.getMonth()+1).toString().padStart(2,"0")}/${g.getFullYear()}`,i.textContent=b,s<=0&&(i.disabled=!0,i.textContent=b.replace("chỗ trống","HẾT CHỖ")),o&&i.value===o&&(i.selected=!0),l.appendChild(i)})},J=document.getElementById("addTourName");J&&J.addEventListener("change",n=>{K("addTourDate",n.target.value)});const X=document.getElementById("edit-tour");X&&X.addEventListener("change",n=>{K("edit-date",n.target.value)}),rt();const D=()=>{const n=document.getElementById("bookingsTableHead"),t=document.getElementById("bookingsTableBody");if(!t||!n)return;const o=f==="upcoming"||f==="ready"||f==="completed";o?n.innerHTML=`
            ...
            `;const l=document.getElementById("filterSearch")?document.getElementById("filterSearch").value.toLowerCase():"",d=document.getElementById("filterTour")?document.getElementById("filterTour").value:"",a=document.getElementById("filterDate")?document.getElementById("filterDate").value:"",r=document.getElementById("filterSale")?document.getElementById("filterSale").value:"",s=document.getElementById("filterStatus")?document.getElementById("filterStatus").value:"";let g=_.filter(e=>{let u=!1;const c=parseInt(e.total_price)>0&&parseInt(e.total_price)===parseInt(e.deposit),x=C=>{if(!C)return!1;try{const w=C.split("/");if(w.length===3){const m=new Date(`${w[2]}-${w[1]}-${w[0]}T00:00:00`),T=new Date;return T.setHours(0,0,0,0),m.getTime()<T.getTime()}else if(C.includes("-")){const m=new Date(`${C}T00:00:00`),T=new Date;return T.setHours(0,0,0,0),m.getTime()<T.getTime()}}catch{}return!1},k=(e.status==="Hoàn thành"||e.status==="Đã đi"||c)&&x(e.date);return f==="consult"?u=e.status==="Chờ tư vấn"&&!k:f==="pending"?u=(!e.status||e.status==="Chờ cọc"||e.status==="Chờ xác nhận cọc")&&!k:f==="upcoming"?u=e.status&&e.status!=="Chờ tư vấn"&&e.status!=="Chờ cọc"&&e.status!=="Chờ xác nhận cọc"&&!c&&e.status!=="Hoàn thành"&&!k:f==="ready"?u=(c||e.status==="Hoàn thành")&&!k:f==="completed"&&(u=k),!(!u||d&&e.tour!==d||a&&O(e.date)!==a||r&&r!=="null"&&e.sale_name!==r||r==="null"&&e.sale_name&&e.sale_name!=="Website"||l&&!`${e.name||""} ${e.phone||""} ${e.customer_id||""}`.toLowerCase().includes(l)||s&&e.status!==s)});const y=document.getElementById("statTitle1"),h=document.getElementById("statTitle2"),b=document.getElementById("statTitle3"),i=document.getElementById("statTotalCustomers"),p=document.getElementById("statTotalRevenue"),v=document.getElementById("statTotalCollected");let B=g.length;if(f==="consult"){y&&(y.textContent="Tổng Leads Tư Vấn"),h&&(h.textContent="Chưa Có Sale Nhận"),b&&(b.textContent="Đã Có Sale Nhận");let e=0,u=0;g.forEach(c=>{!c.sale_id||c.sale_id==="null"||!c.sale_name||c.sale_name==="Website"||c.sale_name==="null"?e++:u++}),i&&(i.textContent=B),p&&(p.textContent=e+" Đơn"),v&&(v.textContent=u+" Đơn")}else if(f==="pending"){y&&(y.textContent="Tổng Khách (Chờ Cọc)"),h&&(h.textContent="Khách Chưa Tư Vấn (Mới)"),b&&(b.textContent="Đã Có Sale Giữ Chỗ");let e=0,u=0;g.forEach(c=>{!c.sale_id||c.sale_id==="null"||!c.sale_name||c.sale_name==="Website"||c.sale_name==="null"?e++:u++}),i&&(i.textContent=B),p&&(p.textContent=e+" Đơn"),v&&(v.textContent=u+" Đơn")}else if(f==="upcoming"){y&&(y.textContent="Tổng Số Khách"),h&&(h.textContent="Chưa Xác Nhận Cọc"),b&&(b.textContent="Chờ Thanh Toán Còn Lại");let e=0,u=0;g.forEach(c=>{c.status==="Chờ xác nhận cọc"?e++:parseInt(c.deposit)>0&&parseInt(c.total_price)>parseInt(c.deposit)&&u++}),i&&(i.textContent=B),p&&(p.textContent=e+" Khách"),v&&(v.textContent=u+" Khách")}else if(f==="ready"){y&&(y.textContent="Khách Sẵn Sàng (Full)"),h&&(h.textContent="Tổng Doanh Thu Tab"),b&&(b.textContent="Thực Thu (Full Tận Nơi)");let e=0,u=0;g.forEach(c=>{e+=parseInt(c.total_price)||0,u+=parseInt(c.deposit)||0}),i&&(i.textContent=B),p&&(p.textContent=e>0?e.toLocaleString("vi-VN")+"đ":"0đ"),v&&(v.textContent=u>0?u.toLocaleString("vi-VN")+"đ":"0đ")}else{y&&(y.textContent="Khách Đã Xong (Lịch Sử)"),h&&(h.textContent="Tổng Doanh Thu Tab"),b&&(b.textContent="Thực Thu (Đã Đóng)");let e=0,u=0;g.forEach(c=>{e+=parseInt(c.total_price)||0,u+=parseInt(c.deposit)||0}),i&&(i.textContent=B),p&&(p.textContent=e>0?e.toLocaleString("vi-VN")+"đ":"0đ"),v&&(v.textContent=u>0?u.toLocaleString("vi-VN")+"đ":"0đ")}const L=o?8:4;if(!g||g.length===0){let e=f==="consult"?"Chưa có khách hàng tư vấn nào.":f==="pending"?"Chưa có Khách chờ cọc.":f==="upcoming"?"Chưa có Khách nào Đã Cọc.":f==="ready"?"Chưa có khách sẵn sàng.":"Danh sách rỗng.";t.innerHTML=`<tr><td colspan="${L}" class="p-8 text-center text-gray-500">${e}</td></tr>`;return}t.innerHTML=g.map(e=>{const u=f==="upcoming"||f==="ready"?"bg-green-50/20 hover:bg-green-50 transition-colors":"hover:bg-gray-100 transition-colors";if(o){const c=parseInt(e.total_price)||0,x=parseInt(e.deposit)||0,k=c-x,C=c>0?c.toLocaleString("vi-VN")+"đ":"Chưa định giá",w=x>0?x.toLocaleString("vi-VN")+"đ":"0đ",m=k>0?k.toLocaleString("vi-VN")+"đ":k===0&&c>0?"Đã thu trọn":"-";let T="";e.status==="Hoàn thành"||e.status==="Đã đi"?T='<span class="bg-gray-100 text-gray-600 border border-gray-200 px-2 py-0.5 rounded text-xs block w-full text-center">Hoàn thành</span>':e.status==="Chờ xác nhận cọc"?T=`<button class="action-btn confirm-deposit-btn bg-csr-orange text-white px-3 py-1.5 rounded text-xs font-bold shadow-sm hover:bg-[#d65503] w-full" data-id="${e.id}">Xác nhận cọc</button>`:x>0&&k===0?T='<span class="bg-green-100 text-green-700 border border-green-200 px-2 py-0.5 rounded text-xs font-medium block w-full text-center">Hoàn tất phí</span>':T='<span class="bg-blue-100 text-blue-700 border border-blue-200 px-2 py-0.5 rounded text-xs font-medium block w-full text-center">Đã Cọc</span>';let M="";return x>0&&(M=`<a href="/invoice.html?id=${e.id}" target="_blank" class="mt-1.5 block text-center text-[11px] text-blue-600 hover:text-blue-800 hover:underline font-medium" onclick="event.stopPropagation()">🧾 Xem hóa đơn</a>`),`
    ...
    `}}).join(""),document.querySelectorAll(".claim-btn").forEach(e=>{e.addEventListener("click",async u=>{u.stopPropagation();const c=e.getAttribute("data-id"),x=localStorage.getItem("csr_user");let k=null,C="Admin";if(x){const w=JSON.parse(x);k=w.id,C=w.fullName||w.full_name||"Admin"}e.textContent="Đang xử lý...",e.disabled=!0;try{if((await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:c,sale_id:k,sale_name:C})})).ok){const m=_.find(T=>T.id==c);m&&(m.sale_id=k,m.sale_name=C),D()}else alert("Lỗi khi nhận khách. Vui lòng thử lại."),e.textContent="Nhận khách",e.disabled=!1}catch(w){console.error("Lỗi nhận khách:",w),e.textContent="Nhận khách",e.disabled=!1}})})},ut=()=>{const n=new Set,t=new Set,o=new Set;_.forEach(r=>{if(r.tour&&n.add(r.tour),r.date){const s=O(r.date);s&&t.add(s)}r.sale_name&&r.sale_name!=="Website"&&r.sale_name!=="null"&&o.add(r.sale_name.trim())});const l=document.getElementById("filterTour"),d=document.getElementById("filterDate"),a=document.getElementById("filterSale");if(l){const r=l.value;l.innerHTML='<option value="">Tất cả Tour</option>',[...n].sort().forEach(s=>l.innerHTML+=`<option value="${s}">${s}</option>`),l.value=r}if(d){const r=d.value;d.innerHTML='<option value="">Tất cả Ngày</option>',[...t].sort().forEach(s=>d.innerHTML+=`<option value="${s}">${s}</option>`),d.value=r}if(a){const r=a.value;a.innerHTML='<option value="">Tất cả Sale</option>',a.innerHTML+='<option value="null">Nguồn Website</option>',[...o].sort().forEach(s=>{a.innerHTML+=`<option value="${s}">${s}</option>`}),a.value=r}};function O(n){if(!n)return"";if(/^\d{1,2}\/\d{1,2}\s*-\s*\d{1,2}\/\d{1,2}$/.test(n))return n.replace(/\s+/g,"");try{if(/^\d{4}-\d{2}-\d{2}$/.test(n)){const[t,o,l]=n.split("-"),d=`${parseInt(l)}/${parseInt(o)}`;return`${d}-${d}`}if(/^\d{1,2}\/\d{1,2}\/\d{4}$/.test(n)){const[t,o,l]=n.split("/"),d=`${parseInt(t)}/${parseInt(o)}`;return`${d}-${d}`}if(n.includes("-"))return n.split("-").map(o=>{const l=o.trim();if(/\/\d{4}$/.test(l)){const[d,a]=l.split("/");return`${parseInt(d)}/${parseInt(a)}`}return l}).join("-")}catch{console.warn("Date normalization failed for:",n)}return n}const S=async()=>{try{_=await(await fetch("/api/bookings")).json(),ut(),D()}catch(n){console.error("Lỗi parse list đơn hàng:",n);const t=document.getElementById("bookingsTableBody");t&&(t.innerHTML='<tr><td colspan="4" class="p-8 text-center text-red-500">Lỗi lấy dữ liệu từ Server.</td></tr>')}};S(),["filterSearch","filterTour","filterDate","filterSale","filterStatus"].forEach(n=>{const t=document.getElementById(n);t&&(n==="filterSearch"?t.addEventListener("input",D):t.addEventListener("change",D))});const Y=document.getElementById("exportExcelBtn");Y&&Y.addEventListener("click",()=>{const n=document.getElementById("filterSearch")?document.getElementById("filterSearch").value.toLowerCase():"",t=document.getElementById("filterTour")?document.getElementById("filterTour").value:"",o=document.getElementById("filterDate")?document.getElementById("filterDate").value:"",l=document.getElementById("filterSale")?document.getElementById("filterSale").value:"";let d=_.filter(i=>{let p=!1;const v=parseInt(i.total_price)>0&&parseInt(i.total_price)===parseInt(i.deposit),B=e=>{if(!e)return!1;try{const u=e.split("/");if(u.length===3){const c=new Date(`${u[2]}-${u[1]}-${u[0]}T00:00:00`),x=new Date;return x.setHours(0,0,0,0),c.getTime()<x.getTime()}else if(e.includes("-")){const c=new Date(`${e}T00:00:00`),x=new Date;return x.setHours(0,0,0,0),c.getTime()<x.getTime()}}catch{}return!1},L=(i.status==="Hoàn thành"||i.status==="Đã đi"||v)&&B(i.date);return f==="consult"?p=i.status==="Chờ tư vấn"&&!L:f==="pending"?p=(!i.status||i.status==="Chờ cọc")&&!L:f==="upcoming"?p=i.status&&i.status!=="Chờ tư vấn"&&i.status!=="Chờ cọc"&&!v&&i.status!=="Hoàn thành"&&!L:f==="ready"?p=(v||i.status==="Hoàn thành")&&!L:f==="completed"&&(p=L),!(!p||t&&i.tour!==t||o&&O(i.date)!==o||l&&l!=="null"&&i.sale_name!==l||l==="null"&&i.sale_name&&i.sale_name!=="Website"||n&&!`${i.name||""} ${i.phone||""} ${i.customer_id||""} `.toLowerCase().includes(n))});if(d.length===0){alert("Danh sách rỗng. Không có dữ liệu để xuất.");return}const r=[["Mã KH (CRM)","Họ và Tên","Số điện thoại","Tên Tour","Lịch trình","Trạng thái","Nguồn / Sale","Đã cọc","Tổng tiền","Ghi chú"]];d.forEach(i=>{r.push([i.customer_id||"",`"${i.name||""}"`,`= "${i.phone||""}"`,`"${i.tour||""}"`,`"${i.date||""}"`,`"${i.status||""}"`,`"${i.sale_name||"Website"}"`,i.deposit||0,i.total_price||0,`"${(i.special||"").replace(/"/g,'""')}"`])});const s="\\uFEFF"+r.map(i=>i.join(",")).join(`
`),g=new Blob([s],{type:"text/csv;charset=utf-8;"}),y=URL.createObjectURL(g),h=document.createElement("a");h.setAttribute("href",y);let b=f==="pending"?"ChoCoc":f==="upcoming"?"SapThamGia":f==="ready"?"ChoLenXe":"LichSu";h.setAttribute("download",`BaoCao_DonHang_${b}_${new Date().toISOString().slice(0,10)}.csv`),document.body.appendChild(h),h.click(),document.body.removeChild(h)});const Q=document.querySelectorAll(".tab-btn");Q.forEach(n=>{n.addEventListener("click",t=>{Q.forEach(o=>{o.classList.remove("border-csr-orange","text-csr-orange"),o.classList.add("border-transparent","text-gray-500")}),t.target.classList.add("border-csr-orange","text-csr-orange"),t.target.classList.remove("border-transparent","text-gray-500"),f=t.target.getAttribute("data-tab"),D()})});const $=document.getElementById("bookingModal"),N=document.getElementById("bookingModalContent"),z=document.getElementById("detailModal"),Z=document.getElementById("detailModalContent"),A=document.getElementById("editModal"),q=document.getElementById("editModalContent"),tt=document.getElementById("addBookingBtn");$&&($.classList.add("duration-200","ease-out"),N.classList.add("duration-300","ease-out")),A&&(A.classList.add("duration-200","ease-out"),q.classList.add("duration-300","ease-out")),tt&&tt.addEventListener("click",()=>{document.getElementById("bookingModalContent").querySelector("h2").innerText="Thêm Khách Hàng (Tạo Đơn)",document.getElementById("submitBookingBtn").innerText="Tạo Phơi Đăng Ký",document.getElementById("editingBookingId").value="",document.getElementById("bookingForm").reset();const n=document.getElementById("loyalty-alert");n&&n.remove(),$.classList.remove("hidden"),setTimeout(()=>{$.classList.add("opacity-100"),N.classList.remove("scale-95","translate-y-4"),N.classList.add("scale-100","translate-y-0")},10)}),window.closeModal=()=>{$&&($.classList.remove("opacity-100"),N.classList.remove("scale-100","translate-y-0"),N.classList.add("scale-95","translate-y-4"),setTimeout(()=>{$.classList.add("hidden")},200))},window.closeDetailModal=()=>{z&&(z.classList.remove("opacity-100"),Z.classList.remove("scale-100","translate-y-0"),Z.classList.add("scale-95","translate-y-4"),setTimeout(()=>{z.classList.add("hidden")},200))},window.closeEditModal=()=>{A&&(A.classList.remove("opacity-100"),q.classList.remove("scale-100","translate-y-0"),q.classList.add("scale-95","translate-y-4"),setTimeout(()=>{A.classList.add("hidden")},200))},window.openRowActionModal=n=>{document.getElementById("ramName").textContent=n.name||"Khách Hàng Này",document.getElementById("ramPhone").textContent=n.customer_id?`🥇 ${n.customer_id.toUpperCase()}`:n.phone||"";const t=`
        ...
        `;document.getElementById("detailContentBlock").innerHTML=d,document.getElementById("detailModal").classList.remove("hidden"),setTimeout(()=>{document.getElementById("detailModal").classList.add("opacity-100");const a=document.getElementById("detailModalContent");a&&(a.classList.remove("scale-95","translate-y-4"),a.classList.add("scale-100","translate-y-0"))},10)},window.actionEdit=async n=>{document.getElementById("rowActionModal").classList.add("hidden");const t=_.find(a=>a.id==n);if(!t)return;document.getElementById("edit-id").value=n,document.getElementById("edit-name").value=t.name||"",document.getElementById("edit-phone").value=t.phone||"",document.getElementById("edit-medal-name").value=t.medal_name||"",document.getElementById("edit-tour").value=t.tour||"",typeof K=="function"&&K("edit-date",t.tour||"",t.date||""),(a=>{let r=a.dob||"";if(typeof r=="string"&&r.includes("/")){const s=r.split("/");s.length===3&&(r=`${s[2]}-${s[1].padStart(2,"0")}-${s[0].padStart(2,"0")}`)}document.getElementById("edit-dob").value=r,document.getElementById("edit-gender").value=a.gender||"Khác",document.getElementById("edit-allergy").value=a.allergy||a.medical_notes||"",document.getElementById("edit-address").value=a.address||"",document.getElementById("edit-diet").value=a.diet||a.dietary||"Không",document.getElementById("edit-trekking-pole").value=a.trekking_pole||"Không"})(t),document.getElementById("edit-status").value=t.status||"Chờ xác nhận cọc",document.getElementById("edit-commitments").checked=!!t.commitments,document.getElementById("edit-special").value=t.special||"";try{const a=localStorage.getItem("csr_user");if(a&&JSON.parse(a).role==="admin"){const s=document.getElementById("edit-sale-container");s&&s.classList.remove("hidden");const g=document.getElementById("edit-sale");g&&(g.value=t.sale_id||"")}}catch{}if((!t.dob||!t.address||!t.gender||t.gender==="Khác")&&(t.customer_id||t.phone))try{const a=t.customer_id||t.phone,r=await fetch(`/api/admin_customers?action=search&keyword=${encodeURIComponent(a)}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({keyword:a})}),s=await r.json();if(r.ok&&s.success){const g=s.data,y=document.getElementById("edit-dob"),h=document.getElementById("edit-address"),b=document.getElementById("edit-allergy"),i=document.getElementById("edit-diet"),p=document.getElementById("edit-gender"),v=document.getElementById("edit-medal-name");y&&!y.value&&(y.value=g.dob||""),h&&!h.value&&(h.value=g.address||""),b&&!b.value&&(b.value=g.medical_notes||""),i&&(i.value==="Không"||!i.value)&&(i.value=g.dietary||"Không"),p&&(p.value==="Khác"||!p.value)&&(p.value=g.gender||"Khác"),v&&!v.value&&(v.value=g.medal_name||"")}}catch(a){console.error("Pull CRM error:",a)}document.getElementById("edit-total").value=(parseInt(t.total_price)||0)+(parseInt(t.discount)||0),document.getElementById("edit-discount").value=t.discount||0,document.getElementById("edit-deposit").value=t.deposit||0,document.getElementById("edit-deposit-required").value=t.deposit_required||1e6,window.updateEditRemaining();const l=document.getElementById("editModal"),d=document.getElementById("editModalContent");l&&d&&(l.classList.remove("hidden"),setTimeout(()=>{l.classList.add("opacity-100"),d.classList.remove("scale-95","translate-y-4"),d.classList.add("scale-100","translate-y-0")},10))},window.actionDelete=async n=>{if(document.getElementById("rowActionModal").classList.add("hidden"),confirm("Bạn có chắc chắn muốn xóa Đơn hàng này? Thao tác không thể hoàn tác."))try{if((await fetch(`/api/bookings?id=${n}`,{method:"DELETE"})).ok)alert("✅ Đã xóa đơn hàng thành công!"),S();else throw new Error("Lỗi từ Server")}catch(t){alert("❌ Không thể xóa bảng ghi này: "+t.message)}},window.updateEditRemaining=()=>{const n=parseInt(document.getElementById("edit-total").value)||0,t=parseInt(document.getElementById("edit-discount").value)||0,o=parseInt(document.getElementById("edit-deposit").value)||0,d=n-t-o,a=document.getElementById("edit-remaining");a&&(a.textContent=d>0?d.toLocaleString("vi-VN")+"đ":"0đ")};const et=document.getElementById("edit-total"),nt=document.getElementById("edit-deposit");et&&et.addEventListener("input",window.updateEditRemaining),nt&&nt.addEventListener("input",window.updateEditRemaining),document.querySelectorAll('#bookingModal button[onclick*="hidden"]').forEach(n=>{n.removeAttribute("onclick"),n.addEventListener("click",window.closeModal)});const ot=async n=>{try{const o=await(await fetch("/api/admin_customers?action=create",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({full_name:n.name,phone:n.phone,cccd:n.id_card||"",dob:n.dob||null,gender:n.gender||"Khác",medical_notes:n.allergy||"",dietary:n.diet||""})})).json();o.success&&o.csr_code&&(console.log("✅ Đã đồng bộ CRM thành công:",n.name,o.csr_code),(await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:n.id,customer_id:o.csr_code})})).ok&&(console.log("✅ Đã cập nhật Booking mã CRM"),typeof S=="function"&&S()))}catch(t){console.error("❌ Lỗi đồng bộ CRM:",t)}},at=document.getElementById("bookingsTableBody");at&&at.addEventListener("click",async n=>{const t=n.target.closest(".action-btn");if(!t){const a=n.target.closest(".row-clickable");if(a){const r=a.getAttribute("data-id"),s=_.find(g=>g.id==r);s&&openRowActionModal(s)}return}const o=t.getAttribute("data-id"),l=_.find(a=>a.id==o);if(!l)return;const d=document.getElementById("rowActionModal");if(d&&d.classList.add("hidden"),t.classList.contains("confirm-deposit-btn")){if(confirm("Xác nhận khách hàng này đã chuyển khoản đặt cọc?"))try{if(t.disabled=!0,t.classList.add("opacity-50"),(await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:o,status:"Đã cọc"})})).ok)alert("✅ Đã xác nhận cọc thành công!"),ot(l),typeof S=="function"&&S();else throw new Error("Lỗi cập nhật API")}catch(a){alert("❌ Lỗi: "+a.message)}finally{t.disabled=!1,t.classList.remove("opacity-50")}return}if(t.classList.contains("delete-btn")){window.actionDelete(o);return}else if(t.classList.contains("process-btn")){const s=(window.location.hostname==="localhost"||window.location.hostname==="127.0.0.1"?"http://localhost:8888":window.location.origin)+`/booking/process.html?id=${o}`;navigator.clipboard.writeText(s).then(()=>{alert("📋 Đã sao chép Link Điền Thông Tin!\\nGửi cho khách: "+s)}).catch(g=>{alert("Lỗi khi Copy Clipboard. Link là: "+s)})}else if(t.classList.contains("pay-terms-btn")){const s=(window.location.hostname==="localhost"||window.location.hostname==="127.0.0.1"?"http://localhost:8888":window.location.origin)+`/pay-terms.html?id=${o}`;navigator.clipboard.writeText(s).then(()=>{alert("📋 Đã sao chép Link Cọc Tour!\\nGửi cho khách: "+s)}).catch(g=>{alert("Lỗi khi Copy Clipboard. Link là: "+s)})}else if(t.classList.contains("send-btn")){const s=(window.location.hostname==="localhost"||window.location.hostname==="127.0.0.1"?"http://localhost:8888":window.location.origin)+`/pay?id=${o}`;navigator.clipboard.writeText(s).then(()=>{alert("📋 Đã sao chép Link Thanh Toán Cọc! Gửi cho khách qua Zalo nhé:\\n"+s)}).catch(g=>{alert("Lỗi khi Copy Clipboard. Link là: "+s)})}else if(t.classList.contains("payment-btn")){const s=(window.location.hostname==="localhost"||window.location.hostname==="127.0.0.1"?"http://localhost:8888":window.location.origin)+`/pay?id=${o}`;window.open(s,"_blank")}else t.classList.contains("view-btn")?(document.getElementById("rowActionModal").classList.add("hidden"),window.actionView(o)):t.classList.contains("edit-btn")&&window.actionEdit(o)});const j=document.getElementById("smartSearch"),E=document.getElementById("searchSuggestions");if(j){let n;j.addEventListener("input",t=>{const o=t.target.value.toLowerCase().trim();if(clearTimeout(n),o.length<2){E&&E.classList.add("hidden");return}n=setTimeout(async()=>{try{const l=await fetch(`/api/admin_customers?action=search&keyword=${o}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({keyword:o})}),d=await l.json();if(l.ok&&d.success){const r=[d.data];E&&(E.innerHTML=r.map(s=>`
                        ...
                        `).join(""),E.classList.remove("hidden"))}else E&&(E.innerHTML='<div class="p-3 text-xs text-gray-400 italic text-center">Không tìm thấy khách hàng cũ</div>',E.classList.remove("hidden"))}catch(l){console.error("Search error:",l)}},300)}),E&&E.addEventListener("click",t=>{const o=t.target.closest(".suggestion-item");if(o){const l=document.getElementById("addFullName"),d=document.getElementById("addPhone"),a=document.getElementById("addCsrCode");l&&(l.value=o.getAttribute("data-name")),d&&(d.value=o.getAttribute("data-phone")),a&&(a.value=o.getAttribute("data-csr"));const r=document.getElementById("addDob"),s=document.getElementById("addGender"),g=document.getElementById("addIdCard"),y=document.getElementById("addAddress");r&&(r.value=o.getAttribute("data-dob")||""),s&&(s.value=o.getAttribute("data-gender")||"Khác"),g&&(g.value=o.getAttribute("data-cccd")||""),y&&(y.value=o.getAttribute("data-address")||"");const h=document.getElementById("addAllergy"),b=document.getElementById("addDiet"),i=document.getElementById("addTrekkingPole"),p=document.getElementById("addMedalName");h&&(h.value=o.getAttribute("data-allergy")||""),b&&(b.value=o.getAttribute("data-diet")||""),i&&(i.value=o.getAttribute("data-trekking_pole")||"Không"),p&&(p.value=o.getAttribute("data-medal-name")||""),window._selectedCustomer={fullName:o.getAttribute("data-name"),phone:o.getAttribute("data-phone"),csrCode:o.getAttribute("data-csr"),dob:o.getAttribute("data-dob")||"",gender:o.getAttribute("data-gender")||"",address:o.getAttribute("data-address")||"",id_card:o.getAttribute("data-cccd")||"",allergy:o.getAttribute("data-allergy")||"",diet:o.getAttribute("data-diet")||"",trekking_pole:o.getAttribute("data-trekking_pole")||"",medal_name:o.getAttribute("data-medal-name")||""},j.value=o.getAttribute("data-name"),E.classList.add("hidden");const v=`
                    ...
                    </div>`,B=document.getElementById("loyalty-alert");B&&B.remove(),j.parentElement.insertAdjacentHTML("afterend",v)}}),document.addEventListener("click",t=>{!j.contains(t.target)&&E&&!E.contains(t.target)&&E.classList.add("hidden")})}const H=document.getElementById("addTourPrice"),P=document.getElementById("addDiscount"),st=document.getElementById("addTotalPriceDisplay"),it=document.getElementById("addTourName"),F=()=>{if(!H||!P||!st)return;const n=parseInt(H.value)||0,t=parseInt(P.value)||0,o=n-t;st.textContent=(o>0?o:0).toLocaleString("vi-VN")+"đ"};H&&H.addEventListener("input",F),P&&P.addEventListener("input",F),it&&it.addEventListener("change",n=>{const t=n.target.value,o=V.find(l=>l.name===t);o&&H&&(H.value=o.price||0,F())});const lt=document.getElementById("searchBtn");lt&&lt.addEventListener("click",n=>{n.preventDefault(),document.getElementById("smartSearch").value||alert("Vui lòng nhập Số Điện Thoại hoặc Mã #CSR")});const R=document.getElementById("bookingForm");R&&R.addEventListener("submit",async n=>{n.preventDefault();const t=n.target.querySelector('button[type="submit"]'),o=t.textContent;t.textContent="Đang lưu hệ thống...",t.disabled=!0;try{const l=document.getElementById("addFullName").value,d=document.getElementById("addPhone").value,a=document.getElementById("addTourName").value,r=document.getElementById("addTourDate").value,s=document.getElementById("addDob").value,g=document.getElementById("addGender").value,y=document.getElementById("addIdCard").value,h=document.getElementById("addAddress").value,b=document.getElementById("addSpecial").value,i=document.getElementById("addAllergy").value,p=document.getElementById("addDiet").value,v=document.getElementById("addTrekkingPole").value,B=document.getElementById("addMedalName").value,L=parseInt(document.getElementById("addTourPrice").value)||0,e=parseInt(document.getElementById("addDiscount").value)||0,u=parseInt(document.getElementById("addDepositRequired").value)||1e6,c=document.getElementById("addCsrCode").value||"",x=localStorage.getItem("csr_user");let k=null,C="Admin";if(x){const I=JSON.parse(x);k=I.id,C=I.fullName||I.full_name||"Admin"}const w=document.getElementById("editingBookingId")?document.getElementById("editingBookingId").value:"",m={name:l,phone:d,tour:a,date:r,status:"Chờ cọc",total_price:L-e,discount:e,deposit:0,sale_id:k,sale_name:C,customer_id:c,deposit_required:u,commitments:!0,dob:s,gender:g,address:h,id_card:y,special:b,allergy:i,diet:p,trekking_pole:v,medal_name:B},T=d.replace(/[^0-9]/g,"");window._selectedCustomer&&(window._selectedCustomer.phone||"").replace(/[^0-9]/g,"")===T&&(m.dob=m.dob||window._selectedCustomer.dob,m.gender=m.gender&&m.gender!=="Khác"?m.gender:window._selectedCustomer.gender||"Khác",m.address=m.address||window._selectedCustomer.address,m.id_card=m.id_card||window._selectedCustomer.id_card,m.allergy=window._selectedCustomer.allergy,m.diet=window._selectedCustomer.diet,m.trekking_pole=window._selectedCustomer.trekking_pole,m.medal_name=window._selectedCustomer.medal_name),w&&(m.id=w,delete m.status,delete m.deposit,delete m.sale_id,delete m.sale_name);const M=await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(m)});if(M.ok){alert(w?"✅ Đã lưu Cập nhật Đơn hàng!":"✅ Thêm Đơn Hàng Thành Công!"),window.closeModal(),R&&R.reset(),window._selectedCustomer=null;const I=document.getElementById("loyalty-alert");I&&I.remove(),document.getElementById("addTotalPriceDisplay")&&(document.getElementById("addTotalPriceDisplay").textContent="0đ"),typeof S=="function"?S():window.location.reload()}else{const I=await M.json();throw new Error(I.error||"Lỗi tạo Booking!")}}catch(l){alert("❌ "+l.message)}finally{t.textContent=o,t.disabled=!1}});const dt=document.getElementById("editForm");dt&&dt.addEventListener("submit",async n=>{n.preventDefault();const t=n.target.querySelector('button[type="submit"]'),o=t.textContent;t.textContent="Đang lưu cập nhật...",t.disabled=!0;try{const l=document.getElementById("edit-id").value,d=document.getElementById("edit-name").value,a=document.getElementById("edit-phone").value,r=document.getElementById("edit-medal-name").value,s=document.getElementById("edit-tour").value,g=document.getElementById("edit-date").value,y=document.getElementById("edit-dob").value,h=document.getElementById("edit-gender").value,b=document.getElementById("edit-status").value,i=document.getElementById("edit-allergy").value,p=document.getElementById("edit-address").value,v=document.getElementById("edit-diet").value,B=document.getElementById("edit-trekking-pole").value,L=document.getElementById("edit-commitments").checked,e=document.getElementById("edit-special").value,u=document.getElementById("edit-sale");let c,x;if(u&&u.parentElement&&!u.parentElement.classList.contains("hidden")&&(c=u.value||null,x="Website",c)){const I=G.find(gt=>String(gt.id)===String(c));I&&(x=I.full_name||I.fullName)}const k=parseInt(document.getElementById("edit-total").value)||0,C=parseInt(document.getElementById("edit-discount").value)||0,w=parseInt(document.getElementById("edit-deposit").value)||0,m=parseInt(document.getElementById("edit-deposit-required").value)||1e6,T={id:l,name:d,phone:a,medal_name:r,tour:s,date:g,dob:y,gender:h,status:b,allergy:i,address:p,diet:v,trekking_pole:B,commitments:L,special:e,sale_id:c,sale_name:x,total_price:k-C,discount:C,deposit:w,deposit_required:m},M=await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(T)});if(M.ok)alert("✅ Đã cập nhật thành công Chi tiết Đơn hàng!"),(b.includes("Đã cọc")||T.total_price>0&&T.total_price===w)&&ot(T),window.closeEditModal(),typeof S=="function"&&S();else{const I=await M.json();throw new Error(I.error||"Gặp sự cố khi Cập nhật API.")}}catch(l){alert("❌ Lỗi Cập nhật: "+l.message)}finally{t.textContent=o,t.disabled=!1}})};export{yt as afterRender,ft as render};
