import{S as z,H as V}from"./Header-CvqOcIss.js";const O=()=>`
    ...
    `,U=async()=>{const p=[{id:"col-stt",label:"STT",always:!0,render:(e,t)=>t+1},{id:"col-name",label:"Họ và Tên",default:!0,render:e=>`<div class="font-bold text-gray-900">${e.name}</div>`},{id:"col-dob",label:"Ngày Sinh",default:!0,render:e=>{if(!e.dob)return"-";const t=e.dob.split("-");return t.length===3?`${t[2]}/${t[1]}/${t[0]}`:e.dob}},{id:"col-phone",label:"SĐT",default:!0,render:e=>`<div class="font-medium text-gray-700">${e.phone||"-"}</div>`},{id:"col-gender",label:"Giới tính",default:!0,render:e=>e.gender||"-"},{id:"col-cccd",label:"CCCD / Passport",default:!0,render:e=>e.id_card||e.cccd||"-"},{id:"col-address",label:"Địa chỉ",default:!0,render:e=>`<div class="max-w-[200px] truncate print:max-w-none print:whitespace-normal" title="${e.address||""}">${e.address||"-"}</div>`},{id:"col-diet",label:"Ăn chay",default:!1,render:e=>e.diet==="Chay"||e.diet==="Có"?'<span class="text-green-600 font-bold">Có</span>':"Không"},{id:"col-pole",label:"Gậy Trekking",default:!1,render:e=>e.trekking_pole==="Có"?'<span class="text-orange-600 font-bold">Có</span>':"Không"},{id:"col-allergy",label:"Dị ứng",default:!1,render:e=>e.medical_notes||e.allergy||"-"},{id:"col-special",label:"Yêu cầu ĐB",default:!1,render:e=>e.special_notes||e.special||"-"},{id:"col-medal",label:"Tên Huy Chương",default:!1,render:e=>e.medal_name||e.name||"-"}],S=new URLSearchParams(window.location.search),i=S.get("tour")||"",g=S.get("date")||"";let m=g;const $=e=>{if(!e)return"";const t=e.split(/[-/.]/);if(t.length!==3)return e;let n,r,o;return t[0].length===4?[o,r,n]=t:[n,r,o]=t,`${n.toString().padStart(2,"0")}/${r.toString().padStart(2,"0")}/${o}`},L=e=>(e||"").toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g,"").replace(/[-_]/g," ").replace(/\s+/g," ").trim();g&&(m=$(g));const D=document.getElementById("roster-title"),I=document.getElementById("roster-subtitle"),B=document.getElementById("print-tour-name"),M=document.getElementById("print-tour-date"),j=document.getElementById("print-total"),v=document.getElementById("filterStatus"),y=document.getElementById("filterGender"),w=document.getElementById("rosterList");let f=[],u=new Set(p.filter(e=>e.default||e.always).map(e=>e.id));i&&(D.textContent=i,B.textContent=i),m&&(I.innerHTML=`Ngày khởi hành: <span class="font-bold text-csr-orange">${m}</span>`,M.textContent=m);const N=()=>{const e=document.getElementById("column-toggles");e.innerHTML=p.filter(t=>!t.always).map(t=>`
        ...
        `).join(""),document.querySelectorAll(".col-toggle-cb").forEach(t=>{t.addEventListener("change",n=>{n.target.checked?u.add(n.target.value):u.delete(n.target.value),R()})})},A=()=>{const e=document.getElementById("table-header-row");e.innerHTML=p.map(t=>`
            <th class="p-4 whitespace-nowrap ${t.id}" data-col="${t.id}">${t.label}</th>
        `).join("")},R=()=>{p.forEach(e=>{const t=u.has(e.id),n=document.querySelector(`th[data-col="${e.id}"]`);n&&(t?n.classList.remove("col-hidden"):n.classList.add("col-hidden")),document.querySelectorAll(`td[data-col="${e.id}"]`).forEach(r=>{t?r.classList.remove("col-hidden"):r.classList.add("col-hidden")})})},k=e=>{if(!e)return null;e.includes(" - ")&&(e=e.split(" - ")[0].trim()),e.includes("T")&&(e=e.split("T")[0]);const t=e.split(/[-/.]/);return t.length===2?{day:parseInt(t[0]),month:parseInt(t[1]),year:null}:t.length===3?t[0].length===4?{day:parseInt(t[2]),month:parseInt(t[1]),year:parseInt(t[0])}:{day:parseInt(t[0]),month:parseInt(t[1]),year:parseInt(t[2])}:null},h=k(g),P=e=>{if(!h)return!0;const t=k(e);return!(!t||t.day!==h.day||t.month!==h.month||t.year&&h.year&&t.year!==h.year)},F=async()=>{try{const t=await(await fetch("/api/bookings")).json();console.log("[Roster] targetTour:",i,"targetDateStr:",g,"targetParsed:",h),console.log("[Roster] Total bookings:",t.length),t.length>0&&t.filter(r=>r.tour&&r.tour.includes("Liêng")).slice(0,3).forEach(r=>console.log("[Roster] Sample Liêng:",{tour:r.tour,date:r.date,parsed:k(r.date)})),f=t.filter(n=>{let r=!0,o=!0;if(i){const s=L(n.tour),d=L(i);r=s===d||s.includes(d)||d.includes(s)}return g&&(o=P(n.date)),r&&o}),console.log("[Roster] Matched bookings:",f.length),f.sort((n,r)=>{const o=(n.status||"").includes("cọc")||n.status==="Hoàn tất"?1:0;return((r.status||"").includes("cọc")||r.status==="Hoàn tất"?1:0)-o}),C()}catch(e){console.error("Lỗi khi tải dữ liệu đoàn:",e),w.innerHTML='<tr><td colspan="12" class="p-8 text-center text-red-500 font-bold">Lỗi khi tải dữ liệu. Vui lòng tải lại trang.</td></tr>'}},C=()=>{const e=v.value.toLowerCase(),t=y.value.toLowerCase(),n=f.filter(r=>{const o=(r.status||"").toLowerCase(),s=e===""||e==="chờ cọc"&&(o.includes("chờ cọc")||o.includes("chờ xác nhận"))||e==="đã cọc"&&(o.includes("đã cọc")||o.includes("hoàn tất")||o.includes("hoàn thành"))||e==="đã hủy"&&o.includes("hủy"),d=(r.gender||"").toLowerCase();return s&&(t===""||d===t)});if(document.getElementById("text-visible-count").textContent=n.length,j.textContent=n.length,n.length===0){w.innerHTML='<tr><td colspan="12" class="p-12 text-center text-gray-500 bg-gray-50">Không tìm thấy khách hàng nào khớp với điều kiện lọc.</td></tr>';return}w.innerHTML=n.map((r,o)=>'<tr class="hover:bg-gray-50/50 transition-colors">'+p.map(s=>`
                    <td class="p-4 align-top ${u.has(s.id)?"":"col-hidden"}" data-col="${s.id}">
                        ${s.render(r,o)}
                    </td>
                `).join("")+"</tr>").join("")},_=()=>{const e=p.filter(a=>u.has(a.id));let t="\\uFEFF";t+=e.map(a=>`"${a.label}"`).join(",")+`
`;const n=v.value.toLowerCase(),r=y.value.toLowerCase();f.filter(a=>{const b=n===""||n==="chờ cọc"&&(a.status==="Chờ cọc"||a.status==="Chờ xác nhận cọc")||n==="đã cọc"&&(a.status==="Đã cọc"||a.status==="Hoàn tất"||a.status==="Hoàn thành")||n==="đã hủy"&&a.status==="Đã hủy",T=r===""||a.gender&&a.gender.toLowerCase()===r;return b&&T}).forEach((a,b)=>{const T=e.map(c=>{let l="";if(c.id==="col-stt")l=b+1;else if(c.id==="col-name")l=a.name||"";else if(c.id==="col-address")l=a.address||"";else if(c.id==="col-phone")l=a.phone?`'${a.phone}`:"";else if(c.id==="col-diet")l=a.diet==="Chay"||a.diet==="Có"?"Có":"Không";else if(c.id==="col-pole")l=a.trekking_pole==="Có"?"Có":"Không";else{const G=String(c.render(a,b)),E=document.createElement("DIV");E.innerHTML=G,l=E.textContent||E.innerText||""}return l=String(l).replace(/"/g,'""'),`"${l}"`});t+=T.join(",")+`
`});const s=new Blob([t],{type:"text/csv;charset=utf-8;"}),d=URL.createObjectURL(s),H=`DS_Doan_${i||"Tour"}_${m||"Date"}.csv`.replace(/[/\\?%*:|"<>]/g,"-"),x=document.createElement("a");x.setAttribute("href",d),x.setAttribute("download",H),document.body.appendChild(x),x.click(),document.body.removeChild(x)};v.addEventListener("change",C),y.addEventListener("change",C),document.getElementById("btnExportPDF").addEventListener("click",()=>{window.print()}),document.getElementById("btnExportExcel").addEventListener("click",()=>{_()}),N(),A(),F()};export{U as afterRender,O as render};
