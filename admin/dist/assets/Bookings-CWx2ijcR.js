import{S as ht,H as bt}from"./Header-CamAZjvb.js";console.log("Bookings V3 - Cache Busted!");const yt=()=>`
    ...
    `,vt=()=>{let M=[],f="consult",N=[],J=[],z=[];const ut=async()=>{try{const[n,t,o]=await Promise.all([fetch("/api/tours"),fetch("/api/schedules"),fetch("/api/users")]);N=n.ok?await n.json():[],J=t.ok?await t.json():[],z=o.ok?await o.json():[],X("addTourName"),X("edit-tour"),pt("edit-sale")}catch(n){console.error("Lỗi tải tours/schedules:",n)}},pt=n=>{const t=document.getElementById(n);t&&(t.innerHTML='<option value="">-- Website --</option>',z.forEach(o=>{const l=document.createElement("option");l.value=o.id,l.textContent=`${o.full_name||o.fullName} (${o.role})`,t.appendChild(l)}))},X=n=>{const t=document.getElementById(n);if(!t)return;const o=t.value,l=t.options[0].outerHTML;t.innerHTML=l,N.filter(i=>i.is_visible!==!1).forEach(i=>{const c=document.createElement("option");c.value=i.name,c.textContent=i.name,i.name===o&&(c.selected=!0),t.appendChild(c)})},V=(n,t,o,l)=>{const i=document.getElementById(n);if(!i)return;if(i.innerHTML='<option value="">-- Chọn Lịch --</option>',!t){i.innerHTML='<option value="">-- Chọn Tour trước --</option>';return}const c=J.filter(s=>s.tour_name===t||t.includes(s.tour_name)||s.tour_name.includes(t));if(c.length===0){i.innerHTML='<option value="">Chưa có lịch cho tour này</option>';return}c.sort((s,a)=>new Date(s.start_date)-new Date(a.start_date)).forEach(s=>{const a=parseInt(s.booked_count)||0,r=(s.slots||0)-a,h=new Date(s.start_date),p=s.end_date?new Date(s.end_date):null,b=k=>`${k.getDate().toString().padStart(2,"0")}/${(k.getMonth()+1).toString().padStart(2,"0")}`;let d=b(h);p&&(d+=" - "+b(p)),s.group_label&&(d+=` · ${s.group_label}`),d+=` (${r>=0?r:0} chỗ trống)`;const m=document.createElement("option"),x=`${h.getDate().toString().padStart(2,"0")}/${(h.getMonth()+1).toString().padStart(2,"0")}/${h.getFullYear()}`;m.value=`${s.id}::${x}`,m.dataset.scheduleId=s.id,m.dataset.date=x,m.textContent=d,r<=0&&(m.disabled=!0,m.textContent=d.replace("chỗ trống","HẾT CHỖ")),(l&&String(s.id)===String(l)||!l&&o&&x===o)&&(m.selected=!0),i.appendChild(m)})},Y=document.getElementById("addTourName");Y&&Y.addEventListener("change",n=>{const t=n.target.value;V("addTourDate",t);const o=N.find(i=>i.name===t),l=document.getElementById("addTourPrice");if(o&&l){const i=parseInt(o.price)||0;l.value=i;const c=document.getElementById("addDiscount"),s=document.getElementById("addTotalPriceDisplay"),a=parseInt(c?c.value:0)||0,r=i-a;s&&(s.textContent=(r>0?r:0).toLocaleString("vi-VN")+"đ")}});const Q=document.getElementById("edit-tour");Q&&Q.addEventListener("change",n=>{const t=n.target.value;V("edit-date",t);const o=N.find(i=>i.name===t),l=document.getElementById("edit-total");o&&l&&(parseInt(l.value)===0||!l.value)&&(l.value=parseInt(o.price)||0,typeof window.updateEditRemaining=="function"&&window.updateEditRemaining())}),ut();const j=()=>{const n=document.getElementById("bookingsTableHead"),t=document.getElementById("bookingsTableBody");if(!t||!n)return;const o=f==="upcoming"||f==="ready"||f==="completed";o?n.innerHTML=`
            ...
            `;const l=document.getElementById("filterSearch")?document.getElementById("filterSearch").value.toLowerCase():"",i=document.getElementById("filterTour")?document.getElementById("filterTour").value:"",c=document.getElementById("filterDate")?document.getElementById("filterDate").value:"",s=document.getElementById("filterSale")?document.getElementById("filterSale").value:"",a=document.getElementById("filterStatus")?document.getElementById("filterStatus").value:"";let r=M.filter(e=>{let g=!1;const u=parseInt(e.total_price)>0&&parseInt(e.total_price)===parseInt(e.deposit),y=C=>{if(!C)return!1;try{const T=C.split("/");if(T.length===3){const L=new Date(`${T[2]}-${T[1]}-${T[0]}T00:00:00`),E=new Date;return E.setHours(0,0,0,0),L.getTime()<E.getTime()}else if(C.includes("-")){const L=new Date(`${C}T00:00:00`),E=new Date;return E.setHours(0,0,0,0),L.getTime()<E.getTime()}}catch{}return!1},w=(e.status==="Hoàn thành"||e.status==="Đã đi"||u)&&y(e.date);return f==="consult"?g=e.status==="Chờ tư vấn"&&!w:f==="pending"?g=(!e.status||e.status==="Chờ cọc"||e.status==="Chờ xác nhận cọc")&&!w:f==="upcoming"?g=e.status&&e.status!=="Chờ tư vấn"&&e.status!=="Chờ cọc"&&e.status!=="Chờ xác nhận cọc"&&!u&&e.status!=="Hoàn thành"&&!w:f==="ready"?g=(u||e.status==="Hoàn thành")&&!w:f==="completed"&&(g=w),!(!g||i&&e.tour!==i||c&&F(e.date)!==c||s&&s!=="null"&&e.sale_name!==s||s==="null"&&e.sale_name&&e.sale_name!=="Website"||l&&!`${e.name||""} ${e.phone||""} ${e.customer_id||""}`.toLowerCase().includes(l)||a&&e.status!==a)});const h=document.getElementById("statTitle1"),p=document.getElementById("statTitle2"),b=document.getElementById("statTitle3"),d=document.getElementById("statTotalCustomers"),m=document.getElementById("statTotalRevenue"),x=document.getElementById("statTotalCollected");let k=r.length;if(f==="consult"){h&&(h.textContent="Tổng Leads Tư Vấn"),p&&(p.textContent="Chưa Có Sale Nhận"),b&&(b.textContent="Đã Có Sale Nhận");let e=0,g=0;r.forEach(u=>{!u.sale_id||u.sale_id==="null"||!u.sale_name||u.sale_name==="Website"||u.sale_name==="null"?e++:g++}),d&&(d.textContent=k),m&&(m.textContent=e+" Đơn"),x&&(x.textContent=g+" Đơn")}else if(f==="pending"){h&&(h.textContent="Tổng Khách (Chờ Cọc)"),p&&(p.textContent="Khách Chưa Tư Vấn (Mới)"),b&&(b.textContent="Đã Có Sale Giữ Chỗ");let e=0,g=0;r.forEach(u=>{!u.sale_id||u.sale_id==="null"||!u.sale_name||u.sale_name==="Website"||u.sale_name==="null"?e++:g++}),d&&(d.textContent=k),m&&(m.textContent=e+" Đơn"),x&&(x.textContent=g+" Đơn")}else if(f==="upcoming"){h&&(h.textContent="Tổng Số Khách"),p&&(p.textContent="Chưa Xác Nhận Cọc"),b&&(b.textContent="Chờ Thanh Toán Còn Lại");let e=0,g=0;r.forEach(u=>{u.status==="Chờ xác nhận cọc"?e++:parseInt(u.deposit)>0&&parseInt(u.total_price)>parseInt(u.deposit)&&g++}),d&&(d.textContent=k),m&&(m.textContent=e+" Khách"),x&&(x.textContent=g+" Khách")}else if(f==="ready"){h&&(h.textContent="Khách Sẵn Sàng (Full)"),p&&(p.textContent="Tổng Doanh Thu Tab"),b&&(b.textContent="Thực Thu (Full Tận Nơi)");let e=0,g=0;r.forEach(u=>{e+=parseInt(u.total_price)||0,g+=parseInt(u.deposit)||0}),d&&(d.textContent=k),m&&(m.textContent=e>0?e.toLocaleString("vi-VN")+"đ":"0đ"),x&&(x.textContent=g>0?g.toLocaleString("vi-VN")+"đ":"0đ")}else{h&&(h.textContent="Khách Đã Xong (Lịch Sử)"),p&&(p.textContent="Tổng Doanh Thu Tab"),b&&(b.textContent="Thực Thu (Đã Đóng)");let e=0,g=0;r.forEach(u=>{e+=parseInt(u.total_price)||0,g+=parseInt(u.deposit)||0}),d&&(d.textContent=k),m&&(m.textContent=e>0?e.toLocaleString("vi-VN")+"đ":"0đ"),x&&(x.textContent=g>0?g.toLocaleString("vi-VN")+"đ":"0đ")}const B=o?8:4;if(!r||r.length===0){let e=f==="consult"?"Chưa có khách hàng tư vấn nào.":f==="pending"?"Chưa có Khách chờ cọc.":f==="upcoming"?"Chưa có Khách nào Đã Cọc.":f==="ready"?"Chưa có khách sẵn sàng.":"Danh sách rỗng.";t.innerHTML=`<tr><td colspan="${B}" class="p-8 text-center text-gray-500">${e}</td></tr>`;return}t.innerHTML=r.map(e=>{const g=f==="upcoming"||f==="ready"?"bg-green-50/20 hover:bg-green-50 transition-colors":"hover:bg-gray-100 transition-colors";if(o){let u=parseInt(e.total_price)||0;if(u===0&&e.tour&&N&&N.length>0){const v=N.find(D=>D.name&&e.tour&&D.name.toLowerCase().trim()===e.tour.toLowerCase().trim());v&&parseInt(v.price)>0&&(u=parseInt(v.price))}const y=parseInt(e.deposit)||0,w=u-y,C=u>0?u.toLocaleString("vi-VN")+"đ":"Chưa định giá",T=y>0?y.toLocaleString("vi-VN")+"đ":"0đ",L=w>0?w.toLocaleString("vi-VN")+"đ":w===0&&u>0?"Đã thu trọn":"-";let E="";e.status==="Hoàn thành"||e.status==="Đã đi"?E='<span class="bg-gray-100 text-gray-600 border border-gray-200 px-2 py-0.5 rounded text-xs block w-full text-center">Hoàn thành</span>':e.status==="Chờ xác nhận cọc"?E=`<button class="action-btn confirm-deposit-btn bg-csr-orange text-white px-3 py-1.5 rounded text-xs font-bold shadow-sm hover:bg-[#d65503] w-full" data-id="${e.id}">Xác nhận cọc</button>`:y>0&&w===0?E='<span class="bg-green-100 text-green-700 border border-green-200 px-2 py-0.5 rounded text-xs font-medium block w-full text-center">Hoàn tất phí</span>':E='<span class="bg-blue-100 text-blue-700 border border-blue-200 px-2 py-0.5 rounded text-xs font-medium block w-full text-center">Đã Cọc</span>';let $="";return y>0&&($=`<a href="/invoice.html?id=${e.id}" target="_blank" class="mt-1.5 block text-center text-[11px] text-blue-600 hover:text-blue-800 hover:underline font-medium" onclick="event.stopPropagation()">🧾 Xem hóa đơn</a>`),`
    ...
    `}}).join(""),document.querySelectorAll(".claim-btn").forEach(e=>{e.addEventListener("click",async g=>{g.stopPropagation();const u=e.getAttribute("data-id"),y=localStorage.getItem("csr_user");let w=null,C="Admin";if(y){const T=JSON.parse(y);w=T.id,C=T.fullName||T.full_name||"Admin"}e.textContent="Đang xử lý...",e.disabled=!0;try{if((await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:u,sale_id:w,sale_name:C})})).ok){const L=M.find(E=>E.id==u);L&&(L.sale_id=w,L.sale_name=C),j()}else alert("Lỗi khi nhận khách. Vui lòng thử lại."),e.textContent="Nhận khách",e.disabled=!1}catch(T){console.error("Lỗi nhận khách:",T),e.textContent="Nhận khách",e.disabled=!1}})})},mt=()=>{const n=new Set,t=new Set,o=new Set;M.forEach(s=>{if(s.tour&&n.add(s.tour),s.date){const a=F(s.date);a&&t.add(a)}s.sale_name&&s.sale_name!=="Website"&&s.sale_name!=="null"&&o.add(s.sale_name.trim())});const l=document.getElementById("filterTour"),i=document.getElementById("filterDate"),c=document.getElementById("filterSale");if(l){const s=l.value;l.innerHTML='<option value="">Tất cả Tour</option>',[...n].sort().forEach(a=>l.innerHTML+=`<option value="${a}">${a}</option>`),l.value=s}if(i){const s=i.value;i.innerHTML='<option value="">Tất cả Ngày</option>',[...t].sort().forEach(a=>i.innerHTML+=`<option value="${a}">${a}</option>`),i.value=s}if(c){const s=c.value;c.innerHTML='<option value="">Tất cả Sale</option>',c.innerHTML+='<option value="null">Nguồn Website</option>',[...o].sort().forEach(a=>{c.innerHTML+=`<option value="${a}">${a}</option>`}),c.value=s}};function F(n){if(!n)return"";if(/^\d{1,2}\/\d{1,2}\s*-\s*\d{1,2}\/\d{1,2}$/.test(n))return n.replace(/\s+/g,"");try{if(/^\d{4}-\d{2}-\d{2}$/.test(n)){const[t,o,l]=n.split("-"),i=`${parseInt(l)}/${parseInt(o)}`;return`${i}-${i}`}if(/^\d{1,2}\/\d{1,2}\/\d{4}$/.test(n)){const[t,o,l]=n.split("/"),i=`${parseInt(t)}/${parseInt(o)}`;return`${i}-${i}`}if(n.includes("-"))return n.split("-").map(o=>{const l=o.trim();if(/\/\d{4}$/.test(l)){const[i,c]=l.split("/");return`${parseInt(i)}/${parseInt(c)}`}return l}).join("-")}catch{console.warn("Date normalization failed for:",n)}return n}const S=async()=>{try{M=await(await fetch("/api/bookings")).json(),mt(),j()}catch(n){console.error("Lỗi parse list đơn hàng:",n);const t=document.getElementById("bookingsTableBody");t&&(t.innerHTML='<tr><td colspan="4" class="p-8 text-center text-red-500">Lỗi lấy dữ liệu từ Server.</td></tr>')}};S(),["filterSearch","filterTour","filterDate","filterSale","filterStatus"].forEach(n=>{const t=document.getElementById(n);t&&(n==="filterSearch"?t.addEventListener("input",j):t.addEventListener("change",j))});const Z=document.getElementById("exportExcelBtn");Z&&Z.addEventListener("click",()=>{const n=document.getElementById("filterSearch")?document.getElementById("filterSearch").value.toLowerCase():"",t=document.getElementById("filterTour")?document.getElementById("filterTour").value:"",o=document.getElementById("filterDate")?document.getElementById("filterDate").value:"",l=document.getElementById("filterSale")?document.getElementById("filterSale").value:"";let i=M.filter(d=>{let m=!1;const x=parseInt(d.total_price)>0&&parseInt(d.total_price)===parseInt(d.deposit),k=e=>{if(!e)return!1;try{const g=e.split("/");if(g.length===3){const u=new Date(`${g[2]}-${g[1]}-${g[0]}T00:00:00`),y=new Date;return y.setHours(0,0,0,0),u.getTime()<y.getTime()}else if(e.includes("-")){const u=new Date(`${e}T00:00:00`),y=new Date;return y.setHours(0,0,0,0),u.getTime()<y.getTime()}}catch{}return!1},B=(d.status==="Hoàn thành"||d.status==="Đã đi"||x)&&k(d.date);return f==="consult"?m=d.status==="Chờ tư vấn"&&!B:f==="pending"?m=(!d.status||d.status==="Chờ cọc")&&!B:f==="upcoming"?m=d.status&&d.status!=="Chờ tư vấn"&&d.status!=="Chờ cọc"&&!x&&d.status!=="Hoàn thành"&&!B:f==="ready"?m=(x||d.status==="Hoàn thành")&&!B:f==="completed"&&(m=B),!(!m||t&&d.tour!==t||o&&F(d.date)!==o||l&&l!=="null"&&d.sale_name!==l||l==="null"&&d.sale_name&&d.sale_name!=="Website"||n&&!`${d.name||""} ${d.phone||""} ${d.customer_id||""} `.toLowerCase().includes(n))});if(i.length===0){alert("Danh sách rỗng. Không có dữ liệu để xuất.");return}const s=[["Mã KH (CRM)","Họ và Tên","Số điện thoại","Tên Tour","Lịch trình","Trạng thái","Nguồn / Sale","Đã cọc","Tổng tiền","Ghi chú"]];i.forEach(d=>{s.push([d.customer_id||"",`"${d.name||""}"`,`= "${d.phone||""}"`,`"${d.tour||""}"`,`"${d.date||""}"`,`"${d.status||""}"`,`"${d.sale_name||"Website"}"`,d.deposit||0,d.total_price||0,`"${(d.special||"").replace(/"/g,'""')}"`])});const a="\\uFEFF"+s.map(d=>d.join(",")).join(`
`),r=new Blob([a],{type:"text/csv;charset=utf-8;"}),h=URL.createObjectURL(r),p=document.createElement("a");p.setAttribute("href",h);let b=f==="pending"?"ChoCoc":f==="upcoming"?"SapThamGia":f==="ready"?"ChoLenXe":"LichSu";p.setAttribute("download",`BaoCao_DonHang_${b}_${new Date().toISOString().slice(0,10)}.csv`),document.body.appendChild(p),p.click(),document.body.removeChild(p)});const tt=document.querySelectorAll(".tab-btn");tt.forEach(n=>{n.addEventListener("click",t=>{tt.forEach(o=>{o.classList.remove("border-csr-orange","text-csr-orange"),o.classList.add("border-transparent","text-gray-500")}),t.target.classList.add("border-csr-orange","text-csr-orange"),t.target.classList.remove("border-transparent","text-gray-500"),f=t.target.getAttribute("data-tab"),j()})});const H=document.getElementById("bookingModal"),A=document.getElementById("bookingModalContent"),U=document.getElementById("detailModal"),et=document.getElementById("detailModalContent"),K=document.getElementById("editModal"),W=document.getElementById("editModalContent"),nt=document.getElementById("addBookingBtn");H&&(H.classList.add("duration-200","ease-out"),A.classList.add("duration-300","ease-out")),K&&(K.classList.add("duration-200","ease-out"),W.classList.add("duration-300","ease-out")),nt&&nt.addEventListener("click",()=>{document.getElementById("bookingModalContent").querySelector("h2").innerText="Thêm Khách Hàng (Tạo Đơn)",document.getElementById("submitBookingBtn").innerText="Tạo Phơi Đăng Ký",document.getElementById("editingBookingId").value="",document.getElementById("bookingForm").reset();const n=document.getElementById("loyalty-alert");n&&n.remove(),H.classList.remove("hidden"),setTimeout(()=>{H.classList.add("opacity-100"),A.classList.remove("scale-95","translate-y-4"),A.classList.add("scale-100","translate-y-0")},10)}),window.closeModal=()=>{H&&(H.classList.remove("opacity-100"),A.classList.remove("scale-100","translate-y-0"),A.classList.add("scale-95","translate-y-4"),setTimeout(()=>{H.classList.add("hidden")},200))},window.closeDetailModal=()=>{U&&(U.classList.remove("opacity-100"),et.classList.remove("scale-100","translate-y-0"),et.classList.add("scale-95","translate-y-4"),setTimeout(()=>{U.classList.add("hidden")},200))},window.closeEditModal=()=>{K&&(K.classList.remove("opacity-100"),W.classList.remove("scale-100","translate-y-0"),W.classList.add("scale-95","translate-y-4"),setTimeout(()=>{K.classList.add("hidden")},200))},window.openRowActionModal=n=>{document.getElementById("ramName").textContent=n.name||"Khách Hàng Này",document.getElementById("ramPhone").textContent=n.customer_id?`🥇 ${n.customer_id.toUpperCase()}`:n.phone||"";const t=`
        ...
        `;document.getElementById("detailContentBlock").innerHTML=i,document.getElementById("detailModal").classList.remove("hidden"),setTimeout(()=>{document.getElementById("detailModal").classList.add("opacity-100");const c=document.getElementById("detailModalContent");c&&(c.classList.remove("scale-95","translate-y-4"),c.classList.add("scale-100","translate-y-0"))},10)},window.actionEdit=async n=>{document.getElementById("rowActionModal").classList.add("hidden");const t=M.find(r=>r.id==n);if(!t)return;document.getElementById("edit-id").value=n,document.getElementById("edit-name").value=t.name||"",document.getElementById("edit-phone").value=t.phone||"",document.getElementById("edit-medal-name").value=t.medal_name||"",document.getElementById("edit-tour").value=t.tour||"",typeof V=="function"&&V("edit-date",t.tour||"",t.date||"",t.schedule_id||null),(r=>{let h=r.dob||"";if(typeof h=="string"&&h.includes("/")){const p=h.split("/");p.length===3&&(h=`${p[2]}-${p[1].padStart(2,"0")}-${p[0].padStart(2,"0")}`)}document.getElementById("edit-dob").value=h,document.getElementById("edit-gender").value=r.gender||"Khác",document.getElementById("edit-allergy").value=r.allergy||r.medical_notes||"",document.getElementById("edit-address").value=r.address||"",document.getElementById("edit-diet").value=r.diet||r.dietary||"Không",document.getElementById("edit-trekking-pole").value=r.trekking_pole||"Không"})(t),document.getElementById("edit-status").value=t.status||"Chờ xác nhận cọc",document.getElementById("edit-commitments").checked=!!t.commitments,document.getElementById("edit-special").value=t.special||"";try{const r=localStorage.getItem("csr_user");if(r&&JSON.parse(r).role==="admin"){const p=document.getElementById("edit-sale-container");p&&p.classList.remove("hidden");const b=document.getElementById("edit-sale");b&&(b.value=t.sale_id||"")}}catch{}if((!t.dob||!t.address||!t.gender||t.gender==="Khác")&&(t.customer_id||t.phone))try{const r=t.customer_id||t.phone,h=await fetch(`/api/admin_customers?action=search&keyword=${encodeURIComponent(r)}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({keyword:r})}),p=await h.json();if(h.ok&&p.success){const b=p.data,d=document.getElementById("edit-dob"),m=document.getElementById("edit-address"),x=document.getElementById("edit-allergy"),k=document.getElementById("edit-diet"),B=document.getElementById("edit-gender"),e=document.getElementById("edit-medal-name");d&&!d.value&&(d.value=b.dob||""),m&&!m.value&&(m.value=b.address||""),x&&!x.value&&(x.value=b.medical_notes||""),k&&(k.value==="Không"||!k.value)&&(k.value=b.dietary||"Không"),B&&(B.value==="Khác"||!B.value)&&(B.value=b.gender||"Khác"),e&&!e.value&&(e.value=b.medal_name||"")}}catch(r){console.error("Pull CRM error:",r)}let i=(parseInt(t.total_price)||0)+(parseInt(t.discount)||0);if(i===0&&t.tour){const r=N.find(h=>h.name===t.tour);r&&parseInt(r.price)>0&&(i=parseInt(r.price))}document.getElementById("edit-total").value=i,document.getElementById("edit-discount").value=t.discount||0,document.getElementById("edit-deposit").value=t.deposit||0;const c=parseInt(t.deposit_required);document.getElementById("edit-deposit-required").value=isNaN(c)?1e6:c,window.updateEditRemaining();const s=document.getElementById("editModal"),a=document.getElementById("editModalContent");s&&a&&(s.classList.remove("hidden"),setTimeout(()=>{s.classList.add("opacity-100"),a.classList.remove("scale-95","translate-y-4"),a.classList.add("scale-100","translate-y-0")},10))},window.actionDelete=async n=>{if(document.getElementById("rowActionModal").classList.add("hidden"),confirm("Bạn có chắc chắn muốn xóa Đơn hàng này? Thao tác không thể hoàn tác."))try{if((await fetch(`/api/bookings?id=${n}`,{method:"DELETE"})).ok)alert("✅ Đã xóa đơn hàng thành công!"),S();else throw new Error("Lỗi từ Server")}catch(t){alert("❌ Không thể xóa bảng ghi này: "+t.message)}},window.updateEditRemaining=()=>{const n=parseInt(document.getElementById("edit-total").value)||0,t=parseInt(document.getElementById("edit-discount").value)||0,o=parseInt(document.getElementById("edit-deposit").value)||0,i=n-t-o,c=document.getElementById("edit-remaining");c&&(c.textContent=i>0?i.toLocaleString("vi-VN")+"đ":"0đ")};const ot=document.getElementById("edit-total"),at=document.getElementById("edit-deposit");ot&&ot.addEventListener("input",window.updateEditRemaining),at&&at.addEventListener("input",window.updateEditRemaining),document.querySelectorAll('#bookingModal button[onclick*="hidden"]').forEach(n=>{n.removeAttribute("onclick"),n.addEventListener("click",window.closeModal)});const st=async n=>{try{const o=await(await fetch("/api/admin_customers?action=create",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({full_name:n.name,phone:n.phone,cccd:n.id_card||"",dob:n.dob||null,gender:n.gender||"Khác",medical_notes:n.allergy||"",dietary:n.diet||""})})).json();o.success&&o.csr_code&&(console.log("✅ Đã đồng bộ CRM thành công:",n.name,o.csr_code),(await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:n.id,customer_id:o.csr_code})})).ok&&(console.log("✅ Đã cập nhật Booking mã CRM"),typeof S=="function"&&S()))}catch(t){console.error("❌ Lỗi đồng bộ CRM:",t)}},dt=document.getElementById("bookingsTableBody");dt&&dt.addEventListener("click",async n=>{const t=n.target.closest(".action-btn");if(!t){const c=n.target.closest(".row-clickable");if(c){const s=c.getAttribute("data-id"),a=M.find(r=>r.id==s);a&&openRowActionModal(a)}return}const o=t.getAttribute("data-id"),l=M.find(c=>c.id==o);if(!l)return;const i=document.getElementById("rowActionModal");if(i&&i.classList.add("hidden"),t.classList.contains("confirm-deposit-btn")){if(confirm("Xác nhận khách hàng này đã chuyển khoản đặt cọc?"))try{if(t.disabled=!0,t.classList.add("opacity-50"),(await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:o,status:"Đã cọc"})})).ok)alert("✅ Đã xác nhận cọc thành công!"),st(l),typeof S=="function"&&S();else throw new Error("Lỗi cập nhật API")}catch(c){alert("❌ Lỗi: "+c.message)}finally{t.disabled=!1,t.classList.remove("opacity-50")}return}if(t.classList.contains("delete-btn")){window.actionDelete(o);return}else if(t.classList.contains("process-btn")){const a=(window.location.hostname==="localhost"||window.location.hostname==="127.0.0.1"?"http://localhost:8888":window.location.origin)+`/booking/process.html?id=${o}`;navigator.clipboard.writeText(a).then(()=>{alert("📋 Đã sao chép Link Điền Thông Tin!\\nGửi cho khách: "+a)}).catch(r=>{alert("Lỗi khi Copy Clipboard. Link là: "+a)})}else if(t.classList.contains("pay-terms-btn")){const a=(window.location.hostname==="localhost"||window.location.hostname==="127.0.0.1"?"http://localhost:8888":window.location.origin)+`/pay-terms.html?id=${o}`;navigator.clipboard.writeText(a).then(()=>{alert("📋 Đã sao chép Link Cọc Tour!\\nGửi cho khách: "+a)}).catch(r=>{alert("Lỗi khi Copy Clipboard. Link là: "+a)})}else if(t.classList.contains("send-btn")){const a=(window.location.hostname==="localhost"||window.location.hostname==="127.0.0.1"?"http://localhost:8888":window.location.origin)+`/pay?id=${o}`;navigator.clipboard.writeText(a).then(()=>{alert("📋 Đã sao chép Link Thanh Toán Cọc! Gửi cho khách qua Zalo nhé:\\n"+a)}).catch(r=>{alert("Lỗi khi Copy Clipboard. Link là: "+a)})}else if(t.classList.contains("payment-btn")){const a=(window.location.hostname==="localhost"||window.location.hostname==="127.0.0.1"?"http://localhost:8888":window.location.origin)+`/pay?id=${o}`;window.open(a,"_blank")}else t.classList.contains("view-btn")?(document.getElementById("rowActionModal").classList.add("hidden"),window.actionView(o)):t.classList.contains("edit-btn")&&window.actionEdit(o)});const P=document.getElementById("smartSearch"),_=document.getElementById("searchSuggestions");if(P){let n;P.addEventListener("input",t=>{const o=t.target.value.toLowerCase().trim();if(clearTimeout(n),o.length<2){_&&_.classList.add("hidden");return}n=setTimeout(async()=>{try{const l=await fetch(`/api/admin_customers?action=search&keyword=${o}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({keyword:o})}),i=await l.json();if(l.ok&&i.success){const s=[i.data];_&&(_.innerHTML=s.map(a=>`
                        ...
                        `).join(""),_.classList.remove("hidden"))}else _&&(_.innerHTML='<div class="p-3 text-xs text-gray-400 italic text-center">Không tìm thấy khách hàng cũ</div>',_.classList.remove("hidden"))}catch(l){console.error("Search error:",l)}},300)}),_&&_.addEventListener("click",t=>{const o=t.target.closest(".suggestion-item");if(o){const l=document.getElementById("addFullName"),i=document.getElementById("addPhone"),c=document.getElementById("addCsrCode");l&&(l.value=o.getAttribute("data-name")),i&&(i.value=o.getAttribute("data-phone")),c&&(c.value=o.getAttribute("data-csr"));const s=document.getElementById("addDob"),a=document.getElementById("addGender"),r=document.getElementById("addIdCard"),h=document.getElementById("addAddress");s&&(s.value=o.getAttribute("data-dob")||""),a&&(a.value=o.getAttribute("data-gender")||"Khác"),r&&(r.value=o.getAttribute("data-cccd")||""),h&&(h.value=o.getAttribute("data-address")||"");const p=document.getElementById("addAllergy"),b=document.getElementById("addDiet"),d=document.getElementById("addTrekkingPole"),m=document.getElementById("addMedalName");p&&(p.value=o.getAttribute("data-allergy")||""),b&&(b.value=o.getAttribute("data-diet")||""),d&&(d.value=o.getAttribute("data-trekking_pole")||"Không"),m&&(m.value=o.getAttribute("data-medal-name")||""),window._selectedCustomer={fullName:o.getAttribute("data-name"),phone:o.getAttribute("data-phone"),csrCode:o.getAttribute("data-csr"),dob:o.getAttribute("data-dob")||"",gender:o.getAttribute("data-gender")||"",address:o.getAttribute("data-address")||"",id_card:o.getAttribute("data-cccd")||"",allergy:o.getAttribute("data-allergy")||"",diet:o.getAttribute("data-diet")||"",trekking_pole:o.getAttribute("data-trekking_pole")||"",medal_name:o.getAttribute("data-medal-name")||""},P.value=o.getAttribute("data-name"),_.classList.add("hidden");const x=`
                    ...
                    </div>`,k=document.getElementById("loyalty-alert");k&&k.remove(),P.parentElement.insertAdjacentHTML("afterend",x)}}),document.addEventListener("click",t=>{!P.contains(t.target)&&_&&!_.contains(t.target)&&_.classList.add("hidden")})}const G=document.getElementById("addTourPrice"),O=document.getElementById("addDiscount"),lt=document.getElementById("addTotalPriceDisplay");document.getElementById("addTourName");const it=()=>{if(!G||!O||!lt)return;const n=parseInt(G.value)||0,t=parseInt(O.value)||0,o=n-t;lt.textContent=(o>0?o:0).toLocaleString("vi-VN")+"đ"};G&&G.addEventListener("input",it),O&&O.addEventListener("input",it);const rt=document.getElementById("searchBtn");rt&&rt.addEventListener("click",n=>{n.preventDefault(),document.getElementById("smartSearch").value||alert("Vui lòng nhập Số Điện Thoại hoặc Mã #CSR")});const q=document.getElementById("bookingForm");q&&q.addEventListener("submit",async n=>{n.preventDefault();const t=n.target.querySelector('button[type="submit"]'),o=t.textContent;t.textContent="Đang lưu hệ thống...",t.disabled=!0;try{const l=document.getElementById("addFullName").value,i=document.getElementById("addPhone").value,c=document.getElementById("addTourName").value,s=document.getElementById("addTourDate").value;let a=null,r=s;if(s&&s.includes("::")){const I=s.split("::");a=parseInt(I[0])||null,r=I[1]}const h=document.getElementById("addDob").value,p=document.getElementById("addGender").value,b=document.getElementById("addIdCard").value,d=document.getElementById("addAddress").value,m=document.getElementById("addSpecial").value,x=document.getElementById("addAllergy").value,k=document.getElementById("addDiet").value,B=document.getElementById("addTrekkingPole").value,e=document.getElementById("addMedalName").value,g=parseInt(document.getElementById("addTourPrice").value)||0,u=parseInt(document.getElementById("addDiscount").value)||0,y=parseInt(document.getElementById("addDepositRequired").value),w=isNaN(y)?1e6:y,C=document.getElementById("addCsrCode").value||"",T=localStorage.getItem("csr_user");let L=null,E="Admin";if(T){const I=JSON.parse(T);L=I.id,E=I.fullName||I.full_name||"Admin"}const $=document.getElementById("editingBookingId")?document.getElementById("editingBookingId").value:"",v={name:l,phone:i,tour:c,date:r,status:"Chờ cọc",total_price:g-u,discount:u,deposit:0,sale_id:L,sale_name:E,customer_id:C,deposit_required:w,commitments:!0,schedule_id:a,dob:h,gender:p,address:d,id_card:b,special:m,allergy:x,diet:k,trekking_pole:B,medal_name:e},D=i.replace(/[^0-9]/g,"");window._selectedCustomer&&(window._selectedCustomer.phone||"").replace(/[^0-9]/g,"")===D&&(v.dob=v.dob||window._selectedCustomer.dob,v.gender=v.gender&&v.gender!=="Khác"?v.gender:window._selectedCustomer.gender||"Khác",v.address=v.address||window._selectedCustomer.address,v.id_card=v.id_card||window._selectedCustomer.id_card,v.allergy=window._selectedCustomer.allergy,v.diet=window._selectedCustomer.diet,v.trekking_pole=window._selectedCustomer.trekking_pole,v.medal_name=window._selectedCustomer.medal_name),$&&(v.id=$,delete v.status,delete v.deposit,delete v.sale_id,delete v.sale_name);const R=await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(v)});if(R.ok){alert($?"✅ Đã lưu Cập nhật Đơn hàng!":"✅ Thêm Đơn Hàng Thành Công!"),window.closeModal(),q&&q.reset(),window._selectedCustomer=null;const I=document.getElementById("loyalty-alert");I&&I.remove(),document.getElementById("addTotalPriceDisplay")&&(document.getElementById("addTotalPriceDisplay").textContent="0đ"),typeof S=="function"?S():window.location.reload()}else{const I=await R.json();throw new Error(I.error||"Lỗi tạo Booking!")}}catch(l){alert("❌ "+l.message)}finally{t.textContent=o,t.disabled=!1}});const ct=document.getElementById("editForm");ct&&ct.addEventListener("submit",async n=>{n.preventDefault();const t=n.target.querySelector('button[type="submit"]'),o=t.textContent;t.textContent="Đang lưu cập nhật...",t.disabled=!0;try{const l=document.getElementById("edit-id").value,i=document.getElementById("edit-name").value,c=document.getElementById("edit-phone").value,s=document.getElementById("edit-medal-name").value,a=document.getElementById("edit-tour").value,r=document.getElementById("edit-date").value;let h=r,p;if(r&&r.includes("::")){const I=r.split("::");p=parseInt(I[0])||null,h=I[1]}const b=document.getElementById("edit-dob").value,d=document.getElementById("edit-gender").value,m=document.getElementById("edit-status").value,x=document.getElementById("edit-allergy").value,k=document.getElementById("edit-address").value,B=document.getElementById("edit-diet").value,e=document.getElementById("edit-trekking-pole").value,g=document.getElementById("edit-commitments").checked,u=document.getElementById("edit-special").value,y=document.getElementById("edit-sale");let w,C;if(y&&y.parentElement&&!y.parentElement.classList.contains("hidden")&&(w=y.value||null,C="Website",w)){const I=z.find(gt=>String(gt.id)===String(w));I&&(C=I.full_name||I.fullName)}const T=parseInt(document.getElementById("edit-total").value)||0,L=parseInt(document.getElementById("edit-discount").value)||0,E=parseInt(document.getElementById("edit-deposit").value)||0,$=parseInt(document.getElementById("edit-deposit-required").value),v=isNaN($)?1e6:$,D={id:l,name:i,phone:c,medal_name:s,tour:a,date:h,dob:b,gender:d,status:m,allergy:x,address:k,diet:B,trekking_pole:e,commitments:g,special:u,sale_id:w,sale_name:C,total_price:T-L,discount:L,deposit:E,deposit_required:v,...p!==void 0?{schedule_id:p}:{}},R=await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(D)});if(R.ok)alert("✅ Đã cập nhật thành công Chi tiết Đơn hàng!"),(m.includes("Đã cọc")||D.total_price>0&&D.total_price===E)&&st(D),window.closeEditModal(),typeof S=="function"&&S();else{const I=await R.json();throw new Error(I.error||"Gặp sự cố khi Cập nhật API.")}}catch(l){alert("❌ Lỗi Cập nhật: "+l.message)}finally{t.textContent=o,t.disabled=!1}})};export{vt as afterRender,yt as render};
