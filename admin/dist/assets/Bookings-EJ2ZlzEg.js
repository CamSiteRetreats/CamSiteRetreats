import { S as rt, H as ct } from "./Header-CvqOcIss.js"; const pt = () => `
    ...
    `, mt = () => {
        let B = [], x = "consult", V = [], z = []; const it = async () => { try { const [n, t] = await Promise.all([fetch("/api/tours"), fetch("/api/schedules")]); V = n.ok ? await n.json() : [], z = t.ok ? await t.json() : [], F("addTourName"), F("edit-tour") } catch (n) { console.error("Lỗi tải tours/schedules:", n) } }, F = n => { const t = document.getElementById(n); if (!t) return; const o = t.value, i = t.options[0].outerHTML; t.innerHTML = i, V.filter(r => r.is_visible !== !1).forEach(r => { const a = document.createElement("option"); a.value = r.name, a.textContent = r.name, r.name === o && (a.selected = !0), t.appendChild(a) }) }, j = (n, t, o) => { const i = document.getElementById(n); if (!i) return; if (i.innerHTML = '<option value="">-- Chọn Lịch --</option>', !t) { i.innerHTML = '<option value="">-- Chọn Tour trước --</option>'; return } const r = z.filter(a => a.tour_name === t || t.includes(a.tour_name) || a.tour_name.includes(t)); if (r.length === 0) { i.innerHTML = '<option value="">Chưa có lịch cho tour này</option>'; return } r.sort((a, l) => new Date(a.start_date) - new Date(l.start_date)).forEach(a => { const l = parseInt(a.booked_count) || 0, s = (a.slots || 0) - l, u = new Date(a.start_date), f = a.end_date ? new Date(a.end_date) : null, g = p => `${p.getDate().toString().padStart(2, "0")}/${(p.getMonth() + 1).toString().padStart(2, "0")}`; let h = g(u); f && (h += " - " + g(f)), h += ` (${s >= 0 ? s : 0} chỗ trống)`; const d = document.createElement("option"); d.value = `${u.getDate().toString().padStart(2, "0")}/${(u.getMonth() + 1).toString().padStart(2, "0")}/${u.getFullYear()}`, d.textContent = h, s <= 0 && (d.disabled = !0, d.textContent = h.replace("chỗ trống", "HẾT CHỖ")), o && d.value === o && (d.selected = !0), i.appendChild(d) }) }, U = document.getElementById("addTourName"); U && U.addEventListener("change", n => { j("addTourDate", n.target.value) }); const J = document.getElementById("edit-tour"); J && J.addEventListener("change", n => { j("edit-date", n.target.value) }), it(); const $ = () => {
            ...
            `; const i = document.getElementById("filterSearch") ? document.getElementById("filterSearch").value.toLowerCase() : "", r = document.getElementById("filterTour") ? document.getElementById("filterTour").value : "", a = document.getElementById("filterDate") ? document.getElementById("filterDate").value : "", l = document.getElementById("filterSale") ? document.getElementById("filterSale").value : "", s = document.getElementById("filterStatus") ? document.getElementById("filterStatus").value : ""; let u = B.filter(e => { let b = !1; const c = parseInt(e.total_price) > 0 && parseInt(e.total_price) === parseInt(e.deposit); return x === "consult" ? b = e.status === "Chờ tư vấn" : x === "pending" ? b = !e.status || e.status === "Chờ cọc" : x === "upcoming" ? b = (e.status === "Chờ xác nhận cọc" || e.status === "Đã cọc" || e.status === "Đã cọc (Chờ đi)") && !c : x === "completed" && (b = c || e.status === "Hoàn thành" || e.status === "Đã đi"), !(!b || r && e.tour !== r || a && e.date !== a || l && String(e.sale_id) !== String(l) || i && !`${e.name || ""} ${e.phone || ""} ${e.customer_id || ""}`.toLowerCase().includes(i) || s && e.status !== s) }); const f = document.getElementById("statTitle1"), g = document.getElementById("statTitle2"), h = document.getElementById("statTitle3"), d = document.getElementById("statTotalCustomers"), p = document.getElementById("statTotalRevenue"), y = document.getElementById("statTotalCollected"); let I = u.length; if (x === "consult") { f && (f.textContent = "Tổng Leads Tư Vấn"), g && (g.textContent = "Chưa Có Sale Nhận"), h && (h.textContent = "Đã Có Sale Nhận"); let e = 0, b = 0; u.forEach(c => { !c.sale_id || c.sale_id === "null" || !c.sale_name || c.sale_name === "Website" || c.sale_name === "null" ? e++ : b++ }), d && (d.textContent = I), p && (p.textContent = e + " Đơn"), y && (y.textContent = b + " Đơn") } else if (x === "pending") { f && (f.textContent = "Tổng Khách (Chờ Cọc)"), g && (g.textContent = "Khách Chưa Tư Vấn (Mới)"), h && (h.textContent = "Đã Có Sale Giữ Chỗ"); let e = 0, b = 0; u.forEach(c => { !c.sale_id || c.sale_id === "null" || !c.sale_name || c.sale_name === "Website" || c.sale_name === "null" ? e++ : b++ }), d && (d.textContent = I), p && (p.textContent = e + " Đơn"), y && (y.textContent = b + " Đơn") } else if (x === "upcoming") { f && (f.textContent = "Tổng Số Khách"), g && (g.textContent = "Chưa Xác Nhận Cọc"), h && (h.textContent = "Chờ Thanh Toán Còn Lại"); let e = 0, b = 0; u.forEach(c => { c.status === "Chờ xác nhận cọc" ? e++ : parseInt(c.deposit) > 0 && parseInt(c.total_price) > parseInt(c.deposit) && b++ }), d && (d.textContent = I), p && (p.textContent = e + " Khách"), y && (y.textContent = b + " Khách") } else { f && (f.textContent = "Khách Sẵn Sàng (Full)"), g && (g.textContent = "Tổng Doanh Thu Tab"), h && (h.textContent = "Thực Thu (Full Tận Nơi)"); let e = 0, b = 0; u.forEach(c => { e += parseInt(c.total_price) || 0, b += parseInt(c.deposit) || 0 }), d && (d.textContent = I), p && (p.textContent = e > 0 ? e.toLocaleString("vi-VN") + "đ" : "0đ"), y && (y.textContent = b > 0 ? b.toLocaleString("vi-VN") + "đ" : "0đ") } const A = o ? 8 : 4; if (!u || u.length === 0) { let e = x === "consult" ? "Chưa có khách hàng tư vấn nào." : x === "pending" ? "Chưa có Khách chờ cọc." : x === "upcoming" ? "Chưa có Khách nào Đã Cọc." : "Danh sách rỗng."; t.innerHTML = `<tr><td colspan="${A}" class="p-8 text-center text-gray-500">${e}</td></tr>`; return } t.innerHTML = u.map(e => {
                const b = x === "upcoming" ? "bg-green-50/20 hover:bg-green-50 transition-colors" : "hover:bg-gray-100 transition-colors"; if (o) {
                    const c = parseInt(e.total_price) || 0, v = parseInt(e.deposit) || 0, k = c - v, C = c > 0 ? c.toLocaleString("vi-VN") + "đ" : "Chưa định giá", w = v > 0 ? v.toLocaleString("vi-VN") + "đ" : "0đ", m = k > 0 ? k.toLocaleString("vi-VN") + "đ" : k === 0 && c > 0 ? "Đã thu trọn" : "-"; let L = ""; e.status === "Hoàn thành" || e.status === "Đã đi" ? L = '<span class="bg-gray-100 text-gray-600 border border-gray-200 px-2 py-0.5 rounded text-xs block w-full text-center">Hoàn thành</span>' : e.status === "Chờ xác nhận cọc" ? L = `<button class="action-btn confirm-deposit-btn bg-csr-orange text-white px-3 py-1.5 rounded text-xs font-bold shadow-sm hover:bg-[#d65503] w-full" data-id="${e.id}">Xác nhận cọc</button>` : v > 0 && k === 0 ? L = '<span class="bg-green-100 text-green-700 border border-green-200 px-2 py-0.5 rounded text-xs font-medium block w-full text-center">Hoàn tất phí</span>' : L = '<span class="bg-blue-100 text-blue-700 border border-blue-200 px-2 py-0.5 rounded text-xs font-medium block w-full text-center">Đã Cọc</span>'; let K = ""; return v > 0 && (K = `<a href="/invoice.html?id=${e.id}" target="_blank" class="mt-1.5 block text-center text-[11px] text-blue-600 hover:text-blue-800 hover:underline font-medium" onclick="event.stopPropagation()">🧾 Xem hóa đơn</a>`), `
                ...
                `} else {
                    const c = e.name ? e.name.substring(0, 2).toUpperCase() : "KH", v = x === "consult", k = !e.sale_id || e.sale_id === "null" || !e.sale_name || e.sale_name === "null"; let C = ""; v && k ? C = `<button class="action-btn claim-btn bg-csr-orange hover:bg-[#d65503] text-white px-3 py-1.5 rounded text-xs font-bold shadow-sm transition-colors" data-id="${e.id}">Nhận khách</button>` : C = `<span class="text-sm text-gray-500">${e.sale_name || "Website"}</span>`; const w = e.customer_id ? `<button class="action-btn payment-btn bg-blue-600 hover:bg-blue-700 text-white px-3 py-1.5 rounded text-xs font-bold shadow-sm transition-colors" data-id="${e.id}">💳 Thanh toán</button>` : `<button class="action-btn pay-terms-btn bg-blue-500 hover:bg-blue-600 text-white px-3 py-1.5 rounded text-xs font-bold shadow-sm transition-colors" data-id="${e.id}">🔗 Gửi Link Cọc</button>`; return `
                <tr class="${b} cursor-pointer row-clickable" data-id="${e.id}">
                    <td class="p-4 align-top">
...
`), u = new Blob([s], { type: "text/csv;charset=utf-8;" }), f = URL.createObjectURL(u), g = document.createElement("a"); g.setAttribute("href", f); let h = x === "pending" ? "ChoCoc" : x === "upcoming" ? "SapThamGia" : "HoanThanh"; g.setAttribute("download", `BaoCao_DonHang_${h}_${new Date().toISOString().slice(0, 10)}.csv`), document.body.appendChild(g), g.click(), document.body.removeChild(g)
        ...
        `; document.getElementById("ramActions").innerHTML = t, document.getElementById("rowActionModal").classList.remove("hidden")
        }, window.addEventListener("click", n => { const t = document.getElementById("rowActionModal"); t && n.target === t && t.classList.add("hidden") }), window.actionView = n => {
            document.getElementById("rowActionModal").classList.add("hidden"); const t = B.find(a => a.id == n); if (!t) return; const o = parseInt(t.deposit) > 0 ? `<span class="text-green-600 font-bold">${parseInt(t.deposit).toLocaleString("vi-VN")}đ</span>` : '<span class="text-yellow-600 font-bold">0đ (Chưa Cọc)</span>', i = t.total_price ? `${parseInt(t.total_price).toLocaleString("vi-VN")}đ` : "Chưa định giá", r = `
            ...
            let n; D.addEventListener("input", t => {
                const o = t.target.value.toLowerCase().trim(); if (clearTimeout(n), o.length < 2) { T && T.classList.add("hidden"); return } n = setTimeout(async () => {
                    try {
                        ...
                        `).join(""), T.classList.remove("hidden"))
                        } else T && (T.innerHTML = '<div class="p-3 text-xs text-gray-400 italic text-center">Không tìm thấy khách hàng cũ</div>', T.classList.remove("hidden"))
                    } catch (i) { console.error("Search error:", i) }
                }, 300)
            }), T && T.addEventListener("click", t => {
                const o = t.target.closest(".suggestion-item"); if (o) {
                    const i = document.getElementById("addFullName"), r = document.getElementById("addPhone"), a = document.getElementById("addCsrCode"); i && (i.value = o.getAttribute("data-name")), r && (r.value = o.getAttribute("data-phone")), a && (a.value = o.getAttribute("data-csr")); const l = document.getElementById("addDob"), s = document.getElementById("addGender"), u = document.getElementById("addIdCard"), f = document.getElementById("addAddress"); l && (l.value = o.getAttribute("data-dob") || ""), s && (s.value = o.getAttribute("data-gender") || "Khác"), u && (u.value = o.getAttribute("data-cccd") || ""), f && (f.value = o.getAttribute("data-address") || ""); const g = document.getElementById("addAllergy"), h = document.getElementById("addDiet"), d = document.getElementById("addTrekkingPole"), p = document.getElementById("addMedalName"); g && (g.value = o.getAttribute("data-allergy") || ""), h && (h.value = o.getAttribute("data-diet") || ""), d && (d.value = o.getAttribute("data-trekking_pole") || "Không"), p && (p.value = o.getAttribute("data-medal-name") || ""), window._selectedCustomer = { fullName: o.getAttribute("data-name"), phone: o.getAttribute("data-phone"), csrCode: o.getAttribute("data-csr"), dob: o.getAttribute("data-dob") || "", gender: o.getAttribute("data-gender") || "", address: o.getAttribute("data-address") || "", id_card: o.getAttribute("data-cccd") || "", allergy: o.getAttribute("data-allergy") || "", diet: o.getAttribute("data-diet") || "", trekking_pole: o.getAttribute("data-trekking_pole") || "", medal_name: o.getAttribute("data-medal-name") || "" }, D.value = o.getAttribute("data-name"), T.classList.add("hidden"); const y = `
