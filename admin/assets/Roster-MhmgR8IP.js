import{S as R,H as _}from"./Header-CvqOcIss.js";const z=()=>`
    ...
    `,V=async()=>{const p=[{id:"col-stt",label:"STT",always:!0,render:(e,t)=>t+1},{id:"col-name",label:"Họ và Tên",default:!0,render:e=>`<div class="font-bold text-gray-900">${e.name}</div>`},{id:"col-dob",label:"Ngày Sinh",default:!0,render:e=>{if(!e.dob)return"-";const t=e.dob.split("-");return t.length===3?`${t[2]}/${t[1]}/${t[0]}`:e.dob}},{id:"col-phone",label:"SĐT",default:!0,render:e=>`<div class="font-medium text-gray-700">${e.phone||"-"}</div>`},{id:"col-gender",label:"Giới tính",default:!0,render:e=>e.gender||"-"},{id:"col-cccd",label:"CCCD / Passport",default:!0,render:e=>e.id_card||e.cccd||"-"},{id:"col-address",label:"Địa chỉ",default:!0,render:e=>`<div class="max-w-[200px] truncate print:max-w-none print:whitespace-normal" title="${e.address||""}">${e.address||"-"}</div>`},{id:"col-diet",label:"Ăn chay",default:!1,render:e=>e.diet==="Chay"||e.diet==="Có"?'<span class="text-green-600 font-bold">Có</span>':"Không"},{id:"col-pole",label:"Gậy Trekking",default:!1,render:e=>e.trekking_pole==="Có"?'<span class="text-orange-600 font-bold">Có</span>':"Không"},{id:"col-allergy",label:"Dị ứng",default:!1,render:e=>e.medical_notes||e.allergy||"-"},{id:"col-special",label:"Yêu cầu ĐB",default:!1,render:e=>e.special_notes||e.special||"-"},{id:"col-medal",label:"Tên Huy Chương",default:!1,render:e=>e.medal_name||e.name||"-"}],E=new URLSearchParams(window.location.search),g=E.get("tour")||"",f=E.get("date")||"";let i=f;const T=e=>{if(!e)return"";const t=e.split(/[-/.]/);if(t.length!==3)return e;let n,a,o;return t[0].length===4?[o,a,n]=t:[n,a,o]=t,`${n.toString().padStart(2,"0")}/${a.toString().padStart(2,"0")}/${o}`},S=e=>(e||"").toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g,"").replace(/[-_]/g," ").replace(/\s+/g," ").trim();f&&(i=T(f));const H=document.getElementById("roster-title"),$=document.getElementById("roster-subtitle"),B=document.getElementById("print-tour-name"),D=document.getElementById("print-tour-date"),j=document.getElementById("print-total"),v=document.getElementById("filterStatus"),b=document.getElementById("filterGender"),y=document.getElementById("rosterList");let m=[],u=new Set(p.filter(e=>e.default||e.always).map(e=>e.id));g&&(H.textContent=g,B.textContent=g),i&&($.innerHTML=`Ngày khởi hành: <span class="font-bold text-csr-orange">${i}</span>`,D.textContent=i);const M=()=>{const e=document.getElementById("column-toggles");e.innerHTML=p.filter(t=>!t.always).map(t=>`
        ...
        `).join(""),document.querySelectorAll(".col-toggle-cb").forEach(t=>{t.addEventListener("change",n=>{n.target.checked?u.add(n.target.value):u.delete(n.target.value),N()})})},I=()=>{const e=document.getElementById("table-header-row");e.innerHTML=p.map(t=>`
            <th class="p-4 whitespace-nowrap ${t.id}" data-col="${t.id}">${t.label}</th>
        `).join("")},N=()=>{p.forEach(e=>{const t=u.has(e.id),n=document.querySelector(`th[data-col="${e.id}"]`);n&&(t?n.classList.remove("col-hidden"):n.classList.add("col-hidden")),document.querySelectorAll(`td[data-col="${e.id}"]`).forEach(a=>{t?a.classList.remove("col-hidden"):a.classList.add("col-hidden")})})},A=async()=>{try{m=(await(await fetch("/api/bookings")).json()).filter(n=>{let a=!0,o=!0;if(g){const s=S(n.tour),d=S(g);a=s===d||s.includes(d)||d.includes(s)}return i&&(o=T(n.date)===i),a&&o}),m.sort((n,a)=>{const o=(n.status||"").includes("cọc")||n.status==="Hoàn tất"?1:0;return((a.status||"").includes("cọc")||a.status==="Hoàn tất"?1:0)-o}),w()}catch(e){console.error("Lỗi khi tải dữ liệu đoàn:",e),y.innerHTML='<tr><td colspan="12" class="p-8 text-center text-red-500 font-bold">Lỗi khi tải dữ liệu. Vui lòng tải lại trang.</td></tr>'}},w=()=>{const e=v.value.toLowerCase(),t=b.value.toLowerCase(),n=m.filter(a=>{const o=(a.status||"").toLowerCase(),s=e===""||e==="chờ cọc"&&(o.includes("chờ cọc")||o.includes("chờ xác nhận"))||e==="đã cọc"&&(o.includes("đã cọc")||o.includes("hoàn tất")||o.includes("hoàn thành"))||e==="đã hủy"&&o.includes("hủy"),d=(a.gender||"").toLowerCase();return s&&(t===""||d===t)});if(document.getElementById("text-visible-count").textContent=n.length,j.textContent=n.length,n.length===0){y.innerHTML='<tr><td colspan="12" class="p-12 text-center text-gray-500 bg-gray-50">Không tìm thấy khách hàng nào khớp với điều kiện lọc.</td></tr>';return}y.innerHTML=n.map((a,o)=>'<tr class="hover:bg-gray-50/50 transition-colors">'+p.map(s=>`
                    <td class="p-4 align-top ${u.has(s.id)?"":"col-hidden"}" data-col="${s.id}">
                        ${s.render(a,o)}
                    </td>
                `).join("")+"</tr>").join("")},F=()=>{const e=p.filter(r=>u.has(r.id));let t="\\uFEFF";t+=e.map(r=>`"${r.label}"`).join(",")+`
`;const n=v.value.toLowerCase(),a=b.value.toLowerCase();m.filter(r=>{const x=n===""||n==="chờ cọc"&&(r.status==="Chờ cọc"||r.status==="Chờ xác nhận cọc")||n==="đã cọc"&&(r.status==="Đã cọc"||r.status==="Hoàn tất"||r.status==="Hoàn thành")||n==="đã hủy"&&r.status==="Đã hủy",k=a===""||r.gender&&r.gender.toLowerCase()===a;return x&&k}).forEach((r,x)=>{const k=e.map(c=>{let l="";if(c.id==="col-stt")l=x+1;else if(c.id==="col-name")l=r.name||"";else if(c.id==="col-address")l=r.address||"";else if(c.id==="col-phone")l=r.phone?`'${r.phone}`:"";else if(c.id==="col-diet")l=r.diet==="Chay"||r.diet==="Có"?"Có":"Không";else if(c.id==="col-pole")l=r.trekking_pole==="Có"?"Có":"Không";else{const P=String(c.render(r,x)),C=document.createElement("DIV");C.innerHTML=P,l=C.textContent||C.innerText||""}return l=String(l).replace(/"/g,'""'),`"${l}"`});t+=k.join(",")+`
`});const s=new Blob([t],{type:"text/csv;charset=utf-8;"}),d=URL.createObjectURL(s),L=`DS_Doan_${g||"Tour"}_${i||"Date"}.csv`.replace(/[/\\?%*:|"<>]/g,"-"),h=document.createElement("a");h.setAttribute("href",d),h.setAttribute("download",L),document.body.appendChild(h),h.click(),document.body.removeChild(h)};v.addEventListener("change",w),b.addEventListener("change",w),document.getElementById("btnExportPDF").addEventListener("click",()=>{window.print()}),document.getElementById("btnExportExcel").addEventListener("click",()=>{F()}),M(),I(),A()};export{V as afterRender,z as render};
