import{S as je,H as Ne}from"./Header-CtRUqTZE.js";const Ae=()=>`
    ...
    `,Re=async()=>{console.log("[Roster] VERSION 3.2 LOADED - READY TO EDIT");const pe=t=>{const e=(t||"").toLowerCase();return e.includes("hoàn tất")||e.includes("hoàn thành")||e.includes("đã thanh toán")||e.includes("thanh toán")?`<span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold bg-emerald-100 text-emerald-700 border border-emerald-200">
                        ...
                        ⏸️ Bảo lưu</span>`:`<span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold bg-amber-100 text-amber-700 border border-amber-200">${t||"Chờ cọc"}</span>`},ue=t=>{let e=[];const r=t.services_booked;if(r&&r!=="[]"&&r!=="null"&&r!==""){if(Array.isArray(r))e=r.map(n=>{const s=n.label||n.name||n.title||"",a=n.price&&Number(n.price)>0?` (+${Number(n.price).toLocaleString("vi-VN")}đ)`:"";return(s+a).trim()}).filter(Boolean);else if(typeof r=="string"&&r.trim().startsWith("["))try{const n=JSON.parse(r);Array.isArray(n)&&n.length>0&&(e=n.map(s=>{const a=s.label||s.name||s.title||"",p=s.price&&Number(s.price)>0?` (+${Number(s.price).toLocaleString("vi-VN")}đ)`:"";return(a+p).trim()}).filter(Boolean))}catch{const s=r.match(/label:\s*['"]([^'"]+)['"]/g)||[],a=r.match(/price:\s*(\d+)/g)||[];s.length>0&&(e=s.map((p,i)=>{const d=p.replace(/label:\s*['"]/,"").replace(/['"]$/,"").trim(),l=a[i]||"",m=l?Number(l.replace(/price:\s*/,"")):0;return(d+(m>0?` (+${m.toLocaleString("vi-VN")}đ)`:"")).trim()}).filter(Boolean))}}if(e.length===0){const n=t.extra_services||t.extraServices||t.extra||t.notes_extra||"";n&&n!=="-"&&n.trim()!==""&&(e=n.split(/[\n,;]+/).map(s=>s.trim()).filter(Boolean))}if(e.length===0)return'<span class="text-gray-300 text-xs italic">Không có</span>';const o=["bg-violet-100 text-violet-700 border-violet-200","bg-fuchsia-100 text-fuchsia-700 border-fuchsia-200","bg-sky-100 text-sky-700 border-sky-200","bg-teal-100 text-teal-700 border-teal-200","bg-orange-100 text-orange-700 border-orange-200","bg-rose-100 text-rose-700 border-rose-200"];return`<div class="flex flex-wrap gap-1.5">${e.map((n,s)=>`<span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold border ${o[s%o.length]}">
        ...
        `).join("")},ke=()=>{L.forEach(t=>{const e=_.has(t.id),r=document.querySelector(`th[data-col="${t.id}"]`);r&&(e?r.classList.remove("col-hidden"):r.classList.add("col-hidden")),document.querySelectorAll(`td[data-col="${t.id}"]`).forEach(o=>{e?o.classList.remove("col-hidden"):o.classList.add("col-hidden")})})},K=t=>{if(!t)return null;t.includes(" - ")&&(t=t.split(" - ")[0].trim()),t.includes("T")&&(t=t.split("T")[0]);const e=t.split(/[-/.]/);return e.length===2?{day:parseInt(e[0]),month:parseInt(e[1]),year:null}:e.length===3?e[0].length===4?{day:parseInt(e[2]),month:parseInt(e[1]),year:parseInt(e[0])}:{day:parseInt(e[0]),month:parseInt(e[1]),year:parseInt(e[2])}:null},M=K($),Ee=t=>{if(!M)return!0;const e=K(t);return!(!e||e.day!==M.day||e.month!==M.month||e.year&&M.year&&e.year!==M.year)},z=async()=>{try{if(A)try{const n=await(await fetch(`/api/guides?schedule_id=${A}`)).json();if(n&&n.length>0){const s=document.getElementById("rosterGuidesSection"),a=document.getElementById("rosterGuidesList"),p={"Hướng Dẫn Viên":"bg-blue-100 text-blue-700 border-blue-200",Leader:"bg-purple-100 text-purple-700 border-purple-200",Photo:"bg-pink-100 text-pink-700 border-pink-200",Guider:"bg-cyan-100 text-cyan-700 border-cyan-200","Hậu Cần":"bg-amber-100 text-amber-700 border-amber-200","Học Việc":"bg-gray-100 text-gray-600 border-gray-200"};s.classList.remove("hidden"),a.innerHTML=n.map(i=>`
                                <div class="flex items-center gap-2 bg-white px-3 py-2 rounded-xl border border-gray-200 shadow-sm print:shadow-none print:border-gray-400">
                                    <span class="text-[10px] font-bold px-2 py-0.5 rounded-full border ${p[i.role]||"bg-gray-100 text-gray-600 border-gray-200"} uppercase tracking-wide print:border-black print:text-black">${i.role}</span>
                            ...
                            `).join("")}}catch(o){console.error("Error loading guides:",o)}const e=await(await fetch("/api/bookings")).json();console.log("[Roster] targetTour:",I,"targetDateStr:",$,"targetParsed:",M),console.log("[Roster] Total bookings:",e.length),e.length>0&&e.filter(n=>n.tour&&n.tour.includes("Liêng")).slice(0,3).forEach(n=>console.log("[Roster] Sample Liêng:",{tour:n.tour,date:n.date,parsed:K(n.date)})),k=e.filter(o=>{if(A){if(String(o.schedule_id)===String(A))return!0;if(!(!ge&&!o.schedule_id))return!1}let n=!0,s=!0;if(I){const a=Z(o.tour),p=Z(I);n=a===p||a.includes(p)||p.includes(a)}return $&&(s=Ee(o.date)),n&&s}),console.log("[Roster] Matched bookings:",k.length);const r=document.getElementById("filterSort")?.value||"newest";r==="newest"?k.sort((o,n)=>parseInt(n.id)-parseInt(o.id)):k.sort((o,n)=>parseInt(o.id)-parseInt(n.id)),console.log(`[Roster] Sorted bookings (${r}):`,k.map(o=>o.id)),H()}catch(t){console.error("Lỗi khi tải dữ liệu đoàn:",t),P.innerHTML='<tr><td colspan="12" class="p-8 text-center text-red-500 font-bold">Lỗi khi tải dữ liệu. Vui lòng tải lại trang.</td></tr>'}},H=()=>{const t=q.value.toLowerCase(),e=V.value.toLowerCase(),r=document.getElementById("filterSearch")?.value?.toLowerCase()||"",o=k.filter(s=>{if(r&&!`${s.name} ${s.phone} ${s.address} ${s.medal_name} ${s.customer_id}`.toLowerCase().includes(r))return!1;const a=(s.status||"").toLowerCase();if(!(t===""||t==="chờ cọc"&&(a.includes("chờ cọc")||a.includes("chờ xác nhận"))||t==="đã cọc"&&(a.includes("đã cọc")||a.includes("hoàn tất")||a.includes("hoàn thành"))||t==="đã hủy"&&a.includes("hủy")))return!1;const i=(s.gender||"").toLowerCase();return e===""||i===e}),n=o.filter(s=>s.status!=="Đã hủy"&&s.status!=="Bảo lưu");if(document.getElementById("text-visible-count").textContent=n.length,ve.textContent=n.length,o.length===0){P.innerHTML='<tr><td colspan="12" class="p-12 text-center text-gray-500 bg-gray-50">Không tìm thấy khách hàng nào khớp với điều kiện lọc.</td></tr>';return}P.innerHTML=o.map((s,a)=>{const p=a%2===0,d=s.status==="Đã hủy"||s.status==="Bảo lưu"?"bg-gray-100 opacity-60":p?"bg-white":"bg-gray-50/40";return`<tr onclick="if(!event.target.closest('.seat-badge') && !event.target.closest('a')) { if(window.actionEdit) window.actionEdit('${s.id}'); else alert('Chưa tải xong logic chỉnh sửa!'); }" class="${d} hover:bg-amber-50/40 transition-colors border-b border-gray-100 group row-clickable cursor-pointer" data-booking-id="${s.id}">`+L.map(l=>`
                    <td class="px-4 py-3 align-middle ${l.id==="col-extra"?"min-w-[220px]":""} ${_.has(l.id)?"":"col-hidden"}" data-col="${l.id}">
                        ${l.render(s,a)}
                    </td>
                `).join("")+"</tr>"}).join("")},Se=()=>{const t=L.filter(l=>_.has(l.id)),e=q.value.toLowerCase(),r=V.value.toLowerCase(),o=document.getElementById("filterSearch")?.value?.toLowerCase()||"",n=k.filter(l=>{if(o&&!`${l.name} ${l.phone} ${l.address} ${l.medal_name} ${l.customer_id}`.toLowerCase().includes(o))return!1;const m=(l.status||"").toLowerCase();if(!(e===""||e==="chờ cọc"&&(m.includes("chờ cọc")||m.includes("chờ xác nhận"))||e==="đã cọc"&&(m.includes("đã cọc")||m.includes("hoàn tất")||m.includes("hoàn thành"))||e==="đã hủy"&&m.includes("hủy")))return!1;const h=(l.gender||"").toLowerCase();return r===""||h===r});let s=`
            ...
            <body>
                <table>
                    ...
                    <tbody>
        `;n.forEach((l,m)=>{s+="<tr>",t.forEach(u=>{let h="";if(u.id==="col-stt")h=m+1;else if(u.id==="col-name")h=l.name||"";else if(u.id==="col-order-id")h=l.id||"";else if(u.id==="col-address")h=l.address||"";else if(u.id==="col-phone")h=l.phone?String(l.phone):"";else if(u.id==="col-diet")h=l.diet==="Ăn chay"||l.diet==="Chay"||l.diet==="Có"?"Có":"Không";else if(u.id==="col-pole")h=l.trekking_pole==="Có"?"Có":"Không";else if(u.id==="col-status")h=l.status||"";else if(u.id==="col-extra"){const x=l.services_booked;if(x&&x!=="[]"&&x!=="null"&&x!==""){const y=c=>c.map(b=>{const f=b.label||b.name||b.title||"",g=b.price&&Number(b.price)>0?` (+${Number(b.price).toLocaleString("vi-VN")}đ)`:"";return(f+g).trim()}).filter(Boolean).join(", ");if(Array.isArray(x))h=y(x);else if(typeof x=="string"&&x.trim().startsWith("["))try{h=y(JSON.parse(x))}catch{const b=x.match(/label:\s*['"]([^'"]+)['"]/g)||[],f=x.match(/price:\s*(\d+)/g)||[];b.length>0&&(h=b.map((g,E)=>{const S=g.replace(/label:\s*['"]/,"").replace(/['"]$/,"").trim(),v=f[E]?Number(f[E].replace(/price:\s*/,"")):0;return(S+(v>0?` (+${v.toLocaleString("vi-VN")}đ)`:"")).trim()}).filter(Boolean).join(", "))}}h||(h=l.extra_services||"")}else{const x=String(u.render(l,m)),y=document.createElement("DIV");y.innerHTML=x,h=y.textContent||y.innerText||""}const w=u.id==="col-phone"||u.id==="col-cccd";s+=`<td ${w?'class="num"':""}>${h}</td>`}),s+="</tr>"}),s+=`
                    </tbody>
                </table>
            </body>
            </html>
        `;const a=new Blob(["\uFEFF",s],{type:"application/vnd.ms-excel;charset=utf-8"}),p=URL.createObjectURL(a),i=`DS_Doan_${I||"Tour"}_${N||"Date"}.xls`.replace(/[/\\?%*:|"<>]/g,"-"),d=document.createElement("a");d.setAttribute("href",p),d.setAttribute("download",i),document.body.appendChild(d),d.click(),document.body.removeChild(d)},ee={solati_16:{label:"🚐 Solati 16 chỗ",totalSeats:15},limo_34:{label:"🚌 Limousine 34 chỗ",totalSeats:34},sleeper_45:{label:"🛏️ Giường Nằm 45 chỗ",totalSeats:46}},Ce=t=>{if(t==="limo_34")return{type:"limo",floors:[{label:null,rows:[[{n:"C1"},{n:"C2"},null,{n:"B1"},{n:"B2"},null,{n:"A1"},{n:"A2"},{n:"LÁI",driver:!0}],[{n:"C3"},{n:"C4"},null,{n:"B3"},{n:"B4"},null,{n:"A3"},{n:"A4"}],[{n:"C5"},{n:"C6"},null,{n:"B5"},{n:"B6"},null,{n:"A5"},{n:"A6"}],[{n:"C7"},{n:"C8"},null,{n:"B7"},{n:"B8"},null,{n:"A7"},{n:"A8"}],[{n:"C9"},{n:"C10"},null,{n:"B9"},{n:"B10"},null,{n:"A9"},{n:"A10"}],[{n:"C11"},{n:"C12"},null,null,null,null,{n:"A11"},{n:"A12"}]]}],legend:['<span class="font-bold text-gray-600">Dãy C</span> (cửa lên)','<span class="font-bold text-gray-600">Dãy B</span> (giữa)','<span class="font-bold text-gray-600">Dãy A</span> (tài xế)']};if(t==="sleeper_45"){const e=(r,o,n)=>{const s=Array.from({length:n-o+1},(p,i)=>`${r}${o+i}`),a=[];for(let p=0;p<s.length;p+=4){const i=[];for(let d=0;d<4;d++)d===2&&i.push(null),i.push(s[p+d]?{n:s[p+d]}:null);a.push(i)}return a};return{type:"sleeper",floors:[{label:"🏠 Tầng Dưới",floorKey:"down",rows:e("A",1,23)},{label:"🏥 Tầng Trên",floorKey:"up",rows:e("B",1,23)}],legend:null}}return{type:"solati",floors:[{label:null,rows:[[{n:"LÁI",driver:!0},null,{n:1},{n:2}],[{n:3},{n:4},null,{n:5}],[{n:6},{n:7},null,{n:8}],[{n:9},{n:10},null,{n:11}],[{n:12},{n:13},{n:14},{n:15}]]}],legend:null}},te=ee[Q]||ee.solati_16;let F=0;const R=()=>{const t=document.getElementById("seatMapGrid");if(!t)return;const e={};k.forEach(d=>{const l=d.status==="Đã hủy"||d.status==="Bảo lưu";d.seat_number&&!l&&(e[String(d.seat_number)]=d)});const r=Object.keys(e).length,o=te.totalSeats,n=document.getElementById("seat-map-count"),s=document.getElementById("seat-count-badge"),a=document.getElementById("vehicle-label");n&&(n.textContent=`${r}/${o}`),s&&(s.textContent=`💺 ${r}/${o} ghế đã xếp`),a&&(a.textContent=te.label);const p=Ce(Q),i=(d,l)=>{if(!d)return'<div class="w-12 h-10 flex-shrink-0"></div>';if(d.driver)return'<div class="seat-btn seat-driver w-10 h-9 rounded-lg flex flex-col items-center justify-center text-[8px] font-bold flex-shrink-0"><span>🚗</span><span>LÁI</span></div>';const m=String(d.n),u=l[m],h=u?(u.name||"").split(" ").pop().slice(0,4):"",x=/[A-Z]/.test(m)?"w-11 h-9":"w-12 h-10";return`<button
            ...
            </button>`};if(p.type==="sleeper")t.innerHTML=`
                ...
                <div class="floor-content">
                    ...
                    `).join("")}
                </div>`,t.querySelectorAll(".floor-tab").forEach(d=>{d.addEventListener("click",()=>{F=parseInt(d.dataset.floor),R()})});else{let d="";p.legend&&(d+=`<div class="flex justify-around text-[9px] text-gray-400 mb-2 px-1">${p.legend.map(l=>`<span>${l}</span>`).join("")}</div>`),p.floors[0].rows.forEach(l=>{const m=l.map(u=>i(u,e)).join("");d+=`<div class="flex items-center justify-center gap-1 mb-1">${m}</div>`}),t.innerHTML=d}t.querySelectorAll(".seat-btn[data-seat]").forEach(d=>{d.addEventListener("click",()=>ne(d.dataset.seat,d.dataset.bookingId))})},ne=(t,e)=>{const r=document.getElementById("seatModal"),o=document.getElementById("seatModalTitle"),n=document.getElementById("seatModalBody");if(!r||!o||!n)return;o.textContent=e?`Ghế ${t} — ${k.find(i=>String(i.id)===String(e))?.name||"?"}`:`Ghế ${t} — Chưa có khách`;const s=k.find(i=>String(i.id)===String(e));let a="";s&&(a+=`<div class="mb-3 p-3 bg-orange-50 border border-orange-200 rounded-xl text-sm">
            ...
            <div class="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Chọn khách khác:</div>`);const p=k.filter(i=>!i.seat_number||String(i.id)===String(e));p.length===0?a+='<p class="text-gray-400 text-sm text-center py-4">Tất cả khách đã được xếp ghế.</p>':a+=p.map(i=>`
                ...
                </button>`).join(""),n.innerHTML=a,r.style.display="flex",n.querySelector("#btnRemoveSeat")?.addEventListener("click",async()=>{await J(e,null),r.style.display="none"}),n.querySelectorAll(".btn-assign-passenger").forEach(i=>{i.addEventListener("click",async()=>{const d=i.dataset.bookingId,l=i.dataset.seat,m=k.find(u=>String(u.seat_number)===String(l)&&String(u.id)!==String(d));m&&await J(m.id,null),await J(d,l),r.style.display="none"})})},J=async(t,e)=>{try{await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:t,seat_number:e||null})});const r=k.find(o=>String(o.id)===String(t));r&&(r.seat_number=e||null),H(),R()}catch(r){console.error("Lỗi lưu ghế:",r),alert("Lỗi lưu số ghế. Vui lòng thử lại.")}};q.addEventListener("change",H),V.addEventListener("change",H),document.getElementById("filterSearch")?.addEventListener("input",H),document.getElementById("filterSort")?.addEventListener("change",z),document.getElementById("btnExportPDF").addEventListener("click",()=>window.print()),document.getElementById("btnExportExcel").addEventListener("click",()=>Se()),document.getElementById("seatModalClose")?.addEventListener("click",()=>{document.getElementById("seatModal").style.display="none"}),document.getElementById("seatModal")?.addEventListener("click",t=>{t.target===document.getElementById("seatModal")&&(document.getElementById("seatModal").style.display="none")}),document.getElementById("rosterList")?.addEventListener("mouseover",t=>{const e=t.target.closest("tr[data-booking-id]");if(!e)return;document.querySelectorAll(".seat-btn").forEach(n=>n.classList.remove("seat-highlighted"));const r=e.dataset.bookingId,o=k.find(n=>String(n.id)===String(r));o?.seat_number&&document.querySelector(`.seat-btn[data-seat="${o.seat_number}"]`)?.classList.add("seat-highlighted")}),document.getElementById("rosterList")?.addEventListener("click",t=>{const e=t.target.closest(".seat-badge");if(!e)return;const r=e.dataset.id,o=k.find(n=>String(n.id)===String(r));ne(o?.seat_number||"?",r)});let D=[],se=[],W=[];const Be=async()=>{try{const[t,e,r]=await Promise.all([fetch("/api/tours"),fetch("/api/schedules"),fetch("/api/users")]);D=t.ok?await t.json():[],se=e.ok?await e.json():[],W=r.ok?await r.json():[],Te("edit-tour"),Ie("edit-sale")}catch(t){console.error("Lỗi tải tours/schedules:",t)}},Ie=t=>{const e=document.getElementById(t);e&&(e.innerHTML='<option value="">-- Website --</option>',W.forEach(r=>{const o=document.createElement("option");o.value=r.id,o.textContent=`${r.full_name||r.fullName} (${r.role})`,e.appendChild(o)}))},Te=t=>{const e=document.getElementById(t);if(!e)return;const r=e.value,o=e.options[0]?.outerHTML||'<option value="">-- Chọn Tour --</option>';e.innerHTML=o,D.filter(n=>n.is_visible!==!1).forEach(n=>{const s=document.createElement("option");s.value=n.name,s.textContent=n.name,n.name===r&&(s.selected=!0),e.appendChild(s)})},re=(t,e,r,o)=>{const n=document.getElementById(t);if(!n||(n.innerHTML='<option value="">-- Chọn Lịch --</option>',!e))return;const s=se.filter(a=>a.tour_name?a.tour_name===e||e.includes(a.tour_name)||a.tour_name.includes(e):!1);if(s.length===0){n.innerHTML='<option value="">Chưa có lịch cho tour này</option>';return}s.sort((a,p)=>new Date(a.start_date)-new Date(p.start_date)).forEach(a=>{const p=parseInt(a.booked_count)||0,i=(a.slots||0)-p,d=new Date(a.start_date),l=a.end_date?new Date(a.end_date):null,m=y=>`${y.getDate().toString().padStart(2,"0")}/${(y.getMonth()+1).toString().padStart(2,"0")}`;let u=m(d);l&&(u+=" - "+m(l)),a.group_label&&(u+=` · ${a.group_label}`),u+=` (${i>=0?i:0} chỗ trống)`;const h=document.createElement("option"),w=`${d.getDate().toString().padStart(2,"0")}/${(d.getMonth()+1).toString().padStart(2,"0")}/${d.getFullYear()}`;h.value=`${a.id}::${w}`,h.dataset.scheduleId=a.id,h.dataset.date=w,h.textContent=u,i<=0&&(h.disabled=!0,h.textContent=u.replace("chỗ trống","HẾT CHỖ"));let x=r||"";if(x.includes("-")&&x.length>=10){const y=x.substring(0,10).split("-");y.length===3&&(x=`${y[2]}/${y[1]}/${y[0]}`)}(o&&String(a.id)===String(o)||!o&&x&&w===x)&&(h.selected=!0),n.appendChild(h)})},oe=document.getElementById("edit-tour");oe&&oe.addEventListener("change",t=>{re("edit-date",t.target.value)}),window.closeEditModal=()=>{document.getElementById("editModal").classList.remove("opacity-100");const t=document.getElementById("editModalContent");t&&(t.classList.remove("scale-100","translate-y-0"),t.classList.add("scale-95","translate-y-4")),setTimeout(()=>document.getElementById("editModal").classList.add("hidden"),300)},window.updateEditRemaining=()=>{const t=parseInt(document.getElementById("edit-total")?.value)||0,e=parseInt(document.getElementById("edit-discount")?.value)||0,r=parseInt(document.getElementById("edit-deposit")?.value)||0;let o=0;document.querySelectorAll(".preset-service-check:checked").forEach(i=>{o+=parseInt(i.dataset.price)||0}),document.querySelectorAll(".service-price-input").forEach(i=>{o+=parseInt(i.value)||0});const n=t-e+o,s=n-r,a=document.getElementById("edit-grand-total");a&&(a.textContent=n.toLocaleString("vi-VN")+"đ");const p=document.getElementById("edit-remaining");p&&(p.textContent=s>0?s.toLocaleString("vi-VN")+"đ":"0đ")},window.updateServicesTotal=()=>{let t=0;document.querySelectorAll(".preset-service-check:checked").forEach(o=>{t+=parseInt(o.dataset.price)||0}),document.querySelectorAll(".service-price-input").forEach(o=>{t+=parseInt(o.value)||0});const e=document.getElementById("servicesTotalDisplay");e&&(e.textContent=t.toLocaleString("vi-VN")+"đ");const r=document.getElementById("servicesTotalRow");return r&&r.classList.toggle("hidden",t===0),window.updateEditRemaining(),t},window.addServiceRow=(t="",e=0)=>{const r=document.getElementById("servicesContainer"),o=document.getElementById("emptyServicesMsg"),n=document.getElementById("servicesTotalRow");if(!r)return;o&&o.classList.add("hidden"),n&&n.classList.remove("hidden");const s=document.createElement("div");s.className="service-row flex items-center gap-2 bg-gray-50 border border-gray-100 rounded-xl px-3 py-2.5",s.innerHTML=`
        ...
        `,s.querySelector(".remove-service-btn").addEventListener("click",()=>{s.remove(),r.querySelectorAll(".service-row").length===0&&(o&&o.classList.remove("hidden"),n&&n.classList.add("hidden")),window.updateServicesTotal()}),s.querySelector(".service-price-input").addEventListener("input",window.updateServicesTotal),r.appendChild(s),window.updateServicesTotal()};const ae=document.getElementById("addServiceBtn");ae&&ae.addEventListener("click",()=>window.addServiceRow()),window.actionEdit=async t=>{try{const e=k.find(c=>c.id==t);if(!e){alert("Không tìm thấy thông tin khách hàng!");return}document.getElementById("edit-id").value=t,document.getElementById("edit-name").value=e.name||"",document.getElementById("edit-phone").value=e.phone||"",document.getElementById("edit-medal-name").value=e.medal_name||"",document.getElementById("edit-tour").value=e.tour||"",re("edit-date",e.tour||"",e.date||"",e.schedule_id||null),(c=>{let b=c.dob||"";if(b&&b.includes("/")){const f=b.split("/");f.length===3&&(b=`${f[2]}-${f[1].padStart(2,"0")}-${f[0].padStart(2,"0")}`)}document.getElementById("edit-dob").value=b,document.getElementById("edit-gender").value=c.gender||"Khác",document.getElementById("edit-allergy").value=c.allergy||c.medical_notes||"",document.getElementById("edit-address").value=c.address||"",document.getElementById("edit-diet").value=c.diet||c.dietary||"",document.getElementById("edit-trekking-pole").value=c.trekking_pole||"Không",document.getElementById("edit-id-card").value=c.id_card||c.cccd||""})(e),document.getElementById("edit-status").value=e.status||"Chờ xác nhận cọc",document.getElementById("edit-commitments").checked=!!e.commitments,document.getElementById("edit-special").value=e.special||"";try{const c=localStorage.getItem("csr_user");if(c&&JSON.parse(c).role==="admin"){const f=document.getElementById("edit-sale-container");f&&f.classList.remove("hidden");const g=document.getElementById("edit-sale");g&&(g.value=e.sale_id||"")}}catch{}let o=0;try{const c=typeof e.services_booked=="string"?JSON.parse(e.services_booked):e.services_booked||[];Array.isArray(c)&&c.forEach(b=>o+=parseInt(b.price)||0)}catch{}let s=(parseInt(e.total_price)||0)+(parseInt(e.discount)||0)-o;if(s===0&&e.tour){const c=D.find(b=>b.name===e.tour);c&&parseInt(c.price)>0&&(s=parseInt(c.price))}document.getElementById("edit-total").value=s,document.getElementById("edit-discount").value=e.discount||0,document.getElementById("edit-deposit").value=e.deposit||0;const a=parseInt(e.deposit_required);document.getElementById("edit-deposit-required").value=isNaN(a)?1e6:a,window.updateEditRemaining();const p=document.getElementById("servicesContainer"),i=document.getElementById("emptyServicesMsg"),d=document.getElementById("servicesTotalRow"),l=document.getElementById("presetServicesBlock"),m=document.getElementById("presetServicesList"),u=document.getElementById("servicesDivider");let h=[];try{if(e.services_booked){const c=typeof e.services_booked=="string"?JSON.parse(e.services_booked):e.services_booked;Array.isArray(c)&&(h=c)}}catch{}if(m&&l){m.innerHTML="";const c=D.find(f=>f.name===e.tour);let b=[];try{if(c?.services){const f=typeof c.services=="string"?JSON.parse(c.services):c.services;Array.isArray(f)&&(b=f)}}catch{}b.length>0?(l.classList.remove("hidden"),u&&u.classList.remove("hidden"),b.forEach(f=>{const g=h.some(S=>(S.label||S.name||"").toLowerCase()===(f.label||"").toLowerCase()),E=document.createElement("label");E.className="preset-service-card flex items-center gap-3 p-3 rounded-xl border-2 cursor-pointer transition-all "+(g?"border-green-400 bg-green-50":"border-gray-100 bg-gray-50"),E.innerHTML=`
                        ...
                        `,E.querySelector(".preset-service-check").addEventListener("change",S=>{E.className="preset-service-card flex items-center gap-3 p-3 rounded-xl border-2 cursor-pointer transition-all "+(S.target.checked?"border-green-400 bg-green-50":"border-gray-100 bg-gray-50"),window.updateServicesTotal()}),m.appendChild(E)})):(l.classList.add("hidden"),u&&u.classList.add("hidden"))}const w=document.getElementById("edit-pickup-point");if(w){const c=D.find(v=>v.name===e.tour);let b=[];try{if(c?.pickup_points){const v=typeof c.pickup_points=="string"?JSON.parse(c.pickup_points):c.pickup_points;Array.isArray(v)&&(b=v)}}catch{}w.innerHTML='<option value="">— Chưa chọn —</option>',b.forEach(v=>{const B=(v.label||v.name||v)+(v.time?` (${v.time})`:""),U=v.label||v.name||v,T=document.createElement("option");T.value=U,T.textContent=B,w.appendChild(T)});const f=document.createElement("option");f.value="__custom__",f.textContent="✏️ Khác (nhập tay)",w.appendChild(f);const g=document.getElementById("edit-pickup-custom"),E=v=>{g&&(v==="__custom__"?(g.classList.remove("hidden"),g.focus()):(g.classList.add("hidden"),g.value=""))},S=e.pickup_point||"";S&&(b.some(B=>(B.label||B.name||B)===S)?w.value=S:(w.value="__custom__",g&&(g.value=S,g.classList.remove("hidden")))),w.addEventListener("change",()=>E(w.value))}if(p){p.querySelectorAll(".service-row").forEach(g=>g.remove());const c=D.find(g=>g.name===e.tour);let b=new Set;try{const g=c?.services,E=g?typeof g=="string"?JSON.parse(g):g:[];Array.isArray(E)&&E.forEach(S=>b.add((S.label||"").toLowerCase()))}catch{}const f=h.filter(g=>!b.has((g.label||g.name||"").toLowerCase()));f.length>0?(i&&i.classList.add("hidden"),d&&d.classList.remove("hidden"),f.forEach(g=>window.addServiceRow(g.label||g.name||"",g.price||0))):i&&i.classList.remove("hidden"),window.updateServicesTotal()}const x=document.getElementById("editModal"),y=document.getElementById("editModalContent");x&&y&&(x.classList.remove("hidden"),setTimeout(()=>{x.classList.add("opacity-100"),y.classList.remove("scale-95","translate-y-4"),y.classList.add("scale-100","translate-y-0")},10))}catch(e){console.error("Lỗi khi mở form Edit:",e),alert("Lỗi khi mở form Edit: "+e.message)}};const le=document.getElementById("editForm");le&&le.addEventListener("submit",async t=>{t.preventDefault();const e=t.target.querySelector('button[type="submit"]'),r=e.textContent;e.textContent="Đang lưu cập nhật...",e.disabled=!0;try{const o=document.getElementById("edit-id").value,n=document.getElementById("edit-name").value,s=document.getElementById("edit-phone").value,a=document.getElementById("edit-medal-name").value,p=document.getElementById("edit-tour").value,i=document.getElementById("edit-date").value;let d=i,l;if(i&&i.includes("::")){const C=i.split("::");l=parseInt(C[0])||null,d=C[1]}const m=document.getElementById("edit-dob").value,u=document.getElementById("edit-gender").value,h=document.getElementById("edit-status").value,w=document.getElementById("edit-allergy").value,x=document.getElementById("edit-address").value,y=document.getElementById("edit-diet").value,c=document.getElementById("edit-trekking-pole").value,b=document.getElementById("edit-commitments").checked,f=document.getElementById("edit-special").value,g=document.getElementById("edit-id-card").value,E=document.getElementById("edit-pickup-point"),S=document.getElementById("edit-pickup-custom"),v=E?.value==="__custom__"?S?.value?.trim()||"":E?.value||"",B=[];document.querySelectorAll(".preset-service-check:checked").forEach(C=>{B.push({label:C.dataset.label,price:parseInt(C.dataset.price)||0})}),document.querySelectorAll(".service-row").forEach(C=>{const O=(C.querySelector(".service-label-input")?.value||"").trim(),De=parseInt(C.querySelector(".service-price-input")?.value)||0;O&&B.push({label:O,price:De})});const U=B.length>0?JSON.stringify(B):null,T=document.getElementById("edit-sale");let G,X;if(T&&!T.parentElement.classList.contains("hidden")&&(G=T.value||null,X="Website",G)){const C=W.find(O=>String(O.id)===String(G));C&&(X=C.full_name||C.fullName)}const Le=parseInt(document.getElementById("edit-total").value)||0,ie=parseInt(document.getElementById("edit-discount").value)||0,$e=parseInt(document.getElementById("edit-deposit").value)||0,de=parseInt(document.getElementById("edit-deposit-required").value),_e=isNaN(de)?1e6:de,Me={id:o,name:n,phone:s,medal_name:a,tour:p,date:d,dob:m,gender:u,status:h,allergy:w,address:x,diet:y,trekking_pole:c,commitments:b,special:f,id_card:g,pickup_point:v,services_booked:U,sale_id:G,sale_name:X,total_price:Le-ie,discount:ie,deposit:$e,deposit_required:_e,...l!==void 0?{schedule_id:l}:{}},ce=await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(Me)});if(ce.ok)alert("✅ Đã cập nhật thành công Chi tiết Đơn hàng!"),window.closeEditModal(),z();else{const C=await ce.json();throw new Error(C.error||"Gặp sự cố khi Cập nhật API.")}}catch(o){alert("❌ Lỗi Cập nhật: "+o.message)}finally{e.textContent=r,e.disabled=!1}}),ye(),we(),Promise.all([z(),Be()]).then(()=>R()).catch(()=>R())};export{Re as afterRender,Ae as render};
