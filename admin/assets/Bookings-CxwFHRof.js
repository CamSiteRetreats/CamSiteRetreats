import{S as $e,H as _e}from"./Header-CARqqxjt.js";console.log("Bookings V3 - Cache Busted!");const De=()=>`
    ...
    `,Ne=()=>{let D=[],S="consult",N=[],te=[],J=[];const Te=async()=>{try{const[n,e,o]=await Promise.all([fetch("/api/tours"),fetch("/api/schedules"),fetch("/api/users")]);N=n.ok?await n.json():[],te=e.ok?await e.json():[],J=o.ok?await o.json():[],U("addTourName"),U("edit-tour"),U("wl-tour"),Ie("edit-sale")}catch(n){console.error("Lỗi tải tours/schedules:",n)}},Ie=n=>{const e=document.getElementById(n);e&&(e.innerHTML='<option value="">-- Website --</option>',J.forEach(o=>{const a=document.createElement("option");a.value=o.id,a.textContent=`${o.full_name||o.fullName} (${o.role})`,e.appendChild(a)}))},U=n=>{const e=document.getElementById(n);if(!e)return;const o=e.value,a=e.options[0].outerHTML;e.innerHTML=a,N.filter(u=>u.is_visible!==!1).forEach(u=>{const p=document.createElement("option");p.value=u.name,p.textContent=u.name,u.name===o&&(p.selected=!0),e.appendChild(p)})},K=(n,e,o,a)=>{const u=document.getElementById(n);if(!u)return;if(u.innerHTML='<option value="">-- Chọn Lịch --</option>',!e){u.innerHTML='<option value="">-- Chọn Tour trước --</option>';return}const p=te.filter(i=>i.tour_name===e||e.includes(i.tour_name)||i.tour_name.includes(e));if(p.length===0){u.innerHTML='<option value="">Chưa có lịch cho tour này</option>';return}p.sort((i,r)=>new Date(i.start_date)-new Date(r.start_date)).forEach(i=>{const r=parseInt(i.booked_count)||0,h=(i.slots||0)-r,y=new Date(i.start_date),g=i.end_date?new Date(i.end_date):null,I=B=>`${B.getDate().toString().padStart(2,"0")}/${(B.getMonth()+1).toString().padStart(2,"0")}`;let E=I(y);g&&(E+=" - "+I(g)),i.group_label&&(E+=` · ${i.group_label}`),E+=` (${h>=0?h:0} chỗ trống)`;const w=document.createElement("option"),m=`${y.getDate().toString().padStart(2,"0")}/${(y.getMonth()+1).toString().padStart(2,"0")}/${y.getFullYear()}`;w.value=`${i.id}::${m}`,w.dataset.scheduleId=i.id,w.dataset.date=m,w.textContent=E,h<=0&&(w.disabled=!0,w.textContent=E.replace("chỗ trống","HẾT CHỖ"));let T=o||"";if(T.includes("-")&&T.length>=10){const B=T.substring(0,10).split("-");B.length===3&&(T=`${B[2]}/${B[1]}/${B[0]}`)}(a&&String(i.id)===String(a)||!a&&T&&m===T)&&(w.selected=!0),u.appendChild(w)})},ne=document.getElementById("addTourName");ne&&ne.addEventListener("change",n=>{const e=n.target.value;K("addTourDate",e);const o=N.find(u=>u.name===e),a=document.getElementById("addTourPrice");if(o&&a){const u=parseInt(o.price)||0;a.value=u;const p=document.getElementById("addDiscount"),i=document.getElementById("addTotalPriceDisplay"),r=parseInt(p?p.value:0)||0,h=u-r;i&&(i.textContent=(h>0?h:0).toLocaleString("vi-VN")+"đ")}});const oe=document.getElementById("edit-tour");oe&&oe.addEventListener("change",n=>{const e=n.target.value;K("edit-date",e);const o=N.find(u=>u.name===e),a=document.getElementById("edit-total");o&&a&&(parseInt(a.value)===0||!a.value)&&(a.value=parseInt(o.price)||0,typeof window.updateEditRemaining=="function"&&window.updateEditRemaining())});const ae=document.getElementById("wl-tour");ae&&ae.addEventListener("change",n=>{const e=n.target.value;K("wl-date",e)}),Te();const R=()=>{const n=document.getElementById("bookingsTableHead"),e=document.getElementById("bookingsTableBody");if(!e||!n)return;const o=S==="upcoming"||S==="ready"||S==="completed";o?n.innerHTML=`
            ...
            `;const a=document.getElementById("filterSearch")?document.getElementById("filterSearch").value.toLowerCase():"",u=document.getElementById("filterTour")?document.getElementById("filterTour").value:"",p=document.getElementById("filterDate")?document.getElementById("filterDate").value:"",i=document.getElementById("filterSale")?document.getElementById("filterSale").value:"",r=document.getElementById("filterStatus")?document.getElementById("filterStatus").value:"";let h=D.filter(t=>{let l=!1;const s=parseInt(t.total_price)>0&&parseInt(t.total_price)===parseInt(t.deposit),d=f=>{if(!f)return!1;try{let k=f.trim();if(k.includes("-")&&!/^\d{4}-\d{2}-\d{2}/.test(k)&&(k=k.split("-")[0].trim()),k.includes("/")){const x=k.split("/");if(x.length>=2){const C=parseInt(x[0],10),v=parseInt(x[1],10)-1,_=x.length===3?parseInt(x[2],10):new Date().getFullYear(),j=new Date(_,v,C),L=new Date;return L.setHours(0,0,0,0),j.getTime()<L.getTime()}}if(/^\d{4}-\d{2}-\d{2}/.test(k)){const x=k.split("-"),C=parseInt(x[0],10),v=parseInt(x[1],10)-1,_=parseInt(x[2],10),j=new Date(C,v,_),L=new Date;return L.setHours(0,0,0,0),j.getTime()<L.getTime()}}catch(k){console.warn("isPastTour error parsing:",f,k)}return!1},c=t.status||"",b=!d(t.date);return S==="consult"?l=c==="Chờ tư vấn"&&b:S==="pending"?l=(c==="Chờ cọc"||c==="Chờ xác nhận cọc"||c==="")&&b:S==="upcoming"?l=(c==="Đã cọc"||c==="Đã cọc (Chờ đi)"||c==="Hoàn tất phí")&&!s&&b:S==="ready"?l=(s||c==="Hoàn thành"||c==="Hoàn tất")&&b&&c!=="Đã hủy"&&c!=="Bảo lưu":S==="completed"&&(l=!b||c==="Đã đi"||c==="Đã hủy"||c==="Bảo lưu"),!(!l||u&&t.tour!==u||p&&Y(t.date)!==p||i&&i!=="null"&&t.sale_name!==i||i==="null"&&t.sale_name&&t.sale_name!=="Website"||a&&!`${t.name||""} ${t.phone||""} ${t.customer_id||""} ${t.id||""} csr${t.id||""} #csr${t.id||""}`.toLowerCase().includes(a)||r&&t.status!==r)});const y=document.getElementById("statTitle1"),g=document.getElementById("statTitle2"),I=document.getElementById("statTitle3"),E=document.getElementById("statTotalCustomers"),w=document.getElementById("statTotalRevenue"),m=document.getElementById("statTotalCollected");let T=h.length;if(S==="consult"){const t=document.getElementById("addWaitingBtn"),l=document.getElementById("addBookingBtn");t&&t.classList.remove("hidden"),l&&l.classList.add("hidden"),y&&(y.textContent="Tổng Đăng Ký Chờ Lịch"),g&&(g.textContent="Chưa Được Liên Hệ"),I&&(I.textContent="Đã Có Sale Phụ Trách");let s=0,d=0;h.forEach(c=>{!c.sale_id||c.sale_id==="null"||!c.sale_name||c.sale_name==="Website"||c.sale_name==="null"?s++:d++}),E&&(E.textContent=T),w&&(w.textContent=s+" Người"),m&&(m.textContent=d+" Người")}else if(S==="pending"){const t=document.getElementById("addWaitingBtn"),l=document.getElementById("addBookingBtn");t&&t.classList.add("hidden"),l&&l.classList.remove("hidden"),y&&(y.textContent="Tổng Khách (Chờ Cọc)"),g&&(g.textContent="Khách Chưa Tư Vấn (Mới)"),I&&(I.textContent="Đã Có Sale Giữ Chỗ");let s=0,d=0;h.forEach(c=>{!c.sale_id||c.sale_id==="null"||!c.sale_name||c.sale_name==="Website"||c.sale_name==="null"?s++:d++}),E&&(E.textContent=T),w&&(w.textContent=s+" Đơn"),m&&(m.textContent=d+" Đơn")}else if(S==="upcoming"){y&&(y.textContent="Tổng Số Khách"),g&&(g.textContent="Chưa Xác Nhận Cọc"),I&&(I.textContent="Chờ Thanh Toán Còn Lại");let t=0,l=0;h.forEach(s=>{s.status==="Chờ xác nhận cọc"?t++:parseInt(s.deposit)>0&&parseInt(s.total_price)>parseInt(s.deposit)&&l++}),E&&(E.textContent=T),w&&(w.textContent=t+" Khách"),m&&(m.textContent=l+" Khách")}else if(S==="ready"){y&&(y.textContent="Khách Đã Hoàn Tất"),g&&(g.textContent="Tổng Doanh Thu Tab"),I&&(I.textContent="Thực Thu");let t=0,l=0;h.forEach(s=>{t+=parseInt(s.total_price)||0,l+=parseInt(s.deposit)||0}),E&&(E.textContent=T),w&&(w.textContent=t>0?t.toLocaleString("vi-VN")+"đ":"0đ"),m&&(m.textContent=l>0?l.toLocaleString("vi-VN")+"đ":"0đ")}else{y&&(y.textContent="Khách Đã Xong (Lịch Sử)"),g&&(g.textContent="Tổng Doanh Thu Tab"),I&&(I.textContent="Thực Thu (Đã Đóng)");let t=0,l=0;h.forEach(s=>{t+=parseInt(s.total_price)||0,l+=parseInt(s.deposit)||0}),E&&(E.textContent=T),w&&(w.textContent=t>0?t.toLocaleString("vi-VN")+"đ":"0đ"),m&&(m.textContent=l>0?l.toLocaleString("vi-VN")+"đ":"0đ")}const B=o?8:4;if(!h||h.length===0){let t=S==="consult"?"Chưa có khách hàng tư vấn nào.":S==="pending"?"Chưa có Khách chờ cọc.":S==="upcoming"?"Chưa có Khách nào Đã Cọc.":S==="ready"?"Chưa có khách hoàn tất.":"Danh sách rỗng.";e.innerHTML=`<tr><td colspan="${B}" class="p-8 text-center text-gray-500">${t}</td></tr>`;return}e.innerHTML=h.map(t=>{const l=S==="upcoming"||S==="ready"?"bg-green-50/20 hover:bg-green-50 transition-colors":"hover:bg-gray-100 transition-colors";if(o){let s=parseInt(t.total_price)||0;if(s===0&&t.tour&&N&&N.length>0){const v=N.find(_=>_.name&&t.tour&&_.name.toLowerCase().trim()===t.tour.toLowerCase().trim());v&&parseInt(v.price)>0&&(s=parseInt(v.price))}const d=parseInt(t.deposit)||0,c=s-d,b=s>0?s.toLocaleString("vi-VN")+"đ":"Chưa định giá",f=d>0?d.toLocaleString("vi-VN")+"đ":"0đ",k=c>0?c.toLocaleString("vi-VN")+"đ":c===0&&s>0?"Đã thu trọn":"-";let x="";return t.status==="Đã hủy"?x='<span class="bg-red-50 text-red-500 border border-red-200 px-2 py-0.5 rounded text-xs block w-full text-center font-bold">❌ Đã hủy</span>':t.status==="Bảo lưu"?x='<span class="bg-gray-100 text-gray-500 border border-gray-300 px-2 py-0.5 rounded text-xs block w-full text-center font-bold">⏸️ Bảo lưu</span>':t.status==="Hoàn thành"||t.status==="Đã đi"?x='<span class="bg-violet-100 text-violet-700 border border-violet-200 px-2 py-0.5 rounded text-xs block w-full text-center font-bold">Hoàn Thành</span>':t.status==="Chờ xác nhận cọc"?x=`<button class="action-btn confirm-deposit-btn bg-csr-orange text-white px-3 py-1.5 rounded text-xs font-bold shadow-sm hover:bg-[#d65503] w-full" data-id="${t.id}">Xác nhận cọc</button>`:d>0&&c===0?x='<span class="bg-emerald-100 text-emerald-700 border border-emerald-200 px-2 py-0.5 rounded text-xs font-bold block w-full text-center">✅ Hoàn Tất Phí</span>':t.status==="Hoàn tất phí"||d>0?x='<span class="bg-blue-100 text-blue-700 border border-blue-200 px-2 py-0.5 rounded text-xs font-medium block w-full text-center">✅ Đã Cọc</span>':x='<span class="bg-yellow-50 text-yellow-700 border border-yellow-200 px-2 py-0.5 rounded text-xs font-medium block w-full text-center">Chưa cọc</span>',`
                <tr class="${l} cursor-pointer row-clickable block md:table-row p-4 md:p-0 mb-4 md:mb-0 glass-panel md:glass-none border-l-4 border-green-500 md:border-none relative" data-id="${t.id}">
                    ...
                    <td class="p-4 align-top text-center hidden md:table-cell">${x}</td>
                ...
                `}else{const s=t.name?t.name.substring(0,2).toUpperCase():"KH",d=S==="consult",c=!t.sale_id||t.sale_id==="null"||!t.sale_name||t.sale_name==="null";let b="";d&&c?b=`<button class="action-btn claim-btn bg-csr-orange hover:bg-[#d65503] text-white px-3 py-1.5 rounded text-xs font-bold shadow-sm transition-colors" data-id="${t.id}">Nhận khách</button>`:b=`<span class="text-sm text-gray-500">${t.sale_name||"Website"}</span>`;const f=`
                ...
                <tr class="${l} cursor-pointer row-clickable block md:table-row p-4 md:p-0 mb-4 md:mb-0 glass-panel md:glass-none border-l-4 border-csr-orange md:border-none relative" data-id="${t.id}">
                    ...
                    <td class="py-3 md:p-4 align-top block md:table-cell border-t md:border-none border-gray-100 mt-3 md:mt-0 pt-3 md:pt-4">
                        ...
                        ${d&&t.special?`<div class="text-xs text-blue-500 mt-1 italic bg-blue-50 md:bg-transparent p-2 md:p-0 rounded">💬 ${t.special}</div>`:""}
                    </td>
    ...
    `}}).join(""),document.querySelectorAll(".claim-btn").forEach(t=>{t.addEventListener("click",async l=>{l.stopPropagation();const s=t.getAttribute("data-id"),d=localStorage.getItem("csr_user");let c=null,b="Admin";if(d){const f=JSON.parse(d);c=f.id,b=f.fullName||f.full_name||"Admin"}t.textContent="Đang xử lý...",t.disabled=!0;try{if((await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:s,sale_id:c,sale_name:b})})).ok){const k=D.find(x=>x.id==s);k&&(k.sale_id=c,k.sale_name=b),R()}else alert("Lỗi khi nhận khách. Vui lòng thử lại."),t.textContent="Nhận khách",t.disabled=!1}catch(f){console.error("Lỗi nhận khách:",f),t.textContent="Nhận khách",t.disabled=!1}})})},Ee=()=>{const n=new Set,e=new Set,o=new Set;D.forEach(i=>{if(i.tour&&n.add(i.tour),i.date){const r=Y(i.date);r&&e.add(r)}i.sale_name&&i.sale_name!=="Website"&&i.sale_name!=="null"&&o.add(i.sale_name.trim())});const a=document.getElementById("filterTour"),u=document.getElementById("filterDate"),p=document.getElementById("filterSale");if(a){const i=a.value;a.innerHTML='<option value="">Tất cả Tour</option>',[...n].sort().forEach(r=>a.innerHTML+=`<option value="${r}">${r}</option>`),a.value=i}if(u){const i=u.value;u.innerHTML='<option value="">Tất cả Ngày</option>',[...e].sort().forEach(r=>u.innerHTML+=`<option value="${r}">${r}</option>`),u.value=i}if(p){const i=p.value;p.innerHTML='<option value="">Tất cả Sale</option>',p.innerHTML+='<option value="null">Nguồn Website</option>',[...o].sort().forEach(r=>{p.innerHTML+=`<option value="${r}">${r}</option>`}),p.value=i}};function Y(n){if(!n)return"";if(/^\d{1,2}\/\d{1,2}\s*-\s*\d{1,2}\/\d{1,2}$/.test(n))return n.replace(/\s+/g,"");try{if(/^\d{4}-\d{2}-\d{2}$/.test(n)){const[e,o,a]=n.split("-"),u=`${parseInt(a)}/${parseInt(o)}`;return`${u}-${u}`}if(/^\d{1,2}\/\d{1,2}\/\d{4}$/.test(n)){const[e,o,a]=n.split("/"),u=`${parseInt(e)}/${parseInt(o)}`;return`${u}-${u}`}if(n.includes("-"))return n.split("-").map(o=>{const a=o.trim();if(/\/\d{4}$/.test(a)){const[u,p]=a.split("/");return`${parseInt(u)}/${parseInt(p)}`}return a}).join("-")}catch{console.warn("Date normalization failed for:",n)}return n}const H=async()=>{try{D=await(await fetch("/api/bookings")).json(),D.sort((e,o)=>parseInt(o.id)-parseInt(e.id)),Ee(),R()}catch(n){console.error("Lỗi parse list đơn hàng:",n);const e=document.getElementById("bookingsTableBody");e&&(e.innerHTML='<tr><td colspan="4" class="p-8 text-center text-red-500">Lỗi lấy dữ liệu từ Server.</td></tr>')}};H(),["filterSearch","filterTour","filterDate","filterSale","filterStatus"].forEach(n=>{const e=document.getElementById(n);e&&(n==="filterSearch"?e.addEventListener("input",R):e.addEventListener("change",R))});const se=document.getElementById("exportExcelBtn");se&&se.addEventListener("click",()=>{const n=document.getElementById("filterSearch")?document.getElementById("filterSearch").value.toLowerCase():"",e=document.getElementById("filterTour")?document.getElementById("filterTour").value:"",o=document.getElementById("filterDate")?document.getElementById("filterDate").value:"",a=document.getElementById("filterSale")?document.getElementById("filterSale").value:"";let u=D.filter(g=>{let I=!1;const E=parseInt(g.total_price)>0&&parseInt(g.total_price)===parseInt(g.deposit),w=T=>{if(!T)return!1;try{let B=T.trim();if(B.includes("-")&&!/^\d{4}-\d{2}-\d{2}/.test(B)&&(B=B.split("-")[0].trim()),B.includes("/")){const t=B.split("/");if(t.length>=2){const l=parseInt(t[0],10),s=parseInt(t[1],10)-1,d=t.length===3?parseInt(t[2],10):new Date().getFullYear(),c=new Date(d,s,l),b=new Date;return b.setHours(0,0,0,0),c.getTime()<b.getTime()}}if(/^\d{4}-\d{2}-\d{2}/.test(B)){const t=B.split("-"),l=parseInt(t[0],10),s=parseInt(t[1],10)-1,d=parseInt(t[2],10),c=new Date(l,s,d),b=new Date;return b.setHours(0,0,0,0),c.getTime()<b.getTime()}}catch(B){console.warn("isPastTour error parsing in export:",T,B)}return!1},m=(g.status==="Hoàn thành"||g.status==="Hoàn tất"||g.status==="Hoàn tất phí"||g.status==="Đã đi"||E)&&w(g.date);return S==="consult"?I=g.status==="Chờ tư vấn"&&!m:S==="pending"?I=(!g.status||g.status==="Chờ cọc")&&!m:S==="upcoming"?I=(g.status==="Đã cọc"||g.status==="Đã cọc (Chờ đi)"||g.status==="Hoàn tất phí")&&!E&&!m:S==="ready"?I=(E||g.status==="Hoàn thành"||g.status==="Hoàn tất")&&!m:S==="completed"&&(I=m),!(!I||e&&g.tour!==e||o&&Y(g.date)!==o||a&&a!=="null"&&g.sale_name!==a||a==="null"&&g.sale_name&&g.sale_name!=="Website"||n&&!`${g.name||""} ${g.phone||""} ${g.customer_id||""} ${g.id||""} csr${g.id||""} #csr${g.id||""}`.toLowerCase().includes(n))});if(u.length===0){alert("Danh sách rỗng. Không có dữ liệu để xuất.");return}let p=`
        ...
        <div class="space-y-4 pb-2">
            ...
            <div class="bg-gradient-to-br from-orange-50 to-amber-50 border border-orange-100 rounded-2xl p-4">
                ...
                <div class="grid grid-cols-2 gap-x-4">
                    ...
                    ${m("Trạng Thái",`<span class="inline-block px-2.5 py-1 rounded-full text-xs font-bold border ${w}">${I||"Chưa rõ"}</span>`)}
            ...
            <div class="bg-gradient-to-br from-csr-orange to-orange-600 rounded-2xl p-4 text-white shadow-lg shadow-orange-500/20">
                ...
                <div class="space-y-2 text-sm">
                    ...
                    <div class="flex justify-between items-center py-1.5 border-b border-white/10">
                        ...
                        <span class="font-bold text-green-100">+ ${o(y)}</span>
                    </div>`:""}
            ...
            ${(()=>{const t=e.phone;if(!t)return"";const l=D.filter(C=>C.phone===t&&!C.status?.includes("hủy")&&!C.status?.includes("Hủy")).sort((C,v)=>new Date(v.created_at||0)-new Date(C.created_at||0)),s=l.length,d=l.reduce((C,v)=>C+(parseInt(v.total_price)||0),0);let c,b,f;s>=5?(c="KHÁCH VIP",b="bg-purple-100 text-purple-700 border-purple-300",f="👑"):s>=2?(c="KHÁCH THÂN THIẾT",b="bg-green-100 text-green-700 border-green-300",f="🌟"):(c="KHÁCH MỚI",b="bg-gray-100 text-gray-500 border-gray-300",f="👋");const k=l.filter(C=>C.id!=n),x=k.length>0?k.slice(0,5).map((C,v)=>{const _=C.status?.includes("Hoàn")?"bg-green-100 text-green-600":C.status?.includes("cọc")||C.status?.includes("Cọc")?"bg-blue-100 text-blue-600":"bg-gray-100 text-gray-500";return`<div class="flex items-center gap-3 py-2 border-b border-gray-100 last:border-none">
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="text-sm font-medium text-gray-800 truncate">${C.tour||"—"}</div>
                                <div class="text-[10px] text-gray-400">${C.date||"—"}${C.medal_name?` • <span class="text-csr-orange font-medium">Huy chương: ${C.medal_name}</span>`:""}</div>
                            </div>
                            <span class="${_} text-[9px] px-2 py-0.5 rounded-full font-bold shrink-0">${C.status||"Mới"}</span>
                    ...
                    <div class="flex items-center justify-between mb-4 gap-3">
                        ...
                        <div class="text-right">
                            ...
                            <div class="text-lg font-black text-gray-900">${d>0?d.toLocaleString("vi-VN")+"đ":"—"}</div>
                        </div>
                    </div>
                    ...
                    ${k.length>0?`
                    <div class="mb-3">
                        <div class="text-[10px] font-bold text-gray-400 uppercase tracking-wide mb-2">Các tour trước đây (${k.length})</div>
                        <div class="bg-white rounded-xl border border-gray-100 px-3">${x}</div>
                    </div>`:x}
            ...
            <div class="bg-blue-50 border border-blue-100 rounded-xl px-4 py-3 flex items-center gap-3">
                ...
                <div>
        ...
        </div>`;document.getElementById("detailContentBlock").innerHTML=B,document.getElementById("detailModal").classList.remove("hidden"),setTimeout(()=>{document.getElementById("detailModal").classList.add("opacity-100");const t=document.getElementById("detailModalContent");t&&(t.classList.remove("scale-95","translate-y-4"),t.classList.add("scale-100","translate-y-0"))},10)},window.actionEdit=async n=>{document.getElementById("rowActionModal").classList.add("hidden");const e=D.find(t=>t.id==n);if(!e)return;document.getElementById("edit-id").value=n,document.getElementById("edit-name").value=e.name||"",document.getElementById("edit-phone").value=e.phone||"",document.getElementById("edit-medal-name").value=e.medal_name||"",document.getElementById("edit-tour").value=e.tour||"",typeof K=="function"&&K("edit-date",e.tour||"",e.date||"",e.schedule_id||null),(t=>{let l=t.dob||"";if(typeof l=="string"&&l.includes("/")){const c=l.split("/");c.length===3&&(l=`${c[2]}-${c[1].padStart(2,"0")}-${c[0].padStart(2,"0")}`)}document.getElementById("edit-dob").value=l,document.getElementById("edit-gender").value=t.gender||"Khác",document.getElementById("edit-allergy").value=t.allergy||t.medical_notes||"",document.getElementById("edit-address").value=t.address||"",document.getElementById("edit-diet").value=t.diet||t.dietary||"",document.getElementById("edit-trekking-pole").value=t.trekking_pole||"Không";const s=document.getElementById("edit-id-card");s&&(s.value=t.id_card||t.cccd||"");const d=document.getElementById("edit-pickup-point");d&&(d.value=t.pickup_point||"")})(e),document.getElementById("edit-status").value=e.status||"Chờ xác nhận cọc",document.getElementById("edit-commitments").checked=!!e.commitments,document.getElementById("edit-special").value=e.special||"";try{const t=localStorage.getItem("csr_user");if(t&&JSON.parse(t).role==="admin"){const s=document.getElementById("edit-sale-container");s&&s.classList.remove("hidden");const d=document.getElementById("edit-sale");d&&(d.value=e.sale_id||"")}}catch{}if((!e.dob||!e.address||!e.gender||e.gender==="Khác")&&(e.customer_id||e.phone))try{const t=e.customer_id||e.phone,l=await fetch(`/api/admin_customers?action=search&keyword=${encodeURIComponent(t)}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({keyword:t})}),s=await l.json();if(l.ok&&s.success){const d=s.data,c=document.getElementById("edit-dob"),b=document.getElementById("edit-address"),f=document.getElementById("edit-allergy"),k=document.getElementById("edit-diet"),x=document.getElementById("edit-gender"),C=document.getElementById("edit-medal-name");c&&!c.value&&(c.value=d.dob||""),b&&!b.value&&(b.value=d.address||""),f&&!f.value&&(f.value=d.medical_notes||""),k&&!k.value&&(k.value=d.dietary||""),x&&(x.value==="Khác"||!x.value)&&(x.value=d.gender||"Khác"),C&&!C.value&&(C.value=d.medal_name||"");const v=document.getElementById("edit-id-card");v&&!v.value&&(v.value=d.cccd||"")}}catch(t){console.error("Pull CRM error:",t)}let a=0;try{const t=typeof e.services_booked=="string"?JSON.parse(e.services_booked):e.services_booked||[];Array.isArray(t)&&t.forEach(l=>a+=parseInt(l.price)||0)}catch{}let p=(parseInt(e.total_price)||0)+(parseInt(e.discount)||0)-a;if(p===0&&e.tour){const t=N.find(l=>l.name===e.tour);t&&parseInt(t.price)>0&&(p=parseInt(t.price))}document.getElementById("edit-total").value=p,document.getElementById("edit-discount").value=e.discount||0,document.getElementById("edit-deposit").value=e.deposit||0;const i=parseInt(e.deposit_required);document.getElementById("edit-deposit-required").value=isNaN(i)?1e6:i,window.updateEditRemaining();const r=document.getElementById("servicesContainer"),h=document.getElementById("emptyServicesMsg"),y=document.getElementById("servicesTotalRow"),g=document.getElementById("presetServicesBlock"),I=document.getElementById("presetServicesList"),E=document.getElementById("servicesDivider");let w=[];try{if(e.services_booked){const t=typeof e.services_booked=="string"?JSON.parse(e.services_booked):e.services_booked;Array.isArray(t)&&(w=t)}}catch{}if(I&&g){I.innerHTML="";const t=N.find(s=>s.name===e.tour);let l=[];try{if(t?.services){const s=typeof t.services=="string"?JSON.parse(t.services):t.services;Array.isArray(s)&&(l=s)}}catch{}l.length>0?(g.classList.remove("hidden"),E&&E.classList.remove("hidden"),l.forEach(s=>{const d=w.some(b=>(b.label||b.name||"").toLowerCase()===(s.label||"").toLowerCase()),c=document.createElement("label");c.className="preset-service-card flex items-center gap-3 p-3 rounded-xl border-2 cursor-pointer transition-all "+(d?"border-green-400 bg-green-50":"border-gray-100 bg-gray-50 hover:border-orange-200"),c.innerHTML=`
                    ...
                    `,c.querySelector(".preset-service-check").addEventListener("change",b=>{c.className="preset-service-card flex items-center gap-3 p-3 rounded-xl border-2 cursor-pointer transition-all "+(b.target.checked?"border-green-400 bg-green-50":"border-gray-100 bg-gray-50 hover:border-orange-200"),window.updateServicesTotal()}),I.appendChild(c)})):(g.classList.add("hidden"),E&&E.classList.add("hidden"))}const m=document.getElementById("edit-pickup-point");if(m){const t=N.find(f=>f.name===e.tour);let l=[];try{if(t?.pickup_points){const f=typeof t.pickup_points=="string"?JSON.parse(t.pickup_points):t.pickup_points;Array.isArray(f)&&(l=f)}}catch{}m.innerHTML='<option value="">— Chưa chọn —</option>',l.forEach(f=>{const k=(f.label||f.name||f)+(f.time?` (${f.time})`:""),x=f.label||f.name||f,C=document.createElement("option");C.value=x,C.textContent=k,m.appendChild(C)});const s=document.createElement("option");s.value="__custom__",s.textContent="✏️ Khác (nhập tay)",m.appendChild(s);const d=document.getElementById("edit-pickup-custom"),c=f=>{d&&(f==="__custom__"?(d.classList.remove("hidden"),d.focus()):(d.classList.add("hidden"),d&&(d.value="")))},b=e.pickup_point||"";b&&(l.some(k=>(k.label||k.name||k)===b)?m.value=b:(m.value="__custom__",d&&(d.value=b,d.classList.remove("hidden")))),m.addEventListener("change",()=>c(m.value))}if(r){r.querySelectorAll(".service-row").forEach(d=>d.remove());const t=N.find(d=>d.name===e.tour);let l=new Set;try{const d=t?.services,c=d?typeof d=="string"?JSON.parse(d):d:[];Array.isArray(c)&&c.forEach(b=>l.add((b.label||"").toLowerCase()))}catch{}const s=w.filter(d=>!l.has((d.label||d.name||"").toLowerCase()));s.length>0?(h&&h.classList.add("hidden"),y&&y.classList.remove("hidden"),s.forEach(d=>window.addServiceRow(d.label||d.name||"",d.price||0))):h&&h.classList.remove("hidden"),window.updateServicesTotal()}const T=document.getElementById("editModal"),B=document.getElementById("editModalContent");T&&B&&(T.classList.remove("hidden"),setTimeout(()=>{T.classList.add("opacity-100"),B.classList.remove("scale-95","translate-y-4"),B.classList.add("scale-100","translate-y-0")},10))},window.actionDelete=async n=>{if(document.getElementById("rowActionModal").classList.add("hidden"),confirm("Bạn có chắc chắn muốn xóa Đơn hàng này? Thao tác không thể hoàn tác."))try{if((await fetch(`/api/bookings?id=${n}`,{method:"DELETE"})).ok)alert("✅ Đã xóa đơn hàng thành công!"),H();else throw new Error("Lỗi từ Server")}catch(e){alert("❌ Không thể xóa bảng ghi này: "+e.message)}},window.updateServicesTotal=()=>{let n=0;document.querySelectorAll(".preset-service-check:checked").forEach(a=>{n+=parseInt(a.dataset.price)||0}),document.querySelectorAll(".service-price-input").forEach(a=>{n+=parseInt(a.value)||0});const e=document.getElementById("servicesTotalDisplay");e&&(e.textContent=n.toLocaleString("vi-VN")+"đ");const o=document.getElementById("servicesTotalRow");return o&&o.classList.toggle("hidden",n===0),window.updateEditRemaining(),n},window.addServiceRow=(n="",e=0)=>{const o=document.getElementById("servicesContainer"),a=document.getElementById("emptyServicesMsg"),u=document.getElementById("servicesTotalRow");if(!o)return;a&&a.classList.add("hidden"),u&&u.classList.remove("hidden");const p=document.createElement("div");p.className="service-row flex items-center gap-2 bg-gray-50 border border-gray-100 rounded-xl px-3 py-2.5",p.innerHTML=`
...
Gửi cho khách: `+r)}).catch(h=>{alert("Lỗi khi Copy Clipboard. Link là: "+r)})}else e.classList.contains("view-btn")?(document.getElementById("rowActionModal").classList.add("hidden"),window.actionView(o)):e.classList.contains("edit-btn")&&window.actionEdit(o)});const q=document.getElementById("smartSearch"),M=document.getElementById("searchSuggestions");if(q){let n;q.addEventListener("input",e=>{const o=e.target.value.toLowerCase().trim();if(clearTimeout(n),o.length<2){M&&M.classList.add("hidden");return}n=setTimeout(async()=>{try{const a=await fetch(`/api/admin_customers?action=search&keyword=${o}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({keyword:o})}),u=await a.json();if(a.ok&&u.success){const i=[u.data];M&&(M.innerHTML=i.map(r=>`
                        ...
                        `).join(""),M.classList.remove("hidden"))}else M&&(M.innerHTML='<div class="p-3 text-xs text-gray-400 italic text-center">Không tìm thấy khách hàng cũ</div>',M.classList.remove("hidden"))}catch(a){console.error("Search error:",a)}},300)}),M&&M.addEventListener("click",e=>{const o=e.target.closest(".suggestion-item");if(o){const a=document.getElementById("addFullName"),u=document.getElementById("addPhone"),p=document.getElementById("addCsrCode");a&&(a.value=o.getAttribute("data-name")),u&&(u.value=o.getAttribute("data-phone")),p&&(p.value=o.getAttribute("data-csr"));const i=document.getElementById("addDob"),r=document.getElementById("addGender"),h=document.getElementById("addIdCard"),y=document.getElementById("addAddress");i&&(i.value=o.getAttribute("data-dob")||""),r&&(r.value=o.getAttribute("data-gender")||"Khác"),h&&(h.value=o.getAttribute("data-cccd")||""),y&&(y.value=o.getAttribute("data-address")||"");const g=document.getElementById("addAllergy"),I=document.getElementById("addDiet"),E=document.getElementById("addTrekkingPole"),w=document.getElementById("addMedalName");g&&(g.value=o.getAttribute("data-allergy")||""),I&&(I.value=o.getAttribute("data-diet")||""),E&&(E.value=o.getAttribute("data-trekking_pole")||"Không"),w&&(w.value=o.getAttribute("data-medal-name")||""),window._selectedCustomer={fullName:o.getAttribute("data-name"),phone:o.getAttribute("data-phone"),csrCode:o.getAttribute("data-csr"),dob:o.getAttribute("data-dob")||"",gender:o.getAttribute("data-gender")||"",address:o.getAttribute("data-address")||"",id_card:o.getAttribute("data-cccd")||"",allergy:o.getAttribute("data-allergy")||"",diet:o.getAttribute("data-diet")||"",trekking_pole:o.getAttribute("data-trekking_pole")||"",medal_name:o.getAttribute("data-medal-name")||""},q.value=o.getAttribute("data-name"),M.classList.add("hidden");const m=`
