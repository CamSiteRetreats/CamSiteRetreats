import{S as j,H as A}from"./Header-n5pNPUhI.js";const K=()=>{let c={role:"sale",fullName:"Guest",id:null};try{const m=localStorage.getItem("csr_user");m&&(c=JSON.parse(m))}catch{}const u=c.role==="admin",x=new Date,w=new Date(x.getFullYear(),x.getMonth(),1),k=x.toISOString().split("T")[0],p=w.toISOString().split("T")[0];return`
                  ...
                  <div class="flex justify-between items-end flex-wrap gap-4">
                      ...
                      <div class="flex items-center gap-3 flex-wrap">
                          <div class="flex items-center gap-2 bg-white border border-gray-200 rounded-xl px-4 py-2.5 shadow-sm">
                              ...
                              <input type="date" id="reportDateFrom" class="text-sm font-bold text-gray-800 border-none outline-none bg-transparent" value="${p}">
                          </div>
                          <span class="text-gray-400 font-bold">→</span>
                          <div class="flex items-center gap-2 bg-white border border-gray-200 rounded-xl px-4 py-2.5 shadow-sm">
                              <svg class="w-4 h-4 text-gray-400 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>
                              <label class="text-xs font-bold text-gray-400 uppercase whitespace-nowrap">Đến ngày</label>
                              <input type="date" id="reportDateTo" class="text-sm font-bold text-gray-800 border-none outline-none bg-transparent" value="${k}">
                          </div>
    ...
    `},V=()=>{let c={role:"sale",fullName:"Guest",id:null};try{const t=localStorage.getItem("csr_user");t&&(c=JSON.parse(t))}catch{}const u=c.role==="admin";let x=[],w=[],k=[],p=[];const m=document.getElementById("reportDateFrom"),v=document.getElementById("reportDateTo"),B=document.getElementById("reportApplyBtn"),C=new Date;C.setHours(0,0,0,0);const g=t=>new Intl.NumberFormat("vi-VN",{style:"currency",currency:"VND"}).format(t||0),H=t=>{if(!t)return null;const e=t.split("-")[0].trim();if(e.includes("/")){const s=e.split("/");if(s.length===3)return new Date(parseInt(s[2]),parseInt(s[1])-1,parseInt(s[0]))}else if(e.includes("-"))return new Date(e);return null},D=()=>{const t=m.value?new Date(m.value):null,e=v.value?new Date(v.value):null;return t&&t.setHours(0,0,0,0),e&&e.setHours(23,59,59,999),{from:t,to:e}},L=(t,e,s)=>{const o=H(t.date);return!(!o||e&&o<e||s&&o>s)},_=()=>{const t={};return k.forEach(e=>{t[String(e.id)]=e.full_name||e.fullName||e.username||`User #${e.id}`}),t},I=async()=>{try{const[t,e,s]=await Promise.all([fetch("/api/admin_tours"),fetch("/api/bookings"),fetch("/api/users")]),o=t.ok?await t.json():{};x=Array.isArray(o)?o:o.data||[],w=e.ok?await e.json():[],k=s.ok?await s.json():[],u&&M(),f()}catch(t){console.error("Lỗi tải data báo cáo:",t),alert("Có lỗi xảy ra khi lấy dữ liệu báo cáo!")}},M=()=>{const t=document.getElementById("commissionSetupContainer");if(!t)return;if(x.length===0){t.innerHTML='<div class="text-sm text-gray-500 text-center py-4">Không có Tour nào trong hệ thống.</div>';return}const e=x.map(s=>{const o=s.commission_rate??5;return`
            ...
            `}).join("")},R=t=>{const e=t.length,s=String(c.id||c.userId||""),o=p.filter(n=>{const d=String(n.sale_id||""),T=n.sale_name&&n.sale_name.toLowerCase()===c.fullName.toLowerCase();return s?d===s:T}),r=o.filter(n=>{if(!(n.status&&(n.status.includes("Hoàn tất")||n.status.includes("Hoàn thành"))))return!1;const T=H(n.date);return T&&T<C}),a=r.length,i=o.reduce((n,d)=>n+(Number(d.total_price)||0),0),l=r.reduce((n,d)=>n+d._commission,0);document.getElementById("sale-stat-total-pax").textContent=e,document.getElementById("sale-stat-success-pax").textContent=a,document.getElementById("sale-stat-revenue").textContent=g(i),document.getElementById("sale-stat-real-comm").textContent=g(l);const b=document.getElementById("saleTourTableBody");if(!b)return;const y={};o.forEach(n=>{const d=n.tour||"Khác";y[d]||(y[d]={tourName:d,bookings:[],totalRev:0,totalComm:0}),y[d].bookings.push(n),y[d].totalRev+=Number(n.total_price)||0,y[d].totalComm+=n._commission});let S=Object.values(y).sort((n,d)=>d.totalRev-n.totalRev);if(window._currentDetailMode="sale",window._currentDetailList=S,S.length===0){b.innerHTML='<tr><td colspan="6" class="p-8 text-center text-gray-400 text-sm">Bạn chưa có đơn hàng nào chốt thành công trong kỳ này.</td></tr>';return}b.innerHTML=S.map((n,d)=>`
        ...
        `).join(""),h.classList.remove("hidden"),requestAnimationFrame(()=>{h.classList.remove("opacity-0"),N.classList.remove("scale-95")})},B.addEventListener("click",f),m.addEventListener("keydown",t=>{t.key==="Enter"&&f()}),v.addEventListener("keydown",t=>{t.key==="Enter"&&f()}),h&&h.addEventListener("click",t=>{t.target===h&&window.closeSaleDetailModal()}),I()};export{V as afterRender,K as render};
