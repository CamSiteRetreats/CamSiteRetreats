import{S as De,H as je}from"./Header-CtRUqTZE.js";const Ae=()=>`
    ...
    `,Re=async()=>{console.log("[Roster] VERSION 3.2 LOADED - READY TO EDIT");const pe=t=>{const e=(t||"").toLowerCase();return e==="hoàn tất phí"||e.includes("hoàn tất")?`<span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold bg-emerald-100 text-emerald-700 border border-emerald-200">
        ...
        `).join("")},ke=()=>{L.forEach(t=>{const e=_.has(t.id),o=document.querySelector(`th[data-col="${t.id}"]`);o&&(e?o.classList.remove("col-hidden"):o.classList.add("col-hidden")),document.querySelectorAll(`td[data-col="${t.id}"]`).forEach(r=>{e?r.classList.remove("col-hidden"):r.classList.add("col-hidden")})})},z=t=>{if(!t)return null;t.includes(" - ")&&(t=t.split(" - ")[0].trim()),t.includes("T")&&(t=t.split("T")[0]);const e=t.split(/[-/.]/);return e.length===2?{day:parseInt(e[0]),month:parseInt(e[1]),year:null}:e.length===3?e[0].length===4?{day:parseInt(e[2]),month:parseInt(e[1]),year:parseInt(e[0])}:{day:parseInt(e[0]),month:parseInt(e[1]),year:parseInt(e[2])}:null},M=z($),Ee=t=>{if(!M)return!0;const e=z(t);return!(!e||e.day!==M.day||e.month!==M.month||e.year&&M.year&&e.year!==M.year)},K=async()=>{try{if(A)try{const n=await(await fetch(`/api/guides?schedule_id=${A}`)).json();if(n&&n.length>0){const s=document.getElementById("rosterGuidesSection"),d=document.getElementById("rosterGuidesList"),c={"Hướng Dẫn Viên":"bg-blue-100 text-blue-700 border-blue-200",Leader:"bg-purple-100 text-purple-700 border-purple-200",Photo:"bg-pink-100 text-pink-700 border-pink-200",Guider:"bg-cyan-100 text-cyan-700 border-cyan-200","Hậu Cần":"bg-amber-100 text-amber-700 border-amber-200","Học Việc":"bg-gray-100 text-gray-600 border-gray-200"};s.classList.remove("hidden"),d.innerHTML=n.map(a=>`
                            ...
                            `).join("")}}catch(r){console.error("Error loading guides:",r)}const e=await(await fetch("/api/bookings")).json();console.log("[Roster] targetTour:",I,"targetDateStr:",$,"targetParsed:",M),console.log("[Roster] Total bookings:",e.length),e.length>0&&e.filter(n=>n.tour&&n.tour.includes("Liêng")).slice(0,3).forEach(n=>console.log("[Roster] Sample Liêng:",{tour:n.tour,date:n.date,parsed:z(n.date)})),k=e.filter(r=>{if(A){if(String(r.schedule_id)===String(A))return!0;if(!(!ge&&!r.schedule_id))return!1}let n=!0,s=!0;if(I){const d=Z(r.tour),c=Z(I);n=d===c||d.includes(c)||c.includes(d)}return $&&(s=Ee(r.date)),n&&s}),console.log("[Roster] Matched bookings:",k.length);const o=document.getElementById("filterSort")?.value||"newest";o==="newest"?k.sort((r,n)=>parseInt(n.id)-parseInt(r.id)):k.sort((r,n)=>parseInt(r.id)-parseInt(n.id)),console.log(`[Roster] Sorted bookings (${o}):`,k.map(r=>r.id)),N()}catch(t){console.error("Lỗi khi tải dữ liệu đoàn:",t),V.innerHTML='<tr><td colspan="12" class="p-8 text-center text-red-500 font-bold">Lỗi khi tải dữ liệu. Vui lòng tải lại trang.</td></tr>'}},N=()=>{const t=q.value.toLowerCase(),e=P.value.toLowerCase(),o=document.getElementById("filterSearch")?.value?.toLowerCase()||"",r=k.filter(s=>{if(o&&!`${s.name} ${s.phone} ${s.address} ${s.medal_name} ${s.customer_id}`.toLowerCase().includes(o))return!1;const c=(s.status||"").toLowerCase(),a=t.toLowerCase();if(!(t===""||c===a||a==="chờ cọc"&&(c==="chờ cọc"||c==="chờ xác nhận cọc")||a==="đã cọc"&&c==="đã cọc"||a==="đã cọc (chờ đi)"&&c==="đã cọc (chờ đi)"||a==="hoàn tất phí"&&c.includes("hoàn tất")||a==="hoàn thành"&&c==="hoàn thành"||a==="bảo lưu"&&c==="bảo lưu"||a==="đã hủy"&&c.includes("hủy")))return!1;const i=(s.gender||"").toLowerCase();return e===""||i===e}),n=r.filter(s=>s.status!=="Đã hủy"&&s.status!=="Bảo lưu");if(document.getElementById("text-visible-count").textContent=n.length,ve.textContent=n.length,r.length===0){V.innerHTML='<tr><td colspan="12" class="p-12 text-center text-gray-500 bg-gray-50">Không tìm thấy khách hàng nào khớp với điều kiện lọc.</td></tr>';return}V.innerHTML=r.map((s,d)=>{const c=d%2===0,l=s.status==="Đã hủy"||s.status==="Bảo lưu"?"bg-gray-100 opacity-60":c?"bg-white":"bg-gray-50/40";return`<tr onclick="if(!event.target.closest('.seat-badge') && !event.target.closest('a')) { if(window.actionEdit) window.actionEdit('${s.id}'); else alert('Chưa tải xong logic chỉnh sửa!'); }" class="${l} hover:bg-amber-50/40 transition-colors border-b border-gray-100 group row-clickable cursor-pointer" data-booking-id="${s.id}">`+L.map(i=>`
                    <td class="px-4 py-3 align-middle ${i.id==="col-extra"?"min-w-[220px]":""} ${_.has(i.id)?"":"col-hidden"}" data-col="${i.id}">
                        ${i.render(s,d)}
                    </td>
                `).join("")+"</tr>"}).join("")},Se=()=>{const t=L.filter(i=>_.has(i.id)),e=q.value.toLowerCase(),o=P.value.toLowerCase(),r=document.getElementById("filterSearch")?.value?.toLowerCase()||"",n=k.filter(i=>{if(r&&!`${i.name} ${i.phone} ${i.address} ${i.medal_name} ${i.customer_id}`.toLowerCase().includes(r))return!1;const m=(i.status||"").toLowerCase();if(!(e===""||e==="chờ cọc"&&(m.includes("chờ cọc")||m.includes("chờ xác nhận"))||e==="đã cọc"&&(m.includes("đã cọc")||m.includes("hoàn tất")||m.includes("hoàn thành"))||e==="đã hủy"&&m.includes("hủy")))return!1;const h=(i.gender||"").toLowerCase();return o===""||h===o});let s=`
        ...
        `;const d=new Blob(["\uFEFF",s],{type:"application/vnd.ms-excel;charset=utf-8"}),c=URL.createObjectURL(d),a=`DS_Doan_${I||"Tour"}_${j||"Date"}.xls`.replace(/[/\\?%*:|"<>]/g,"-"),l=document.createElement("a");l.setAttribute("href",c),l.setAttribute("download",a),document.body.appendChild(l),l.click(),document.body.removeChild(l)},ee={solati_16:{label:"🚐 Solati 16 chỗ",totalSeats:15},limo_34:{label:"🚌 Limousine 34 chỗ",totalSeats:34},sleeper_45:{label:"🛏️ Giường Nằm 45 chỗ",totalSeats:46}},Ce=t=>{if(t==="limo_34")return{type:"limo",floors:[{label:null,rows:[[{n:"C1"},{n:"C2"},null,{n:"B1"},{n:"B2"},null,{n:"A1"},{n:"A2"},{n:"LÁI",driver:!0}],[{n:"C3"},{n:"C4"},null,{n:"B3"},{n:"B4"},null,{n:"A3"},{n:"A4"}],[{n:"C5"},{n:"C6"},null,{n:"B5"},{n:"B6"},null,{n:"A5"},{n:"A6"}],[{n:"C7"},{n:"C8"},null,{n:"B7"},{n:"B8"},null,{n:"A7"},{n:"A8"}],[{n:"C9"},{n:"C10"},null,{n:"B9"},{n:"B10"},null,{n:"A9"},{n:"A10"}],[{n:"C11"},{n:"C12"},null,null,null,null,{n:"A11"},{n:"A12"}]]}],legend:['<span class="font-bold text-gray-600">Dãy C</span> (cửa lên)','<span class="font-bold text-gray-600">Dãy B</span> (giữa)','<span class="font-bold text-gray-600">Dãy A</span> (tài xế)']};if(t==="sleeper_45"){const e=(o,r,n)=>{const s=Array.from({length:n-r+1},(c,a)=>`${o}${r+a}`),d=[];for(let c=0;c<s.length;c+=4){const a=[];for(let l=0;l<4;l++)l===2&&a.push(null),a.push(s[c+l]?{n:s[c+l]}:null);d.push(a)}return d};return{type:"sleeper",floors:[{label:"🏠 Tầng Dưới",floorKey:"down",rows:e("A",1,23)},{label:"🏥 Tầng Trên",floorKey:"up",rows:e("B",1,23)}],legend:null}}return{type:"solati",floors:[{label:null,rows:[[{n:"LÁI",driver:!0},null,{n:1},{n:2}],[{n:3},{n:4},null,{n:5}],[{n:6},{n:7},null,{n:8}],[{n:9},{n:10},null,{n:11}],[{n:12},{n:13},{n:14},{n:15}]]}],legend:null}},te=ee[Y]||ee.solati_16;let F=0;const R=()=>{const t=document.getElementById("seatMapGrid");if(!t)return;const e={};k.forEach(l=>{const i=l.status==="Đã hủy"||l.status==="Bảo lưu";l.seat_number&&!i&&(e[String(l.seat_number)]=l)});const o=Object.keys(e).length,r=te.totalSeats,n=document.getElementById("seat-map-count"),s=document.getElementById("seat-count-badge"),d=document.getElementById("vehicle-label");n&&(n.textContent=`${o}/${r}`),s&&(s.textContent=`💺 ${o}/${r} ghế đã xếp`),d&&(d.textContent=te.label);const c=Ce(Y),a=(l,i)=>{if(!l)return'<div class="w-12 h-10 flex-shrink-0"></div>';if(l.driver)return'<div class="seat-btn seat-driver w-10 h-9 rounded-lg flex flex-col items-center justify-center text-[8px] font-bold flex-shrink-0"><span>🚗</span><span>LÁI</span></div>';const m=String(l.n),u=i[m],h=u?(u.name||"").split(" ").pop().slice(0,4):"",x=/[A-Z]/.test(m)?"w-11 h-9":"w-12 h-10";return`<button
            ...
            <div class="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Chọn khách khác:</div>`);const c=k.filter(a=>!a.seat_number||String(a.id)===String(e));c.length===0?d+='<p class="text-gray-400 text-sm text-center py-4">Tất cả khách đã được xếp ghế.</p>':d+=c.map(a=>`
                ...
                </button>`).join(""),n.innerHTML=d,o.style.display="flex",n.querySelector("#btnRemoveSeat")?.addEventListener("click",async()=>{await J(e,null),o.style.display="none"}),n.querySelectorAll(".btn-assign-passenger").forEach(a=>{a.addEventListener("click",async()=>{const l=a.dataset.bookingId,i=a.dataset.seat,m=k.find(u=>String(u.seat_number)===String(i)&&String(u.id)!==String(l));m&&await J(m.id,null),await J(l,i),o.style.display="none"})})},J=async(t,e)=>{try{await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:t,seat_number:e||null})});const o=k.find(r=>String(r.id)===String(t));o&&(o.seat_number=e||null),N(),R()}catch(o){console.error("Lỗi lưu ghế:",o),alert("Lỗi lưu số ghế. Vui lòng thử lại.")}};q.addEventListener("change",N),P.addEventListener("change",N),document.getElementById("filterSearch")?.addEventListener("input",N),document.getElementById("filterSort")?.addEventListener("change",K),document.getElementById("btnExportPDF").addEventListener("click",()=>window.print()),document.getElementById("btnExportExcel").addEventListener("click",()=>Se()),document.getElementById("seatModalClose")?.addEventListener("click",()=>{document.getElementById("seatModal").style.display="none"}),document.getElementById("seatModal")?.addEventListener("click",t=>{t.target===document.getElementById("seatModal")&&(document.getElementById("seatModal").style.display="none")}),document.getElementById("rosterList")?.addEventListener("mouseover",t=>{const e=t.target.closest("tr[data-booking-id]");if(!e)return;document.querySelectorAll(".seat-btn").forEach(n=>n.classList.remove("seat-highlighted"));const o=e.dataset.bookingId,r=k.find(n=>String(n.id)===String(o));r?.seat_number&&document.querySelector(`.seat-btn[data-seat="${r.seat_number}"]`)?.classList.add("seat-highlighted")}),document.getElementById("rosterList")?.addEventListener("click",t=>{const e=t.target.closest(".seat-badge");if(!e)return;const o=e.dataset.id,r=k.find(n=>String(n.id)===String(o));ne(r?.seat_number||"?",o)});let H=[],se=[],W=[];const Be=async()=>{try{const[t,e,o]=await Promise.all([fetch("/api/tours"),fetch("/api/schedules"),fetch("/api/users")]);H=t.ok?await t.json():[],se=e.ok?await e.json():[],W=o.ok?await o.json():[],Te("edit-tour"),Ie("edit-sale")}catch(t){console.error("Lỗi tải tours/schedules:",t)}},Ie=t=>{const e=document.getElementById(t);e&&(e.innerHTML='<option value="">-- Website --</option>',W.forEach(o=>{const r=document.createElement("option");r.value=o.id,r.textContent=`${o.full_name||o.fullName} (${o.role})`,e.appendChild(r)}))},Te=t=>{const e=document.getElementById(t);if(!e)return;const o=e.value,r=e.options[0]?.outerHTML||'<option value="">-- Chọn Tour --</option>';e.innerHTML=r,H.filter(n=>n.is_visible!==!1).forEach(n=>{const s=document.createElement("option");s.value=n.name,s.textContent=n.name,n.name===o&&(s.selected=!0),e.appendChild(s)})},oe=(t,e,o,r)=>{const n=document.getElementById(t);if(!n||(n.innerHTML='<option value="">-- Chọn Lịch --</option>',!e))return;const s=se.filter(d=>d.tour_name?d.tour_name===e||e.includes(d.tour_name)||d.tour_name.includes(e):!1);if(s.length===0){n.innerHTML='<option value="">Chưa có lịch cho tour này</option>';return}s.sort((d,c)=>new Date(d.start_date)-new Date(c.start_date)).forEach(d=>{const c=parseInt(d.booked_count)||0,a=(d.slots||0)-c,l=new Date(d.start_date),i=d.end_date?new Date(d.end_date):null,m=y=>`${y.getDate().toString().padStart(2,"0")}/${(y.getMonth()+1).toString().padStart(2,"0")}`;let u=m(l);i&&(u+=" - "+m(i)),d.group_label&&(u+=` · ${d.group_label}`),u+=` (${a>=0?a:0} chỗ trống)`;const h=document.createElement("option"),w=`${l.getDate().toString().padStart(2,"0")}/${(l.getMonth()+1).toString().padStart(2,"0")}/${l.getFullYear()}`;h.value=`${d.id}::${w}`,h.dataset.scheduleId=d.id,h.dataset.date=w,h.textContent=u,a<=0&&(h.disabled=!0,h.textContent=u.replace("chỗ trống","HẾT CHỖ"));let x=o||"";if(x.includes("-")&&x.length>=10){const y=x.substring(0,10).split("-");y.length===3&&(x=`${y[2]}/${y[1]}/${y[0]}`)}(r&&String(d.id)===String(r)||!r&&x&&w===x)&&(h.selected=!0),n.appendChild(h)})},re=document.getElementById("edit-tour");re&&re.addEventListener("change",t=>{oe("edit-date",t.target.value)}),window.closeEditModal=()=>{document.getElementById("editModal").classList.remove("opacity-100");const t=document.getElementById("editModalContent");t&&(t.classList.remove("scale-100","translate-y-0"),t.classList.add("scale-95","translate-y-4")),setTimeout(()=>document.getElementById("editModal").classList.add("hidden"),300)},window.updateEditRemaining=()=>{const t=parseInt(document.getElementById("edit-total")?.value)||0,e=parseInt(document.getElementById("edit-discount")?.value)||0,o=parseInt(document.getElementById("edit-deposit")?.value)||0;let r=0;document.querySelectorAll(".preset-service-check:checked").forEach(l=>{r+=parseInt(l.dataset.price)||0}),document.querySelectorAll(".service-price-input").forEach(l=>{r+=parseInt(l.value)||0});const n=t-e+r,s=n-o,d=document.getElementById("edit-grand-total");d&&(d.textContent=n.toLocaleString("vi-VN")+"đ");const c=document.getElementById("edit-remaining");c&&(c.textContent=s>0?s.toLocaleString("vi-VN")+"đ":"0đ");const a=document.getElementById("edit-status");if(a){const l=a.value;["Hoàn thành","Đã hủy","Bảo lưu"].includes(l)||(s<=0&&n>0?a.value="Hoàn tất phí":s>0&&o>0?["Chờ tư vấn","Chờ cọc","Chờ xác nhận cọc"].includes(l)&&(a.value="Đã cọc"):o===0&&n>0&&["Đã cọc","Hoàn tất phí","Đã cọc (Chờ đi)"].includes(l)&&(a.value="Chờ xác nhận cọc"))}},window.updateServicesTotal=()=>{let t=0;document.querySelectorAll(".preset-service-check:checked").forEach(r=>{t+=parseInt(r.dataset.price)||0}),document.querySelectorAll(".service-price-input").forEach(r=>{t+=parseInt(r.value)||0});const e=document.getElementById("servicesTotalDisplay");e&&(e.textContent=t.toLocaleString("vi-VN")+"đ");const o=document.getElementById("servicesTotalRow");return o&&o.classList.toggle("hidden",t===0),window.updateEditRemaining(),t},window.addServiceRow=(t="",e=0)=>{const o=document.getElementById("servicesContainer"),r=document.getElementById("emptyServicesMsg"),n=document.getElementById("servicesTotalRow");if(!o)return;r&&r.classList.add("hidden"),n&&n.classList.remove("hidden");const s=document.createElement("div");s.className="service-row flex items-center gap-2 bg-gray-50 border border-gray-100 rounded-xl px-3 py-2.5",s.innerHTML=`
