import{S as gt,H as pt}from"./Header-n5pNPUhI.js";console.log("Bookings V3 - Cache Busted!");const bt=()=>`
    ...
    `,ft=()=>{let _=[],y="consult",H=[],F=[],G=[];const dt=async()=>{try{const[n,t,o]=await Promise.all([fetch("/api/tours"),fetch("/api/schedules"),fetch("/api/users")]);H=n.ok?await n.json():[],F=t.ok?await t.json():[],G=o.ok?await o.json():[],U("addTourName"),U("edit-tour"),rt("edit-sale")}catch(n){console.error("Lỗi tải tours/schedules:",n)}},rt=n=>{const t=document.getElementById(n);t&&(t.innerHTML='<option value="">-- Website --</option>',G.forEach(o=>{const l=document.createElement("option");l.value=o.id,l.textContent=`${o.full_name||o.fullName} (${o.role})`,t.appendChild(l)}))},U=n=>{const t=document.getElementById(n);if(!t)return;const o=t.value,l=t.options[0].outerHTML;t.innerHTML=l,H.filter(d=>d.is_visible!==!1).forEach(d=>{const s=document.createElement("option");s.value=d.name,s.textContent=d.name,d.name===o&&(s.selected=!0),t.appendChild(s)})},K=(n,t,o)=>{const l=document.getElementById(n);if(!l)return;if(l.innerHTML='<option value="">-- Chọn Lịch --</option>',!t){l.innerHTML='<option value="">-- Chọn Tour trước --</option>';return}const d=F.filter(s=>s.tour_name===t||t.includes(s.tour_name)||s.tour_name.includes(t));if(d.length===0){l.innerHTML='<option value="">Chưa có lịch cho tour này</option>';return}d.sort((s,r)=>new Date(s.start_date)-new Date(r.start_date)).forEach(s=>{const r=parseInt(s.booked_count)||0,a=(s.slots||0)-r,u=new Date(s.start_date),m=s.end_date?new Date(s.end_date):null,p=b=>`${b.getDate().toString().padStart(2,"0")}/${(b.getMonth()+1).toString().padStart(2,"0")}`;let f=p(u);m&&(f+=" - "+p(m)),f+=` (${a>=0?a:0} chỗ trống)`;const i=document.createElement("option");i.value=`${u.getDate().toString().padStart(2,"0")}/${(u.getMonth()+1).toString().padStart(2,"0")}/${u.getFullYear()}`,i.textContent=f,a<=0&&(i.disabled=!0,i.textContent=f.replace("chỗ trống","HẾT CHỖ")),o&&i.value===o&&(i.selected=!0),l.appendChild(i)})},W=document.getElementById("addTourName");W&&W.addEventListener("change",n=>{const t=n.target.value;K("addTourDate",t);const o=H.find(d=>d.name===t),l=document.getElementById("addTourPrice");if(o&&l){const d=parseInt(o.price)||0;l.value=d;const s=document.getElementById("addDiscount"),r=document.getElementById("addTotalPriceDisplay"),a=parseInt(s?s.value:0)||0,u=d-a;r&&(r.textContent=(u>0?u:0).toLocaleString("vi-VN")+"đ")}});const J=document.getElementById("edit-tour");J&&J.addEventListener("change",n=>{const t=n.target.value;K("edit-date",t);const o=H.find(d=>d.name===t),l=document.getElementById("edit-total");o&&l&&(parseInt(l.value)===0||!l.value)&&(l.value=parseInt(o.price)||0,typeof window.updateEditRemaining=="function"&&window.updateEditRemaining())}),dt();const D=()=>{const n=document.getElementById("bookingsTableHead"),t=document.getElementById("bookingsTableBody");if(!t||!n)return;const o=y==="upcoming"||y==="ready"||y==="completed";o?n.innerHTML=`
            ...
            `;const l=document.getElementById("filterSearch")?document.getElementById("filterSearch").value.toLowerCase():"",d=document.getElementById("filterTour")?document.getElementById("filterTour").value:"",s=document.getElementById("filterDate")?document.getElementById("filterDate").value:"",r=document.getElementById("filterSale")?document.getElementById("filterSale").value:"",a=document.getElementById("filterStatus")?document.getElementById("filterStatus").value:"";let u=_.filter(e=>{let g=!1;const c=parseInt(e.total_price)>0&&parseInt(e.total_price)===parseInt(e.deposit),x=C=>{if(!C)return!1;try{const w=C.split("/");if(w.length===3){const h=new Date(`${w[2]}-${w[1]}-${w[0]}T00:00:00`),T=new Date;return T.setHours(0,0,0,0),h.getTime()<T.getTime()}else if(C.includes("-")){const h=new Date(`${C}T00:00:00`),T=new Date;return T.setHours(0,0,0,0),h.getTime()<T.getTime()}}catch{}return!1},k=(e.status==="Hoàn thành"||e.status==="Đã đi"||c)&&x(e.date);return y==="consult"?g=e.status==="Chờ tư vấn"&&!k:y==="pending"?g=(!e.status||e.status==="Chờ cọc"||e.status==="Chờ xác nhận cọc")&&!k:y==="upcoming"?g=e.status&&e.status!=="Chờ tư vấn"&&e.status!=="Chờ cọc"&&e.status!=="Chờ xác nhận cọc"&&!c&&e.status!=="Hoàn thành"&&!k:y==="ready"?g=(c||e.status==="Hoàn thành")&&!k:y==="completed"&&(g=k),!(!g||d&&e.tour!==d||s&&O(e.date)!==s||r&&r!=="null"&&e.sale_name!==r||r==="null"&&e.sale_name&&e.sale_name!=="Website"||l&&!`${e.name||""} ${e.phone||""} ${e.customer_id||""}`.toLowerCase().includes(l)||a&&e.status!==a)});const m=document.getElementById("statTitle1"),p=document.getElementById("statTitle2"),f=document.getElementById("statTitle3"),i=document.getElementById("statTotalCustomers"),b=document.getElementById("statTotalRevenue"),v=document.getElementById("statTotalCollected");let I=u.length;if(y==="consult"){m&&(m.textContent="Tổng Leads Tư Vấn"),p&&(p.textContent="Chưa Có Sale Nhận"),f&&(f.textContent="Đã Có Sale Nhận");let e=0,g=0;u.forEach(c=>{!c.sale_id||c.sale_id==="null"||!c.sale_name||c.sale_name==="Website"||c.sale_name==="null"?e++:g++}),i&&(i.textContent=I),b&&(b.textContent=e+" Đơn"),v&&(v.textContent=g+" Đơn")}else if(y==="pending"){m&&(m.textContent="Tổng Khách (Chờ Cọc)"),p&&(p.textContent="Khách Chưa Tư Vấn (Mới)"),f&&(f.textContent="Đã Có Sale Giữ Chỗ");let e=0,g=0;u.forEach(c=>{!c.sale_id||c.sale_id==="null"||!c.sale_name||c.sale_name==="Website"||c.sale_name==="null"?e++:g++}),i&&(i.textContent=I),b&&(b.textContent=e+" Đơn"),v&&(v.textContent=g+" Đơn")}else if(y==="upcoming"){m&&(m.textContent="Tổng Số Khách"),p&&(p.textContent="Chưa Xác Nhận Cọc"),f&&(f.textContent="Chờ Thanh Toán Còn Lại");let e=0,g=0;u.forEach(c=>{c.status==="Chờ xác nhận cọc"?e++:parseInt(c.deposit)>0&&parseInt(c.total_price)>parseInt(c.deposit)&&g++}),i&&(i.textContent=I),b&&(b.textContent=e+" Khách"),v&&(v.textContent=g+" Khách")}else if(y==="ready"){m&&(m.textContent="Khách Sẵn Sàng (Full)"),p&&(p.textContent="Tổng Doanh Thu Tab"),f&&(f.textContent="Thực Thu (Full Tận Nơi)");let e=0,g=0;u.forEach(c=>{e+=parseInt(c.total_price)||0,g+=parseInt(c.deposit)||0}),i&&(i.textContent=I),b&&(b.textContent=e>0?e.toLocaleString("vi-VN")+"đ":"0đ"),v&&(v.textContent=g>0?g.toLocaleString("vi-VN")+"đ":"0đ")}else{m&&(m.textContent="Khách Đã Xong (Lịch Sử)"),p&&(p.textContent="Tổng Doanh Thu Tab"),f&&(f.textContent="Thực Thu (Đã Đóng)");let e=0,g=0;u.forEach(c=>{e+=parseInt(c.total_price)||0,g+=parseInt(c.deposit)||0}),i&&(i.textContent=I),b&&(b.textContent=e>0?e.toLocaleString("vi-VN")+"đ":"0đ"),v&&(v.textContent=g>0?g.toLocaleString("vi-VN")+"đ":"0đ")}const E=o?8:4;if(!u||u.length===0){let e=y==="consult"?"Chưa có khách hàng tư vấn nào.":y==="pending"?"Chưa có Khách chờ cọc.":y==="upcoming"?"Chưa có Khách nào Đã Cọc.":y==="ready"?"Chưa có khách sẵn sàng.":"Danh sách rỗng.";t.innerHTML=`<tr><td colspan="${E}" class="p-8 text-center text-gray-500">${e}</td></tr>`;return}t.innerHTML=u.map(e=>{const g=y==="upcoming"||y==="ready"?"bg-green-50/20 hover:bg-green-50 transition-colors":"hover:bg-gray-100 transition-colors";if(o){const c=parseInt(e.total_price)||0,x=parseInt(e.deposit)||0,k=c-x,C=c>0?c.toLocaleString("vi-VN")+"đ":"Chưa định giá",w=x>0?x.toLocaleString("vi-VN")+"đ":"0đ",h=k>0?k.toLocaleString("vi-VN")+"đ":k===0&&c>0?"Đã thu trọn":"-";let T="";e.status==="Hoàn thành"||e.status==="Đã đi"?T='<span class="bg-gray-100 text-gray-600 border border-gray-200 px-2 py-0.5 rounded text-xs block w-full text-center">Hoàn thành</span>':e.status==="Chờ xác nhận cọc"?T=`<button class="action-btn confirm-deposit-btn bg-csr-orange text-white px-3 py-1.5 rounded text-xs font-bold shadow-sm hover:bg-[#d65503] w-full" data-id="${e.id}">Xác nhận cọc</button>`:x>0&&k===0?T='<span class="bg-green-100 text-green-700 border border-green-200 px-2 py-0.5 rounded text-xs font-medium block w-full text-center">Hoàn tất phí</span>':T='<span class="bg-blue-100 text-blue-700 border border-blue-200 px-2 py-0.5 rounded text-xs font-medium block w-full text-center">Đã Cọc</span>';let M="";return x>0&&(M=`<a href="/invoice.html?id=${e.id}" target="_blank" class="mt-1.5 block text-center text-[11px] text-blue-600 hover:text-blue-800 hover:underline font-medium" onclick="event.stopPropagation()">🧾 Xem hóa đơn</a>`),`
                <tr class="${g} cursor-pointer row-clickable" data-id="${e.id}">
                    ...
                    <td class="p-4 align-top text-center">${T}${M}</td>
                    ...
                    <td class="p-4 align-top text-right text-sm font-bold text-red-500">${h}</td>
                ...
                `}else{const c=e.name?e.name.substring(0,2).toUpperCase():"KH",x=y==="consult",k=!e.sale_id||e.sale_id==="null"||!e.sale_name||e.sale_name==="null";let C="";x&&k?C=`<button class="action-btn claim-btn bg-csr-orange hover:bg-[#d65503] text-white px-3 py-1.5 rounded text-xs font-bold shadow-sm transition-colors" data-id="${e.id}">Nhận khách</button>`:C=`<span class="text-sm text-gray-500">${e.sale_name||"Website"}</span>`;const w=`
    ...
    `}}).join(""),document.querySelectorAll(".claim-btn").forEach(e=>{e.addEventListener("click",async g=>{g.stopPropagation();const c=e.getAttribute("data-id"),x=localStorage.getItem("csr_user");let k=null,C="Admin";if(x){const w=JSON.parse(x);k=w.id,C=w.fullName||w.full_name||"Admin"}e.textContent="Đang xử lý...",e.disabled=!0;try{if((await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:c,sale_id:k,sale_name:C})})).ok){const h=_.find(T=>T.id==c);h&&(h.sale_id=k,h.sale_name=C),D()}else alert("Lỗi khi nhận khách. Vui lòng thử lại."),e.textContent="Nhận khách",e.disabled=!1}catch(w){console.error("Lỗi nhận khách:",w),e.textContent="Nhận khách",e.disabled=!1}})})},ct=()=>{const n=new Set,t=new Set,o=new Set;_.forEach(r=>{if(r.tour&&n.add(r.tour),r.date){const a=O(r.date);a&&t.add(a)}r.sale_name&&r.sale_name!=="Website"&&r.sale_name!=="null"&&o.add(r.sale_name.trim())});const l=document.getElementById("filterTour"),d=document.getElementById("filterDate"),s=document.getElementById("filterSale");if(l){const r=l.value;l.innerHTML='<option value="">Tất cả Tour</option>',[...n].sort().forEach(a=>l.innerHTML+=`<option value="${a}">${a}</option>`),l.value=r}if(d){const r=d.value;d.innerHTML='<option value="">Tất cả Ngày</option>',[...t].sort().forEach(a=>d.innerHTML+=`<option value="${a}">${a}</option>`),d.value=r}if(s){const r=s.value;s.innerHTML='<option value="">Tất cả Sale</option>',s.innerHTML+='<option value="null">Nguồn Website</option>',[...o].sort().forEach(a=>{s.innerHTML+=`<option value="${a}">${a}</option>`}),s.value=r}};function O(n){if(!n)return"";if(/^\d{1,2}\/\d{1,2}\s*-\s*\d{1,2}\/\d{1,2}$/.test(n))return n.replace(/\s+/g,"");try{if(/^\d{4}-\d{2}-\d{2}$/.test(n)){const[t,o,l]=n.split("-"),d=`${parseInt(l)}/${parseInt(o)}`;return`${d}-${d}`}if(/^\d{1,2}\/\d{1,2}\/\d{4}$/.test(n)){const[t,o,l]=n.split("/"),d=`${parseInt(t)}/${parseInt(o)}`;return`${d}-${d}`}if(n.includes("-"))return n.split("-").map(o=>{const l=o.trim();if(/\/\d{4}$/.test(l)){const[d,s]=l.split("/");return`${parseInt(d)}/${parseInt(s)}`}return l}).join("-")}catch{console.warn("Date normalization failed for:",n)}return n}const S=async()=>{try{_=await(await fetch("/api/bookings")).json(),ct(),D()}catch(n){console.error("Lỗi parse list đơn hàng:",n);const t=document.getElementById("bookingsTableBody");t&&(t.innerHTML='<tr><td colspan="4" class="p-8 text-center text-red-500">Lỗi lấy dữ liệu từ Server.</td></tr>')}};S(),["filterSearch","filterTour","filterDate","filterSale","filterStatus"].forEach(n=>{const t=document.getElementById(n);t&&(n==="filterSearch"?t.addEventListener("input",D):t.addEventListener("change",D))});const X=document.getElementById("exportExcelBtn");X&&X.addEventListener("click",()=>{const n=document.getElementById("filterSearch")?document.getElementById("filterSearch").value.toLowerCase():"",t=document.getElementById("filterTour")?document.getElementById("filterTour").value:"",o=document.getElementById("filterDate")?document.getElementById("filterDate").value:"",l=document.getElementById("filterSale")?document.getElementById("filterSale").value:"";let d=_.filter(i=>{let b=!1;const v=parseInt(i.total_price)>0&&parseInt(i.total_price)===parseInt(i.deposit),I=e=>{if(!e)return!1;try{const g=e.split("/");if(g.length===3){const c=new Date(`${g[2]}-${g[1]}-${g[0]}T00:00:00`),x=new Date;return x.setHours(0,0,0,0),c.getTime()<x.getTime()}else if(e.includes("-")){const c=new Date(`${e}T00:00:00`),x=new Date;return x.setHours(0,0,0,0),c.getTime()<x.getTime()}}catch{}return!1},E=(i.status==="Hoàn thành"||i.status==="Đã đi"||v)&&I(i.date);return y==="consult"?b=i.status==="Chờ tư vấn"&&!E:y==="pending"?b=(!i.status||i.status==="Chờ cọc")&&!E:y==="upcoming"?b=i.status&&i.status!=="Chờ tư vấn"&&i.status!=="Chờ cọc"&&!v&&i.status!=="Hoàn thành"&&!E:y==="ready"?b=(v||i.status==="Hoàn thành")&&!E:y==="completed"&&(b=E),!(!b||t&&i.tour!==t||o&&O(i.date)!==o||l&&l!=="null"&&i.sale_name!==l||l==="null"&&i.sale_name&&i.sale_name!=="Website"||n&&!`${i.name||""} ${i.phone||""} ${i.customer_id||""} `.toLowerCase().includes(n))});if(d.length===0){alert("Danh sách rỗng. Không có dữ liệu để xuất.");return}const r=[["Mã KH (CRM)","Họ và Tên","Số điện thoại","Tên Tour","Lịch trình","Trạng thái","Nguồn / Sale","Đã cọc","Tổng tiền","Ghi chú"]];d.forEach(i=>{r.push([i.customer_id||"",`"${i.name||""}"`,`= "${i.phone||""}"`,`"${i.tour||""}"`,`"${i.date||""}"`,`"${i.status||""}"`,`"${i.sale_name||"Website"}"`,i.deposit||0,i.total_price||0,`"${(i.special||"").replace(/"/g,'""')}"`])});const a="\\uFEFF"+r.map(i=>i.join(",")).join(`
`),u=new Blob([a],{type:"text/csv;charset=utf-8;"}),m=URL.createObjectURL(u),p=document.createElement("a");p.setAttribute("href",m);let f=y==="pending"?"ChoCoc":y==="upcoming"?"SapThamGia":y==="ready"?"ChoLenXe":"LichSu";p.setAttribute("download",`BaoCao_DonHang_${f}_${new Date().toISOString().slice(0,10)}.csv`),document.body.appendChild(p),p.click(),document.body.removeChild(p)});const Y=document.querySelectorAll(".tab-btn");Y.forEach(n=>{n.addEventListener("click",t=>{Y.forEach(o=>{o.classList.remove("border-csr-orange","text-csr-orange"),o.classList.add("border-transparent","text-gray-500")}),t.target.classList.add("border-csr-orange","text-csr-orange"),t.target.classList.remove("border-transparent","text-gray-500"),y=t.target.getAttribute("data-tab"),D()})});const $=document.getElementById("bookingModal"),N=document.getElementById("bookingModalContent"),z=document.getElementById("detailModal"),Q=document.getElementById("detailModalContent"),A=document.getElementById("editModal"),q=document.getElementById("editModalContent"),Z=document.getElementById("addBookingBtn");$&&($.classList.add("duration-200","ease-out"),N.classList.add("duration-300","ease-out")),A&&(A.classList.add("duration-200","ease-out"),q.classList.add("duration-300","ease-out")),Z&&Z.addEventListener("click",()=>{document.getElementById("bookingModalContent").querySelector("h2").innerText="Thêm Khách Hàng (Tạo Đơn)",document.getElementById("submitBookingBtn").innerText="Tạo Phơi Đăng Ký",document.getElementById("editingBookingId").value="",document.getElementById("bookingForm").reset();const n=document.getElementById("loyalty-alert");n&&n.remove(),$.classList.remove("hidden"),setTimeout(()=>{$.classList.add("opacity-100"),N.classList.remove("scale-95","translate-y-4"),N.classList.add("scale-100","translate-y-0")},10)}),window.closeModal=()=>{$&&($.classList.remove("opacity-100"),N.classList.remove("scale-100","translate-y-0"),N.classList.add("scale-95","translate-y-4"),setTimeout(()=>{$.classList.add("hidden")},200))},window.closeDetailModal=()=>{z&&(z.classList.remove("opacity-100"),Q.classList.remove("scale-100","translate-y-0"),Q.classList.add("scale-95","translate-y-4"),setTimeout(()=>{z.classList.add("hidden")},200))},window.closeEditModal=()=>{A&&(A.classList.remove("opacity-100"),q.classList.remove("scale-100","translate-y-0"),q.classList.add("scale-95","translate-y-4"),setTimeout(()=>{A.classList.add("hidden")},200))},window.openRowActionModal=n=>{document.getElementById("ramName").textContent=n.name||"Khách Hàng Này",document.getElementById("ramPhone").textContent=n.customer_id?`🥇 ${n.customer_id.toUpperCase()}`:n.phone||"";const t=`
        ...
        `;document.getElementById("detailContentBlock").innerHTML=d,document.getElementById("detailModal").classList.remove("hidden"),setTimeout(()=>{document.getElementById("detailModal").classList.add("opacity-100");const s=document.getElementById("detailModalContent");s&&(s.classList.remove("scale-95","translate-y-4"),s.classList.add("scale-100","translate-y-0"))},10)},window.actionEdit=async n=>{document.getElementById("rowActionModal").classList.add("hidden");const t=_.find(a=>a.id==n);if(!t)return;document.getElementById("edit-id").value=n,document.getElementById("edit-name").value=t.name||"",document.getElementById("edit-phone").value=t.phone||"",document.getElementById("edit-medal-name").value=t.medal_name||"",document.getElementById("edit-tour").value=t.tour||"",typeof K=="function"&&K("edit-date",t.tour||"",t.date||""),(a=>{let u=a.dob||"";if(typeof u=="string"&&u.includes("/")){const m=u.split("/");m.length===3&&(u=`${m[2]}-${m[1].padStart(2,"0")}-${m[0].padStart(2,"0")}`)}document.getElementById("edit-dob").value=u,document.getElementById("edit-gender").value=a.gender||"Khác",document.getElementById("edit-allergy").value=a.allergy||a.medical_notes||"",document.getElementById("edit-address").value=a.address||"",document.getElementById("edit-diet").value=a.diet||a.dietary||"Không",document.getElementById("edit-trekking-pole").value=a.trekking_pole||"Không"})(t),document.getElementById("edit-status").value=t.status||"Chờ xác nhận cọc",document.getElementById("edit-commitments").checked=!!t.commitments,document.getElementById("edit-special").value=t.special||"";try{const a=localStorage.getItem("csr_user");if(a&&JSON.parse(a).role==="admin"){const m=document.getElementById("edit-sale-container");m&&m.classList.remove("hidden");const p=document.getElementById("edit-sale");p&&(p.value=t.sale_id||"")}}catch{}if((!t.dob||!t.address||!t.gender||t.gender==="Khác")&&(t.customer_id||t.phone))try{const a=t.customer_id||t.phone,u=await fetch(`/api/admin_customers?action=search&keyword=${encodeURIComponent(a)}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({keyword:a})}),m=await u.json();if(u.ok&&m.success){const p=m.data,f=document.getElementById("edit-dob"),i=document.getElementById("edit-address"),b=document.getElementById("edit-allergy"),v=document.getElementById("edit-diet"),I=document.getElementById("edit-gender"),E=document.getElementById("edit-medal-name");f&&!f.value&&(f.value=p.dob||""),i&&!i.value&&(i.value=p.address||""),b&&!b.value&&(b.value=p.medical_notes||""),v&&(v.value==="Không"||!v.value)&&(v.value=p.dietary||"Không"),I&&(I.value==="Khác"||!I.value)&&(I.value=p.gender||"Khác"),E&&!E.value&&(E.value=p.medal_name||"")}}catch(a){console.error("Pull CRM error:",a)}let d=(parseInt(t.total_price)||0)+(parseInt(t.discount)||0);if(d===0&&t.tour){const a=H.find(u=>u.name===t.tour);a&&parseInt(a.price)>0&&(d=parseInt(a.price))}document.getElementById("edit-total").value=d,document.getElementById("edit-discount").value=t.discount||0,document.getElementById("edit-deposit").value=t.deposit||0,document.getElementById("edit-deposit-required").value=t.deposit_required||1e6,window.updateEditRemaining();const s=document.getElementById("editModal"),r=document.getElementById("editModalContent");s&&r&&(s.classList.remove("hidden"),setTimeout(()=>{s.classList.add("opacity-100"),r.classList.remove("scale-95","translate-y-4"),r.classList.add("scale-100","translate-y-0")},10))},window.actionDelete=async n=>{if(document.getElementById("rowActionModal").classList.add("hidden"),confirm("Bạn có chắc chắn muốn xóa Đơn hàng này? Thao tác không thể hoàn tác."))try{if((await fetch(`/api/bookings?id=${n}`,{method:"DELETE"})).ok)alert("✅ Đã xóa đơn hàng thành công!"),S();else throw new Error("Lỗi từ Server")}catch(t){alert("❌ Không thể xóa bảng ghi này: "+t.message)}},window.updateEditRemaining=()=>{const n=parseInt(document.getElementById("edit-total").value)||0,t=parseInt(document.getElementById("edit-discount").value)||0,o=parseInt(document.getElementById("edit-deposit").value)||0,d=n-t-o,s=document.getElementById("edit-remaining");s&&(s.textContent=d>0?d.toLocaleString("vi-VN")+"đ":"0đ")};const tt=document.getElementById("edit-total"),et=document.getElementById("edit-deposit");tt&&tt.addEventListener("input",window.updateEditRemaining),et&&et.addEventListener("input",window.updateEditRemaining),document.querySelectorAll('#bookingModal button[onclick*="hidden"]').forEach(n=>{n.removeAttribute("onclick"),n.addEventListener("click",window.closeModal)});const nt=async n=>{try{const o=await(await fetch("/api/admin_customers?action=create",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({full_name:n.name,phone:n.phone,cccd:n.id_card||"",dob:n.dob||null,gender:n.gender||"Khác",medical_notes:n.allergy||"",dietary:n.diet||""})})).json();o.success&&o.csr_code&&(console.log("✅ Đã đồng bộ CRM thành công:",n.name,o.csr_code),(await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:n.id,customer_id:o.csr_code})})).ok&&(console.log("✅ Đã cập nhật Booking mã CRM"),typeof S=="function"&&S()))}catch(t){console.error("❌ Lỗi đồng bộ CRM:",t)}},ot=document.getElementById("bookingsTableBody");ot&&ot.addEventListener("click",async n=>{const t=n.target.closest(".action-btn");if(!t){const s=n.target.closest(".row-clickable");if(s){const r=s.getAttribute("data-id"),a=_.find(u=>u.id==r);a&&openRowActionModal(a)}return}const o=t.getAttribute("data-id"),l=_.find(s=>s.id==o);if(!l)return;const d=document.getElementById("rowActionModal");if(d&&d.classList.add("hidden"),t.classList.contains("confirm-deposit-btn")){if(confirm("Xác nhận khách hàng này đã chuyển khoản đặt cọc?"))try{if(t.disabled=!0,t.classList.add("opacity-50"),(await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:o,status:"Đã cọc"})})).ok)alert("✅ Đã xác nhận cọc thành công!"),nt(l),typeof S=="function"&&S();else throw new Error("Lỗi cập nhật API")}catch(s){alert("❌ Lỗi: "+s.message)}finally{t.disabled=!1,t.classList.remove("opacity-50")}return}if(t.classList.contains("delete-btn")){window.actionDelete(o);return}else if(t.classList.contains("process-btn")){const a=(window.location.hostname==="localhost"||window.location.hostname==="127.0.0.1"?"http://localhost:8888":window.location.origin)+`/booking/process.html?id=${o}`;navigator.clipboard.writeText(a).then(()=>{alert("📋 Đã sao chép Link Điền Thông Tin!\\nGửi cho khách: "+a)}).catch(u=>{alert("Lỗi khi Copy Clipboard. Link là: "+a)})}else if(t.classList.contains("pay-terms-btn")){const a=(window.location.hostname==="localhost"||window.location.hostname==="127.0.0.1"?"http://localhost:8888":window.location.origin)+`/pay-terms.html?id=${o}`;navigator.clipboard.writeText(a).then(()=>{alert("📋 Đã sao chép Link Cọc Tour!\\nGửi cho khách: "+a)}).catch(u=>{alert("Lỗi khi Copy Clipboard. Link là: "+a)})}else if(t.classList.contains("send-btn")){const a=(window.location.hostname==="localhost"||window.location.hostname==="127.0.0.1"?"http://localhost:8888":window.location.origin)+`/pay?id=${o}`;navigator.clipboard.writeText(a).then(()=>{alert("📋 Đã sao chép Link Thanh Toán Cọc! Gửi cho khách qua Zalo nhé:\\n"+a)}).catch(u=>{alert("Lỗi khi Copy Clipboard. Link là: "+a)})}else if(t.classList.contains("payment-btn")){const a=(window.location.hostname==="localhost"||window.location.hostname==="127.0.0.1"?"http://localhost:8888":window.location.origin)+`/pay?id=${o}`;window.open(a,"_blank")}else t.classList.contains("view-btn")?(document.getElementById("rowActionModal").classList.add("hidden"),window.actionView(o)):t.classList.contains("edit-btn")&&window.actionEdit(o)});const j=document.getElementById("smartSearch"),L=document.getElementById("searchSuggestions");if(j){let n;j.addEventListener("input",t=>{const o=t.target.value.toLowerCase().trim();if(clearTimeout(n),o.length<2){L&&L.classList.add("hidden");return}n=setTimeout(async()=>{try{const l=await fetch(`/api/admin_customers?action=search&keyword=${o}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({keyword:o})}),d=await l.json();if(l.ok&&d.success){const r=[d.data];L&&(L.innerHTML=r.map(a=>`
                        ...
                        `).join(""),L.classList.remove("hidden"))}else L&&(L.innerHTML='<div class="p-3 text-xs text-gray-400 italic text-center">Không tìm thấy khách hàng cũ</div>',L.classList.remove("hidden"))}catch(l){console.error("Search error:",l)}},300)}),L&&L.addEventListener("click",t=>{const o=t.target.closest(".suggestion-item");if(o){const l=document.getElementById("addFullName"),d=document.getElementById("addPhone"),s=document.getElementById("addCsrCode");l&&(l.value=o.getAttribute("data-name")),d&&(d.value=o.getAttribute("data-phone")),s&&(s.value=o.getAttribute("data-csr"));const r=document.getElementById("addDob"),a=document.getElementById("addGender"),u=document.getElementById("addIdCard"),m=document.getElementById("addAddress");r&&(r.value=o.getAttribute("data-dob")||""),a&&(a.value=o.getAttribute("data-gender")||"Khác"),u&&(u.value=o.getAttribute("data-cccd")||""),m&&(m.value=o.getAttribute("data-address")||"");const p=document.getElementById("addAllergy"),f=document.getElementById("addDiet"),i=document.getElementById("addTrekkingPole"),b=document.getElementById("addMedalName");p&&(p.value=o.getAttribute("data-allergy")||""),f&&(f.value=o.getAttribute("data-diet")||""),i&&(i.value=o.getAttribute("data-trekking_pole")||"Không"),b&&(b.value=o.getAttribute("data-medal-name")||""),window._selectedCustomer={fullName:o.getAttribute("data-name"),phone:o.getAttribute("data-phone"),csrCode:o.getAttribute("data-csr"),dob:o.getAttribute("data-dob")||"",gender:o.getAttribute("data-gender")||"",address:o.getAttribute("data-address")||"",id_card:o.getAttribute("data-cccd")||"",allergy:o.getAttribute("data-allergy")||"",diet:o.getAttribute("data-diet")||"",trekking_pole:o.getAttribute("data-trekking_pole")||"",medal_name:o.getAttribute("data-medal-name")||""},j.value=o.getAttribute("data-name"),L.classList.add("hidden");const v=`
                    ...
                    </div>`,I=document.getElementById("loyalty-alert");I&&I.remove(),j.parentElement.insertAdjacentHTML("afterend",v)}}),document.addEventListener("click",t=>{!j.contains(t.target)&&L&&!L.contains(t.target)&&L.classList.add("hidden")})}const P=document.getElementById("addTourPrice"),R=document.getElementById("addDiscount"),at=document.getElementById("addTotalPriceDisplay");document.getElementById("addTourName");const st=()=>{if(!P||!R||!at)return;const n=parseInt(P.value)||0,t=parseInt(R.value)||0,o=n-t;at.textContent=(o>0?o:0).toLocaleString("vi-VN")+"đ"};P&&P.addEventListener("input",st),R&&R.addEventListener("input",st);const it=document.getElementById("searchBtn");it&&it.addEventListener("click",n=>{n.preventDefault(),document.getElementById("smartSearch").value||alert("Vui lòng nhập Số Điện Thoại hoặc Mã #CSR")});const V=document.getElementById("bookingForm");V&&V.addEventListener("submit",async n=>{n.preventDefault();const t=n.target.querySelector('button[type="submit"]'),o=t.textContent;t.textContent="Đang lưu hệ thống...",t.disabled=!0;try{const l=document.getElementById("addFullName").value,d=document.getElementById("addPhone").value,s=document.getElementById("addTourName").value,r=document.getElementById("addTourDate").value,a=document.getElementById("addDob").value,u=document.getElementById("addGender").value,m=document.getElementById("addIdCard").value,p=document.getElementById("addAddress").value,f=document.getElementById("addSpecial").value,i=document.getElementById("addAllergy").value,b=document.getElementById("addDiet").value,v=document.getElementById("addTrekkingPole").value,I=document.getElementById("addMedalName").value,E=parseInt(document.getElementById("addTourPrice").value)||0,e=parseInt(document.getElementById("addDiscount").value)||0,g=parseInt(document.getElementById("addDepositRequired").value)||1e6,c=document.getElementById("addCsrCode").value||"",x=localStorage.getItem("csr_user");let k=null,C="Admin";if(x){const B=JSON.parse(x);k=B.id,C=B.fullName||B.full_name||"Admin"}const w=document.getElementById("editingBookingId")?document.getElementById("editingBookingId").value:"",h={name:l,phone:d,tour:s,date:r,status:"Chờ cọc",total_price:E-e,discount:e,deposit:0,sale_id:k,sale_name:C,customer_id:c,deposit_required:g,commitments:!0,dob:a,gender:u,address:p,id_card:m,special:f,allergy:i,diet:b,trekking_pole:v,medal_name:I},T=d.replace(/[^0-9]/g,"");window._selectedCustomer&&(window._selectedCustomer.phone||"").replace(/[^0-9]/g,"")===T&&(h.dob=h.dob||window._selectedCustomer.dob,h.gender=h.gender&&h.gender!=="Khác"?h.gender:window._selectedCustomer.gender||"Khác",h.address=h.address||window._selectedCustomer.address,h.id_card=h.id_card||window._selectedCustomer.id_card,h.allergy=window._selectedCustomer.allergy,h.diet=window._selectedCustomer.diet,h.trekking_pole=window._selectedCustomer.trekking_pole,h.medal_name=window._selectedCustomer.medal_name),w&&(h.id=w,delete h.status,delete h.deposit,delete h.sale_id,delete h.sale_name);const M=await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(h)});if(M.ok){alert(w?"✅ Đã lưu Cập nhật Đơn hàng!":"✅ Thêm Đơn Hàng Thành Công!"),window.closeModal(),V&&V.reset(),window._selectedCustomer=null;const B=document.getElementById("loyalty-alert");B&&B.remove(),document.getElementById("addTotalPriceDisplay")&&(document.getElementById("addTotalPriceDisplay").textContent="0đ"),typeof S=="function"?S():window.location.reload()}else{const B=await M.json();throw new Error(B.error||"Lỗi tạo Booking!")}}catch(l){alert("❌ "+l.message)}finally{t.textContent=o,t.disabled=!1}});const lt=document.getElementById("editForm");lt&&lt.addEventListener("submit",async n=>{n.preventDefault();const t=n.target.querySelector('button[type="submit"]'),o=t.textContent;t.textContent="Đang lưu cập nhật...",t.disabled=!0;try{const l=document.getElementById("edit-id").value,d=document.getElementById("edit-name").value,s=document.getElementById("edit-phone").value,r=document.getElementById("edit-medal-name").value,a=document.getElementById("edit-tour").value,u=document.getElementById("edit-date").value,m=document.getElementById("edit-dob").value,p=document.getElementById("edit-gender").value,f=document.getElementById("edit-status").value,i=document.getElementById("edit-allergy").value,b=document.getElementById("edit-address").value,v=document.getElementById("edit-diet").value,I=document.getElementById("edit-trekking-pole").value,E=document.getElementById("edit-commitments").checked,e=document.getElementById("edit-special").value,g=document.getElementById("edit-sale");let c,x;if(g&&g.parentElement&&!g.parentElement.classList.contains("hidden")&&(c=g.value||null,x="Website",c)){const B=G.find(ut=>String(ut.id)===String(c));B&&(x=B.full_name||B.fullName)}const k=parseInt(document.getElementById("edit-total").value)||0,C=parseInt(document.getElementById("edit-discount").value)||0,w=parseInt(document.getElementById("edit-deposit").value)||0,h=parseInt(document.getElementById("edit-deposit-required").value)||1e6,T={id:l,name:d,phone:s,medal_name:r,tour:a,date:u,dob:m,gender:p,status:f,allergy:i,address:b,diet:v,trekking_pole:I,commitments:E,special:e,sale_id:c,sale_name:x,total_price:k-C,discount:C,deposit:w,deposit_required:h},M=await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(T)});if(M.ok)alert("✅ Đã cập nhật thành công Chi tiết Đơn hàng!"),(f.includes("Đã cọc")||T.total_price>0&&T.total_price===w)&&nt(T),window.closeEditModal(),typeof S=="function"&&S();else{const B=await M.json();throw new Error(B.error||"Gặp sự cố khi Cập nhật API.")}}catch(l){alert("❌ Lỗi Cập nhật: "+l.message)}finally{t.textContent=o,t.disabled=!1}})};export{ft as afterRender,bt as render};
