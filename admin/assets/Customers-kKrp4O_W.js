import{S as _,H as $}from"./Header-DOba4blu.js";const H=()=>`
    ...
    `,L=()=>{const i=document.getElementById("crmTableBody"),b=document.getElementById("crmSearchInput");let l=[],v=[];const g=async()=>{i.innerHTML='<tr><td colspan="6" class="text-center py-8 text-gray-400">Đang tải danh sách khách hàng...</td></tr>';try{const[t,s]=await Promise.all([fetch("/api/admin_customers"),fetch("/api/bookings")]),n=t.ok?await t.json():{data:[]};v=s.ok?await s.json():[],l=(n.data||[]).map(e=>y(e)),m(l),w(l)}catch(t){console.error("Error loading CRM:",t),i.innerHTML='<tr><td colspan="6" class="text-center py-4 text-red-500">Lỗi kết nối server.</td></tr>'}},y=t=>{const s=v.filter(n=>n.phone===t.phone&&n.status&&!n.status.includes("hủy")).sort((n,e)=>new Date(e.created_at)-new Date(n.created_at));return t._bookings=s,t._tourCount=s.length,t._latestTour=s.length>0?s[0].tour:null,t._latestDate=s.length>0?s[0].date:null,t._tourCount>=5?t._discount={pct:7,label:"Giảm 7%",color:"bg-purple-100 text-purple-600 border-purple-200"}:t._tourCount>=2?t._discount={pct:5,label:"Giảm 5%",color:"bg-green-100 text-green-600 border-green-200"}:t._discount=null,t},m=t=>{const s=document.getElementById("resultCount");if(s&&(s.textContent=`${t.length} khách hàng`),t.length===0){i.innerHTML='<tr><td colspan="6" class="text-center py-8 text-gray-400">Không tìm thấy khách hàng nào.</td></tr>';return}let n="";try{const e=localStorage.getItem("csr_user");e&&(n=JSON.parse(e).role||"")}catch{}i.innerHTML=t.map((e,u)=>{const c=(e.full_name||"?")[0].toUpperCase(),r=["bg-orange-500","bg-blue-500","bg-green-500","bg-purple-500","bg-pink-500","bg-teal-500"],o=r[Math.abs(c.charCodeAt(0))%r.length],x=e._latestTour?`<span class="text-sm text-gray-700">${e._latestTour}</span><div class="text-[10px] text-gray-400 mt-0.5">${e._latestDate||""}</div>`:'<span class="text-gray-300 text-xs">Chưa có</span>',h=e._discount?`<span class="text-xs px-2.5 py-1 rounded-full font-bold border ${e._discount.color}">${e._discount.label}</span>`:'<span class="text-gray-300 text-xs">—</span>',p=e._tourCount>0?`<span class="inline-flex items-center justify-center w-8 h-8 rounded-full text-sm font-bold ${e._tourCount>=5?"bg-purple-100 text-purple-600":e._tourCount>=2?"bg-green-100 text-green-600":"bg-gray-100 text-gray-600"}">${e._tourCount}</span>`:'<span class="text-gray-300">0</span>';return`
                <tr class="hover:bg-gray-50 transition-colors block md:table-row p-4 md:p-0 mb-4 md:mb-0 glass-panel md:glass-none border-l-4 border-csr-orange md:border-none relative">
                    ...
                    <td class="p-0 md:p-4 block md:table-cell mb-4 md:mb-0 pt-3 border-t md:border-none border-gray-50">
                        <div class="flex items-center gap-3">
                            <div class="w-12 h-12 md:w-9 md:h-9 rounded-full ${o} flex items-center justify-center text-white text-base md:text-sm font-bold shrink-0">${c}</div>
                            <div class="min-w-0">
                    ...
                    <td class="py-2 md:p-4 block md:table-cell">
                        <div class="flex justify-between items-center md:block">
                            ...
                            <div class="text-right md:text-center">${p}</div>
                        </div>
                    </td>
                    <td class="py-2 md:p-4 block md:table-cell">
                        <div class="flex justify-between items-center md:block">
                            <span class="md:hidden text-[10px] text-gray-400 uppercase font-medium">Hạng mức</span>
                            <div class="text-right">${discountHtml?h:"—"}</div>
                        </div>
                    </td>
                    <td class="py-4 md:p-4 block md:table-cell border-t md:border-none border-gray-50 mt-2">
                        <div class="flex items-center justify-center gap-2">
                             <button class="detail-btn bg-csr-orange hover:bg-orange-600 text-white md:bg-transparent md:text-csr-orange md:hover:bg-csr-orange/10 px-4 py-2.5 md:px-3 md:py-1.5 rounded-xl md:rounded-lg text-xs font-bold transition-colors flex-1 md:flex-none flex justify-center items-center" data-idx="${u}">
                                 Chi Tiết
                             </button>
                             ${n==="admin"?`
            ...
            `}).join("")},w=t=>{document.getElementById("stat-total").textContent=t.length,document.getElementById("stat-returning").textContent=t.filter(s=>s._tourCount>=2).length,document.getElementById("stat-discount").textContent=t.filter(s=>s._discount).length};b&&b.addEventListener("input",t=>{const s=t.target.value.toLowerCase().trim();if(!s){m(l);return}const n=l.filter(e=>e.csr_code&&e.csr_code.toLowerCase().includes(s)||e.full_name&&e.full_name.toLowerCase().includes(s)||e.phone&&e.phone.includes(s));m(n)});const d=document.getElementById("customerDetailModal"),f=document.getElementById("customerDetailContent"),k=document.getElementById("customerDetailBody");window.closeCustomerDetail=()=>{d.classList.add("opacity-0"),f.classList.add("scale-95"),setTimeout(()=>d.classList.add("hidden"),200)};const C=t=>{const s=r=>{if(!r)return"-";const o=new Date(r);return isNaN(o)?r:`${o.getDate().toString().padStart(2,"0")}/${(o.getMonth()+1).toString().padStart(2,"0")}/${o.getFullYear()}`},n=t._bookings[0]||{},e=[{label:"Mã Khách Hàng",value:t.csr_code,color:"text-csr-orange font-mono font-bold"},{label:"Họ và Tên",value:t.full_name},{label:"Số Điện Thoại",value:t.phone},{label:"CCCD / CMND",value:t.cccd||n.id_card||""},{label:"Ngày Sinh",value:t.dob?s(t.dob):n.dob||""},{label:"Giới Tính",value:t.gender||n.gender||""},{label:"Tên In Huy Chương",value:n.medal_name||"(Dùng tên thật)",color:"text-csr-orange font-bold text-sm"},{label:"Địa Chỉ",value:n.address||""},{label:"Chế Độ Ăn",value:t.dietary||n.diet||""},{label:"Dị Ứng / Y Tế",value:t.medical_notes||n.allergy||""},{label:"Gậy Trekking",value:n.trekking_pole||""},{label:"Ghi Chú Đặc Biệt",value:n.special||""}].filter(r=>r.value),u=t._discount?`<div class="p-4 rounded-xl ${t._discount.color} border text-center">
                  ...
                  Chưa đủ điều kiện giảm giá (cần tham gia ≥ 2 tour)
               </div>`,c=t._bookings.length>0?t._bookings.map((r,o)=>{const x=a=>a?a.includes("Đã cọc")?["bg-green-100 text-green-600",a]:a.includes("Hoàn")?["bg-blue-100 text-blue-600",a]:a.includes("Chờ")?["bg-yellow-100 text-yellow-600",a]:["bg-gray-100 text-gray-500",a]:["bg-gray-100 text-gray-500","Mới"],[h,p]=x(r.status);return`
                    <div class="flex items-center gap-4 p-3 rounded-lg ${o===0?"bg-csr-orange/5 border border-csr-orange/20":"hover:bg-gray-50"}">
                        ...
                        <span class="${h} text-[10px] px-2 py-0.5 rounded-full font-bold shrink-0">${p}</span>
                    </div>
