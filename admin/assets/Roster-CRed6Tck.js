import{S as K,H as O}from"./Header-CamAZjvb.js";const q=()=>`
    ...
    `,X=async()=>{const g=[{id:"col-stt",label:"STT",always:!0,render:(e,t)=>t+1},{id:"col-name",label:"Họ và Tên",default:!0,render:e=>`<div class="font-bold text-gray-900">${e.name}</div>`},{id:"col-dob",label:"Ngày Sinh",default:!0,render:e=>{if(!e.dob)return"-";const t=e.dob.split("-");return t.length===3?`${t[2]}/${t[1]}/${t[0]}`:e.dob}},{id:"col-phone",label:"SĐT",default:!0,render:e=>`<div class="font-medium text-gray-700">${e.phone||"-"}</div>`},{id:"col-gender",label:"Giới tính",default:!0,render:e=>e.gender||"-"},{id:"col-cccd",label:"CCCD / Passport",default:!0,render:e=>e.id_card||e.cccd||"-"},{id:"col-address",label:"Địa chỉ",default:!0,render:e=>`<div class="max-w-[200px] truncate print:max-w-none print:whitespace-normal" title="${e.address||""}">${e.address||"-"}</div>`},{id:"col-diet",label:"Ăn chay",default:!1,render:e=>e.diet==="Chay"||e.diet==="Có"?'<span class="text-green-600 font-bold">Có</span>':"Không"},{id:"col-pole",label:"Gậy Trekking",default:!1,render:e=>e.trekking_pole==="Có"?'<span class="text-orange-600 font-bold">Có</span>':"Không"},{id:"col-allergy",label:"Dị ứng",default:!1,render:e=>e.medical_notes||e.allergy||"-"},{id:"col-special",label:"Yêu cầu ĐB",default:!1,render:e=>e.special_notes||e.special||"-"},{id:"col-medal",label:"Tên Huy Chương",default:!1,render:e=>e.medal_name||e.name||"-"}],w=new URLSearchParams(window.location.search),p=w.get("tour")||"",m=w.get("date")||"",L=w.get("scheduleId")||"";let b=m;const D=e=>{if(!e)return"";const t=e.split(/[-/.]/);if(t.length!==3)return e;let o,r,n;return t[0].length===4?[n,r,o]=t:[o,r,n]=t,`${o.toString().padStart(2,"0")}/${r.toString().padStart(2,"0")}/${n}`},$=e=>(e||"").toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g,"").replace(/[-_]/g," ").replace(/\s+/g," ").trim();m&&(b=D(m));const j=document.getElementById("roster-title"),R=document.getElementById("roster-subtitle"),N=document.getElementById("print-tour-name"),_=document.getElementById("print-tour-date"),A=document.getElementById("print-total"),k=document.getElementById("filterStatus"),C=document.getElementById("filterGender"),S=document.getElementById("rosterList");let h=[],f=new Set(g.filter(e=>e.default||e.always).map(e=>e.id));p&&(j.textContent=p,N.textContent=p),b&&(R.innerHTML=`Ngày khởi hành: <span class="font-bold text-csr-orange">${b}</span>`,_.textContent=b);const P=()=>{const e=document.getElementById("column-toggles");e.innerHTML=g.filter(t=>!t.always).map(t=>`
        ...
        `).join(""),document.querySelectorAll(".col-toggle-cb").forEach(t=>{t.addEventListener("change",o=>{o.target.checked?f.add(o.target.value):f.delete(o.target.value),G()})})},z=()=>{const e=document.getElementById("table-header-row");e.innerHTML=g.map(t=>`
            <th class="p-4 whitespace-nowrap ${t.id}" data-col="${t.id}">${t.label}</th>
        `).join("")},G=()=>{g.forEach(e=>{const t=f.has(e.id),o=document.querySelector(`th[data-col="${e.id}"]`);o&&(t?o.classList.remove("col-hidden"):o.classList.add("col-hidden")),document.querySelectorAll(`td[data-col="${e.id}"]`).forEach(r=>{t?r.classList.remove("col-hidden"):r.classList.add("col-hidden")})})},E=e=>{if(!e)return null;e.includes(" - ")&&(e=e.split(" - ")[0].trim()),e.includes("T")&&(e=e.split("T")[0]);const t=e.split(/[-/.]/);return t.length===2?{day:parseInt(t[0]),month:parseInt(t[1]),year:null}:t.length===3?t[0].length===4?{day:parseInt(t[2]),month:parseInt(t[1]),year:parseInt(t[0])}:{day:parseInt(t[0]),month:parseInt(t[1]),year:parseInt(t[2])}:null},x=E(m),F=e=>{if(!x)return!0;const t=E(e);return!(!t||t.day!==x.day||t.month!==x.month||t.year&&x.year&&t.year!==x.year)},I=async()=>{try{const t=await(await fetch("/api/bookings")).json();console.log("[Roster] targetTour:",p,"targetDateStr:",m,"targetParsed:",x),console.log("[Roster] Total bookings:",t.length),t.length>0&&t.filter(n=>n.tour&&n.tour.includes("Liêng")).slice(0,3).forEach(n=>console.log("[Roster] Sample Liêng:",{tour:n.tour,date:n.date,parsed:E(n.date)})),h=t.filter(r=>{if(L&&r.schedule_id)return String(r.schedule_id)===String(L);let n=!0,s=!0;if(p){const l=$(r.tour),u=$(p);n=l===u||l.includes(u)||u.includes(l)}return m&&(s=F(r.date)),n&&s}),console.log("[Roster] Matched bookings:",h.length);const o=document.getElementById("filterSort")?.value||"newest";o==="newest"?h.sort((r,n)=>parseInt(n.id)-parseInt(r.id)):h.sort((r,n)=>parseInt(r.id)-parseInt(n.id)),console.log(`[Roster] Sorted bookings (${o}):`,h.map(r=>r.id)),y()}catch(e){console.error("Lỗi khi tải dữ liệu đoàn:",e),S.innerHTML='<tr><td colspan="12" class="p-8 text-center text-red-500 font-bold">Lỗi khi tải dữ liệu. Vui lòng tải lại trang.</td></tr>'}},y=()=>{const e=k.value.toLowerCase(),t=C.value.toLowerCase(),o=document.getElementById("filterSearch")?.value?.toLowerCase()||"",r=h.filter(n=>{if(o&&!`${n.name} ${n.phone} ${n.address} ${n.medal_name} ${n.customer_id}`.toLowerCase().includes(o))return!1;const s=(n.status||"").toLowerCase();if(!(e===""||e==="chờ cọc"&&(s.includes("chờ cọc")||s.includes("chờ xác nhận"))||e==="đã cọc"&&(s.includes("đã cọc")||s.includes("hoàn tất")||s.includes("hoàn thành"))||e==="đã hủy"&&s.includes("hủy")))return!1;const u=(n.gender||"").toLowerCase();return t===""||u===t});if(document.getElementById("text-visible-count").textContent=r.length,A.textContent=r.length,r.length===0){S.innerHTML='<tr><td colspan="12" class="p-12 text-center text-gray-500 bg-gray-50">Không tìm thấy khách hàng nào khớp với điều kiện lọc.</td></tr>';return}S.innerHTML=r.map((n,s)=>'<tr class="hover:bg-gray-50/50 transition-colors">'+g.map(l=>`
                    <td class="p-4 align-top ${f.has(l.id)?"":"col-hidden"}" data-col="${l.id}">
                        ${l.render(n,s)}
                    </td>
                `).join("")+"</tr>").join("")},V=()=>{const e=g.filter(a=>f.has(a.id)),t=k.value.toLowerCase(),o=C.value.toLowerCase(),r=document.getElementById("filterSearch")?.value?.toLowerCase()||"",n=h.filter(a=>{if(r&&!`${a.name} ${a.phone} ${a.address} ${a.medal_name} ${a.customer_id}`.toLowerCase().includes(r))return!1;const c=(a.status||"").toLowerCase();if(!(t===""||t==="chờ cọc"&&(c.includes("chờ cọc")||c.includes("chờ xác nhận"))||t==="đã cọc"&&(c.includes("đã cọc")||c.includes("hoàn tất")||c.includes("hoàn thành"))||t==="đã hủy"&&c.includes("hủy")))return!1;const d=(a.gender||"").toLowerCase();return o===""||d===o});let s=`
            ...
            <body>
                <table>
                    ...
                    <tbody>
        `;n.forEach((a,c)=>{s+="<tr>",e.forEach(i=>{let d="";if(i.id==="col-stt")d=c+1;else if(i.id==="col-name")d=a.name||"";else if(i.id==="col-address")d=a.address||"";else if(i.id==="col-phone")d=a.phone?String(a.phone):"";else if(i.id==="col-diet")d=a.diet==="Chay"||a.diet==="Có"?"Có":"Không";else if(i.id==="col-pole")d=a.trekking_pole==="Có"?"Có":"Không";else{const M=String(i.render(a,c)),T=document.createElement("DIV");T.innerHTML=M,d=T.textContent||T.innerText||""}const H=i.id==="col-phone"||i.id==="col-cccd";s+=`<td ${H?'class="num"':""}>${d}</td>`}),s+="</tr>"}),s+=`
                    </tbody>
                </table>
            </body>
            </html>
        `;const l=new Blob(["\uFEFF",s],{type:"application/vnd.ms-excel;charset=utf-8"}),u=URL.createObjectURL(l),B=`DS_Doan_${p||"Tour"}_${b||"Date"}.xls`.replace(/[/\\?%*:|"<>]/g,"-"),v=document.createElement("a");v.setAttribute("href",u),v.setAttribute("download",B),document.body.appendChild(v),v.click(),document.body.removeChild(v)};k.addEventListener("change",y),C.addEventListener("change",y),document.getElementById("filterSearch")?.addEventListener("input",y),document.getElementById("filterSort")?.addEventListener("change",I),document.getElementById("btnExportPDF").addEventListener("click",()=>{window.print()}),document.getElementById("btnExportExcel").addEventListener("click",()=>{V()}),P(),z(),I()};export{X as afterRender,q as render};
