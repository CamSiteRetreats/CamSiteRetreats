...
`,$=()=>{const n=document.getElementById("scheduleFilter"),i=document.getElementById("copyLinkBtn"),o=document.getElementById("statsRow"),c=document.getElementById("avgBars"),u=document.getElementById("reviewList"),r=document.getElementById("reviewListWrap"),v=document.getElementById("emptyState"),g=document.getElementById("noReviewsState"),h=async()=>{try{const s=await fetch("/api/schedules"),l=(s.ok?await s.json():[]).sort((a,t)=>new Date(t.start_date)-new Date(a.start_date));n.innerHTML='<option value="">-- Chọn chuyến đi --</option>',l.forEach(a=>{const t=new Date(a.start_date),m=`${a.tour_name} — ${t.toLocaleDateString("vi-VN")}`;n.insertAdjacentHTML("beforeend",`<option value="${a.id}">${m}</option>`)}),v.classList.remove("hidden")}catch(s){console.error(s)}},y=async s=>{o.classList.add("hidden"),c.classList.add("hidden"),r.classList.add("hidden"),g.classList.add("hidden"),v.classList.add("hidden");try{const d=localStorage.getItem("csr_admin_token"),l=await fetch(`/api/reviews?admin=1&schedule_id=${s}`,{headers:{Authorization:`Bearer ${d}`}}),{reviews:a,stats:t}=await l.json();if(!a||a.length===0){g.classList.remove("hidden");return}document.getElementById("stat-total").textContent=t.total??a.length,document.getElementById("stat-overall").textContent=t.avg_overall?"⭐ "+t.avg_overall:"--",document.getElementById("stat-vehicle").textContent=t.avg_vehicle?"⭐ "+t.avg_vehicle:"--",document.getElementById("stat-guide").textContent=t.avg_guide?"⭐ "+t.avg_guide:"--",o.classList.remove("hidden");const m=[{label:"🚐 Xe cộ",val:t.avg_vehicle},{label:"🧭 Hướng dẫn viên",val:t.avg_guide},{label:"🍽️ Bữa ăn",val:t.avg_meals},{label:"⭐ Tổng thể",val:t.avg_overall}];document.getElementById("avgBarsContent").innerHTML=m.map(e=>{const p=e.val?(e.val/5*100).toFixed(0):0,b=p>=80?"bg-green-500":p>=60?"bg-yellow-400":"bg-red-400";return`
            ...
            `).join(""),document.getElementById("reviewListCount").textContent=`${a.length} phiếu`,r.classList.remove("hidden")}catch(d){console.error("Error loading reviews:",d)}};n.addEventListener("change",()=>{const s=n.value;if(!s){v.classList.remove("hidden"),o.classList.add("hidden"),c.classList.add("hidden"),r.classList.add("hidden"),g.classList.add("hidden"),i.classList.add("hidden");return}i.classList.remove("hidden"),y(s)}),i.addEventListener("click",()=>{const s=n.value;if(!s)return;const d=`${window.location.origin}/review?schedule_id=${s}`;navigator.clipboard.writeText(d).then(()=>{const l=i.innerHTML;i.innerHTML="✅ Đã copy!",setTimeout(()=>i.innerHTML=l,2e3)})}),h()};export{$ as afterRender,_ as render};
