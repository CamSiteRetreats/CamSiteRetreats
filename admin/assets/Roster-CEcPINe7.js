import{S as De,H as je}from"./Header-CARqqxjt.js";const Ae=()=>`
    ...
    `,Re=async()=>{console.log("[Roster] VERSION 3.2 LOADED - READY TO EDIT");const pe=t=>{const e=(t||"").toLowerCase();return e==="hoàn tất phí"||e.includes("hoàn tất")?`<span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold bg-emerald-100 text-emerald-700 border border-emerald-200">
                        ...
                        ⏸️ Bảo lưu</span>`:`<span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold bg-amber-100 text-amber-700 border border-amber-200">${t||"Chờ cọc"}</span>`},ue=t=>{let e=[];const s=t.services_booked;if(s&&s!=="[]"&&s!=="null"&&s!==""){if(Array.isArray(s))e=s.map(n=>{const o=n.label||n.name||n.title||"",d=n.price&&Number(n.price)>0?` (+${Number(n.price).toLocaleString("vi-VN")}đ)`:"";return(o+d).trim()}).filter(Boolean);else if(typeof s=="string"&&s.trim().startsWith("["))try{const n=JSON.parse(s);Array.isArray(n)&&n.length>0&&(e=n.map(o=>{const d=o.label||o.name||o.title||"",c=o.price&&Number(o.price)>0?` (+${Number(o.price).toLocaleString("vi-VN")}đ)`:"";return(d+c).trim()}).filter(Boolean))}catch{const o=s.match(/label:\s*['"]([^'"]+)['"]/g)||[],d=s.match(/price:\s*(\d+)/g)||[];o.length>0&&(e=o.map((c,a)=>{const l=c.replace(/label:\s*['"]/,"").replace(/['"]$/,"").trim(),i=d[a]||"",m=i?Number(i.replace(/price:\s*/,"")):0;return(l+(m>0?` (+${m.toLocaleString("vi-VN")}đ)`:"")).trim()}).filter(Boolean))}}if(e.length===0){const n=t.extra_services||t.extraServices||t.extra||t.notes_extra||"";n&&n!=="-"&&n.trim()!==""&&(e=n.split(/[\n,;]+/).map(o=>o.trim()).filter(Boolean))}if(e.length===0)return'<span class="text-gray-300 text-xs italic">Không có</span>';const r=["bg-violet-100 text-violet-700 border-violet-200","bg-fuchsia-100 text-fuchsia-700 border-fuchsia-200","bg-sky-100 text-sky-700 border-sky-200","bg-teal-100 text-teal-700 border-teal-200","bg-orange-100 text-orange-700 border-orange-200","bg-rose-100 text-rose-700 border-rose-200"];return`<div class="flex flex-wrap gap-1.5">${e.map((n,o)=>`<span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold border ${r[o%r.length]}">
                        <svg class="w-2.5 h-2.5" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/></svg>
                        ${n}</span>`).join("")}</div>`},Q=t=>{if(!t)return!1;const e=new Date().getMonth()+1,s=t.split("-");return s.length===3?parseInt(s[1])===e:!1},L=[{id:"col-stt",label:"STT",always:!0,render:(t,e)=>`<span class="text-gray-400 font-mono text-xs">${e+1}</span>`},{id:"col-name",label:"Họ và Tên",default:!0,render:t=>{const e=Q(t.dob)?' <span title="Sinh nhật tháng này!" class="text-pink-500">🎂</span>':"";return`<div class="font-bold text-gray-900 text-sm">${t.name||"-"}${e}</div>`}},{id:"col-dob",label:"Ngày Sinh",default:!0,render:t=>{if(!t.dob)return'<span class="text-gray-300">—</span>';const e=t.dob.split("-"),s=e.length===3?`${e[2]}/${e[1]}/${e[0]}`:t.dob,r=Q(t.dob)?" 🎂":"";return`<span class="text-gray-700 text-sm tabular-nums">${s}${r}</span>`}},{id:"col-gender",label:"Giới Tính",default:!0,render:t=>{const e=t.gender||"";return e==="Nam"?'<span class="text-blue-600 font-bold text-xs">👨 Nam</span>':e==="Nữ"?'<span class="text-pink-600 font-bold text-xs">👩 Nữ</span>':'<span class="text-gray-400 text-xs">—</span>'}},{id:"col-phone",label:"Số Điện Thoại",default:!0,render:t=>`<a href="tel:${t.phone||""}" class="font-medium text-blue-600 hover:text-blue-800 hover:underline tabular-nums text-sm">${t.phone||"—"}</a>`},{id:"col-pickup",label:"Điểm Đón",default:!0,render:t=>t.pickup_point?`<span class="text-xs bg-blue-50 text-blue-700 border border-blue-200 px-2 py-0.5 rounded-full font-medium">${t.pickup_point}</span>`:'<span class="text-gray-300 text-xs">—</span>'},{id:"col-seat",label:"💺 Ghế",default:!0,render:t=>{const e=t.seat_number||"";return`<div class="flex items-center gap-1">
        ...
        `).join(""),document.querySelectorAll(".col-toggle-cb").forEach(e=>{e.addEventListener("change",s=>{s.target.checked?_.add(s.target.value):_.delete(s.target.value),ke()})})},we=()=>{const t=document.getElementById("table-header-row");t.innerHTML=L.map(e=>`
            <th class="p-4 whitespace-nowrap ${e.id}" data-col="${e.id}">${e.label}</th>
        `).join("")},ke=()=>{L.forEach(t=>{const e=_.has(t.id),s=document.querySelector(`th[data-col="${t.id}"]`);s&&(e?s.classList.remove("col-hidden"):s.classList.add("col-hidden")),document.querySelectorAll(`td[data-col="${t.id}"]`).forEach(r=>{e?r.classList.remove("col-hidden"):r.classList.add("col-hidden")})})},z=t=>{if(!t)return null;t.includes(" - ")&&(t=t.split(" - ")[0].trim()),t.includes("T")&&(t=t.split("T")[0]);const e=t.split(/[-/.]/);return e.length===2?{day:parseInt(e[0]),month:parseInt(e[1]),year:null}:e.length===3?e[0].length===4?{day:parseInt(e[2]),month:parseInt(e[1]),year:parseInt(e[0])}:{day:parseInt(e[0]),month:parseInt(e[1]),year:parseInt(e[2])}:null},M=z($),Ee=t=>{if(!M)return!0;const e=z(t);return!(!e||e.day!==M.day||e.month!==M.month||e.year&&M.year&&e.year!==M.year)},K=async()=>{try{if(A)try{const n=await(await fetch(`/api/guides?schedule_id=${A}`)).json();if(n&&n.length>0){const o=document.getElementById("rosterGuidesSection"),d=document.getElementById("rosterGuidesList"),c={"Hướng Dẫn Viên":"bg-blue-100 text-blue-700 border-blue-200",Leader:"bg-purple-100 text-purple-700 border-purple-200",Photo:"bg-pink-100 text-pink-700 border-pink-200",Guider:"bg-cyan-100 text-cyan-700 border-cyan-200","Hậu Cần":"bg-amber-100 text-amber-700 border-amber-200","Học Việc":"bg-gray-100 text-gray-600 border-gray-200"};o.classList.remove("hidden"),d.innerHTML=n.map(a=>`
                            ...
                            `).join("")}}catch(r){console.error("Error loading guides:",r)}const e=await(await fetch("/api/bookings")).json();console.log("[Roster] targetTour:",I,"targetDateStr:",$,"targetParsed:",M),console.log("[Roster] Total bookings:",e.length),e.length>0&&e.filter(n=>n.tour&&n.tour.includes("Liêng")).slice(0,3).forEach(n=>console.log("[Roster] Sample Liêng:",{tour:n.tour,date:n.date,parsed:z(n.date)})),k=e.filter(r=>{if(A){if(String(r.schedule_id)===String(A))return!0;if(!(!ge&&!r.schedule_id))return!1}let n=!0,o=!0;if(I){const d=Z(r.tour),c=Z(I);n=d===c||d.includes(c)||c.includes(d)}return $&&(o=Ee(r.date)),n&&o}),console.log("[Roster] Matched bookings:",k.length);const s=document.getElementById("filterSort")?.value||"newest";s==="newest"?k.sort((r,n)=>parseInt(n.id)-parseInt(r.id)):k.sort((r,n)=>parseInt(r.id)-parseInt(n.id)),console.log(`[Roster] Sorted bookings (${s}):`,k.map(r=>r.id)),N()}catch(t){console.error("Lỗi khi tải dữ liệu đoàn:",t),V.innerHTML='<tr><td colspan="12" class="p-8 text-center text-red-500 font-bold">Lỗi khi tải dữ liệu. Vui lòng tải lại trang.</td></tr>'}},N=()=>{const t=q.value.toLowerCase(),e=P.value.toLowerCase(),s=document.getElementById("filterSearch")?.value?.toLowerCase()||"",r=k.filter(o=>{if(s&&!`${o.name} ${o.phone} ${o.address} ${o.medal_name} ${o.customer_id}`.toLowerCase().includes(s))return!1;const c=(o.status||"").toLowerCase(),a=t.toLowerCase();if(!(t===""||c===a||a==="chờ cọc"&&(c==="chờ cọc"||c==="chờ xác nhận cọc")||a==="đã cọc"&&c==="đã cọc"||a==="đã cọc (chờ đi)"&&c==="đã cọc (chờ đi)"||a==="hoàn tất phí"&&c.includes("hoàn tất")||a==="hoàn thành"&&c==="hoàn thành"||a==="bảo lưu"&&c==="bảo lưu"||a==="đã hủy"&&c.includes("hủy")))return!1;const i=(o.gender||"").toLowerCase();return e===""||i===e}),n=r.filter(o=>o.status!=="Đã hủy"&&o.status!=="Bảo lưu");if(document.getElementById("text-visible-count").textContent=n.length,ve.textContent=n.length,r.length===0){V.innerHTML='<tr><td colspan="12" class="p-12 text-center text-gray-500 bg-gray-50">Không tìm thấy khách hàng nào khớp với điều kiện lọc.</td></tr>';return}V.innerHTML=r.map((o,d)=>{const c=d%2===0,l=o.status==="Đã hủy"||o.status==="Bảo lưu"?"bg-gray-100 opacity-60":c?"bg-white":"bg-gray-50/40";return`<tr onclick="if(!event.target.closest('.seat-badge') && !event.target.closest('a')) { if(window.actionEdit) window.actionEdit('${o.id}'); else alert('Chưa tải xong logic chỉnh sửa!'); }" class="${l} hover:bg-amber-50/40 transition-colors border-b border-gray-100 group row-clickable cursor-pointer" data-booking-id="${o.id}">`+L.map(i=>`
                    <td class="px-4 py-3 align-middle ${i.id==="col-extra"?"min-w-[220px]":""} ${_.has(i.id)?"":"col-hidden"}" data-col="${i.id}">
                        ${i.render(o,d)}
                    </td>
                `).join("")+"</tr>"}).join("")},Se=()=>{const t=L.filter(i=>_.has(i.id)),e=q.value.toLowerCase(),s=P.value.toLowerCase(),r=document.getElementById("filterSearch")?.value?.toLowerCase()||"",n=k.filter(i=>{if(r&&!`${i.name} ${i.phone} ${i.address} ${i.medal_name} ${i.customer_id}`.toLowerCase().includes(r))return!1;const m=(i.status||"").toLowerCase();if(!(e===""||e==="chờ cọc"&&(m.includes("chờ cọc")||m.includes("chờ xác nhận"))||e==="đã cọc"&&(m.includes("đã cọc")||m.includes("hoàn tất")||m.includes("hoàn thành"))||e==="đã hủy"&&m.includes("hủy")))return!1;const h=(i.gender||"").toLowerCase();return s===""||h===s});let o=`
            ...
            <body>
                <table>
                    <thead>
                        <tr>
                            ${t.map(i=>`<th>${i.label}</th>`).join("")}
                        </tr>
                    </thead>
                    <tbody>
        `;n.forEach((i,m)=>{o+="<tr>",t.forEach(u=>{let h="";if(u.id==="col-stt")h=m+1;else if(u.id==="col-name")h=i.name||"";else if(u.id==="col-order-id")h=i.id||"";else if(u.id==="col-address")h=i.address||"";else if(u.id==="col-phone")h=i.phone?String(i.phone):"";else if(u.id==="col-diet")h=i.diet==="Ăn chay"||i.diet==="Chay"||i.diet==="Có"?"Có":"Không";else if(u.id==="col-pole")h=i.trekking_pole==="Có"?"Có":"Không";else if(u.id==="col-status")h=i.status||"";else if(u.id==="col-extra"){const x=i.services_booked;if(x&&x!=="[]"&&x!=="null"&&x!==""){const y=p=>p.map(b=>{const f=b.label||b.name||b.title||"",g=b.price&&Number(b.price)>0?` (+${Number(b.price).toLocaleString("vi-VN")}đ)`:"";return(f+g).trim()}).filter(Boolean).join(", ");if(Array.isArray(x))h=y(x);else if(typeof x=="string"&&x.trim().startsWith("["))try{h=y(JSON.parse(x))}catch{const b=x.match(/label:\s*['"]([^'"]+)['"]/g)||[],f=x.match(/price:\s*(\d+)/g)||[];b.length>0&&(h=b.map((g,E)=>{const S=g.replace(/label:\s*['"]/,"").replace(/['"]$/,"").trim(),v=f[E]?Number(f[E].replace(/price:\s*/,"")):0;return(S+(v>0?` (+${v.toLocaleString("vi-VN")}đ)`:"")).trim()}).filter(Boolean).join(", "))}}h||(h=i.extra_services||"")}else{const x=String(u.render(i,m)),y=document.createElement("DIV");y.innerHTML=x,h=y.textContent||y.innerText||""}const w=u.id==="col-phone"||u.id==="col-cccd";o+=`<td ${w?'class="num"':""}>${h}</td>`}),o+="</tr>"}),o+=`
                    </tbody>
                </table>
            </body>
            </html>
        `;const d=new Blob(["\uFEFF",o],{type:"application/vnd.ms-excel;charset=utf-8"}),c=URL.createObjectURL(d),a=`DS_Doan_${I||"Tour"}_${j||"Date"}.xls`.replace(/[/\\?%*:|"<>]/g,"-"),l=document.createElement("a");l.setAttribute("href",c),l.setAttribute("download",a),document.body.appendChild(l),l.click(),document.body.removeChild(l)},ee={solati_16:{label:"🚐 Solati 16 chỗ",totalSeats:15},limo_34:{label:"🚌 Limousine 34 chỗ",totalSeats:34},sleeper_45:{label:"🛏️ Giường Nằm 45 chỗ",totalSeats:46}},Ce=t=>{if(t==="limo_34")return{type:"limo",floors:[{label:null,rows:[[{n:"C1"},{n:"C2"},null,{n:"B1"},{n:"B2"},null,{n:"A1"},{n:"A2"},{n:"LÁI",driver:!0}],[{n:"C3"},{n:"C4"},null,{n:"B3"},{n:"B4"},null,{n:"A3"},{n:"A4"}],[{n:"C5"},{n:"C6"},null,{n:"B5"},{n:"B6"},null,{n:"A5"},{n:"A6"}],[{n:"C7"},{n:"C8"},null,{n:"B7"},{n:"B8"},null,{n:"A7"},{n:"A8"}],[{n:"C9"},{n:"C10"},null,{n:"B9"},{n:"B10"},null,{n:"A9"},{n:"A10"}],[{n:"C11"},{n:"C12"},null,null,null,null,{n:"A11"},{n:"A12"}]]}],legend:['<span class="font-bold text-gray-600">Dãy C</span> (cửa lên)','<span class="font-bold text-gray-600">Dãy B</span> (giữa)','<span class="font-bold text-gray-600">Dãy A</span> (tài xế)']};if(t==="sleeper_45"){const e=(s,r,n)=>{const o=Array.from({length:n-r+1},(c,a)=>`${s}${r+a}`),d=[];for(let c=0;c<o.length;c+=4){const a=[];for(let l=0;l<4;l++)l===2&&a.push(null),a.push(o[c+l]?{n:o[c+l]}:null);d.push(a)}return d};return{type:"sleeper",floors:[{label:"🏠 Tầng Dưới",floorKey:"down",rows:e("A",1,23)},{label:"🏥 Tầng Trên",floorKey:"up",rows:e("B",1,23)}],legend:null}}return{type:"solati",floors:[{label:null,rows:[[{n:"LÁI",driver:!0},null,{n:1},{n:2}],[{n:3},{n:4},null,{n:5}],[{n:6},{n:7},null,{n:8}],[{n:9},{n:10},null,{n:11}],[{n:12},{n:13},{n:14},{n:15}]]}],legend:null}},te=ee[Y]||ee.solati_16;let F=0;const R=()=>{const t=document.getElementById("seatMapGrid");if(!t)return;const e={};k.forEach(l=>{const i=l.status==="Đã hủy"||l.status==="Bảo lưu";l.seat_number&&!i&&(e[String(l.seat_number)]=l)});const s=Object.keys(e).length,r=te.totalSeats,n=document.getElementById("seat-map-count"),o=document.getElementById("seat-count-badge"),d=document.getElementById("vehicle-label");n&&(n.textContent=`${s}/${r}`),o&&(o.textContent=`💺 ${s}/${r} ghế đã xếp`),d&&(d.textContent=te.label);const c=Ce(Y),a=(l,i)=>{if(!l)return'<div class="w-12 h-10 flex-shrink-0"></div>';if(l.driver)return'<div class="seat-btn seat-driver w-10 h-9 rounded-lg flex flex-col items-center justify-center text-[8px] font-bold flex-shrink-0"><span>🚗</span><span>LÁI</span></div>';const m=String(l.n),u=i[m],h=u?(u.name||"").split(" ").pop().slice(0,4):"",x=/[A-Z]/.test(m)?"w-11 h-9":"w-12 h-10";return`<button
                ...
                <div class="flex gap-2 mb-3">
                    ${c.floors.map((l,i)=>`
                        <button class="floor-tab flex-1 py-1.5 text-xs font-bold rounded-lg transition-colors ${F===i?"bg-csr-orange text-white":"bg-gray-100 text-gray-500 hover:bg-gray-200"}" data-floor="${i}">
                            ${l.label}
                        </button>
                    `).join("")}
                </div>
                <div class="floor-content">
                    ${c.floors.map((l,i)=>`
                        <div class="floor-rows ${F===i?"":"hidden"}" data-floor="${i}">
                            ${l.rows.map(m=>`
                    ...
                    `).join("")}
                </div>`,t.querySelectorAll(".floor-tab").forEach(l=>{l.addEventListener("click",()=>{F=parseInt(l.dataset.floor),R()})});else{let l="";c.legend&&(l+=`<div class="flex justify-around text-[9px] text-gray-400 mb-2 px-1">${c.legend.map(i=>`<span>${i}</span>`).join("")}</div>`),c.floors[0].rows.forEach(i=>{const m=i.map(u=>a(u,e)).join("");l+=`<div class="flex items-center justify-center gap-1 mb-1">${m}</div>`}),t.innerHTML=l}t.querySelectorAll(".seat-btn[data-seat]").forEach(l=>{l.addEventListener("click",()=>ne(l.dataset.seat,l.dataset.bookingId))})},ne=(t,e)=>{const s=document.getElementById("seatModal"),r=document.getElementById("seatModalTitle"),n=document.getElementById("seatModalBody");if(!s||!r||!n)return;r.textContent=e?`Ghế ${t} — ${k.find(a=>String(a.id)===String(e))?.name||"?"}`:`Ghế ${t} — Chưa có khách`;const o=k.find(a=>String(a.id)===String(e));let d="";o&&(d+=`<div class="mb-3 p-3 bg-orange-50 border border-orange-200 rounded-xl text-sm">
                <div class="font-bold text-gray-800">${o.name}</div>
                <div class="text-gray-500 text-xs">${o.phone||""} ${o.gender?"· "+o.gender:""}</div>
                ${o.pickup_point?`<div class="text-xs text-blue-600 mt-1">📍 ${o.pickup_point}</div>`:""}
            </div>
            ...
            <div class="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Chọn khách khác:</div>`);const c=k.filter(a=>!a.seat_number||String(a.id)===String(e));c.length===0?d+='<p class="text-gray-400 text-sm text-center py-4">Tất cả khách đã được xếp ghế.</p>':d+=c.map(a=>`
                ...
                </button>`).join(""),n.innerHTML=d,s.style.display="flex",n.querySelector("#btnRemoveSeat")?.addEventListener("click",async()=>{await J(e,null),s.style.display="none"}),n.querySelectorAll(".btn-assign-passenger").forEach(a=>{a.addEventListener("click",async()=>{const l=a.dataset.bookingId,i=a.dataset.seat,m=k.find(u=>String(u.seat_number)===String(i)&&String(u.id)!==String(l));m&&await J(m.id,null),await J(l,i),s.style.display="none"})})},J=async(t,e)=>{try{await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:t,seat_number:e||null})});const s=k.find(r=>String(r.id)===String(t));s&&(s.seat_number=e||null),N(),R()}catch(s){console.error("Lỗi lưu ghế:",s),alert("Lỗi lưu số ghế. Vui lòng thử lại.")}};q.addEventListener("change",N),P.addEventListener("change",N),document.getElementById("filterSearch")?.addEventListener("input",N),document.getElementById("filterSort")?.addEventListener("change",K),document.getElementById("btnExportPDF").addEventListener("click",()=>window.print()),document.getElementById("btnExportExcel").addEventListener("click",()=>Se()),document.getElementById("seatModalClose")?.addEventListener("click",()=>{document.getElementById("seatModal").style.display="none"}),document.getElementById("seatModal")?.addEventListener("click",t=>{t.target===document.getElementById("seatModal")&&(document.getElementById("seatModal").style.display="none")}),document.getElementById("rosterList")?.addEventListener("mouseover",t=>{const e=t.target.closest("tr[data-booking-id]");if(!e)return;document.querySelectorAll(".seat-btn").forEach(n=>n.classList.remove("seat-highlighted"));const s=e.dataset.bookingId,r=k.find(n=>String(n.id)===String(s));r?.seat_number&&document.querySelector(`.seat-btn[data-seat="${r.seat_number}"]`)?.classList.add("seat-highlighted")}),document.getElementById("rosterList")?.addEventListener("click",t=>{const e=t.target.closest(".seat-badge");if(!e)return;const s=e.dataset.id,r=k.find(n=>String(n.id)===String(s));ne(r?.seat_number||"?",s)});let H=[],se=[],W=[];const Be=async()=>{try{const[t,e,s]=await Promise.all([fetch("/api/tours"),fetch("/api/schedules"),fetch("/api/users")]);H=t.ok?await t.json():[],se=e.ok?await e.json():[],W=s.ok?await s.json():[],Te("edit-tour"),Ie("edit-sale")}catch(t){console.error("Lỗi tải tours/schedules:",t)}},Ie=t=>{const e=document.getElementById(t);e&&(e.innerHTML='<option value="">-- Website --</option>',W.forEach(s=>{const r=document.createElement("option");r.value=s.id,r.textContent=`${s.full_name||s.fullName} (${s.role})`,e.appendChild(r)}))},Te=t=>{const e=document.getElementById(t);if(!e)return;const s=e.value,r=e.options[0]?.outerHTML||'<option value="">-- Chọn Tour --</option>';e.innerHTML=r,H.filter(n=>n.is_visible!==!1).forEach(n=>{const o=document.createElement("option");o.value=n.name,o.textContent=n.name,n.name===s&&(o.selected=!0),e.appendChild(o)})},oe=(t,e,s,r)=>{const n=document.getElementById(t);if(!n||(n.innerHTML='<option value="">-- Chọn Lịch --</option>',!e))return;const o=se.filter(d=>d.tour_name?d.tour_name===e||e.includes(d.tour_name)||d.tour_name.includes(e):!1);if(o.length===0){n.innerHTML='<option value="">Chưa có lịch cho tour này</option>';return}o.sort((d,c)=>new Date(d.start_date)-new Date(c.start_date)).forEach(d=>{const c=parseInt(d.booked_count)||0,a=(d.slots||0)-c,l=new Date(d.start_date),i=d.end_date?new Date(d.end_date):null,m=y=>`${y.getDate().toString().padStart(2,"0")}/${(y.getMonth()+1).toString().padStart(2,"0")}`;let u=m(l);i&&(u+=" - "+m(i)),d.group_label&&(u+=` · ${d.group_label}`),u+=` (${a>=0?a:0} chỗ trống)`;const h=document.createElement("option"),w=`${l.getDate().toString().padStart(2,"0")}/${(l.getMonth()+1).toString().padStart(2,"0")}/${l.getFullYear()}`;h.value=`${d.id}::${w}`,h.dataset.scheduleId=d.id,h.dataset.date=w,h.textContent=u,a<=0&&(h.disabled=!0,h.textContent=u.replace("chỗ trống","HẾT CHỖ"));let x=s||"";if(x.includes("-")&&x.length>=10){const y=x.substring(0,10).split("-");y.length===3&&(x=`${y[2]}/${y[1]}/${y[0]}`)}(r&&String(d.id)===String(r)||!r&&x&&w===x)&&(h.selected=!0),n.appendChild(h)})},re=document.getElementById("edit-tour");re&&re.addEventListener("change",t=>{oe("edit-date",t.target.value)}),window.closeEditModal=()=>{document.getElementById("editModal").classList.remove("opacity-100");const t=document.getElementById("editModalContent");t&&(t.classList.remove("scale-100","translate-y-0"),t.classList.add("scale-95","translate-y-4")),setTimeout(()=>document.getElementById("editModal").classList.add("hidden"),300)},window.updateEditRemaining=()=>{const t=parseInt(document.getElementById("edit-total")?.value)||0,e=parseInt(document.getElementById("edit-discount")?.value)||0,s=parseInt(document.getElementById("edit-deposit")?.value)||0;let r=0;document.querySelectorAll(".preset-service-check:checked").forEach(l=>{r+=parseInt(l.dataset.price)||0}),document.querySelectorAll(".service-price-input").forEach(l=>{r+=parseInt(l.value)||0});const n=t-e+r,o=n-s,d=document.getElementById("edit-grand-total");d&&(d.textContent=n.toLocaleString("vi-VN")+"đ");const c=document.getElementById("edit-remaining");c&&(c.textContent=o>0?o.toLocaleString("vi-VN")+"đ":"0đ");const a=document.getElementById("edit-status");if(a){const l=a.value;["Chờ tư vấn","Chờ cọc","Chờ xác nhận cọc"].includes(l)&&(s>0&&o>0?a.value="Đã cọc":s===0&&(a.value=l))}},window.updateServicesTotal=()=>{let t=0;document.querySelectorAll(".preset-service-check:checked").forEach(r=>{t+=parseInt(r.dataset.price)||0}),document.querySelectorAll(".service-price-input").forEach(r=>{t+=parseInt(r.value)||0});const e=document.getElementById("servicesTotalDisplay");e&&(e.textContent=t.toLocaleString("vi-VN")+"đ");const s=document.getElementById("servicesTotalRow");return s&&s.classList.toggle("hidden",t===0),window.updateEditRemaining(),t},window.addServiceRow=(t="",e=0)=>{const s=document.getElementById("servicesContainer"),r=document.getElementById("emptyServicesMsg"),n=document.getElementById("servicesTotalRow");if(!s)return;r&&r.classList.add("hidden"),n&&n.classList.remove("hidden");const o=document.createElement("div");o.className="service-row flex items-center gap-2 bg-gray-50 border border-gray-100 rounded-xl px-3 py-2.5",o.innerHTML=`
        ...
        `,o.querySelector(".remove-service-btn").addEventListener("click",()=>{o.remove(),s.querySelectorAll(".service-row").length===0&&(r&&r.classList.remove("hidden"),n&&n.classList.add("hidden")),window.updateServicesTotal()}),o.querySelector(".service-price-input").addEventListener("input",window.updateServicesTotal),s.appendChild(o),window.updateServicesTotal()};const ae=document.getElementById("addServiceBtn");ae&&ae.addEventListener("click",()=>window.addServiceRow()),window.actionEdit=async t=>{try{const e=k.find(p=>p.id==t);if(!e){alert("Không tìm thấy thông tin khách hàng!");return}document.getElementById("edit-id").value=t,document.getElementById("edit-name").value=e.name||"",document.getElementById("edit-phone").value=e.phone||"",document.getElementById("edit-medal-name").value=e.medal_name||"",document.getElementById("edit-tour").value=e.tour||"",oe("edit-date",e.tour||"",e.date||"",e.schedule_id||null),(p=>{let b=p.dob||"";if(b&&b.includes("/")){const f=b.split("/");f.length===3&&(b=`${f[2]}-${f[1].padStart(2,"0")}-${f[0].padStart(2,"0")}`)}document.getElementById("edit-dob").value=b,document.getElementById("edit-gender").value=p.gender||"Khác",document.getElementById("edit-allergy").value=p.allergy||p.medical_notes||"",document.getElementById("edit-address").value=p.address||"",document.getElementById("edit-diet").value=p.diet||p.dietary||"",document.getElementById("edit-trekking-pole").value=p.trekking_pole||"Không",document.getElementById("edit-id-card").value=p.id_card||p.cccd||""})(e),document.getElementById("edit-status").value=e.status||"Chờ xác nhận cọc",document.getElementById("edit-commitments").checked=!!e.commitments,document.getElementById("edit-special").value=e.special||"";try{const p=localStorage.getItem("csr_user");if(p&&JSON.parse(p).role==="admin"){const f=document.getElementById("edit-sale-container");f&&f.classList.remove("hidden");const g=document.getElementById("edit-sale");g&&(g.value=e.sale_id||"")}}catch{}let r=0;try{const p=typeof e.services_booked=="string"?JSON.parse(e.services_booked):e.services_booked||[];Array.isArray(p)&&p.forEach(b=>r+=parseInt(b.price)||0)}catch{}let o=(parseInt(e.total_price)||0)+(parseInt(e.discount)||0)-r;if(o===0&&e.tour){const p=H.find(b=>b.name===e.tour);p&&parseInt(p.price)>0&&(o=parseInt(p.price))}document.getElementById("edit-total").value=o,document.getElementById("edit-discount").value=e.discount||0,document.getElementById("edit-deposit").value=e.deposit||0;const d=parseInt(e.deposit_required);document.getElementById("edit-deposit-required").value=isNaN(d)?1e6:d,window.updateEditRemaining();const c=document.getElementById("servicesContainer"),a=document.getElementById("emptyServicesMsg"),l=document.getElementById("servicesTotalRow"),i=document.getElementById("presetServicesBlock"),m=document.getElementById("presetServicesList"),u=document.getElementById("servicesDivider");let h=[];try{if(e.services_booked){const p=typeof e.services_booked=="string"?JSON.parse(e.services_booked):e.services_booked;Array.isArray(p)&&(h=p)}}catch{}if(m&&i){m.innerHTML="";const p=H.find(f=>f.name===e.tour);let b=[];try{if(p?.services){const f=typeof p.services=="string"?JSON.parse(p.services):p.services;Array.isArray(f)&&(b=f)}}catch{}b.length>0?(i.classList.remove("hidden"),u&&u.classList.remove("hidden"),b.forEach(f=>{const g=h.some(S=>(S.label||S.name||"").toLowerCase()===(f.label||"").toLowerCase()),E=document.createElement("label");E.className="preset-service-card flex items-center gap-3 p-3 rounded-xl border-2 cursor-pointer transition-all "+(g?"border-green-400 bg-green-50":"border-gray-100 bg-gray-50"),E.innerHTML=`
                        ...
                        `,E.querySelector(".preset-service-check").addEventListener("change",S=>{E.className="preset-service-card flex items-center gap-3 p-3 rounded-xl border-2 cursor-pointer transition-all "+(S.target.checked?"border-green-400 bg-green-50":"border-gray-100 bg-gray-50"),window.updateServicesTotal()}),m.appendChild(E)})):(i.classList.add("hidden"),u&&u.classList.add("hidden"))}const w=document.getElementById("edit-pickup-point");if(w){const p=H.find(v=>v.name===e.tour);let b=[];try{if(p?.pickup_points){const v=typeof p.pickup_points=="string"?JSON.parse(p.pickup_points):p.pickup_points;Array.isArray(v)&&(b=v)}}catch{}w.innerHTML='<option value="">— Chưa chọn —</option>',b.forEach(v=>{const B=(v.label||v.name||v)+(v.time?` (${v.time})`:""),U=v.label||v.name||v,T=document.createElement("option");T.value=U,T.textContent=B,w.appendChild(T)});const f=document.createElement("option");f.value="__custom__",f.textContent="✏️ Khác (nhập tay)",w.appendChild(f);const g=document.getElementById("edit-pickup-custom"),E=v=>{g&&(v==="__custom__"?(g.classList.remove("hidden"),g.focus()):(g.classList.add("hidden"),g.value=""))},S=e.pickup_point||"";S&&(b.some(B=>(B.label||B.name||B)===S)?w.value=S:(w.value="__custom__",g&&(g.value=S,g.classList.remove("hidden")))),w.addEventListener("change",()=>E(w.value))}if(c){c.querySelectorAll(".service-row").forEach(g=>g.remove());const p=H.find(g=>g.name===e.tour);let b=new Set;try{const g=p?.services,E=g?typeof g=="string"?JSON.parse(g):g:[];Array.isArray(E)&&E.forEach(S=>b.add((S.label||"").toLowerCase()))}catch{}const f=h.filter(g=>!b.has((g.label||g.name||"").toLowerCase()));f.length>0?(a&&a.classList.add("hidden"),l&&l.classList.remove("hidden"),f.forEach(g=>window.addServiceRow(g.label||g.name||"",g.price||0))):a&&a.classList.remove("hidden"),window.updateServicesTotal()}const x=document.getElementById("editModal"),y=document.getElementById("editModalContent");x&&y&&(x.classList.remove("hidden"),setTimeout(()=>{x.classList.add("opacity-100"),y.classList.remove("scale-95","translate-y-4"),y.classList.add("scale-100","translate-y-0")},10))}catch(e){console.error("Lỗi khi mở form Edit:",e),alert("Lỗi khi mở form Edit: "+e.message)}};const le=document.getElementById("editForm");le&&le.addEventListener("submit",async t=>{t.preventDefault();const e=t.target.querySelector('button[type="submit"]'),s=e.textContent;e.textContent="Đang lưu cập nhật...",e.disabled=!0;try{const r=document.getElementById("edit-id").value,n=document.getElementById("edit-name").value,o=document.getElementById("edit-phone").value,d=document.getElementById("edit-medal-name").value,c=document.getElementById("edit-tour").value,a=document.getElementById("edit-date").value;let l=a,i;if(a&&a.includes("::")){const C=a.split("::");i=parseInt(C[0])||null,l=C[1]}const m=document.getElementById("edit-dob").value,u=document.getElementById("edit-gender").value,h=document.getElementById("edit-status").value,w=document.getElementById("edit-allergy").value,x=document.getElementById("edit-address").value,y=document.getElementById("edit-diet").value,p=document.getElementById("edit-trekking-pole").value,b=document.getElementById("edit-commitments").checked,f=document.getElementById("edit-special").value,g=document.getElementById("edit-id-card").value,E=document.getElementById("edit-pickup-point"),S=document.getElementById("edit-pickup-custom"),v=E?.value==="__custom__"?S?.value?.trim()||"":E?.value||"",B=[];document.querySelectorAll(".preset-service-check:checked").forEach(C=>{B.push({label:C.dataset.label,price:parseInt(C.dataset.price)||0})}),document.querySelectorAll(".service-row").forEach(C=>{const O=(C.querySelector(".service-label-input")?.value||"").trim(),He=parseInt(C.querySelector(".service-price-input")?.value)||0;O&&B.push({label:O,price:He})});const U=B.length>0?JSON.stringify(B):null,T=document.getElementById("edit-sale");let G,X;if(T&&!T.parentElement.classList.contains("hidden")&&(G=T.value||null,X="Website",G)){const C=W.find(O=>String(O.id)===String(G));C&&(X=C.full_name||C.fullName)}const Le=parseInt(document.getElementById("edit-total").value)||0,ie=parseInt(document.getElementById("edit-discount").value)||0,$e=parseInt(document.getElementById("edit-deposit").value)||0,de=parseInt(document.getElementById("edit-deposit-required").value),_e=isNaN(de)?1e6:de,Me={id:r,name:n,phone:o,medal_name:d,tour:c,date:l,dob:m,gender:u,status:h,allergy:w,address:x,diet:y,trekking_pole:p,commitments:b,special:f,id_card:g,pickup_point:v,services_booked:U,sale_id:G,sale_name:X,total_price:Le-ie,discount:ie,deposit:$e,deposit_required:_e,...i!==void 0?{schedule_id:i}:{}},ce=await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(Me)});if(ce.ok)alert("✅ Đã cập nhật thành công Chi tiết Đơn hàng!"),window.closeEditModal(),K();else{const C=await ce.json();throw new Error(C.error||"Gặp sự cố khi Cập nhật API.")}}catch(r){alert("❌ Lỗi Cập nhật: "+r.message)}finally{e.textContent=s,e.disabled=!1}}),ye(),we(),Promise.all([K(),Be()]).then(()=>R()).catch(()=>R())};export{Re as afterRender,Ae as render};
