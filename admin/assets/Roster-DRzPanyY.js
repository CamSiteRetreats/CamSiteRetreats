    ...
    `,Re=async()=>{console.log("[Roster] VERSION 3.2 LOADED - READY TO EDIT");const ce=t=>{const e=(t||"").toLowerCase();return e.includes("hoàn tất")||e.includes("hoàn thành")||e.includes("đã thanh toán")||e.includes("thanh toán")?`<span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold bg-emerald-100 text-emerald-700 border border-emerald-200">
        ...
        `).join("")},we=()=>{I.forEach(t=>{const e=L.has(t.id),s=document.querySelector(`th[data-col="${t.id}"]`);s&&(e?s.classList.remove("col-hidden"):s.classList.add("col-hidden")),document.querySelectorAll(`td[data-col="${t.id}"]`).forEach(o=>{e?o.classList.remove("col-hidden"):o.classList.add("col-hidden")})})},V=t=>{if(!t)return null;t.includes(" - ")&&(t=t.split(" - ")[0].trim()),t.includes("T")&&(t=t.split("T")[0]);const e=t.split(/[-/.]/);return e.length===2?{day:parseInt(e[0]),month:parseInt(e[1]),year:null}:e.length===3?e[0].length===4?{day:parseInt(e[2]),month:parseInt(e[1]),year:parseInt(e[0])}:{day:parseInt(e[0]),month:parseInt(e[1]),year:parseInt(e[2])}:null},$=V(T),ke=t=>{if(!$)return!0;const e=V(t);return!(!e||e.day!==$.day||e.month!==$.month||e.year&&$.year&&e.year!==$.year)},K=async()=>{try{if(H)try{const n=await(await fetch(`/api/guides?schedule_id=${H}`)).json();if(n&&n.length>0){const r=document.getElementById("rosterGuidesSection"),a=document.getElementById("rosterGuidesList"),u={"Hướng Dẫn Viên":"bg-blue-100 text-blue-700 border-blue-200",Leader:"bg-purple-100 text-purple-700 border-purple-200",Photo:"bg-pink-100 text-pink-700 border-pink-200",Guider:"bg-cyan-100 text-cyan-700 border-cyan-200","Hậu Cần":"bg-amber-100 text-amber-700 border-amber-200","Học Việc":"bg-gray-100 text-gray-600 border-gray-200"};r.classList.remove("hidden"),a.innerHTML=n.map(c=>`
                                <div class="flex items-center gap-2 bg-white px-3 py-2 rounded-xl border border-gray-200 shadow-sm print:shadow-none print:border-gray-400">
                                    <span class="text-[10px] font-bold px-2 py-0.5 rounded-full border ${u[c.role]||"bg-gray-100 text-gray-600 border-gray-200"} uppercase tracking-wide print:border-black print:text-black">${c.role}</span>
                                    <span class="font-bold text-gray-900 text-sm print:text-black">${c.full_name}</span>
                                    ${c.phone?`<a href="tel:${c.phone}" class="text-xs text-gray-500 hover:text-indigo-600 font-medium ml-1 print:text-black print:no-underline">(${c.phone})</a>`:""}
                                </div>
                            `).join("")}}catch(o){console.error("Error loading guides:",o)}const e=await(await fetch("/api/bookings")).json();console.log("[Roster] targetTour:",B,"targetDateStr:",T,"targetParsed:",$),console.log("[Roster] Total bookings:",e.length),e.length>0&&e.filter(n=>n.tour&&n.tour.includes("Liêng")).slice(0,3).forEach(n=>console.log("[Roster] Sample Liêng:",{tour:n.tour,date:n.date,parsed:V(n.date)})),w=e.filter(o=>{if(H){if(String(o.schedule_id)===String(H))return!0;if(!(!ue&&!o.schedule_id))return!1}let n=!0,r=!0;if(B){const a=Q(o.tour),u=Q(B);n=a===u||a.includes(u)||u.includes(a)}return T&&(r=ke(o.date)),n&&r}),console.log("[Roster] Matched bookings:",w.length);const s=document.getElementById("filterSort")?.value||"newest";s==="newest"?w.sort((o,n)=>parseInt(n.id)-parseInt(o.id)):w.sort((o,n)=>parseInt(o.id)-parseInt(n.id)),console.log(`[Roster] Sorted bookings (${s}):`,w.map(o=>o.id)),j()}catch(t){console.error("Lỗi khi tải dữ liệu đoàn:",t),P.innerHTML='<tr><td colspan="12" class="p-8 text-center text-red-500 font-bold">Lỗi khi tải dữ liệu. Vui lòng tải lại trang.</td></tr>'}},j=()=>{const t=O.value.toLowerCase(),e=q.value.toLowerCase(),s=document.getElementById("filterSearch")?.value?.toLowerCase()||"",o=w.filter(r=>{if(s&&!`${r.name} ${r.phone} ${r.address} ${r.medal_name} ${r.customer_id}`.toLowerCase().includes(s))return!1;const a=(r.status||"").toLowerCase();if(!(t===""||t==="chờ cọc"&&(a.includes("chờ cọc")||a.includes("chờ xác nhận"))||t==="đã cọc"&&(a.includes("đã cọc")||a.includes("hoàn tất")||a.includes("hoàn thành"))||t==="đã hủy"&&a.includes("hủy")))return!1;const c=(r.gender||"").toLowerCase();return e===""||c===e}),n=o.filter(r=>r.status!=="Đã hủy"&&r.status!=="Bảo lưu");if(document.getElementById("text-visible-count").textContent=n.length,xe.textContent=n.length,o.length===0){P.innerHTML='<tr><td colspan="12" class="p-12 text-center text-gray-500 bg-gray-50">Không tìm thấy khách hàng nào khớp với điều kiện lọc.</td></tr>';return}P.innerHTML=o.map((r,a)=>{const u=a%2===0,d=r.status==="Đã hủy"||r.status==="Bảo lưu"?"bg-gray-100 opacity-60":u?"bg-white":"bg-gray-50/40";return`<tr onclick="if(!event.target.closest('.seat-badge') && !event.target.closest('a')) { if(window.actionEdit) window.actionEdit('${r.id}'); else alert('Chưa tải xong logic chỉnh sửa!'); }" class="${d} hover:bg-amber-50/40 transition-colors border-b border-gray-100 group row-clickable cursor-pointer" data-booking-id="${r.id}">`+I.map(l=>`
            ...
            <body>
                <table>
                    ...
                    <tbody>
        `;n.forEach((l,m)=>{r+="<tr>",t.forEach(p=>{let h="";if(p.id==="col-stt")h=m+1;else if(p.id==="col-name")h=l.name||"";else if(p.id==="col-order-id")h=l.id||"";else if(p.id==="col-address")h=l.address||"";else if(p.id==="col-phone")h=l.phone?String(l.phone):"";else if(p.id==="col-diet")h=l.diet==="Ăn chay"||l.diet==="Chay"||l.diet==="Có"?"Có":"Không";else if(p.id==="col-pole")h=l.trekking_pole==="Có"?"Có":"Không";else if(p.id==="col-status")h=l.status||"";else if(p.id==="col-extra"){const i=l.services_booked;if(i&&i!=="[]"&&i!=="null"&&i!==""){const b=f=>f.map(g=>{const v=g.label||g.name||g.title||"",y=g.price&&Number(g.price)>0?` (+${Number(g.price).toLocaleString("vi-VN")}đ)`:"";return(v+y).trim()}).filter(Boolean).join(", ");if(Array.isArray(i))h=b(i);else if(typeof i=="string"&&i.trim().startsWith("["))try{h=b(JSON.parse(i))}catch{const g=i.match(/label:\s*['"]([^'"]+)['"]/g)||[],v=i.match(/price:\s*(\d+)/g)||[];g.length>0&&(h=g.map((y,x)=>{const S=y.replace(/label:\s*['"]/,"").replace(/['"]$/,"").trim(),_=v[x]?Number(v[x].replace(/price:\s*/,"")):0;return(S+(_>0?` (+${_.toLocaleString("vi-VN")}đ)`:"")).trim()}).filter(Boolean).join(", "))}}h||(h=l.extra_services||"")}else{const i=String(p.render(l,m)),b=document.createElement("DIV");b.innerHTML=i,h=b.textContent||b.innerText||""}const E=p.id==="col-phone"||p.id==="col-cccd";r+=`<td ${E?'class="num"':""}>${h}</td>`}),r+="</tr>"}),r+=`
                    </tbody>
                </table>
            </body>
            </html>
        `;const a=new Blob(["\uFEFF",r],{type:"application/vnd.ms-excel;charset=utf-8"}),u=URL.createObjectURL(a),c=`DS_Doan_${B||"Tour"}_${D||"Date"}.xls`.replace(/[/\\?%*:|"<>]/g,"-"),d=document.createElement("a");d.setAttribute("href",u),d.setAttribute("download",c),document.body.appendChild(d),d.click(),document.body.removeChild(d)},Z={solati_16:{label:"🚐 Solati 16 chỗ",totalSeats:15},limo_34:{label:"🚌 Limousine 34 chỗ",totalSeats:34},sleeper_45:{label:"🛏️ Giường Nằm 45 chỗ",totalSeats:46}},Se=t=>{if(t==="limo_34")return{type:"limo",floors:[{label:null,rows:[[{n:"C1"},{n:"C2"},null,{n:"B1"},{n:"B2"},null,{n:"A1"},{n:"A2"},{n:"LÁI",driver:!0}],[{n:"C3"},{n:"C4"},null,{n:"B3"},{n:"B4"},null,{n:"A3"},{n:"A4"}],[{n:"C5"},{n:"C6"},null,{n:"B5"},{n:"B6"},null,{n:"A5"},{n:"A6"}],[{n:"C7"},{n:"C8"},null,{n:"B7"},{n:"B8"},null,{n:"A7"},{n:"A8"}],[{n:"C9"},{n:"C10"},null,{n:"B9"},{n:"B10"},null,{n:"A9"},{n:"A10"}],[{n:"C11"},{n:"C12"},null,null,null,null,{n:"A11"},{n:"A12"}]]}],legend:['<span class="font-bold text-gray-600">Dãy C</span> (cửa lên)','<span class="font-bold text-gray-600">Dãy B</span> (giữa)','<span class="font-bold text-gray-600">Dãy A</span> (tài xế)']};if(t==="sleeper_45"){const e=(s,o,n)=>{const r=Array.from({length:n-o+1},(u,c)=>`${s}${o+c}`),a=[];for(let u=0;u<r.length;u+=4){const c=[];for(let d=0;d<4;d++)d===2&&c.push(null),c.push(r[u+d]?{n:r[u+d]}:null);a.push(c)}return a};return{type:"sleeper",floors:[{label:"🏠 Tầng Dưới",floorKey:"down",rows:e("A",1,23)},{label:"🏥 Tầng Trên",floorKey:"up",rows:e("B",1,23)}],legend:null}}return{type:"solati",floors:[{label:null,rows:[[{n:"LÁI",driver:!0},null,{n:1},{n:2}],[{n:3},{n:4},null,{n:5}],[{n:6},{n:7},null,{n:8}],[{n:9},{n:10},null,{n:11}],[{n:12},{n:13},{n:14},{n:15}]]}],legend:null}},ee=Z[Y]||Z.solati_16;let z=0;const A=()=>{const t=document.getElementById("seatMapGrid");if(!t)return;const e={};w.forEach(d=>{const l=d.status==="Đã hủy"||d.status==="Bảo lưu";d.seat_number&&!l&&(e[String(d.seat_number)]=d)});const s=Object.keys(e).length,o=ee.totalSeats,n=document.getElementById("seat-map-count"),r=document.getElementById("seat-count-badge"),a=document.getElementById("vehicle-label");n&&(n.textContent=`${s}/${o}`),r&&(r.textContent=`💺 ${s}/${o} ghế đã xếp`),a&&(a.textContent=ee.label);const u=Se(Y),c=(d,l)=>{if(!d)return'<div class="w-12 h-10 flex-shrink-0"></div>';if(d.driver)return'<div class="seat-btn seat-driver w-10 h-9 rounded-lg flex flex-col items-center justify-center text-[8px] font-bold flex-shrink-0"><span>🚗</span><span>LÁI</span></div>';const m=String(d.n),p=l[m],h=p?(p.name||"").split(" ").pop().slice(0,4):"",i=/[A-Z]/.test(m)?"w-11 h-9":"w-12 h-10";return`<button
                class="seat-btn ${p?"seat-assigned":"seat-empty"} ${i} rounded-lg flex flex-col items-center justify-center flex-shrink-0 group relative"
                data-seat="${m}"
                ...
                <div class="floor-content">
                    ...
                    `).join("")}
                </div>`,t.querySelectorAll(".floor-tab").forEach(d=>{d.addEventListener("click",()=>{z=parseInt(d.dataset.floor),A()})});else{let d="";u.legend&&(d+=`<div class="flex justify-around text-[9px] text-gray-400 mb-2 px-1">${u.legend.map(l=>`<span>${l}</span>`).join("")}</div>`),u.floors[0].rows.forEach(l=>{const m=l.map(p=>c(p,e)).join("");d+=`<div class="flex items-center justify-center gap-1 mb-1">${m}</div>`}),t.innerHTML=d}t.querySelectorAll(".seat-btn[data-seat]").forEach(d=>{d.addEventListener("click",()=>te(d.dataset.seat,d.dataset.bookingId))})},te=(t,e)=>{const s=document.getElementById("seatModal"),o=document.getElementById("seatModalTitle"),n=document.getElementById("seatModalBody");if(!s||!o||!n)return;o.textContent=e?`Ghế ${t} — ${w.find(c=>String(c.id)===String(e))?.name||"?"}`:`Ghế ${t} — Chưa có khách`;const r=w.find(c=>String(c.id)===String(e));let a="";r&&(a+=`<div class="mb-3 p-3 bg-orange-50 border border-orange-200 rounded-xl text-sm">
        ...
        `,r.querySelector(".remove-service-btn").addEventListener("click",()=>{r.remove(),s.querySelectorAll(".service-row").length===0&&(o&&o.classList.remove("hidden"),n&&n.classList.add("hidden")),window.updateServicesTotal()}),r.querySelector(".service-price-input").addEventListener("input",window.updateServicesTotal),s.appendChild(r),window.updateServicesTotal()};const oe=document.getElementById("addServiceBtn");oe&&oe.addEventListener("click",()=>window.addServiceRow()),window.actionEdit=async t=>{try{const e=w.find(i=>i.id==t);if(!e){alert("Không tìm thấy thông tin khách hàng!");return}document.getElementById("edit-id").value=t,document.getElementById("edit-name").value=e.name||"",document.getElementById("edit-phone").value=e.phone||"",document.getElementById("edit-medal-name").value=e.medal_name||"",document.getElementById("edit-tour").value=e.tour||"",se("edit-date",e.tour||"",e.date||"",e.schedule_id||null),(i=>{let b=i.dob||"";if(b&&b.includes("/")){const f=b.split("/");f.length===3&&(b=`${f[2]}-${f[1].padStart(2,"0")}-${f[0].padStart(2,"0")}`)}document.getElementById("edit-dob").value=b,document.getElementById("edit-gender").value=i.gender||"Khác",document.getElementById("edit-allergy").value=i.allergy||i.medical_notes||"",document.getElementById("edit-address").value=i.address||"",document.getElementById("edit-diet").value=i.diet||i.dietary||"",document.getElementById("edit-trekking-pole").value=i.trekking_pole||"Không",document.getElementById("edit-id-card").value=i.id_card||i.cccd||""})(e),document.getElementById("edit-status").value=e.status||"Chờ xác nhận cọc",document.getElementById("edit-commitments").checked=!!e.commitments,document.getElementById("edit-special").value=e.special||"";try{const i=localStorage.getItem("csr_user");if(i&&JSON.parse(i).role==="admin"){const f=document.getElementById("edit-sale-container");f&&f.classList.remove("hidden");const g=document.getElementById("edit-sale");g&&(g.value=e.sale_id||"")}}catch{}const o=(parseInt(e.total_price)||0)+(parseInt(e.discount)||0);document.getElementById("edit-total").value=o,document.getElementById("edit-discount").value=e.discount||0,document.getElementById("edit-deposit").value=e.deposit||0;const n=parseInt(e.deposit_required);document.getElementById("edit-deposit-required").value=isNaN(n)?1e6:n,window.updateEditRemaining();const r=document.getElementById("servicesContainer"),a=document.getElementById("emptyServicesMsg"),u=document.getElementById("servicesTotalRow"),c=document.getElementById("presetServicesBlock"),d=document.getElementById("presetServicesList"),l=document.getElementById("servicesDivider");let m=[];try{if(e.services_booked){const i=typeof e.services_booked=="string"?JSON.parse(e.services_booked):e.services_booked;Array.isArray(i)&&(m=i)}}catch{}if(d&&c){d.innerHTML="";const i=N.find(f=>f.name===e.tour);let b=[];try{if(i?.services){const f=typeof i.services=="string"?JSON.parse(i.services):i.services;Array.isArray(f)&&(b=f)}}catch{}b.length>0?(c.classList.remove("hidden"),l&&l.classList.remove("hidden"),b.forEach(f=>{const g=m.some(y=>(y.label||y.name||"").toLowerCase()===(f.label||"").toLowerCase()),v=document.createElement("label");v.className="preset-service-card flex items-center gap-3 p-3 rounded-xl border-2 cursor-pointer transition-all "+(g?"border-green-400 bg-green-50":"border-gray-100 bg-gray-50"),v.innerHTML=`
                        ...
                        `,v.querySelector(".preset-service-check").addEventListener("change",y=>{v.className="preset-service-card flex items-center gap-3 p-3 rounded-xl border-2 cursor-pointer transition-all "+(y.target.checked?"border-green-400 bg-green-50":"border-gray-100 bg-gray-50"),window.updateServicesTotal()}),d.appendChild(v)})):(c.classList.add("hidden"),l&&l.classList.add("hidden"))}const p=document.getElementById("edit-pickup-point");if(p){const i=N.find(x=>x.name===e.tour);let b=[];try{if(i?.pickup_points){const x=typeof i.pickup_points=="string"?JSON.parse(i.pickup_points):i.pickup_points;Array.isArray(x)&&(b=x)}}catch{}p.innerHTML='<option value="">— Chưa chọn —</option>',b.forEach(x=>{const S=(x.label||x.name||x)+(x.time?` (${x.time})`:""),_=x.label||x.name||x,C=document.createElement("option");C.value=_,C.textContent=S,p.appendChild(C)});const f=document.createElement("option");f.value="__custom__",f.textContent="✏️ Khác (nhập tay)",p.appendChild(f);const g=document.getElementById("edit-pickup-custom"),v=x=>{g&&(x==="__custom__"?(g.classList.remove("hidden"),g.focus()):(g.classList.add("hidden"),g.value=""))},y=e.pickup_point||"";y&&(b.some(S=>(S.label||S.name||S)===y)?p.value=y:(p.value="__custom__",g&&(g.value=y,g.classList.remove("hidden")))),p.addEventListener("change",()=>v(p.value))}if(r){r.querySelectorAll(".service-row").forEach(g=>g.remove());const i=N.find(g=>g.name===e.tour);let b=new Set;try{const g=i?.services,v=g?typeof g=="string"?JSON.parse(g):g:[];Array.isArray(v)&&v.forEach(y=>b.add((y.label||"").toLowerCase()))}catch{}const f=m.filter(g=>!b.has((g.label||g.name||"").toLowerCase()));f.length>0?(a&&a.classList.add("hidden"),u&&u.classList.remove("hidden"),f.forEach(g=>window.addServiceRow(g.label||g.name||"",g.price||0))):a&&a.classList.remove("hidden"),window.updateServicesTotal()}const h=document.getElementById("editModal"),E=document.getElementById("editModalContent");h&&E&&(h.classList.remove("hidden"),setTimeout(()=>{h.classList.add("opacity-100"),E.classList.remove("scale-95","translate-y-4"),E.classList.add("scale-100","translate-y-0")},10))}catch(e){console.error("Lỗi khi mở form Edit:",e),alert("Lỗi khi mở form Edit: "+e.message)}};const ae=document.getElementById("editForm");ae&&ae.addEventListener("submit",async t=>{t.preventDefault();const e=t.target.querySelector('button[type="submit"]'),s=e.textContent;e.textContent="Đang lưu cập nhật...",e.disabled=!0;try{const o=document.getElementById("edit-id").value,n=document.getElementById("edit-name").value,r=document.getElementById("edit-phone").value,a=document.getElementById("edit-medal-name").value,u=document.getElementById("edit-tour").value,c=document.getElementById("edit-date").value;let d=c,l;if(c&&c.includes("::")){const k=c.split("::");l=parseInt(k[0])||null,d=k[1]}const m=document.getElementById("edit-dob").value,p=document.getElementById("edit-gender").value,h=document.getElementById("edit-status").value,E=document.getElementById("edit-allergy").value,i=document.getElementById("edit-address").value,b=document.getElementById("edit-diet").value,f=document.getElementById("edit-trekking-pole").value,g=document.getElementById("edit-commitments").checked,v=document.getElementById("edit-special").value,y=document.getElementById("edit-id-card").value,x=document.getElementById("edit-pickup-point"),S=document.getElementById("edit-pickup-custom"),_=x?.value==="__custom__"?S?.value?.trim()||"":x?.value||"",C=[];document.querySelectorAll(".preset-service-check:checked").forEach(k=>{C.push({label:k.dataset.label,price:parseInt(k.dataset.price)||0})}),document.querySelectorAll(".service-row").forEach(k=>{const G=(k.querySelector(".service-label-input")?.value||"").trim(),De=parseInt(k.querySelector(".service-price-input")?.value)||0;G&&C.push({label:G,price:De})});const Te=C.length>0?JSON.stringify(C):null,W=document.getElementById("edit-sale");let R,U;if(W&&!W.parentElement.classList.contains("hidden")&&(R=W.value||null,U="Website",R)){const k=J.find(G=>String(G.id)===String(R));k&&(U=k.full_name||k.fullName)}const Le=parseInt(document.getElementById("edit-total").value)||0,le=parseInt(document.getElementById("edit-discount").value)||0,$e=parseInt(document.getElementById("edit-deposit").value)||0,ie=parseInt(document.getElementById("edit-deposit-required").value),_e=isNaN(ie)?1e6:ie,Me={id:o,name:n,phone:r,medal_name:a,tour:u,date:d,dob:m,gender:p,status:h,allergy:E,address:i,diet:b,trekking_pole:f,commitments:g,special:v,id_card:y,pickup_point:_,services_booked:Te,sale_id:R,sale_name:U,total_price:Le-le,discount:le,deposit:$e,deposit_required:_e,...l!==void 0?{schedule_id:l}:{}},de=await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(Me)});if(de.ok)alert("✅ Đã cập nhật thành công Chi tiết Đơn hàng!"),window.closeEditModal(),K();else{const k=await de.json();throw new Error(k.error||"Gặp sự cố khi Cập nhật API.")}}catch(o){alert("❌ Lỗi Cập nhật: "+o.message)}finally{e.textContent=s,e.disabled=!1}}),ve(),ye(),Promise.all([K(),Ce()]).then(()=>A()).catch(()=>A())};export{Re as afterRender,Ae as render};
