import{S as $e,H as _e}from"./Header-CtRUqTZE.js";console.log("Bookings V3 - Cache Busted!");const De=()=>`
    ...
    `,Ne=()=>{let H=[],B="consult",D=[],te=[],J=[];const Ce=async()=>{try{const[n,e,o]=await Promise.all([fetch("/api/tours"),fetch("/api/schedules"),fetch("/api/users")]);D=n.ok?await n.json():[],te=e.ok?await e.json():[],J=o.ok?await o.json():[],U("addTourName"),U("edit-tour"),U("wl-tour"),Ie("edit-sale")}catch(n){console.error("Lỗi tải tours/schedules:",n)}},Ie=n=>{const e=document.getElementById(n);e&&(e.innerHTML='<option value="">-- Website --</option>',J.forEach(o=>{const a=document.createElement("option");a.value=o.id,a.textContent=`${o.full_name||o.fullName} (${o.role})`,e.appendChild(a)}))},U=n=>{const e=document.getElementById(n);if(!e)return;const o=e.value,a=e.options[0].outerHTML;e.innerHTML=a,D.filter(c=>c.is_visible!==!1).forEach(c=>{const u=document.createElement("option");u.value=c.name,u.textContent=c.name,c.name===o&&(u.selected=!0),e.appendChild(u)})},P=(n,e,o,a)=>{const c=document.getElementById(n);if(!c)return;if(c.innerHTML='<option value="">-- Chọn Lịch --</option>',!e){c.innerHTML='<option value="">-- Chọn Tour trước --</option>';return}const u=te.filter(s=>s.tour_name===e||e.includes(s.tour_name)||s.tour_name.includes(e));if(u.length===0){c.innerHTML='<option value="">Chưa có lịch cho tour này</option>';return}u.sort((s,i)=>new Date(s.start_date)-new Date(i.start_date)).forEach(s=>{const i=parseInt(s.booked_count)||0,m=(s.slots||0)-i,T=new Date(s.start_date),p=s.end_date?new Date(s.end_date):null,C=L=>`${L.getDate().toString().padStart(2,"0")}/${(L.getMonth()+1).toString().padStart(2,"0")}`;let I=C(T);p&&(I+=" - "+C(p)),s.group_label&&(I+=` · ${s.group_label}`),I+=` (${m>=0?m:0} chỗ trống)`;const v=document.createElement("option"),h=`${T.getDate().toString().padStart(2,"0")}/${(T.getMonth()+1).toString().padStart(2,"0")}/${T.getFullYear()}`;v.value=`${s.id}::${h}`,v.dataset.scheduleId=s.id,v.dataset.date=h,v.textContent=I,m<=0&&(v.disabled=!0,v.textContent=I.replace("chỗ trống","HẾT CHỖ"));let y=o||"";if(y.includes("-")&&y.length>=10){const L=y.substring(0,10).split("-");L.length===3&&(y=`${L[2]}/${L[1]}/${L[0]}`)}(a&&String(s.id)===String(a)||!a&&y&&h===y)&&(v.selected=!0),c.appendChild(v)})},ne=document.getElementById("addTourName");ne&&ne.addEventListener("change",n=>{const e=n.target.value;P("addTourDate",e);const o=D.find(c=>c.name===e),a=document.getElementById("addTourPrice");if(o&&a){const c=parseInt(o.price)||0;a.value=c;const u=document.getElementById("addDiscount"),s=document.getElementById("addTotalPriceDisplay"),i=parseInt(u?u.value:0)||0,m=c-i;s&&(s.textContent=(m>0?m:0).toLocaleString("vi-VN")+"đ")}});const oe=document.getElementById("edit-tour");oe&&oe.addEventListener("change",n=>{const e=n.target.value;P("edit-date",e);const o=D.find(c=>c.name===e),a=document.getElementById("edit-total");o&&a&&(parseInt(a.value)===0||!a.value)&&(a.value=parseInt(o.price)||0,typeof window.updateEditRemaining=="function"&&window.updateEditRemaining())});const ae=document.getElementById("wl-tour");ae&&ae.addEventListener("change",n=>{const e=n.target.value;P("wl-date",e)}),Ce();const R=()=>{const n=document.getElementById("bookingsTableHead"),e=document.getElementById("bookingsTableBody");if(!e||!n)return;const o=B==="upcoming"||B==="ready"||B==="completed";o?n.innerHTML=`
            ...
            `;const a=document.getElementById("filterSearch")?document.getElementById("filterSearch").value.toLowerCase():"",c=document.getElementById("filterTour")?document.getElementById("filterTour").value:"",u=document.getElementById("filterDate")?document.getElementById("filterDate").value:"",s=document.getElementById("filterSale")?document.getElementById("filterSale").value:"",i=document.getElementById("filterStatus")?document.getElementById("filterStatus").value:"";let m=H.filter(t=>{let l=!1;const d=parseInt(t.total_price)>0&&parseInt(t.total_price)===parseInt(t.deposit),r=b=>{if(!b)return!1;try{const E=b.split("/");if(E.length===3){const k=new Date(`${E[2]}-${E[1]}-${E[0]}T00:00:00`),x=new Date;return x.setHours(0,0,0,0),k.getTime()<x.getTime()}else if(b.includes("-")){const k=new Date(`${b}T00:00:00`),x=new Date;return x.setHours(0,0,0,0),k.getTime()<x.getTime()}}catch{}return!1},g=t.status||"",f=!r(t.date);return B==="consult"?l=g==="Chờ tư vấn"&&f:B==="pending"?l=(g==="Chờ cọc"||g==="Chờ xác nhận cọc"||g==="")&&f:B==="upcoming"?l=(g==="Đã cọc"||g==="Đã cọc (Chờ đi)")&&!d&&f:B==="ready"?l=(d||g==="Hoàn thành")&&f&&g!=="Đã hủy"&&g!=="Bảo lưu":B==="completed"&&(l=!f||g==="Đã đi"||g==="Đã hủy"||g==="Bảo lưu"),!(!l||c&&t.tour!==c||u&&X(t.date)!==u||s&&s!=="null"&&t.sale_name!==s||s==="null"&&t.sale_name&&t.sale_name!=="Website"||a&&!`${t.name||""} ${t.phone||""} ${t.customer_id||""} ${t.id||""} csr${t.id||""} #csr${t.id||""}`.toLowerCase().includes(a)||i&&t.status!==i)});const T=document.getElementById("statTitle1"),p=document.getElementById("statTitle2"),C=document.getElementById("statTitle3"),I=document.getElementById("statTotalCustomers"),v=document.getElementById("statTotalRevenue"),h=document.getElementById("statTotalCollected");let y=m.length;if(B==="consult"){const t=document.getElementById("addWaitingBtn"),l=document.getElementById("addBookingBtn");t&&t.classList.remove("hidden"),l&&l.classList.add("hidden"),T&&(T.textContent="Tổng Đăng Ký Chờ Lịch"),p&&(p.textContent="Chưa Được Liên Hệ"),C&&(C.textContent="Đã Có Sale Phụ Trách");let d=0,r=0;m.forEach(g=>{!g.sale_id||g.sale_id==="null"||!g.sale_name||g.sale_name==="Website"||g.sale_name==="null"?d++:r++}),I&&(I.textContent=y),v&&(v.textContent=d+" Người"),h&&(h.textContent=r+" Người")}else if(B==="pending"){const t=document.getElementById("addWaitingBtn"),l=document.getElementById("addBookingBtn");t&&t.classList.add("hidden"),l&&l.classList.remove("hidden"),T&&(T.textContent="Tổng Khách (Chờ Cọc)"),p&&(p.textContent="Khách Chưa Tư Vấn (Mới)"),C&&(C.textContent="Đã Có Sale Giữ Chỗ");let d=0,r=0;m.forEach(g=>{!g.sale_id||g.sale_id==="null"||!g.sale_name||g.sale_name==="Website"||g.sale_name==="null"?d++:r++}),I&&(I.textContent=y),v&&(v.textContent=d+" Đơn"),h&&(h.textContent=r+" Đơn")}else if(B==="upcoming"){T&&(T.textContent="Tổng Số Khách"),p&&(p.textContent="Chưa Xác Nhận Cọc"),C&&(C.textContent="Chờ Thanh Toán Còn Lại");let t=0,l=0;m.forEach(d=>{d.status==="Chờ xác nhận cọc"?t++:parseInt(d.deposit)>0&&parseInt(d.total_price)>parseInt(d.deposit)&&l++}),I&&(I.textContent=y),v&&(v.textContent=t+" Khách"),h&&(h.textContent=l+" Khách")}else if(B==="ready"){T&&(T.textContent="Khách Sẵn Sàng (Full)"),p&&(p.textContent="Tổng Doanh Thu Tab"),C&&(C.textContent="Thực Thu (Full Tận Nơi)");let t=0,l=0;m.forEach(d=>{t+=parseInt(d.total_price)||0,l+=parseInt(d.deposit)||0}),I&&(I.textContent=y),v&&(v.textContent=t>0?t.toLocaleString("vi-VN")+"đ":"0đ"),h&&(h.textContent=l>0?l.toLocaleString("vi-VN")+"đ":"0đ")}else{T&&(T.textContent="Khách Đã Xong (Lịch Sử)"),p&&(p.textContent="Tổng Doanh Thu Tab"),C&&(C.textContent="Thực Thu (Đã Đóng)");let t=0,l=0;m.forEach(d=>{t+=parseInt(d.total_price)||0,l+=parseInt(d.deposit)||0}),I&&(I.textContent=y),v&&(v.textContent=t>0?t.toLocaleString("vi-VN")+"đ":"0đ"),h&&(h.textContent=l>0?l.toLocaleString("vi-VN")+"đ":"0đ")}const L=o?8:4;if(!m||m.length===0){let t=B==="consult"?"Chưa có khách hàng tư vấn nào.":B==="pending"?"Chưa có Khách chờ cọc.":B==="upcoming"?"Chưa có Khách nào Đã Cọc.":B==="ready"?"Chưa có khách sẵn sàng.":"Danh sách rỗng.";e.innerHTML=`<tr><td colspan="${L}" class="p-8 text-center text-gray-500">${t}</td></tr>`;return}e.innerHTML=m.map(t=>{const l=B==="upcoming"||B==="ready"?"bg-green-50/20 hover:bg-green-50 transition-colors":"hover:bg-gray-100 transition-colors";if(o){let d=parseInt(t.total_price)||0;if(d===0&&t.tour&&D&&D.length>0){const w=D.find(N=>N.name&&t.tour&&N.name.toLowerCase().trim()===t.tour.toLowerCase().trim());w&&parseInt(w.price)>0&&(d=parseInt(w.price))}const r=parseInt(t.deposit)||0,g=d-r,f=d>0?d.toLocaleString("vi-VN")+"đ":"Chưa định giá",b=r>0?r.toLocaleString("vi-VN")+"đ":"0đ",E=g>0?g.toLocaleString("vi-VN")+"đ":g===0&&d>0?"Đã thu trọn":"-";let k="";t.status==="Đã hủy"?k='<span class="bg-red-50 text-red-500 border border-red-200 px-2 py-0.5 rounded text-xs block w-full text-center font-bold">❌ Đã hủy</span>':t.status==="Bảo lưu"?k='<span class="bg-gray-100 text-gray-500 border border-gray-300 px-2 py-0.5 rounded text-xs block w-full text-center font-bold">⏸️ Bảo lưu</span>':t.status==="Hoàn tất phí"?k='<span class="bg-emerald-100 text-emerald-700 border border-emerald-200 px-2 py-0.5 rounded text-xs font-bold block w-full text-center">Hoàn Tất Phí</span>':t.status==="Hoàn thành"||t.status==="Đã đi"?k='<span class="bg-violet-100 text-violet-700 border border-violet-200 px-2 py-0.5 rounded text-xs block w-full text-center font-bold">Hoàn Thành</span>':t.status==="Chờ xác nhận cọc"?k=`<button class="action-btn confirm-deposit-btn bg-csr-orange text-white px-3 py-1.5 rounded text-xs font-bold shadow-sm hover:bg-[#d65503] w-full" data-id="${t.id}">Xác nhận cọc</button>`:r>0&&g===0?k='<span class="bg-green-100 text-green-700 border border-green-200 px-2 py-0.5 rounded text-xs font-medium block w-full text-center">Hoàn tất phí</span>':k='<span class="bg-blue-100 text-blue-700 border border-blue-200 px-2 py-0.5 rounded text-xs font-medium block w-full text-center">Đã Cọc</span>';let x="";return r>0&&(x=`<a href="/invoice.html?id=${t.id}" target="_blank" class="mt-1.5 block text-center text-[11px] text-blue-600 hover:text-blue-800 hover:underline font-medium" onclick="event.stopPropagation()">🧾 Xem hóa đơn</a>`),`
                <tr class="${l} cursor-pointer row-clickable block md:table-row p-4 md:p-0 mb-4 md:mb-0 glass-panel md:glass-none border-l-4 border-green-500 md:border-none relative" data-id="${t.id}">
                    ...
                    <td class="p-4 align-top text-center hidden md:table-cell">${k}${x}</td>
    ...
    `}}).join(""),document.querySelectorAll(".claim-btn").forEach(t=>{t.addEventListener("click",async l=>{l.stopPropagation();const d=t.getAttribute("data-id"),r=localStorage.getItem("csr_user");let g=null,f="Admin";if(r){const b=JSON.parse(r);g=b.id,f=b.fullName||b.full_name||"Admin"}t.textContent="Đang xử lý...",t.disabled=!0;try{if((await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:d,sale_id:g,sale_name:f})})).ok){const E=H.find(k=>k.id==d);E&&(E.sale_id=g,E.sale_name=f),R()}else alert("Lỗi khi nhận khách. Vui lòng thử lại."),t.textContent="Nhận khách",t.disabled=!1}catch(b){console.error("Lỗi nhận khách:",b),t.textContent="Nhận khách",t.disabled=!1}})})},Ee=()=>{const n=new Set,e=new Set,o=new Set;H.forEach(s=>{if(s.tour&&n.add(s.tour),s.date){const i=X(s.date);i&&e.add(i)}s.sale_name&&s.sale_name!=="Website"&&s.sale_name!=="null"&&o.add(s.sale_name.trim())});const a=document.getElementById("filterTour"),c=document.getElementById("filterDate"),u=document.getElementById("filterSale");if(a){const s=a.value;a.innerHTML='<option value="">Tất cả Tour</option>',[...n].sort().forEach(i=>a.innerHTML+=`<option value="${i}">${i}</option>`),a.value=s}if(c){const s=c.value;c.innerHTML='<option value="">Tất cả Ngày</option>',[...e].sort().forEach(i=>c.innerHTML+=`<option value="${i}">${i}</option>`),c.value=s}if(u){const s=u.value;u.innerHTML='<option value="">Tất cả Sale</option>',u.innerHTML+='<option value="null">Nguồn Website</option>',[...o].sort().forEach(i=>{u.innerHTML+=`<option value="${i}">${i}</option>`}),u.value=s}};function X(n){if(!n)return"";if(/^\d{1,2}\/\d{1,2}\s*-\s*\d{1,2}\/\d{1,2}$/.test(n))return n.replace(/\s+/g,"");try{if(/^\d{4}-\d{2}-\d{2}$/.test(n)){const[e,o,a]=n.split("-"),c=`${parseInt(a)}/${parseInt(o)}`;return`${c}-${c}`}if(/^\d{1,2}\/\d{1,2}\/\d{4}$/.test(n)){const[e,o,a]=n.split("/"),c=`${parseInt(e)}/${parseInt(o)}`;return`${c}-${c}`}if(n.includes("-"))return n.split("-").map(o=>{const a=o.trim();if(/\/\d{4}$/.test(a)){const[c,u]=a.split("/");return`${parseInt(c)}/${parseInt(u)}`}return a}).join("-")}catch{console.warn("Date normalization failed for:",n)}return n}const M=async()=>{try{H=await(await fetch("/api/bookings")).json(),H.sort((e,o)=>parseInt(o.id)-parseInt(e.id)),Ee(),R()}catch(n){console.error("Lỗi parse list đơn hàng:",n);const e=document.getElementById("bookingsTableBody");e&&(e.innerHTML='<tr><td colspan="4" class="p-8 text-center text-red-500">Lỗi lấy dữ liệu từ Server.</td></tr>')}};M(),["filterSearch","filterTour","filterDate","filterSale","filterStatus"].forEach(n=>{const e=document.getElementById(n);e&&(n==="filterSearch"?e.addEventListener("input",R):e.addEventListener("change",R))});const se=document.getElementById("exportExcelBtn");se&&se.addEventListener("click",()=>{const n=document.getElementById("filterSearch")?document.getElementById("filterSearch").value.toLowerCase():"",e=document.getElementById("filterTour")?document.getElementById("filterTour").value:"",o=document.getElementById("filterDate")?document.getElementById("filterDate").value:"",a=document.getElementById("filterSale")?document.getElementById("filterSale").value:"";let c=H.filter(p=>{let C=!1;const I=parseInt(p.total_price)>0&&parseInt(p.total_price)===parseInt(p.deposit),v=y=>{if(!y)return!1;try{const L=y.split("/");if(L.length===3){const t=new Date(`${L[2]}-${L[1]}-${L[0]}T00:00:00`),l=new Date;return l.setHours(0,0,0,0),t.getTime()<l.getTime()}else if(y.includes("-")){const t=new Date(`${y}T00:00:00`),l=new Date;return l.setHours(0,0,0,0),t.getTime()<l.getTime()}}catch{}return!1},h=(p.status==="Hoàn thành"||p.status==="Đã đi"||I)&&v(p.date);return B==="consult"?C=p.status==="Chờ tư vấn"&&!h:B==="pending"?C=(!p.status||p.status==="Chờ cọc")&&!h:B==="upcoming"?C=p.status&&p.status!=="Chờ tư vấn"&&p.status!=="Chờ cọc"&&!I&&p.status!=="Hoàn thành"&&!h:B==="ready"?C=(I||p.status==="Hoàn thành")&&!h:B==="completed"&&(C=h),!(!C||e&&p.tour!==e||o&&X(p.date)!==o||a&&a!=="null"&&p.sale_name!==a||a==="null"&&p.sale_name&&p.sale_name!=="Website"||n&&!`${p.name||""} ${p.phone||""} ${p.customer_id||""} ${p.id||""} csr${p.id||""} #csr${p.id||""}`.toLowerCase().includes(n))});if(c.length===0){alert("Danh sách rỗng. Không có dữ liệu để xuất.");return}let u=`
        ...
        `;document.getElementById("ramActions").innerHTML=e,document.getElementById("rowActionModal").classList.remove("hidden")},window.addEventListener("click",n=>{const e=document.getElementById("rowActionModal");e&&n.target===e&&e.classList.add("hidden")}),window.actionView=n=>{document.getElementById("rowActionModal").classList.add("hidden");const e=H.find(t=>t.id==n);if(!e)return;const o=t=>parseInt(t||0).toLocaleString("vi-VN")+"đ",a=t=>{if(!t)return"—";if(/^\d{4}-\d{2}-\d{2}/.test(t)){const[l,d,r]=t.split("-");return`${r}/${d}/${l}`}return t},c=parseInt(e.total_price)||0,u=parseInt(e.deposit)||0,s=parseInt(e.discount)||0,i=c-u;let m=[];try{if(e.services_booked){const t=typeof e.services_booked=="string"?JSON.parse(e.services_booked):e.services_booked;Array.isArray(t)&&(m=t)}}catch{}const T=m.reduce((t,l)=>t+(parseInt(l.price)||0),0),C={"Chờ tư vấn":"Chờ lịch"}[e.status]||e.status,v={"Chờ tư vấn":"bg-purple-100 text-purple-700 border-purple-200","Chờ cọc":"bg-yellow-100 text-yellow-700 border-yellow-200","Chờ xác nhận cọc":"bg-orange-100 text-orange-700 border-orange-200","Đã cọc":"bg-blue-100 text-blue-700 border-blue-200","Đã cọc (Chờ đi)":"bg-blue-100 text-blue-700 border-blue-200","Hoàn tất phí":"bg-emerald-100 text-emerald-700 border-emerald-200","Hoàn thành":"bg-violet-100 text-violet-700 border-violet-200","Bảo lưu":"bg-gray-100 text-gray-700 border-gray-300","Đã hủy":"bg-red-50 text-red-600 border-red-200 line-through"}[e.status]||"bg-gray-100 text-gray-700 border-gray-200",h=(t,l,d="")=>`<div class="flex items-start gap-3 py-2.5 border-b border-gray-100 last:border-none">
            ...
            </div>`,y=(t,l,d="bg-blue-50 text-blue-600")=>`<div class="flex items-center gap-2.5 mb-3">
        ...
        <div class="space-y-4 pb-2">
            ...
            <div class="bg-gradient-to-br from-orange-50 to-amber-50 border border-orange-100 rounded-2xl p-4">
                ...
                <div class="grid grid-cols-2 gap-x-4">
                    ...
                    ${h("Trạng Thái",`<span class="inline-block px-2.5 py-1 rounded-full text-xs font-bold border ${v}">${C||"Chưa rõ"}</span>`)}
            ...
            <div class="bg-gradient-to-br from-csr-orange to-orange-600 rounded-2xl p-4 text-white shadow-lg shadow-orange-500/20">
                ...
                <div class="space-y-2 text-sm">
                    ...
                    <div class="flex justify-between items-center py-1.5 border-b border-white/10">
                        ...
                        <span class="font-bold text-green-100">+ ${o(T)}</span>
                    </div>`:""}
            ...
            ${(()=>{const t=e.phone;if(!t)return"";const l=H.filter(x=>x.phone===t&&!x.status?.includes("hủy")&&!x.status?.includes("Hủy")).sort((x,w)=>new Date(w.created_at||0)-new Date(x.created_at||0)),d=l.length,r=l.reduce((x,w)=>x+(parseInt(w.total_price)||0),0);let g,f,b;d>=5?(g="KHÁCH VIP",f="bg-purple-100 text-purple-700 border-purple-300",b="👑"):d>=2?(g="KHÁCH THÂN THIẾT",f="bg-green-100 text-green-700 border-green-300",b="🌟"):(g="KHÁCH MỚI",f="bg-gray-100 text-gray-500 border-gray-300",b="👋");const E=l.filter(x=>x.id!=n),k=E.length>0?E.slice(0,5).map((x,w)=>{const N=x.status?.includes("Hoàn")?"bg-green-100 text-green-600":x.status?.includes("cọc")||x.status?.includes("Cọc")?"bg-blue-100 text-blue-600":"bg-gray-100 text-gray-500";return`<div class="flex items-center gap-3 py-2 border-b border-gray-100 last:border-none">
                    ...
                    <div class="mb-3">
                        ...
                        <div class="bg-white rounded-xl border border-gray-100 px-3">${k}</div>
                    </div>`:k}
...
Dạ mình thanh toán ở link này giúp em nhé: ${(window.location.hostname==="localhost"||window.location.hostname==="127.0.0.1"?"http://localhost:8888":window.location.origin)+`/pay?id=${o}`}`;navigator.clipboard.writeText(m).then(()=>{const T=e.textContent;e.textContent="✅ Đã copy!",e.classList.remove("bg-blue-500","hover:bg-blue-600"),e.classList.add("bg-green-500"),setTimeout(()=>{e.textContent=T,e.classList.add("bg-blue-500","hover:bg-blue-600"),e.classList.remove("bg-green-500")},2e3)}).catch(()=>{alert(`🔗 Nội dung đã chuẩn bị (copy thủ công):
...
`+m)})}else e.classList.contains("view-btn")?(document.getElementById("rowActionModal").classList.add("hidden"),window.actionView(o)):e.classList.contains("edit-btn")&&window.actionEdit(o)});const q=document.getElementById("smartSearch"),_=document.getElementById("searchSuggestions");if(q){let n;q.addEventListener("input",e=>{const o=e.target.value.toLowerCase().trim();if(clearTimeout(n),o.length<2){_&&_.classList.add("hidden");return}n=setTimeout(async()=>{try{const a=await fetch(`/api/admin_customers?action=search&keyword=${o}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({keyword:o})}),c=await a.json();if(a.ok&&c.success){const s=[c.data];_&&(_.innerHTML=s.map(i=>`
                        ...
                        `).join(""),_.classList.remove("hidden"))}else _&&(_.innerHTML='<div class="p-3 text-xs text-gray-400 italic text-center">Không tìm thấy khách hàng cũ</div>',_.classList.remove("hidden"))}catch(a){console.error("Search error:",a)}},300)}),_&&_.addEventListener("click",e=>{const o=e.target.closest(".suggestion-item");if(o){const a=document.getElementById("addFullName"),c=document.getElementById("addPhone"),u=document.getElementById("addCsrCode");a&&(a.value=o.getAttribute("data-name")),c&&(c.value=o.getAttribute("data-phone")),u&&(u.value=o.getAttribute("data-csr"));const s=document.getElementById("addDob"),i=document.getElementById("addGender"),m=document.getElementById("addIdCard"),T=document.getElementById("addAddress");s&&(s.value=o.getAttribute("data-dob")||""),i&&(i.value=o.getAttribute("data-gender")||"Khác"),m&&(m.value=o.getAttribute("data-cccd")||""),T&&(T.value=o.getAttribute("data-address")||"");const p=document.getElementById("addAllergy"),C=document.getElementById("addDiet"),I=document.getElementById("addTrekkingPole"),v=document.getElementById("addMedalName");p&&(p.value=o.getAttribute("data-allergy")||""),C&&(C.value=o.getAttribute("data-diet")||""),I&&(I.value=o.getAttribute("data-trekking_pole")||"Không"),v&&(v.value=o.getAttribute("data-medal-name")||""),window._selectedCustomer={fullName:o.getAttribute("data-name"),phone:o.getAttribute("data-phone"),csrCode:o.getAttribute("data-csr"),dob:o.getAttribute("data-dob")||"",gender:o.getAttribute("data-gender")||"",address:o.getAttribute("data-address")||"",id_card:o.getAttribute("data-cccd")||"",allergy:o.getAttribute("data-allergy")||"",diet:o.getAttribute("data-diet")||"",trekking_pole:o.getAttribute("data-trekking_pole")||"",medal_name:o.getAttribute("data-medal-name")||""},q.value=o.getAttribute("data-name"),_.classList.add("hidden");const h=`
                    ...
                    </div>`,y=document.getElementById("loyalty-alert");y&&y.remove(),q.parentElement.insertAdjacentHTML("afterend",h)}}),document.addEventListener("click",e=>{!q.contains(e.target)&&_&&!_.contains(e.target)&&_.classList.add("hidden")})}const G=document.getElementById("addTourPrice"),z=document.getElementById("addDiscount"),me=document.getElementById("addTotalPriceDisplay");document.getElementById("addTourName");const he=()=>{if(!G||!z||!me)return;const n=parseInt(G.value)||0,e=parseInt(z.value)||0,o=n-e;me.textContent=(o>0?o:0).toLocaleString("vi-VN")+"đ"};G&&G.addEventListener("input",he),z&&z.addEventListener("input",he);const be=document.getElementById("searchBtn");be&&be.addEventListener("click",n=>{n.preventDefault(),document.getElementById("smartSearch").value||alert("Vui lòng nhập Số Điện Thoại hoặc Mã #CSR")});const W=document.getElementById("bookingForm");W&&W.addEventListener("submit",async n=>{n.preventDefault();const e=n.target.querySelector('button[type="submit"]'),o=e.textContent;e.textContent="Đang lưu hệ thống...",e.disabled=!0;try{const a=document.getElementById("addFullName").value,c=document.getElementById("addPhone").value,u=document.getElementById("addTourName").value,s=document.getElementById("addTourDate").value;let i=null,m=s;if(s&&s.includes("::")){const S=s.split("::");i=parseInt(S[0])||null,m=S[1]}const T=document.getElementById("addDob").value,p=document.getElementById("addGender").value,C=document.getElementById("addIdCard").value,I=document.getElementById("addAddress").value,v=document.getElementById("addSpecial").value,h=document.getElementById("addAllergy").value,y=document.getElementById("addDiet").value,L=document.getElementById("addTrekkingPole").value,t=document.getElementById("addMedalName").value,l=parseInt(document.getElementById("addTourPrice").value)||0,d=parseInt(document.getElementById("addDiscount").value)||0,r=parseInt(document.getElementById("addDepositRequired").value),g=isNaN(r)?1e6:r,f=document.getElementById("addCsrCode").value||"",b=localStorage.getItem("csr_user");let E=null,k="Admin";if(b){const S=JSON.parse(b);E=S.id,k=S.fullName||S.full_name||"Admin"}const x=document.getElementById("editingBookingId")?document.getElementById("editingBookingId").value:"",w={name:a,phone:c,tour:u,date:m,status:"Chờ cọc",total_price:l-d,discount:d,deposit:0,sale_id:E,sale_name:k,customer_id:f,deposit_required:g,commitments:!0,schedule_id:i,dob:T,gender:p,address:I,id_card:C,special:v,allergy:h,diet:y,trekking_pole:L,medal_name:t},N=c.replace(/[^0-9]/g,"");window._selectedCustomer&&(window._selectedCustomer.phone||"").replace(/[^0-9]/g,"")===N&&(w.dob=w.dob||window._selectedCustomer.dob,w.gender=w.gender&&w.gender!=="Khác"?w.gender:window._selectedCustomer.gender||"Khác",w.address=w.address||window._selectedCustomer.address,w.id_card=w.id_card||window._selectedCustomer.id_card,w.allergy=window._selectedCustomer.allergy,w.diet=window._selectedCustomer.diet,w.trekking_pole=window._selectedCustomer.trekking_pole,w.medal_name=window._selectedCustomer.medal_name),x&&(w.id=x,delete w.status,delete w.deposit,delete w.sale_id,delete w.sale_name);const A=await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(w)});if(A.ok){alert(x?"✅ Đã lưu Cập nhật Đơn hàng!":"✅ Thêm Đơn Hàng Thành Công!"),window.closeModal(),W&&W.reset(),window._selectedCustomer=null;const S=document.getElementById("loyalty-alert");S&&S.remove(),document.getElementById("addTotalPriceDisplay")&&(document.getElementById("addTotalPriceDisplay").textContent="0đ"),typeof M=="function"?M():window.location.reload()}else{const S=await A.json();throw new Error(S.error||"Lỗi tạo Booking!")}}catch(a){alert("❌ "+a.message)}finally{e.textContent=o,e.disabled=!1}});const fe=document.getElementById("editForm");fe&&fe.addEventListener("submit",async n=>{n.preventDefault();const e=n.target.querySelector('button[type="submit"]'),o=e.textContent;e.textContent="Đang lưu cập nhật...",e.disabled=!0;try{const a=document.getElementById("edit-id").value,c=document.getElementById("edit-name").value,u=document.getElementById("edit-phone").value,s=document.getElementById("edit-medal-name").value,i=document.getElementById("edit-tour").value,m=document.getElementById("edit-date").value;let T=m,p;if(m&&m.includes("::")){const $=m.split("::");p=parseInt($[0])||null,T=$[1]}const C=document.getElementById("edit-dob").value,I=document.getElementById("edit-gender").value,v=document.getElementById("edit-status").value,h=document.getElementById("edit-allergy").value,y=document.getElementById("edit-address").value,L=document.getElementById("edit-diet").value,t=document.getElementById("edit-trekking-pole").value,l=document.getElementById("edit-commitments").checked,d=document.getElementById("edit-special").value,r=document.getElementById("edit-id-card"),g=r?r.value:"",f=document.getElementById("edit-pickup-point"),b=document.getElementById("edit-pickup-custom"),E=f?.value==="__custom__"?b?.value?.trim()||"":f?.value||"",k=[];document.querySelectorAll(".preset-service-check:checked").forEach($=>{const K=$.dataset.label,ee=parseInt($.dataset.price)||0;K&&k.push({label:K,price:ee})}),document.querySelectorAll(".service-row").forEach($=>{const K=($.querySelector(".service-label-input")?.value||"").trim(),ee=parseInt($.querySelector(".service-price-input")?.value)||0;K&&k.push({label:K,price:ee})});const w=k.length>0?JSON.stringify(k):null,N=document.getElementById("edit-sale");let A,S;if(N&&N.parentElement&&!N.parentElement.classList.contains("hidden")&&(A=N.value||null,S="Website",A)){const $=J.find(K=>String(K.id)===String(A));$&&(S=$.full_name||$.fullName)}const Be=parseInt(document.getElementById("edit-total").value)||0,ve=parseInt(document.getElementById("edit-discount").value)||0,ye=parseInt(document.getElementById("edit-deposit").value)||0,we=parseInt(document.getElementById("edit-deposit-required").value),Le=isNaN(we)?1e6:we;let ke=0;k.forEach($=>ke+=$.price);const Se=Be-ve+ke,F={id:a,name:c,phone:u,medal_name:s,tour:i,date:T,dob:C,gender:I,status:v,allergy:h,address:y,diet:L,trekking_pole:t,commitments:l,special:d,id_card:g,pickup_point:E,services_booked:w,sale_id:A,sale_name:S,total_price:Se,discount:ve,deposit:ye,deposit_required:Le,...p!==void 0?{schedule_id:p}:{}},Te=await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(F)});if(Te.ok)alert("✅ Đã cập nhật thành công Chi tiết Đơn hàng!"),(v.includes("Đã cọc")||F.total_price>0&&F.total_price===ye)&&pe(F),window.closeEditModal(),typeof M=="function"&&M();else{const $=await Te.json();throw new Error($.error||"Gặp sự cố khi Cập nhật API.")}}catch(a){alert("❌ Lỗi Cập nhật: "+a.message)}finally{e.textContent=o,e.disabled=!1}}),window.openWaitingModal=()=>{document.getElementById("waitingModal").classList.remove("hidden"),setTimeout(()=>{document.getElementById("waitingModal").classList.add("opacity-100");const n=document.getElementById("waitingModalContent");n&&(n.classList.remove("scale-95"),n.classList.add("scale-100"))},10)},window.closeWaitingModal=()=>{document.getElementById("waitingModal").classList.remove("opacity-100");const n=document.getElementById("waitingModalContent");n&&(n.classList.remove("scale-100"),n.classList.add("scale-95")),setTimeout(()=>{document.getElementById("waitingModal").classList.add("hidden")},300)};const xe=document.getElementById("addWaitingBtn");xe&&xe.addEventListener("click",()=>window.openWaitingModal());const Q=document.getElementById("waitingForm");Q&&Q.addEventListener("submit",async n=>{n.preventDefault();const e=document.getElementById("wl-submit");e.innerHTML="Đang lưu...",e.disabled=!0;const o=document.getElementById("wl-zalo").value?`Tên Zalo / Ghi Chú: ${document.getElementById("wl-zalo").value}`:"";let a={id:"null",name:"Website"};try{const s=localStorage.getItem("csr_user");if(s){const i=JSON.parse(s);a={id:i.id,name:i.full_name||i.fullName}}}catch{}const c={name:document.getElementById("wl-name").value,phone:document.getElementById("wl-phone").value,tour:document.getElementById("wl-tour").value,date:"Mở (Chưa có ngày)",status:"Chờ tư vấn",special:o,sale_id:a.id,sale_name:a.name},u=document.getElementById("wl-date");if(u&&u.value){const s=u.value.split("::");s.length===2&&s[0]&&s[0]!=="undefined"&&(c.schedule_id=s[0],c.date=s[1])}try{const i=await(await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(c)})).json();i.success?(window.closeWaitingModal(),Q.reset(),typeof M=="function"?M():(alert("✅ Đã thêm vào danh sách chờ!"),window.location.reload())):alert("Lỗi lưu danh sách: "+i.message)}catch(s){console.error(s),alert("Có lỗi xảy ra, vui lòng thử lại.")}finally{e.innerHTML="Lưu Vào Danh Sách",e.disabled=!1}})};export{Ne as afterRender,De as render};
