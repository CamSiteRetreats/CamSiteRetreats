import{S as $e,H as _e}from"./Header-CtRUqTZE.js";console.log("Bookings V3 - Cache Busted!");const De=()=>`
    ...
    `,Ne=()=>{let H=[],B="consult",D=[],te=[],J=[];const Te=async()=>{try{const[n,e,o]=await Promise.all([fetch("/api/tours"),fetch("/api/schedules"),fetch("/api/users")]);D=n.ok?await n.json():[],te=e.ok?await e.json():[],J=o.ok?await o.json():[],U("addTourName"),U("edit-tour"),U("wl-tour"),Ie("edit-sale")}catch(n){console.error("Lỗi tải tours/schedules:",n)}},Ie=n=>{const e=document.getElementById(n);e&&(e.innerHTML='<option value="">-- Website --</option>',J.forEach(o=>{const a=document.createElement("option");a.value=o.id,a.textContent=`${o.full_name||o.fullName} (${o.role})`,e.appendChild(a)}))},U=n=>{const e=document.getElementById(n);if(!e)return;const o=e.value,a=e.options[0].outerHTML;e.innerHTML=a,D.filter(c=>c.is_visible!==!1).forEach(c=>{const u=document.createElement("option");u.value=c.name,u.textContent=c.name,c.name===o&&(u.selected=!0),e.appendChild(u)})},P=(n,e,o,a)=>{const c=document.getElementById(n);if(!c)return;if(c.innerHTML='<option value="">-- Chọn Lịch --</option>',!e){c.innerHTML='<option value="">-- Chọn Tour trước --</option>';return}const u=te.filter(s=>s.tour_name===e||e.includes(s.tour_name)||s.tour_name.includes(e));if(u.length===0){c.innerHTML='<option value="">Chưa có lịch cho tour này</option>';return}u.sort((s,i)=>new Date(s.start_date)-new Date(i.start_date)).forEach(s=>{const i=parseInt(s.booked_count)||0,m=(s.slots||0)-i,k=new Date(s.start_date),p=s.end_date?new Date(s.end_date):null,C=L=>`${L.getDate().toString().padStart(2,"0")}/${(L.getMonth()+1).toString().padStart(2,"0")}`;let T=C(k);p&&(T+=" - "+C(p)),s.group_label&&(T+=` · ${s.group_label}`),T+=` (${m>=0?m:0} chỗ trống)`;const v=document.createElement("option"),h=`${k.getDate().toString().padStart(2,"0")}/${(k.getMonth()+1).toString().padStart(2,"0")}/${k.getFullYear()}`;v.value=`${s.id}::${h}`,v.dataset.scheduleId=s.id,v.dataset.date=h,v.textContent=T,m<=0&&(v.disabled=!0,v.textContent=T.replace("chỗ trống","HẾT CHỖ"));let y=o||"";if(y.includes("-")&&y.length>=10){const L=y.substring(0,10).split("-");L.length===3&&(y=`${L[2]}/${L[1]}/${L[0]}`)}(a&&String(s.id)===String(a)||!a&&y&&h===y)&&(v.selected=!0),c.appendChild(v)})},ne=document.getElementById("addTourName");ne&&ne.addEventListener("change",n=>{const e=n.target.value;P("addTourDate",e);const o=D.find(c=>c.name===e),a=document.getElementById("addTourPrice");if(o&&a){const c=parseInt(o.price)||0;a.value=c;const u=document.getElementById("addDiscount"),s=document.getElementById("addTotalPriceDisplay"),i=parseInt(u?u.value:0)||0,m=c-i;s&&(s.textContent=(m>0?m:0).toLocaleString("vi-VN")+"đ")}});const oe=document.getElementById("edit-tour");oe&&oe.addEventListener("change",n=>{const e=n.target.value;P("edit-date",e);const o=D.find(c=>c.name===e),a=document.getElementById("edit-total");o&&a&&(parseInt(a.value)===0||!a.value)&&(a.value=parseInt(o.price)||0,typeof window.updateEditRemaining=="function"&&window.updateEditRemaining())});const ae=document.getElementById("wl-tour");ae&&ae.addEventListener("change",n=>{const e=n.target.value;P("wl-date",e)}),Te();const R=()=>{const n=document.getElementById("bookingsTableHead"),e=document.getElementById("bookingsTableBody");if(!e||!n)return;const o=B==="upcoming"||B==="ready"||B==="completed";o?n.innerHTML=`
            ...
            `;const a=document.getElementById("filterSearch")?document.getElementById("filterSearch").value.toLowerCase():"",c=document.getElementById("filterTour")?document.getElementById("filterTour").value:"",u=document.getElementById("filterDate")?document.getElementById("filterDate").value:"",s=document.getElementById("filterSale")?document.getElementById("filterSale").value:"",i=document.getElementById("filterStatus")?document.getElementById("filterStatus").value:"";let m=H.filter(t=>{let l=!1;const d=parseInt(t.total_price)>0&&parseInt(t.total_price)===parseInt(t.deposit),r=b=>{if(!b)return!1;try{const E=b.split("/");if(E.length===3){const I=new Date(`${E[2]}-${E[1]}-${E[0]}T00:00:00`),x=new Date;return x.setHours(0,0,0,0),I.getTime()<x.getTime()}else if(b.includes("-")){const I=new Date(`${b}T00:00:00`),x=new Date;return x.setHours(0,0,0,0),I.getTime()<x.getTime()}}catch{}return!1},g=t.status||"",f=!r(t.date);return B==="consult"?l=g==="Chờ tư vấn"&&f:B==="pending"?l=(g==="Chờ cọc"||g==="Chờ xác nhận cọc"||g==="")&&f:B==="upcoming"?l=(g==="Đã cọc"||g==="Đã cọc (Chờ đi)")&&!d&&f:B==="ready"?l=(d||g==="Hoàn thành")&&f&&g!=="Đã hủy"&&g!=="Bảo lưu":B==="completed"&&(l=!f||g==="Đã đi"||g==="Đã hủy"||g==="Bảo lưu"),!(!l||c&&t.tour!==c||u&&X(t.date)!==u||s&&s!=="null"&&t.sale_name!==s||s==="null"&&t.sale_name&&t.sale_name!=="Website"||a&&!`${t.name||""} ${t.phone||""} ${t.customer_id||""} ${t.id||""} csr${t.id||""} #csr${t.id||""}`.toLowerCase().includes(a)||i&&t.status!==i)});const k=document.getElementById("statTitle1"),p=document.getElementById("statTitle2"),C=document.getElementById("statTitle3"),T=document.getElementById("statTotalCustomers"),v=document.getElementById("statTotalRevenue"),h=document.getElementById("statTotalCollected");let y=m.length;if(B==="consult"){const t=document.getElementById("addWaitingBtn"),l=document.getElementById("addBookingBtn");t&&t.classList.remove("hidden"),l&&l.classList.add("hidden"),k&&(k.textContent="Tổng Đăng Ký Chờ Lịch"),p&&(p.textContent="Chưa Được Liên Hệ"),C&&(C.textContent="Đã Có Sale Phụ Trách");let d=0,r=0;m.forEach(g=>{!g.sale_id||g.sale_id==="null"||!g.sale_name||g.sale_name==="Website"||g.sale_name==="null"?d++:r++}),T&&(T.textContent=y),v&&(v.textContent=d+" Người"),h&&(h.textContent=r+" Người")}else if(B==="pending"){const t=document.getElementById("addWaitingBtn"),l=document.getElementById("addBookingBtn");t&&t.classList.add("hidden"),l&&l.classList.remove("hidden"),k&&(k.textContent="Tổng Khách (Chờ Cọc)"),p&&(p.textContent="Khách Chưa Tư Vấn (Mới)"),C&&(C.textContent="Đã Có Sale Giữ Chỗ");let d=0,r=0;m.forEach(g=>{!g.sale_id||g.sale_id==="null"||!g.sale_name||g.sale_name==="Website"||g.sale_name==="null"?d++:r++}),T&&(T.textContent=y),v&&(v.textContent=d+" Đơn"),h&&(h.textContent=r+" Đơn")}else if(B==="upcoming"){k&&(k.textContent="Tổng Số Khách"),p&&(p.textContent="Chưa Xác Nhận Cọc"),C&&(C.textContent="Chờ Thanh Toán Còn Lại");let t=0,l=0;m.forEach(d=>{d.status==="Chờ xác nhận cọc"?t++:parseInt(d.deposit)>0&&parseInt(d.total_price)>parseInt(d.deposit)&&l++}),T&&(T.textContent=y),v&&(v.textContent=t+" Khách"),h&&(h.textContent=l+" Khách")}else if(B==="ready"){k&&(k.textContent="Khách Sẵn Sàng (Full)"),p&&(p.textContent="Tổng Doanh Thu Tab"),C&&(C.textContent="Thực Thu (Full Tận Nơi)");let t=0,l=0;m.forEach(d=>{t+=parseInt(d.total_price)||0,l+=parseInt(d.deposit)||0}),T&&(T.textContent=y),v&&(v.textContent=t>0?t.toLocaleString("vi-VN")+"đ":"0đ"),h&&(h.textContent=l>0?l.toLocaleString("vi-VN")+"đ":"0đ")}else{k&&(k.textContent="Khách Đã Xong (Lịch Sử)"),p&&(p.textContent="Tổng Doanh Thu Tab"),C&&(C.textContent="Thực Thu (Đã Đóng)");let t=0,l=0;m.forEach(d=>{t+=parseInt(d.total_price)||0,l+=parseInt(d.deposit)||0}),T&&(T.textContent=y),v&&(v.textContent=t>0?t.toLocaleString("vi-VN")+"đ":"0đ"),h&&(h.textContent=l>0?l.toLocaleString("vi-VN")+"đ":"0đ")}const L=o?8:4;if(!m||m.length===0){let t=B==="consult"?"Chưa có khách hàng tư vấn nào.":B==="pending"?"Chưa có Khách chờ cọc.":B==="upcoming"?"Chưa có Khách nào Đã Cọc.":B==="ready"?"Chưa có khách sẵn sàng.":"Danh sách rỗng.";e.innerHTML=`<tr><td colspan="${L}" class="p-8 text-center text-gray-500">${t}</td></tr>`;return}e.innerHTML=m.map(t=>{const l=B==="upcoming"||B==="ready"?"bg-green-50/20 hover:bg-green-50 transition-colors":"hover:bg-gray-100 transition-colors";if(o){let d=parseInt(t.total_price)||0;if(d===0&&t.tour&&D&&D.length>0){const w=D.find(N=>N.name&&t.tour&&N.name.toLowerCase().trim()===t.tour.toLowerCase().trim());w&&parseInt(w.price)>0&&(d=parseInt(w.price))}const r=parseInt(t.deposit)||0,g=d-r,f=d>0?d.toLocaleString("vi-VN")+"đ":"Chưa định giá",b=r>0?r.toLocaleString("vi-VN")+"đ":"0đ",E=g>0?g.toLocaleString("vi-VN")+"đ":g===0&&d>0?"Đã thu trọn":"-";let I="";t.status==="Đã hủy"?I='<span class="bg-red-50 text-red-500 border border-red-200 px-2 py-0.5 rounded text-xs block w-full text-center font-bold">❌ Đã hủy</span>':t.status==="Bảo lưu"?I='<span class="bg-gray-100 text-gray-500 border border-gray-300 px-2 py-0.5 rounded text-xs block w-full text-center font-bold">⏸️ Bảo lưu</span>':t.status==="Hoàn thành"||t.status==="Đã đi"?I='<span class="bg-gray-100 text-gray-600 border border-gray-200 px-2 py-0.5 rounded text-xs block w-full text-center">Hoàn thành</span>':t.status==="Chờ xác nhận cọc"?I=`<button class="action-btn confirm-deposit-btn bg-csr-orange text-white px-3 py-1.5 rounded text-xs font-bold shadow-sm hover:bg-[#d65503] w-full" data-id="${t.id}">Xác nhận cọc</button>`:r>0&&g===0?I='<span class="bg-green-100 text-green-700 border border-green-200 px-2 py-0.5 rounded text-xs font-medium block w-full text-center">Hoàn tất phí</span>':I='<span class="bg-blue-100 text-blue-700 border border-blue-200 px-2 py-0.5 rounded text-xs font-medium block w-full text-center">Đã Cọc</span>';let x="";return r>0&&(x=`<a href="/invoice.html?id=${t.id}" target="_blank" class="mt-1.5 block text-center text-[11px] text-blue-600 hover:text-blue-800 hover:underline font-medium" onclick="event.stopPropagation()">🧾 Xem hóa đơn</a>`),`
                <tr class="${l} cursor-pointer row-clickable block md:table-row p-4 md:p-0 mb-4 md:mb-0 glass-panel md:glass-none border-l-4 border-green-500 md:border-none relative" data-id="${t.id}">
                    ...
                    <td class="p-4 align-top text-center hidden md:table-cell">${I}${x}</td>
    ...
    `}}).join(""),document.querySelectorAll(".claim-btn").forEach(t=>{t.addEventListener("click",async l=>{l.stopPropagation();const d=t.getAttribute("data-id"),r=localStorage.getItem("csr_user");let g=null,f="Admin";if(r){const b=JSON.parse(r);g=b.id,f=b.fullName||b.full_name||"Admin"}t.textContent="Đang xử lý...",t.disabled=!0;try{if((await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:d,sale_id:g,sale_name:f})})).ok){const E=H.find(I=>I.id==d);E&&(E.sale_id=g,E.sale_name=f),R()}else alert("Lỗi khi nhận khách. Vui lòng thử lại."),t.textContent="Nhận khách",t.disabled=!1}catch(b){console.error("Lỗi nhận khách:",b),t.textContent="Nhận khách",t.disabled=!1}})})},Ee=()=>{const n=new Set,e=new Set,o=new Set;H.forEach(s=>{if(s.tour&&n.add(s.tour),s.date){const i=X(s.date);i&&e.add(i)}s.sale_name&&s.sale_name!=="Website"&&s.sale_name!=="null"&&o.add(s.sale_name.trim())});const a=document.getElementById("filterTour"),c=document.getElementById("filterDate"),u=document.getElementById("filterSale");if(a){const s=a.value;a.innerHTML='<option value="">Tất cả Tour</option>',[...n].sort().forEach(i=>a.innerHTML+=`<option value="${i}">${i}</option>`),a.value=s}if(c){const s=c.value;c.innerHTML='<option value="">Tất cả Ngày</option>',[...e].sort().forEach(i=>c.innerHTML+=`<option value="${i}">${i}</option>`),c.value=s}if(u){const s=u.value;u.innerHTML='<option value="">Tất cả Sale</option>',u.innerHTML+='<option value="null">Nguồn Website</option>',[...o].sort().forEach(i=>{u.innerHTML+=`<option value="${i}">${i}</option>`}),u.value=s}};function X(n){if(!n)return"";if(/^\d{1,2}\/\d{1,2}\s*-\s*\d{1,2}\/\d{1,2}$/.test(n))return n.replace(/\s+/g,"");try{if(/^\d{4}-\d{2}-\d{2}$/.test(n)){const[e,o,a]=n.split("-"),c=`${parseInt(a)}/${parseInt(o)}`;return`${c}-${c}`}if(/^\d{1,2}\/\d{1,2}\/\d{4}$/.test(n)){const[e,o,a]=n.split("/"),c=`${parseInt(e)}/${parseInt(o)}`;return`${c}-${c}`}if(n.includes("-"))return n.split("-").map(o=>{const a=o.trim();if(/\/\d{4}$/.test(a)){const[c,u]=a.split("/");return`${parseInt(c)}/${parseInt(u)}`}return a}).join("-")}catch{console.warn("Date normalization failed for:",n)}return n}const M=async()=>{try{H=await(await fetch("/api/bookings")).json(),H.sort((e,o)=>parseInt(o.id)-parseInt(e.id)),Ee(),R()}catch(n){console.error("Lỗi parse list đơn hàng:",n);const e=document.getElementById("bookingsTableBody");e&&(e.innerHTML='<tr><td colspan="4" class="p-8 text-center text-red-500">Lỗi lấy dữ liệu từ Server.</td></tr>')}};M(),["filterSearch","filterTour","filterDate","filterSale","filterStatus"].forEach(n=>{const e=document.getElementById(n);e&&(n==="filterSearch"?e.addEventListener("input",R):e.addEventListener("change",R))});const se=document.getElementById("exportExcelBtn");se&&se.addEventListener("click",()=>{const n=document.getElementById("filterSearch")?document.getElementById("filterSearch").value.toLowerCase():"",e=document.getElementById("filterTour")?document.getElementById("filterTour").value:"",o=document.getElementById("filterDate")?document.getElementById("filterDate").value:"",a=document.getElementById("filterSale")?document.getElementById("filterSale").value:"";let c=H.filter(p=>{let C=!1;const T=parseInt(p.total_price)>0&&parseInt(p.total_price)===parseInt(p.deposit),v=y=>{if(!y)return!1;try{const L=y.split("/");if(L.length===3){const t=new Date(`${L[2]}-${L[1]}-${L[0]}T00:00:00`),l=new Date;return l.setHours(0,0,0,0),t.getTime()<l.getTime()}else if(y.includes("-")){const t=new Date(`${y}T00:00:00`),l=new Date;return l.setHours(0,0,0,0),t.getTime()<l.getTime()}}catch{}return!1},h=(p.status==="Hoàn thành"||p.status==="Đã đi"||T)&&v(p.date);return B==="consult"?C=p.status==="Chờ tư vấn"&&!h:B==="pending"?C=(!p.status||p.status==="Chờ cọc")&&!h:B==="upcoming"?C=p.status&&p.status!=="Chờ tư vấn"&&p.status!=="Chờ cọc"&&!T&&p.status!=="Hoàn thành"&&!h:B==="ready"?C=(T||p.status==="Hoàn thành")&&!h:B==="completed"&&(C=h),!(!C||e&&p.tour!==e||o&&X(p.date)!==o||a&&a!=="null"&&p.sale_name!==a||a==="null"&&p.sale_name&&p.sale_name!=="Website"||n&&!`${p.name||""} ${p.phone||""} ${p.customer_id||""} ${p.id||""} csr${p.id||""} #csr${p.id||""}`.toLowerCase().includes(n))});if(c.length===0){alert("Danh sách rỗng. Không có dữ liệu để xuất.");return}let u=`
        ...
        <div class="space-y-4 pb-2">
            ...
            <div class="bg-gradient-to-br from-csr-orange to-orange-600 rounded-2xl p-4 text-white shadow-lg shadow-orange-500/20">
                ...
                <div class="space-y-2 text-sm">
                    ...
                    <div class="flex justify-between items-center py-1.5 border-b border-white/10">
                        ...
                        <span class="font-bold text-green-100">+ ${o(k)}</span>
                    </div>`:""}
            ...
            ${(()=>{const t=e.phone;if(!t)return"";const l=H.filter(x=>x.phone===t&&!x.status?.includes("hủy")&&!x.status?.includes("Hủy")).sort((x,w)=>new Date(w.created_at||0)-new Date(x.created_at||0)),d=l.length,r=l.reduce((x,w)=>x+(parseInt(w.total_price)||0),0);let g,f,b;d>=5?(g="KHÁCH VIP",f="bg-purple-100 text-purple-700 border-purple-300",b="👑"):d>=2?(g="KHÁCH THÂN THIẾT",f="bg-green-100 text-green-700 border-green-300",b="🌟"):(g="KHÁCH MỚI",f="bg-gray-100 text-gray-500 border-gray-300",b="👋");const E=l.filter(x=>x.id!=n),I=E.length>0?E.slice(0,5).map((x,w)=>{const N=x.status?.includes("Hoàn")?"bg-green-100 text-green-600":x.status?.includes("cọc")||x.status?.includes("Cọc")?"bg-blue-100 text-blue-600":"bg-gray-100 text-gray-500";return`<div class="flex items-center gap-3 py-2 border-b border-gray-100 last:border-none">
                    ...
                    <div class="mb-3">
                        ...
                        <div class="bg-white rounded-xl border border-gray-100 px-3">${I}</div>
                    </div>`:I}
...
Dạ mình thanh toán ở link này giúp em nhé: ${(window.location.hostname==="localhost"||window.location.hostname==="127.0.0.1"?"http://localhost:8888":window.location.origin)+`/pay?id=${o}`}`;navigator.clipboard.writeText(m).then(()=>{const k=e.textContent;e.textContent="✅ Đã copy!",e.classList.remove("bg-blue-500","hover:bg-blue-600"),e.classList.add("bg-green-500"),setTimeout(()=>{e.textContent=k,e.classList.add("bg-blue-500","hover:bg-blue-600"),e.classList.remove("bg-green-500")},2e3)}).catch(()=>{alert(`🔗 Nội dung đã chuẩn bị (copy thủ công):
...
`+m)})}else e.classList.contains("view-btn")?(document.getElementById("rowActionModal").classList.add("hidden"),window.actionView(o)):e.classList.contains("edit-btn")&&window.actionEdit(o)});const q=document.getElementById("smartSearch"),_=document.getElementById("searchSuggestions");if(q){let n;q.addEventListener("input",e=>{const o=e.target.value.toLowerCase().trim();if(clearTimeout(n),o.length<2){_&&_.classList.add("hidden");return}n=setTimeout(async()=>{try{const a=await fetch(`/api/admin_customers?action=search&keyword=${o}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({keyword:o})}),c=await a.json();if(a.ok&&c.success){const s=[c.data];_&&(_.innerHTML=s.map(i=>`
                        ...
                        `).join(""),_.classList.remove("hidden"))}else _&&(_.innerHTML='<div class="p-3 text-xs text-gray-400 italic text-center">Không tìm thấy khách hàng cũ</div>',_.classList.remove("hidden"))}catch(a){console.error("Search error:",a)}},300)}),_&&_.addEventListener("click",e=>{const o=e.target.closest(".suggestion-item");if(o){const a=document.getElementById("addFullName"),c=document.getElementById("addPhone"),u=document.getElementById("addCsrCode");a&&(a.value=o.getAttribute("data-name")),c&&(c.value=o.getAttribute("data-phone")),u&&(u.value=o.getAttribute("data-csr"));const s=document.getElementById("addDob"),i=document.getElementById("addGender"),m=document.getElementById("addIdCard"),k=document.getElementById("addAddress");s&&(s.value=o.getAttribute("data-dob")||""),i&&(i.value=o.getAttribute("data-gender")||"Khác"),m&&(m.value=o.getAttribute("data-cccd")||""),k&&(k.value=o.getAttribute("data-address")||"");const p=document.getElementById("addAllergy"),C=document.getElementById("addDiet"),T=document.getElementById("addTrekkingPole"),v=document.getElementById("addMedalName");p&&(p.value=o.getAttribute("data-allergy")||""),C&&(C.value=o.getAttribute("data-diet")||""),T&&(T.value=o.getAttribute("data-trekking_pole")||"Không"),v&&(v.value=o.getAttribute("data-medal-name")||""),window._selectedCustomer={fullName:o.getAttribute("data-name"),phone:o.getAttribute("data-phone"),csrCode:o.getAttribute("data-csr"),dob:o.getAttribute("data-dob")||"",gender:o.getAttribute("data-gender")||"",address:o.getAttribute("data-address")||"",id_card:o.getAttribute("data-cccd")||"",allergy:o.getAttribute("data-allergy")||"",diet:o.getAttribute("data-diet")||"",trekking_pole:o.getAttribute("data-trekking_pole")||"",medal_name:o.getAttribute("data-medal-name")||""},q.value=o.getAttribute("data-name"),_.classList.add("hidden");const h=`
