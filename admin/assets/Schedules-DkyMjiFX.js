import{S as C,H as $}from"./Header-BkqStZbn.js";const F=()=>`
    ...
    `,D=()=>{let b=[],h="",u="upcoming";const y="/api/schedules",E="/api/tours",g=document.getElementById("scheduleList"),i=document.getElementById("scheduleModal"),p=document.getElementById("scheduleModalContent"),w=document.getElementById("scheduleForm"),k=(t=null)=>{const r=document.getElementById("scheduleModalTitle");if(t){r.textContent="Chỉnh Sửa Lịch Trình",document.getElementById("sch-edit-id").value=t.id,document.getElementById("sch-tour-name").value=t.tour_name;const e=s=>s&&s.includes("T")?s.split("T")[0]:s;document.getElementById("sch-start-date").value=e(t.start_date),document.getElementById("sch-end-date").value=e(t.end_date),document.getElementById("sch-slots").value=t.slots,document.getElementById("sch-status").value=t.status||"Đang mở",document.getElementById("sch-group-label").value=t.group_label||""}else r.textContent="Thêm Lịch Khởi Hành",w.reset(),document.getElementById("sch-edit-id").value="";i.classList.remove("hidden"),setTimeout(()=>{i.classList.add("opacity-100"),p.classList.remove("scale-95"),p.classList.add("scale-100")},10)},m=()=>{i.classList.remove("opacity-100"),p.classList.remove("scale-100"),p.classList.add("scale-95"),setTimeout(()=>{i.classList.add("hidden")},200)};document.getElementById("addScheduleBtn").addEventListener("click",()=>k()),document.getElementById("closeScheduleModalBtn").addEventListener("click",m),document.getElementById("cancelScheduleBtn").addEventListener("click",m),i.addEventListener("click",t=>{t.target===i&&m()});const B=async()=>{try{const t=await fetch(E);if(!t.ok)throw new Error("Failed to load tours");const r=await t.json(),e=document.getElementById("sch-tour-name");e.innerHTML='<option value="">-- Chọn Tour --</option>',r.forEach(a=>{e.innerHTML+=`<option value="${a.name}">${a.name}</option>`});const s=document.getElementById("tourFilterList");let n=`
                <label class="flex items-center gap-3 cursor-pointer group">
                    <input type="radio" name="tourFilter" value="" class="text-csr-orange focus:ring-csr-orange border-gray-200 bg-gray-100" checked>
                    <span class="text-gray-500 group-hover:text-gray-900 transition-colors text-sm">Tất cả Tuyến</span>
                </label>
            `;r.forEach(a=>{n+=`
                ...
                `}),s.innerHTML=n,s.querySelectorAll('input[name="tourFilter"]').forEach(a=>{a.addEventListener("change",o=>{h=o.target.value,f()})}),document.querySelectorAll('input[name="timeFilter"]').forEach(a=>{a.addEventListener("change",o=>{u=o.target.value,document.querySelectorAll('input[name="timeFilter"]').forEach(c=>{const l=c.nextElementSibling;c.checked?(l.classList.add("text-gray-700","font-medium"),l.classList.remove("text-gray-500")):(l.classList.remove("text-gray-700","font-medium"),l.classList.add("text-gray-500"))}),f()})})}catch(t){console.error("Error loading tours:",t)}},x=async()=>{g.innerHTML='<div class="text-center py-12 text-gray-400">Đang tải lịch trình...</div>';try{const t=await fetch(y);if(!t.ok)throw new Error("Failed to fetch schedules");b=await t.json(),f()}catch(t){console.error("Error loading schedules:",t),g.innerHTML='<div class="text-center py-12 text-red-400 font-medium">Lỗi kết nối server.</div>'}},f=()=>{let t=b;h&&(t=t.filter(e=>e.tour_name===h));const r=new Date;if(r.setHours(0,0,0,0),t=t.filter(e=>{const s=new Date(e.start_date);s.setHours(0,0,0,0);const n=s<r;return u==="upcoming"?!n:u==="past"?n:!0}),t.sort((e,s)=>{const n=new Date(e.start_date),a=new Date(s.start_date);return u==="upcoming"?n-a:a-n}),t.length===0){g.innerHTML='<div class="text-center py-12 text-gray-400 italic">Chưa có lịch trình nào'+(h?" cho tuyến này":"")+(u==="past"?" trong quá khứ":" sắp tới")+".</div>";return}g.innerHTML=t.map(e=>{const s=new Date(e.start_date),n=new Date(e.end_date),a=s.toLocaleDateString("vi-VN"),o=n.toLocaleDateString("vi-VN"),c=`${s.getDate().toString().padStart(2,"0")}/${(s.getMonth()+1).toString().padStart(2,"0")}`,l=parseInt(e.booked_count)||0,v=parseInt(e.slots)||0,L=Math.max(0,v-l),T=v>0?Math.min(100,Math.round(l/v*100)):0,M=e.group_label?`<span class="ml-2 text-[10px] font-bold uppercase px-2 py-0.5 rounded-md border bg-blue-50 text-blue-600 border-blue-200">${e.group_label}</span>`:"";let d=e.status||"Đang mở";L<=0?d="Hết chỗ":L<=3&&(d="Sắp hết"),e.status==="Hết chỗ"&&(d="Hết chỗ");const I=d==="Đang mở"?"text-green-600 bg-green-50 border-green-200":d==="Sắp hết"?"text-orange-600 bg-orange-50 border-orange-200":"text-red-500 bg-red-50 border-red-200",H=d==="Đang mở"?"bg-green-500":d==="Sắp hết"?"bg-orange-500":"bg-red-500";return`
                    ...
                    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                        <div class="flex items-center gap-4 flex-1 min-w-0">
                            ...
                            <div class="flex-1 min-w-0">
                                <h3 class="text-lg font-bold text-gray-900 mb-1 tracking-wide truncate">${e.tour_name}${M}</h3>
                                <div class="flex items-center gap-4 text-sm text-gray-500 flex-wrap">
                                    ...
                                    <span class="text-[10px] font-bold uppercase px-2 py-0.5 rounded-md border ${I}">${d}</span>
                                </div>
                                <!-- Progress Bar -->
                        ...
                        <div class="flex gap-2 shrink-0">
                            ...
                            <button class="sch-details-btn bg-csr-orange hover:opacity-90 text-white rounded-lg px-4 py-2.5 text-sm font-medium transition-all flex items-center gap-2" data-id="${e.id}" data-tour="${e.tour_name}" data-date="${e.start_date}" title="Xem Danh Sách Đoàn">
            ...
            `}).join(""),g.addEventListener("click",S)},S=async t=>{const r=t.target.closest(".sch-edit-btn"),e=t.target.closest(".sch-delete-btn"),s=t.target.closest(".sch-details-btn");if(r){const n=r.getAttribute("data-id"),a=b.find(o=>o.id==n);a&&k(a)}if(e){const n=e.getAttribute("data-id");if(confirm("Bạn có chắc chắn muốn xóa lịch trình này?"))try{if((await fetch(`${y}?id=${n}`,{method:"DELETE"})).ok)x();else throw new Error("Delete failed")}catch(a){alert("Lỗi khi xóa: "+a.message)}}if(s){const n=s.getAttribute("data-id"),a=s.getAttribute("data-tour"),o=s.getAttribute("data-date"),c=o&&o.includes("T")?o.split("T")[0]:o,l=`/admin/roster?tour=${encodeURIComponent(a)}&date=${encodeURIComponent(c)}&scheduleId=${n}`;history.pushState(null,null,l),window.dispatchEvent(new Event("popstate"))}};w.addEventListener("submit",async t=>{t.preventDefault();const r=t.target.querySelector('button[type="submit"]'),e=r.textContent;r.textContent="Đang lưu...",r.disabled=!0;const s=document.getElementById("sch-edit-id").value,n={id:s?parseInt(s):null,tour_name:document.getElementById("sch-tour-name").value,start_date:document.getElementById("sch-start-date").value,end_date:document.getElementById("sch-end-date").value,slots:parseInt(document.getElementById("sch-slots").value),status:document.getElementById("sch-status").value,group_label:document.getElementById("sch-group-label").value.trim()||null};try{if(!(await fetch(y,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(n)})).ok)throw new Error("Failed to save schedule");m(),x(),alert(s?"✅ Cập nhật lịch trình thành công!":"✅ Thêm lịch trình mới thành công! Form đặt chỗ trên website sẽ tự cập nhật trong 30 giây.")}catch(a){alert("❌ Lỗi: "+a.message)}finally{r.textContent=e,r.disabled=!1}}),B(),x()};export{D as afterRender,F as render};
