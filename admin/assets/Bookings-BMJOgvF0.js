import{S as Be,H as Le}from"./Header-CtRUqTZE.js";console.log("Bookings V3 - Cache Busted!");const _e=()=>`
    ...
    `,Me=()=>{let H=[],E="consult",N=[],te=[],J=[];const Te=async()=>{try{const[n,e,o]=await Promise.all([fetch("/api/tours"),fetch("/api/schedules"),fetch("/api/users")]);N=n.ok?await n.json():[],te=e.ok?await e.json():[],J=o.ok?await o.json():[],U("addTourName"),U("edit-tour"),U("wl-tour"),Ce("edit-sale")}catch(n){console.error("Lỗi tải tours/schedules:",n)}},Ce=n=>{const e=document.getElementById(n);e&&(e.innerHTML='<option value="">-- Website --</option>',J.forEach(o=>{const d=document.createElement("option");d.value=o.id,d.textContent=`${o.full_name||o.fullName} (${o.role})`,e.appendChild(d)}))},U=n=>{const e=document.getElementById(n);if(!e)return;const o=e.value,d=e.options[0].outerHTML;e.innerHTML=d,N.filter(l=>l.is_visible!==!1).forEach(l=>{const r=document.createElement("option");r.value=l.name,r.textContent=l.name,l.name===o&&(r.selected=!0),e.appendChild(r)})},R=(n,e,o,d)=>{const l=document.getElementById(n);if(!l)return;if(l.innerHTML='<option value="">-- Chọn Lịch --</option>',!e){l.innerHTML='<option value="">-- Chọn Tour trước --</option>';return}const r=te.filter(s=>s.tour_name===e||e.includes(s.tour_name)||s.tour_name.includes(e));if(r.length===0){l.innerHTML='<option value="">Chưa có lịch cho tour này</option>';return}r.sort((s,i)=>new Date(s.start_date)-new Date(i.start_date)).forEach(s=>{const i=parseInt(s.booked_count)||0,g=(s.slots||0)-i,v=new Date(s.start_date),u=s.end_date?new Date(s.end_date):null,y=c=>`${c.getDate().toString().padStart(2,"0")}/${(c.getMonth()+1).toString().padStart(2,"0")}`;let k=y(v);u&&(k+=" - "+y(u)),s.group_label&&(k+=` · ${s.group_label}`),k+=` (${g>=0?g:0} chỗ trống)`;const x=document.createElement("option"),m=`${v.getDate().toString().padStart(2,"0")}/${(v.getMonth()+1).toString().padStart(2,"0")}/${v.getFullYear()}`;x.value=`${s.id}::${m}`,x.dataset.scheduleId=s.id,x.dataset.date=m,x.textContent=k,g<=0&&(x.disabled=!0,x.textContent=k.replace("chỗ trống","HẾT CHỖ")),(d&&String(s.id)===String(d)||!d&&o&&m===o)&&(x.selected=!0),l.appendChild(x)})},ne=document.getElementById("addTourName");ne&&ne.addEventListener("change",n=>{const e=n.target.value;R("addTourDate",e);const o=N.find(l=>l.name===e),d=document.getElementById("addTourPrice");if(o&&d){const l=parseInt(o.price)||0;d.value=l;const r=document.getElementById("addDiscount"),s=document.getElementById("addTotalPriceDisplay"),i=parseInt(r?r.value:0)||0,g=l-i;s&&(s.textContent=(g>0?g:0).toLocaleString("vi-VN")+"đ")}});const oe=document.getElementById("edit-tour");oe&&oe.addEventListener("change",n=>{const e=n.target.value;R("edit-date",e);const o=N.find(l=>l.name===e),d=document.getElementById("edit-total");o&&d&&(parseInt(d.value)===0||!d.value)&&(d.value=parseInt(o.price)||0,typeof window.updateEditRemaining=="function"&&window.updateEditRemaining())});const ae=document.getElementById("wl-tour");ae&&ae.addEventListener("change",n=>{const e=n.target.value;R("wl-date",e)}),Te();const P=()=>{const n=document.getElementById("bookingsTableHead"),e=document.getElementById("bookingsTableBody");if(!e||!n)return;const o=E==="upcoming"||E==="ready"||E==="completed";o?n.innerHTML=`
            ...
            `;const d=document.getElementById("filterSearch")?document.getElementById("filterSearch").value.toLowerCase():"",l=document.getElementById("filterTour")?document.getElementById("filterTour").value:"",r=document.getElementById("filterDate")?document.getElementById("filterDate").value:"",s=document.getElementById("filterSale")?document.getElementById("filterSale").value:"",i=document.getElementById("filterStatus")?document.getElementById("filterStatus").value:"";let g=H.filter(t=>{let a=!1;const p=parseInt(t.total_price)>0&&parseInt(t.total_price)===parseInt(t.deposit),b=C=>{if(!C)return!1;try{const B=C.split("/");if(B.length===3){const L=new Date(`${B[2]}-${B[1]}-${B[0]}T00:00:00`),w=new Date;return w.setHours(0,0,0,0),L.getTime()<w.getTime()}else if(C.includes("-")){const L=new Date(`${C}T00:00:00`),w=new Date;return w.setHours(0,0,0,0),L.getTime()<w.getTime()}}catch{}return!1},h=t.status||"",I=!b(t.date);return E==="consult"?a=h==="Chờ tư vấn"&&I:E==="pending"?a=(h==="Chờ cọc"||h==="Chờ xác nhận cọc"||h==="")&&I:E==="upcoming"?a=h==="Đã cọc"&&!p&&I:E==="ready"?a=(p||h==="Hoàn thành"||h==="Đã cọc (Chờ đi)")&&I&&h!=="Đã hủy"&&h!=="Bảo lưu":E==="completed"&&(a=!I||h==="Đã đi"||h==="Đã hủy"||h==="Bảo lưu"),!(!a||l&&t.tour!==l||r&&X(t.date)!==r||s&&s!=="null"&&t.sale_name!==s||s==="null"&&t.sale_name&&t.sale_name!=="Website"||d&&!`${t.name||""} ${t.phone||""} ${t.customer_id||""} ${t.id||""} csr${t.id||""} #csr${t.id||""}`.toLowerCase().includes(d)||i&&t.status!==i)});const v=document.getElementById("statTitle1"),u=document.getElementById("statTitle2"),y=document.getElementById("statTitle3"),k=document.getElementById("statTotalCustomers"),x=document.getElementById("statTotalRevenue"),m=document.getElementById("statTotalCollected");let c=g.length;if(E==="consult"){const t=document.getElementById("addWaitingBtn"),a=document.getElementById("addBookingBtn");t&&t.classList.remove("hidden"),a&&a.classList.add("hidden"),v&&(v.textContent="Tổng Đăng Ký Chờ Lịch"),u&&(u.textContent="Chưa Được Liên Hệ"),y&&(y.textContent="Đã Có Sale Phụ Trách");let p=0,b=0;g.forEach(h=>{!h.sale_id||h.sale_id==="null"||!h.sale_name||h.sale_name==="Website"||h.sale_name==="null"?p++:b++}),k&&(k.textContent=c),x&&(x.textContent=p+" Người"),m&&(m.textContent=b+" Người")}else if(E==="pending"){const t=document.getElementById("addWaitingBtn"),a=document.getElementById("addBookingBtn");t&&t.classList.add("hidden"),a&&a.classList.remove("hidden"),v&&(v.textContent="Tổng Khách (Chờ Cọc)"),u&&(u.textContent="Khách Chưa Tư Vấn (Mới)"),y&&(y.textContent="Đã Có Sale Giữ Chỗ");let p=0,b=0;g.forEach(h=>{!h.sale_id||h.sale_id==="null"||!h.sale_name||h.sale_name==="Website"||h.sale_name==="null"?p++:b++}),k&&(k.textContent=c),x&&(x.textContent=p+" Đơn"),m&&(m.textContent=b+" Đơn")}else if(E==="upcoming"){v&&(v.textContent="Tổng Số Khách"),u&&(u.textContent="Chưa Xác Nhận Cọc"),y&&(y.textContent="Chờ Thanh Toán Còn Lại");let t=0,a=0;g.forEach(p=>{p.status==="Chờ xác nhận cọc"?t++:parseInt(p.deposit)>0&&parseInt(p.total_price)>parseInt(p.deposit)&&a++}),k&&(k.textContent=c),x&&(x.textContent=t+" Khách"),m&&(m.textContent=a+" Khách")}else if(E==="ready"){v&&(v.textContent="Khách Sẵn Sàng (Full)"),u&&(u.textContent="Tổng Doanh Thu Tab"),y&&(y.textContent="Thực Thu (Full Tận Nơi)");let t=0,a=0;g.forEach(p=>{t+=parseInt(p.total_price)||0,a+=parseInt(p.deposit)||0}),k&&(k.textContent=c),x&&(x.textContent=t>0?t.toLocaleString("vi-VN")+"đ":"0đ"),m&&(m.textContent=a>0?a.toLocaleString("vi-VN")+"đ":"0đ")}else{v&&(v.textContent="Khách Đã Xong (Lịch Sử)"),u&&(u.textContent="Tổng Doanh Thu Tab"),y&&(y.textContent="Thực Thu (Đã Đóng)");let t=0,a=0;g.forEach(p=>{t+=parseInt(p.total_price)||0,a+=parseInt(p.deposit)||0}),k&&(k.textContent=c),x&&(x.textContent=t>0?t.toLocaleString("vi-VN")+"đ":"0đ"),m&&(m.textContent=a>0?a.toLocaleString("vi-VN")+"đ":"0đ")}const T=o?8:4;if(!g||g.length===0){let t=E==="consult"?"Chưa có khách hàng tư vấn nào.":E==="pending"?"Chưa có Khách chờ cọc.":E==="upcoming"?"Chưa có Khách nào Đã Cọc.":E==="ready"?"Chưa có khách sẵn sàng.":"Danh sách rỗng.";e.innerHTML=`<tr><td colspan="${T}" class="p-8 text-center text-gray-500">${t}</td></tr>`;return}e.innerHTML=g.map(t=>{const a=E==="upcoming"||E==="ready"?"bg-green-50/20 hover:bg-green-50 transition-colors":"hover:bg-gray-100 transition-colors";if(o){let p=parseInt(t.total_price)||0;if(p===0&&t.tour&&N&&N.length>0){const f=N.find(D=>D.name&&t.tour&&D.name.toLowerCase().trim()===t.tour.toLowerCase().trim());f&&parseInt(f.price)>0&&(p=parseInt(f.price))}const b=parseInt(t.deposit)||0,h=p-b,I=p>0?p.toLocaleString("vi-VN")+"đ":"Chưa định giá",C=b>0?b.toLocaleString("vi-VN")+"đ":"0đ",B=h>0?h.toLocaleString("vi-VN")+"đ":h===0&&p>0?"Đã thu trọn":"-";let L="";t.status==="Đã hủy"?L='<span class="bg-red-50 text-red-500 border border-red-200 px-2 py-0.5 rounded text-xs block w-full text-center font-bold">❌ Đã hủy</span>':t.status==="Bảo lưu"?L='<span class="bg-gray-100 text-gray-500 border border-gray-300 px-2 py-0.5 rounded text-xs block w-full text-center font-bold">⏸️ Bảo lưu</span>':t.status==="Hoàn thành"||t.status==="Đã đi"?L='<span class="bg-gray-100 text-gray-600 border border-gray-200 px-2 py-0.5 rounded text-xs block w-full text-center">Hoàn thành</span>':t.status==="Chờ xác nhận cọc"?L=`<button class="action-btn confirm-deposit-btn bg-csr-orange text-white px-3 py-1.5 rounded text-xs font-bold shadow-sm hover:bg-[#d65503] w-full" data-id="${t.id}">Xác nhận cọc</button>`:b>0&&h===0?L='<span class="bg-green-100 text-green-700 border border-green-200 px-2 py-0.5 rounded text-xs font-medium block w-full text-center">Hoàn tất phí</span>':L='<span class="bg-blue-100 text-blue-700 border border-blue-200 px-2 py-0.5 rounded text-xs font-medium block w-full text-center">Đã Cọc</span>';let w="";return b>0&&(w=`<a href="/invoice.html?id=${t.id}" target="_blank" class="mt-1.5 block text-center text-[11px] text-blue-600 hover:text-blue-800 hover:underline font-medium" onclick="event.stopPropagation()">🧾 Xem hóa đơn</a>`),`
                <tr class="${a} cursor-pointer row-clickable block md:table-row p-4 md:p-0 mb-4 md:mb-0 glass-panel md:glass-none border-l-4 border-green-500 md:border-none relative" data-id="${t.id}">
                    ...
                    <td class="p-4 align-top text-center hidden md:table-cell">${L}${w}</td>
                ...
                `}else{const p=t.name?t.name.substring(0,2).toUpperCase():"KH",b=E==="consult",h=!t.sale_id||t.sale_id==="null"||!t.sale_name||t.sale_name==="null";let I="";b&&h?I=`<button class="action-btn claim-btn bg-csr-orange hover:bg-[#d65503] text-white px-3 py-1.5 rounded text-xs font-bold shadow-sm transition-colors" data-id="${t.id}">Nhận khách</button>`:I=`<span class="text-sm text-gray-500">${t.sale_name||"Website"}</span>`;const C=`
                ...
                <tr class="${a} cursor-pointer row-clickable block md:table-row p-4 md:p-0 mb-4 md:mb-0 glass-panel md:glass-none border-l-4 border-csr-orange md:border-none relative" data-id="${t.id}">
                    <td class="p-0 md:p-4 align-top block md:table-cell">
                        <div class="flex items-center gap-3">
                            <div class="w-12 h-12 md:w-10 md:h-10 rounded-full bg-csr-orange/20 text-csr-orange flex items-center justify-center font-bold text-base md:text-sm shrink-0">${p}</div>
                            <div class="min-w-0">
                    ...
                    <td class="py-3 md:p-4 align-top block md:table-cell border-t md:border-none border-gray-100 mt-3 md:mt-0 pt-3 md:pt-4">
                        ...
                        ${b&&t.special?`<div class="text-xs text-blue-500 mt-1 italic bg-blue-50 md:bg-transparent p-2 md:p-0 rounded">💬 ${t.special}</div>`:""}
                    </td>
    ...
    `}}).join(""),document.querySelectorAll(".claim-btn").forEach(t=>{t.addEventListener("click",async a=>{a.stopPropagation();const p=t.getAttribute("data-id"),b=localStorage.getItem("csr_user");let h=null,I="Admin";if(b){const C=JSON.parse(b);h=C.id,I=C.fullName||C.full_name||"Admin"}t.textContent="Đang xử lý...",t.disabled=!0;try{if((await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:p,sale_id:h,sale_name:I})})).ok){const B=H.find(L=>L.id==p);B&&(B.sale_id=h,B.sale_name=I),P()}else alert("Lỗi khi nhận khách. Vui lòng thử lại."),t.textContent="Nhận khách",t.disabled=!1}catch(C){console.error("Lỗi nhận khách:",C),t.textContent="Nhận khách",t.disabled=!1}})})},Ie=()=>{const n=new Set,e=new Set,o=new Set;H.forEach(s=>{if(s.tour&&n.add(s.tour),s.date){const i=X(s.date);i&&e.add(i)}s.sale_name&&s.sale_name!=="Website"&&s.sale_name!=="null"&&o.add(s.sale_name.trim())});const d=document.getElementById("filterTour"),l=document.getElementById("filterDate"),r=document.getElementById("filterSale");if(d){const s=d.value;d.innerHTML='<option value="">Tất cả Tour</option>',[...n].sort().forEach(i=>d.innerHTML+=`<option value="${i}">${i}</option>`),d.value=s}if(l){const s=l.value;l.innerHTML='<option value="">Tất cả Ngày</option>',[...e].sort().forEach(i=>l.innerHTML+=`<option value="${i}">${i}</option>`),l.value=s}if(r){const s=r.value;r.innerHTML='<option value="">Tất cả Sale</option>',r.innerHTML+='<option value="null">Nguồn Website</option>',[...o].sort().forEach(i=>{r.innerHTML+=`<option value="${i}">${i}</option>`}),r.value=s}};function X(n){if(!n)return"";if(/^\d{1,2}\/\d{1,2}\s*-\s*\d{1,2}\/\d{1,2}$/.test(n))return n.replace(/\s+/g,"");try{if(/^\d{4}-\d{2}-\d{2}$/.test(n)){const[e,o,d]=n.split("-"),l=`${parseInt(d)}/${parseInt(o)}`;return`${l}-${l}`}if(/^\d{1,2}\/\d{1,2}\/\d{4}$/.test(n)){const[e,o,d]=n.split("/"),l=`${parseInt(e)}/${parseInt(o)}`;return`${l}-${l}`}if(n.includes("-"))return n.split("-").map(o=>{const d=o.trim();if(/\/\d{4}$/.test(d)){const[l,r]=d.split("/");return`${parseInt(l)}/${parseInt(r)}`}return d}).join("-")}catch{console.warn("Date normalization failed for:",n)}return n}const M=async()=>{try{H=await(await fetch("/api/bookings")).json(),H.sort((e,o)=>parseInt(o.id)-parseInt(e.id)),Ie(),P()}catch(n){console.error("Lỗi parse list đơn hàng:",n);const e=document.getElementById("bookingsTableBody");e&&(e.innerHTML='<tr><td colspan="4" class="p-8 text-center text-red-500">Lỗi lấy dữ liệu từ Server.</td></tr>')}};M(),["filterSearch","filterTour","filterDate","filterSale","filterStatus"].forEach(n=>{const e=document.getElementById(n);e&&(n==="filterSearch"?e.addEventListener("input",P):e.addEventListener("change",P))});const se=document.getElementById("exportExcelBtn");se&&se.addEventListener("click",()=>{const n=document.getElementById("filterSearch")?document.getElementById("filterSearch").value.toLowerCase():"",e=document.getElementById("filterTour")?document.getElementById("filterTour").value:"",o=document.getElementById("filterDate")?document.getElementById("filterDate").value:"",d=document.getElementById("filterSale")?document.getElementById("filterSale").value:"";let l=H.filter(u=>{let y=!1;const k=parseInt(u.total_price)>0&&parseInt(u.total_price)===parseInt(u.deposit),x=c=>{if(!c)return!1;try{const T=c.split("/");if(T.length===3){const t=new Date(`${T[2]}-${T[1]}-${T[0]}T00:00:00`),a=new Date;return a.setHours(0,0,0,0),t.getTime()<a.getTime()}else if(c.includes("-")){const t=new Date(`${c}T00:00:00`),a=new Date;return a.setHours(0,0,0,0),t.getTime()<a.getTime()}}catch{}return!1},m=(u.status==="Hoàn thành"||u.status==="Đã đi"||k)&&x(u.date);return E==="consult"?y=u.status==="Chờ tư vấn"&&!m:E==="pending"?y=(!u.status||u.status==="Chờ cọc")&&!m:E==="upcoming"?y=u.status&&u.status!=="Chờ tư vấn"&&u.status!=="Chờ cọc"&&!k&&u.status!=="Hoàn thành"&&!m:E==="ready"?y=(k||u.status==="Hoàn thành")&&!m:E==="completed"&&(y=m),!(!y||e&&u.tour!==e||o&&X(u.date)!==o||d&&d!=="null"&&u.sale_name!==d||d==="null"&&u.sale_name&&u.sale_name!=="Website"||n&&!`${u.name||""} ${u.phone||""} ${u.customer_id||""} ${u.id||""} csr${u.id||""} #csr${u.id||""}`.toLowerCase().includes(n))});if(l.length===0){alert("Danh sách rỗng. Không có dữ liệu để xuất.");return}let r=`
        ...
        <div class="space-y-4 pb-2">
            ...
            ${(()=>{const t=e.phone;if(!t)return"";const a=H.filter(w=>w.phone===t&&!w.status?.includes("hủy")&&!w.status?.includes("Hủy")).sort((w,f)=>new Date(f.created_at||0)-new Date(w.created_at||0)),p=a.length,b=a.reduce((w,f)=>w+(parseInt(f.total_price)||0),0);let h,I,C;p>=5?(h="KHÁCH VIP",I="bg-purple-100 text-purple-700 border-purple-300",C="👑"):p>=2?(h="KHÁCH THÂN THIẾT",I="bg-green-100 text-green-700 border-green-300",C="🌟"):(h="KHÁCH MỚI",I="bg-gray-100 text-gray-500 border-gray-300",C="👋");const B=a.filter(w=>w.id!=n),L=B.length>0?B.slice(0,5).map((w,f)=>{const D=w.status?.includes("Hoàn")?"bg-green-100 text-green-600":w.status?.includes("cọc")||w.status?.includes("Cọc")?"bg-blue-100 text-blue-600":"bg-gray-100 text-gray-500";return`<div class="flex items-center gap-3 py-2 border-b border-gray-100 last:border-none">
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="text-sm font-medium text-gray-800 truncate">${w.tour||"—"}</div>
                                <div class="text-[10px] text-gray-400">${w.date||"—"}${w.medal_name?` • <span class="text-csr-orange font-medium">Huy chương: ${w.medal_name}</span>`:""}</div>
                            </div>
                            <span class="${D} text-[9px] px-2 py-0.5 rounded-full font-bold shrink-0">${w.status||"Mới"}</span>
                    ...
                    <div class="flex items-center justify-between mb-4 gap-3">
                        <div class="flex items-center gap-2">
                            ...
                            <span class="text-xs font-black uppercase tracking-widest px-3 py-1.5 rounded-full border ${I}">${h}</span>
                        </div>
                        <div class="text-right">
                            <div class="text-[10px] text-gray-400 uppercase font-semibold tracking-wide">Tổng đã chi (LTV)</div>
                            <div class="text-lg font-black text-gray-900">${b>0?b.toLocaleString("vi-VN")+"đ":"—"}</div>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center gap-2 mb-3 text-sm text-gray-500">
                        ...
                        <span class="font-black text-gray-800 text-base">${p}</span>
        ...
        </div>`;document.getElementById("detailContentBlock").innerHTML=T,document.getElementById("detailModal").classList.remove("hidden"),setTimeout(()=>{document.getElementById("detailModal").classList.add("opacity-100");const t=document.getElementById("detailModalContent");t&&(t.classList.remove("scale-95","translate-y-4"),t.classList.add("scale-100","translate-y-0"))},10)},window.actionEdit=async n=>{document.getElementById("rowActionModal").classList.add("hidden");const e=H.find(c=>c.id==n);if(!e)return;document.getElementById("edit-id").value=n,document.getElementById("edit-name").value=e.name||"",document.getElementById("edit-phone").value=e.phone||"",document.getElementById("edit-medal-name").value=e.medal_name||"",document.getElementById("edit-tour").value=e.tour||"",typeof R=="function"&&R("edit-date",e.tour||"",e.date||"",e.schedule_id||null),(c=>{let T=c.dob||"";if(typeof T=="string"&&T.includes("/")){const p=T.split("/");p.length===3&&(T=`${p[2]}-${p[1].padStart(2,"0")}-${p[0].padStart(2,"0")}`)}document.getElementById("edit-dob").value=T,document.getElementById("edit-gender").value=c.gender||"Khác",document.getElementById("edit-allergy").value=c.allergy||c.medical_notes||"",document.getElementById("edit-address").value=c.address||"",document.getElementById("edit-diet").value=c.diet||c.dietary||"",document.getElementById("edit-trekking-pole").value=c.trekking_pole||"Không";const t=document.getElementById("edit-id-card");t&&(t.value=c.id_card||c.cccd||"");const a=document.getElementById("edit-pickup-point");a&&(a.value=c.pickup_point||"")})(e),document.getElementById("edit-status").value=e.status||"Chờ xác nhận cọc",document.getElementById("edit-commitments").checked=!!e.commitments,document.getElementById("edit-special").value=e.special||"";try{const c=localStorage.getItem("csr_user");if(c&&JSON.parse(c).role==="admin"){const t=document.getElementById("edit-sale-container");t&&t.classList.remove("hidden");const a=document.getElementById("edit-sale");a&&(a.value=e.sale_id||"")}}catch{}if((!e.dob||!e.address||!e.gender||e.gender==="Khác")&&(e.customer_id||e.phone))try{const c=e.customer_id||e.phone,T=await fetch(`/api/admin_customers?action=search&keyword=${encodeURIComponent(c)}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({keyword:c})}),t=await T.json();if(T.ok&&t.success){const a=t.data,p=document.getElementById("edit-dob"),b=document.getElementById("edit-address"),h=document.getElementById("edit-allergy"),I=document.getElementById("edit-diet"),C=document.getElementById("edit-gender"),B=document.getElementById("edit-medal-name");p&&!p.value&&(p.value=a.dob||""),b&&!b.value&&(b.value=a.address||""),h&&!h.value&&(h.value=a.medical_notes||""),I&&!I.value&&(I.value=a.dietary||""),C&&(C.value==="Khác"||!C.value)&&(C.value=a.gender||"Khác"),B&&!B.value&&(B.value=a.medal_name||"");const L=document.getElementById("edit-id-card");L&&!L.value&&(L.value=a.cccd||"")}}catch(c){console.error("Pull CRM error:",c)}let l=(parseInt(e.total_price)||0)+(parseInt(e.discount)||0);if(l===0&&e.tour){const c=N.find(T=>T.name===e.tour);c&&parseInt(c.price)>0&&(l=parseInt(c.price))}document.getElementById("edit-total").value=l,document.getElementById("edit-discount").value=e.discount||0,document.getElementById("edit-deposit").value=e.deposit||0;const r=parseInt(e.deposit_required);document.getElementById("edit-deposit-required").value=isNaN(r)?1e6:r,window.updateEditRemaining();const s=document.getElementById("servicesContainer"),i=document.getElementById("emptyServicesMsg"),g=document.getElementById("servicesTotalRow"),v=document.getElementById("presetServicesBlock"),u=document.getElementById("presetServicesList"),y=document.getElementById("servicesDivider");let k=[];try{if(e.services_booked){const c=typeof e.services_booked=="string"?JSON.parse(e.services_booked):e.services_booked;Array.isArray(c)&&(k=c)}}catch{}if(u&&v){u.innerHTML="";const c=N.find(t=>t.name===e.tour);let T=[];try{if(c?.services){const t=typeof c.services=="string"?JSON.parse(c.services):c.services;Array.isArray(t)&&(T=t)}}catch{}T.length>0?(v.classList.remove("hidden"),y&&y.classList.remove("hidden"),T.forEach(t=>{const a=k.some(b=>(b.label||b.name||"").toLowerCase()===(t.label||"").toLowerCase()),p=document.createElement("label");p.className="preset-service-card flex items-center gap-3 p-3 rounded-xl border-2 cursor-pointer transition-all "+(a?"border-green-400 bg-green-50":"border-gray-100 bg-gray-50 hover:border-orange-200"),p.innerHTML=`
...
`+g)})}else e.classList.contains("view-btn")?(document.getElementById("rowActionModal").classList.add("hidden"),window.actionView(o)):e.classList.contains("edit-btn")&&window.actionEdit(o)});const O=document.getElementById("smartSearch"),S=document.getElementById("searchSuggestions");if(O){let n;O.addEventListener("input",e=>{const o=e.target.value.toLowerCase().trim();if(clearTimeout(n),o.length<2){S&&S.classList.add("hidden");return}n=setTimeout(async()=>{try{const d=await fetch(`/api/admin_customers?action=search&keyword=${o}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({keyword:o})}),l=await d.json();if(d.ok&&l.success){const s=[l.data];S&&(S.innerHTML=s.map(i=>`
                        ...
                        `).join(""),S.classList.remove("hidden"))}else S&&(S.innerHTML='<div class="p-3 text-xs text-gray-400 italic text-center">Không tìm thấy khách hàng cũ</div>',S.classList.remove("hidden"))}catch(d){console.error("Search error:",d)}},300)}),S&&S.addEventListener("click",e=>{const o=e.target.closest(".suggestion-item");if(o){const d=document.getElementById("addFullName"),l=document.getElementById("addPhone"),r=document.getElementById("addCsrCode");d&&(d.value=o.getAttribute("data-name")),l&&(l.value=o.getAttribute("data-phone")),r&&(r.value=o.getAttribute("data-csr"));const s=document.getElementById("addDob"),i=document.getElementById("addGender"),g=document.getElementById("addIdCard"),v=document.getElementById("addAddress");s&&(s.value=o.getAttribute("data-dob")||""),i&&(i.value=o.getAttribute("data-gender")||"Khác"),g&&(g.value=o.getAttribute("data-cccd")||""),v&&(v.value=o.getAttribute("data-address")||"");const u=document.getElementById("addAllergy"),y=document.getElementById("addDiet"),k=document.getElementById("addTrekkingPole"),x=document.getElementById("addMedalName");u&&(u.value=o.getAttribute("data-allergy")||""),y&&(y.value=o.getAttribute("data-diet")||""),k&&(k.value=o.getAttribute("data-trekking_pole")||"Không"),x&&(x.value=o.getAttribute("data-medal-name")||""),window._selectedCustomer={fullName:o.getAttribute("data-name"),phone:o.getAttribute("data-phone"),csrCode:o.getAttribute("data-csr"),dob:o.getAttribute("data-dob")||"",gender:o.getAttribute("data-gender")||"",address:o.getAttribute("data-address")||"",id_card:o.getAttribute("data-cccd")||"",allergy:o.getAttribute("data-allergy")||"",diet:o.getAttribute("data-diet")||"",trekking_pole:o.getAttribute("data-trekking_pole")||"",medal_name:o.getAttribute("data-medal-name")||""},O.value=o.getAttribute("data-name"),S.classList.add("hidden");const m=`
