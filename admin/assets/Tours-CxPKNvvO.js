import{S as y,H as f}from"./Header-DjwXxBfR.js";const I=()=>{let n={role:"sale"};try{const a=localStorage.getItem("csr_user");a&&(n=JSON.parse(a))}catch{}const o=n.role==="admin";return`
        ...
        <div class="flex flex-col flex-1 w-full overflow-hidden">
          ...
          <main class="flex-1 overflow-x-hidden overflow-y-auto bg-gray-50 p-6">
               <div class="max-w-7xl mx-auto space-y-6">
                  ...
                  ${o?"":`<div class="flex items-center gap-3 bg-amber-50 border border-amber-200 text-amber-700 rounded-xl px-4 py-3 text-sm font-medium">
                  ...
                  <div class="flex justify-between items-end">
                      ...
                      ${o?`<button id="addTourBtn" class="btn-primary flex items-center gap-2">
    ...
    `},k=()=>{const n=document.getElementById("toursTableBody"),o=document.getElementById("tourModal"),a=document.getElementById("tourModalContent"),u=document.getElementById("tourForm");let d=[];const g="/api/tours",m=(e=null)=>{const t=document.getElementById("tourModalTitle");e?(t.textContent="Chỉnh Sửa Tour",document.getElementById("tour-edit-id").value=e.id,document.getElementById("tour-name").value=e.name||"",document.getElementById("tour-short-desc").value=e.short_desc||e.shortDesc||"",document.getElementById("tour-image").value=e.image||"",document.getElementById("tour-image2").value=e.image2||"",document.getElementById("tour-image3").value=e.image3||"",document.getElementById("tour-image4").value=e.image4||"",document.getElementById("tour-duration").value=e.duration||"",document.getElementById("tour-price").value=e.price||"",document.getElementById("tour-level").value=e.level||"Trung Bình",document.getElementById("tour-region").value=e.region||"Miền Nam",document.getElementById("tour-type").value=e.type||"TREKKING",document.getElementById("tour-altitude").value=e.altitude||"",document.getElementById("tour-sort-order").value=e.sort_order||0,document.getElementById("tour-custom-domain").value=e.custom_domain||"",document.getElementById("tour-is-visible").checked=e.is_visible!==!1):(t.textContent="Thêm Tour Mới",u.reset(),document.getElementById("tour-edit-id").value="",document.getElementById("tour-is-visible").checked=!0,document.getElementById("tour-custom-domain").value="https://camsiteretreats.com/tour/"),o.classList.remove("hidden"),setTimeout(()=>{o.classList.add("opacity-100"),a.classList.remove("scale-95"),a.classList.add("scale-100")},10)},c=()=>{o.classList.remove("opacity-100"),a.classList.remove("scale-100"),a.classList.add("scale-95"),setTimeout(()=>{o.classList.add("hidden")},200)};let b={role:"sale"};try{const e=localStorage.getItem("csr_user");e&&(b=JSON.parse(e))}catch{}const v=b.role==="admin";v&&(document.getElementById("addTourBtn")?.addEventListener("click",()=>m()),document.getElementById("closeTourModalBtn")?.addEventListener("click",c),document.getElementById("cancelTourBtn")?.addEventListener("click",c),o?.addEventListener("click",e=>{e.target===o&&c()}));const p=async()=>{n.innerHTML='<tr><td colspan="7" class="text-center py-8 text-gray-400">Đang tải danh sách tour...</td></tr>';try{const e=await fetch(g);if(!e.ok)throw new Error("Failed to load tours");d=await e.json(),h()}catch(e){console.error("Error loading tours:",e),n.innerHTML='<tr><td colspan="7" class="text-center py-4 text-red-500">Lỗi kết nối server.</td></tr>'}},h=()=>{if(d.length===0){n.innerHTML='<tr><td colspan="7" class="text-center py-8 text-gray-400">Chưa có tour nào.</td></tr>';return}n.innerHTML=d.map(e=>{const t=!e.price||e.price==="Update"||e.price==="0"||parseInt(e.price)===0?'<span class="text-gray-400 italic">Update...</span>':`<span class="font-bold text-csr-orange">${parseInt(e.price).toLocaleString("vi-VN")}đ</span>`,s=e.level==="Dễ"?"bg-green-100 text-green-700":e.level==="Khó"?"bg-red-100 text-red-600":"bg-orange-100 text-orange-700",l=e.is_visible!==!1?'<span class="bg-green-100 text-green-600 text-[10px] px-2 py-0.5 rounded-full font-bold">Hiển thị</span>':'<span class="bg-gray-200 text-gray-500 text-[10px] px-2 py-0.5 rounded-full font-bold">Đã ẩn</span>',r=v?`
                ...
                </div>`:'<span class="text-xs text-gray-400 italic">Chỉ xem</span>';return`
                <tr class="hover:bg-gray-50 transition-colors group" data-tour-id="${e.id}">
                    ...
                    <td class="p-4">
                        <span class="px-2 py-0.5 ${s} rounded text-[10px] font-bold uppercase">${e.level||"-"}</span>
                    </td>
                    <td class="p-4">${t}</td>
                    <td class="p-4 text-center">${l}</td>
                    <td class="p-4 text-right">${r}</td>
                </tr>
            `}).join("")};n.addEventListener("click",async e=>{const t=e.target.closest(".tour-edit-btn"),s=e.target.closest(".tour-delete-btn");if(t){const l=parseInt(t.getAttribute("data-id")),r=d.find(i=>i.id===l);r&&m(r)}if(s){const l=s.getAttribute("data-id");if(confirm("Dữ liệu lịch trình liên quan có thể bị ảnh hưởng. Bạn vẫn muốn xóa tour này?"))try{if((await fetch(`${g}?id=${l}`,{method:"DELETE"})).ok)p();else throw new Error("Delete failed")}catch(r){alert("Lỗi khi xóa: "+r.message)}}}),u.addEventListener("submit",async e=>{e.preventDefault();const t=u.querySelector('button[type="submit"]'),s=t.textContent;t.textContent="Đang lưu...",t.disabled=!0;const l=document.getElementById("tour-edit-id").value,r={id:l?parseInt(l):null,name:document.getElementById("tour-name").value,image:document.getElementById("tour-image").value,image2:document.getElementById("tour-image2").value||null,image3:document.getElementById("tour-image3").value||null,image4:document.getElementById("tour-image4").value||null,shortDesc:document.getElementById("tour-short-desc").value,altitude:document.getElementById("tour-altitude").value||null,level:document.getElementById("tour-level").value,region:document.getElementById("tour-region").value,type:document.getElementById("tour-type").value,duration:document.getElementById("tour-duration").value,price:document.getElementById("tour-price").value,sort_order:parseInt(document.getElementById("tour-sort-order").value)||0,custom_domain:document.getElementById("tour-custom-domain").value.trim()||null,is_visible:document.getElementById("tour-is-visible").checked};try{const i=await fetch(g,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(r)});if(!i.ok){const x=await i.json();throw new Error(x.error||"Save failed")}c(),p(),alert(l?"✅ Cập nhật tour thành công!":"✅ Thêm tour mới thành công! Website sẽ tự đồng bộ trong vài giây.")}catch(i){alert("❌ Lỗi: "+i.message)}finally{t.textContent=s,t.disabled=!1}}),p()};export{k as afterRender,I as render};
