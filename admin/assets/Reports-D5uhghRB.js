import{S as q,H as F}from"./Header-CARqqxjt.js";const G=()=>{let f={role:"sale",fullName:"Guest",id:null};try{const y=localStorage.getItem("csr_user");y&&(f=JSON.parse(y))}catch{}const T=f.role==="admin";return`
    ...
    `},U=()=>{let f={role:"sale",fullName:"Guest",id:null};try{const t=localStorage.getItem("csr_user");t&&(f=JSON.parse(t))}catch{}const T=f.role==="admin";let y=[],S=[],L=[],x=null,u=new Set,k=[];const b=t=>new Intl.NumberFormat("vi-VN").format(Math.round(t||0))+"đ",P=t=>{if(!t)return 0;try{const s=Array.isArray(t)?t:JSON.parse(t);if(Array.isArray(s))return s.reduce((e,n)=>e+(parseInt(n.price)||0),0)}catch{return(String(t).match(/price:\s*(\d+)/g)||[]).reduce((n,o)=>n+Number(o.replace(/price:\s*/,"")),0)}return 0},C=(t,s)=>{const e=s.find(c=>c.name&&t.tour&&c.name.toLowerCase().trim()===t.tour.toLowerCase().trim()),n=parseFloat(t.commission_rate??e?.commission_rate??5),o=P(t.services_booked),d=Math.max(0,(parseInt(t.total_price)||0)-o),i=d*(n/100);return{rate:n,servicesTotal:o,basePrice:d,commission:i}},A=t=>{if(!t)return null;const s=t.split("-")[0].trim();if(s.includes("/")){const e=s.split("/");if(e.length===3)return new Date(parseInt(e[2]),parseInt(e[1])-1,parseInt(e[0]));if(e.length===2)return new Date(new Date().getFullYear(),parseInt(e[1])-1,parseInt(e[0]))}return s.match(/^\d{4}-/)?new Date(s):null},H=new Date;H.setHours(0,0,0,0);const $=t=>{const s=t.status&&(t.status.includes("Hoàn tất")||t.status.includes("Hoàn thành")),e=(parseInt(t.deposit)||0)>0&&(parseInt(t.total_price)||0)>0&&(parseInt(t.deposit)||0)>=(parseInt(t.total_price)||0);return s||e},B=t=>t.status&&(t.status.includes("Đã cọc")||t.status.includes("Hoàn tất")||t.status.includes("Hoàn thành")),w=t=>{if(!$(t))return!1;const s=A(t.date);return s&&s<H},_=async()=>{try{const[t,s,e]=await Promise.all([fetch("/api/admin_tours"),fetch("/api/bookings"),fetch("/api/users")]),n=t.ok?await t.json():{};y=Array.isArray(n)?n:n.data||[],S=s.ok?await s.json():[],L=e.ok?await e.json():[],T?(j(),N()):D()}catch(t){console.error("Lỗi tải dữ liệu báo cáo:",t)}},j=()=>{const t=document.getElementById("commissionSetupContainer");if(!t)return;if(y.length===0){t.innerHTML='<div class="text-sm text-gray-500 text-center py-4">Không có Tour nào.</div>';return}const s=y.map(e=>{const n=e.commission_rate??5;return`
                ...
                </div>`}).join("");t.innerHTML=`<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">${s}</div>`,document.getElementById("commissionSetupToggle")?.addEventListener("click",()=>{t.classList.toggle("hidden"),document.getElementById("commissionSetupChevron")?.classList.toggle("rotate-180")})};window.saveCommission=async t=>{const s=document.getElementById(`comm_input_${t}`);if(!s)return;const e=parseFloat(s.value);if(isNaN(e)||e<0||e>100){alert("Tỉ lệ hoa hồng không hợp lệ (0–100)");return}const n=s.nextElementSibling;n.textContent="...",n.disabled=!0;try{if((await fetch(`/api/admin_tours?id=${t}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({commission_rate:e})})).ok){const d=y.find(i=>i.id===t);d&&(d.commission_rate=e),n.classList.add("bg-green-500"),n.textContent="✓ Đã lưu!",setTimeout(()=>{n.classList.remove("bg-green-500"),n.textContent="Lưu",n.disabled=!1},1500),N()}else throw new Error}catch{alert("Lỗi khi lưu!"),n.textContent="Lưu",n.disabled=!1}};const N=()=>{const t=document.getElementById("salesTableBody");if(!t)return;const s=S.filter(i=>B(i)),e={},n={};L.forEach(i=>{const c=String(i.id),a=i.full_name||i.fullName||i.username||`User #${i.id}`;e[c]=a,n[a.trim().toLowerCase()]=c});const o={};s.forEach(i=>{const c=String(i.sale_id||"").trim();let a=c,p=i.sale_name||"Admin / Tự Đặt";if(c&&e[c])p=e[c];else{const h=n[(i.sale_name||"").trim().toLowerCase()];h?(a=h,p=e[h]):a=i.sale_name||"no-id"}o[a]||(o[a]={key:a,saleId:c||a,name:p,bookings:[]}),o[a].bookings.push(i)});const d=Object.values(o).sort((i,c)=>i.name.localeCompare(c.name));if(d.length===0){t.innerHTML='<tr><td colspan="6" class="p-8 text-center text-gray-400 text-sm">Chưa có dữ liệu nào trong kỳ này.</td></tr>';return}t.innerHTML=d.map((i,c)=>{const a=i.bookings.filter(r=>!w(r)&&!r.commission_paid).length,p=i.bookings.filter(r=>w(r)&&!r.commission_paid).length,h=i.bookings.filter(r=>!r.commission_paid).reduce((r,g)=>r+C(g,y).commission,0);return`
                <tr class="hover:bg-gray-50 transition-colors">
                    ...
                    <td class="p-4 border-b border-gray-100 text-center">
                        <span class="inline-block bg-green-100 text-green-700 text-xs font-bold px-2.5 py-1 rounded-full">${p} đơn</span>
                    </td>
                ...
                <div class="mb-1">
                    ...
                    ${h.map(g=>c(g,r)).join("")}
                </div>`,c=(a,p=!1)=>{const{rate:h,servicesTotal:r,commission:g}=C(a,y),m=a.commission_paid,v=!m,l=u.has(a.id);return`
                <div class="flex items-center gap-3 px-4 py-3 border-b border-gray-100 hover:bg-gray-50 transition-colors ${m?"opacity-60":""}">
                    <input type="checkbox" class="order-checkbox w-4 h-4 accent-csr-orange shrink-0 ${v?"":"cursor-not-allowed"} ${p&&!m?"completed-checkbox":""}"
                    ...
                    <div class="shrink-0 w-24 text-right">
                        ${m?'<span class="inline-block bg-green-100 text-green-700 text-[10px] font-bold px-2 py-1 rounded-full">✅ Đã TT HH</span>':w(a)?'<span class="inline-block bg-green-50 text-green-600 text-[10px] font-bold px-2 py-1 rounded border border-green-200">Hoàn thành</span>':$(a)?'<span class="inline-block bg-emerald-50 text-emerald-600 text-[10px] font-bold px-2 py-1 rounded border border-emerald-200">Hoàn tất phí</span>':'<span class="inline-block bg-blue-50 text-blue-600 text-[10px] font-bold px-2 py-1 rounded border border-blue-200">Đã cọc</span>'}
                    </div>
                </div>`};t.innerHTML=`
            ...
            ${i("ĐÃ HOÀN THÀNH","green",e,!0)}
            ${i("ĐÃ CỌC","blue",n,!1)}
            ${i("ĐÃ THANH TOÁN HOA HỒNG","gray",o,!1)}
        `},M=()=>{const t=document.getElementById("tabContentPayHistory");if(t){if(k.length===0){t.innerHTML='<div class="p-8 text-center text-gray-400 text-sm">Chưa có lần thanh toán nào cho nhân viên này.</div>';return}t.innerHTML=`
            <table class="w-full text-sm text-left border-collapse">
                ...
                <tbody class="divide-y divide-gray-100">
                    ${k.map(s=>{const e=new Date(s.created_at),n=`${e.getDate().toString().padStart(2,"0")}/${(e.getMonth()+1).toString().padStart(2,"0")}/${e.getFullYear()}`;return`<tr class="hover:bg-gray-50 transition-colors">
            ...
            </table>`}};window.onOrderCheck=t=>{const s=parseInt(t.dataset.id);t.checked?u.add(s):u.delete(s),I()},window.toggleSelectAll=t=>{document.querySelectorAll(".order-checkbox:not(:disabled)").forEach(e=>{e.checked=t.checked;const n=parseInt(e.dataset.id);t.checked?u.add(n):u.delete(n)}),I()},window.selectAllCompleted=()=>{const t=document.querySelectorAll(".completed-checkbox:not(:disabled)"),s=[...t].every(n=>n.checked);t.forEach(n=>{n.checked=!s;const o=parseInt(n.dataset.id);s?u.delete(o):u.add(o)});const e=document.getElementById("selectAllOrders");if(e){const n=document.querySelectorAll(".order-checkbox:not(:disabled)"),o=[...n].filter(d=>d.checked).length;e.indeterminate=o>0&&o<n.length,e.checked=o===n.length&&n.length>0}I()};const I=()=>{const t=u.size,s=x?.bookings.filter(o=>u.has(o.id)).reduce((o,d)=>o+C(d,y).commission,0)||0,e=document.getElementById("paySelectionInfo"),n=document.getElementById("payCommissionBtn");e&&(e.textContent=t>0?`Đã chọn ${t} đơn · Tổng HH: ${b(s)}`:"Chọn đơn để thanh toán hoa hồng"),n&&(n.disabled=t===0)};window.openPayConfirmModal=()=>{const t=u.size;if(t===0)return;const s=x?.bookings.filter(a=>u.has(a.id)).reduce((a,p)=>a+C(p,y).commission,0)||0;document.getElementById("confirmSaleName").textContent=x?.name||"—",document.getElementById("confirmOrderCount").textContent=`${t} đơn`,document.getElementById("confirmTotalComm").textContent=b(s),document.getElementById("payNote").value="";const e=document.getElementById("paymentInfoBox"),n=document.getElementById("noPaymentInfoBox");let o=L.find(a=>String(a.id)===String(x.saleId));!o&&x.saleId==="unassigned"&&(o=null);const d=o?.payment_info;if(d&&(d.bank_name||d.account_number||d.qr_code)){document.getElementById("payBankName").textContent=d.bank_name||"—",document.getElementById("payBankAccount").textContent=d.account_number||"—",document.getElementById("payBankOwner").textContent=d.account_name||"—";const a=document.getElementById("payQrBox");d.qr_code?(a.innerHTML=`<p class="text-xs text-gray-500 mb-2 font-medium">Mã QR Quét Nhanh:</p><img src="${d.qr_code}" class="w-32 h-32 object-contain rounded-xl border-2 border-gray-200 p-1 shadow-sm mix-blend-multiply">`,a.classList.remove("hidden")):(a.innerHTML="",a.classList.add("hidden")),e.classList.remove("hidden"),n.classList.add("hidden")}else e.classList.add("hidden"),n.classList.remove("hidden"),x.saleId==="unassigned"&&n.classList.add("hidden");const i=document.getElementById("payConfirmModal"),c=document.getElementById("payConfirmModalInner");i.classList.remove("hidden"),requestAnimationFrame(()=>{i.classList.remove("opacity-0"),c.classList.remove("scale-95")})},window.closePayConfirmModal=()=>{const t=document.getElementById("payConfirmModal"),s=document.getElementById("payConfirmModalInner");t.classList.add("opacity-0"),s.classList.add("scale-95"),setTimeout(()=>t.classList.add("hidden"),200)},window.confirmPayCommission=async()=>{const t=document.getElementById("confirmPayBtn");t.textContent="Đang xử lý...",t.disabled=!0;const s=document.getElementById("payNote").value.trim(),e=f.fullName||f.full_name||"Admin";try{const n=await fetch("/api/commission_payments",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({sale_id:x.saleId,sale_name:x.name,booking_ids:[...u],paid_by:e,note:s})});if(!n.ok)throw new Error(await n.text());if(window.closePayConfirmModal(),u.clear(),await _(),x&&window._salesMap){const o=window._salesMap[x.key];if(o){x=o;try{const d=await fetch(`/api/commission_payments?sale_id=${encodeURIComponent(x.saleId)}`);k=d.ok?await d.json():[]}catch{k=[]}E(),M(),I(),document.getElementById("panelSaleName").textContent=x.name}}alert("✅ Đã thanh toán hoa hồng thành công!")}catch(n){console.error("Lỗi thanh toán hoa hồng:",n),alert("❌ Lỗi: "+n.message)}finally{t.textContent="✅ Xác Nhận Thanh Toán",t.disabled=!1}};const D=async()=>{const t=String(f.id||f.userId||""),s=(f.fullName||f.full_name||"").toLowerCase(),e=S.filter(r=>{const g=String(r.sale_id||"").trim(),m=(r.sale_name||"").toLowerCase()===s;return!(t&&g===t)&&!m?!1:B(r)}),n=e.filter(r=>w(r)),o=e.reduce((r,g)=>r+(parseInt(g.total_price)||0),0),d=n.reduce((r,g)=>r+C(g,y).commission,0),c=S.filter(r=>{const g=String(r.sale_id||"").trim(),m=(r.sale_name||"").toLowerCase()===s;return t&&g===t||m}).length,a=(r,g)=>{const m=document.getElementById(r);m&&(m.textContent=g)};a("sale-stat-total-pax",c),a("sale-stat-success-pax",n.length),a("sale-stat-revenue",b(o)),a("sale-stat-real-comm",b(d));const p=document.getElementById("saleOrdersContainer");if(p)if(e.length===0)p.innerHTML='<div class="p-8 text-center text-gray-400 text-sm">Chưa có đơn nào trong kỳ này.</div>';else{const r=e.filter(l=>w(l)&&!l.commission_paid),g=e.filter(l=>!w(l)&&!l.commission_paid),m=e.filter(l=>l.commission_paid),v=[...r,...g,...m];p.innerHTML=`
                    <div class="overflow-x-auto">
                        <table class="w-full text-sm text-left border-collapse">
                            <thead><tr class="bg-gray-50 border-b border-gray-200 text-xs uppercase text-gray-400 font-bold">
                            ...
                            <tbody class="divide-y divide-gray-100">
                                ${v.map((l,O)=>{const{commission:R}=C(l,y);return`<tr class="hover:bg-gray-50 transition-colors ${l.commission_paid?"opacity-70":""}">
                                        ...
                                        <td class="p-3 text-right font-black text-csr-orange">${b(R)}</td>
                    ...
                    </div>`}const h=document.getElementById("salePaymentHistoryContainer");if(h&&t)try{const r=await fetch(`/api/commission_payments?sale_id=${encodeURIComponent(t)}`),g=r.ok?await r.json():[];g.length===0?h.innerHTML='<div class="p-6 text-center text-gray-400 text-sm">Chưa có lần thanh toán nào.</div>':h.innerHTML=`
                        <table class="w-full text-sm text-left border-collapse">
                            <thead><tr class="bg-gray-50 border-b border-gray-200 text-xs uppercase text-gray-400 font-bold">
                        ...
                        </table>`}catch{h.innerHTML='<div class="p-6 text-center text-gray-400 text-sm">Lỗi tải lịch sử.</div>'}};document.getElementById("saleDetailPanel")?.addEventListener("click",t=>{t.target.id==="saleDetailPanel"&&window.closeSalePanel()}),_()};export{U as afterRender,G as render};
