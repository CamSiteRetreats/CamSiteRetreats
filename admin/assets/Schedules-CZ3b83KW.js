import{S,H as I}from"./Header-CvqOcIss.js";const H=()=>`
    ...
    `,$=()=>{let v=[],u="";const m="/api/schedules",k="/api/tours",c=document.getElementById("scheduleList"),d=document.getElementById("scheduleModal"),h=document.getElementById("scheduleModalContent"),y=document.getElementById("scheduleForm"),x=(t=null)=>{const e=document.getElementById("scheduleModalTitle");if(t){e.textContent="Chỉnh Sửa Lịch Trình",document.getElementById("sch-edit-id").value=t.id,document.getElementById("sch-tour-name").value=t.tour_name;const a=n=>n&&n.includes("T")?n.split("T")[0]:n;document.getElementById("sch-start-date").value=a(t.start_date),document.getElementById("sch-end-date").value=a(t.end_date),document.getElementById("sch-slots").value=t.slots,document.getElementById("sch-status").value=t.status||"Đang mở"}else e.textContent="Thêm Lịch Khởi Hành",y.reset(),document.getElementById("sch-edit-id").value="";d.classList.remove("hidden"),setTimeout(()=>{d.classList.add("opacity-100"),h.classList.remove("scale-95"),h.classList.add("scale-100")},10)},g=()=>{d.classList.remove("opacity-100"),h.classList.remove("scale-100"),h.classList.add("scale-95"),setTimeout(()=>{d.classList.add("hidden")},200)};document.getElementById("addScheduleBtn").addEventListener("click",()=>x()),document.getElementById("closeScheduleModalBtn").addEventListener("click",g),document.getElementById("cancelScheduleBtn").addEventListener("click",g),d.addEventListener("click",t=>{t.target===d&&g()});const L=async()=>{try{const t=await fetch(k);if(!t.ok)throw new Error("Failed to load tours");const e=await t.json(),a=document.getElementById("sch-tour-name");a.innerHTML='<option value="">-- Chọn Tour --</option>',e.forEach(s=>{a.innerHTML+=`<option value="${s.name}">${s.name}</option>`});const n=document.getElementById("tourFilterList");let o=`
                <label class="flex items-center gap-3 cursor-pointer group">
                    <input type="radio" name="tourFilter" value="" class="text-csr-orange focus:ring-csr-orange border-gray-200 bg-gray-100" checked>
                    <span class="text-gray-500 group-hover:text-gray-900 transition-colors text-sm">Tất cả Tuyến</span>
                </label>
            `;e.forEach(s=>{o+=`
                ...
                `}),n.innerHTML=o,n.querySelectorAll('input[name="tourFilter"]').forEach(s=>{s.addEventListener("change",l=>{u=l.target.value,f()})})}catch(t){console.error("Error loading tours:",t)}},b=async()=>{c.innerHTML='<div class="text-center py-12 text-gray-400">Đang tải lịch trình...</div>';try{const t=await fetch(m);if(!t.ok)throw new Error("Failed to fetch schedules");v=await t.json(),f()}catch(t){console.error("Error loading schedules:",t),c.innerHTML='<div class="text-center py-12 text-red-400 font-medium">Lỗi kết nối server.</div>'}},f=()=>{let t=v;if(u&&(t=t.filter(e=>e.tour_name===u)),t.length===0){c.innerHTML='<div class="text-center py-12 text-gray-400 italic">Chưa có lịch trình nào'+(u?" cho tuyến này":"")+".</div>";return}c.innerHTML=t.map(e=>{const a=new Date(e.start_date),n=new Date(e.end_date),o=a.toLocaleDateString("vi-VN"),s=n.toLocaleDateString("vi-VN"),l=`${a.getDate().toString().padStart(2,"0")}/${(a.getMonth()+1).toString().padStart(2,"0")}`,p=parseInt(e.booked_count)||0,i=parseInt(e.slots)||0,w=Math.max(0,i-p),B=i>0?Math.min(100,Math.round(p/i*100)):0;let r=e.status||"Đang mở";w<=0?r="Hết chỗ":w<=3&&(r="Sắp hết"),e.status==="Hết chỗ"&&(r="Hết chỗ");const M=r==="Đang mở"?"text-green-600 bg-green-50 border-green-200":r==="Sắp hết"?"text-orange-600 bg-orange-50 border-orange-200":"text-red-500 bg-red-50 border-red-200",T=r==="Đang mở"?"bg-green-500":r==="Sắp hết"?"bg-orange-500":"bg-red-500";return`
                <div class="glass-panel p-5 border-l-4 border-csr-orange hover:shadow-md transition-all duration-200" data-schedule-id="${e.id}">
                    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <div class="flex items-center gap-4 text-sm text-gray-500 flex-wrap">
                                    ...
                                    <span class="flex items-center gap-1.5">
                                        ...
                                        <strong class="text-csr-orange">${p}</strong> / ${i} chỗ
                                    </span>
                        ...
                        <div class="flex gap-2 shrink-0">
                            ...
                            <button class="sch-details-btn bg-csr-orange hover:opacity-90 text-white rounded-lg px-4 py-2.5 text-sm font-medium transition-all flex items-center gap-2" data-tour="${e.tour_name}" data-date="${e.start_date}" title="In Danh Sách Đoàn">
            ...
            `}).join(""),c.addEventListener("click",E)},E=async t=>{const e=t.target.closest(".sch-edit-btn"),a=t.target.closest(".sch-delete-btn"),n=t.target.closest(".sch-details-btn");if(e){const o=e.getAttribute("data-id"),s=v.find(l=>l.id==o);s&&x(s)}if(a){const o=a.getAttribute("data-id");if(confirm("Bạn có chắc chắn muốn xóa lịch trình này?"))try{if((await fetch(`${m}?id=${o}`,{method:"DELETE"})).ok)b();else throw new Error("Delete failed")}catch(s){alert("Lỗi khi xóa: "+s.message)}}if(n){const o=n.getAttribute("data-tour"),s=n.getAttribute("data-date"),l=s&&s.includes("T")?s.split("T")[0]:s,i=`${window.location.origin}/ADMIN/schedule-details?tour=${encodeURIComponent(o)}&date=${encodeURIComponent(l)}`;window.open(i,"_blank")}};y.addEventListener("submit",async t=>{t.preventDefault();const e=t.target.querySelector('button[type="submit"]'),a=e.textContent;e.textContent="Đang lưu...",e.disabled=!0;const n=document.getElementById("sch-edit-id").value,o={id:n?parseInt(n):null,tour_name:document.getElementById("sch-tour-name").value,start_date:document.getElementById("sch-start-date").value,end_date:document.getElementById("sch-end-date").value,slots:parseInt(document.getElementById("sch-slots").value),status:document.getElementById("sch-status").value};try{if(!(await fetch(m,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(o)})).ok)throw new Error("Failed to save schedule");g(),b(),alert(n?"✅ Cập nhật lịch trình thành công!":"✅ Thêm lịch trình mới thành công! Form đặt chỗ trên website sẽ tự cập nhật trong 30 giây.")}catch(s){alert("❌ Lỗi: "+s.message)}finally{e.textContent=a,e.disabled=!1}}),L(),b()};export{$ as afterRender,H as render};
