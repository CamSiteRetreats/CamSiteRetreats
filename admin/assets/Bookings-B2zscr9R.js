import{S as mt,H as ht}from"./Header-n5pNPUhI.js";console.log("Bookings V3 - Cache Busted!");const yt=()=>`
    ...
    `,xt=()=>{let M=[],b="consult",H=[],W=[],z=[];const ct=async()=>{try{const[n,t,o]=await Promise.all([fetch("/api/tours"),fetch("/api/schedules"),fetch("/api/users")]);H=n.ok?await n.json():[],W=t.ok?await t.json():[],z=o.ok?await o.json():[],J("addTourName"),J("edit-tour"),ut("edit-sale")}catch(n){console.error("Lỗi tải tours/schedules:",n)}},ut=n=>{const t=document.getElementById(n);t&&(t.innerHTML='<option value="">-- Website --</option>',z.forEach(o=>{const l=document.createElement("option");l.value=o.id,l.textContent=`${o.full_name||o.fullName} (${o.role})`,t.appendChild(l)}))},J=n=>{const t=document.getElementById(n);if(!t)return;const o=t.value,l=t.options[0].outerHTML;t.innerHTML=l,H.filter(d=>d.is_visible!==!1).forEach(d=>{const r=document.createElement("option");r.value=d.name,r.textContent=d.name,d.name===o&&(r.selected=!0),t.appendChild(r)})},R=(n,t,o,l)=>{const d=document.getElementById(n);if(!d)return;if(d.innerHTML='<option value="">-- Chọn Lịch --</option>',!t){d.innerHTML='<option value="">-- Chọn Tour trước --</option>';return}const r=W.filter(s=>s.tour_name===t||t.includes(s.tour_name)||s.tour_name.includes(t));if(r.length===0){d.innerHTML='<option value="">Chưa có lịch cho tour này</option>';return}r.sort((s,a)=>new Date(s.start_date)-new Date(a.start_date)).forEach(s=>{const a=parseInt(s.booked_count)||0,u=(s.slots||0)-a,h=new Date(s.start_date),p=s.end_date?new Date(s.end_date):null,y=k=>`${k.getDate().toString().padStart(2,"0")}/${(k.getMonth()+1).toString().padStart(2,"0")}`;let i=y(h);p&&(i+=" - "+y(p)),s.group_label&&(i+=` · ${s.group_label}`),i+=` (${u>=0?u:0} chỗ trống)`;const g=document.createElement("option"),f=`${h.getDate().toString().padStart(2,"0")}/${(h.getMonth()+1).toString().padStart(2,"0")}/${h.getFullYear()}`;g.value=`${s.id}::${f}`,g.dataset.scheduleId=s.id,g.dataset.date=f,g.textContent=i,u<=0&&(g.disabled=!0,g.textContent=i.replace("chỗ trống","HẾT CHỖ")),(l&&String(s.id)===String(l)||!l&&o&&f===o)&&(g.selected=!0),d.appendChild(g)})},X=document.getElementById("addTourName");X&&X.addEventListener("change",n=>{const t=n.target.value;R("addTourDate",t);const o=H.find(d=>d.name===t),l=document.getElementById("addTourPrice");if(o&&l){const d=parseInt(o.price)||0;l.value=d;const r=document.getElementById("addDiscount"),s=document.getElementById("addTotalPriceDisplay"),a=parseInt(r?r.value:0)||0,u=d-a;s&&(s.textContent=(u>0?u:0).toLocaleString("vi-VN")+"đ")}});const Y=document.getElementById("edit-tour");Y&&Y.addEventListener("change",n=>{const t=n.target.value;R("edit-date",t);const o=H.find(d=>d.name===t),l=document.getElementById("edit-total");o&&l&&(parseInt(l.value)===0||!l.value)&&(l.value=parseInt(o.price)||0,typeof window.updateEditRemaining=="function"&&window.updateEditRemaining())}),ct();const A=()=>{const n=document.getElementById("bookingsTableHead"),t=document.getElementById("bookingsTableBody");if(!t||!n)return;const o=b==="upcoming"||b==="ready"||b==="completed";o?n.innerHTML=`
            ...
            `;const l=document.getElementById("filterSearch")?document.getElementById("filterSearch").value.toLowerCase():"",d=document.getElementById("filterTour")?document.getElementById("filterTour").value:"",r=document.getElementById("filterDate")?document.getElementById("filterDate").value:"",s=document.getElementById("filterSale")?document.getElementById("filterSale").value:"",a=document.getElementById("filterStatus")?document.getElementById("filterStatus").value:"";let u=M.filter(e=>{let m=!1;const c=parseInt(e.total_price)>0&&parseInt(e.total_price)===parseInt(e.deposit),x=C=>{if(!C)return!1;try{const T=C.split("/");if(T.length===3){const L=new Date(`${T[2]}-${T[1]}-${T[0]}T00:00:00`),E=new Date;return E.setHours(0,0,0,0),L.getTime()<E.getTime()}else if(C.includes("-")){const L=new Date(`${C}T00:00:00`),E=new Date;return E.setHours(0,0,0,0),L.getTime()<E.getTime()}}catch{}return!1},w=(e.status==="Hoàn thành"||e.status==="Đã đi"||c)&&x(e.date);return b==="consult"?m=e.status==="Chờ tư vấn"&&!w:b==="pending"?m=(!e.status||e.status==="Chờ cọc"||e.status==="Chờ xác nhận cọc")&&!w:b==="upcoming"?m=e.status&&e.status!=="Chờ tư vấn"&&e.status!=="Chờ cọc"&&e.status!=="Chờ xác nhận cọc"&&!c&&e.status!=="Hoàn thành"&&!w:b==="ready"?m=(c||e.status==="Hoàn thành")&&!w:b==="completed"&&(m=w),!(!m||d&&e.tour!==d||r&&q(e.date)!==r||s&&s!=="null"&&e.sale_name!==s||s==="null"&&e.sale_name&&e.sale_name!=="Website"||l&&!`${e.name||""} ${e.phone||""} ${e.customer_id||""}`.toLowerCase().includes(l)||a&&e.status!==a)});const h=document.getElementById("statTitle1"),p=document.getElementById("statTitle2"),y=document.getElementById("statTitle3"),i=document.getElementById("statTotalCustomers"),g=document.getElementById("statTotalRevenue"),f=document.getElementById("statTotalCollected");let k=u.length;if(b==="consult"){h&&(h.textContent="Tổng Leads Tư Vấn"),p&&(p.textContent="Chưa Có Sale Nhận"),y&&(y.textContent="Đã Có Sale Nhận");let e=0,m=0;u.forEach(c=>{!c.sale_id||c.sale_id==="null"||!c.sale_name||c.sale_name==="Website"||c.sale_name==="null"?e++:m++}),i&&(i.textContent=k),g&&(g.textContent=e+" Đơn"),f&&(f.textContent=m+" Đơn")}else if(b==="pending"){h&&(h.textContent="Tổng Khách (Chờ Cọc)"),p&&(p.textContent="Khách Chưa Tư Vấn (Mới)"),y&&(y.textContent="Đã Có Sale Giữ Chỗ");let e=0,m=0;u.forEach(c=>{!c.sale_id||c.sale_id==="null"||!c.sale_name||c.sale_name==="Website"||c.sale_name==="null"?e++:m++}),i&&(i.textContent=k),g&&(g.textContent=e+" Đơn"),f&&(f.textContent=m+" Đơn")}else if(b==="upcoming"){h&&(h.textContent="Tổng Số Khách"),p&&(p.textContent="Chưa Xác Nhận Cọc"),y&&(y.textContent="Chờ Thanh Toán Còn Lại");let e=0,m=0;u.forEach(c=>{c.status==="Chờ xác nhận cọc"?e++:parseInt(c.deposit)>0&&parseInt(c.total_price)>parseInt(c.deposit)&&m++}),i&&(i.textContent=k),g&&(g.textContent=e+" Khách"),f&&(f.textContent=m+" Khách")}else if(b==="ready"){h&&(h.textContent="Khách Sẵn Sàng (Full)"),p&&(p.textContent="Tổng Doanh Thu Tab"),y&&(y.textContent="Thực Thu (Full Tận Nơi)");let e=0,m=0;u.forEach(c=>{e+=parseInt(c.total_price)||0,m+=parseInt(c.deposit)||0}),i&&(i.textContent=k),g&&(g.textContent=e>0?e.toLocaleString("vi-VN")+"đ":"0đ"),f&&(f.textContent=m>0?m.toLocaleString("vi-VN")+"đ":"0đ")}else{h&&(h.textContent="Khách Đã Xong (Lịch Sử)"),p&&(p.textContent="Tổng Doanh Thu Tab"),y&&(y.textContent="Thực Thu (Đã Đóng)");let e=0,m=0;u.forEach(c=>{e+=parseInt(c.total_price)||0,m+=parseInt(c.deposit)||0}),i&&(i.textContent=k),g&&(g.textContent=e>0?e.toLocaleString("vi-VN")+"đ":"0đ"),f&&(f.textContent=m>0?m.toLocaleString("vi-VN")+"đ":"0đ")}const B=o?8:4;if(!u||u.length===0){let e=b==="consult"?"Chưa có khách hàng tư vấn nào.":b==="pending"?"Chưa có Khách chờ cọc.":b==="upcoming"?"Chưa có Khách nào Đã Cọc.":b==="ready"?"Chưa có khách sẵn sàng.":"Danh sách rỗng.";t.innerHTML=`<tr><td colspan="${B}" class="p-8 text-center text-gray-500">${e}</td></tr>`;return}t.innerHTML=u.map(e=>{const m=b==="upcoming"||b==="ready"?"bg-green-50/20 hover:bg-green-50 transition-colors":"hover:bg-gray-100 transition-colors";if(o){let c=parseInt(e.total_price)||0;if(c===0&&e.tour&&H&&H.length>0){const $=H.find(D=>D.name&&e.tour&&D.name.toLowerCase().trim()===e.tour.toLowerCase().trim());$&&parseInt($.price)>0&&(c=parseInt($.price))}const x=parseInt(e.deposit)||0,w=c-x,C=c>0?c.toLocaleString("vi-VN")+"đ":"Chưa định giá",T=x>0?x.toLocaleString("vi-VN")+"đ":"0đ",L=w>0?w.toLocaleString("vi-VN")+"đ":w===0&&c>0?"Đã thu trọn":"-";let E="";e.status==="Hoàn thành"||e.status==="Đã đi"?E='<span class="bg-gray-100 text-gray-600 border border-gray-200 px-2 py-0.5 rounded text-xs block w-full text-center">Hoàn thành</span>':e.status==="Chờ xác nhận cọc"?E=`<button class="action-btn confirm-deposit-btn bg-csr-orange text-white px-3 py-1.5 rounded text-xs font-bold shadow-sm hover:bg-[#d65503] w-full" data-id="${e.id}">Xác nhận cọc</button>`:x>0&&w===0?E='<span class="bg-green-100 text-green-700 border border-green-200 px-2 py-0.5 rounded text-xs font-medium block w-full text-center">Hoàn tất phí</span>':E='<span class="bg-blue-100 text-blue-700 border border-blue-200 px-2 py-0.5 rounded text-xs font-medium block w-full text-center">Đã Cọc</span>';let v="";return x>0&&(v=`<a href="/invoice.html?id=${e.id}" target="_blank" class="mt-1.5 block text-center text-[11px] text-blue-600 hover:text-blue-800 hover:underline font-medium" onclick="event.stopPropagation()">🧾 Xem hóa đơn</a>`),`
                <tr class="${m} cursor-pointer row-clickable" data-id="${e.id}">
                    ...
                    <td class="p-4 align-top text-center">${E}${v}</td>
                ...
                `}else{const c=e.name?e.name.substring(0,2).toUpperCase():"KH",x=b==="consult",w=!e.sale_id||e.sale_id==="null"||!e.sale_name||e.sale_name==="null";let C="";x&&w?C=`<button class="action-btn claim-btn bg-csr-orange hover:bg-[#d65503] text-white px-3 py-1.5 rounded text-xs font-bold shadow-sm transition-colors" data-id="${e.id}">Nhận khách</button>`:C=`<span class="text-sm text-gray-500">${e.sale_name||"Website"}</span>`;const T=`
    ...
    `}}).join(""),document.querySelectorAll(".claim-btn").forEach(e=>{e.addEventListener("click",async m=>{m.stopPropagation();const c=e.getAttribute("data-id"),x=localStorage.getItem("csr_user");let w=null,C="Admin";if(x){const T=JSON.parse(x);w=T.id,C=T.fullName||T.full_name||"Admin"}e.textContent="Đang xử lý...",e.disabled=!0;try{if((await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:c,sale_id:w,sale_name:C})})).ok){const L=M.find(E=>E.id==c);L&&(L.sale_id=w,L.sale_name=C),A()}else alert("Lỗi khi nhận khách. Vui lòng thử lại."),e.textContent="Nhận khách",e.disabled=!1}catch(T){console.error("Lỗi nhận khách:",T),e.textContent="Nhận khách",e.disabled=!1}})})},gt=()=>{const n=new Set,t=new Set,o=new Set;M.forEach(s=>{if(s.tour&&n.add(s.tour),s.date){const a=q(s.date);a&&t.add(a)}s.sale_name&&s.sale_name!=="Website"&&s.sale_name!=="null"&&o.add(s.sale_name.trim())});const l=document.getElementById("filterTour"),d=document.getElementById("filterDate"),r=document.getElementById("filterSale");if(l){const s=l.value;l.innerHTML='<option value="">Tất cả Tour</option>',[...n].sort().forEach(a=>l.innerHTML+=`<option value="${a}">${a}</option>`),l.value=s}if(d){const s=d.value;d.innerHTML='<option value="">Tất cả Ngày</option>',[...t].sort().forEach(a=>d.innerHTML+=`<option value="${a}">${a}</option>`),d.value=s}if(r){const s=r.value;r.innerHTML='<option value="">Tất cả Sale</option>',r.innerHTML+='<option value="null">Nguồn Website</option>',[...o].sort().forEach(a=>{r.innerHTML+=`<option value="${a}">${a}</option>`}),r.value=s}};function q(n){if(!n)return"";if(/^\d{1,2}\/\d{1,2}\s*-\s*\d{1,2}\/\d{1,2}$/.test(n))return n.replace(/\s+/g,"");try{if(/^\d{4}-\d{2}-\d{2}$/.test(n)){const[t,o,l]=n.split("-"),d=`${parseInt(l)}/${parseInt(o)}`;return`${d}-${d}`}if(/^\d{1,2}\/\d{1,2}\/\d{4}$/.test(n)){const[t,o,l]=n.split("/"),d=`${parseInt(t)}/${parseInt(o)}`;return`${d}-${d}`}if(n.includes("-"))return n.split("-").map(o=>{const l=o.trim();if(/\/\d{4}$/.test(l)){const[d,r]=l.split("/");return`${parseInt(d)}/${parseInt(r)}`}return l}).join("-")}catch{console.warn("Date normalization failed for:",n)}return n}const S=async()=>{try{M=await(await fetch("/api/bookings")).json(),gt(),A()}catch(n){console.error("Lỗi parse list đơn hàng:",n);const t=document.getElementById("bookingsTableBody");t&&(t.innerHTML='<tr><td colspan="4" class="p-8 text-center text-red-500">Lỗi lấy dữ liệu từ Server.</td></tr>')}};S(),["filterSearch","filterTour","filterDate","filterSale","filterStatus"].forEach(n=>{const t=document.getElementById(n);t&&(n==="filterSearch"?t.addEventListener("input",A):t.addEventListener("change",A))});const Q=document.getElementById("exportExcelBtn");Q&&Q.addEventListener("click",()=>{const n=document.getElementById("filterSearch")?document.getElementById("filterSearch").value.toLowerCase():"",t=document.getElementById("filterTour")?document.getElementById("filterTour").value:"",o=document.getElementById("filterDate")?document.getElementById("filterDate").value:"",l=document.getElementById("filterSale")?document.getElementById("filterSale").value:"";let d=M.filter(i=>{let g=!1;const f=parseInt(i.total_price)>0&&parseInt(i.total_price)===parseInt(i.deposit),k=e=>{if(!e)return!1;try{const m=e.split("/");if(m.length===3){const c=new Date(`${m[2]}-${m[1]}-${m[0]}T00:00:00`),x=new Date;return x.setHours(0,0,0,0),c.getTime()<x.getTime()}else if(e.includes("-")){const c=new Date(`${e}T00:00:00`),x=new Date;return x.setHours(0,0,0,0),c.getTime()<x.getTime()}}catch{}return!1},B=(i.status==="Hoàn thành"||i.status==="Đã đi"||f)&&k(i.date);return b==="consult"?g=i.status==="Chờ tư vấn"&&!B:b==="pending"?g=(!i.status||i.status==="Chờ cọc")&&!B:b==="upcoming"?g=i.status&&i.status!=="Chờ tư vấn"&&i.status!=="Chờ cọc"&&!f&&i.status!=="Hoàn thành"&&!B:b==="ready"?g=(f||i.status==="Hoàn thành")&&!B:b==="completed"&&(g=B),!(!g||t&&i.tour!==t||o&&q(i.date)!==o||l&&l!=="null"&&i.sale_name!==l||l==="null"&&i.sale_name&&i.sale_name!=="Website"||n&&!`${i.name||""} ${i.phone||""} ${i.customer_id||""} `.toLowerCase().includes(n))});if(d.length===0){alert("Danh sách rỗng. Không có dữ liệu để xuất.");return}const s=[["Mã KH (CRM)","Họ và Tên","Số điện thoại","Tên Tour","Lịch trình","Trạng thái","Nguồn / Sale","Đã cọc","Tổng tiền","Ghi chú"]];d.forEach(i=>{s.push([i.customer_id||"",`"${i.name||""}"`,`= "${i.phone||""}"`,`"${i.tour||""}"`,`"${i.date||""}"`,`"${i.status||""}"`,`"${i.sale_name||"Website"}"`,i.deposit||0,i.total_price||0,`"${(i.special||"").replace(/"/g,'""')}"`])});const a="\\uFEFF"+s.map(i=>i.join(",")).join(`
`),u=new Blob([a],{type:"text/csv;charset=utf-8;"}),h=URL.createObjectURL(u),p=document.createElement("a");p.setAttribute("href",h);let y=b==="pending"?"ChoCoc":b==="upcoming"?"SapThamGia":b==="ready"?"ChoLenXe":"LichSu";p.setAttribute("download",`BaoCao_DonHang_${y}_${new Date().toISOString().slice(0,10)}.csv`),document.body.appendChild(p),p.click(),document.body.removeChild(p)});const Z=document.querySelectorAll(".tab-btn");Z.forEach(n=>{n.addEventListener("click",t=>{Z.forEach(o=>{o.classList.remove("border-csr-orange","text-csr-orange"),o.classList.add("border-transparent","text-gray-500")}),t.target.classList.add("border-csr-orange","text-csr-orange"),t.target.classList.remove("border-transparent","text-gray-500"),b=t.target.getAttribute("data-tab"),A()})});const N=document.getElementById("bookingModal"),j=document.getElementById("bookingModalContent"),F=document.getElementById("detailModal"),tt=document.getElementById("detailModalContent"),K=document.getElementById("editModal"),U=document.getElementById("editModalContent"),et=document.getElementById("addBookingBtn");N&&(N.classList.add("duration-200","ease-out"),j.classList.add("duration-300","ease-out")),K&&(K.classList.add("duration-200","ease-out"),U.classList.add("duration-300","ease-out")),et&&et.addEventListener("click",()=>{document.getElementById("bookingModalContent").querySelector("h2").innerText="Thêm Khách Hàng (Tạo Đơn)",document.getElementById("submitBookingBtn").innerText="Tạo Phơi Đăng Ký",document.getElementById("editingBookingId").value="",document.getElementById("bookingForm").reset();const n=document.getElementById("loyalty-alert");n&&n.remove(),N.classList.remove("hidden"),setTimeout(()=>{N.classList.add("opacity-100"),j.classList.remove("scale-95","translate-y-4"),j.classList.add("scale-100","translate-y-0")},10)}),window.closeModal=()=>{N&&(N.classList.remove("opacity-100"),j.classList.remove("scale-100","translate-y-0"),j.classList.add("scale-95","translate-y-4"),setTimeout(()=>{N.classList.add("hidden")},200))},window.closeDetailModal=()=>{F&&(F.classList.remove("opacity-100"),tt.classList.remove("scale-100","translate-y-0"),tt.classList.add("scale-95","translate-y-4"),setTimeout(()=>{F.classList.add("hidden")},200))},window.closeEditModal=()=>{K&&(K.classList.remove("opacity-100"),U.classList.remove("scale-100","translate-y-0"),U.classList.add("scale-95","translate-y-4"),setTimeout(()=>{K.classList.add("hidden")},200))},window.openRowActionModal=n=>{document.getElementById("ramName").textContent=n.name||"Khách Hàng Này",document.getElementById("ramPhone").textContent=n.customer_id?`🥇 ${n.customer_id.toUpperCase()}`:n.phone||"";const t=`
        ...
        `;document.getElementById("detailContentBlock").innerHTML=d,document.getElementById("detailModal").classList.remove("hidden"),setTimeout(()=>{document.getElementById("detailModal").classList.add("opacity-100");const r=document.getElementById("detailModalContent");r&&(r.classList.remove("scale-95","translate-y-4"),r.classList.add("scale-100","translate-y-0"))},10)},window.actionEdit=async n=>{document.getElementById("rowActionModal").classList.add("hidden");const t=M.find(a=>a.id==n);if(!t)return;document.getElementById("edit-id").value=n,document.getElementById("edit-name").value=t.name||"",document.getElementById("edit-phone").value=t.phone||"",document.getElementById("edit-medal-name").value=t.medal_name||"",document.getElementById("edit-tour").value=t.tour||"",typeof R=="function"&&R("edit-date",t.tour||"",t.date||"",t.schedule_id||null),(a=>{let u=a.dob||"";if(typeof u=="string"&&u.includes("/")){const h=u.split("/");h.length===3&&(u=`${h[2]}-${h[1].padStart(2,"0")}-${h[0].padStart(2,"0")}`)}document.getElementById("edit-dob").value=u,document.getElementById("edit-gender").value=a.gender||"Khác",document.getElementById("edit-allergy").value=a.allergy||a.medical_notes||"",document.getElementById("edit-address").value=a.address||"",document.getElementById("edit-diet").value=a.diet||a.dietary||"Không",document.getElementById("edit-trekking-pole").value=a.trekking_pole||"Không"})(t),document.getElementById("edit-status").value=t.status||"Chờ xác nhận cọc",document.getElementById("edit-commitments").checked=!!t.commitments,document.getElementById("edit-special").value=t.special||"";try{const a=localStorage.getItem("csr_user");if(a&&JSON.parse(a).role==="admin"){const h=document.getElementById("edit-sale-container");h&&h.classList.remove("hidden");const p=document.getElementById("edit-sale");p&&(p.value=t.sale_id||"")}}catch{}if((!t.dob||!t.address||!t.gender||t.gender==="Khác")&&(t.customer_id||t.phone))try{const a=t.customer_id||t.phone,u=await fetch(`/api/admin_customers?action=search&keyword=${encodeURIComponent(a)}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({keyword:a})}),h=await u.json();if(u.ok&&h.success){const p=h.data,y=document.getElementById("edit-dob"),i=document.getElementById("edit-address"),g=document.getElementById("edit-allergy"),f=document.getElementById("edit-diet"),k=document.getElementById("edit-gender"),B=document.getElementById("edit-medal-name");y&&!y.value&&(y.value=p.dob||""),i&&!i.value&&(i.value=p.address||""),g&&!g.value&&(g.value=p.medical_notes||""),f&&(f.value==="Không"||!f.value)&&(f.value=p.dietary||"Không"),k&&(k.value==="Khác"||!k.value)&&(k.value=p.gender||"Khác"),B&&!B.value&&(B.value=p.medal_name||"")}}catch(a){console.error("Pull CRM error:",a)}let d=(parseInt(t.total_price)||0)+(parseInt(t.discount)||0);if(d===0&&t.tour){const a=H.find(u=>u.name===t.tour);a&&parseInt(a.price)>0&&(d=parseInt(a.price))}document.getElementById("edit-total").value=d,document.getElementById("edit-discount").value=t.discount||0,document.getElementById("edit-deposit").value=t.deposit||0,document.getElementById("edit-deposit-required").value=t.deposit_required||1e6,window.updateEditRemaining();const r=document.getElementById("editModal"),s=document.getElementById("editModalContent");r&&s&&(r.classList.remove("hidden"),setTimeout(()=>{r.classList.add("opacity-100"),s.classList.remove("scale-95","translate-y-4"),s.classList.add("scale-100","translate-y-0")},10))},window.actionDelete=async n=>{if(document.getElementById("rowActionModal").classList.add("hidden"),confirm("Bạn có chắc chắn muốn xóa Đơn hàng này? Thao tác không thể hoàn tác."))try{if((await fetch(`/api/bookings?id=${n}`,{method:"DELETE"})).ok)alert("✅ Đã xóa đơn hàng thành công!"),S();else throw new Error("Lỗi từ Server")}catch(t){alert("❌ Không thể xóa bảng ghi này: "+t.message)}},window.updateEditRemaining=()=>{const n=parseInt(document.getElementById("edit-total").value)||0,t=parseInt(document.getElementById("edit-discount").value)||0,o=parseInt(document.getElementById("edit-deposit").value)||0,d=n-t-o,r=document.getElementById("edit-remaining");r&&(r.textContent=d>0?d.toLocaleString("vi-VN")+"đ":"0đ")};const nt=document.getElementById("edit-total"),ot=document.getElementById("edit-deposit");nt&&nt.addEventListener("input",window.updateEditRemaining),ot&&ot.addEventListener("input",window.updateEditRemaining),document.querySelectorAll('#bookingModal button[onclick*="hidden"]').forEach(n=>{n.removeAttribute("onclick"),n.addEventListener("click",window.closeModal)});const at=async n=>{try{const o=await(await fetch("/api/admin_customers?action=create",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({full_name:n.name,phone:n.phone,cccd:n.id_card||"",dob:n.dob||null,gender:n.gender||"Khác",medical_notes:n.allergy||"",dietary:n.diet||""})})).json();o.success&&o.csr_code&&(console.log("✅ Đã đồng bộ CRM thành công:",n.name,o.csr_code),(await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:n.id,customer_id:o.csr_code})})).ok&&(console.log("✅ Đã cập nhật Booking mã CRM"),typeof S=="function"&&S()))}catch(t){console.error("❌ Lỗi đồng bộ CRM:",t)}},st=document.getElementById("bookingsTableBody");st&&st.addEventListener("click",async n=>{const t=n.target.closest(".action-btn");if(!t){const r=n.target.closest(".row-clickable");if(r){const s=r.getAttribute("data-id"),a=M.find(u=>u.id==s);a&&openRowActionModal(a)}return}const o=t.getAttribute("data-id"),l=M.find(r=>r.id==o);if(!l)return;const d=document.getElementById("rowActionModal");if(d&&d.classList.add("hidden"),t.classList.contains("confirm-deposit-btn")){if(confirm("Xác nhận khách hàng này đã chuyển khoản đặt cọc?"))try{if(t.disabled=!0,t.classList.add("opacity-50"),(await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:o,status:"Đã cọc"})})).ok)alert("✅ Đã xác nhận cọc thành công!"),at(l),typeof S=="function"&&S();else throw new Error("Lỗi cập nhật API")}catch(r){alert("❌ Lỗi: "+r.message)}finally{t.disabled=!1,t.classList.remove("opacity-50")}return}if(t.classList.contains("delete-btn")){window.actionDelete(o);return}else if(t.classList.contains("process-btn")){const a=(window.location.hostname==="localhost"||window.location.hostname==="127.0.0.1"?"http://localhost:8888":window.location.origin)+`/booking/process.html?id=${o}`;navigator.clipboard.writeText(a).then(()=>{alert("📋 Đã sao chép Link Điền Thông Tin!\\nGửi cho khách: "+a)}).catch(u=>{alert("Lỗi khi Copy Clipboard. Link là: "+a)})}else if(t.classList.contains("pay-terms-btn")){const a=(window.location.hostname==="localhost"||window.location.hostname==="127.0.0.1"?"http://localhost:8888":window.location.origin)+`/pay-terms.html?id=${o}`;navigator.clipboard.writeText(a).then(()=>{alert("📋 Đã sao chép Link Cọc Tour!\\nGửi cho khách: "+a)}).catch(u=>{alert("Lỗi khi Copy Clipboard. Link là: "+a)})}else if(t.classList.contains("send-btn")){const a=(window.location.hostname==="localhost"||window.location.hostname==="127.0.0.1"?"http://localhost:8888":window.location.origin)+`/pay?id=${o}`;navigator.clipboard.writeText(a).then(()=>{alert("📋 Đã sao chép Link Thanh Toán Cọc! Gửi cho khách qua Zalo nhé:\\n"+a)}).catch(u=>{alert("Lỗi khi Copy Clipboard. Link là: "+a)})}else if(t.classList.contains("payment-btn")){const a=(window.location.hostname==="localhost"||window.location.hostname==="127.0.0.1"?"http://localhost:8888":window.location.origin)+`/pay?id=${o}`;window.open(a,"_blank")}else t.classList.contains("view-btn")?(document.getElementById("rowActionModal").classList.add("hidden"),window.actionView(o)):t.classList.contains("edit-btn")&&window.actionEdit(o)});const P=document.getElementById("smartSearch"),_=document.getElementById("searchSuggestions");if(P){let n;P.addEventListener("input",t=>{const o=t.target.value.toLowerCase().trim();if(clearTimeout(n),o.length<2){_&&_.classList.add("hidden");return}n=setTimeout(async()=>{try{const l=await fetch(`/api/admin_customers?action=search&keyword=${o}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({keyword:o})}),d=await l.json();if(l.ok&&d.success){const s=[d.data];_&&(_.innerHTML=s.map(a=>`
                        ...
                        `).join(""),_.classList.remove("hidden"))}else _&&(_.innerHTML='<div class="p-3 text-xs text-gray-400 italic text-center">Không tìm thấy khách hàng cũ</div>',_.classList.remove("hidden"))}catch(l){console.error("Search error:",l)}},300)}),_&&_.addEventListener("click",t=>{const o=t.target.closest(".suggestion-item");if(o){const l=document.getElementById("addFullName"),d=document.getElementById("addPhone"),r=document.getElementById("addCsrCode");l&&(l.value=o.getAttribute("data-name")),d&&(d.value=o.getAttribute("data-phone")),r&&(r.value=o.getAttribute("data-csr"));const s=document.getElementById("addDob"),a=document.getElementById("addGender"),u=document.getElementById("addIdCard"),h=document.getElementById("addAddress");s&&(s.value=o.getAttribute("data-dob")||""),a&&(a.value=o.getAttribute("data-gender")||"Khác"),u&&(u.value=o.getAttribute("data-cccd")||""),h&&(h.value=o.getAttribute("data-address")||"");const p=document.getElementById("addAllergy"),y=document.getElementById("addDiet"),i=document.getElementById("addTrekkingPole"),g=document.getElementById("addMedalName");p&&(p.value=o.getAttribute("data-allergy")||""),y&&(y.value=o.getAttribute("data-diet")||""),i&&(i.value=o.getAttribute("data-trekking_pole")||"Không"),g&&(g.value=o.getAttribute("data-medal-name")||""),window._selectedCustomer={fullName:o.getAttribute("data-name"),phone:o.getAttribute("data-phone"),csrCode:o.getAttribute("data-csr"),dob:o.getAttribute("data-dob")||"",gender:o.getAttribute("data-gender")||"",address:o.getAttribute("data-address")||"",id_card:o.getAttribute("data-cccd")||"",allergy:o.getAttribute("data-allergy")||"",diet:o.getAttribute("data-diet")||"",trekking_pole:o.getAttribute("data-trekking_pole")||"",medal_name:o.getAttribute("data-medal-name")||""},P.value=o.getAttribute("data-name"),_.classList.add("hidden");const f=`
                    ...
                    </div>`,k=document.getElementById("loyalty-alert");k&&k.remove(),P.parentElement.insertAdjacentHTML("afterend",f)}}),document.addEventListener("click",t=>{!P.contains(t.target)&&_&&!_.contains(t.target)&&_.classList.add("hidden")})}const V=document.getElementById("addTourPrice"),G=document.getElementById("addDiscount"),it=document.getElementById("addTotalPriceDisplay");document.getElementById("addTourName");const lt=()=>{if(!V||!G||!it)return;const n=parseInt(V.value)||0,t=parseInt(G.value)||0,o=n-t;it.textContent=(o>0?o:0).toLocaleString("vi-VN")+"đ"};V&&V.addEventListener("input",lt),G&&G.addEventListener("input",lt);const dt=document.getElementById("searchBtn");dt&&dt.addEventListener("click",n=>{n.preventDefault(),document.getElementById("smartSearch").value||alert("Vui lòng nhập Số Điện Thoại hoặc Mã #CSR")});const O=document.getElementById("bookingForm");O&&O.addEventListener("submit",async n=>{n.preventDefault();const t=n.target.querySelector('button[type="submit"]'),o=t.textContent;t.textContent="Đang lưu hệ thống...",t.disabled=!0;try{const l=document.getElementById("addFullName").value,d=document.getElementById("addPhone").value,r=document.getElementById("addTourName").value,s=document.getElementById("addTourDate").value;let a=null,u=s;if(s&&s.includes("::")){const I=s.split("::");a=parseInt(I[0])||null,u=I[1]}const h=document.getElementById("addDob").value,p=document.getElementById("addGender").value,y=document.getElementById("addIdCard").value,i=document.getElementById("addAddress").value,g=document.getElementById("addSpecial").value,f=document.getElementById("addAllergy").value,k=document.getElementById("addDiet").value,B=document.getElementById("addTrekkingPole").value,e=document.getElementById("addMedalName").value,m=parseInt(document.getElementById("addTourPrice").value)||0,c=parseInt(document.getElementById("addDiscount").value)||0,x=parseInt(document.getElementById("addDepositRequired").value)||1e6,w=document.getElementById("addCsrCode").value||"",C=localStorage.getItem("csr_user");let T=null,L="Admin";if(C){const I=JSON.parse(C);T=I.id,L=I.fullName||I.full_name||"Admin"}const E=document.getElementById("editingBookingId")?document.getElementById("editingBookingId").value:"",v={name:l,phone:d,tour:r,date:u,status:"Chờ cọc",total_price:m-c,discount:c,deposit:0,sale_id:T,sale_name:L,customer_id:w,deposit_required:x,commitments:!0,schedule_id:a,dob:h,gender:p,address:i,id_card:y,special:g,allergy:f,diet:k,trekking_pole:B,medal_name:e},$=d.replace(/[^0-9]/g,"");window._selectedCustomer&&(window._selectedCustomer.phone||"").replace(/[^0-9]/g,"")===$&&(v.dob=v.dob||window._selectedCustomer.dob,v.gender=v.gender&&v.gender!=="Khác"?v.gender:window._selectedCustomer.gender||"Khác",v.address=v.address||window._selectedCustomer.address,v.id_card=v.id_card||window._selectedCustomer.id_card,v.allergy=window._selectedCustomer.allergy,v.diet=window._selectedCustomer.diet,v.trekking_pole=window._selectedCustomer.trekking_pole,v.medal_name=window._selectedCustomer.medal_name),E&&(v.id=E,delete v.status,delete v.deposit,delete v.sale_id,delete v.sale_name);const D=await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(v)});if(D.ok){alert(E?"✅ Đã lưu Cập nhật Đơn hàng!":"✅ Thêm Đơn Hàng Thành Công!"),window.closeModal(),O&&O.reset(),window._selectedCustomer=null;const I=document.getElementById("loyalty-alert");I&&I.remove(),document.getElementById("addTotalPriceDisplay")&&(document.getElementById("addTotalPriceDisplay").textContent="0đ"),typeof S=="function"?S():window.location.reload()}else{const I=await D.json();throw new Error(I.error||"Lỗi tạo Booking!")}}catch(l){alert("❌ "+l.message)}finally{t.textContent=o,t.disabled=!1}});const rt=document.getElementById("editForm");rt&&rt.addEventListener("submit",async n=>{n.preventDefault();const t=n.target.querySelector('button[type="submit"]'),o=t.textContent;t.textContent="Đang lưu cập nhật...",t.disabled=!0;try{const l=document.getElementById("edit-id").value,d=document.getElementById("edit-name").value,r=document.getElementById("edit-phone").value,s=document.getElementById("edit-medal-name").value,a=document.getElementById("edit-tour").value,u=document.getElementById("edit-date").value;let h=u,p;if(u&&u.includes("::")){const I=u.split("::");p=parseInt(I[0])||null,h=I[1]}const y=document.getElementById("edit-dob").value,i=document.getElementById("edit-gender").value,g=document.getElementById("edit-status").value,f=document.getElementById("edit-allergy").value,k=document.getElementById("edit-address").value,B=document.getElementById("edit-diet").value,e=document.getElementById("edit-trekking-pole").value,m=document.getElementById("edit-commitments").checked,c=document.getElementById("edit-special").value,x=document.getElementById("edit-sale");let w,C;if(x&&x.parentElement&&!x.parentElement.classList.contains("hidden")&&(w=x.value||null,C="Website",w)){const I=z.find(pt=>String(pt.id)===String(w));I&&(C=I.full_name||I.fullName)}const T=parseInt(document.getElementById("edit-total").value)||0,L=parseInt(document.getElementById("edit-discount").value)||0,E=parseInt(document.getElementById("edit-deposit").value)||0,v=parseInt(document.getElementById("edit-deposit-required").value)||1e6,$={id:l,name:d,phone:r,medal_name:s,tour:a,date:h,dob:y,gender:i,status:g,allergy:f,address:k,diet:B,trekking_pole:e,commitments:m,special:c,sale_id:w,sale_name:C,total_price:T-L,discount:L,deposit:E,deposit_required:v,...p!==void 0?{schedule_id:p}:{}},D=await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify($)});if(D.ok)alert("✅ Đã cập nhật thành công Chi tiết Đơn hàng!"),(g.includes("Đã cọc")||$.total_price>0&&$.total_price===E)&&at($),window.closeEditModal(),typeof S=="function"&&S();else{const I=await D.json();throw new Error(I.error||"Gặp sự cố khi Cập nhật API.")}}catch(l){alert("❌ Lỗi Cập nhật: "+l.message)}finally{t.textContent=o,t.disabled=!1}})};export{xt as afterRender,yt as render};
