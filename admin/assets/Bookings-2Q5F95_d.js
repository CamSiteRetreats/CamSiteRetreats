import{S as xe,H as ye}from"./Header-Dmidaim7.js";console.log("Bookings V3 - Cache Busted!");const ke=()=>`
    ...
    `,Ce=()=>{let _=[],C="consult",D=[],Y=[],q=[];const me=async()=>{try{const[n,e,a]=await Promise.all([fetch("/api/tours"),fetch("/api/schedules"),fetch("/api/users")]);D=n.ok?await n.json():[],Y=e.ok?await e.json():[],q=a.ok?await a.json():[],W("addTourName"),W("edit-tour"),W("wl-tour"),he("edit-sale")}catch(n){console.error("Lỗi tải tours/schedules:",n)}},he=n=>{const e=document.getElementById(n);e&&(e.innerHTML='<option value="">-- Website --</option>',q.forEach(a=>{const i=document.createElement("option");i.value=a.id,i.textContent=`${a.full_name||a.fullName} (${a.role})`,e.appendChild(i)}))},W=n=>{const e=document.getElementById(n);if(!e)return;const a=e.value,i=e.options[0].outerHTML;e.innerHTML=i,D.filter(r=>r.is_visible!==!1).forEach(r=>{const c=document.createElement("option");c.value=r.name,c.textContent=r.name,r.name===a&&(c.selected=!0),e.appendChild(c)})},j=(n,e,a,i)=>{const r=document.getElementById(n);if(!r)return;if(r.innerHTML='<option value="">-- Chọn Lịch --</option>',!e){r.innerHTML='<option value="">-- Chọn Tour trước --</option>';return}const c=Y.filter(o=>o.tour_name===e||e.includes(o.tour_name)||o.tour_name.includes(e));if(c.length===0){r.innerHTML='<option value="">Chưa có lịch cho tour này</option>';return}c.sort((o,s)=>new Date(o.start_date)-new Date(s.start_date)).forEach(o=>{const s=parseInt(o.booked_count)||0,l=(o.slots||0)-s,m=new Date(o.start_date),d=o.end_date?new Date(o.end_date):null,b=x=>`${x.getDate().toString().padStart(2,"0")}/${(x.getMonth()+1).toString().padStart(2,"0")}`;let y=b(m);d&&(y+=" - "+b(d)),o.group_label&&(y+=` · ${o.group_label}`),y+=` (${l>=0?l:0} chỗ trống)`;const f=document.createElement("option"),p=`${m.getDate().toString().padStart(2,"0")}/${(m.getMonth()+1).toString().padStart(2,"0")}/${m.getFullYear()}`;f.value=`${o.id}::${p}`,f.dataset.scheduleId=o.id,f.dataset.date=p,f.textContent=y,l<=0&&(f.disabled=!0,f.textContent=y.replace("chỗ trống","HẾT CHỖ")),(i&&String(o.id)===String(i)||!i&&a&&p===a)&&(f.selected=!0),r.appendChild(f)})},Z=document.getElementById("addTourName");Z&&Z.addEventListener("change",n=>{const e=n.target.value;j("addTourDate",e);const a=D.find(r=>r.name===e),i=document.getElementById("addTourPrice");if(a&&i){const r=parseInt(a.price)||0;i.value=r;const c=document.getElementById("addDiscount"),o=document.getElementById("addTotalPriceDisplay"),s=parseInt(c?c.value:0)||0,l=r-s;o&&(o.textContent=(l>0?l:0).toLocaleString("vi-VN")+"đ")}});const Q=document.getElementById("edit-tour");Q&&Q.addEventListener("change",n=>{const e=n.target.value;j("edit-date",e);const a=D.find(r=>r.name===e),i=document.getElementById("edit-total");a&&i&&(parseInt(i.value)===0||!i.value)&&(i.value=parseInt(a.price)||0,typeof window.updateEditRemaining=="function"&&window.updateEditRemaining())});const ee=document.getElementById("wl-tour");ee&&ee.addEventListener("change",n=>{const e=n.target.value;j("wl-date",e)}),me();const A=()=>{const n=document.getElementById("bookingsTableHead"),e=document.getElementById("bookingsTableBody");if(!e||!n)return;const a=C==="upcoming"||C==="ready"||C==="completed";a?n.innerHTML=`
            ...
            `;const i=document.getElementById("filterSearch")?document.getElementById("filterSearch").value.toLowerCase():"",r=document.getElementById("filterTour")?document.getElementById("filterTour").value:"",c=document.getElementById("filterDate")?document.getElementById("filterDate").value:"",o=document.getElementById("filterSale")?document.getElementById("filterSale").value:"",s=document.getElementById("filterStatus")?document.getElementById("filterStatus").value:"";let l=_.filter(t=>{let u=!1;const g=parseInt(t.total_price)>0&&parseInt(t.total_price)===parseInt(t.deposit),v=T=>{if(!T)return!1;try{const I=T.split("/");if(I.length===3){const L=new Date(`${I[2]}-${I[1]}-${I[0]}T00:00:00`),E=new Date;return E.setHours(0,0,0,0),L.getTime()<E.getTime()}else if(T.includes("-")){const L=new Date(`${T}T00:00:00`),E=new Date;return E.setHours(0,0,0,0),L.getTime()<E.getTime()}}catch{}return!1},h=(t.status==="Hoàn thành"||t.status==="Đã đi"||g)&&v(t.date);return C==="consult"?u=t.status==="Chờ tư vấn"&&!h:C==="pending"?u=(!t.status||t.status==="Chờ cọc"||t.status==="Chờ xác nhận cọc")&&!h:C==="upcoming"?u=t.status&&t.status!=="Chờ tư vấn"&&t.status!=="Chờ cọc"&&t.status!=="Chờ xác nhận cọc"&&!g&&t.status!=="Hoàn thành"&&!h:C==="ready"?u=(g||t.status==="Hoàn thành")&&!h:C==="completed"&&(u=h),!(!u||r&&t.tour!==r||c&&F(t.date)!==c||o&&o!=="null"&&t.sale_name!==o||o==="null"&&t.sale_name&&t.sale_name!=="Website"||i&&!`${t.name||""} ${t.phone||""} ${t.customer_id||""} ${t.id||""} csr${t.id||""} #csr${t.id||""}`.toLowerCase().includes(i)||s&&t.status!==s)});const m=document.getElementById("statTitle1"),d=document.getElementById("statTitle2"),b=document.getElementById("statTitle3"),y=document.getElementById("statTotalCustomers"),f=document.getElementById("statTotalRevenue"),p=document.getElementById("statTotalCollected");let x=l.length;if(C==="consult"){const t=document.getElementById("addWaitingBtn"),u=document.getElementById("addBookingBtn");t&&t.classList.remove("hidden"),u&&u.classList.add("hidden"),m&&(m.textContent="Tổng Đăng Ký Chờ Lịch"),d&&(d.textContent="Chưa Được Liên Hệ"),b&&(b.textContent="Đã Có Sale Phụ Trách");let g=0,v=0;l.forEach(h=>{!h.sale_id||h.sale_id==="null"||!h.sale_name||h.sale_name==="Website"||h.sale_name==="null"?g++:v++}),y&&(y.textContent=x),f&&(f.textContent=g+" Người"),p&&(p.textContent=v+" Người")}else if(C==="pending"){const t=document.getElementById("addWaitingBtn"),u=document.getElementById("addBookingBtn");t&&t.classList.add("hidden"),u&&u.classList.remove("hidden"),m&&(m.textContent="Tổng Khách (Chờ Cọc)"),d&&(d.textContent="Khách Chưa Tư Vấn (Mới)"),b&&(b.textContent="Đã Có Sale Giữ Chỗ");let g=0,v=0;l.forEach(h=>{!h.sale_id||h.sale_id==="null"||!h.sale_name||h.sale_name==="Website"||h.sale_name==="null"?g++:v++}),y&&(y.textContent=x),f&&(f.textContent=g+" Đơn"),p&&(p.textContent=v+" Đơn")}else if(C==="upcoming"){m&&(m.textContent="Tổng Số Khách"),d&&(d.textContent="Chưa Xác Nhận Cọc"),b&&(b.textContent="Chờ Thanh Toán Còn Lại");let t=0,u=0;l.forEach(g=>{g.status==="Chờ xác nhận cọc"?t++:parseInt(g.deposit)>0&&parseInt(g.total_price)>parseInt(g.deposit)&&u++}),y&&(y.textContent=x),f&&(f.textContent=t+" Khách"),p&&(p.textContent=u+" Khách")}else if(C==="ready"){m&&(m.textContent="Khách Sẵn Sàng (Full)"),d&&(d.textContent="Tổng Doanh Thu Tab"),b&&(b.textContent="Thực Thu (Full Tận Nơi)");let t=0,u=0;l.forEach(g=>{t+=parseInt(g.total_price)||0,u+=parseInt(g.deposit)||0}),y&&(y.textContent=x),f&&(f.textContent=t>0?t.toLocaleString("vi-VN")+"đ":"0đ"),p&&(p.textContent=u>0?u.toLocaleString("vi-VN")+"đ":"0đ")}else{m&&(m.textContent="Khách Đã Xong (Lịch Sử)"),d&&(d.textContent="Tổng Doanh Thu Tab"),b&&(b.textContent="Thực Thu (Đã Đóng)");let t=0,u=0;l.forEach(g=>{t+=parseInt(g.total_price)||0,u+=parseInt(g.deposit)||0}),y&&(y.textContent=x),f&&(f.textContent=t>0?t.toLocaleString("vi-VN")+"đ":"0đ"),p&&(p.textContent=u>0?u.toLocaleString("vi-VN")+"đ":"0đ")}const $=a?8:4;if(!l||l.length===0){let t=C==="consult"?"Chưa có khách hàng tư vấn nào.":C==="pending"?"Chưa có Khách chờ cọc.":C==="upcoming"?"Chưa có Khách nào Đã Cọc.":C==="ready"?"Chưa có khách sẵn sàng.":"Danh sách rỗng.";e.innerHTML=`<tr><td colspan="${$}" class="p-8 text-center text-gray-500">${t}</td></tr>`;return}e.innerHTML=l.map(t=>{const u=C==="upcoming"||C==="ready"?"bg-green-50/20 hover:bg-green-50 transition-colors":"hover:bg-gray-100 transition-colors";if(a){let g=parseInt(t.total_price)||0;if(g===0&&t.tour&&D&&D.length>0){const w=D.find(H=>H.name&&t.tour&&H.name.toLowerCase().trim()===t.tour.toLowerCase().trim());w&&parseInt(w.price)>0&&(g=parseInt(w.price))}const v=parseInt(t.deposit)||0,h=g-v,T=g>0?g.toLocaleString("vi-VN")+"đ":"Chưa định giá",I=v>0?v.toLocaleString("vi-VN")+"đ":"0đ",L=h>0?h.toLocaleString("vi-VN")+"đ":h===0&&g>0?"Đã thu trọn":"-";let E="";t.status==="Hoàn thành"||t.status==="Đã đi"?E='<span class="bg-gray-100 text-gray-600 border border-gray-200 px-2 py-0.5 rounded text-xs block w-full text-center">Hoàn thành</span>':t.status==="Chờ xác nhận cọc"?E=`<button class="action-btn confirm-deposit-btn bg-csr-orange text-white px-3 py-1.5 rounded text-xs font-bold shadow-sm hover:bg-[#d65503] w-full" data-id="${t.id}">Xác nhận cọc</button>`:v>0&&h===0?E='<span class="bg-green-100 text-green-700 border border-green-200 px-2 py-0.5 rounded text-xs font-medium block w-full text-center">Hoàn tất phí</span>':E='<span class="bg-blue-100 text-blue-700 border border-blue-200 px-2 py-0.5 rounded text-xs font-medium block w-full text-center">Đã Cọc</span>';let k="";return v>0&&(k=`<a href="/invoice.html?id=${t.id}" target="_blank" class="mt-1.5 block text-center text-[11px] text-blue-600 hover:text-blue-800 hover:underline font-medium" onclick="event.stopPropagation()">🧾 Xem hóa đơn</a>`),`
    ...
    `}}).join(""),document.querySelectorAll(".claim-btn").forEach(t=>{t.addEventListener("click",async u=>{u.stopPropagation();const g=t.getAttribute("data-id"),v=localStorage.getItem("csr_user");let h=null,T="Admin";if(v){const I=JSON.parse(v);h=I.id,T=I.fullName||I.full_name||"Admin"}t.textContent="Đang xử lý...",t.disabled=!0;try{if((await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:g,sale_id:h,sale_name:T})})).ok){const L=_.find(E=>E.id==g);L&&(L.sale_id=h,L.sale_name=T),A()}else alert("Lỗi khi nhận khách. Vui lòng thử lại."),t.textContent="Nhận khách",t.disabled=!1}catch(I){console.error("Lỗi nhận khách:",I),t.textContent="Nhận khách",t.disabled=!1}})})},be=()=>{const n=new Set,e=new Set,a=new Set;_.forEach(o=>{if(o.tour&&n.add(o.tour),o.date){const s=F(o.date);s&&e.add(s)}o.sale_name&&o.sale_name!=="Website"&&o.sale_name!=="null"&&a.add(o.sale_name.trim())});const i=document.getElementById("filterTour"),r=document.getElementById("filterDate"),c=document.getElementById("filterSale");if(i){const o=i.value;i.innerHTML='<option value="">Tất cả Tour</option>',[...n].sort().forEach(s=>i.innerHTML+=`<option value="${s}">${s}</option>`),i.value=o}if(r){const o=r.value;r.innerHTML='<option value="">Tất cả Ngày</option>',[...e].sort().forEach(s=>r.innerHTML+=`<option value="${s}">${s}</option>`),r.value=o}if(c){const o=c.value;c.innerHTML='<option value="">Tất cả Sale</option>',c.innerHTML+='<option value="null">Nguồn Website</option>',[...a].sort().forEach(s=>{c.innerHTML+=`<option value="${s}">${s}</option>`}),c.value=o}};function F(n){if(!n)return"";if(/^\d{1,2}\/\d{1,2}\s*-\s*\d{1,2}\/\d{1,2}$/.test(n))return n.replace(/\s+/g,"");try{if(/^\d{4}-\d{2}-\d{2}$/.test(n)){const[e,a,i]=n.split("-"),r=`${parseInt(i)}/${parseInt(a)}`;return`${r}-${r}`}if(/^\d{1,2}\/\d{1,2}\/\d{4}$/.test(n)){const[e,a,i]=n.split("/"),r=`${parseInt(e)}/${parseInt(a)}`;return`${r}-${r}`}if(n.includes("-"))return n.split("-").map(a=>{const i=a.trim();if(/\/\d{4}$/.test(i)){const[r,c]=i.split("/");return`${parseInt(r)}/${parseInt(c)}`}return i}).join("-")}catch{console.warn("Date normalization failed for:",n)}return n}const M=async()=>{try{_=await(await fetch("/api/bookings")).json(),_.sort((e,a)=>parseInt(a.id)-parseInt(e.id)),be(),A()}catch(n){console.error("Lỗi parse list đơn hàng:",n);const e=document.getElementById("bookingsTableBody");e&&(e.innerHTML='<tr><td colspan="4" class="p-8 text-center text-red-500">Lỗi lấy dữ liệu từ Server.</td></tr>')}};M(),["filterSearch","filterTour","filterDate","filterSale","filterStatus"].forEach(n=>{const e=document.getElementById(n);e&&(n==="filterSearch"?e.addEventListener("input",A):e.addEventListener("change",A))});const te=document.getElementById("exportExcelBtn");te&&te.addEventListener("click",()=>{const n=document.getElementById("filterSearch")?document.getElementById("filterSearch").value.toLowerCase():"",e=document.getElementById("filterTour")?document.getElementById("filterTour").value:"",a=document.getElementById("filterDate")?document.getElementById("filterDate").value:"",i=document.getElementById("filterSale")?document.getElementById("filterSale").value:"";let r=_.filter(d=>{let b=!1;const y=parseInt(d.total_price)>0&&parseInt(d.total_price)===parseInt(d.deposit),f=x=>{if(!x)return!1;try{const $=x.split("/");if($.length===3){const t=new Date(`${$[2]}-${$[1]}-${$[0]}T00:00:00`),u=new Date;return u.setHours(0,0,0,0),t.getTime()<u.getTime()}else if(x.includes("-")){const t=new Date(`${x}T00:00:00`),u=new Date;return u.setHours(0,0,0,0),t.getTime()<u.getTime()}}catch{}return!1},p=(d.status==="Hoàn thành"||d.status==="Đã đi"||y)&&f(d.date);return C==="consult"?b=d.status==="Chờ tư vấn"&&!p:C==="pending"?b=(!d.status||d.status==="Chờ cọc")&&!p:C==="upcoming"?b=d.status&&d.status!=="Chờ tư vấn"&&d.status!=="Chờ cọc"&&!y&&d.status!=="Hoàn thành"&&!p:C==="ready"?b=(y||d.status==="Hoàn thành")&&!p:C==="completed"&&(b=p),!(!b||e&&d.tour!==e||a&&F(d.date)!==a||i&&i!=="null"&&d.sale_name!==i||i==="null"&&d.sale_name&&d.sale_name!=="Website"||n&&!`${d.name||""} ${d.phone||""} ${d.customer_id||""} ${d.id||""} csr${d.id||""} #csr${d.id||""}`.toLowerCase().includes(n))});if(r.length===0){alert("Danh sách rỗng. Không có dữ liệu để xuất.");return}let c=`
            ...
            `;const o=new Blob(["\uFEFF",c],{type:"application/vnd.ms-excel;charset=utf-8"}),s=URL.createObjectURL(o),l=document.createElement("a");l.setAttribute("href",s);let m=C==="pending"?"ChoCoc":C==="upcoming"?"SapThamGia":C==="ready"?"ChoLenXe":"LichSu";l.setAttribute("download",`BaoCao_DonHang_${m}_${new Date().toISOString().slice(0,10)}.xls`),document.body.appendChild(l),l.click(),document.body.removeChild(l)});const ne=document.querySelectorAll(".tab-btn");ne.forEach(n=>{n.addEventListener("click",e=>{ne.forEach(a=>{a.classList.remove("border-csr-orange","text-csr-orange"),a.classList.add("border-transparent","text-gray-500")}),e.target.classList.add("border-csr-orange","text-csr-orange"),e.target.classList.remove("border-transparent","text-gray-500"),C=e.target.getAttribute("data-tab"),A()})});const N=document.getElementById("bookingModal"),K=document.getElementById("bookingModalContent"),J=document.getElementById("detailModal"),ae=document.getElementById("detailModalContent"),P=document.getElementById("editModal"),U=document.getElementById("editModalContent"),oe=document.getElementById("addBookingBtn");N&&(N.classList.add("duration-200","ease-out"),K.classList.add("duration-300","ease-out")),P&&(P.classList.add("duration-200","ease-out"),U.classList.add("duration-300","ease-out")),oe&&oe.addEventListener("click",()=>{document.getElementById("bookingModalContent").querySelector("h2").innerText="Thêm Khách Hàng (Tạo Đơn)",document.getElementById("submitBookingBtn").innerText="Tạo Phơi Đăng Ký",document.getElementById("editingBookingId").value="",document.getElementById("bookingForm").reset();const n=document.getElementById("loyalty-alert");n&&n.remove(),N.classList.remove("hidden"),setTimeout(()=>{N.classList.add("opacity-100"),K.classList.remove("scale-95","translate-y-4"),K.classList.add("scale-100","translate-y-0")},10)}),window.closeModal=()=>{N&&(N.classList.remove("opacity-100"),K.classList.remove("scale-100","translate-y-0"),K.classList.add("scale-95","translate-y-4"),setTimeout(()=>{N.classList.add("hidden")},200))},window.closeDetailModal=()=>{J&&(J.classList.remove("opacity-100"),ae.classList.remove("scale-100","translate-y-0"),ae.classList.add("scale-95","translate-y-4"),setTimeout(()=>{J.classList.add("hidden")},200))},window.closeEditModal=()=>{P&&(P.classList.remove("opacity-100"),U.classList.remove("scale-100","translate-y-0"),U.classList.add("scale-95","translate-y-4"),setTimeout(()=>{P.classList.add("hidden")},200))},window.openRowActionModal=n=>{document.getElementById("ramName").textContent=n.name||"Khách Hàng Này",document.getElementById("ramPhone").textContent=`#CSR${n.id} - ${n.phone||""}`;const e=`
        ...
        </div>`;document.getElementById("detailContentBlock").innerHTML=$,document.getElementById("detailModal").classList.remove("hidden"),setTimeout(()=>{document.getElementById("detailModal").classList.add("opacity-100");const t=document.getElementById("detailModalContent");t&&(t.classList.remove("scale-95","translate-y-4"),t.classList.add("scale-100","translate-y-0"))},10)},window.actionEdit=async n=>{document.getElementById("rowActionModal").classList.add("hidden");const e=_.find(l=>l.id==n);if(!e)return;document.getElementById("edit-id").value=n,document.getElementById("edit-name").value=e.name||"",document.getElementById("edit-phone").value=e.phone||"",document.getElementById("edit-medal-name").value=e.medal_name||"",document.getElementById("edit-tour").value=e.tour||"",typeof j=="function"&&j("edit-date",e.tour||"",e.date||"",e.schedule_id||null),(l=>{let m=l.dob||"";if(typeof m=="string"&&m.includes("/")){const d=m.split("/");d.length===3&&(m=`${d[2]}-${d[1].padStart(2,"0")}-${d[0].padStart(2,"0")}`)}document.getElementById("edit-dob").value=m,document.getElementById("edit-gender").value=l.gender||"Khác",document.getElementById("edit-allergy").value=l.allergy||l.medical_notes||"",document.getElementById("edit-address").value=l.address||"",document.getElementById("edit-diet").value=l.diet||l.dietary||"Không",document.getElementById("edit-trekking-pole").value=l.trekking_pole||"Không"})(e),document.getElementById("edit-status").value=e.status||"Chờ xác nhận cọc",document.getElementById("edit-commitments").checked=!!e.commitments,document.getElementById("edit-special").value=e.special||"";try{const l=localStorage.getItem("csr_user");if(l&&JSON.parse(l).role==="admin"){const d=document.getElementById("edit-sale-container");d&&d.classList.remove("hidden");const b=document.getElementById("edit-sale");b&&(b.value=e.sale_id||"")}}catch{}if((!e.dob||!e.address||!e.gender||e.gender==="Khác")&&(e.customer_id||e.phone))try{const l=e.customer_id||e.phone,m=await fetch(`/api/admin_customers?action=search&keyword=${encodeURIComponent(l)}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({keyword:l})}),d=await m.json();if(m.ok&&d.success){const b=d.data,y=document.getElementById("edit-dob"),f=document.getElementById("edit-address"),p=document.getElementById("edit-allergy"),x=document.getElementById("edit-diet"),$=document.getElementById("edit-gender"),t=document.getElementById("edit-medal-name");y&&!y.value&&(y.value=b.dob||""),f&&!f.value&&(f.value=b.address||""),p&&!p.value&&(p.value=b.medical_notes||""),x&&(x.value==="Không"||!x.value)&&(x.value=b.dietary||"Không"),$&&($.value==="Khác"||!$.value)&&($.value=b.gender||"Khác"),t&&!t.value&&(t.value=b.medal_name||"")}}catch(l){console.error("Pull CRM error:",l)}let r=(parseInt(e.total_price)||0)+(parseInt(e.discount)||0);if(r===0&&e.tour){const l=D.find(m=>m.name===e.tour);l&&parseInt(l.price)>0&&(r=parseInt(l.price))}document.getElementById("edit-total").value=r,document.getElementById("edit-discount").value=e.discount||0,document.getElementById("edit-deposit").value=e.deposit||0;const c=parseInt(e.deposit_required);document.getElementById("edit-deposit-required").value=isNaN(c)?1e6:c,window.updateEditRemaining();const o=document.getElementById("editModal"),s=document.getElementById("editModalContent");o&&s&&(o.classList.remove("hidden"),setTimeout(()=>{o.classList.add("opacity-100"),s.classList.remove("scale-95","translate-y-4"),s.classList.add("scale-100","translate-y-0")},10))},window.actionDelete=async n=>{if(document.getElementById("rowActionModal").classList.add("hidden"),confirm("Bạn có chắc chắn muốn xóa Đơn hàng này? Thao tác không thể hoàn tác."))try{if((await fetch(`/api/bookings?id=${n}`,{method:"DELETE"})).ok)alert("✅ Đã xóa đơn hàng thành công!"),M();else throw new Error("Lỗi từ Server")}catch(e){alert("❌ Không thể xóa bảng ghi này: "+e.message)}},window.updateEditRemaining=()=>{const n=parseInt(document.getElementById("edit-total").value)||0,e=parseInt(document.getElementById("edit-discount").value)||0,a=parseInt(document.getElementById("edit-deposit").value)||0,r=n-e-a,c=document.getElementById("edit-remaining");c&&(c.textContent=r>0?r.toLocaleString("vi-VN")+"đ":"0đ")};const se=document.getElementById("edit-total"),de=document.getElementById("edit-deposit");se&&se.addEventListener("input",window.updateEditRemaining),de&&de.addEventListener("input",window.updateEditRemaining),document.querySelectorAll('#bookingModal button[onclick*="hidden"]').forEach(n=>{n.removeAttribute("onclick"),n.addEventListener("click",window.closeModal)});const ie=async n=>{try{const a=await(await fetch("/api/admin_customers?action=create",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({full_name:n.name,phone:n.phone,cccd:n.id_card||"",dob:n.dob||null,gender:n.gender||"Khác",medical_notes:n.allergy||"",dietary:n.diet||""})})).json();a.success&&a.csr_code&&(console.log("✅ Đã đồng bộ CRM thành công:",n.name,a.csr_code),(await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:n.id,customer_id:a.csr_code})})).ok&&(console.log("✅ Đã cập nhật Booking mã CRM"),typeof M=="function"&&M()))}catch(e){console.error("❌ Lỗi đồng bộ CRM:",e)}},le=document.getElementById("bookingsTableBody");le&&le.addEventListener("click",async n=>{const e=n.target.closest(".action-btn");if(!e){const c=n.target.closest(".row-clickable");if(c){const o=c.getAttribute("data-id"),s=_.find(l=>l.id==o);s&&openRowActionModal(s)}return}const a=e.getAttribute("data-id"),i=_.find(c=>c.id==a);if(!i)return;const r=document.getElementById("rowActionModal");if(r&&r.classList.add("hidden"),e.classList.contains("confirm-deposit-btn")){if(confirm("Xác nhận khách hàng này đã chuyển khoản đặt cọc?"))try{if(e.disabled=!0,e.classList.add("opacity-50"),(await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:a,status:"Đã cọc"})})).ok)alert("✅ Đã xác nhận cọc thành công!"),ie(i),typeof M=="function"&&M();else throw new Error("Lỗi cập nhật API")}catch(c){alert("❌ Lỗi: "+c.message)}finally{e.disabled=!1,e.classList.remove("opacity-50")}return}if(e.classList.contains("delete-btn")){window.actionDelete(a);return}else if(e.classList.contains("process-btn")){const s=(window.location.hostname==="localhost"||window.location.hostname==="127.0.0.1"?"http://localhost:8888":window.location.origin)+`/booking/process.html?id=${a}`;navigator.clipboard.writeText(s).then(()=>{alert("📋 Đã sao chép Link Điền Thông Tin!\\nGửi cho khách: "+s)}).catch(l=>{alert("Lỗi khi Copy Clipboard. Link là: "+s)})}else if(e.classList.contains("pay-terms-btn")){const s=(window.location.hostname==="localhost"||window.location.hostname==="127.0.0.1"?"http://localhost:8888":window.location.origin)+`/pay-terms.html?id=${a}`;navigator.clipboard.writeText(s).then(()=>{alert("📋 Đã sao chép Link Cọc Tour!\\nGửi cho khách: "+s)}).catch(l=>{alert("Lỗi khi Copy Clipboard. Link là: "+s)})}else if(e.classList.contains("send-btn")){const s=(window.location.hostname==="localhost"||window.location.hostname==="127.0.0.1"?"http://localhost:8888":window.location.origin)+`/pay?id=${a}`;navigator.clipboard.writeText(s).then(()=>{alert("📋 Đã sao chép Link Thanh Toán Cọc! Gửi cho khách qua Zalo nhé:\\n"+s)}).catch(l=>{alert("Lỗi khi Copy Clipboard. Link là: "+s)})}else if(e.classList.contains("payment-btn")){const s=(window.location.hostname==="localhost"||window.location.hostname==="127.0.0.1"?"http://localhost:8888":window.location.origin)+`/pay?id=${a}`;navigator.clipboard.writeText(s).then(()=>{const l=e.textContent;e.textContent="✅ Đã copy!",e.classList.remove("bg-blue-500","hover:bg-blue-600"),e.classList.add("bg-green-500"),setTimeout(()=>{e.textContent=l,e.classList.add("bg-blue-500","hover:bg-blue-600"),e.classList.remove("bg-green-500")},2e3)}).catch(()=>{alert(`🔗 Link thanh toán:
`+s)})}else e.classList.contains("view-btn")?(document.getElementById("rowActionModal").classList.add("hidden"),window.actionView(a)):e.classList.contains("edit-btn")&&window.actionEdit(a)});const R=document.getElementById("smartSearch"),S=document.getElementById("searchSuggestions");if(R){let n;R.addEventListener("input",e=>{const a=e.target.value.toLowerCase().trim();if(clearTimeout(n),a.length<2){S&&S.classList.add("hidden");return}n=setTimeout(async()=>{try{const i=await fetch(`/api/admin_customers?action=search&keyword=${a}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({keyword:a})}),r=await i.json();if(i.ok&&r.success){const o=[r.data];S&&(S.innerHTML=o.map(s=>`
