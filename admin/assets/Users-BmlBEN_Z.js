import{S as g,H as b}from"./Header-DFYmXmuY.js";const x=()=>{let d={role:"sale",fullName:"Guest"};try{const s=localStorage.getItem("csr_user");s&&(d=JSON.parse(s))}catch{}return d.role!=="admin"?`
    ...
    `},y=()=>{let d={role:"sale",fullName:"Guest"};try{const t=localStorage.getItem("csr_user");t&&(d=JSON.parse(t))}catch{}if(d.role!=="admin")return;let s=[];const i=async()=>{try{const t=await fetch("/api/users");t.ok?(s=await t.json(),m()):document.getElementById("usersTableBody").innerHTML='<tr><td colspan="7" class="p-8 text-center text-red-500">Lỗi lấy dữ liệu</td></tr>'}catch(t){console.error(t),document.getElementById("usersTableBody").innerHTML='<tr><td colspan="7" class="p-8 text-center text-red-500">Lỗi kết nối server</td></tr>'}},m=()=>{const t=document.getElementById("usersTableBody");if(t){if(s.length===0){t.innerHTML='<tr><td colspan="7" class="p-8 text-center text-gray-500 font-medium">Chưa có tài khoản nào.</td></tr>';return}t.innerHTML=s.map(e=>{const r=e.username==="admin";let l='<span class="bg-gray-100 text-gray-600 px-2 py-1 rounded text-[10px] font-bold uppercase border border-gray-200">Sale</span>';e.role==="admin"?l='<span class="bg-red-50 text-red-600 px-2 py-1 rounded text-[10px] font-bold uppercase border border-red-100">Admin</span>':e.role==="tour_guide"&&(l='<span class="bg-teal-50 text-teal-600 px-2 py-1 rounded text-[10px] font-bold uppercase border border-teal-100">🧭 Tour Guide</span>');let a='<span class="px-2 py-1 bg-green-50 text-green-600 text-[10px] font-bold rounded uppercase flex items-center gap-1 w-max mx-auto"><div class="w-1.5 h-1.5 rounded-full bg-green-500"></div> Active</span>';return e.status!=="active"&&(a='<span class="px-2 py-1 bg-gray-100 text-gray-500 text-[10px] font-bold rounded uppercase flex items-center gap-1 w-max mx-auto"><div class="w-1.5 h-1.5 rounded-full bg-gray-400"></div> Locked</span>'),`
