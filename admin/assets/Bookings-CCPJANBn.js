import{S as ct,H as ut}from"./Header-CvqOcIss.js";const mt=()=>`
    ...
    `,ht=()=>{let B=[],x="consult",V=[],q=[];const lt=async()=>{try{const[e,t]=await Promise.all([fetch("/api/tours"),fetch("/api/schedules")]);V=e.ok?await e.json():[],q=t.ok?await t.json():[],F("addTourName"),F("edit-tour")}catch(e){console.error("Lỗi tải tours/schedules:",e)}},F=e=>{const t=document.getElementById(e);if(!t)return;const o=t.value,i=t.options[0].outerHTML;t.innerHTML=i,V.filter(l=>l.is_visible!==!1).forEach(l=>{const a=document.createElement("option");a.value=l.name,a.textContent=l.name,l.name===o&&(a.selected=!0),t.appendChild(a)})},K=(e,t,o)=>{const i=document.getElementById(e);if(!i)return;if(i.innerHTML='<option value="">-- Chọn Lịch --</option>',!t){i.innerHTML='<option value="">-- Chọn Tour trước --</option>';return}const l=q.filter(a=>a.tour_name===t||t.includes(a.tour_name)||a.tour_name.includes(t));if(l.length===0){i.innerHTML='<option value="">Chưa có lịch cho tour này</option>';return}l.sort((a,r)=>new Date(a.start_date)-new Date(r.start_date)).forEach(a=>{const r=parseInt(a.booked_count)||0,s=(a.slots||0)-r,u=new Date(a.start_date),f=a.end_date?new Date(a.end_date):null,g=p=>`${p.getDate().toString().padStart(2,"0")}/${(p.getMonth()+1).toString().padStart(2,"0")}`;let h=g(u);f&&(h+=" - "+g(f)),h+=` (${s>=0?s:0} chỗ trống)`;const d=document.createElement("option");d.value=`${u.getDate().toString().padStart(2,"0")}/${(u.getMonth()+1).toString().padStart(2,"0")}/${u.getFullYear()}`,d.textContent=h,s<=0&&(d.disabled=!0,d.textContent=h.replace("chỗ trống","HẾT CHỖ")),o&&d.value===o&&(d.selected=!0),i.appendChild(d)})},U=document.getElementById("addTourName");U&&U.addEventListener("change",e=>{K("addTourDate",e.target.value)});const J=document.getElementById("edit-tour");J&&J.addEventListener("change",e=>{K("edit-date",e.target.value)}),lt();const $=()=>{const e=document.getElementById("bookingsTableHead"),t=document.getElementById("bookingsTableBody");if(!t||!e)return;const o=x==="upcoming"||x==="completed";o?e.innerHTML=`
            ...
            `;const i=document.getElementById("filterSearch")?document.getElementById("filterSearch").value.toLowerCase():"",l=document.getElementById("filterTour")?document.getElementById("filterTour").value:"",a=document.getElementById("filterDate")?document.getElementById("filterDate").value:"",r=document.getElementById("filterSale")?document.getElementById("filterSale").value:"",s=document.getElementById("filterStatus")?document.getElementById("filterStatus").value:"";let u=B.filter(n=>{let b=!1;const c=parseInt(n.total_price)>0&&parseInt(n.total_price)===parseInt(n.deposit);return x==="consult"?b=n.status==="Chờ tư vấn":x==="pending"?b=!n.status||n.status==="Chờ cọc":x==="upcoming"?b=(n.status==="Chờ xác nhận cọc"||n.status==="Đã cọc"||n.status==="Đã cọc (Chờ đi)")&&!c:x==="completed"&&(b=c||n.status==="Hoàn thành"||n.status==="Đã đi"),!(!b||l&&n.tour!==l||a&&W(n.date)!==a||r&&String(n.sale_id)!==String(r)||i&&!`${n.name||""} ${n.phone||""} ${n.customer_id||""}`.toLowerCase().includes(i)||s&&n.status!==s)});const f=document.getElementById("statTitle1"),g=document.getElementById("statTitle2"),h=document.getElementById("statTitle3"),d=document.getElementById("statTotalCustomers"),p=document.getElementById("statTotalRevenue"),y=document.getElementById("statTotalCollected");let I=u.length;if(x==="consult"){f&&(f.textContent="Tổng Leads Tư Vấn"),g&&(g.textContent="Chưa Có Sale Nhận"),h&&(h.textContent="Đã Có Sale Nhận");let n=0,b=0;u.forEach(c=>{!c.sale_id||c.sale_id==="null"||!c.sale_name||c.sale_name==="Website"||c.sale_name==="null"?n++:b++}),d&&(d.textContent=I),p&&(p.textContent=n+" Đơn"),y&&(y.textContent=b+" Đơn")}else if(x==="pending"){f&&(f.textContent="Tổng Khách (Chờ Cọc)"),g&&(g.textContent="Khách Chưa Tư Vấn (Mới)"),h&&(h.textContent="Đã Có Sale Giữ Chỗ");let n=0,b=0;u.forEach(c=>{!c.sale_id||c.sale_id==="null"||!c.sale_name||c.sale_name==="Website"||c.sale_name==="null"?n++:b++}),d&&(d.textContent=I),p&&(p.textContent=n+" Đơn"),y&&(y.textContent=b+" Đơn")}else if(x==="upcoming"){f&&(f.textContent="Tổng Số Khách"),g&&(g.textContent="Chưa Xác Nhận Cọc"),h&&(h.textContent="Chờ Thanh Toán Còn Lại");let n=0,b=0;u.forEach(c=>{c.status==="Chờ xác nhận cọc"?n++:parseInt(c.deposit)>0&&parseInt(c.total_price)>parseInt(c.deposit)&&b++}),d&&(d.textContent=I),p&&(p.textContent=n+" Khách"),y&&(y.textContent=b+" Khách")}else{f&&(f.textContent="Khách Sẵn Sàng (Full)"),g&&(g.textContent="Tổng Doanh Thu Tab"),h&&(h.textContent="Thực Thu (Full Tận Nơi)");let n=0,b=0;u.forEach(c=>{n+=parseInt(c.total_price)||0,b+=parseInt(c.deposit)||0}),d&&(d.textContent=I),p&&(p.textContent=n>0?n.toLocaleString("vi-VN")+"đ":"0đ"),y&&(y.textContent=b>0?b.toLocaleString("vi-VN")+"đ":"0đ")}const A=o?8:4;if(!u||u.length===0){let n=x==="consult"?"Chưa có khách hàng tư vấn nào.":x==="pending"?"Chưa có Khách chờ cọc.":x==="upcoming"?"Chưa có Khách nào Đã Cọc.":"Danh sách rỗng.";t.innerHTML=`<tr><td colspan="${A}" class="p-8 text-center text-gray-500">${n}</td></tr>`;return}t.innerHTML=u.map(n=>{const b=x==="upcoming"?"bg-green-50/20 hover:bg-green-50 transition-colors":"hover:bg-gray-100 transition-colors";if(o){const c=parseInt(n.total_price)||0,v=parseInt(n.deposit)||0,C=c-v,w=c>0?c.toLocaleString("vi-VN")+"đ":"Chưa định giá",k=v>0?v.toLocaleString("vi-VN")+"đ":"0đ",m=C>0?C.toLocaleString("vi-VN")+"đ":C===0&&c>0?"Đã thu trọn":"-";let L="";n.status==="Hoàn thành"||n.status==="Đã đi"?L='<span class="bg-gray-100 text-gray-600 border border-gray-200 px-2 py-0.5 rounded text-xs block w-full text-center">Hoàn thành</span>':n.status==="Chờ xác nhận cọc"?L=`<button class="action-btn confirm-deposit-btn bg-csr-orange text-white px-3 py-1.5 rounded text-xs font-bold shadow-sm hover:bg-[#d65503] w-full" data-id="${n.id}">Xác nhận cọc</button>`:v>0&&C===0?L='<span class="bg-green-100 text-green-700 border border-green-200 px-2 py-0.5 rounded text-xs font-medium block w-full text-center">Hoàn tất phí</span>':L='<span class="bg-blue-100 text-blue-700 border border-blue-200 px-2 py-0.5 rounded text-xs font-medium block w-full text-center">Đã Cọc</span>';let j="";return v>0&&(j=`<a href="/invoice.html?id=${n.id}" target="_blank" class="mt-1.5 block text-center text-[11px] text-blue-600 hover:text-blue-800 hover:underline font-medium" onclick="event.stopPropagation()">🧾 Xem hóa đơn</a>`),`
                <tr class="${b} cursor-pointer row-clickable" data-id="${n.id}">
                    ...
                    <td class="p-4 align-top text-center">${L}${j}</td>
                ...
                `}else{const c=n.name?n.name.substring(0,2).toUpperCase():"KH",v=x==="consult",C=!n.sale_id||n.sale_id==="null"||!n.sale_name||n.sale_name==="null";let w="";v&&C?w=`<button class="action-btn claim-btn bg-csr-orange hover:bg-[#d65503] text-white px-3 py-1.5 rounded text-xs font-bold shadow-sm transition-colors" data-id="${n.id}">Nhận khách</button>`:w=`<span class="text-sm text-gray-500">${n.sale_name||"Website"}</span>`;const k=n.customer_id?`<button class="action-btn payment-btn bg-blue-600 hover:bg-blue-700 text-white px-3 py-1.5 rounded text-xs font-bold shadow-sm transition-colors" data-id="${n.id}">💳 Thanh toán</button>`:`<button class="action-btn process-btn bg-csr-orange hover:bg-[#d65503] text-white px-3 py-1.5 rounded text-xs font-bold shadow-sm transition-colors mr-1" data-id="${n.id}">📝 Thông Tin</button>
                       <button class="action-btn pay-terms-btn bg-blue-500 hover:bg-blue-600 text-white px-3 py-1.5 rounded text-xs font-bold shadow-sm transition-colors" data-id="${n.id}">🔗 Cọc Tour</button>`;return`
                <tr class="${b} cursor-pointer row-clickable" data-id="${n.id}">
                    <td class="p-4 align-top">
                    ...
                    <td class="p-4 align-top">
                        ...
                        ${v&&n.special?`<div class="text-xs text-blue-500 mt-1 italic">💬 ${n.special}</div>`:""}
                    </td>
    ...
    `}}).join(""),document.querySelectorAll(".claim-btn").forEach(n=>{n.addEventListener("click",async b=>{b.stopPropagation();const c=n.getAttribute("data-id"),v=localStorage.getItem("csr_user");let C=null,w="Admin";if(v){const k=JSON.parse(v);C=k.id,w=k.fullName||k.full_name||"Admin"}n.textContent="Đang xử lý...",n.disabled=!0;try{if((await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:c,sale_id:C,sale_name:w})})).ok){const m=B.find(L=>L.id==c);m&&(m.sale_id=C,m.sale_name=w),$()}else alert("Lỗi khi nhận khách. Vui lòng thử lại."),n.textContent="Nhận khách",n.disabled=!1}catch(k){console.error("Lỗi nhận khách:",k),n.textContent="Nhận khách",n.disabled=!1}})})},rt=()=>{const e=new Set,t=new Set,o=new Map;B.forEach(r=>{if(r.tour&&e.add(r.tour),r.date){const s=W(r.date);s&&t.add(s)}r.sale_id&&r.sale_name&&o.set(r.sale_id,r.sale_name)});const i=document.getElementById("filterTour"),l=document.getElementById("filterDate"),a=document.getElementById("filterSale");if(i){const r=i.value;i.innerHTML='<option value="">Tất cả Tour</option>',[...e].sort().forEach(s=>i.innerHTML+=`<option value="${s}">${s}</option>`),i.value=r}if(l){const r=l.value;l.innerHTML='<option value="">Tất cả Ngày</option>',[...t].sort().forEach(s=>l.innerHTML+=`<option value="${s}">${s}</option>`),l.value=r}if(a){const r=a.value;a.innerHTML='<option value="">Tất cả Sale</option>',a.innerHTML+='<option value="null">Nguồn Website</option>';for(let[s,u]of o)a.innerHTML+=`<option value="${s}">${u}</option>`;a.value=r}};function W(e){if(!e)return"";if(/^\d{1,2}\/\d{1,2}\s*-\s*\d{1,2}\/\d{1,2}$/.test(e))return e.replace(/\s+/g,"");try{if(/^\d{4}-\d{2}-\d{2}$/.test(e)){const[t,o,i]=e.split("-"),l=`${parseInt(i)}/${parseInt(o)}`;return`${l}-${l}`}if(/^\d{1,2}\/\d{1,2}\/\d{4}$/.test(e)){const[t,o,i]=e.split("/"),l=`${parseInt(t)}/${parseInt(o)}`;return`${l}-${l}`}if(e.includes("-"))return e.split("-").map(o=>{const i=o.trim();if(/\/\d{4}$/.test(i)){const[l,a]=i.split("/");return`${parseInt(l)}/${parseInt(a)}`}return i}).join("-")}catch{console.warn("Date normalization failed for:",e)}return e}const E=async()=>{try{B=await(await fetch("/api/bookings")).json(),rt(),$()}catch(e){console.error("Lỗi parse list đơn hàng:",e);const t=document.getElementById("bookingsTableBody");t&&(t.innerHTML='<tr><td colspan="4" class="p-8 text-center text-red-500">Lỗi lấy dữ liệu từ Server.</td></tr>')}};E(),["filterSearch","filterTour","filterDate","filterSale","filterStatus"].forEach(e=>{const t=document.getElementById(e);t&&(e==="filterSearch"?t.addEventListener("input",$):t.addEventListener("change",$))});const X=document.getElementById("exportExcelBtn");X&&X.addEventListener("click",()=>{const e=document.getElementById("filterSearch")?document.getElementById("filterSearch").value.toLowerCase():"",t=document.getElementById("filterTour")?document.getElementById("filterTour").value:"",o=document.getElementById("filterDate")?document.getElementById("filterDate").value:"",i=document.getElementById("filterSale")?document.getElementById("filterSale").value:"";let l=B.filter(d=>{let p=!1;return x==="pending"?p=d.status==="Chờ xác nhận cọc"||parseInt(d.deposit)===0||!d.deposit:x==="upcoming"?p=d.status!=="Chờ xác nhận cọc"&&parseInt(d.deposit)>0:x==="completed"&&(p=d.status==="Hoàn thành"),!(!p||t&&d.tour!==t||o&&d.date!==o||i&&String(d.sale_id)!==String(i)||e&&!`${d.name||""} ${d.phone||""} ${d.customer_id||""} `.toLowerCase().includes(e))});if(l.length===0){alert("Danh sách rỗng. Không có dữ liệu để xuất.");return}const r=[["Mã KH (CRM)","Họ và Tên","Số điện thoại","Tên Tour","Lịch trình","Trạng thái","Nguồn / Sale","Đã cọc","Tổng tiền","Ghi chú"]];l.forEach(d=>{r.push([d.customer_id||"",`"${d.name||""}"`,`= "${d.phone||""}"`,`"${d.tour||""}"`,`"${d.date||""}"`,`"${d.status||""}"`,`"${d.sale_name||"Website"}"`,d.deposit||0,d.total_price||0,`"${(d.special||"").replace(/"/g,'""')}"`])});const s="\\uFEFF"+r.map(d=>d.join(",")).join(`
`),u=new Blob([s],{type:"text/csv;charset=utf-8;"}),f=URL.createObjectURL(u),g=document.createElement("a");g.setAttribute("href",f);let h=x==="pending"?"ChoCoc":x==="upcoming"?"SapThamGia":"HoanThanh";g.setAttribute("download",`BaoCao_DonHang_${h}_${new Date().toISOString().slice(0,10)}.csv`),document.body.appendChild(g),g.click(),document.body.removeChild(g)});const Y=document.querySelectorAll(".tab-btn");Y.forEach(e=>{e.addEventListener("click",t=>{Y.forEach(o=>{o.classList.remove("border-csr-orange","text-csr-orange"),o.classList.add("border-transparent","text-gray-500")}),t.target.classList.add("border-csr-orange","text-csr-orange"),t.target.classList.remove("border-transparent","text-gray-500"),x=t.target.getAttribute("data-tab"),$()})});const M=document.getElementById("bookingModal"),H=document.getElementById("bookingModalContent"),G=document.getElementById("detailModal"),Q=document.getElementById("detailModalContent"),N=document.getElementById("editModal"),O=document.getElementById("editModalContent"),Z=document.getElementById("addBookingBtn");M&&(M.classList.add("duration-200","ease-out"),H.classList.add("duration-300","ease-out")),N&&(N.classList.add("duration-200","ease-out"),O.classList.add("duration-300","ease-out")),Z&&Z.addEventListener("click",()=>{document.getElementById("bookingModalContent").querySelector("h2").innerText="Thêm Khách Hàng (Tạo Đơn)",document.getElementById("submitBookingBtn").innerText="Tạo Phơi Đăng Ký",document.getElementById("editingBookingId").value="",document.getElementById("bookingForm").reset();const e=document.getElementById("loyalty-alert");e&&e.remove(),M.classList.remove("hidden"),setTimeout(()=>{M.classList.add("opacity-100"),H.classList.remove("scale-95","translate-y-4"),H.classList.add("scale-100","translate-y-0")},10)}),window.closeModal=()=>{M&&(M.classList.remove("opacity-100"),H.classList.remove("scale-100","translate-y-0"),H.classList.add("scale-95","translate-y-4"),setTimeout(()=>{M.classList.add("hidden")},200))},window.closeDetailModal=()=>{G&&(G.classList.remove("opacity-100"),Q.classList.remove("scale-100","translate-y-0"),Q.classList.add("scale-95","translate-y-4"),setTimeout(()=>{G.classList.add("hidden")},200))},window.closeEditModal=()=>{N&&(N.classList.remove("opacity-100"),O.classList.remove("scale-100","translate-y-0"),O.classList.add("scale-95","translate-y-4"),setTimeout(()=>{N.classList.add("hidden")},200))},window.openRowActionModal=e=>{document.getElementById("ramName").textContent=e.name||"Khách Hàng Này",document.getElementById("ramPhone").textContent=e.customer_id?`🥇 ${e.customer_id.toUpperCase()}`:e.phone||"";const t=`
        ...
        `;document.getElementById("detailContentBlock").innerHTML=l,document.getElementById("detailModal").classList.remove("hidden"),setTimeout(()=>{document.getElementById("detailModal").classList.add("opacity-100");const a=document.getElementById("detailModalContent");a&&(a.classList.remove("scale-95","translate-y-4"),a.classList.add("scale-100","translate-y-0"))},10)},window.actionEdit=async e=>{document.getElementById("rowActionModal").classList.add("hidden");const t=B.find(a=>a.id==e);if(!t)return;if(document.getElementById("edit-id").value=e,document.getElementById("edit-name").value=t.name||"",document.getElementById("edit-phone").value=t.phone||"",document.getElementById("edit-medal-name").value=t.medal_name||"",document.getElementById("edit-tour").value=t.tour||"",typeof K=="function"&&K("edit-date",t.tour||"",t.date||""),(a=>{let r=a.dob||"";if(typeof r=="string"&&r.includes("/")){const s=r.split("/");s.length===3&&(r=`${s[2]}-${s[1].padStart(2,"0")}-${s[0].padStart(2,"0")}`)}document.getElementById("edit-dob").value=r,document.getElementById("edit-gender").value=a.gender||"Khác",document.getElementById("edit-allergy").value=a.allergy||a.medical_notes||"",document.getElementById("edit-address").value=a.address||"",document.getElementById("edit-diet").value=a.diet||a.dietary||"Không",document.getElementById("edit-trekking-pole").value=a.trekking_pole||"Không"})(t),document.getElementById("edit-status").value=t.status||"Chờ xác nhận cọc",document.getElementById("edit-commitments").checked=!!t.commitments,document.getElementById("edit-special").value=t.special||"",(!t.dob||!t.address||!t.gender||t.gender==="Khác")&&(t.customer_id||t.phone))try{const a=t.customer_id||t.phone,r=await fetch(`/api/admin_customers?action=search&keyword=${encodeURIComponent(a)}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({keyword:a})}),s=await r.json();if(r.ok&&s.success){const u=s.data,f=document.getElementById("edit-dob"),g=document.getElementById("edit-address"),h=document.getElementById("edit-allergy"),d=document.getElementById("edit-diet"),p=document.getElementById("edit-gender"),y=document.getElementById("edit-medal-name");f&&!f.value&&(f.value=u.dob||""),g&&!g.value&&(g.value=u.address||""),h&&!h.value&&(h.value=u.medical_notes||""),d&&(d.value==="Không"||!d.value)&&(d.value=u.dietary||"Không"),p&&(p.value==="Khác"||!p.value)&&(p.value=u.gender||"Khác"),y&&!y.value&&(y.value=u.medal_name||"")}}catch(a){console.error("Pull CRM error:",a)}document.getElementById("edit-total").value=(parseInt(t.total_price)||0)+(parseInt(t.discount)||0),document.getElementById("edit-discount").value=t.discount||0,document.getElementById("edit-deposit").value=t.deposit||0,document.getElementById("edit-deposit-required").value=t.deposit_required||1e6,window.updateEditRemaining();const i=document.getElementById("editModal"),l=document.getElementById("editModalContent");i&&l&&(i.classList.remove("hidden"),setTimeout(()=>{i.classList.add("opacity-100"),l.classList.remove("scale-95","translate-y-4"),l.classList.add("scale-100","translate-y-0")},10))},window.actionDelete=async e=>{if(document.getElementById("rowActionModal").classList.add("hidden"),confirm("Bạn có chắc chắn muốn xóa Đơn hàng này? Thao tác không thể hoàn tác."))try{if((await fetch(`/api/bookings?id=${e}`,{method:"DELETE"})).ok)alert("✅ Đã xóa đơn hàng thành công!"),E();else throw new Error("Lỗi từ Server")}catch(t){alert("❌ Không thể xóa bảng ghi này: "+t.message)}},window.updateEditRemaining=()=>{const e=parseInt(document.getElementById("edit-total").value)||0,t=parseInt(document.getElementById("edit-discount").value)||0,o=parseInt(document.getElementById("edit-deposit").value)||0,l=e-t-o,a=document.getElementById("edit-remaining");a&&(a.textContent=l>0?l.toLocaleString("vi-VN")+"đ":"0đ")};const tt=document.getElementById("edit-total"),et=document.getElementById("edit-deposit");tt&&tt.addEventListener("input",window.updateEditRemaining),et&&et.addEventListener("input",window.updateEditRemaining),document.querySelectorAll('#bookingModal button[onclick*="hidden"]').forEach(e=>{e.removeAttribute("onclick"),e.addEventListener("click",window.closeModal)});const nt=async e=>{try{const o=await(await fetch("/api/admin_customers?action=create",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({full_name:e.name,phone:e.phone,cccd:e.id_card||"",dob:e.dob||null,gender:e.gender||"Khác",medical_notes:e.allergy||"",dietary:e.diet||""})})).json();o.success&&o.csr_code&&(console.log("✅ Đã đồng bộ CRM thành công:",e.name,o.csr_code),(await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:e.id,customer_id:o.csr_code})})).ok&&(console.log("✅ Đã cập nhật Booking mã CRM"),typeof E=="function"&&E()))}catch(t){console.error("❌ Lỗi đồng bộ CRM:",t)}},ot=document.getElementById("bookingsTableBody");ot&&ot.addEventListener("click",async e=>{const t=e.target.closest(".action-btn");if(!t){const a=e.target.closest(".row-clickable");if(a){const r=a.getAttribute("data-id"),s=B.find(u=>u.id==r);s&&openRowActionModal(s)}return}const o=t.getAttribute("data-id"),i=B.find(a=>a.id==o);if(!i)return;const l=document.getElementById("rowActionModal");if(l&&l.classList.add("hidden"),t.classList.contains("confirm-deposit-btn")){if(confirm("Xác nhận khách hàng này đã chuyển khoản đặt cọc?"))try{if(t.disabled=!0,t.classList.add("opacity-50"),(await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:o,status:"Đã cọc"})})).ok)alert("✅ Đã xác nhận cọc thành công!"),nt(i),typeof E=="function"&&E();else throw new Error("Lỗi cập nhật API")}catch(a){alert("❌ Lỗi: "+a.message)}finally{t.disabled=!1,t.classList.remove("opacity-50")}return}if(t.classList.contains("delete-btn")){window.actionDelete(o);return}else if(t.classList.contains("process-btn")){const s=(window.location.hostname==="localhost"||window.location.hostname==="127.0.0.1"?"http://localhost:8888":window.location.origin)+`/booking/process.html?id=${o}`;navigator.clipboard.writeText(s).then(()=>{alert("📋 Đã sao chép Link Điền Thông Tin!\\nGửi cho khách: "+s)}).catch(u=>{alert("Lỗi khi Copy Clipboard. Link là: "+s)})}else if(t.classList.contains("pay-terms-btn")){const s=(window.location.hostname==="localhost"||window.location.hostname==="127.0.0.1"?"http://localhost:8888":window.location.origin)+`/pay-terms.html?id=${o}`;navigator.clipboard.writeText(s).then(()=>{alert("📋 Đã sao chép Link Cọc Tour!\\nGửi cho khách: "+s)}).catch(u=>{alert("Lỗi khi Copy Clipboard. Link là: "+s)})}else if(t.classList.contains("send-btn")){const s=(window.location.hostname==="localhost"||window.location.hostname==="127.0.0.1"?"http://localhost:8888":window.location.origin)+`/pay/${o}`;navigator.clipboard.writeText(s).then(()=>{alert("📋 Đã sao chép Link Thanh Toán Cọc! Gửi cho khách qua Zalo nhé:\\n"+s)}).catch(u=>{alert("Lỗi khi Copy Clipboard. Link là: "+s)})}else if(t.classList.contains("payment-btn")){const s=(window.location.hostname==="localhost"||window.location.hostname==="127.0.0.1"?"http://localhost:8888":window.location.origin)+`/pay/${o}`;window.open(s,"_blank")}else t.classList.contains("view-btn")?(document.getElementById("rowActionModal").classList.add("hidden"),window.actionView(o)):t.classList.contains("edit-btn")&&window.actionEdit(o)});const D=document.getElementById("smartSearch"),T=document.getElementById("searchSuggestions");if(D){let e;D.addEventListener("input",t=>{const o=t.target.value.toLowerCase().trim();if(clearTimeout(e),o.length<2){T&&T.classList.add("hidden");return}e=setTimeout(async()=>{try{const i=await fetch(`/api/admin_customers?action=search&keyword=${o}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({keyword:o})}),l=await i.json();if(i.ok&&l.success){const r=[l.data];T&&(T.innerHTML=r.map(s=>`
                        ...
                        `).join(""),T.classList.remove("hidden"))}else T&&(T.innerHTML='<div class="p-3 text-xs text-gray-400 italic text-center">Không tìm thấy khách hàng cũ</div>',T.classList.remove("hidden"))}catch(i){console.error("Search error:",i)}},300)}),T&&T.addEventListener("click",t=>{const o=t.target.closest(".suggestion-item");if(o){const i=document.getElementById("addFullName"),l=document.getElementById("addPhone"),a=document.getElementById("addCsrCode");i&&(i.value=o.getAttribute("data-name")),l&&(l.value=o.getAttribute("data-phone")),a&&(a.value=o.getAttribute("data-csr"));const r=document.getElementById("addDob"),s=document.getElementById("addGender"),u=document.getElementById("addIdCard"),f=document.getElementById("addAddress");r&&(r.value=o.getAttribute("data-dob")||""),s&&(s.value=o.getAttribute("data-gender")||"Khác"),u&&(u.value=o.getAttribute("data-cccd")||""),f&&(f.value=o.getAttribute("data-address")||"");const g=document.getElementById("addAllergy"),h=document.getElementById("addDiet"),d=document.getElementById("addTrekkingPole"),p=document.getElementById("addMedalName");g&&(g.value=o.getAttribute("data-allergy")||""),h&&(h.value=o.getAttribute("data-diet")||""),d&&(d.value=o.getAttribute("data-trekking_pole")||"Không"),p&&(p.value=o.getAttribute("data-medal-name")||""),window._selectedCustomer={fullName:o.getAttribute("data-name"),phone:o.getAttribute("data-phone"),csrCode:o.getAttribute("data-csr"),dob:o.getAttribute("data-dob")||"",gender:o.getAttribute("data-gender")||"",address:o.getAttribute("data-address")||"",id_card:o.getAttribute("data-cccd")||"",allergy:o.getAttribute("data-allergy")||"",diet:o.getAttribute("data-diet")||"",trekking_pole:o.getAttribute("data-trekking_pole")||"",medal_name:o.getAttribute("data-medal-name")||""},D.value=o.getAttribute("data-name"),T.classList.add("hidden");const y=`
                    ...
                    </div>`,I=document.getElementById("loyalty-alert");I&&I.remove(),D.parentElement.insertAdjacentHTML("afterend",y)}}),document.addEventListener("click",t=>{!D.contains(t.target)&&T&&!T.contains(t.target)&&T.classList.add("hidden")})}const _=document.getElementById("addTourPrice"),P=document.getElementById("addDiscount"),at=document.getElementById("addTotalPriceDisplay"),st=document.getElementById("addTourName"),z=()=>{if(!_||!P||!at)return;const e=parseInt(_.value)||0,t=parseInt(P.value)||0,o=e-t;at.textContent=(o>0?o:0).toLocaleString("vi-VN")+"đ"};_&&_.addEventListener("input",z),P&&P.addEventListener("input",z),st&&st.addEventListener("change",e=>{const t=e.target.value,o=V.find(i=>i.name===t);o&&_&&(_.value=o.price||0,z())});const it=document.getElementById("searchBtn");it&&it.addEventListener("click",e=>{e.preventDefault(),document.getElementById("smartSearch").value||alert("Vui lòng nhập Số Điện Thoại hoặc Mã #CSR")});const R=document.getElementById("bookingForm");R&&R.addEventListener("submit",async e=>{e.preventDefault();const t=e.target.querySelector('button[type="submit"]'),o=t.textContent;t.textContent="Đang lưu hệ thống...",t.disabled=!0;try{const i=document.getElementById("addFullName").value,l=document.getElementById("addPhone").value,a=document.getElementById("addTourName").value,r=document.getElementById("addTourDate").value,s=document.getElementById("addDob").value,u=document.getElementById("addGender").value,f=document.getElementById("addIdCard").value,g=document.getElementById("addAddress").value,h=document.getElementById("addSpecial").value,d=document.getElementById("addAllergy").value,p=document.getElementById("addDiet").value,y=document.getElementById("addTrekkingPole").value,I=document.getElementById("addMedalName").value,A=parseInt(document.getElementById("addTourPrice").value)||0,n=parseInt(document.getElementById("addDiscount").value)||0,b=parseInt(document.getElementById("addDepositRequired").value)||1e6,c=document.getElementById("addCsrCode").value||"",v=localStorage.getItem("csr_user");let C=null,w="Admin";if(v){const S=JSON.parse(v);C=S.id,w=S.fullName||S.full_name||"Admin"}const k=document.getElementById("editingBookingId")?document.getElementById("editingBookingId").value:"",m={name:i,phone:l,tour:a,date:r,status:"Chờ cọc",total_price:A-n,discount:n,deposit:0,sale_id:C,sale_name:w,customer_id:c,deposit_required:b,commitments:!0,dob:s,gender:u,address:g,id_card:f,special:h,allergy:d,diet:p,trekking_pole:y,medal_name:I},L=l.replace(/[^0-9]/g,"");window._selectedCustomer&&(window._selectedCustomer.phone||"").replace(/[^0-9]/g,"")===L&&(m.dob=m.dob||window._selectedCustomer.dob,m.gender=m.gender&&m.gender!=="Khác"?m.gender:window._selectedCustomer.gender||"Khác",m.address=m.address||window._selectedCustomer.address,m.id_card=m.id_card||window._selectedCustomer.id_card,m.allergy=window._selectedCustomer.allergy,m.diet=window._selectedCustomer.diet,m.trekking_pole=window._selectedCustomer.trekking_pole,m.medal_name=window._selectedCustomer.medal_name),k&&(m.id=k,delete m.status,delete m.deposit,delete m.sale_id,delete m.sale_name);const j=await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(m)});if(j.ok){alert(k?"✅ Đã lưu Cập nhật Đơn hàng!":"✅ Thêm Đơn Hàng Thành Công!"),window.closeModal(),R&&R.reset(),window._selectedCustomer=null;const S=document.getElementById("loyalty-alert");S&&S.remove(),document.getElementById("addTotalPriceDisplay")&&(document.getElementById("addTotalPriceDisplay").textContent="0đ"),typeof E=="function"?E():window.location.reload()}else{const S=await j.json();throw new Error(S.error||"Lỗi tạo Booking!")}}catch(i){alert("❌ "+i.message)}finally{t.textContent=o,t.disabled=!1}});const dt=document.getElementById("editForm");dt&&dt.addEventListener("submit",async e=>{e.preventDefault();const t=e.target.querySelector('button[type="submit"]'),o=t.textContent;t.textContent="Đang lưu cập nhật...",t.disabled=!0;try{const i=document.getElementById("edit-id").value,l=document.getElementById("edit-name").value,a=document.getElementById("edit-phone").value,r=document.getElementById("edit-medal-name").value,s=document.getElementById("edit-tour").value,u=document.getElementById("edit-date").value,f=document.getElementById("edit-dob").value,g=document.getElementById("edit-gender").value,h=document.getElementById("edit-status").value,d=document.getElementById("edit-allergy").value,p=document.getElementById("edit-address").value,y=document.getElementById("edit-diet").value,I=document.getElementById("edit-trekking-pole").value,A=document.getElementById("edit-commitments").checked,n=document.getElementById("edit-special").value,b=parseInt(document.getElementById("edit-total").value)||0,c=parseInt(document.getElementById("edit-discount").value)||0,v=parseInt(document.getElementById("edit-deposit").value)||0,C=parseInt(document.getElementById("edit-deposit-required").value)||1e6,w={id:i,name:l,phone:a,medal_name:r,tour:s,date:u,dob:f,gender:g,status:h,allergy:d,address:p,diet:y,trekking_pole:I,commitments:A,special:n,total_price:b-c,discount:c,deposit:v,deposit_required:C},k=await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(w)});if(k.ok)alert("✅ Đã cập nhật thành công Chi tiết Đơn hàng!"),(h.includes("Đã cọc")||w.total_price>0&&w.total_price===v)&&nt(w),window.closeEditModal(),typeof E=="function"&&E();else{const m=await k.json();throw new Error(m.error||"Gặp sự cố khi Cập nhật API.")}}catch(i){alert("❌ Lỗi Cập nhật: "+i.message)}finally{t.textContent=o,t.disabled=!1}})};export{ht as afterRender,mt as render};
