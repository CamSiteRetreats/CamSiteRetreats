import{S as ee,H as te}from"./Header-CtRUqTZE.js";const ne=()=>`
    ...
    `,ae=async()=>{const G=e=>{const t=(e||"").toLowerCase();return t.includes("hoàn tất")||t.includes("hoàn thành")||t.includes("đã thanh toán")||t.includes("thanh toán")?`<span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold bg-emerald-100 text-emerald-700 border border-emerald-200">
        ...
        `).join("")},X=()=>{b.forEach(e=>{const t=v.has(e.id),a=document.querySelector(`th[data-col="${e.id}"]`);a&&(t?a.classList.remove("col-hidden"):a.classList.add("col-hidden")),document.querySelectorAll(`td[data-col="${e.id}"]`).forEach(o=>{t?o.classList.remove("col-hidden"):o.classList.add("col-hidden")})})},I=e=>{if(!e)return null;e.includes(" - ")&&(e=e.split(" - ")[0].trim()),e.includes("T")&&(e=e.split("T")[0]);const t=e.split(/[-/.]/);return t.length===2?{day:parseInt(t[0]),month:parseInt(t[1]),year:null}:t.length===3?t[0].length===4?{day:parseInt(t[2]),month:parseInt(t[1]),year:parseInt(t[0])}:{day:parseInt(t[0]),month:parseInt(t[1]),year:parseInt(t[2])}:null},w=I(y),Q=e=>{if(!w)return!0;const t=I(e);return!(!t||t.day!==w.day||t.month!==w.month||t.year&&w.year&&t.year!==w.year)},_=async()=>{try{if($)try{const r=await(await fetch(`/api/guides?schedule_id=${$}`)).json();if(r&&r.length>0){const n=document.getElementById("rosterGuidesSection"),d=document.getElementById("rosterGuidesList"),l={"Hướng Dẫn Viên":"bg-blue-100 text-blue-700 border-blue-200",Leader:"bg-purple-100 text-purple-700 border-purple-200",Photo:"bg-pink-100 text-pink-700 border-pink-200",Guider:"bg-cyan-100 text-cyan-700 border-cyan-200","Hậu Cần":"bg-amber-100 text-amber-700 border-amber-200","Học Việc":"bg-gray-100 text-gray-600 border-gray-200"};n.classList.remove("hidden"),d.innerHTML=r.map(g=>`
                            ...
                            `).join("")}}catch(o){console.error("Error loading guides:",o)}const t=await(await fetch("/api/bookings")).json();console.log("[Roster] targetTour:",f,"targetDateStr:",y,"targetParsed:",w),console.log("[Roster] Total bookings:",t.length),t.length>0&&t.filter(r=>r.tour&&r.tour.includes("Liêng")).slice(0,3).forEach(r=>console.log("[Roster] Sample Liêng:",{tour:r.tour,date:r.date,parsed:I(r.date)})),x=t.filter(o=>{if($){if(String(o.schedule_id)===String($))return!0;if(!(!P&&!o.schedule_id))return!1}let r=!0,n=!0;if(f){const d=N(o.tour),l=N(f);r=d===l||d.includes(l)||l.includes(d)}return y&&(n=Q(o.date)),r&&n}),console.log("[Roster] Matched bookings:",x.length);const a=document.getElementById("filterSort")?.value||"newest";a==="newest"?x.sort((o,r)=>parseInt(r.id)-parseInt(o.id)):x.sort((o,r)=>parseInt(o.id)-parseInt(r.id)),console.log(`[Roster] Sorted bookings (${a}):`,x.map(o=>o.id)),L()}catch(e){console.error("Lỗi khi tải dữ liệu đoàn:",e),M.innerHTML='<tr><td colspan="12" class="p-8 text-center text-red-500 font-bold">Lỗi khi tải dữ liệu. Vui lòng tải lại trang.</td></tr>'}},L=()=>{const e=E.value.toLowerCase(),t=B.value.toLowerCase(),a=document.getElementById("filterSearch")?.value?.toLowerCase()||"",o=x.filter(r=>{if(a&&!`${r.name} ${r.phone} ${r.address} ${r.medal_name} ${r.customer_id}`.toLowerCase().includes(a))return!1;const n=(r.status||"").toLowerCase();if(!(e===""||e==="chờ cọc"&&(n.includes("chờ cọc")||n.includes("chờ xác nhận"))||e==="đã cọc"&&(n.includes("đã cọc")||n.includes("hoàn tất")||n.includes("hoàn thành"))||e==="đã hủy"&&n.includes("hủy")))return!1;const l=(r.gender||"").toLowerCase();return t===""||l===t});if(document.getElementById("text-visible-count").textContent=o.length,U.textContent=o.length,o.length===0){M.innerHTML='<tr><td colspan="12" class="p-12 text-center text-gray-500 bg-gray-50">Không tìm thấy khách hàng nào khớp với điều kiện lọc.</td></tr>';return}M.innerHTML=o.map((r,n)=>`<tr class="${n%2===0?"bg-white":"bg-gray-50/40"} hover:bg-amber-50/40 transition-colors border-b border-gray-100 group">`+b.map(l=>`
                    <td class="px-4 py-3 align-middle ${l.id==="col-extra"?"min-w-[220px]":""} ${v.has(l.id)?"":"col-hidden"}" data-col="${l.id}">
                        ${l.render(r,n)}
                    </td>
                `).join("")+"</tr>").join("")},Y=()=>{const e=b.filter(s=>v.has(s.id)),t=E.value.toLowerCase(),a=B.value.toLowerCase(),o=document.getElementById("filterSearch")?.value?.toLowerCase()||"",r=x.filter(s=>{if(o&&!`${s.name} ${s.phone} ${s.address} ${s.medal_name} ${s.customer_id}`.toLowerCase().includes(o))return!1;const u=(s.status||"").toLowerCase();if(!(t===""||t==="chờ cọc"&&(u.includes("chờ cọc")||u.includes("chờ xác nhận"))||t==="đã cọc"&&(u.includes("đã cọc")||u.includes("hoàn tất")||u.includes("hoàn thành"))||t==="đã hủy"&&u.includes("hủy")))return!1;const i=(s.gender||"").toLowerCase();return a===""||i===a});let n=`
            ...
            <body>
                <table>
                    ...
                    <tbody>
        `;r.forEach((s,u)=>{n+="<tr>",e.forEach(p=>{let i="";if(p.id==="col-stt")i=u+1;else if(p.id==="col-name")i=s.name||"";else if(p.id==="col-order-id")i=s.id||"";else if(p.id==="col-address")i=s.address||"";else if(p.id==="col-phone")i=s.phone?String(s.phone):"";else if(p.id==="col-diet")i=s.diet==="Ăn chay"||s.diet==="Chay"||s.diet==="Có"?"Có":"Không";else if(p.id==="col-pole")i=s.trekking_pole==="Có"?"Có":"Không";else if(p.id==="col-status")i=s.status||"";else if(p.id==="col-extra"){const c=s.services_booked;if(c&&c!=="[]"&&c!=="null"&&c!==""){const k=D=>D.map(h=>{const T=h.label||h.name||h.title||"",H=h.price&&Number(h.price)>0?` (+${Number(h.price).toLocaleString("vi-VN")}đ)`:"";return(T+H).trim()}).filter(Boolean).join(", ");if(Array.isArray(c))i=k(c);else if(typeof c=="string"&&c.trim().startsWith("["))try{i=k(JSON.parse(c))}catch{const h=c.match(/label:\s*['"]([^'"]+)['"]/g)||[],T=c.match(/price:\s*(\d+)/g)||[];h.length>0&&(i=h.map((H,R)=>{const Z=H.replace(/label:\s*['"]/,"").replace(/['"]$/,"").trim(),A=T[R]?Number(T[R].replace(/price:\s*/,"")):0;return(Z+(A>0?` (+${A.toLocaleString("vi-VN")}đ)`:"")).trim()}).filter(Boolean).join(", "))}}i||(i=s.extra_services||"")}else{const c=String(p.render(s,u)),k=document.createElement("DIV");k.innerHTML=c,i=k.textContent||k.innerText||""}const j=p.id==="col-phone"||p.id==="col-cccd";n+=`<td ${j?'class="num"':""}>${i}</td>`}),n+="</tr>"}),n+=`
                    </tbody>
                </table>
            </body>
            </html>
        `;const d=new Blob(["\uFEFF",n],{type:"application/vnd.ms-excel;charset=utf-8"}),l=URL.createObjectURL(d),g=`DS_Doan_${f||"Tour"}_${S||"Date"}.xls`.replace(/[/\\?%*:|"<>]/g,"-"),m=document.createElement("a");m.setAttribute("href",l),m.setAttribute("download",g),document.body.appendChild(m),m.click(),document.body.removeChild(m)};E.addEventListener("change",L),B.addEventListener("change",L),document.getElementById("filterSearch")?.addEventListener("input",L),document.getElementById("filterSort")?.addEventListener("change",_),document.getElementById("btnExportPDF").addEventListener("click",()=>{window.print()}),document.getElementById("btnExportExcel").addEventListener("click",()=>{Y()}),W(),q(),_()};export{ae as afterRender,ne as render};
