import{S as nt,H as ot}from"./Header-CvqOcIss.js";const lt=()=>`
    ...
    `,dt=()=>{let T=[],y="pending",D=[],A=[];const tt=async()=>{try{const[e,t]=await Promise.all([fetch("/api/tours"),fetch("/api/schedules")]);D=e.ok?await e.json():[],A=t.ok?await t.json():[],R("addTourName"),R("edit-tour")}catch(e){console.error("Lỗi tải tours/schedules:",e)}},R=e=>{const t=document.getElementById(e);if(!t)return;const s=t.value,i=t.options[0].outerHTML;t.innerHTML=i,D.filter(r=>r.is_visible!==!1).forEach(r=>{const a=document.createElement("option");a.value=r.name,a.textContent=r.name,r.name===s&&(a.selected=!0),t.appendChild(a)})},$=(e,t,s)=>{const i=document.getElementById(e);if(!i)return;if(i.innerHTML='<option value="">-- Chọn Lịch --</option>',!t){i.innerHTML='<option value="">-- Chọn Tour trước --</option>';return}const r=A.filter(a=>a.tour_name===t||t.includes(a.tour_name)||a.tour_name.includes(t));if(r.length===0){i.innerHTML='<option value="">Chưa có lịch cho tour này</option>';return}r.sort((a,o)=>new Date(a.start_date)-new Date(o.start_date)).forEach(a=>{const o=parseInt(a.booked_count)||0,l=(a.slots||0)-o,u=new Date(a.start_date),m=a.end_date?new Date(a.end_date):null,g=x=>`${x.getDate().toString().padStart(2,"0")}/${(x.getMonth()+1).toString().padStart(2,"0")}`;let p=g(u);m&&(p+=" - "+g(m)),p+=` (${l>=0?l:0} chỗ trống)`;const d=document.createElement("option");d.value=`${u.getDate().toString().padStart(2,"0")}/${(u.getMonth()+1).toString().padStart(2,"0")}/${u.getFullYear()}`,d.textContent=p,l<=0&&(d.disabled=!0,d.textContent=p.replace("chỗ trống","HẾT CHỖ")),s&&d.value===s&&(d.selected=!0),i.appendChild(d)})},V=document.getElementById("addTourName");V&&V.addEventListener("change",e=>{$("addTourDate",e.target.value)});const P=document.getElementById("edit-tour");P&&P.addEventListener("change",e=>{$("edit-date",e.target.value)}),tt();const N=()=>{const e=document.getElementById("bookingsTableHead"),t=document.getElementById("bookingsTableBody");if(!t||!e)return;const s=y==="upcoming"||y==="completed";s?e.innerHTML=`
            ...
            `;const i=document.getElementById("filterSearch")?document.getElementById("filterSearch").value.toLowerCase():"",r=document.getElementById("filterTour")?document.getElementById("filterTour").value:"",a=document.getElementById("filterDate")?document.getElementById("filterDate").value:"",o=document.getElementById("filterSale")?document.getElementById("filterSale").value:"";let l=T.filter(n=>{let h=!1;const c=parseInt(n.total_price)>0&&parseInt(n.total_price)===parseInt(n.deposit);return y==="pending"?h=!n.status||n.status==="Chờ cọc":y==="upcoming"?h=(n.status==="Chờ xác nhận cọc"||n.status==="Đã cọc"||n.status==="Đã cọc (Chờ đi)")&&!c:y==="completed"&&(h=c||n.status==="Hoàn thành"||n.status==="Đã đi"),!(!h||r&&n.tour!==r||a&&n.date!==a||o&&String(n.sale_id)!==String(o)||i&&!`${n.name||""} ${n.phone||""} ${n.customer_id||""}`.toLowerCase().includes(i))});const u=document.getElementById("statTitle1"),m=document.getElementById("statTitle2"),g=document.getElementById("statTitle3"),p=document.getElementById("statTotalCustomers"),d=document.getElementById("statTotalRevenue"),x=document.getElementById("statTotalCollected");let B=l.length;if(y==="pending"){u&&(u.textContent="Tổng Khách (Chờ Cọc)"),m&&(m.textContent="Khách Chưa Tư Vấn (Mới)"),g&&(g.textContent="Đã Có Sale Giữ Chỗ");let n=0,h=0;l.forEach(c=>{!c.sale_id||c.sale_id==="null"||!c.sale_name||c.sale_name==="Website"||c.sale_name==="null"?n++:h++}),p&&(p.textContent=B),d&&(d.textContent=n+" Đơn"),x&&(x.textContent=h+" Đơn")}else if(y==="upcoming"){u&&(u.textContent="Tổng Số Khách"),m&&(m.textContent="Chưa Xác Nhận Cọc"),g&&(g.textContent="Chờ Thanh Toán Còn Lại");let n=0,h=0;l.forEach(c=>{c.status==="Chờ xác nhận cọc"?n++:parseInt(c.deposit)>0&&parseInt(c.total_price)>parseInt(c.deposit)&&h++}),p&&(p.textContent=B),d&&(d.textContent=n+" Khách"),x&&(x.textContent=h+" Khách")}else{u&&(u.textContent="Khách Sẵn Sàng (Full)"),m&&(m.textContent="Tổng Doanh Thu Tab"),g&&(g.textContent="Thực Thu (Full Tận Nơi)");let n=0,h=0;l.forEach(c=>{n+=parseInt(c.total_price)||0,h+=parseInt(c.deposit)||0}),p&&(p.textContent=B),d&&(d.textContent=n>0?n.toLocaleString("vi-VN")+"đ":"0đ"),x&&(x.textContent=h>0?h.toLocaleString("vi-VN")+"đ":"0đ")}const M=s?8:4;if(!l||l.length===0){let n=y==="pending"?"Chưa có Khách chờ cọc.":y==="upcoming"?"Chưa có Khách nào Đã Cọc.":"Danh sách rỗng.";t.innerHTML=`<tr><td colspan="${M}" class="p-8 text-center text-gray-500">${n}</td></tr>`;return}t.innerHTML=l.map(n=>{const h=y==="upcoming"?"bg-green-50/20 hover:bg-green-50 transition-colors":"hover:bg-gray-100 transition-colors";if(s){const c=parseInt(n.total_price)||0,f=parseInt(n.deposit)||0,k=c-f,C=c>0?c.toLocaleString("vi-VN")+"đ":"Chưa định giá",I=f>0?f.toLocaleString("vi-VN")+"đ":"0đ",_=k>0?k.toLocaleString("vi-VN")+"đ":k===0&&c>0?"Đã thu trọn":"-";let b="";return n.status==="Hoàn thành"||n.status==="Đã đi"?b='<span class="bg-gray-100 text-gray-600 border border-gray-200 px-2 py-0.5 rounded text-xs block w-full text-center">Hoàn thành</span>':n.status==="Chờ xác nhận cọc"?b=`<button class="action-btn confirm-deposit-btn bg-csr-orange text-white px-3 py-1.5 rounded text-xs font-bold shadow-sm hover:bg-[#d65503] w-full" data-id="${n.id}">Xác nhận cọc</button>`:f>0&&k===0?b='<span class="bg-green-100 text-green-700 border border-green-200 px-2 py-0.5 rounded text-xs font-medium block w-full text-center">Hoàn tất phí</span>':b='<span class="bg-blue-100 text-blue-700 border border-blue-200 px-2 py-0.5 rounded text-xs font-medium block w-full text-center">Đã Cọc</span>',`
                <tr class="${h} cursor-pointer row-clickable" data-id="${n.id}">
                    ...
                    <td class="p-4 align-top text-center">${b}</td>
                    <td class="p-4 align-top text-right text-sm font-medium text-gray-700">${C}</td>
                    <td class="p-4 align-top text-right text-sm font-medium text-green-600">${I}</td>
                    <td class="p-4 align-top text-right text-sm font-bold text-red-500">${_}</td>
                ...
                `}}).join("")},et=()=>{const e=new Set,t=new Set,s=new Map;T.forEach(o=>{o.tour&&e.add(o.tour),o.date&&t.add(o.date),o.sale_id&&o.sale_name&&s.set(o.sale_id,o.sale_name)});const i=document.getElementById("filterTour"),r=document.getElementById("filterDate"),a=document.getElementById("filterSale");if(i){const o=i.value;i.innerHTML='<option value="">Tất cả Tour</option>',[...e].sort().forEach(l=>i.innerHTML+=`<option value="${l}">${l}</option>`),i.value=o}if(r){const o=r.value;r.innerHTML='<option value="">Tất cả Ngày</option>',[...t].sort().forEach(l=>r.innerHTML+=`<option value="${l}">${l}</option>`),r.value=o}if(a){const o=a.value;a.innerHTML='<option value="">Tất cả Sale</option>',a.innerHTML+='<option value="null">Nguồn Website</option>';for(let[l,u]of s)a.innerHTML+=`<option value="${l}">${u}</option>`;a.value=o}},w=async()=>{try{T=await(await fetch("/api/bookings")).json(),et(),N()}catch(e){console.error("Lỗi parse list đơn hàng:",e);const t=document.getElementById("bookingsTableBody");t&&(t.innerHTML='<tr><td colspan="4" class="p-8 text-center text-red-500">Lỗi lấy dữ liệu từ Server.</td></tr>')}};w(),["filterSearch","filterTour","filterDate","filterSale"].forEach(e=>{const t=document.getElementById(e);t&&(e==="filterSearch"?t.addEventListener("input",N):t.addEventListener("change",N))});const q=document.getElementById("exportExcelBtn");q&&q.addEventListener("click",()=>{const e=document.getElementById("filterSearch")?document.getElementById("filterSearch").value.toLowerCase():"",t=document.getElementById("filterTour")?document.getElementById("filterTour").value:"",s=document.getElementById("filterDate")?document.getElementById("filterDate").value:"",i=document.getElementById("filterSale")?document.getElementById("filterSale").value:"";let r=T.filter(d=>{let x=!1;return y==="pending"?x=d.status==="Chờ xác nhận cọc"||parseInt(d.deposit)===0||!d.deposit:y==="upcoming"?x=d.status!=="Chờ xác nhận cọc"&&parseInt(d.deposit)>0:y==="completed"&&(x=d.status==="Hoàn thành"),!(!x||t&&d.tour!==t||s&&d.date!==s||i&&String(d.sale_id)!==String(i)||e&&!`${d.name||""} ${d.phone||""} ${d.customer_id||""}`.toLowerCase().includes(e))});if(r.length===0){alert("Danh sách rỗng. Không có dữ liệu để xuất.");return}const o=[["Mã KH (CRM)","Họ và Tên","Số điện thoại","Tên Tour","Lịch trình","Trạng thái","Nguồn / Sale","Đã cọc","Tổng tiền","Ghi chú"]];r.forEach(d=>{o.push([d.customer_id||"",`"${d.name||""}"`,`="${d.phone||""}"`,`"${d.tour||""}"`,`"${d.date||""}"`,`"${d.status||""}"`,`"${d.sale_name||"Website"}"`,d.deposit||0,d.total_price||0,`"${(d.special||"").replace(/"/g,'""')}"`])});const l="\\uFEFF"+o.map(d=>d.join(",")).join(`
`),u=new Blob([l],{type:"text/csv;charset=utf-8;"}),m=URL.createObjectURL(u),g=document.createElement("a");g.setAttribute("href",m);let p=y==="pending"?"ChoCoc":y==="upcoming"?"SapThamGia":"HoanThanh";g.setAttribute("download",`BaoCao_DonHang_${p}_${new Date().toISOString().slice(0,10)}.csv`),document.body.appendChild(g),g.click(),document.body.removeChild(g)});const z=document.querySelectorAll(".tab-btn");z.forEach(e=>{e.addEventListener("click",t=>{z.forEach(s=>{s.classList.remove("border-csr-orange","text-csr-orange"),s.classList.add("border-transparent","text-gray-500")}),t.target.classList.add("border-csr-orange","text-csr-orange"),t.target.classList.remove("border-transparent","text-gray-500"),y=t.target.getAttribute("data-tab"),N()})});const E=document.getElementById("bookingModal"),L=document.getElementById("bookingModalContent"),K=document.getElementById("detailModal"),F=document.getElementById("detailModalContent"),S=document.getElementById("editModal"),j=document.getElementById("editModalContent"),G=document.getElementById("addBookingBtn");E&&(E.classList.add("duration-200","ease-out"),L.classList.add("duration-300","ease-out")),S&&(S.classList.add("duration-200","ease-out"),j.classList.add("duration-300","ease-out")),G&&G.addEventListener("click",()=>{document.getElementById("bookingModalContent").querySelector("h2").innerText="Thêm Khách Hàng (Tạo Đơn)",document.getElementById("submitBookingBtn").innerText="Tạo Phơi Đăng Ký",document.getElementById("editingBookingId").value="",document.getElementById("bookingForm").reset();const e=document.getElementById("loyalty-alert");e&&e.remove(),E.classList.remove("hidden"),setTimeout(()=>{E.classList.add("opacity-100"),L.classList.remove("scale-95","translate-y-4"),L.classList.add("scale-100","translate-y-0")},10)}),window.closeModal=()=>{E&&(E.classList.remove("opacity-100"),L.classList.remove("scale-100","translate-y-0"),L.classList.add("scale-95","translate-y-4"),setTimeout(()=>{E.classList.add("hidden")},200))},window.closeDetailModal=()=>{K&&(K.classList.remove("opacity-100"),F.classList.remove("scale-100","translate-y-0"),F.classList.add("scale-95","translate-y-4"),setTimeout(()=>{K.classList.add("hidden")},200))},window.closeEditModal=()=>{S&&(S.classList.remove("opacity-100"),j.classList.remove("scale-100","translate-y-0"),j.classList.add("scale-95","translate-y-4"),setTimeout(()=>{S.classList.add("hidden")},200))},window.openRowActionModal=e=>{document.getElementById("ramName").textContent=e.name||"Khách Hàng Này",document.getElementById("ramPhone").textContent=e.customer_id?`🥇 ${e.customer_id.toUpperCase()}`:e.phone||"";const t=`
        ...
        `;document.getElementById("detailContentBlock").innerHTML=r,document.getElementById("detailModal").classList.remove("hidden"),setTimeout(()=>{document.getElementById("detailModal").classList.add("opacity-100");const a=document.getElementById("detailModalContent");a&&(a.classList.remove("scale-95","translate-y-4"),a.classList.add("scale-100","translate-y-0"))},10)},window.actionEdit=e=>{document.getElementById("rowActionModal").classList.add("hidden");const t=T.find(o=>o.id==e);if(!t)return;document.getElementById("edit-id").value=e,document.getElementById("edit-name").value=t.name||"",document.getElementById("edit-phone").value=t.phone||"",document.getElementById("edit-medal-name").value=t.medal_name||"",document.getElementById("edit-tour").value=t.tour||"",typeof $=="function"&&$("edit-date",t.tour||"",t.date||"");const s=document.getElementById("edit-date");if(s&&t.date&&s.value!==t.date){const o=document.createElement("option");o.value=t.date,o.textContent=t.date+" (dữ liệu cũ)",o.selected=!0,s.appendChild(o)}document.getElementById("edit-dob").value=t.dob||"",document.getElementById("edit-gender").value=t.gender||"Khác",document.getElementById("edit-status").value=t.status||"Chờ xác nhận cọc",document.getElementById("edit-allergy").value=t.allergy||"",document.getElementById("edit-address").value=t.address||"",document.getElementById("edit-diet").value=t.diet||"Không",document.getElementById("edit-trekking-pole").value=t.trekking_pole||"Không",document.getElementById("edit-commitments").checked=!!t.commitments,document.getElementById("edit-special").value=t.special||"",document.getElementById("edit-total").value=t.total_price||0,document.getElementById("edit-deposit").value=t.deposit||0,(()=>{const o=parseInt(document.getElementById("edit-total").value)||0,l=parseInt(document.getElementById("edit-deposit").value)||0,u=o-l,m=document.getElementById("edit-remaining");m&&(m.textContent=u>0?u.toLocaleString("vi-VN")+"đ":"0đ")})();const r=document.getElementById("editModal"),a=document.getElementById("editModalContent");r&&a&&(r.classList.remove("hidden"),setTimeout(()=>{r.classList.add("opacity-100"),a.classList.remove("scale-95","translate-y-4"),a.classList.add("scale-100","translate-y-0")},10))},window.actionDelete=async e=>{if(document.getElementById("rowActionModal").classList.add("hidden"),confirm("Bạn có chắc chắn muốn xóa Đơn hàng này? Thao tác không thể hoàn tác."))try{if((await fetch(`/api/bookings?id=${e}`,{method:"DELETE"})).ok)alert("✅ Đã xóa đơn hàng thành công!"),w();else throw new Error("Lỗi từ Server")}catch(t){alert("❌ Không thể xóa bảng ghi này: "+t.message)}};const O=()=>{const e=parseInt(document.getElementById("edit-total").value)||0,t=parseInt(document.getElementById("edit-deposit").value)||0,s=e-t,i=document.getElementById("edit-remaining");i&&(i.textContent=s>0?s.toLocaleString("vi-VN")+"đ":"0đ")},U=document.getElementById("edit-total"),J=document.getElementById("edit-deposit");U&&U.addEventListener("input",O),J&&J.addEventListener("input",O),document.querySelectorAll('#bookingModal button[onclick*="hidden"]').forEach(e=>{e.removeAttribute("onclick"),e.addEventListener("click",window.closeModal)});const Y=async e=>{try{const s=await(await fetch("/api/admin_customers?action=create",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({full_name:e.name,phone:e.phone,cccd:e.id_card||"",dob:e.dob||null,gender:e.gender||"Khác",medical_notes:e.allergy||"",dietary:e.diet||""})})).json();s.success&&s.csr_code&&(console.log("✅ Đã đồng bộ CRM thành công:",e.name,s.csr_code),(await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:e.id,customer_id:s.csr_code})})).ok&&(console.log("✅ Đã cập nhật Booking mã CRM"),typeof w=="function"&&w()))}catch(t){console.error("❌ Lỗi đồng bộ CRM:",t)}},W=document.getElementById("bookingsTableBody");W&&W.addEventListener("click",async e=>{const t=e.target.closest(".action-btn");if(!t){const a=e.target.closest(".row-clickable");if(a){const o=a.getAttribute("data-id"),l=T.find(u=>u.id==o);l&&openRowActionModal(l)}return}const s=t.getAttribute("data-id"),i=T.find(a=>a.id==s);if(!i)return;const r=document.getElementById("rowActionModal");if(r&&r.classList.add("hidden"),t.classList.contains("confirm-deposit-btn")){if(confirm("Xác nhận khách hàng này đã chuyển khoản đặt cọc?"))try{if(t.disabled=!0,t.classList.add("opacity-50"),(await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:s,status:"Đã cọc"})})).ok)alert("✅ Đã xác nhận cọc thành công!"),Y(i),typeof w=="function"&&w();else throw new Error("Lỗi cập nhật API")}catch(a){alert("❌ Lỗi: "+a.message)}finally{t.disabled=!1,t.classList.remove("opacity-50")}return}if(t.classList.contains("delete-btn")){window.actionDelete(s);return}else if(t.classList.contains("process-btn")){const l=(window.location.hostname==="localhost"||window.location.hostname==="127.0.0.1"?"http://localhost:8888":window.location.origin)+`/booking/process.html?id=${s}`;navigator.clipboard.writeText(l).then(()=>{alert("📋 Đã sao chép Link Form Điền Thông Tin Cơ Bản! Gửi cho khách qua Zalo nhé:\\n"+l)}).catch(u=>{alert("Lỗi khi Copy Clipboard. Link là: "+l)})}else if(t.classList.contains("send-btn")){const l=(window.location.hostname==="localhost"||window.location.hostname==="127.0.0.1"?"http://localhost:8888":window.location.origin)+`/pay/${s}`;navigator.clipboard.writeText(l).then(()=>{alert("📋 Đã sao chép Link Thanh Toán Cọc! Gửi cho khách qua Zalo nhé:\\n"+l)}).catch(u=>{alert("Lỗi khi Copy Clipboard. Link là: "+l)})}else if(t.classList.contains("payment-btn")){const l=(window.location.hostname==="localhost"||window.location.hostname==="127.0.0.1"?"http://localhost:8888":window.location.origin)+`/pay/${s}`;window.open(l,"_blank")}else t.classList.contains("view-btn")?(document.getElementById("rowActionModal").classList.add("hidden"),window.actionView(s)):t.classList.contains("edit-btn")&&window.actionEdit(s)});const X=document.getElementById("searchBtn");X&&X.addEventListener("click",async e=>{e.preventDefault();const t=document.getElementById("smartSearch").value,s=e.target;if(!t){alert("Vui lòng nhập Số Điện Thoại hoặc Mã #CSR");return}s.innerHTML='<svg class="animate-spin h-4 w-4 text-gray-900 mx-auto" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>',s.disabled=!0;try{const r=await fetch("/api/admin_customers?action=search",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({keyword:t})}),a=await r.json();if(r.ok&&a.success){const o=a.data;if(document.getElementById("addFullName")&&(document.getElementById("addFullName").value=o.full_name||""),document.getElementById("addPhone")&&(document.getElementById("addPhone").value=o.phone||""),document.getElementById("addCsrCode")&&(document.getElementById("addCsrCode").value=o.csr_code||""),document.getElementById("addDob")&&o.dob){let g=o.dob;if(g.includes("/")){const p=g.split("/");g=p[2]+"-"+p[1].padStart(2,"0")+"-"+p[0].padStart(2,"0")}document.getElementById("addDob").value=g}document.getElementById("addGender")&&(document.getElementById("addGender").value=o.gender||"Khác"),document.getElementById("addIdCard")&&(document.getElementById("addIdCard").value=o.cccd||""),document.getElementById("addAllergy")&&(document.getElementById("addAllergy").value=o.medical_notes||""),document.getElementById("addDiet")&&(document.getElementById("addDiet").value=o.dietary||"Bình Thường"),document.getElementById("addAddress")&&(document.getElementById("addAddress").value=o.address||""),document.getElementById("addTrekkingPole")&&(document.getElementById("addTrekkingPole").value=o.trekking_pole||"Không"),document.getElementById("addMedalName")&&(document.getElementById("addMedalName").value=o.medal_name||"");let l="";o.loyalty_tier==="VIP"?l=`<span class="bg-yellow-500/20 text-yellow-500 ml-2 px-2 py-0.5 rounded text-xs">⭐ Khách VIP - Đã đi ${o.tour_count} chuyến</span>`:o.loyalty_tier==="Member"&&(l='<span class="bg-csr-orange/20 text-csr-orange ml-2 px-2 py-0.5 rounded text-xs">✨ Thành viên</span>');const u=`
                    ...
                    </div>`,m=document.getElementById("loyalty-alert");m&&m.remove(),document.getElementById("smartSearch").parentElement.insertAdjacentHTML("afterend",u)}else throw new Error(a.message||"Không tìm thấy dữ liệu.")}catch(i){alert("❌ "+i.message),document.getElementById("bookingForm").reset();const r=document.getElementById("loyalty-alert");r&&r.remove()}finally{s.innerHTML="Tra cứu",s.disabled=!1}});const Q=document.getElementById("bookingForm");Q&&Q.addEventListener("submit",async e=>{e.preventDefault();const t=e.target.querySelector('button[type="submit"]'),s=t.textContent;t.textContent="Đang lưu hệ thống...",t.disabled=!0;try{const i=document.getElementById("addFullName").value,r=document.getElementById("addPhone").value,a=document.getElementById("addTourName").value,o=document.getElementById("addTourDate").value,l=document.getElementById("addNote").value,u=document.getElementById("addDob").value||null,m=document.getElementById("addGender").value||"Khác",g=document.getElementById("addIdCard").value||"",p=document.getElementById("addAddress").value||"",d=document.getElementById("addDiet").value||"Bình Thường",x=document.getElementById("addTrekkingPole").value||"Không",B=document.getElementById("addAllergy").value||"",M=document.getElementById("addDepositRequired"),n=M?parseInt(M.value):1e6,h=document.getElementById("addMedalName")?document.getElementById("addMedalName").value:"",c=localStorage.getItem("csr_admin_session");let f=null,k="Admin";if(c){const v=JSON.parse(c);f=v.id,k=v.full_name}const C=document.getElementById("editingBookingId")?document.getElementById("editingBookingId").value:"",I=D.find(v=>v.name===a),_=I&&parseInt(I.price)||0,b={name:i,phone:r,tour:a,date:o,status:"Chờ cọc",total_price:_,deposit:0,sale_id:f,sale_name:k,customer_id:"",dob:u,gender:m,id_card:g,address:p,diet:d,trekking_pole:x,allergy:B,special:l,medal_name:h,deposit_required:n};C&&(b.id=C,delete b.status,delete b.deposit,delete b.total_price,delete b.sale_id,delete b.sale_name,delete b.customer_id);const H=await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(b)});if(H.ok){alert(C?"✅ Đã lưu Cập nhật Đơn hàng!":"✅ Thêm Đơn Hàng Thành Công! (Khách chờ thông tin/cọc)"),window.closeModal();const v=document.getElementById("loyalty-alert");v&&v.remove(),typeof w=="function"?w():window.location.reload()}else{const v=await H.json();throw new Error(v.error||"Lỗi tạo Booking!")}}catch(i){alert("❌ "+i.message)}finally{t.textContent=s,t.disabled=!1}});const Z=document.getElementById("editForm");Z&&Z.addEventListener("submit",async e=>{e.preventDefault();const t=e.target.querySelector('button[type="submit"]'),s=t.textContent;t.textContent="Đang lưu cập nhật...",t.disabled=!0;try{const i=document.getElementById("edit-id").value,r=document.getElementById("edit-name").value,a=document.getElementById("edit-phone").value,o=document.getElementById("edit-medal-name").value,l=document.getElementById("edit-tour").value,u=document.getElementById("edit-date").value,m=document.getElementById("edit-dob").value,g=document.getElementById("edit-gender").value,p=document.getElementById("edit-status").value,d=document.getElementById("edit-allergy").value,x=document.getElementById("edit-address").value,B=document.getElementById("edit-diet").value,M=document.getElementById("edit-trekking-pole").value,n=document.getElementById("edit-commitments").checked,h=document.getElementById("edit-special").value,c=document.getElementById("edit-total").value,f=document.getElementById("edit-deposit").value,k=document.getElementById("edit-deposit-required")?document.getElementById("edit-deposit-required").value:"",C=c?parseInt(c):0,I=f?parseInt(f):0,_=k?parseInt(k):1e6,b={id:i,name:r,phone:a,medal_name:o,tour:l,date:u,dob:m,gender:g,status:p,allergy:d,address:x,diet:B,trekking_pole:M,commitments:n,special:h,total_price:C,deposit:I,deposit_required:_},H=await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(b)});if(H.ok)alert("✅ Đã cập nhật thành công Chi tiết Đơn hàng!"),(p.includes("Đã cọc")||C>0&&C===I)&&Y(b),window.closeEditModal(),typeof w=="function"&&w();else{const v=await H.json();throw new Error(v.error||"Gặp sự cố khi Cập nhật API.")}}catch(i){alert("❌ Lỗi Cập nhật: "+i.message)}finally{t.textContent=s,t.disabled=!1}})};export{dt as afterRender,lt as render};
