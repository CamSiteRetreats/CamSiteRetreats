import{S as $e,H as _e}from"./Header-CARqqxjt.js";console.log("Bookings V3 - Cache Busted!");const De=()=>`
    ...
    `,Ne=()=>{let D=[],B="consult",N=[],te=[],J=[];const Te=async()=>{try{const[n,e,o]=await Promise.all([fetch("/api/tours"),fetch("/api/schedules"),fetch("/api/users")]);N=n.ok?await n.json():[],te=e.ok?await e.json():[],J=o.ok?await o.json():[],U("addTourName"),U("edit-tour"),U("wl-tour"),Ie("edit-sale")}catch(n){console.error("Lỗi tải tours/schedules:",n)}},Ie=n=>{const e=document.getElementById(n);e&&(e.innerHTML='<option value="">-- Website --</option>',J.forEach(o=>{const a=document.createElement("option");a.value=o.id,a.textContent=`${o.full_name||o.fullName} (${o.role})`,e.appendChild(a)}))},U=n=>{const e=document.getElementById(n);if(!e)return;const o=e.value,a=e.options[0].outerHTML;e.innerHTML=a,N.filter(u=>u.is_visible!==!1).forEach(u=>{const p=document.createElement("option");p.value=u.name,p.textContent=u.name,u.name===o&&(p.selected=!0),e.appendChild(p)})},K=(n,e,o,a)=>{const u=document.getElementById(n);if(!u)return;if(u.innerHTML='<option value="">-- Chọn Lịch --</option>',!e){u.innerHTML='<option value="">-- Chọn Tour trước --</option>';return}const p=te.filter(i=>i.tour_name===e||e.includes(i.tour_name)||i.tour_name.includes(e));if(p.length===0){u.innerHTML='<option value="">Chưa có lịch cho tour này</option>';return}p.sort((i,r)=>new Date(i.start_date)-new Date(r.start_date)).forEach(i=>{const r=new Date(i.start_date),h=`${r.getDate().toString().padStart(2,"0")}/${(r.getMonth()+1).toString().padStart(2,"0")}/${r.getFullYear()}`;let x=o||"";if(x.includes("-")&&x.length>=10){const d=x.substring(0,10).split("-");d.length===3&&(x=`${d[2]}/${d[1]}/${d[0]}`)}const g=a&&String(i.id)===String(a)||x&&h===x,C=new Date;if(C.setHours(0,0,0,0),r<C&&!g)return;const E=parseInt(i.booked_count)||0,I=(i.slots||0)-E,m=i.end_date?new Date(i.end_date):null,T=d=>`${d.getDate().toString().padStart(2,"0")}/${(d.getMonth()+1).toString().padStart(2,"0")}`;let S=T(r);m&&(S+=" - "+T(m)),i.group_label&&(S+=` · ${i.group_label}`),S+=` (${I>=0?I:0} chỗ trống)`;const t=document.createElement("option");t.value=`${i.id}::${h}`,t.dataset.scheduleId=i.id,t.dataset.date=h,t.textContent=S,I<=0&&(t.disabled=!0,t.textContent=S.replace("chỗ trống","HẾT CHỖ")),(a&&String(i.id)===String(a)||!a&&x&&h===x)&&(t.selected=!0),u.appendChild(t)})},ne=document.getElementById("addTourName");ne&&ne.addEventListener("change",n=>{const e=n.target.value;K("addTourDate",e);const o=N.find(u=>u.name===e),a=document.getElementById("addTourPrice");if(o&&a){const u=parseInt(o.price)||0;a.value=u;const p=document.getElementById("addDiscount"),i=document.getElementById("addTotalPriceDisplay"),r=parseInt(p?p.value:0)||0,h=u-r;i&&(i.textContent=(h>0?h:0).toLocaleString("vi-VN")+"đ")}});const oe=document.getElementById("edit-tour");oe&&oe.addEventListener("change",n=>{const e=n.target.value;K("edit-date",e);const o=N.find(u=>u.name===e),a=document.getElementById("edit-total");o&&a&&(parseInt(a.value)===0||!a.value)&&(a.value=parseInt(o.price)||0,typeof window.updateEditRemaining=="function"&&window.updateEditRemaining())});const ae=document.getElementById("wl-tour");ae&&ae.addEventListener("change",n=>{const e=n.target.value;K("wl-date",e)}),Te();const R=()=>{const n=document.getElementById("bookingsTableHead"),e=document.getElementById("bookingsTableBody");if(!e||!n)return;const o=B==="upcoming"||B==="ready"||B==="completed";o?n.innerHTML=`
            ...
            `;const a=document.getElementById("filterSearch")?document.getElementById("filterSearch").value.toLowerCase():"",u=document.getElementById("filterTour")?document.getElementById("filterTour").value:"",p=document.getElementById("filterDate")?document.getElementById("filterDate").value:"",i=document.getElementById("filterSale")?document.getElementById("filterSale").value:"",r=document.getElementById("filterStatus")?document.getElementById("filterStatus").value:"";let h=D.filter(t=>{let d=!1;const s=parseInt(t.total_price)>0&&parseInt(t.total_price)===parseInt(t.deposit),l=f=>{if(!f)return!1;try{let w=f.trim();if(w.includes("-")&&!/^\d{4}-\d{2}-\d{2}/.test(w)&&(w=w.split("-")[0].trim()),w.includes("/")){const v=w.split("/");if(v.length>=2){const k=parseInt(v[0],10),y=parseInt(v[1],10)-1,_=v.length===3?parseInt(v[2],10):new Date().getFullYear(),j=new Date(_,y,k),L=new Date;return L.setHours(0,0,0,0),j.getTime()<L.getTime()}}if(/^\d{4}-\d{2}-\d{2}/.test(w)){const v=w.split("-"),k=parseInt(v[0],10),y=parseInt(v[1],10)-1,_=parseInt(v[2],10),j=new Date(k,y,_),L=new Date;return L.setHours(0,0,0,0),j.getTime()<L.getTime()}}catch(w){console.warn("isPastTour error parsing:",f,w)}return!1},c=t.status||"",b=!l(t.date);return B==="consult"?d=c==="Chờ tư vấn"&&b:B==="pending"?d=(c==="Chờ cọc"||c==="Chờ xác nhận cọc"||c==="")&&b:B==="upcoming"?d=(c==="Đã cọc"||c==="Đã cọc (Chờ đi)"||c==="Hoàn tất phí")&&!s&&b:B==="ready"?d=(s||c==="Hoàn thành"||c==="Hoàn tất")&&b&&c!=="Đã hủy"&&c!=="Bảo lưu":B==="completed"&&(d=!b||c==="Đã đi"||c==="Đã hủy"||c==="Bảo lưu"),!(!d||u&&t.tour!==u||p&&Y(t.date)!==p||i&&i!=="null"&&t.sale_name!==i||i==="null"&&t.sale_name&&t.sale_name!=="Website"||a&&!`${t.name||""} ${t.phone||""} ${t.customer_id||""} ${t.id||""} csr${t.id||""} #csr${t.id||""}`.toLowerCase().includes(a)||r&&t.status!==r)});const x=document.getElementById("statTitle1"),g=document.getElementById("statTitle2"),C=document.getElementById("statTitle3"),E=document.getElementById("statTotalCustomers"),I=document.getElementById("statTotalRevenue"),m=document.getElementById("statTotalCollected");let T=h.length;if(B==="consult"){const t=document.getElementById("addWaitingBtn"),d=document.getElementById("addBookingBtn");t&&t.classList.remove("hidden"),d&&d.classList.add("hidden"),x&&(x.textContent="Tổng Đăng Ký Chờ Lịch"),g&&(g.textContent="Chưa Được Liên Hệ"),C&&(C.textContent="Đã Có Sale Phụ Trách");let s=0,l=0;h.forEach(c=>{!c.sale_id||c.sale_id==="null"||!c.sale_name||c.sale_name==="Website"||c.sale_name==="null"?s++:l++}),E&&(E.textContent=T),I&&(I.textContent=s+" Người"),m&&(m.textContent=l+" Người")}else if(B==="pending"){const t=document.getElementById("addWaitingBtn"),d=document.getElementById("addBookingBtn");t&&t.classList.add("hidden"),d&&d.classList.remove("hidden"),x&&(x.textContent="Tổng Khách (Chờ Cọc)"),g&&(g.textContent="Khách Chưa Tư Vấn (Mới)"),C&&(C.textContent="Đã Có Sale Giữ Chỗ");let s=0,l=0;h.forEach(c=>{!c.sale_id||c.sale_id==="null"||!c.sale_name||c.sale_name==="Website"||c.sale_name==="null"?s++:l++}),E&&(E.textContent=T),I&&(I.textContent=s+" Đơn"),m&&(m.textContent=l+" Đơn")}else if(B==="upcoming"){x&&(x.textContent="Tổng Số Khách"),g&&(g.textContent="Chưa Xác Nhận Cọc"),C&&(C.textContent="Chờ Thanh Toán Còn Lại");let t=0,d=0;h.forEach(s=>{s.status==="Chờ xác nhận cọc"?t++:parseInt(s.deposit)>0&&parseInt(s.total_price)>parseInt(s.deposit)&&d++}),E&&(E.textContent=T),I&&(I.textContent=t+" Khách"),m&&(m.textContent=d+" Khách")}else if(B==="ready"){x&&(x.textContent="Khách Đã Hoàn Tất"),g&&(g.textContent="Tổng Doanh Thu Tab"),C&&(C.textContent="Thực Thu");let t=0,d=0;h.forEach(s=>{t+=parseInt(s.total_price)||0,d+=parseInt(s.deposit)||0}),E&&(E.textContent=T),I&&(I.textContent=t>0?t.toLocaleString("vi-VN")+"đ":"0đ"),m&&(m.textContent=d>0?d.toLocaleString("vi-VN")+"đ":"0đ")}else{x&&(x.textContent="Khách Đã Xong (Lịch Sử)"),g&&(g.textContent="Tổng Doanh Thu Tab"),C&&(C.textContent="Thực Thu (Đã Đóng)");let t=0,d=0;h.forEach(s=>{t+=parseInt(s.total_price)||0,d+=parseInt(s.deposit)||0}),E&&(E.textContent=T),I&&(I.textContent=t>0?t.toLocaleString("vi-VN")+"đ":"0đ"),m&&(m.textContent=d>0?d.toLocaleString("vi-VN")+"đ":"0đ")}const S=o?8:4;if(!h||h.length===0){let t=B==="consult"?"Chưa có khách hàng tư vấn nào.":B==="pending"?"Chưa có Khách chờ cọc.":B==="upcoming"?"Chưa có Khách nào Đã Cọc.":B==="ready"?"Chưa có khách hoàn tất.":"Danh sách rỗng.";e.innerHTML=`<tr><td colspan="${S}" class="p-8 text-center text-gray-500">${t}</td></tr>`;return}e.innerHTML=h.map(t=>{const d=B==="upcoming"||B==="ready"?"bg-green-50/20 hover:bg-green-50 transition-colors":"hover:bg-gray-100 transition-colors";if(o){let s=parseInt(t.total_price)||0;if(s===0&&t.tour&&N&&N.length>0){const y=N.find(_=>_.name&&t.tour&&_.name.toLowerCase().trim()===t.tour.toLowerCase().trim());y&&parseInt(y.price)>0&&(s=parseInt(y.price))}const l=parseInt(t.deposit)||0,c=s-l,b=s>0?s.toLocaleString("vi-VN")+"đ":"Chưa định giá",f=l>0?l.toLocaleString("vi-VN")+"đ":"0đ",w=c>0?c.toLocaleString("vi-VN")+"đ":c===0&&s>0?"Đã thu trọn":"-";let v="";return t.status==="Đã hủy"?v='<span class="bg-red-50 text-red-500 border border-red-200 px-2 py-0.5 rounded text-xs block w-full text-center font-bold">❌ Đã hủy</span>':t.status==="Bảo lưu"?v='<span class="bg-gray-100 text-gray-500 border border-gray-300 px-2 py-0.5 rounded text-xs block w-full text-center font-bold">⏸️ Bảo lưu</span>':t.status==="Hoàn thành"||t.status==="Đã đi"?v='<span class="bg-violet-100 text-violet-700 border border-violet-200 px-2 py-0.5 rounded text-xs block w-full text-center font-bold">Hoàn Thành</span>':t.status==="Chờ xác nhận cọc"?v=`<button class="action-btn confirm-deposit-btn bg-csr-orange text-white px-3 py-1.5 rounded text-xs font-bold shadow-sm hover:bg-[#d65503] w-full" data-id="${t.id}">Xác nhận cọc</button>`:l>0&&c===0?v='<span class="bg-emerald-100 text-emerald-700 border border-emerald-200 px-2 py-0.5 rounded text-xs font-bold block w-full text-center">✅ Hoàn Tất Phí</span>':t.status==="Hoàn tất phí"||l>0?v='<span class="bg-blue-100 text-blue-700 border border-blue-200 px-2 py-0.5 rounded text-xs font-medium block w-full text-center">✅ Đã Cọc</span>':v='<span class="bg-yellow-50 text-yellow-700 border border-yellow-200 px-2 py-0.5 rounded text-xs font-medium block w-full text-center">Chưa cọc</span>',`
                <tr class="${d} cursor-pointer row-clickable block md:table-row p-4 md:p-0 mb-4 md:mb-0 glass-panel md:glass-none border-l-4 border-green-500 md:border-none relative" data-id="${t.id}">
                    ...
                    <td class="p-4 align-top text-center hidden md:table-cell">${v}</td>
                ...
                `}else{const s=t.name?t.name.substring(0,2).toUpperCase():"KH",l=B==="consult",c=!t.sale_id||t.sale_id==="null"||!t.sale_name||t.sale_name==="null";let b="";l&&c?b=`<button class="action-btn claim-btn bg-csr-orange hover:bg-[#d65503] text-white px-3 py-1.5 rounded text-xs font-bold shadow-sm transition-colors" data-id="${t.id}">Nhận khách</button>`:b=`<span class="text-sm text-gray-500">${t.sale_name||"Website"}</span>`;const f=`
                ...
                `;return`
                <tr class="${d} cursor-pointer row-clickable block md:table-row p-4 md:p-0 mb-4 md:mb-0 glass-panel md:glass-none border-l-4 border-csr-orange md:border-none relative" data-id="${t.id}">
                    <td class="p-0 md:p-4 align-top block md:table-cell">
                        <div class="flex items-center gap-3">
                            <div class="w-12 h-12 md:w-10 md:h-10 rounded-full bg-csr-orange/20 text-csr-orange flex items-center justify-center font-bold text-base md:text-sm shrink-0">${s}</div>
                            <div class="min-w-0">
                    ...
                    <td class="py-3 md:p-4 align-top block md:table-cell border-t md:border-none border-gray-100 mt-3 md:mt-0 pt-3 md:pt-4">
                        ...
                        ${l&&t.special?`<div class="text-xs text-blue-500 mt-1 italic bg-blue-50 md:bg-transparent p-2 md:p-0 rounded">💬 ${t.special}</div>`:""}
                    </td>
    ...
    `}}).join(""),document.querySelectorAll(".claim-btn").forEach(t=>{t.addEventListener("click",async d=>{d.stopPropagation();const s=t.getAttribute("data-id"),l=localStorage.getItem("csr_user");let c=null,b="Admin";if(l){const f=JSON.parse(l);c=f.id,b=f.fullName||f.full_name||"Admin"}t.textContent="Đang xử lý...",t.disabled=!0;try{if((await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:s,sale_id:c,sale_name:b})})).ok){const w=D.find(v=>v.id==s);w&&(w.sale_id=c,w.sale_name=b),R()}else alert("Lỗi khi nhận khách. Vui lòng thử lại."),t.textContent="Nhận khách",t.disabled=!1}catch(f){console.error("Lỗi nhận khách:",f),t.textContent="Nhận khách",t.disabled=!1}})})},Ee=()=>{const n=new Set,e=new Set,o=new Set;D.forEach(i=>{if(i.tour&&n.add(i.tour),i.date){const r=Y(i.date);r&&e.add(r)}i.sale_name&&i.sale_name!=="Website"&&i.sale_name!=="null"&&o.add(i.sale_name.trim())});const a=document.getElementById("filterTour"),u=document.getElementById("filterDate"),p=document.getElementById("filterSale");if(a){const i=a.value;a.innerHTML='<option value="">Tất cả Tour</option>',[...n].sort().forEach(r=>a.innerHTML+=`<option value="${r}">${r}</option>`),a.value=i}if(u){const i=u.value;u.innerHTML='<option value="">Tất cả Ngày</option>',[...e].sort().forEach(r=>u.innerHTML+=`<option value="${r}">${r}</option>`),u.value=i}if(p){const i=p.value;p.innerHTML='<option value="">Tất cả Sale</option>',p.innerHTML+='<option value="null">Nguồn Website</option>',[...o].sort().forEach(r=>{p.innerHTML+=`<option value="${r}">${r}</option>`}),p.value=i}};function Y(n){if(!n)return"";if(/^\d{1,2}\/\d{1,2}\s*-\s*\d{1,2}\/\d{1,2}$/.test(n))return n.replace(/\s+/g,"");try{if(/^\d{4}-\d{2}-\d{2}$/.test(n)){const[e,o,a]=n.split("-"),u=`${parseInt(a)}/${parseInt(o)}`;return`${u}-${u}`}if(/^\d{1,2}\/\d{1,2}\/\d{4}$/.test(n)){const[e,o,a]=n.split("/"),u=`${parseInt(e)}/${parseInt(o)}`;return`${u}-${u}`}if(n.includes("-"))return n.split("-").map(o=>{const a=o.trim();if(/\/\d{4}$/.test(a)){const[u,p]=a.split("/");return`${parseInt(u)}/${parseInt(p)}`}return a}).join("-")}catch{console.warn("Date normalization failed for:",n)}return n}const H=async()=>{try{D=await(await fetch("/api/bookings")).json(),D.sort((e,o)=>parseInt(o.id)-parseInt(e.id)),Ee(),R()}catch(n){console.error("Lỗi parse list đơn hàng:",n);const e=document.getElementById("bookingsTableBody");e&&(e.innerHTML='<tr><td colspan="4" class="p-8 text-center text-red-500">Lỗi lấy dữ liệu từ Server.</td></tr>')}};H(),["filterSearch","filterTour","filterDate","filterSale","filterStatus"].forEach(n=>{const e=document.getElementById(n);e&&(n==="filterSearch"?e.addEventListener("input",R):e.addEventListener("change",R))});const se=document.getElementById("exportExcelBtn");se&&se.addEventListener("click",()=>{const n=document.getElementById("filterSearch")?document.getElementById("filterSearch").value.toLowerCase():"",e=document.getElementById("filterTour")?document.getElementById("filterTour").value:"",o=document.getElementById("filterDate")?document.getElementById("filterDate").value:"",a=document.getElementById("filterSale")?document.getElementById("filterSale").value:"";let u=D.filter(g=>{let C=!1;const E=parseInt(g.total_price)>0&&parseInt(g.total_price)===parseInt(g.deposit),I=T=>{if(!T)return!1;try{let S=T.trim();if(S.includes("-")&&!/^\d{4}-\d{2}-\d{2}/.test(S)&&(S=S.split("-")[0].trim()),S.includes("/")){const t=S.split("/");if(t.length>=2){const d=parseInt(t[0],10),s=parseInt(t[1],10)-1,l=t.length===3?parseInt(t[2],10):new Date().getFullYear(),c=new Date(l,s,d),b=new Date;return b.setHours(0,0,0,0),c.getTime()<b.getTime()}}if(/^\d{4}-\d{2}-\d{2}/.test(S)){const t=S.split("-"),d=parseInt(t[0],10),s=parseInt(t[1],10)-1,l=parseInt(t[2],10),c=new Date(d,s,l),b=new Date;return b.setHours(0,0,0,0),c.getTime()<b.getTime()}}catch(S){console.warn("isPastTour error parsing in export:",T,S)}return!1},m=(g.status==="Hoàn thành"||g.status==="Hoàn tất"||g.status==="Hoàn tất phí"||g.status==="Đã đi"||E)&&I(g.date);return B==="consult"?C=g.status==="Chờ tư vấn"&&!m:B==="pending"?C=(!g.status||g.status==="Chờ cọc")&&!m:B==="upcoming"?C=(g.status==="Đã cọc"||g.status==="Đã cọc (Chờ đi)"||g.status==="Hoàn tất phí")&&!E&&!m:B==="ready"?C=(E||g.status==="Hoàn thành"||g.status==="Hoàn tất")&&!m:B==="completed"&&(C=m),!(!C||e&&g.tour!==e||o&&Y(g.date)!==o||a&&a!=="null"&&g.sale_name!==a||a==="null"&&g.sale_name&&g.sale_name!=="Website"||n&&!`${g.name||""} ${g.phone||""} ${g.customer_id||""} ${g.id||""} csr${g.id||""} #csr${g.id||""}`.toLowerCase().includes(n))});if(u.length===0){alert("Danh sách rỗng. Không có dữ liệu để xuất.");return}let p=`
            ...
            `;u.forEach(g=>{p+=`
                    <tr>
                        <td class="num">#CSR${g.id||""}</td>
                        <td>${(g.name||"").replace(/</g,"&lt;")}</td>
                        <td class="num">${g.phone||""}</td>
                        <td>${(g.tour||"").replace(/</g,"&lt;")}</td>
                        <td class="num">${g.date||""}</td>
                        <td>${(g.status||"").replace(/</g,"&lt;")}</td>
                        <td>${(g.sale_name||"Website").replace(/</g,"&lt;")}</td>
                        <td>${g.deposit||0}</td>
                        <td>${g.total_price||0}</td>
                        <td>${(g.special||"").replace(/</g,"&lt;")}</td>
                    </tr>
                `}),p+=`
                        </tbody>
                    </table>
            ...
            `;const i=new Blob(["\uFEFF",p],{type:"application/vnd.ms-excel;charset=utf-8"}),r=URL.createObjectURL(i),h=document.createElement("a");h.setAttribute("href",r);let x=B==="pending"?"ChoCoc":B==="upcoming"?"SapThamGia":B==="ready"?"ChoLenXe":"LichSu";h.setAttribute("download",`BaoCao_DonHang_${x}_${new Date().toISOString().slice(0,10)}.xls`),document.body.appendChild(h),h.click(),document.body.removeChild(h)});const ie=document.querySelectorAll(".tab-btn");ie.forEach(n=>{n.addEventListener("click",e=>{ie.forEach(o=>{o.classList.remove("border-csr-orange","text-csr-orange"),o.classList.add("border-transparent","text-gray-500")}),e.target.classList.add("border-csr-orange","text-csr-orange"),e.target.classList.remove("border-transparent","text-gray-500"),B=e.target.getAttribute("data-tab"),R()})});const A=document.getElementById("bookingModal"),V=document.getElementById("bookingModalContent"),X=document.getElementById("detailModal"),de=document.getElementById("detailModalContent"),O=document.getElementById("editModal"),Q=document.getElementById("editModalContent"),le=document.getElementById("addBookingBtn");A&&(A.classList.add("duration-200","ease-out"),V.classList.add("duration-300","ease-out")),O&&(O.classList.add("duration-200","ease-out"),Q.classList.add("duration-300","ease-out")),le&&le.addEventListener("click",()=>{document.getElementById("bookingModalContent").querySelector("h2").innerText="Thêm Khách Hàng (Tạo Đơn)",document.getElementById("submitBookingBtn").innerText="Tạo Phơi Đăng Ký",document.getElementById("editingBookingId").value="",document.getElementById("bookingForm").reset();const n=document.getElementById("loyalty-alert");n&&n.remove(),A.classList.remove("hidden"),setTimeout(()=>{A.classList.add("opacity-100"),V.classList.remove("scale-95","translate-y-4"),V.classList.add("scale-100","translate-y-0")},10)}),window.closeModal=()=>{A&&(A.classList.remove("opacity-100"),V.classList.remove("scale-100","translate-y-0"),V.classList.add("scale-95","translate-y-4"),setTimeout(()=>{A.classList.add("hidden")},200))},window.closeDetailModal=()=>{X&&(X.classList.remove("opacity-100"),de.classList.remove("scale-100","translate-y-0"),de.classList.add("scale-95","translate-y-4"),setTimeout(()=>{X.classList.add("hidden")},200))},window.closeEditModal=()=>{O&&(O.classList.remove("opacity-100"),Q.classList.remove("scale-100","translate-y-0"),Q.classList.add("scale-95","translate-y-4"),setTimeout(()=>{O.classList.add("hidden")},200))},window.openRowActionModal=n=>{document.getElementById("ramName").textContent=n.name||"Khách Hàng Này",document.getElementById("ramPhone").textContent=`#CSR${n.id} - ${n.phone||""}`;const e=`
        ...
        `;document.getElementById("ramActions").innerHTML=e,document.getElementById("rowActionModal").classList.remove("hidden")},window.addEventListener("click",n=>{const e=document.getElementById("rowActionModal");e&&n.target===e&&e.classList.add("hidden")}),window.actionView=n=>{document.getElementById("rowActionModal").classList.add("hidden");const e=D.find(t=>t.id==n);if(!e)return;const o=t=>parseInt(t||0).toLocaleString("vi-VN")+"đ",a=t=>{if(!t)return"—";if(/^\d{4}-\d{2}-\d{2}/.test(t)){const[d,s,l]=t.split("-");return`${l}/${s}/${d}`}return t},u=parseInt(e.total_price)||0,p=parseInt(e.deposit)||0,i=parseInt(e.discount)||0,r=u-p;let h=[];try{if(e.services_booked){const t=typeof e.services_booked=="string"?JSON.parse(e.services_booked):e.services_booked;Array.isArray(t)&&(h=t)}}catch{}const x=h.reduce((t,d)=>t+(parseInt(d.price)||0),0),C={"Chờ tư vấn":"Chờ lịch"}[e.status]||e.status,I={"Chờ tư vấn":"bg-purple-100 text-purple-700 border-purple-200","Chờ cọc":"bg-yellow-100 text-yellow-700 border-yellow-200","Chờ xác nhận cọc":"bg-orange-100 text-orange-700 border-orange-200","Đã cọc":"bg-blue-100 text-blue-700 border-blue-200","Đã cọc (Chờ đi)":"bg-blue-100 text-blue-700 border-blue-200","Hoàn tất phí":"bg-emerald-100 text-emerald-700 border-emerald-200","Hoàn thành":"bg-violet-100 text-violet-700 border-violet-200","Bảo lưu":"bg-gray-100 text-gray-700 border-gray-300","Đã hủy":"bg-red-50 text-red-600 border-red-200 line-through"}[e.status]||"bg-gray-100 text-gray-700 border-gray-200",m=(t,d,s="")=>`<div class="flex items-start gap-3 py-2.5 border-b border-gray-100 last:border-none">
                <span class="text-xs font-semibold text-gray-400 uppercase tracking-wide min-w-[110px] pt-0.5">${t}</span>
                <span class="text-sm font-medium text-gray-800 flex-1 ${s}">${d||"—"}</span>
            </div>`,T=(t,d,s="bg-blue-50 text-blue-600")=>`<div class="flex items-center gap-2.5 mb-3">
                <div class="w-7 h-7 rounded-lg ${s} flex items-center justify-center flex-shrink-0 text-sm">${t}</div>
                <h3 class="font-bold text-gray-800 text-sm uppercase tracking-wider">${d}</h3>
            </div>`,S=`
        <div class="space-y-4 pb-2">

            <!-- ① Thông tin Tour -->
            <div class="bg-gradient-to-br from-orange-50 to-amber-50 border border-orange-100 rounded-2xl p-4">
                ${T("🗺️","Thông tin Tour","bg-orange-100 text-orange-600")}
                <div class="grid grid-cols-2 gap-x-4">
                    ...
                    ${m("Ngày Khởi Hành",`<span class="font-bold text-gray-900">${a(e.date)}</span>`)}
                    ${m("Ngày Đăng Ký",e.created_at?a(e.created_at.split("T")[0]):"—")}
                    ${m("Trạng Thái",`<span class="inline-block px-2.5 py-1 rounded-full text-xs font-bold border ${I}">${C||"Chưa rõ"}</span>`)}
            ...
            <!-- ② Thông tin cá nhân -->
            <div class="bg-white border border-gray-200 rounded-2xl p-4">
                ${T("👤","Thông tin Cá nhân","bg-blue-50 text-blue-600")}
            ...
            <!-- ③ Thông tin Hậu cần -->
            <div class="bg-white border border-gray-200 rounded-2xl p-4">
                ${T("🎒","Thông tin Hậu cần","bg-purple-50 text-purple-600")}
            ...
            <!-- ④ Dịch vụ đi kèm -->
            <div class="bg-white border border-gray-200 rounded-2xl p-4">
                ${T("✨","Dịch Vụ Đi Kèm","bg-green-50 text-green-600")}
                ${h.length>0?h.map(t=>`
            ...
            <div class="bg-gradient-to-br from-csr-orange to-orange-600 rounded-2xl p-4 text-white shadow-lg shadow-orange-500/20">
                ${T("💰","Giá Chi Tiết","bg-white/20 text-white")}
                <div class="space-y-2 text-sm">
                    <div class="flex justify-between items-center py-1.5 border-b border-white/10">
                        ...
                        <span class="font-bold">${o(u+i)}</span>
                    </div>
                    ${i>0?`
                    <div class="flex justify-between items-center py-1.5 border-b border-white/10">
                        <span class="text-orange-50 font-medium">Giảm Giá / Coupon</span>
                        <span class="font-bold text-yellow-200">- ${o(i)}</span>
                    </div>`:""}
                    ${x>0?`
                    <div class="flex justify-between items-center py-1.5 border-b border-white/10">
                        <span class="text-orange-50 font-medium">Dịch Vụ Bổ Sung</span>
                        <span class="font-bold text-green-100">+ ${o(x)}</span>
                    </div>`:""}
                    <div class="flex justify-between items-center py-2.5 border-b border-white/20">
                        ...
                        <span class="font-black text-white text-xl drop-shadow-sm">${o(u)}</span>
                    </div>
                    <div class="flex justify-between items-center py-1.5 border-b border-white/10">
                        <div>
                        ...
                        <span class="font-bold text-green-100">${p>0?o(p):"0đ (Chưa cọc)"}</span>
                    </div>
                    <div class="flex justify-between items-center py-3 bg-white text-gray-900 rounded-xl px-4 mt-3 shadow-inner">
                        ...
                        <span class="font-black text-2xl ${r>0?"text-red-600":"text-green-600"}">
                            ${r>0?o(r):"✅ ĐÃ THU ĐỦ"}
                        </span>
                    </div>
                </div>
            ...
            ${(()=>{const t=e.phone;if(!t)return"";const d=D.filter(k=>k.phone===t&&!k.status?.includes("hủy")&&!k.status?.includes("Hủy")).sort((k,y)=>new Date(y.created_at||0)-new Date(k.created_at||0)),s=d.length,l=d.reduce((k,y)=>k+(parseInt(y.total_price)||0),0);let c,b,f;s>=5?(c="KHÁCH VIP",b="bg-purple-100 text-purple-700 border-purple-300",f="👑"):s>=2?(c="KHÁCH THÂN THIẾT",b="bg-green-100 text-green-700 border-green-300",f="🌟"):(c="KHÁCH MỚI",b="bg-gray-100 text-gray-500 border-gray-300",f="👋");const w=d.filter(k=>k.id!=n),v=w.length>0?w.slice(0,5).map((k,y)=>{const _=k.status?.includes("Hoàn")?"bg-green-100 text-green-600":k.status?.includes("cọc")||k.status?.includes("Cọc")?"bg-blue-100 text-blue-600":"bg-gray-100 text-gray-500";return`<div class="flex items-center gap-3 py-2 border-b border-gray-100 last:border-none">
                            ...
                            <span class="${_} text-[9px] px-2 py-0.5 rounded-full font-bold shrink-0">${k.status||"Mới"}</span>
                    ...
                    ${T("🏅","Hành Trình Cùng Cam Site","bg-slate-100 text-slate-600")}

                    <!-- Tier + LTV -->
                    <div class="flex items-center justify-between mb-4 gap-3">
                        <div class="flex items-center gap-2">
                            <span class="text-lg">${f}</span>
                            <span class="text-xs font-black uppercase tracking-widest px-3 py-1.5 rounded-full border ${b}">${c}</span>
                        </div>
                        <div class="text-right">
                            <div class="text-[10px] text-gray-400 uppercase font-semibold tracking-wide">Tổng đã chi (LTV)</div>
                            <div class="text-lg font-black text-gray-900">${l>0?l.toLocaleString("vi-VN")+"đ":"—"}</div>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center gap-2 mb-3 text-sm text-gray-500">
                        ...
                        <span class="font-black text-gray-800 text-base">${s}</span>
                    ...
                    ${w.length>0?`
                    <div class="mb-3">
                        <div class="text-[10px] font-bold text-gray-400 uppercase tracking-wide mb-2">Các tour trước đây (${w.length})</div>
                        <div class="bg-white rounded-xl border border-gray-100 px-3">${v}</div>
                    </div>`:v}
                ...
                <div>
                    ...
                    <div class="text-sm font-bold text-blue-800">${e.sale_name}</div>
        ...
        </div>`;document.getElementById("detailContentBlock").innerHTML=S,document.getElementById("detailModal").classList.remove("hidden"),setTimeout(()=>{document.getElementById("detailModal").classList.add("opacity-100");const t=document.getElementById("detailModalContent");t&&(t.classList.remove("scale-95","translate-y-4"),t.classList.add("scale-100","translate-y-0"))},10)},window.actionEdit=async n=>{document.getElementById("rowActionModal").classList.add("hidden");const e=D.find(t=>t.id==n);if(!e)return;document.getElementById("edit-id").value=n,document.getElementById("edit-name").value=e.name||"",document.getElementById("edit-phone").value=e.phone||"",document.getElementById("edit-medal-name").value=e.medal_name||"",document.getElementById("edit-tour").value=e.tour||"",typeof K=="function"&&K("edit-date",e.tour||"",e.date||"",e.schedule_id||null),(t=>{let d=t.dob||"";if(typeof d=="string"&&d.includes("/")){const c=d.split("/");c.length===3&&(d=`${c[2]}-${c[1].padStart(2,"0")}-${c[0].padStart(2,"0")}`)}document.getElementById("edit-dob").value=d,document.getElementById("edit-gender").value=t.gender||"Khác",document.getElementById("edit-allergy").value=t.allergy||t.medical_notes||"",document.getElementById("edit-address").value=t.address||"",document.getElementById("edit-diet").value=t.diet||t.dietary||"",document.getElementById("edit-trekking-pole").value=t.trekking_pole||"Không";const s=document.getElementById("edit-id-card");s&&(s.value=t.id_card||t.cccd||"");const l=document.getElementById("edit-pickup-point");l&&(l.value=t.pickup_point||"")})(e),document.getElementById("edit-status").value=e.status||"Chờ xác nhận cọc",document.getElementById("edit-commitments").checked=!!e.commitments,document.getElementById("edit-special").value=e.special||"";try{const t=localStorage.getItem("csr_user");if(t&&JSON.parse(t).role==="admin"){const s=document.getElementById("edit-sale-container");s&&s.classList.remove("hidden");const l=document.getElementById("edit-sale");l&&(l.value=e.sale_id||"")}}catch{}if((!e.dob||!e.address||!e.gender||e.gender==="Khác")&&(e.customer_id||e.phone))try{const t=e.customer_id||e.phone,d=await fetch(`/api/admin_customers?action=search&keyword=${encodeURIComponent(t)}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({keyword:t})}),s=await d.json();if(d.ok&&s.success){const l=s.data,c=document.getElementById("edit-dob"),b=document.getElementById("edit-address"),f=document.getElementById("edit-allergy"),w=document.getElementById("edit-diet"),v=document.getElementById("edit-gender"),k=document.getElementById("edit-medal-name");c&&!c.value&&(c.value=l.dob||""),b&&!b.value&&(b.value=l.address||""),f&&!f.value&&(f.value=l.medical_notes||""),w&&!w.value&&(w.value=l.dietary||""),v&&(v.value==="Khác"||!v.value)&&(v.value=l.gender||"Khác"),k&&!k.value&&(k.value=l.medal_name||"");const y=document.getElementById("edit-id-card");y&&!y.value&&(y.value=l.cccd||"")}}catch(t){console.error("Pull CRM error:",t)}let a=0;try{const t=typeof e.services_booked=="string"?JSON.parse(e.services_booked):e.services_booked||[];Array.isArray(t)&&t.forEach(d=>a+=parseInt(d.price)||0)}catch{}let p=(parseInt(e.total_price)||0)+(parseInt(e.discount)||0)-a;if(p===0&&e.tour){const t=N.find(d=>d.name===e.tour);t&&parseInt(t.price)>0&&(p=parseInt(t.price))}document.getElementById("edit-total").value=p,document.getElementById("edit-discount").value=e.discount||0,document.getElementById("edit-deposit").value=e.deposit||0;const i=parseInt(e.deposit_required);document.getElementById("edit-deposit-required").value=isNaN(i)?1e6:i,window.updateEditRemaining();const r=document.getElementById("servicesContainer"),h=document.getElementById("emptyServicesMsg"),x=document.getElementById("servicesTotalRow"),g=document.getElementById("presetServicesBlock"),C=document.getElementById("presetServicesList"),E=document.getElementById("servicesDivider");let I=[];try{if(e.services_booked){const t=typeof e.services_booked=="string"?JSON.parse(e.services_booked):e.services_booked;Array.isArray(t)&&(I=t)}}catch{}if(C&&g){C.innerHTML="";const t=N.find(s=>s.name===e.tour);let d=[];try{if(t?.services){const s=typeof t.services=="string"?JSON.parse(t.services):t.services;Array.isArray(s)&&(d=s)}}catch{}d.length>0?(g.classList.remove("hidden"),E&&E.classList.remove("hidden"),d.forEach(s=>{const l=I.some(b=>(b.label||b.name||"").toLowerCase()===(s.label||"").toLowerCase()),c=document.createElement("label");c.className="preset-service-card flex items-center gap-3 p-3 rounded-xl border-2 cursor-pointer transition-all "+(l?"border-green-400 bg-green-50":"border-gray-100 bg-gray-50 hover:border-orange-200"),c.innerHTML=`
                        <input type="checkbox" class="preset-service-check w-4 h-4 rounded accent-green-500 shrink-0" 
                               ...
                               ${l?"checked":""}>
                        ${s.image?`<img src="${s.image}" class="w-10 h-10 rounded-lg object-cover shrink-0">`:'<div class="w-10 h-10 rounded-lg bg-green-100 flex items-center justify-center text-green-500 text-lg shrink-0">✨</div>'}
                        <div class="flex-1 min-w-0">
                            <div class="font-bold text-sm text-gray-800">${s.label||""}</div>
                            ${s.description?`<div class="text-xs text-gray-400 truncate">${s.description}</div>`:""}
                        </div>
                        <div class="text-sm font-black text-green-600 shrink-0">+${parseInt(s.price||0).toLocaleString("vi-VN")}đ</div>
                    `,c.querySelector(".preset-service-check").addEventListener("change",b=>{c.className="preset-service-card flex items-center gap-3 p-3 rounded-xl border-2 cursor-pointer transition-all "+(b.target.checked?"border-green-400 bg-green-50":"border-gray-100 bg-gray-50 hover:border-orange-200"),window.updateServicesTotal()}),C.appendChild(c)})):(g.classList.add("hidden"),E&&E.classList.add("hidden"))}const m=document.getElementById("edit-pickup-point");if(m){const t=N.find(f=>f.name===e.tour);let d=[];try{if(t?.pickup_points){const f=typeof t.pickup_points=="string"?JSON.parse(t.pickup_points):t.pickup_points;Array.isArray(f)&&(d=f)}}catch{}m.innerHTML='<option value="">— Chưa chọn —</option>',d.forEach(f=>{const w=(f.label||f.name||f)+(f.time?` (${f.time})`:""),v=f.label||f.name||f,k=document.createElement("option");k.value=v,k.textContent=w,m.appendChild(k)});const s=document.createElement("option");s.value="__custom__",s.textContent="✏️ Khác (nhập tay)",m.appendChild(s);const l=document.getElementById("edit-pickup-custom"),c=f=>{l&&(f==="__custom__"?(l.classList.remove("hidden"),l.focus()):(l.classList.add("hidden"),l&&(l.value="")))},b=e.pickup_point||"";b&&(d.some(w=>(w.label||w.name||w)===b)?m.value=b:(m.value="__custom__",l&&(l.value=b,l.classList.remove("hidden")))),m.addEventListener("change",()=>c(m.value))}if(r){r.querySelectorAll(".service-row").forEach(l=>l.remove());const t=N.find(l=>l.name===e.tour);let d=new Set;try{const l=t?.services,c=l?typeof l=="string"?JSON.parse(l):l:[];Array.isArray(c)&&c.forEach(b=>d.add((b.label||"").toLowerCase()))}catch{}const s=I.filter(l=>!d.has((l.label||l.name||"").toLowerCase()));s.length>0?(h&&h.classList.add("hidden"),x&&x.classList.remove("hidden"),s.forEach(l=>window.addServiceRow(l.label||l.name||"",l.price||0))):h&&h.classList.remove("hidden"),window.updateServicesTotal()}const T=document.getElementById("editModal"),S=document.getElementById("editModalContent");T&&S&&(T.classList.remove("hidden"),setTimeout(()=>{T.classList.add("opacity-100"),S.classList.remove("scale-95","translate-y-4"),S.classList.add("scale-100","translate-y-0")},10))},window.actionDelete=async n=>{if(document.getElementById("rowActionModal").classList.add("hidden"),confirm("Bạn có chắc chắn muốn xóa Đơn hàng này? Thao tác không thể hoàn tác."))try{if((await fetch(`/api/bookings?id=${n}`,{method:"DELETE"})).ok)alert("✅ Đã xóa đơn hàng thành công!"),H();else throw new Error("Lỗi từ Server")}catch(e){alert("❌ Không thể xóa bảng ghi này: "+e.message)}},window.updateServicesTotal=()=>{let n=0;document.querySelectorAll(".preset-service-check:checked").forEach(a=>{n+=parseInt(a.dataset.price)||0}),document.querySelectorAll(".service-price-input").forEach(a=>{n+=parseInt(a.value)||0});const e=document.getElementById("servicesTotalDisplay");e&&(e.textContent=n.toLocaleString("vi-VN")+"đ");const o=document.getElementById("servicesTotalRow");return o&&o.classList.toggle("hidden",n===0),window.updateEditRemaining(),n},window.addServiceRow=(n="",e=0)=>{const o=document.getElementById("servicesContainer"),a=document.getElementById("emptyServicesMsg"),u=document.getElementById("servicesTotalRow");if(!o)return;a&&a.classList.add("hidden"),u&&u.classList.remove("hidden");const p=document.createElement("div");p.className="service-row flex items-center gap-2 bg-gray-50 border border-gray-100 rounded-xl px-3 py-2.5",p.innerHTML=`
        ...
        `,p.querySelector(".remove-service-btn").addEventListener("click",()=>{p.remove(),o.querySelectorAll(".service-row").length===0&&(a&&a.classList.remove("hidden"),u&&u.classList.add("hidden")),window.updateServicesTotal()}),p.querySelector(".service-price-input").addEventListener("input",window.updateServicesTotal),o.appendChild(p),window.updateServicesTotal()};const re=document.getElementById("addServiceBtn");re&&re.addEventListener("click",()=>window.addServiceRow()),window.updateEditRemaining=()=>{const n=parseInt(document.getElementById("edit-total")?.value)||0,e=parseInt(document.getElementById("edit-discount")?.value)||0,o=parseInt(document.getElementById("edit-deposit")?.value)||0;let a=0;document.querySelectorAll(".preset-service-check:checked").forEach(x=>{a+=parseInt(x.dataset.price)||0}),document.querySelectorAll(".service-price-input").forEach(x=>{a+=parseInt(x.value)||0});const u=n-e+a,p=u-o,i=document.getElementById("edit-grand-total");i&&(i.textContent=u.toLocaleString("vi-VN")+"đ");const r=document.getElementById("edit-remaining");r&&(r.textContent=p>0?p.toLocaleString("vi-VN")+"đ":"0đ");const h=document.getElementById("edit-status");if(h){const x=h.value;["Chờ tư vấn","Chờ cọc","Chờ xác nhận cọc"].includes(x)&&(o>0&&p>0?h.value="Đã cọc":o===0&&(h.value=x))}};const ce=document.getElementById("edit-total"),ue=document.getElementById("edit-deposit");ce&&ce.addEventListener("input",window.updateEditRemaining),ue&&ue.addEventListener("input",window.updateEditRemaining),document.querySelectorAll('#bookingModal button[onclick*="hidden"]').forEach(n=>{n.removeAttribute("onclick"),n.addEventListener("click",window.closeModal)});const pe=async n=>{try{const o=await(await fetch("/api/admin_customers?action=create",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({full_name:n.name,phone:n.phone,cccd:n.id_card||"",dob:n.dob||null,gender:n.gender||"Khác",medical_notes:n.allergy||"",dietary:n.diet||""})})).json();o.success&&o.csr_code&&(console.log("✅ Đã đồng bộ CRM thành công:",n.name,o.csr_code),(await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:n.id,customer_id:o.csr_code})})).ok&&(console.log("✅ Đã cập nhật Booking mã CRM"),typeof H=="function"&&H()))}catch(e){console.error("❌ Lỗi đồng bộ CRM:",e)}},ge=document.getElementById("bookingsTableBody");ge&&ge.addEventListener("click",async n=>{const e=n.target.closest(".action-btn");if(!e){const p=n.target.closest(".row-clickable");if(p){const i=p.getAttribute("data-id"),r=D.find(h=>h.id==i);r&&openRowActionModal(r)}return}const o=e.getAttribute("data-id"),a=D.find(p=>p.id==o);if(!a)return;const u=document.getElementById("rowActionModal");if(u&&u.classList.add("hidden"),e.classList.contains("confirm-deposit-btn")){if(confirm("Xác nhận khách hàng này đã chuyển khoản đặt cọc?"))try{if(e.disabled=!0,e.classList.add("opacity-50"),(await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:o,status:"Đã cọc"})})).ok)alert("✅ Đã xác nhận cọc thành công!"),pe(a),typeof H=="function"&&H();else throw new Error("Lỗi cập nhật API")}catch(p){alert("❌ Lỗi: "+p.message)}finally{e.disabled=!1,e.classList.remove("opacity-50")}return}if(e.classList.contains("delete-btn")){window.actionDelete(o);return}else if(e.classList.contains("process-btn")){const r=(window.location.hostname==="localhost"||window.location.hostname==="127.0.0.1"?"http://localhost:8888":window.location.origin)+`/booking/process.html?id=${o}`;navigator.clipboard.writeText(r).then(()=>{alert(`📋 Đã sao chép Link Điền Thông Tin!
Gửi cho khách: `+r)}).catch(h=>{alert("Lỗi khi Copy Clipboard. Link là: "+r)})}else e.classList.contains("view-btn")?(document.getElementById("rowActionModal").classList.add("hidden"),window.actionView(o)):e.classList.contains("edit-btn")&&window.actionEdit(o)});const q=document.getElementById("smartSearch"),M=document.getElementById("searchSuggestions");if(q){let n;q.addEventListener("input",e=>{const o=e.target.value.toLowerCase().trim();if(clearTimeout(n),o.length<2){M&&M.classList.add("hidden");return}n=setTimeout(async()=>{try{const a=await fetch(`/api/admin_customers?action=search&keyword=${o}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({keyword:o})}),u=await a.json();if(a.ok&&u.success){const i=[u.data];M&&(M.innerHTML=i.map(r=>`
                            <div class="suggestion-item p-3 hover:bg-orange-50 cursor-pointer border-b border-gray-100 last:border-0 transition-colors" 
                                 ...
                                 data-cccd="${r.cccd||""}"
                                 data-allergy="${r.medical_notes||""}"
                                 data-diet="${r.dietary||""}"
                                 data-trekking_pole="${r.trekking_pole||""}"
                                 data-medal-name="${r.medal_name||""}">
                                <div class="font-bold text-gray-900 text-sm">${r.full_name}</div>
                                <div class="flex justify-between items-center mt-1">
                                    <span class="text-xs text-gray-500">${r.phone}</span>
                                    ${r.csr_code?`<span class="bg-orange-100 text-csr-orange text-[10px] font-bold px-2 py-0.5 rounded">${r.csr_code}</span>`:""}
                                </div>
                            </div>
                        `).join(""),M.classList.remove("hidden"))}else M&&(M.innerHTML='<div class="p-3 text-xs text-gray-400 italic text-center">Không tìm thấy khách hàng cũ</div>',M.classList.remove("hidden"))}catch(a){console.error("Search error:",a)}},300)}),M&&M.addEventListener("click",e=>{const o=e.target.closest(".suggestion-item");if(o){const a=document.getElementById("addFullName"),u=document.getElementById("addPhone"),p=document.getElementById("addCsrCode");a&&(a.value=o.getAttribute("data-name")),u&&(u.value=o.getAttribute("data-phone")),p&&(p.value=o.getAttribute("data-csr"));const i=document.getElementById("addDob"),r=document.getElementById("addGender"),h=document.getElementById("addIdCard"),x=document.getElementById("addAddress");i&&(i.value=o.getAttribute("data-dob")||""),r&&(r.value=o.getAttribute("data-gender")||"Khác"),h&&(h.value=o.getAttribute("data-cccd")||""),x&&(x.value=o.getAttribute("data-address")||"");const g=document.getElementById("addAllergy"),C=document.getElementById("addDiet"),E=document.getElementById("addTrekkingPole"),I=document.getElementById("addMedalName");g&&(g.value=o.getAttribute("data-allergy")||""),C&&(C.value=o.getAttribute("data-diet")||""),E&&(E.value=o.getAttribute("data-trekking_pole")||"Không"),I&&(I.value=o.getAttribute("data-medal-name")||""),window._selectedCustomer={fullName:o.getAttribute("data-name"),phone:o.getAttribute("data-phone"),csrCode:o.getAttribute("data-csr"),dob:o.getAttribute("data-dob")||"",gender:o.getAttribute("data-gender")||"",address:o.getAttribute("data-address")||"",id_card:o.getAttribute("data-cccd")||"",allergy:o.getAttribute("data-allergy")||"",diet:o.getAttribute("data-diet")||"",trekking_pole:o.getAttribute("data-trekking_pole")||"",medal_name:o.getAttribute("data-medal-name")||""},q.value=o.getAttribute("data-name"),M.classList.add("hidden");const m=`
                    ...
                    </div>`,T=document.getElementById("loyalty-alert");T&&T.remove(),q.parentElement.insertAdjacentHTML("afterend",m)}}),document.addEventListener("click",e=>{!q.contains(e.target)&&M&&!M.contains(e.target)&&M.classList.add("hidden")})}const z=document.getElementById("addTourPrice"),G=document.getElementById("addDiscount"),me=document.getElementById("addTotalPriceDisplay");document.getElementById("addTourName");const he=()=>{if(!z||!G||!me)return;const n=parseInt(z.value)||0,e=parseInt(G.value)||0,o=n-e;me.textContent=(o>0?o:0).toLocaleString("vi-VN")+"đ"};z&&z.addEventListener("input",he),G&&G.addEventListener("input",he);const be=document.getElementById("searchBtn");be&&be.addEventListener("click",n=>{n.preventDefault(),document.getElementById("smartSearch").value||alert("Vui lòng nhập Số Điện Thoại hoặc Mã #CSR")});const W=document.getElementById("bookingForm");W&&W.addEventListener("submit",async n=>{n.preventDefault();const e=n.target.querySelector('button[type="submit"]'),o=e.textContent;e.textContent="Đang lưu hệ thống...",e.disabled=!0;try{const a=document.getElementById("addFullName").value,u=document.getElementById("addPhone").value,p=document.getElementById("addTourName").value,i=document.getElementById("addTourDate").value;let r=null,h=i;if(i&&i.includes("::")){const L=i.split("::");r=parseInt(L[0])||null,h=L[1]}const x=document.getElementById("addDob").value,g=document.getElementById("addGender").value,C=document.getElementById("addIdCard").value,E=document.getElementById("addAddress").value,I=document.getElementById("addSpecial").value,m=document.getElementById("addAllergy").value,T=document.getElementById("addDiet").value,S=document.getElementById("addTrekkingPole").value,t=document.getElementById("addMedalName").value,d=parseInt(document.getElementById("addTourPrice").value)||0,s=parseInt(document.getElementById("addDiscount").value)||0,l=parseInt(document.getElementById("addDepositRequired").value),c=isNaN(l)?1e6:l,b=document.getElementById("addCsrCode").value||"",f=localStorage.getItem("csr_user");let w=null,v="Admin";if(f){const L=JSON.parse(f);w=L.id,v=L.fullName||L.full_name||"Admin"}const k=document.getElementById("editingBookingId")?document.getElementById("editingBookingId").value:"",y={name:a,phone:u,tour:p,date:h,status:"Chờ cọc",total_price:d-s,discount:s,deposit:0,sale_id:w,sale_name:v,customer_id:b,deposit_required:c,commitments:!0,schedule_id:r,admin_create:!0,dob:x,gender:g,address:E,id_card:C,special:I,allergy:m,diet:T,trekking_pole:S,medal_name:t},_=u.replace(/[^0-9]/g,"");window._selectedCustomer&&(window._selectedCustomer.phone||"").replace(/[^0-9]/g,"")===_&&(y.dob=y.dob||window._selectedCustomer.dob,y.gender=y.gender&&y.gender!=="Khác"?y.gender:window._selectedCustomer.gender||"Khác",y.address=y.address||window._selectedCustomer.address,y.id_card=y.id_card||window._selectedCustomer.id_card,y.allergy=window._selectedCustomer.allergy,y.diet=window._selectedCustomer.diet,y.trekking_pole=window._selectedCustomer.trekking_pole,y.medal_name=window._selectedCustomer.medal_name),k&&(y.id=k,delete y.status,delete y.deposit,delete y.sale_id,delete y.sale_name);const j=await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(y)});if(j.ok){alert(k?"✅ Đã lưu Cập nhật Đơn hàng!":"✅ Thêm Đơn Hàng Thành Công!"),window.closeModal(),W&&W.reset(),window._selectedCustomer=null;const L=document.getElementById("loyalty-alert");L&&L.remove(),document.getElementById("addTotalPriceDisplay")&&(document.getElementById("addTotalPriceDisplay").textContent="0đ"),typeof H=="function"?H():window.location.reload()}else{const L=await j.json();throw new Error(L.error||"Lỗi tạo Booking!")}}catch(a){alert("❌ "+a.message)}finally{e.textContent=o,e.disabled=!1}});const fe=document.getElementById("editForm");fe&&fe.addEventListener("submit",async n=>{n.preventDefault();const e=n.target.querySelector('button[type="submit"]'),o=e.textContent;e.textContent="Đang lưu cập nhật...",e.disabled=!0;try{const a=document.getElementById("edit-id").value,u=document.getElementById("edit-name").value,p=document.getElementById("edit-phone").value,i=document.getElementById("edit-medal-name").value,r=document.getElementById("edit-tour").value,h=document.getElementById("edit-date").value;let x=h,g;if(h&&h.includes("::")){const $=h.split("::");g=parseInt($[0])||null,x=$[1]}const C=document.getElementById("edit-dob").value,E=document.getElementById("edit-gender").value,I=document.getElementById("edit-status").value,m=document.getElementById("edit-allergy").value,T=document.getElementById("edit-address").value,S=document.getElementById("edit-diet").value,t=document.getElementById("edit-trekking-pole").value,d=document.getElementById("edit-commitments").checked,s=document.getElementById("edit-special").value,l=document.getElementById("edit-id-card"),c=l?l.value:"",b=document.getElementById("edit-pickup-point"),f=document.getElementById("edit-pickup-custom"),w=b?.value==="__custom__"?f?.value?.trim()||"":b?.value||"",v=[];document.querySelectorAll(".preset-service-check:checked").forEach($=>{const P=$.dataset.label,ee=parseInt($.dataset.price)||0;P&&v.push({label:P,price:ee})}),document.querySelectorAll(".service-row").forEach($=>{const P=($.querySelector(".service-label-input")?.value||"").trim(),ee=parseInt($.querySelector(".service-price-input")?.value)||0;P&&v.push({label:P,price:ee})});const y=v.length>0?JSON.stringify(v):null,_=document.getElementById("edit-sale");let j,L;if(_&&_.parentElement&&!_.parentElement.classList.contains("hidden")&&(j=_.value||null,L="Website",j)){const $=J.find(P=>String(P.id)===String(j));$&&(L=$.full_name||$.fullName)}const Be=parseInt(document.getElementById("edit-total").value)||0,ve=parseInt(document.getElementById("edit-discount").value)||0,ye=parseInt(document.getElementById("edit-deposit").value)||0,we=parseInt(document.getElementById("edit-deposit-required").value),Se=isNaN(we)?1e6:we;let ke=0;v.forEach($=>ke+=$.price);const Le=Be-ve+ke,F={id:a,name:u,phone:p,medal_name:i,tour:r,date:x,dob:C,gender:E,status:I,allergy:m,address:T,diet:S,trekking_pole:t,commitments:d,special:s,id_card:c,pickup_point:w,services_booked:y,sale_id:j,sale_name:L,total_price:Le,discount:ve,deposit:ye,deposit_required:Se,...g!==void 0?{schedule_id:g}:{}},Ce=await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(F)});if(Ce.ok)alert("✅ Đã cập nhật thành công Chi tiết Đơn hàng!"),(I.includes("Đã cọc")||F.total_price>0&&F.total_price===ye)&&pe(F),window.closeEditModal(),typeof H=="function"&&H();else{const $=await Ce.json();throw new Error($.error||"Gặp sự cố khi Cập nhật API.")}}catch(a){alert("❌ Lỗi Cập nhật: "+a.message)}finally{e.textContent=o,e.disabled=!1}}),window.openWaitingModal=()=>{document.getElementById("waitingModal").classList.remove("hidden"),setTimeout(()=>{document.getElementById("waitingModal").classList.add("opacity-100");const n=document.getElementById("waitingModalContent");n&&(n.classList.remove("scale-95"),n.classList.add("scale-100"))},10)},window.closeWaitingModal=()=>{document.getElementById("waitingModal").classList.remove("opacity-100");const n=document.getElementById("waitingModalContent");n&&(n.classList.remove("scale-100"),n.classList.add("scale-95")),setTimeout(()=>{document.getElementById("waitingModal").classList.add("hidden")},300)};const xe=document.getElementById("addWaitingBtn");xe&&xe.addEventListener("click",()=>window.openWaitingModal());const Z=document.getElementById("waitingForm");Z&&Z.addEventListener("submit",async n=>{n.preventDefault();const e=document.getElementById("wl-submit");e.innerHTML="Đang lưu...",e.disabled=!0;const o=document.getElementById("wl-zalo").value?`Tên Zalo / Ghi Chú: ${document.getElementById("wl-zalo").value}`:"";let a={id:"null",name:"Website"};try{const i=localStorage.getItem("csr_user");if(i){const r=JSON.parse(i);a={id:r.id,name:r.full_name||r.fullName}}}catch{}const u={name:document.getElementById("wl-name").value,phone:document.getElementById("wl-phone").value,tour:document.getElementById("wl-tour").value,date:"Mở (Chưa có ngày)",status:"Chờ tư vấn",special:o,sale_id:a.id,sale_name:a.name},p=document.getElementById("wl-date");if(p&&p.value){const i=p.value.split("::");i.length===2&&i[0]&&i[0]!=="undefined"&&(u.schedule_id=i[0],u.date=i[1])}try{const r=await(await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(u)})).json();r.success?(window.closeWaitingModal(),Z.reset(),typeof H=="function"?H():(alert("✅ Đã thêm vào danh sách chờ!"),window.location.reload())):alert("Lỗi lưu danh sách: "+r.message)}catch(i){console.error(i),alert("Có lỗi xảy ra, vui lòng thử lại.")}finally{e.innerHTML="Lưu Vào Danh Sách",e.disabled=!1}})};export{Ne as afterRender,De as render};
