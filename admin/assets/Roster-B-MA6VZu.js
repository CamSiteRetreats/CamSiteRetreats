import{S as G,H as z}from"./Header-CvqOcIss.js";const K=()=>`
    ...
    `,Y=async()=>{const g=[{id:"col-stt",label:"STT",always:!0,render:(e,t)=>t+1},{id:"col-name",label:"Họ và Tên",default:!0,render:e=>`<div class="font-bold text-gray-900">${e.name}</div>`},{id:"col-dob",label:"Ngày Sinh",default:!0,render:e=>{if(!e.dob)return"-";const t=e.dob.split("-");return t.length===3?`${t[2]}/${t[1]}/${t[0]}`:e.dob}},{id:"col-phone",label:"SĐT",default:!0,render:e=>`<div class="font-medium text-gray-700">${e.phone||"-"}</div>`},{id:"col-gender",label:"Giới tính",default:!0,render:e=>e.gender||"-"},{id:"col-cccd",label:"CCCD / Passport",default:!0,render:e=>e.id_card||e.cccd||"-"},{id:"col-address",label:"Địa chỉ",default:!0,render:e=>`<div class="max-w-[200px] truncate print:max-w-none print:whitespace-normal" title="${e.address||""}">${e.address||"-"}</div>`},{id:"col-diet",label:"Ăn chay",default:!1,render:e=>e.diet==="Chay"||e.diet==="Có"?'<span class="text-green-600 font-bold">Có</span>':"Không"},{id:"col-pole",label:"Gậy Trekking",default:!1,render:e=>e.trekking_pole==="Có"?'<span class="text-orange-600 font-bold">Có</span>':"Không"},{id:"col-allergy",label:"Dị ứng",default:!1,render:e=>e.medical_notes||e.allergy||"-"},{id:"col-special",label:"Yêu cầu ĐB",default:!1,render:e=>e.special_notes||e.special||"-"},{id:"col-medal",label:"Tên Huy Chương",default:!1,render:e=>e.medal_name||e.name||"-"}],T=new URLSearchParams(window.location.search),i=T.get("tour")||"",p=T.get("date")||"";let h=p;const D=e=>{if(!e)return"";const t=e.split(/[-/.]/);if(t.length!==3)return e;let n,r,o;return t[0].length===4?[o,r,n]=t:[n,r,o]=t,`${n.toString().padStart(2,"0")}/${r.toString().padStart(2,"0")}/${o}`},S=e=>(e||"").toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g,"").replace(/[-_]/g," ").replace(/\s+/g," ").trim();p&&(h=D(p));const H=document.getElementById("roster-title"),M=document.getElementById("roster-subtitle"),B=document.getElementById("print-tour-name"),j=document.getElementById("print-tour-date"),I=document.getElementById("print-total"),b=document.getElementById("filterStatus"),v=document.getElementById("filterGender"),y=document.getElementById("rosterList");let m=[],u=new Set(g.filter(e=>e.default||e.always).map(e=>e.id));i&&(H.textContent=i,B.textContent=i),h&&(M.innerHTML=`Ngày khởi hành: <span class="font-bold text-csr-orange">${h}</span>`,j.textContent=h);const N=()=>{const e=document.getElementById("column-toggles");e.innerHTML=g.filter(t=>!t.always).map(t=>`
        ...
        `).join(""),document.querySelectorAll(".col-toggle-cb").forEach(t=>{t.addEventListener("change",n=>{n.target.checked?u.add(n.target.value):u.delete(n.target.value),R()})})},A=()=>{const e=document.getElementById("table-header-row");e.innerHTML=g.map(t=>`
            <th class="p-4 whitespace-nowrap ${t.id}" data-col="${t.id}">${t.label}</th>
        `).join("")},R=()=>{g.forEach(e=>{const t=u.has(e.id),n=document.querySelector(`th[data-col="${e.id}"]`);n&&(t?n.classList.remove("col-hidden"):n.classList.add("col-hidden")),document.querySelectorAll(`td[data-col="${e.id}"]`).forEach(r=>{t?r.classList.remove("col-hidden"):r.classList.add("col-hidden")})})},E=e=>{if(!e)return"";e.includes("T")&&(e=e.split("T")[0]);const t=e.split(/[-/.]/);if(t.length!==3)return e;let n,r,o;return t[0].length===4?[o,r,n]=t:[n,r,o]=t,`${o}-${r.toString().padStart(2,"0")}-${n.toString().padStart(2,"0")}`},L=E(p),F=async()=>{try{const t=await(await fetch("/api/bookings")).json();console.log("[Roster] targetTour:",i,"targetDateStr:",p,"targetYMD:",L),console.log("[Roster] Total bookings:",t.length),t.length>0&&console.log("[Roster] Sample booking:",{tour:t[0].tour,date:t[0].date,status:t[0].status}),m=t.filter(n=>{let r=!0,o=!0;if(i){const s=S(n.tour),d=S(i);r=s===d||s.includes(d)||d.includes(s)}return p&&(o=E(n.date)===L),r&&o}),console.log("[Roster] Matched bookings:",m.length),m.sort((n,r)=>{const o=(n.status||"").includes("cọc")||n.status==="Hoàn tất"?1:0;return((r.status||"").includes("cọc")||r.status==="Hoàn tất"?1:0)-o}),w()}catch(e){console.error("Lỗi khi tải dữ liệu đoàn:",e),y.innerHTML='<tr><td colspan="12" class="p-8 text-center text-red-500 font-bold">Lỗi khi tải dữ liệu. Vui lòng tải lại trang.</td></tr>'}},w=()=>{const e=b.value.toLowerCase(),t=v.value.toLowerCase(),n=m.filter(r=>{const o=(r.status||"").toLowerCase(),s=e===""||e==="chờ cọc"&&(o.includes("chờ cọc")||o.includes("chờ xác nhận"))||e==="đã cọc"&&(o.includes("đã cọc")||o.includes("hoàn tất")||o.includes("hoàn thành"))||e==="đã hủy"&&o.includes("hủy"),d=(r.gender||"").toLowerCase();return s&&(t===""||d===t)});if(document.getElementById("text-visible-count").textContent=n.length,I.textContent=n.length,n.length===0){y.innerHTML='<tr><td colspan="12" class="p-12 text-center text-gray-500 bg-gray-50">Không tìm thấy khách hàng nào khớp với điều kiện lọc.</td></tr>';return}y.innerHTML=n.map((r,o)=>'<tr class="hover:bg-gray-50/50 transition-colors">'+g.map(s=>`
                    <td class="p-4 align-top ${u.has(s.id)?"":"col-hidden"}" data-col="${s.id}">
                        ${s.render(r,o)}
                    </td>
                `).join("")+"</tr>").join("")},P=()=>{const e=g.filter(a=>u.has(a.id));let t="\\uFEFF";t+=e.map(a=>`"${a.label}"`).join(",")+`
`;const n=b.value.toLowerCase(),r=v.value.toLowerCase();m.filter(a=>{const x=n===""||n==="chờ cọc"&&(a.status==="Chờ cọc"||a.status==="Chờ xác nhận cọc")||n==="đã cọc"&&(a.status==="Đã cọc"||a.status==="Hoàn tất"||a.status==="Hoàn thành")||n==="đã hủy"&&a.status==="Đã hủy",k=r===""||a.gender&&a.gender.toLowerCase()===r;return x&&k}).forEach((a,x)=>{const k=e.map(c=>{let l="";if(c.id==="col-stt")l=x+1;else if(c.id==="col-name")l=a.name||"";else if(c.id==="col-address")l=a.address||"";else if(c.id==="col-phone")l=a.phone?`'${a.phone}`:"";else if(c.id==="col-diet")l=a.diet==="Chay"||a.diet==="Có"?"Có":"Không";else if(c.id==="col-pole")l=a.trekking_pole==="Có"?"Có":"Không";else{const _=String(c.render(a,x)),C=document.createElement("DIV");C.innerHTML=_,l=C.textContent||C.innerText||""}return l=String(l).replace(/"/g,'""'),`"${l}"`});t+=k.join(",")+`
`});const s=new Blob([t],{type:"text/csv;charset=utf-8;"}),d=URL.createObjectURL(s),$=`DS_Doan_${i||"Tour"}_${h||"Date"}.csv`.replace(/[/\\?%*:|"<>]/g,"-"),f=document.createElement("a");f.setAttribute("href",d),f.setAttribute("download",$),document.body.appendChild(f),f.click(),document.body.removeChild(f)};b.addEventListener("change",w),v.addEventListener("change",w),document.getElementById("btnExportPDF").addEventListener("click",()=>{window.print()}),document.getElementById("btnExportExcel").addEventListener("click",()=>{P()}),N(),A(),F()};export{Y as afterRender,K as render};
