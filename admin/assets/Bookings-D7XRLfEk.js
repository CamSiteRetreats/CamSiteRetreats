import{S as xe,H as ye}from"./Header-Dmidaim7.js";console.log("Bookings V3 - Cache Busted!");const ke=()=>`
    ...
    `,Ce=()=>{let _=[],C="consult",D=[],Y=[],q=[];const me=async()=>{try{const[n,e,a]=await Promise.all([fetch("/api/tours"),fetch("/api/schedules"),fetch("/api/users")]);D=n.ok?await n.json():[],Y=e.ok?await e.json():[],q=a.ok?await a.json():[],W("addTourName"),W("edit-tour"),W("wl-tour"),he("edit-sale")}catch(n){console.error("Lỗi tải tours/schedules:",n)}},he=n=>{const e=document.getElementById(n);e&&(e.innerHTML='<option value="">-- Website --</option>',q.forEach(a=>{const l=document.createElement("option");l.value=a.id,l.textContent=`${a.full_name||a.fullName} (${a.role})`,e.appendChild(l)}))},W=n=>{const e=document.getElementById(n);if(!e)return;const a=e.value,l=e.options[0].outerHTML;e.innerHTML=l,D.filter(r=>r.is_visible!==!1).forEach(r=>{const c=document.createElement("option");c.value=r.name,c.textContent=r.name,r.name===a&&(c.selected=!0),e.appendChild(c)})},j=(n,e,a,l)=>{const r=document.getElementById(n);if(!r)return;if(r.innerHTML='<option value="">-- Chọn Lịch --</option>',!e){r.innerHTML='<option value="">-- Chọn Tour trước --</option>';return}const c=Y.filter(o=>o.tour_name===e||e.includes(o.tour_name)||o.tour_name.includes(e));if(c.length===0){r.innerHTML='<option value="">Chưa có lịch cho tour này</option>';return}c.sort((o,s)=>new Date(o.start_date)-new Date(s.start_date)).forEach(o=>{const s=parseInt(o.booked_count)||0,i=(o.slots||0)-s,m=new Date(o.start_date),d=o.end_date?new Date(o.end_date):null,b=x=>`${x.getDate().toString().padStart(2,"0")}/${(x.getMonth()+1).toString().padStart(2,"0")}`;let y=b(m);d&&(y+=" - "+b(d)),o.group_label&&(y+=` · ${o.group_label}`),y+=` (${i>=0?i:0} chỗ trống)`;const f=document.createElement("option"),p=`${m.getDate().toString().padStart(2,"0")}/${(m.getMonth()+1).toString().padStart(2,"0")}/${m.getFullYear()}`;f.value=`${o.id}::${p}`,f.dataset.scheduleId=o.id,f.dataset.date=p,f.textContent=y,i<=0&&(f.disabled=!0,f.textContent=y.replace("chỗ trống","HẾT CHỖ")),(l&&String(o.id)===String(l)||!l&&a&&p===a)&&(f.selected=!0),r.appendChild(f)})},Z=document.getElementById("addTourName");Z&&Z.addEventListener("change",n=>{const e=n.target.value;j("addTourDate",e);const a=D.find(r=>r.name===e),l=document.getElementById("addTourPrice");if(a&&l){const r=parseInt(a.price)||0;l.value=r;const c=document.getElementById("addDiscount"),o=document.getElementById("addTotalPriceDisplay"),s=parseInt(c?c.value:0)||0,i=r-s;o&&(o.textContent=(i>0?i:0).toLocaleString("vi-VN")+"đ")}});const Q=document.getElementById("edit-tour");Q&&Q.addEventListener("change",n=>{const e=n.target.value;j("edit-date",e);const a=D.find(r=>r.name===e),l=document.getElementById("edit-total");a&&l&&(parseInt(l.value)===0||!l.value)&&(l.value=parseInt(a.price)||0,typeof window.updateEditRemaining=="function"&&window.updateEditRemaining())});const ee=document.getElementById("wl-tour");ee&&ee.addEventListener("change",n=>{const e=n.target.value;j("wl-date",e)}),me();const A=()=>{const n=document.getElementById("bookingsTableHead"),e=document.getElementById("bookingsTableBody");if(!e||!n)return;const a=C==="upcoming"||C==="ready"||C==="completed";a?n.innerHTML=`
            ...
            `;const l=document.getElementById("filterSearch")?document.getElementById("filterSearch").value.toLowerCase():"",r=document.getElementById("filterTour")?document.getElementById("filterTour").value:"",c=document.getElementById("filterDate")?document.getElementById("filterDate").value:"",o=document.getElementById("filterSale")?document.getElementById("filterSale").value:"",s=document.getElementById("filterStatus")?document.getElementById("filterStatus").value:"";let i=_.filter(t=>{let u=!1;const g=parseInt(t.total_price)>0&&parseInt(t.total_price)===parseInt(t.deposit),v=T=>{if(!T)return!1;try{const I=T.split("/");if(I.length===3){const L=new Date(`${I[2]}-${I[1]}-${I[0]}T00:00:00`),E=new Date;return E.setHours(0,0,0,0),L.getTime()<E.getTime()}else if(T.includes("-")){const L=new Date(`${T}T00:00:00`),E=new Date;return E.setHours(0,0,0,0),L.getTime()<E.getTime()}}catch{}return!1},h=(t.status==="Hoàn thành"||t.status==="Đã đi"||g)&&v(t.date);return C==="consult"?u=t.status==="Chờ tư vấn"&&!h:C==="pending"?u=(!t.status||t.status==="Chờ cọc"||t.status==="Chờ xác nhận cọc")&&!h:C==="upcoming"?u=t.status&&t.status!=="Chờ tư vấn"&&t.status!=="Chờ cọc"&&t.status!=="Chờ xác nhận cọc"&&!g&&t.status!=="Hoàn thành"&&!h:C==="ready"?u=(g||t.status==="Hoàn thành")&&!h:C==="completed"&&(u=h),!(!u||r&&t.tour!==r||c&&F(t.date)!==c||o&&o!=="null"&&t.sale_name!==o||o==="null"&&t.sale_name&&t.sale_name!=="Website"||l&&!`${t.name||""} ${t.phone||""} ${t.customer_id||""} ${t.id||""} csr${t.id||""} #csr${t.id||""}`.toLowerCase().includes(l)||s&&t.status!==s)});const m=document.getElementById("statTitle1"),d=document.getElementById("statTitle2"),b=document.getElementById("statTitle3"),y=document.getElementById("statTotalCustomers"),f=document.getElementById("statTotalRevenue"),p=document.getElementById("statTotalCollected");let x=i.length;if(C==="consult"){const t=document.getElementById("addWaitingBtn"),u=document.getElementById("addBookingBtn");t&&t.classList.remove("hidden"),u&&u.classList.add("hidden"),m&&(m.textContent="Tổng Đăng Ký Chờ Lịch"),d&&(d.textContent="Chưa Được Liên Hệ"),b&&(b.textContent="Đã Có Sale Phụ Trách");let g=0,v=0;i.forEach(h=>{!h.sale_id||h.sale_id==="null"||!h.sale_name||h.sale_name==="Website"||h.sale_name==="null"?g++:v++}),y&&(y.textContent=x),f&&(f.textContent=g+" Người"),p&&(p.textContent=v+" Người")}else if(C==="pending"){const t=document.getElementById("addWaitingBtn"),u=document.getElementById("addBookingBtn");t&&t.classList.add("hidden"),u&&u.classList.remove("hidden"),m&&(m.textContent="Tổng Khách (Chờ Cọc)"),d&&(d.textContent="Khách Chưa Tư Vấn (Mới)"),b&&(b.textContent="Đã Có Sale Giữ Chỗ");let g=0,v=0;i.forEach(h=>{!h.sale_id||h.sale_id==="null"||!h.sale_name||h.sale_name==="Website"||h.sale_name==="null"?g++:v++}),y&&(y.textContent=x),f&&(f.textContent=g+" Đơn"),p&&(p.textContent=v+" Đơn")}else if(C==="upcoming"){m&&(m.textContent="Tổng Số Khách"),d&&(d.textContent="Chưa Xác Nhận Cọc"),b&&(b.textContent="Chờ Thanh Toán Còn Lại");let t=0,u=0;i.forEach(g=>{g.status==="Chờ xác nhận cọc"?t++:parseInt(g.deposit)>0&&parseInt(g.total_price)>parseInt(g.deposit)&&u++}),y&&(y.textContent=x),f&&(f.textContent=t+" Khách"),p&&(p.textContent=u+" Khách")}else if(C==="ready"){m&&(m.textContent="Khách Sẵn Sàng (Full)"),d&&(d.textContent="Tổng Doanh Thu Tab"),b&&(b.textContent="Thực Thu (Full Tận Nơi)");let t=0,u=0;i.forEach(g=>{t+=parseInt(g.total_price)||0,u+=parseInt(g.deposit)||0}),y&&(y.textContent=x),f&&(f.textContent=t>0?t.toLocaleString("vi-VN")+"đ":"0đ"),p&&(p.textContent=u>0?u.toLocaleString("vi-VN")+"đ":"0đ")}else{m&&(m.textContent="Khách Đã Xong (Lịch Sử)"),d&&(d.textContent="Tổng Doanh Thu Tab"),b&&(b.textContent="Thực Thu (Đã Đóng)");let t=0,u=0;i.forEach(g=>{t+=parseInt(g.total_price)||0,u+=parseInt(g.deposit)||0}),y&&(y.textContent=x),f&&(f.textContent=t>0?t.toLocaleString("vi-VN")+"đ":"0đ"),p&&(p.textContent=u>0?u.toLocaleString("vi-VN")+"đ":"0đ")}const $=a?8:4;if(!i||i.length===0){let t=C==="consult"?"Chưa có khách hàng tư vấn nào.":C==="pending"?"Chưa có Khách chờ cọc.":C==="upcoming"?"Chưa có Khách nào Đã Cọc.":C==="ready"?"Chưa có khách sẵn sàng.":"Danh sách rỗng.";e.innerHTML=`<tr><td colspan="${$}" class="p-8 text-center text-gray-500">${t}</td></tr>`;return}e.innerHTML=i.map(t=>{const u=C==="upcoming"||C==="ready"?"bg-green-50/20 hover:bg-green-50 transition-colors":"hover:bg-gray-100 transition-colors";if(a){let g=parseInt(t.total_price)||0;if(g===0&&t.tour&&D&&D.length>0){const w=D.find(H=>H.name&&t.tour&&H.name.toLowerCase().trim()===t.tour.toLowerCase().trim());w&&parseInt(w.price)>0&&(g=parseInt(w.price))}const v=parseInt(t.deposit)||0,h=g-v,T=g>0?g.toLocaleString("vi-VN")+"đ":"Chưa định giá",I=v>0?v.toLocaleString("vi-VN")+"đ":"0đ",L=h>0?h.toLocaleString("vi-VN")+"đ":h===0&&g>0?"Đã thu trọn":"-";let E="";t.status==="Hoàn thành"||t.status==="Đã đi"?E='<span class="bg-gray-100 text-gray-600 border border-gray-200 px-2 py-0.5 rounded text-xs block w-full text-center">Hoàn thành</span>':t.status==="Chờ xác nhận cọc"?E=`<button class="action-btn confirm-deposit-btn bg-csr-orange text-white px-3 py-1.5 rounded text-xs font-bold shadow-sm hover:bg-[#d65503] w-full" data-id="${t.id}">Xác nhận cọc</button>`:v>0&&h===0?E='<span class="bg-green-100 text-green-700 border border-green-200 px-2 py-0.5 rounded text-xs font-medium block w-full text-center">Hoàn tất phí</span>':E='<span class="bg-blue-100 text-blue-700 border border-blue-200 px-2 py-0.5 rounded text-xs font-medium block w-full text-center">Đã Cọc</span>';let k="";return v>0&&(k=`<a href="/invoice.html?id=${t.id}" target="_blank" class="mt-1.5 block text-center text-[11px] text-blue-600 hover:text-blue-800 hover:underline font-medium" onclick="event.stopPropagation()">🧾 Xem hóa đơn</a>`),`
                ...
                `}else{const g=t.name?t.name.substring(0,2).toUpperCase():"KH",v=C==="consult",h=!t.sale_id||t.sale_id==="null"||!t.sale_name||t.sale_name==="null";let T="";v&&h?T=`<button class="action-btn claim-btn bg-csr-orange hover:bg-[#d65503] text-white px-3 py-1.5 rounded text-xs font-bold shadow-sm transition-colors" data-id="${t.id}">Nhận khách</button>`:T=`<span class="text-sm text-gray-500">${t.sale_name||"Website"}</span>`;const I=`
                ...
                <tr class="${u} cursor-pointer row-clickable block md:table-row p-4 md:p-0 mb-4 md:mb-0 glass-panel md:glass-none border-l-4 border-csr-orange md:border-none relative" data-id="${t.id}">
                    ...
                    <td class="py-3 md:p-4 align-top block md:table-cell border-t md:border-none border-gray-100 mt-3 md:mt-0 pt-3 md:pt-4">
                        ...
                        ${v&&t.special?`<div class="text-xs text-blue-500 mt-1 italic bg-blue-50 md:bg-transparent p-2 md:p-0 rounded">💬 ${t.special}</div>`:""}
                    </td>
    ...
    `}}).join(""),document.querySelectorAll(".claim-btn").forEach(t=>{t.addEventListener("click",async u=>{u.stopPropagation();const g=t.getAttribute("data-id"),v=localStorage.getItem("csr_user");let h=null,T="Admin";if(v){const I=JSON.parse(v);h=I.id,T=I.fullName||I.full_name||"Admin"}t.textContent="Đang xử lý...",t.disabled=!0;try{if((await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:g,sale_id:h,sale_name:T})})).ok){const L=_.find(E=>E.id==g);L&&(L.sale_id=h,L.sale_name=T),A()}else alert("Lỗi khi nhận khách. Vui lòng thử lại."),t.textContent="Nhận khách",t.disabled=!1}catch(I){console.error("Lỗi nhận khách:",I),t.textContent="Nhận khách",t.disabled=!1}})})},be=()=>{const n=new Set,e=new Set,a=new Set;_.forEach(o=>{if(o.tour&&n.add(o.tour),o.date){const s=F(o.date);s&&e.add(s)}o.sale_name&&o.sale_name!=="Website"&&o.sale_name!=="null"&&a.add(o.sale_name.trim())});const l=document.getElementById("filterTour"),r=document.getElementById("filterDate"),c=document.getElementById("filterSale");if(l){const o=l.value;l.innerHTML='<option value="">Tất cả Tour</option>',[...n].sort().forEach(s=>l.innerHTML+=`<option value="${s}">${s}</option>`),l.value=o}if(r){const o=r.value;r.innerHTML='<option value="">Tất cả Ngày</option>',[...e].sort().forEach(s=>r.innerHTML+=`<option value="${s}">${s}</option>`),r.value=o}if(c){const o=c.value;c.innerHTML='<option value="">Tất cả Sale</option>',c.innerHTML+='<option value="null">Nguồn Website</option>',[...a].sort().forEach(s=>{c.innerHTML+=`<option value="${s}">${s}</option>`}),c.value=o}};function F(n){if(!n)return"";if(/^\d{1,2}\/\d{1,2}\s*-\s*\d{1,2}\/\d{1,2}$/.test(n))return n.replace(/\s+/g,"");try{if(/^\d{4}-\d{2}-\d{2}$/.test(n)){const[e,a,l]=n.split("-"),r=`${parseInt(l)}/${parseInt(a)}`;return`${r}-${r}`}if(/^\d{1,2}\/\d{1,2}\/\d{4}$/.test(n)){const[e,a,l]=n.split("/"),r=`${parseInt(e)}/${parseInt(a)}`;return`${r}-${r}`}if(n.includes("-"))return n.split("-").map(a=>{const l=a.trim();if(/\/\d{4}$/.test(l)){const[r,c]=l.split("/");return`${parseInt(r)}/${parseInt(c)}`}return l}).join("-")}catch{console.warn("Date normalization failed for:",n)}return n}const M=async()=>{try{_=await(await fetch("/api/bookings")).json(),_.sort((e,a)=>parseInt(a.id)-parseInt(e.id)),be(),A()}catch(n){console.error("Lỗi parse list đơn hàng:",n);const e=document.getElementById("bookingsTableBody");e&&(e.innerHTML='<tr><td colspan="4" class="p-8 text-center text-red-500">Lỗi lấy dữ liệu từ Server.</td></tr>')}};M(),["filterSearch","filterTour","filterDate","filterSale","filterStatus"].forEach(n=>{const e=document.getElementById(n);e&&(n==="filterSearch"?e.addEventListener("input",A):e.addEventListener("change",A))});const te=document.getElementById("exportExcelBtn");te&&te.addEventListener("click",()=>{const n=document.getElementById("filterSearch")?document.getElementById("filterSearch").value.toLowerCase():"",e=document.getElementById("filterTour")?document.getElementById("filterTour").value:"",a=document.getElementById("filterDate")?document.getElementById("filterDate").value:"",l=document.getElementById("filterSale")?document.getElementById("filterSale").value:"";let r=_.filter(d=>{let b=!1;const y=parseInt(d.total_price)>0&&parseInt(d.total_price)===parseInt(d.deposit),f=x=>{if(!x)return!1;try{const $=x.split("/");if($.length===3){const t=new Date(`${$[2]}-${$[1]}-${$[0]}T00:00:00`),u=new Date;return u.setHours(0,0,0,0),t.getTime()<u.getTime()}else if(x.includes("-")){const t=new Date(`${x}T00:00:00`),u=new Date;return u.setHours(0,0,0,0),t.getTime()<u.getTime()}}catch{}return!1},p=(d.status==="Hoàn thành"||d.status==="Đã đi"||y)&&f(d.date);return C==="consult"?b=d.status==="Chờ tư vấn"&&!p:C==="pending"?b=(!d.status||d.status==="Chờ cọc")&&!p:C==="upcoming"?b=d.status&&d.status!=="Chờ tư vấn"&&d.status!=="Chờ cọc"&&!y&&d.status!=="Hoàn thành"&&!p:C==="ready"?b=(y||d.status==="Hoàn thành")&&!p:C==="completed"&&(b=p),!(!b||e&&d.tour!==e||a&&F(d.date)!==a||l&&l!=="null"&&d.sale_name!==l||l==="null"&&d.sale_name&&d.sale_name!=="Website"||n&&!`${d.name||""} ${d.phone||""} ${d.customer_id||""} ${d.id||""} csr${d.id||""} #csr${d.id||""}`.toLowerCase().includes(n))});if(r.length===0){alert("Danh sách rỗng. Không có dữ liệu để xuất.");return}let c=`
            ...
            `;const o=new Blob(["\uFEFF",c],{type:"application/vnd.ms-excel;charset=utf-8"}),s=URL.createObjectURL(o),i=document.createElement("a");i.setAttribute("href",s);let m=C==="pending"?"ChoCoc":C==="upcoming"?"SapThamGia":C==="ready"?"ChoLenXe":"LichSu";i.setAttribute("download",`BaoCao_DonHang_${m}_${new Date().toISOString().slice(0,10)}.xls`),document.body.appendChild(i),i.click(),document.body.removeChild(i)});const ne=document.querySelectorAll(".tab-btn");ne.forEach(n=>{n.addEventListener("click",e=>{ne.forEach(a=>{a.classList.remove("border-csr-orange","text-csr-orange"),a.classList.add("border-transparent","text-gray-500")}),e.target.classList.add("border-csr-orange","text-csr-orange"),e.target.classList.remove("border-transparent","text-gray-500"),C=e.target.getAttribute("data-tab"),A()})});const N=document.getElementById("bookingModal"),K=document.getElementById("bookingModalContent"),U=document.getElementById("detailModal"),ae=document.getElementById("detailModalContent"),P=document.getElementById("editModal"),J=document.getElementById("editModalContent"),oe=document.getElementById("addBookingBtn");N&&(N.classList.add("duration-200","ease-out"),K.classList.add("duration-300","ease-out")),P&&(P.classList.add("duration-200","ease-out"),J.classList.add("duration-300","ease-out")),oe&&oe.addEventListener("click",()=>{document.getElementById("bookingModalContent").querySelector("h2").innerText="Thêm Khách Hàng (Tạo Đơn)",document.getElementById("submitBookingBtn").innerText="Tạo Phơi Đăng Ký",document.getElementById("editingBookingId").value="",document.getElementById("bookingForm").reset();const n=document.getElementById("loyalty-alert");n&&n.remove(),N.classList.remove("hidden"),setTimeout(()=>{N.classList.add("opacity-100"),K.classList.remove("scale-95","translate-y-4"),K.classList.add("scale-100","translate-y-0")},10)}),window.closeModal=()=>{N&&(N.classList.remove("opacity-100"),K.classList.remove("scale-100","translate-y-0"),K.classList.add("scale-95","translate-y-4"),setTimeout(()=>{N.classList.add("hidden")},200))},window.closeDetailModal=()=>{U&&(U.classList.remove("opacity-100"),ae.classList.remove("scale-100","translate-y-0"),ae.classList.add("scale-95","translate-y-4"),setTimeout(()=>{U.classList.add("hidden")},200))},window.closeEditModal=()=>{P&&(P.classList.remove("opacity-100"),J.classList.remove("scale-100","translate-y-0"),J.classList.add("scale-95","translate-y-4"),setTimeout(()=>{P.classList.add("hidden")},200))},window.openRowActionModal=n=>{document.getElementById("ramName").textContent=n.name||"Khách Hàng Này",document.getElementById("ramPhone").textContent=`#CSR${n.id} - ${n.phone||""}`;const e=`
        ...
        `;document.getElementById("ramActions").innerHTML=e,document.getElementById("rowActionModal").classList.remove("hidden")},window.addEventListener("click",n=>{const e=document.getElementById("rowActionModal");e&&n.target===e&&e.classList.add("hidden")}),window.actionView=n=>{document.getElementById("rowActionModal").classList.add("hidden");const e=_.find(t=>t.id==n);if(!e)return;const a=t=>parseInt(t||0).toLocaleString("vi-VN")+"đ",l=t=>{if(!t)return"—";if(/^\d{4}-\d{2}-\d{2}/.test(t)){const[u,g,v]=t.split("-");return`${v}/${g}/${u}`}return t},r=parseInt(e.total_price)||0,c=parseInt(e.deposit)||0,o=parseInt(e.discount)||0,s=r-c;let i=[];try{if(e.services_booked){const t=typeof e.services_booked=="string"?JSON.parse(e.services_booked):e.services_booked;Array.isArray(t)&&(i=t)}}catch{}const m=i.reduce((t,u)=>t+(parseInt(u.price)||0),0),b={"Chờ tư vấn":"Chờ lịch"}[e.status]||e.status,f={"Chờ tư vấn":"bg-purple-100 text-purple-700 border-purple-200","Chờ cọc":"bg-yellow-100 text-yellow-700 border-yellow-200","Chờ xác nhận cọc":"bg-orange-100 text-orange-700 border-orange-200","Đã cọc":"bg-blue-100 text-blue-700 border-blue-200","Đã cọc (Chờ đi)":"bg-blue-100 text-blue-700 border-blue-200","Hoàn thành":"bg-green-100 text-green-700 border-green-200"}[e.status]||"bg-gray-100 text-gray-700 border-gray-200",p=(t,u,g="")=>`<div class="flex items-start gap-3 py-2.5 border-b border-gray-100 last:border-none">
            ...
            </div>`,x=(t,u,g="bg-blue-50 text-blue-600")=>`<div class="flex items-center gap-2.5 mb-3">
        ...
        <div class="space-y-4 pb-2">
            ...
            <div class="bg-gradient-to-br from-orange-50 to-amber-50 border border-orange-100 rounded-2xl p-4">
                ${x("🗺️","Thông tin Tour","bg-orange-100 text-orange-600")}
                <div class="grid grid-cols-2 gap-x-4">
                    ...
                    ${p("Trạng Thái",`<span class="inline-block px-2.5 py-1 rounded-full text-xs font-bold border ${f}">${b||"Chưa rõ"}</span>`)}
            ...
            <div class="bg-white border border-gray-200 rounded-2xl p-4">
                ${x("👤","Thông tin Cá nhân","bg-blue-50 text-blue-600")}
            ...
            <!-- ③ Thông tin Hậu cần -->
            <div class="bg-white border border-gray-200 rounded-2xl p-4">
                ${x("🎒","Thông tin Hậu cần","bg-purple-50 text-purple-600")}
            ...
            <!-- ④ Dịch vụ đi kèm -->
            <div class="bg-white border border-gray-200 rounded-2xl p-4">
                ${x("✨","Dịch Vụ Đi Kèm","bg-green-50 text-green-600")}
                ${i.length>0?i.map(t=>`
                        <div class="flex items-center justify-between py-2.5 border-b border-gray-100 last:border-none">
                            ...
                            <span class="text-sm font-bold text-green-600">+${a(t.price)}</span>
            ...
            <div class="bg-gradient-to-br from-csr-orange to-orange-600 rounded-2xl p-4 text-white shadow-lg shadow-orange-500/20">
                ${x("💰","Giá Chi Tiết","bg-white/20 text-white")}
                <div class="space-y-2 text-sm">
                    <div class="flex justify-between items-center py-1.5 border-b border-white/10">
                        ...
                        <span class="font-bold">${a(r+o)}</span>
                    </div>
                    ${o>0?`
                    <div class="flex justify-between items-center py-1.5 border-b border-white/10">
                        <span class="text-orange-50 font-medium">Giảm Giá / Coupon</span>
                        <span class="font-bold text-yellow-200">- ${a(o)}</span>
                    </div>`:""}
                    ${m>0?`
                    <div class="flex justify-between items-center py-1.5 border-b border-white/10">
                        <span class="text-orange-50 font-medium">Dịch Vụ Bổ Sung</span>
                        <span class="font-bold text-green-100">+ ${a(m)}</span>
                    </div>`:""}
                    <div class="flex justify-between items-center py-2.5 border-b border-white/20">
                        ...
                        <span class="font-black text-white text-xl drop-shadow-sm">${a(r)}</span>
                    </div>
                    <div class="flex justify-between items-center py-1.5 border-b border-white/10">
                        <div>
                        ...
                        <span class="font-bold text-green-100">${c>0?a(c):"0đ (Chưa cọc)"}</span>
                    </div>
            ...
            ${(()=>{const t=e.phone;if(!t)return"";const u=_.filter(k=>k.phone===t&&!k.status?.includes("hủy")&&!k.status?.includes("Hủy")).sort((k,w)=>new Date(w.created_at||0)-new Date(k.created_at||0)),g=u.length,v=u.reduce((k,w)=>k+(parseInt(w.total_price)||0),0);let h,T,I;g>=5?(h="KHÁCH VIP",T="bg-purple-100 text-purple-700 border-purple-300",I="👑"):g>=2?(h="KHÁCH THÂN THIẾT",T="bg-green-100 text-green-700 border-green-300",I="🌟"):(h="KHÁCH MỚI",T="bg-gray-100 text-gray-500 border-gray-300",I="👋");const L=u.filter(k=>k.id!=n),E=L.length>0?L.slice(0,5).map((k,w)=>{const H=k.status?.includes("Hoàn")?"bg-green-100 text-green-600":k.status?.includes("cọc")||k.status?.includes("Cọc")?"bg-blue-100 text-blue-600":"bg-gray-100 text-gray-500";return`<div class="flex items-center gap-3 py-2 border-b border-gray-100 last:border-none">
                            <div class="w-7 h-7 rounded-full bg-orange-100 flex items-center justify-center text-csr-orange text-xs font-bold shrink-0">${w+1}</div>
                    ...
                    ${x("🏅","Hành Trình Cùng Cam Site","bg-slate-100 text-slate-600")}

                    <!-- Tier + LTV -->
                    <div class="flex items-center justify-between mb-4 gap-3">
                        <div class="flex items-center gap-2">
                            <span class="text-lg">${I}</span>
                            <span class="text-xs font-black uppercase tracking-widest px-3 py-1.5 rounded-full border ${T}">${h}</span>
                        </div>
                        <div class="text-right">
                            <div class="text-[10px] text-gray-400 uppercase font-semibold tracking-wide">Tổng đã chi (LTV)</div>
                            <div class="text-lg font-black text-gray-900">${v>0?v.toLocaleString("vi-VN")+"đ":"—"}</div>
                        </div>
                    </div>
        ...
        </div>`;document.getElementById("detailContentBlock").innerHTML=$,document.getElementById("detailModal").classList.remove("hidden"),setTimeout(()=>{document.getElementById("detailModal").classList.add("opacity-100");const t=document.getElementById("detailModalContent");t&&(t.classList.remove("scale-95","translate-y-4"),t.classList.add("scale-100","translate-y-0"))},10)},window.actionEdit=async n=>{document.getElementById("rowActionModal").classList.add("hidden");const e=_.find(i=>i.id==n);if(!e)return;document.getElementById("edit-id").value=n,document.getElementById("edit-name").value=e.name||"",document.getElementById("edit-phone").value=e.phone||"",document.getElementById("edit-medal-name").value=e.medal_name||"",document.getElementById("edit-tour").value=e.tour||"",typeof j=="function"&&j("edit-date",e.tour||"",e.date||"",e.schedule_id||null),(i=>{let m=i.dob||"";if(typeof m=="string"&&m.includes("/")){const d=m.split("/");d.length===3&&(m=`${d[2]}-${d[1].padStart(2,"0")}-${d[0].padStart(2,"0")}`)}document.getElementById("edit-dob").value=m,document.getElementById("edit-gender").value=i.gender||"Khác",document.getElementById("edit-allergy").value=i.allergy||i.medical_notes||"",document.getElementById("edit-address").value=i.address||"",document.getElementById("edit-diet").value=i.diet||i.dietary||"Không",document.getElementById("edit-trekking-pole").value=i.trekking_pole||"Không"})(e),document.getElementById("edit-status").value=e.status||"Chờ xác nhận cọc",document.getElementById("edit-commitments").checked=!!e.commitments,document.getElementById("edit-special").value=e.special||"";try{const i=localStorage.getItem("csr_user");if(i&&JSON.parse(i).role==="admin"){const d=document.getElementById("edit-sale-container");d&&d.classList.remove("hidden");const b=document.getElementById("edit-sale");b&&(b.value=e.sale_id||"")}}catch{}if((!e.dob||!e.address||!e.gender||e.gender==="Khác")&&(e.customer_id||e.phone))try{const i=e.customer_id||e.phone,m=await fetch(`/api/admin_customers?action=search&keyword=${encodeURIComponent(i)}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({keyword:i})}),d=await m.json();if(m.ok&&d.success){const b=d.data,y=document.getElementById("edit-dob"),f=document.getElementById("edit-address"),p=document.getElementById("edit-allergy"),x=document.getElementById("edit-diet"),$=document.getElementById("edit-gender"),t=document.getElementById("edit-medal-name");y&&!y.value&&(y.value=b.dob||""),f&&!f.value&&(f.value=b.address||""),p&&!p.value&&(p.value=b.medical_notes||""),x&&(x.value==="Không"||!x.value)&&(x.value=b.dietary||"Không"),$&&($.value==="Khác"||!$.value)&&($.value=b.gender||"Khác"),t&&!t.value&&(t.value=b.medal_name||"")}}catch(i){console.error("Pull CRM error:",i)}let r=(parseInt(e.total_price)||0)+(parseInt(e.discount)||0);if(r===0&&e.tour){const i=D.find(m=>m.name===e.tour);i&&parseInt(i.price)>0&&(r=parseInt(i.price))}document.getElementById("edit-total").value=r,document.getElementById("edit-discount").value=e.discount||0,document.getElementById("edit-deposit").value=e.deposit||0;const c=parseInt(e.deposit_required);document.getElementById("edit-deposit-required").value=isNaN(c)?1e6:c,window.updateEditRemaining();const o=document.getElementById("editModal"),s=document.getElementById("editModalContent");o&&s&&(o.classList.remove("hidden"),setTimeout(()=>{o.classList.add("opacity-100"),s.classList.remove("scale-95","translate-y-4"),s.classList.add("scale-100","translate-y-0")},10))},window.actionDelete=async n=>{if(document.getElementById("rowActionModal").classList.add("hidden"),confirm("Bạn có chắc chắn muốn xóa Đơn hàng này? Thao tác không thể hoàn tác."))try{if((await fetch(`/api/bookings?id=${n}`,{method:"DELETE"})).ok)alert("✅ Đã xóa đơn hàng thành công!"),M();else throw new Error("Lỗi từ Server")}catch(e){alert("❌ Không thể xóa bảng ghi này: "+e.message)}},window.updateEditRemaining=()=>{const n=parseInt(document.getElementById("edit-total").value)||0,e=parseInt(document.getElementById("edit-discount").value)||0,a=parseInt(document.getElementById("edit-deposit").value)||0,r=n-e-a,c=document.getElementById("edit-remaining");c&&(c.textContent=r>0?r.toLocaleString("vi-VN")+"đ":"0đ")};const se=document.getElementById("edit-total"),de=document.getElementById("edit-deposit");se&&se.addEventListener("input",window.updateEditRemaining),de&&de.addEventListener("input",window.updateEditRemaining),document.querySelectorAll('#bookingModal button[onclick*="hidden"]').forEach(n=>{n.removeAttribute("onclick"),n.addEventListener("click",window.closeModal)});const ie=async n=>{try{const a=await(await fetch("/api/admin_customers?action=create",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({full_name:n.name,phone:n.phone,cccd:n.id_card||"",dob:n.dob||null,gender:n.gender||"Khác",medical_notes:n.allergy||"",dietary:n.diet||""})})).json();a.success&&a.csr_code&&(console.log("✅ Đã đồng bộ CRM thành công:",n.name,a.csr_code),(await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:n.id,customer_id:a.csr_code})})).ok&&(console.log("✅ Đã cập nhật Booking mã CRM"),typeof M=="function"&&M()))}catch(e){console.error("❌ Lỗi đồng bộ CRM:",e)}},le=document.getElementById("bookingsTableBody");le&&le.addEventListener("click",async n=>{const e=n.target.closest(".action-btn");if(!e){const c=n.target.closest(".row-clickable");if(c){const o=c.getAttribute("data-id"),s=_.find(i=>i.id==o);s&&openRowActionModal(s)}return}const a=e.getAttribute("data-id"),l=_.find(c=>c.id==a);if(!l)return;const r=document.getElementById("rowActionModal");if(r&&r.classList.add("hidden"),e.classList.contains("confirm-deposit-btn")){if(confirm("Xác nhận khách hàng này đã chuyển khoản đặt cọc?"))try{if(e.disabled=!0,e.classList.add("opacity-50"),(await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:a,status:"Đã cọc"})})).ok)alert("✅ Đã xác nhận cọc thành công!"),ie(l),typeof M=="function"&&M();else throw new Error("Lỗi cập nhật API")}catch(c){alert("❌ Lỗi: "+c.message)}finally{e.disabled=!1,e.classList.remove("opacity-50")}return}if(e.classList.contains("delete-btn")){window.actionDelete(a);return}else if(e.classList.contains("process-btn")){const s=(window.location.hostname==="localhost"||window.location.hostname==="127.0.0.1"?"http://localhost:8888":window.location.origin)+`/booking/process.html?id=${a}`;navigator.clipboard.writeText(s).then(()=>{alert("📋 Đã sao chép Link Điền Thông Tin!\\nGửi cho khách: "+s)}).catch(i=>{alert("Lỗi khi Copy Clipboard. Link là: "+s)})}else if(e.classList.contains("pay-terms-btn")){const s=(window.location.hostname==="localhost"||window.location.hostname==="127.0.0.1"?"http://localhost:8888":window.location.origin)+`/pay-terms.html?id=${a}`;navigator.clipboard.writeText(s).then(()=>{alert("📋 Đã sao chép Link Cọc Tour!\\nGửi cho khách: "+s)}).catch(i=>{alert("Lỗi khi Copy Clipboard. Link là: "+s)})}else if(e.classList.contains("send-btn")){const s=(window.location.hostname==="localhost"||window.location.hostname==="127.0.0.1"?"http://localhost:8888":window.location.origin)+`/pay?id=${a}`;navigator.clipboard.writeText(s).then(()=>{alert("📋 Đã sao chép Link Thanh Toán Cọc! Gửi cho khách qua Zalo nhé:\\n"+s)}).catch(i=>{alert("Lỗi khi Copy Clipboard. Link là: "+s)})}else if(e.classList.contains("payment-btn")){const i=`Dạ sắp đến ngày khởi hành rồi, cảm ơn anh chị đã tin tưởng và lựa chọn CAM SITE RETREATS trong hành trình lần này, em xin phép nhận nốt thanh toán phần tiền còn lại để chuẩn bị thật tốt cho chuyến đi nhennn!!!
Dạ mình thanh toán ở link này giúp em nhé: ${(window.location.hostname==="localhost"||window.location.hostname==="127.0.0.1"?"http://localhost:8888":window.location.origin)+`/pay?id=${a}`}`;navigator.clipboard.writeText(i).then(()=>{const m=e.textContent;e.textContent="✅ Đã copy!",e.classList.remove("bg-blue-500","hover:bg-blue-600"),e.classList.add("bg-green-500"),setTimeout(()=>{e.textContent=m,e.classList.add("bg-blue-500","hover:bg-blue-600"),e.classList.remove("bg-green-500")},2e3)}).catch(()=>{alert(`🔗 Nội dung đã chuẩn bị (copy thủ công):

`+i)})}else e.classList.contains("view-btn")?(document.getElementById("rowActionModal").classList.add("hidden"),window.actionView(a)):e.classList.contains("edit-btn")&&window.actionEdit(a)});const R=document.getElementById("smartSearch"),S=document.getElementById("searchSuggestions");if(R){let n;R.addEventListener("input",e=>{const a=e.target.value.toLowerCase().trim();if(clearTimeout(n),a.length<2){S&&S.classList.add("hidden");return}n=setTimeout(async()=>{try{const l=await fetch(`/api/admin_customers?action=search&keyword=${a}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({keyword:a})}),r=await l.json();if(l.ok&&r.success){const o=[r.data];S&&(S.innerHTML=o.map(s=>`
                        ...
                        `).join(""),S.classList.remove("hidden"))}else S&&(S.innerHTML='<div class="p-3 text-xs text-gray-400 italic text-center">Không tìm thấy khách hàng cũ</div>',S.classList.remove("hidden"))}catch(l){console.error("Search error:",l)}},300)}),S&&S.addEventListener("click",e=>{const a=e.target.closest(".suggestion-item");if(a){const l=document.getElementById("addFullName"),r=document.getElementById("addPhone"),c=document.getElementById("addCsrCode");l&&(l.value=a.getAttribute("data-name")),r&&(r.value=a.getAttribute("data-phone")),c&&(c.value=a.getAttribute("data-csr"));const o=document.getElementById("addDob"),s=document.getElementById("addGender"),i=document.getElementById("addIdCard"),m=document.getElementById("addAddress");o&&(o.value=a.getAttribute("data-dob")||""),s&&(s.value=a.getAttribute("data-gender")||"Khác"),i&&(i.value=a.getAttribute("data-cccd")||""),m&&(m.value=a.getAttribute("data-address")||"");const d=document.getElementById("addAllergy"),b=document.getElementById("addDiet"),y=document.getElementById("addTrekkingPole"),f=document.getElementById("addMedalName");d&&(d.value=a.getAttribute("data-allergy")||""),b&&(b.value=a.getAttribute("data-diet")||""),y&&(y.value=a.getAttribute("data-trekking_pole")||"Không"),f&&(f.value=a.getAttribute("data-medal-name")||""),window._selectedCustomer={fullName:a.getAttribute("data-name"),phone:a.getAttribute("data-phone"),csrCode:a.getAttribute("data-csr"),dob:a.getAttribute("data-dob")||"",gender:a.getAttribute("data-gender")||"",address:a.getAttribute("data-address")||"",id_card:a.getAttribute("data-cccd")||"",allergy:a.getAttribute("data-allergy")||"",diet:a.getAttribute("data-diet")||"",trekking_pole:a.getAttribute("data-trekking_pole")||"",medal_name:a.getAttribute("data-medal-name")||""},R.value=a.getAttribute("data-name"),S.classList.add("hidden");const p=`
                    <div id="loyalty-alert" class="mt-4 p-3 bg-green-500/10 border border-green-500/30 text-green-600 rounded-lg text-sm flex items-center">
                        ...
                        Nhận diện khách thân thiết: <b>${a.getAttribute("data-csr")}</b>. Hệ thống sẽ kế thừa toàn bộ hồ sơ cũ!
                    </div>`,x=document.getElementById("loyalty-alert");x&&x.remove(),R.parentElement.insertAdjacentHTML("afterend",p)}}),document.addEventListener("click",e=>{!R.contains(e.target)&&S&&!S.contains(e.target)&&S.classList.add("hidden")})}const G=document.getElementById("addTourPrice"),O=document.getElementById("addDiscount"),re=document.getElementById("addTotalPriceDisplay");document.getElementById("addTourName");const ce=()=>{if(!G||!O||!re)return;const n=parseInt(G.value)||0,e=parseInt(O.value)||0,a=n-e;re.textContent=(a>0?a:0).toLocaleString("vi-VN")+"đ"};G&&G.addEventListener("input",ce),O&&O.addEventListener("input",ce);const ue=document.getElementById("searchBtn");ue&&ue.addEventListener("click",n=>{n.preventDefault(),document.getElementById("smartSearch").value||alert("Vui lòng nhập Số Điện Thoại hoặc Mã #CSR")});const z=document.getElementById("bookingForm");z&&z.addEventListener("submit",async n=>{n.preventDefault();const e=n.target.querySelector('button[type="submit"]'),a=e.textContent;e.textContent="Đang lưu hệ thống...",e.disabled=!0;try{const l=document.getElementById("addFullName").value,r=document.getElementById("addPhone").value,c=document.getElementById("addTourName").value,o=document.getElementById("addTourDate").value;let s=null,i=o;if(o&&o.includes("::")){const B=o.split("::");s=parseInt(B[0])||null,i=B[1]}const m=document.getElementById("addDob").value,d=document.getElementById("addGender").value,b=document.getElementById("addIdCard").value,y=document.getElementById("addAddress").value,f=document.getElementById("addSpecial").value,p=document.getElementById("addAllergy").value,x=document.getElementById("addDiet").value,$=document.getElementById("addTrekkingPole").value,t=document.getElementById("addMedalName").value,u=parseInt(document.getElementById("addTourPrice").value)||0,g=parseInt(document.getElementById("addDiscount").value)||0,v=parseInt(document.getElementById("addDepositRequired").value),h=isNaN(v)?1e6:v,T=document.getElementById("addCsrCode").value||"",I=localStorage.getItem("csr_user");let L=null,E="Admin";if(I){const B=JSON.parse(I);L=B.id,E=B.fullName||B.full_name||"Admin"}const k=document.getElementById("editingBookingId")?document.getElementById("editingBookingId").value:"",w={name:l,phone:r,tour:c,date:i,status:"Chờ cọc",total_price:u-g,discount:g,deposit:0,sale_id:L,sale_name:E,customer_id:T,deposit_required:h,commitments:!0,schedule_id:s,dob:m,gender:d,address:y,id_card:b,special:f,allergy:p,diet:x,trekking_pole:$,medal_name:t},H=r.replace(/[^0-9]/g,"");window._selectedCustomer&&(window._selectedCustomer.phone||"").replace(/[^0-9]/g,"")===H&&(w.dob=w.dob||window._selectedCustomer.dob,w.gender=w.gender&&w.gender!=="Khác"?w.gender:window._selectedCustomer.gender||"Khác",w.address=w.address||window._selectedCustomer.address,w.id_card=w.id_card||window._selectedCustomer.id_card,w.allergy=window._selectedCustomer.allergy,w.diet=window._selectedCustomer.diet,w.trekking_pole=window._selectedCustomer.trekking_pole,w.medal_name=window._selectedCustomer.medal_name),k&&(w.id=k,delete w.status,delete w.deposit,delete w.sale_id,delete w.sale_name);const V=await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(w)});if(V.ok){alert(k?"✅ Đã lưu Cập nhật Đơn hàng!":"✅ Thêm Đơn Hàng Thành Công!"),window.closeModal(),z&&z.reset(),window._selectedCustomer=null;const B=document.getElementById("loyalty-alert");B&&B.remove(),document.getElementById("addTotalPriceDisplay")&&(document.getElementById("addTotalPriceDisplay").textContent="0đ"),typeof M=="function"?M():window.location.reload()}else{const B=await V.json();throw new Error(B.error||"Lỗi tạo Booking!")}}catch(l){alert("❌ "+l.message)}finally{e.textContent=a,e.disabled=!1}});const pe=document.getElementById("editForm");pe&&pe.addEventListener("submit",async n=>{n.preventDefault();const e=n.target.querySelector('button[type="submit"]'),a=e.textContent;e.textContent="Đang lưu cập nhật...",e.disabled=!0;try{const l=document.getElementById("edit-id").value,r=document.getElementById("edit-name").value,c=document.getElementById("edit-phone").value,o=document.getElementById("edit-medal-name").value,s=document.getElementById("edit-tour").value,i=document.getElementById("edit-date").value;let m=i,d;if(i&&i.includes("::")){const B=i.split("::");d=parseInt(B[0])||null,m=B[1]}const b=document.getElementById("edit-dob").value,y=document.getElementById("edit-gender").value,f=document.getElementById("edit-status").value,p=document.getElementById("edit-allergy").value,x=document.getElementById("edit-address").value,$=document.getElementById("edit-diet").value,t=document.getElementById("edit-trekking-pole").value,u=document.getElementById("edit-commitments").checked,g=document.getElementById("edit-special").value,v=document.getElementById("edit-sale");let h,T;if(v&&v.parentElement&&!v.parentElement.classList.contains("hidden")&&(h=v.value||null,T="Website",h)){const B=q.find(fe=>String(fe.id)===String(h));B&&(T=B.full_name||B.fullName)}const I=parseInt(document.getElementById("edit-total").value)||0,L=parseInt(document.getElementById("edit-discount").value)||0,E=parseInt(document.getElementById("edit-deposit").value)||0,k=parseInt(document.getElementById("edit-deposit-required").value),w=isNaN(k)?1e6:k,H={id:l,name:r,phone:c,medal_name:o,tour:s,date:m,dob:b,gender:y,status:f,allergy:p,address:x,diet:$,trekking_pole:t,commitments:u,special:g,sale_id:h,sale_name:T,total_price:I-L,discount:L,deposit:E,deposit_required:w,...d!==void 0?{schedule_id:d}:{}},V=await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(H)});if(V.ok)alert("✅ Đã cập nhật thành công Chi tiết Đơn hàng!"),(f.includes("Đã cọc")||H.total_price>0&&H.total_price===E)&&ie(H),window.closeEditModal(),typeof M=="function"&&M();else{const B=await V.json();throw new Error(B.error||"Gặp sự cố khi Cập nhật API.")}}catch(l){alert("❌ Lỗi Cập nhật: "+l.message)}finally{e.textContent=a,e.disabled=!1}}),window.openWaitingModal=()=>{document.getElementById("waitingModal").classList.remove("hidden"),setTimeout(()=>{document.getElementById("waitingModal").classList.add("opacity-100");const n=document.getElementById("waitingModalContent");n&&(n.classList.remove("scale-95"),n.classList.add("scale-100"))},10)},window.closeWaitingModal=()=>{document.getElementById("waitingModal").classList.remove("opacity-100");const n=document.getElementById("waitingModalContent");n&&(n.classList.remove("scale-100"),n.classList.add("scale-95")),setTimeout(()=>{document.getElementById("waitingModal").classList.add("hidden")},300)};const ge=document.getElementById("addWaitingBtn");ge&&ge.addEventListener("click",()=>window.openWaitingModal());const X=document.getElementById("waitingForm");X&&X.addEventListener("submit",async n=>{n.preventDefault();const e=document.getElementById("wl-submit");e.innerHTML="Đang lưu...",e.disabled=!0;const a=document.getElementById("wl-zalo").value?`Tên Zalo / Ghi Chú: ${document.getElementById("wl-zalo").value}`:"";let l={id:"null",name:"Website"};try{const o=localStorage.getItem("csr_user");if(o){const s=JSON.parse(o);l={id:s.id,name:s.full_name||s.fullName}}}catch{}const r={name:document.getElementById("wl-name").value,phone:document.getElementById("wl-phone").value,tour:document.getElementById("wl-tour").value,date:"Mở (Chưa có ngày)",status:"Chờ tư vấn",special:a,sale_id:l.id,sale_name:l.name},c=document.getElementById("wl-date");if(c&&c.value){const o=c.value.split("::");o.length===2&&o[0]&&o[0]!=="undefined"&&(r.schedule_id=o[0],r.date=o[1])}try{const s=await(await fetch("/api/bookings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(r)})).json();s.success?(window.closeWaitingModal(),X.reset(),typeof M=="function"?M():(alert("✅ Đã thêm vào danh sách chờ!"),window.location.reload())):alert("Lỗi lưu danh sách: "+s.message)}catch(o){console.error(o),alert("Có lỗi xảy ra, vui lòng thử lại.")}finally{e.innerHTML="Lưu Vào Danh Sách",e.disabled=!1}})};export{Ce as afterRender,ke as render};
