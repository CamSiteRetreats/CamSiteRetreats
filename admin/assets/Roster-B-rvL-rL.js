import{S as A,H as _}from"./Header-CvqOcIss.js";const V=()=>`
    ...
    `,z=async()=>{const p=[{id:"col-stt",label:"STT",always:!0,render:(t,e)=>e+1},{id:"col-name",label:"Họ và Tên",default:!0,render:t=>`<div class="font-bold text-gray-900">${t.name}</div>`},{id:"col-dob",label:"Ngày Sinh",default:!0,render:t=>{if(!t.dob)return"-";const e=t.dob.split("-");return e.length===3?`${e[2]}/${e[1]}/${e[0]}`:t.dob}},{id:"col-phone",label:"SĐT",default:!0,render:t=>`<div class="font-medium text-gray-700">${t.phone||"-"}</div>`},{id:"col-gender",label:"Giới tính",default:!0,render:t=>t.gender||"-"},{id:"col-cccd",label:"CCCD / Passport",default:!0,render:t=>t.id_card||t.cccd||"-"},{id:"col-address",label:"Địa chỉ",default:!0,render:t=>`<div class="max-w-[200px] truncate print:max-w-none print:whitespace-normal" title="${t.address||""}">${t.address||"-"}</div>`},{id:"col-diet",label:"Ăn chay",default:!0,render:t=>t.diet==="Chay"||t.diet==="Có"?'<span class="text-green-600 font-bold">Có</span>':"Không"},{id:"col-pole",label:"Gậy Trekking",default:!0,render:t=>t.trekking_pole==="Có"?'<span class="text-orange-600 font-bold">Có</span>':"Không"},{id:"col-allergy",label:"Dị ứng",default:!1,render:t=>t.medical_notes||t.allergy||"-"},{id:"col-special",label:"Yêu cầu ĐB",default:!1,render:t=>t.special_notes||t.special||"-"},{id:"col-medal",label:"Tên Huy Chương",default:!1,render:t=>t.medal_name||t.name||"-"}],C=new URLSearchParams(window.location.search),i=C.get("tour")||"",f=C.get("date")||"";let d=f;const E=t=>{if(!t)return"";const e=t.split(/[-/]/);return e.length!==3?t:e[0].length===4?`${e[2].padStart(2,"0")}/${e[1].padStart(2,"0")}/${e[0]}`:`${e[0].padStart(2,"0")}/${e[1].padStart(2,"0")}/${e[2]}`};f&&(d=E(f));const T=document.getElementById("roster-title"),$=document.getElementById("roster-subtitle"),S=document.getElementById("print-tour-name"),L=document.getElementById("print-tour-date"),H=document.getElementById("print-total"),v=document.getElementById("filterStatus"),x=document.getElementById("filterGender"),b=document.getElementById("rosterList");let h=[],u=new Set(p.filter(t=>t.default||t.always).map(t=>t.id));i&&(T.textContent=`DS Đoàn: ${i}`,S.textContent=`Tuyến: ${i}`),d&&($.innerHTML=`<span class="font-semibold text-gray-700">Khởi hành:</span> ${d}`,L.textContent=`Ngày: ${d}`);const B=()=>{const t=document.getElementById("column-toggles");t.innerHTML=p.filter(e=>!e.always).map(e=>`
            <label class="flex items-center gap-2 cursor-pointer group bg-gray-50 px-3 py-2 rounded-lg border border-gray-200 hover:border-csr-orange/50 transition-colors">
                <input type="checkbox" value="${e.id}" class="col-toggle-cb text-csr-orange focus:ring-csr-orange rounded" ${u.has(e.id)?"checked":""}>
                <span class="text-sm font-medium text-gray-600 group-hover:text-gray-900 select-none">${e.label}</span>
            </label>
        `).join(""),document.querySelectorAll(".col-toggle-cb").forEach(e=>{e.addEventListener("change",n=>{n.target.checked?u.add(n.target.value):u.delete(n.target.value),j()})})},D=()=>{const t=document.getElementById("table-header-row");t.innerHTML=p.map(e=>`
            <th class="p-4 whitespace-nowrap ${e.id}" data-col="${e.id}">${e.label}</th>
        `).join("")},j=()=>{p.forEach(t=>{const e=u.has(t.id),n=document.querySelector(`th[data-col="${t.id}"]`);n&&(e?n.classList.remove("col-hidden"):n.classList.add("col-hidden")),document.querySelectorAll(`td[data-col="${t.id}"]`).forEach(r=>{e?r.classList.remove("col-hidden"):r.classList.add("col-hidden")})})},M=async()=>{try{h=(await(await fetch("/api/bookings")).json()).filter(n=>{let r=!0,s=!0;return i&&(r=n.tour===i||n.tour&&n.tour.startsWith(i)),d&&(s=E(n.date)===d),r&&s}),h.sort((n,r)=>{const s=(n.status||"").includes("cọc")||n.status==="Hoàn tất"?1:0;return((r.status||"").includes("cọc")||r.status==="Hoàn tất"?1:0)-s}),y()}catch(t){console.error("Lỗi khi tải dữ liệu đoàn:",t),b.innerHTML='<tr><td colspan="12" class="p-8 text-center text-red-500 font-bold">Lỗi khi tải dữ liệu. Vui lòng tải lại trang.</td></tr>'}},y=()=>{const t=v.value.toLowerCase(),e=x.value.toLowerCase(),n=h.filter(r=>{const s=t===""||t==="chờ cọc"&&(r.status==="Chờ cọc"||r.status==="Chờ xác nhận cọc")||t==="đã cọc"&&(r.status==="Đã cọc"||r.status==="Hoàn tất"||r.status==="Hoàn thành")||t==="đã hủy"&&r.status==="Đã hủy",l=e===""||r.gender&&r.gender.toLowerCase()===e;return s&&l});if(document.getElementById("text-visible-count").textContent=n.length,H.textContent=`Tổng khách: ${n.length}`,n.length===0){b.innerHTML='<tr><td colspan="12" class="p-12 text-center text-gray-500 bg-gray-50">Không tìm thấy khách hàng nào khớp với điều kiện lọc.</td></tr>';return}b.innerHTML=n.map((r,s)=>'<tr class="hover:bg-gray-50/50 transition-colors">'+p.map(l=>`
                    <td class="p-4 align-top ${u.has(l.id)?"":"col-hidden"}" data-col="${l.id}">
                        ${l.render(r,s)}
                    </td>
                `).join("")+"</tr>").join("")},I=()=>{const t=p.filter(a=>u.has(a.id));let e="\\uFEFF";e+=t.map(a=>`"${a.label}"`).join(",")+`
`;const n=v.value.toLowerCase(),r=x.value.toLowerCase();h.filter(a=>{const m=n===""||n==="chờ cọc"&&(a.status==="Chờ cọc"||a.status==="Chờ xác nhận cọc")||n==="đã cọc"&&(a.status==="Đã cọc"||a.status==="Hoàn tất"||a.status==="Hoàn thành")||n==="đã hủy"&&a.status==="Đã hủy",w=r===""||a.gender&&a.gender.toLowerCase()===r;return m&&w}).forEach((a,m)=>{const w=t.map(c=>{let o="";if(c.id==="col-stt")o=m+1;else if(c.id==="col-name")o=a.name||"";else if(c.id==="col-address")o=a.address||"";else if(c.id==="col-phone")o=a.phone?`'${a.phone}`:"";else if(c.id==="col-diet")o=a.diet==="Chay"||a.diet==="Có"?"Có":"Không";else if(c.id==="col-pole")o=a.trekking_pole==="Có"?"Có":"Không";else{const P=String(c.render(a,m)),k=document.createElement("DIV");k.innerHTML=P,o=k.textContent||k.innerText||""}return o=String(o).replace(/"/g,'""'),`"${o}"`});e+=w.join(",")+`
`});const l=new Blob([e],{type:"text/csv;charset=utf-8;"}),N=URL.createObjectURL(l),F=`DS_Doan_${i||"Tour"}_${d||"Date"}.csv`.replace(/[/\\?%*:|"<>]/g,"-"),g=document.createElement("a");g.setAttribute("href",N),g.setAttribute("download",F),document.body.appendChild(g),g.click(),document.body.removeChild(g)};v.addEventListener("change",y),x.addEventListener("change",y),document.getElementById("btnExportPDF").addEventListener("click",()=>{window.print()}),document.getElementById("btnExportExcel").addEventListener("click",()=>{I()}),B(),D(),M()};export{z as afterRender,V as render};
