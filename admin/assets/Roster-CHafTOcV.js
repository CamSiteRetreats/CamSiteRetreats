import{S as je,H as De}from"./Header-CtRUqTZE.js";const He=()=>`
    ...
    `,Ae=async()=>{const ce=t=>{const e=(t||"").toLowerCase();return e.includes("hoàn tất")||e.includes("hoàn thành")||e.includes("đã thanh toán")||e.includes("thanh toán")?`<span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold bg-emerald-100 text-emerald-700 border border-emerald-200">
        ...
        `).join(""),document.querySelectorAll(".col-toggle-cb").forEach(e=>{e.addEventListener("change",s=>{s.target.checked?C.add(s.target.value):C.delete(s.target.value),we()})})},ye=()=>{const t=document.getElementById("table-header-row");t.innerHTML=E.map(e=>`
            <th class="p-4 whitespace-nowrap ${e.id}" data-col="${e.id}">${e.label}</th>
        `).join("")},we=()=>{E.forEach(t=>{const e=C.has(t.id),s=document.querySelector(`th[data-col="${t.id}"]`);s&&(e?s.classList.remove("col-hidden"):s.classList.add("col-hidden")),document.querySelectorAll(`td[data-col="${t.id}"]`).forEach(r=>{e?r.classList.remove("col-hidden"):r.classList.add("col-hidden")})})},P=t=>{if(!t)return null;t.includes(" - ")&&(t=t.split(" - ")[0].trim()),t.includes("T")&&(t=t.split("T")[0]);const e=t.split(/[-/.]/);return e.length===2?{day:parseInt(e[0]),month:parseInt(e[1]),year:null}:e.length===3?e[0].length===4?{day:parseInt(e[2]),month:parseInt(e[1]),year:parseInt(e[0])}:{day:parseInt(e[0]),month:parseInt(e[1]),year:parseInt(e[2])}:null},B=P(S),ke=t=>{if(!B)return!0;const e=P(t);return!(!e||e.day!==B.day||e.month!==B.month||e.year&&B.year&&e.year!==B.year)},V=async()=>{try{if(_)try{const n=await(await fetch(`/api/guides?schedule_id=${_}`)).json();if(n&&n.length>0){const a=document.getElementById("rosterGuidesSection"),o=document.getElementById("rosterGuidesList"),c={"Hướng Dẫn Viên":"bg-blue-100 text-blue-700 border-blue-200",Leader:"bg-purple-100 text-purple-700 border-purple-200",Photo:"bg-pink-100 text-pink-700 border-pink-200",Guider:"bg-cyan-100 text-cyan-700 border-cyan-200","Hậu Cần":"bg-amber-100 text-amber-700 border-amber-200","Học Việc":"bg-gray-100 text-gray-600 border-gray-200"};a.classList.remove("hidden"),o.innerHTML=n.map(i=>`
