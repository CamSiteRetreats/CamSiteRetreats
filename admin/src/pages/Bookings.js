...
export const afterRender = () => {
    ...
    const loadBookings = async () => {
        try {
            ...
            // Sắp xếp: Luôn ưu tiên những người MỚI NHẤT hiện lên TRÊN ĐẦU (Sắp xếp theo ID giảm dần)
            currentBookings.sort((a, b) => parseInt(b.id) - parseInt(a.id));
    ...
    window.actionView = (bookingId) => {
        ...
        <div class="space-y-4 pb-2">
            ...
            ${(() => {
                // Lấy toàn bộ đơn hàng của cùng số điện thoại
                const phone = booking.phone;
                if (!phone) return '';

                const allPhoneBookings = currentBookings
                    .filter(b => b.phone === phone && !b.status?.includes('hủy') && !b.status?.includes('Hủy'))
                    .sort((a, b) => new Date(b.created_at || 0) - new Date(a.created_at || 0));

                const tourCount = allPhoneBookings.length;
                const ltv = allPhoneBookings.reduce((sum, b) => sum + (parseInt(b.total_price) || 0), 0);

                // Xác định hạng khách hàng
                let tierBadge, tierColor, tierIcon;
                if (tourCount >= 5) {
                    tierBadge = 'KHÁCH VIP'; tierColor = 'bg-purple-100 text-purple-700 border-purple-300'; tierIcon = '👑';
                } else if (tourCount >= 2) {
                    tierBadge = 'KHÁCH THÂN THIẾT'; tierColor = 'bg-green-100 text-green-700 border-green-300'; tierIcon = '🌟';
                } else {
                    tierBadge = 'KHÁCH MỚI'; tierColor = 'bg-gray-100 text-gray-500 border-gray-300'; tierIcon = '👋';
                }

                // Lịch sử các đơn hàng (ngoại trừ đơn hiện tại)
                const pastBookings = allPhoneBookings.filter(b => b.id != bookingId);

                const historyHtml = pastBookings.length > 0
                    ? pastBookings.slice(0, 5).map((b, i) => {
                        const sc = b.status?.includes('Hoàn') ? 'bg-green-100 text-green-600'
                            : b.status?.includes('cọc') || b.status?.includes('Cọc') ? 'bg-blue-100 text-blue-600'
                            : 'bg-gray-100 text-gray-500';
                        return `<div class="flex items-center gap-3 py-2 border-b border-gray-100 last:border-none">
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="text-sm font-medium text-gray-800 truncate">${b.tour || '—'}</div>
                            ...
                            <span class="${sc} text-[9px] px-2 py-0.5 rounded-full font-bold shrink-0">${b.status || 'Mới'}</span>
                        </div>`;
                    }).join('')
                    : `<div class="text-center py-3 text-gray-400 text-xs italic">Đây là lần đầu tiên khách đặt tour.</div>`;

                return `<div class="bg-gradient-to-br from-slate-50 to-gray-50 border border-gray-200 rounded-2xl p-4">
                    ${sectionHeader('🏅', 'Hành Trình Cùng Cam Site', 'bg-slate-100 text-slate-600')}

                    <!-- Tier + LTV -->
                    <div class="flex items-center justify-between mb-4 gap-3">
                        <div class="flex items-center gap-2">
                            <span class="text-lg">${tierIcon}</span>
                            <span class="text-xs font-black uppercase tracking-widest px-3 py-1.5 rounded-full border ${tierColor}">${tierBadge}</span>
                        </div>
                        <div class="text-right">
                            <div class="text-[10px] text-gray-400 uppercase font-semibold tracking-wide">Tổng đã chi (LTV)</div>
                            <div class="text-lg font-black text-gray-900">${ltv > 0 ? ltv.toLocaleString('vi-VN') + 'đ' : '—'}</div>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center gap-2 mb-3 text-sm text-gray-500">
                        ...
                        <span class="font-black text-gray-800 text-base">${tourCount}</span>
                    ...
                    <div class="mb-3">
                        <div class="text-[10px] font-bold text-gray-400 uppercase tracking-wide mb-2">Các tour trước đây (${pastBookings.length})</div>
                        <div class="bg-white rounded-xl border border-gray-100 px-3">${historyHtml}</div>
                    </div>` : historyHtml}
