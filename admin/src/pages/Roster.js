...
export const afterRender = async () => {
    ...
    // ═══════════════════════════════════════════
    // SEAT MAP — Sơ đồ xe Ford Transit 16 chỗ
    // ═══════════════════════════════════════════
    // Layout: driver + 15 passenger seats
    // Rows: [driver,1] [2,3,_,4,5] [6,7,_,8,9] [10,11,_,12,13] [_,14,15,_]
    const SEAT_ROWS = [
    ...
    const renderSeatMap = () => {
        ...
        if (countEl) countEl.textContent = `${assignedCount}/15`;
        if (badgeEl) badgeEl.textContent = `💺 ${assignedCount}/15 ghế đã xếp`;

        grid.innerHTML = SEAT_ROWS.map((row, ri) => {
            const cells = row.map(seat => {
                if (!seat) return `<div class="w-12 h-10 flex-shrink-0"></div>`;
                if (seat.driver) return `<div class="seat-btn seat-driver w-12 h-10 rounded-lg flex flex-col items-center justify-center text-[9px] font-bold flex-shrink-0"><span>🚗</span><span>LÁI</span></div>`;
                ...
                return `<button
                    class="seat-btn ${b ? 'seat-assigned' : 'seat-empty'} w-12 h-10 rounded-lg flex flex-col items-center justify-center flex-shrink-0 group relative"
                    data-seat="${sn}"
                    data-booking-id="${b ? b.id : ''}"
                    title="${b ? b.name + ' — Ghế ' + sn : 'Ghế ' + sn + ' (trống)'}">
                    <span class="text-[9px] font-bold leading-none">${sn}</span>
                    ${b ? `<span class="text-[9px] leading-none mt-0.5 truncate max-w-[44px] px-0.5">${nameAbbr}</span>` : ''}
                </button>`;
            });
            return `<div class="flex items-center justify-center gap-1.5">${cells.join('')}</div>`;
        }).join('');
